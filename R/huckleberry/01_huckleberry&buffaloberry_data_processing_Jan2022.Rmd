...
#Data for abundance
Huck_Laura_Berries<-st_read ( dsn = "D:\\Hucklberry\\Laura_OPENINGS_All2.shp", stringsAsFactors = T)
...
Now transform to ESPG 9001 to use soil layers from Clayton.

```{r}
Huck_Laura_Plant_9001<-st_transform(Huck_Laura_Plant_3005, 9001)
crs(Huck_Laura_Plant_9001)
```
...
crs(SoilStack) #ESPG 9001

crs(Huck_Laura_Plant_9001) #9001

#Get Coordinates
Huck_Laura_Plant_9001$coords_x1<-st_coordinates(Huck_Laura_Plant_9001)
head(Huck_Laura_Plant_9001)

pointCoordinates_LauraPlant2<-data.frame(Huck_Laura_Plant_9001$coords_x1[,1], Huck_Laura_Plant_9001$coords_x1[,2])
head(pointCoordinates_LauraPlant2)
extent(Huck_Laura_Plant_9001) #Transform did not go to what soil layers are in
extent(SoilStack) #Transform did not go to what soil layers are in
extent(Huck_Laura_Plant_3005) #for comparison; see that ymin and max within bounds of soil layer, but x min and max are not. Still need more investigation to fix.

#Extract values from SoilStack_2 for GPS coordinates in data file
SOIL_huck1_LauraPlant=raster::extract(SoilStack, pointCoordinates_LauraPlant2)
head(SOIL_huck1_LauraPlant) #ALL NAs

#Combine back into original data set
Huck_Laura_Plant_9001<-cbind(Huck_Laura_Plant_9001, SOIL_huck1_LauraPlant)
head(Huck_Laura_Plant_9001)
...
```{r}
#Data for percent cover
Huck_Laura_Plant_burn<-st_read ( dsn = "D:\\Hucklberry\\Laura_OPENINGS_PerCov_FirePoints.shp", stringsAsFactors = T)
...
Huck_Laura_Plant$ID1<-paste(Huck_Laura_Plant$Site_ID, Huck_Laura_Plant$YEAR)

cutblock_plots_openings_Huck_Laura_Plant<-left_join(Huck_Laura_Plant, huck_Laura_plantcover_climate, by="ID1")
head(cutblock_plots_openings_Huck_Laura_Plant)
...
2021
#2021 is complicated in that ClimateBC does not yet have annual data for 2021, so the best we can do is either the decadal average, or repeating 2020
```{r}
...
Huck_Laura_climate_data$ID1<-paste(Huck_Laura_climate_data$Site_ID, Huck_Laura_climate_data$Year)

#Below may need updating once file brought in to proper names
Huck_Laura_Berries$ID1<-paste(Huck_Laura_Berries$Site_ID, Huck_Laura_Berries$YEAR)
  
cutblock_plots_openings_Laura_Huck<-left_join(Huck_Laura_Berries, Huck_Laura_climate_data, by="ID1")
head(cutblock_plots_openings_Laura_Huck)


cutblock_plots_openings_Laura_Huck<- cutblock_plots_openings_Laura_Huck %>% distinct() #Back to expected 986 data rows
