...
```{r}
model.new.1c <- gam (Fruit.AbunTEST ~ DN1_SILSYS_GP + PREP_ME + Burned + Manual2 + Chem2 + G_BGC_ZONE + s(OPEN_GRSAR) + Mounding + s(TimeSinceCutblock, by=PREP_ME) + s(TimeSinceCutblock, by=Chem2) + s(TimeSinceCutblock, by=Mounding) + START_SEASON + s(heatload) + s(Species.He),
...
summary(model.new.1c) #Deviance explained 99.8%
...
plot(model.new.1c, select=5) #Flat line at 0?? Check what is wrong.
plot(model.new.1c, select=6)
plot(model.new.1c, select=7)


library(visreg)
visreg(model.new.1c, "DN1_SILSYS_GP")
...
visreg(model.new.1c, "START_SEASON")

visreg(model.new.1c, "TimeSinceCutblock", by= "Chem2")
