...
```{r}
cutblock_plots_openings_productivity_huck<- st_read ( dsn = "C:\\cora\\Hucklberry\\Grizzly Bear stuff\\berries\\data\\Clayton_huckleberry_cutblocks_RESULTS.shp", stringsAsFactors = T) #986 observations
str(cutblock_plots_openings_productivity_huck) #381 observations
...
table(cutblock_plots_openings_productivity_huck$Site_ID) 
```
...
write.csv(cutblock_plots_openings_productivity_huck_df, file="C:\\cora\\Hucklberry\\Grizzly Bear stuff\\berries\\data\\Clayton_cutblocks_DEM_Huckleberry.csv")

```

Take file and modify in excel to get unique ID values to append for Climate data, and to update the current climate variables to have a "_0" at the end to represent that these climate variables pertain to the year of collection. While here, I also created columns for the years and months in which PREP1 and PREP2 occured to better estimate when mechanical disturbance occurred for number of years since logging began, and for seasonality.
...
cutblock_plots_openings_productivity_huck_df$Long2<-round(cutblock_plots_openings_productivity_huck_df$coords_x1DD[,1], 4)

cutblock_plots_openings_productivity_huck_df$Long2
table(cutblock_plots_openings_productivity_huck_df$Long2)
...
table(HuckBuff_Clayton_Normal$ID3) #

cutblock_plots_openings_productivity_huck_df$ID3<-paste(cutblock_plots_openings_productivity_huck_df$Site.ID, cutblock_plots_openings_productivity_huck_df$Species, cutblock_plots_openings_productivity_huck_df$Long2)
#cutblock_plots_openings_productivity_huck_df$ID3 
table(cutblock_plots_openings_productivity_huck_df$ID3)


cutblock_plots_openings_productivity_huck_df2<-left_join(cutblock_plots_openings_productivity_huck_df, HuckBuff_Clayton_Normal, by="ID3")

head(cutblock_plots_openings_productivity_huck_df2) #399 - gained 18 extra. Something went wrong.
table(cutblock_plots_openings_productivity_huck_df2$ID3)
```

If have duplicates, run below.
```{r}
duplicated(cutblock_plots_openings_productivity_huck_df2)

cutblock_plots_openings_productivity_huck_df2b<- cutblock_plots_openings_productivity_huck_df2 %>% distinct()

head(cutblock_plots_openings_productivity_huck_df2b) #381 data points, good
cutblock_plots_openings_productivity_huck_df2<-cutblock_plots_openings_productivity_huck_df2b
...
```{r}
cutblock_plots_openings_productivity_huck_df2$Tave_at #I am not sure what these units are

cutblock_plots_openings_productivity_huck_df2$Tave_at_dec #Some NAs. Probably rom dropping the wrong one in the repeat. Fix.
...
cutblock_plots_openings_productivity_huck_HuckBuff_ALL_df3<-left_join(cutblock_plots_openings_productivity_huck_df2, HuckBuff_climate_data, by="ID3")
head(cutblock_plots_openings_productivity_huck_HuckBuff_ALL_df3)
```


```{r}
duplicated(cutblock_plots_openings_productivity_huck_HuckBuff_ALL_df3)
cutblock_plots_openings_productivity_huck_HuckBuff_ALL_df3b<- cutblock_plots_openings_productivity_huck_HuckBuff_ALL_df3 %>% distinct() #Back to expected 986 data rows

head(cutblock_plots_openings_productivity_huck_HuckBuff_ALL_df3b)
##Note that the climate in the year of data collection here does not have a"_0" afterwards, whereas subsequent data files do have "_0". This will need to be updated.
...
```{r}
cutblock_plots_openings_productivity_huck_HuckBuff_ALL_df3b$heatload <- heatload (aspect = cutblock_plots_openings_productivity_huck_HuckBuff_ALL_df3b$aspect_ha_bc, slope = cutblock_plots_openings_productivity_huck_HuckBuff_ALL_df3b$slope_ha_bc, latitude = cutblock_plots_openings_productivity_huck_HuckBuff_ALL_df3b$Latitude.x, equation = 3)
 
cutblock_plots_openings_productivity_huck_HuckBuff_ALL_df3b$heatload 
hist(cutblock_plots_openings_productivity_huck_HuckBuff_ALL_df3b$heatload)

```

Some other investigations.
```{r}
cutblock_plots_openings_productivity_huck_HuckBuff_ALL_df3b$Fruit.Abun #Mostly NAs, even for locations where it indicated that fruit was present.
table(cutblock_plots_openings_productivity_huck_HuckBuff_ALL_df3b$CutBlock_O) #829 were in cutblocks
...
```{r}
write.csv(cutblock_plots_openings_productivity_huck_HuckBuff_ALL_df3b, file="C:\\cora\\Hucklberry\\Grizzly Bear stuff\\berries\\data\\cutblock_plots_openings_productivity_huck_HuckBuff_ALL_df3b.csv")
```


If need to load back in:
```{r}
cutblock_plots_openings_productivity_huck_HuckBuff_ALL_df3b<-read.csv(file="C:\\cora\\Hucklberry\\Grizzly Bear stuff\\berries\\data\\cutblock_plots_openings_productivity_huck_HuckBuff_ALL_df3b.csv")
head(cutblock_plots_openings_productivity_huck_HuckBuff_ALL_df3b)
...
```{r}
cutblock_plots_openings_productivity_mowatBuff<- st_read ( dsn = "C:\\cora\\Hucklberry\\Mackenzie_Points_Buff_OPENINGS.shp", stringsAsFactors = T)

head(cutblock_plots_openings_productivity_mowatBuff)
names(cutblock_plots_openings_productivity_mowatBuff) #68 observations

#cutblock_plots_openings_productivity_mowatBuff<-cutblock_plots_openings_productivity_mowatBuff[-(68:78)]
...
str(cutblock_plots_openings_productivity_mowatBuff2)#68 values
...
Append climate data.
#Originally data was used that had climate data for all of the plots from Mackenzie; however, matching has proven difficult, and a new extraction was made for the 65 data points that come from the above data.
```{r}
...
```{r}
cutblock_plots_openings_productivity_mowatBuff$LONG2<-round(cutblock_plots_openings_productivity_mowatBuff$LONGITUDE, digits=4)
cutblock_plots_openings_productivity_mowatBuff$ID2<-paste(cutblock_plots_openings_productivity_mowatBuff$Study_Area, cutblock_plots_openings_productivity_mowatBuff$LONG2)
table(cutblock_plots_openings_productivity_mowatBuff$ID2)

Mowat_Buff_Climate$Longitude2<-round(Mowat_Buff_Climate$Longitude, digits=4)
Mowat_Buff_Climate$Longitude2

Mowat_Buff_Climate$ID2<-paste(Mowat_Buff_Climate$Study_Area, Mowat_Buff_Climate$Longitude2)
...
cutblock_plots_openings_productivity_mowatBuff2<-left_join(cutblock_plots_openings_productivity_mowatBuff_df, Mowat_Buff_Climate, by="ID2") #Coming in with climate data being ALL NAs...
...
```{r}
cutblock_plots_openings_productivity_mowatBuff_FIRE<- st_read ( dsn = "C:\\cora\\Hucklberry\\Mackenzie_Points_Buff_OPENINGS_Fire.shp", stringsAsFactors = T)
head(cutblock_plots_openings_productivity_mowatBuff_FIRE) #29
str(cutblock_plots_openings_productivity_mowatBuff_FIRE)


#make as dataframes
cutblock_plots_openings_productivity_mowatBuff_df<-st_drop_geometry(cutblock_plots_openings_productivity_mowatBuff2)
str(cutblock_plots_openings_productivity_mowatBuff_df) #68 observations

cutblock_plots_openings_productivity_mowatBuff_FIRE_df<-st_drop_geometry(cutblock_plots_openings_productivity_mowatBuff_FIRE)
str(cutblock_plots_openings_productivity_mowatBuff_FIRE_df) #29 observations
head(cutblock_plots_openings_productivity_mowatBuff_FIRE_df) #I think that some had more than one fire; so find out which ones duplicated, and then keep more recent fire date
cutblock_plots_openings_productivity_mowatBuff_FIRE_df$LONGITUDE2<-round(cutblock_plots_openings_productivity_mowatBuff_FIRE_df$LONGITUDE, digits=4)
cutblock_plots_openings_productivity_mowatBuff_FIRE_df$LONGITUDE2

cutblock_plots_openings_productivity_mowatBuff_FIRE_df$ID2<-paste(cutblock_plots_openings_productivity_mowatBuff_FIRE_df$Study_Area, cutblock_plots_openings_productivity_mowatBuff_FIRE_df$LONGITUDE2)

cutblock_plots_openings_productivity_mowatBuff_FIRE_df$ID1<-paste(cutblock_plots_openings_productivity_mowatBuff_FIRE_df$Drainage, cutblock_plots_openings_productivity_mowatBuff_FIRE_df$LONGITUDE)
table(cutblock_plots_openings_productivity_mowatBuff_FIRE_df$ID1)
#Duplicates: 
...
table(cutblock_plots_openings_productivity_mowatBuff_FIRE_df$ID2, cutblock_plots_openings_productivity_mowatBuff_FIRE_df$FIRE_YEAR)
...
head(cutblock_plots_openings_productivity_mowatBuff_FIRE_df)
cutblock_plots_openings_productivity_mowatBuff_FIRE_df[21,]
cutblock_plots_openings_productivity_mowatBuff_FIRE_df[22,]

#Flathead_River -114.492559
cutblock_plots_openings_productivity_mowatBuff_FIRE_df<-cutblock_plots_openings_productivity_mowatBuff_FIRE_df[-c(21),]

# Commerce -114.485356
cutblock_plots_openings_productivity_mowatBuff_FIRE_df[10,]
cutblock_plots_openings_productivity_mowatBuff_FIRE_df[9,]

cutblock_plots_openings_productivity_mowatBuff_FIRE_df<-cutblock_plots_openings_productivity_mowatBuff_FIRE_df[-c(10),]

#Border_FSR - all 4 are in a row, with the first one being most recent fire
cutblock_plots_openings_productivity_mowatBuff_FIRE_df<-cutblock_plots_openings_productivity_mowatBuff_FIRE_df[-c(8),]
cutblock_plots_openings_productivity_mowatBuff_FIRE_df<-cutblock_plots_openings_productivity_mowatBuff_FIRE_df[-c(7),]
cutblock_plots_openings_productivity_mowatBuff_FIRE_df<-cutblock_plots_openings_productivity_mowatBuff_FIRE_df[-c(6),]

#Tyax_Road -122.749799 -- both 2009 - appear identical, remove one
cutblock_plots_openings_productivity_mowatBuff_FIRE_df<-cutblock_plots_openings_productivity_mowatBuff_FIRE_df[-c(4),]

cutblock_plots_openings_productivity_mowatBuff_FIRE_df<-data.frame(cutblock_plots_openings_productivity_mowatBuff_FIRE_df)
head(cutblock_plots_openings_productivity_mowatBuff_FIRE_df)

#Check
table(cutblock_plots_openings_productivity_mowatBuff_FIRE_df$ID2, cutblock_plots_openings_productivity_mowatBuff_FIRE_df$FIRE_YEAR)

table(cutblock_plots_openings_productivity_mowatBuff2$ID2) #
table(cutblock_plots_openings_productivity_mowatBuff_FIRE_df$ID2) #N

#When joining, we should get 68 observations
cutblock_plots_openings_productivity_mowatBuff3<-full_join(cutblock_plots_openings_productivity_mowatBuff2, cutblock_plots_openings_productivity_mowatBuff_FIRE_df, by="ID2") # 68 observations
names(cutblock_plots_openings_productivity_mowatBuff3) #Notice many repeat variables because .x and .y here, likely naming was different from shape files and how saved
...
```{r}
names(cutblock_plots_openings_productivity_mowatBuff3)
cutblock_plots_openings_productivity_mowatBuff3$Date.x
cutblock_plots_openings_productivity_mowatBuff3$Year<-c(2020)
cutblock_plots_openings_productivity_mowatBuff3$Year
cutblock_plots_openings_productivity_mowatBuff3$TimeSinceFire<-(cutblock_plots_openings_productivity_mowatBuff3$Year-cutblock_plots_openings_productivity_mowatBuff3$FIRE_YEAR)
cutblock_plots_openings_productivity_mowatBuff3$TimeSinceFire #Change NAs to a really long time, the max value; in Clayton's data. this is 118

cutblock_plots_openings_productivity_mowatBuff3$TimeSinceFire[is.na(cutblock_plots_openings_productivity_mowatBuff3$TimeSinceFire)]<-118


#Create cutblock year
library(stringi)
cutblock_plots_openings_productivity_mowatBuff3$Cutblock_Year<-stri_sub(cutblock_plots_openings_productivity_mowatBuff3$DST_STR_DT.x,1,4)

cutblock_plots_openings_productivity_mowatBuff3$Cutblock_Year
str(cutblock_plots_openings_productivity_mowatBuff3$Cutblock_Year)
cutblock_plots_openings_productivity_mowatBuff3$Cutblock_Year<-as.numeric(cutblock_plots_openings_productivity_mowatBuff3$Cutblock_Year)

cutblock_plots_openings_productivity_mowatBuff3$TimeSinceCutblock<-(cutblock_plots_openings_productivity_mowatBuff3$Year-cutblock_plots_openings_productivity_mowatBuff3$Cutblock_Year)
cutblock_plots_openings_productivity_mowatBuff3$TimeSinceCutblock 

```

Add heatload.
```{r}
cutblock_plots_openings_productivity_mowatBuff3$heatload <- heatload (aspect = cutblock_plots_openings_productivity_mowatBuff3$aspect_ha_bc_3005, slope = cutblock_plots_openings_productivity_mowatBuff3$slope_ha_bc_3005, latitude = cutblock_plots_openings_productivity_mowatBuff3$LATITUDE.x, equation = 3)
 
cutblock_plots_openings_productivity_mowatBuff3$heatload 
hist(cutblock_plots_openings_productivity_mowatBuff3$heatload)
```

Save new file.
```{r}
write.csv(cutblock_plots_openings_productivity_mowatBuff3, file = "C:\\cora\\Hucklberry\\Grizzly Bear stuff\\berries\\data\\MowatMac_Buff_Openings_DEM_Soil_Climate_March23.csv")
```

If need to load back in data.
#Modified to remove the .y repeats, and the .x naming
```{r}
cutblock_plots_openings_productivity_mowatBuff3<-read.csv(file = "C:\\cora\\Hucklberry\\Grizzly Bear stuff\\berries\\data\\MowatMac_Buff_Openings_DEM_Soil_Climate_March23b.csv")
...
cutblock_plots_openings_productivity_BECHuck<- st_read ( dsn = "C:\\cora\\Hucklberry\\Grizzly Bear stuff\\berries\\data\\BEC_huck_Openings_duplicates_removed.shp", stringsAsFactors = T)
head(cutblock_plots_openings_productivity_BECHuck)

cutblock_plots_openings_productivity_activity_BECHuck<- st_read ( dsn = "C:\\cora\\Hucklberry\\Grizzly Bear stuff\\berries\\data\\BEC_huck_Openings_Activity_duplicates_removed.shp", stringsAsFactors = T)
...
cutblock_plots_openings_productivity_Huck_ALL_df<-st_drop_geometry(cutblock_plots_openings_productivity)

cutblock_plots_openings_productivity_mowatBuff_df<-st_drop_geometry(cutblock_plots_openings_productivity_mowatBuff)

cutblock_plots_openings_productivity_BECHuck_df<-st_drop_geometry(cutblock_plots_openings_productivity_BECHuck)
...
```{r}
names(cutblock_plots_openings_productivity_BECHuck_df)
...
st_write(cutblock_plots_openings_productivity_BECHuck_df, dsn= "C:\\cora\\Hucklberry\\Grizzly Bear stuff\\berries\\data\\BEC_cutblock_plots_openings_productivity_BECHuck_df.csv")
...
#Rename ID1 and ID2
head(cutblock_plots_openings_productivity_BECHuck_df)
names(cutblock_plots_openings_productivity_BECHuck_df)
...
cutblock_plots_openings_productivity_BECHuck_df_<-merge(cutblock_plots_openings_productivity_BECHuck_df, BEC_ClimateBC_Data, by=c("ProjectID", "PlotNumber"))
head(cutblock_plots_openings_productivity_BECHuck_df_)
```
...
cutblock_plots_openings_productivity_mowatBuff3<-read.csv(file = "C:\\cora\\Hucklberry\\Grizzly Bear stuff\\berries\\data\\MowatMac_Buff_Openings_DEM_Soil_Climate_March23b.csv")
head(cutblock_plots_openings_productivity_mowatBuff3)
str(cutblock_plots_openings_productivity_mowatBuff3) #68 observations of 474 variables
```


```{r}
## 2. cutblock_plots_openings_productivity_Buff_ALL_df2, cutblock_plots_openings_productivity_mowatBuff3
names(cutblock_plots_openings_productivity_Buff_ALL_df2)
names(cutblock_plots_openings_productivity_mowatBuff3)
head(cutblock_plots_openings_productivity_mowatBuff3)
#cutblock_plots_openings_productivity_mowatBuff3$Species<-c("Shepherdia_canadensis")

cutblock_plots_openings_productivity_mowatBuff3$Clear_Cut
cutblock_plots_openings_productivity_mowatBuff3$TimeSinceCutblock
cutblock_plots_openings_productivity_Buff_ALL_df2$TimeSinceC
cutblock_plots_openings_productivity_mowatBuff3$Canopy_Cov
cutblock_plots_openings_productivity_mowatBuff3$Canopy_C_1
cutblock_plots_openings_productivity_Buff_ALL_df2$LandsatCC_

cutblock_plots_openings_productivity_mowatBuff3$Species_Co #Buff Species Percent cover by category
cutblock_plots_openings_productivity_mowatBuff3$PERCENT__2
cutblock_plots_openings_productivity_Buff_ALL_df2$Species.Co

cutblock_plots_openings_productivity_mowatBuff3$Buffalober #Whether present or not. Some 0s.
...
cutblock_plots_openings_productivity_mowatBuff3$Buffalob_1 #Buff_Fruit_Stage
cutblock_plots_openings_productivity_mowatBuff3$Buffalob_2 # Unsure - all 0s
cutblock_plots_openings_productivity_mowatBuff3$Buffalob_3
cutblock_plots_openings_productivity_mowatBuff3$Buffalob_4 # 
cutblock_plots_openings_productivity_mowatBuff3$Buffalob_5 
cutblock_plots_openings_productivity_mowatBuff3$Fruit_Coun #Very large numbers
cutblock_plots_openings_productivity_mowatBuff3$Fruit_Cove
cutblock_plots_openings_productivity_mowatBuff3$Fruit_Pres #0 and No same
...
cutblock_plots_openings_productivity_mowatBuff3$count #--> count_of_Buff_Estimate_per_m2
cutblock_plots_openings_productivity_mowatBuff3$count_of_1 #--> count_of_Buffaloberrries_per_plot
cutblock_plots_openings_productivity_mowatBuff3$Buff_Fru_3 # --> Buff_Fruiting_Plant_Height_cm
cutblock_plots_openings_productivity_mowatBuff3$Latitude
cutblock_plots_openings_productivity_Buff_ALL_df2$Latitude

#mowat
cutblock_plots_openings_productivity_mowatBuff3b<-rename(cutblock_plots_openings_productivity_mowatBuff3,
...
```{r}
# fill in non-overlapping columns with NAs
cutblock_plots_openings_productivity_Buff_ALL_df2b[setdiff(names(cutblock_plots_openings_productivity_mowatBuff3b), names(cutblock_plots_openings_productivity_Buff_ALL_df2b))] <- NA
cutblock_plots_openings_productivity_mowatBuff3b[setdiff(names(cutblock_plots_openings_productivity_Buff_ALL_df2b), names(cutblock_plots_openings_productivity_mowatBuff3b))] <- NA

#Combine
str(cutblock_plots_openings_productivity_Buff_ALL_df2b) #191 observations, 610 variables
str(cutblock_plots_openings_productivity_mowatBuff3b) #68 observations, 610 variables

cutblock_plots_openings_productivity_Buff_ALL_df2b$DATASET<-c(1)
cutblock_plots_openings_productivity_mowatBuff3b$DATASET<-c(2)

cutblock_plots_openings_productivity_Buff_ALL_df<-smartbind(cutblock_plots_openings_productivity_Buff_ALL_df2b, cutblock_plots_openings_productivity_mowatBuff3b)
cutblock_plots_openings_productivity_Buff_ALL_df<-rbind(cutblock_plots_openings_productivity_Buff_ALL_df2b, cutblock_plots_openings_productivity_mowatBuff3b)
