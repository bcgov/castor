...
nharv<-raster("C:/Users/KLOCHHEA/clus/nharv.tif")
uwr.pixels<-data.table(c(t(raster::as.matrix(uwr.ras))))
class(uwr.pixels$V1)

uwr.pixels$V1<-as.integer(uwr.pixels$V1)

sett<-uwr.pixels[uwr.pixels$V1 == 194756,]
test<-nharv
test[]<-uwr.pixels$V1

writeRaster(test, file=paste0(here(), "/uwr.tif"), format="GTiff", overwrite=TRUE)

#upload to db
system("cmd.exe", input = paste0('raster2pgsql -s 3005 -d -I -C -M -N 2147483648  ', here(), '/uwr.tif -t 100x100 rast.zone_cond_uwr | psql -d clus'), show.output.on.console = FALSE, invisible = TRUE)
...
dbExecute(conn, "DELETE FROM public.zone_uwr WHERE zoneid IN
