...
zone_og <- data.table (zoneid = as.integer (c(1:5)), # check these.....
                       ...
                       reference_zone = 'comparison_nass.rast_oldgrw', 
                       percentage = as.numeric (c (44, 44, 37, 17, 17)), 
                       ndt = as.integer (0), 
                       label = c ('luAlluvial_con', 'luAlluvial_decid', 'luMountains',
                                  'luUpland_con', 'luUpland_decid'), 
                       multi_condition = as.character (''), 
                       denom = as.character (''), # productive forest????
                       start = as.integer (0), 
                       stop = as.integer (250))
  
  
# Qinglin: where is rlu_oldgrw?
rast.og <- raster::raster (paste0 (data.dir, "gisData\\grids\\rlu_oldgrw.tif")) # ???
