...
variable= 'age', type ='le', threshold= 12, percentage= 10, ndt =as.integer(0))
...
```{r, denning}
#SBS: WET
denning.sbs.wet<- habitat[(species_cd_1 %in% c( 'ACT', 'AC', 'AT') | species_cd_2 %in% c( 'ACT', 'AC', 'AT') | (species_cd_1 %in% c('SX', 'SE', 'SXW', 'SS') & species_pct_1 == 100)) & crown_closure >= 30 & quad_diam_125 >= 28.5 & basal_area >= 29.7 & proj_age_1 >= 125 & bec_zone_code == 'SBS' & bec_subzone %in% c('wk','mk','mm','mw'), "feature_id"]
...
denning.sbs.dry<- habitat[(species_cd_1 %in% c( 'ACT', 'AC', 'AT') | species_cd_2 %in% c( 'ACT', 'AC', 'AT') | (species_cd_1 %in% c('SX', 'SE', 'SXW', 'SS') & species_pct_1 == 100)) & crown_closure >= 20 & quad_diam_125 >= 28 & basal_area >= 28 & proj_age_1 >= 125 & bec_zone_code == 'SBS' & bec_subzone %in% c('dw','dh','dk'), "feature_id"]
...
denning.dry.for<- habitat[(species_cd_1 %in% c( 'ACT', 'AC', 'AT') | species_cd_2 %in% c( 'ACT', 'AC', 'AT') & proj_age_1 >= 135) | ((species_cd_1 %in% c('FDI', 'FD', 'F') & species_pct_1 == 100) & proj_age_1 >= 207 & crown_closure >= 20 & quad_diam_125 >= 34.3) & bec_zone_code %in% c('SBPS', 'IDF','MS') & bec_subzone %in% c('mc','mk','dk','dc','mw','dw','ww','xc','xk','dv','dm'), "feature_id"]
...
rest.rust.sbs.wet<- habitat[(species_cd_1 %in% c('SX', 'SE', 'SXW', 'SS') | species_cd_2 %in% c('SX', 'SE', 'SXW', 'SS') | species_cd_3 %in% c('SX', 'SE', 'SXW', 'SS') ) & crown_closure >= 30 & quad_diam_125 >= 22.7 & basal_area >= 35 & proj_age_1 >= 135 & bec_zone_code == 'SBS' & bec_subzone %in% c('wk','mk','mm','mw'), "feature_id"]

#SBS DRY: Sx leading, secondary, or tertiary species Crown closure ≥25% QMD_125 ≥19.6 cm Basal area ≥32 m²/ha Stand age ≥72 years
rest.rust.sbs.dry<- habitat[(species_cd_1 %in% c('SX', 'SE', 'SXW', 'SS') | species_cd_2 %in% c('SX', 'SE', 'SXW', 'SS') | species_cd_3 %in% c('SX', 'SE', 'SXW', 'SS') ) & crown_closure >= 25 & quad_diam_125 >= 19.6 & basal_area >= 32 & proj_age_1 >= 72 & bec_zone_code == 'SBS' & bec_subzone %in% c('dw','dh','dk'), "feature_id"]

#DRY FOREST:Sx leading, secondary, or tertiary species Crown closure ≥40% QMD_125 ≥20.1 cm Stand age ≥83 years
rest.rust.dry.for<- habitat[(species_cd_1 %in% c('SX', 'SE', 'SXW', 'SS') | species_cd_2 %in% c('SX', 'SE', 'SXW', 'SS') | species_cd_3 %in% c('SX', 'SE', 'SXW', 'SS') ) & proj_age_1 >= 83 & crown_closure >= 40 & quad_diam_125 >= 20.1 & bec_zone_code %in% c('SBPS', 'IDF','MS') & bec_subzone %in% c('mc','mk','dk','dc','mw','dw','ww','xc','xk','dv','dm'), "feature_id"]
