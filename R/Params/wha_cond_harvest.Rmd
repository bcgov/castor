...
ogc_fids_noharv<-wha_cond[wha_cond$timber_har == 'NO HARVEST ZONE', "ogc_fid"]
...
wha_cond<-wha_cond[!(wha_cond$timber_har == 'NO HARVEST ZONE'), ] #get only the conditional harvesting ones now.
...
Will no adversely effect lichen productivity, result in a network of connected forest cover for visual screening and lichen, result in pre-harvest pine leading stands to be established as pine leading stands. see http://www.env.gov.bc.ca/wld/documents/wha/RATA_9-032_034_041_073,104_106,144,145_ord.pdf
```{r, w9032}
...
dbExecute(conn, "Alter Table zone.wha_2021 set denom = null where denom is not null;")
dbExecute(conn, "Alter Table zone.wha_2021 set multi_condition = null where multi_condition is not null;")
dbExecute(conn, "ALTER TABLE zone.wha_2021 SET start = 0 WHERE start IS not null;")
dbExecute(conn, "ALTER TABLE zone.wha_2021 SET stop = 250 WHERE stop IS not null;")
