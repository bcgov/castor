...
case
  WHEN ntrldstrbn = 'NDT1' then 1
  WHEN ntrldstrbn = 'NDT2' then 2
  WHEN ntrldstrbn = 'NDT3' then 3
  WHEN ntrldstrbn = 'NDT4' then 4
  else 5
...
where zone in ('CWH','ICH','ESSF','MH','CDF','SBS','SWB','BWBS','SBPS','MS','IDF','PP','BG') and ntrldstrbn in ('NDT1','NDT2','NDT3','NDT4');")
...
from vri.veg_comp_lyr_r1_poly2023 
...
#### CCLUP
```{r, cclup}
beo.table[!is.na(cclup_id), key_beo:=link_key]
```

#### Prince george biodiversity order
```{r, pg_bio_order}
beo.table[!is.na(unit), key_beo:= unit]
```
