...
print(head(zone_beo_table))# check
...
dbExecute(conn, "ALTER TABLE zone_uwr INHERIT zone_constraints") #Note from uwr_cond_harvest.rmd
dbDisconnect(conn)
```

# Spatial No Harvesting Constraints 

These are areas that can contribute to other zonal constraints. For instance a spatial OGMA constraint refers to a mapped out spatial location for no harvesting. This area may be within a larger BEO that would include these pixels as part of the age constraint.
...
system("cmd.exe", input = paste0('raster2pgsql -s 3005 -d -I -C -M ', here(), '/nharv.tif -t 100x100 rast.zone_nharv2 | psql -d clus'), show.output.on.console = FALSE, invisible = TRUE)
