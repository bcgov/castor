...
dist500.rate.test<- lme(lambda.dift ~ core.dist500.rate*matrix.dist500.rate,random=~ 1|herd_name, method = 'ML', data = model.data, control = con1)
summary(dist500.rate.test)
```
...
#Test
```{r, test99}
out<-data.table(cbind(model.data[,c("lambda.dift", "herd_name", "year", "pop_estimate", "core.dist500.per","matrix.dist500.per","core.dist500.rate","matrix.dist500.rate")], fitted = predict(dist500.rate.reml, level =1)))
out<-out[herd_name %in% c('Hart_Ranges', 'Narrow_Lake'),]
