...
```{r, core_matrix}
if(FALSE){for.area<-data.table(getTableQuery("create table caribou_sm_forest_area as (select a.herd_name, a.bc_habitat, sum(ST_Area(ST_Intersection(a.geom, b.geom))) FROM (select herd_name, bc_habitat, wkb_geometry as geom from public.bc_caribou_linework_v20200507_shp_core_matrix where herd_name in ('Barkerville', 'Wells_Gray_North','Central_Selkirks','Columbia_North','Columbia_South', 'Monashee', 'Narrow_Lake', 'North_Cariboo', 'Purcell_Central', 'Purcells_South', 'South_Selkirks','Frisby_Boulder')) as a, (select shape as geom from veg_comp_lyr_r1_poly2019 where bclcs_level_2 = 'T') as b WHERE ST_Intersects(a.geom, b.geom) GROUP BY a.herd_name, a.bc_habitat);"))
}
for.area<-data.table(getTableQuery(paste0("SELECT herd_name, bc_habitat as critical_hab, sum/10000 as area_for from caribou_sm_forest_area where herd_name <> 'Hart_Ranges'")))
if(FALSE){
...
for.area<-rbindlist(list(for.area, data.table( herd_name= 'Hart_South', critical_hab = c('Core', 'Matrix'), area_for = c(1865470516/10000, 3921853454/10000))))
areas<-for.area
```
...
caribou_pop<-data.table(read.csv("T:/FOR/VIC/HTS/ANA/PROJECTS/CLUS/Data/caribou/population/TylerMuhly_CaribouPopData_1Mar2021/compiled.csv"))
setnames(caribou_pop, c("block", "pop_est"), c("herd_name", "pop_estimate"))
caribou_pop<-caribou_pop[,year := as.integer(year)][,herd_name:= trimws(herd_name)]

#Export to postgres
conn<-DBI::dbConnect(dbDriver("PostgreSQL"), host=keyring::key_get('dbhost', keyring = 'postgreSQL'), dbname = keyring::key_get('dbname', keyring = 'postgreSQL'), port='5432' ,user=keyring::key_get('dbuser', keyring = 'postgreSQL') ,password= keyring::key_get('dbpass', keyring = 'postgreSQL'))
DBI::dbWriteTable(conn, c("public", "caribou_pop_20210420"), value= caribou_pop, row.names = FALSE, overwrite = TRUE) 
dbDisconnect(conn)
}
#Get the population estimates from the database
caribou_pop<-data.table(getTableQuery("SELECT * FROM caribou_pop_20210420 where method in ('MC' , 'MC-JHE', 'MC-LP') and best = 'Y' and herd_name in ('Barkerville', 'Central Selkirks', 'Columbia North', 'Columbia South',  
 'Frisby Boulder', 'Hart South', 'Monashee',  'Narrow Lake', 'North Cariboo', 'Purcells South','Purcell Central', 'Wells Gray North') or (herd_name = 'Columbia South' and year in (2004, 2002) and method = 'MNKA' )
 order by herd_name, year;"))
#caribou_pop[!is.na(Pop_est_old), pop_estimate:=Pop_est_old]
caribou_pop<-caribou_pop[method %in% c('MC', 'MNKA'), method:= 'MC-LP']

   
pop_mnge<-data.table(xmin =c(2007,2020, 2003, 2003, 2003, 2020, NA, NA, NA, NA, NA, 2001),
                     xmax = c(2020,2020, 2020, 2020, 2020, 2020, NA, NA, NA, NA, NA, 2020),
                     herd_name= c("Barkerville","Central Selkirks", "Columbia North",   "Columbia South",   "Frisby Boulder", "Hart South", "Monashee", "Narrow Lake","North Cariboo","Purcell Central", "Purcells South",  "Wells Gray North" ))
...
caribou_pop$xn<-0
caribou_pop[herd_name=='Barkerville' & year == 1997, xn:=-0.3]
caribou_pop[herd_name=='Central Selkirks' & year == 1997, xn:=0.6]
caribou_pop[herd_name=='Central Selkirks' & year == 1999, xn:=0.3]
caribou_pop[herd_name=='Columbia North' & year == 1997, xn:=0.5]
caribou_pop[herd_name=='Columbia South' & year == 1997, xn:=0.3]
caribou_pop[herd_name=='Frisby Boulder' & year == 2002, xn:=0.3]
caribou_pop[herd_name=='Wells Gray North' & year == 2005, xn:=0.6]
caribou_pop[herd_name=='Wells Gray North' & year == 2006, xn:=0.9]

caribou_pop$yn_l<- 0.2
caribou_pop$yn_u<- -1.1
caribou_pop[herd_name=='Central Selkirks' & year == 1997, yn_l:=-2.0]
caribou_pop[herd_name=='Central Selkirks' & year == 1997, yn_u:=-3.3]
caribou_pop[herd_name=='Central Selkirks' & year > 2010, yn_l:=-2.0]
caribou_pop[herd_name=='Central Selkirks' & year > 2010, yn_u:=-3.3]
caribou_pop[herd_name=='Columbia South' & year == 1996, yn_l:=-2.0]
caribou_pop[herd_name=='Columbia South' & year == 1996, yn_u:=-3.3]
caribou_pop[herd_name=='Columbia North' & year == 1997, yn_l:=-2.0]
caribou_pop[herd_name=='Columbia North' & year == 1997, yn_u:=-3.3]
caribou_pop[herd_name=='Frisby Boulder' & year == 1997, yn_l:=-2.0]
caribou_pop[herd_name=='Frisby Boulder' & year == 1997, yn_u:=-3.3]
caribou_pop[herd_name=='Wells Gray North' & year == 1994, yn_l:=-2.5]
caribou_pop[herd_name=='Wells Gray North' & year == 1994, yn_u:=-3.8]
caribou_pop[herd_name=='Wells Gray North' & year == 1996, yn_l:=-2.5]
caribou_pop[herd_name=='Wells Gray North' & year == 1996, yn_u:=-3.8]
caribou_pop[herd_name=='Wells Gray North' & year == 1999, yn_l:=-6.9]
caribou_pop[herd_name=='Wells Gray North' & year == 1999, yn_u:=-8.2]
caribou_pop[herd_name=='Wells Gray North' & year == 2005, yn_l:=-6.9]
caribou_pop[herd_name=='Wells Gray North' & year == 2005, yn_u:=-8.2]
caribou_pop[herd_name=='Hart South' & year == 2020, yn_l:=-2.0]
caribou_pop[herd_name=='Hart South' & year == 2020, yn_u:=-3.3]
caribou_pop[herd_name=='Barkerville' & year == 2005, yn_l:=-6.9]
caribou_pop[herd_name=='Barkerville' & year == 2005, yn_u:=-8.2]
caribou_pop[herd_name=='Barkerville' & year == 2001, yn_l:=-6.9]
caribou_pop[herd_name=='Barkerville' & year == 2001, yn_u:=-8.2]

fig.pop<-ggplot(data = caribou_pop, aes(x=year, y = as.integer(pop_estimate)))+ 
  facet_wrap(~herd_name, nrow= 4, scales= 'free')+
  scale_y_continuous(limits=c(-1,NA),breaks = integer_breaks(n=4))+
  scale_x_continuous(breaks = integer_breaks())+
  geom_rect(data=pop_mnge, inherit.aes = FALSE, aes(x = NULL,y = NULL,xmin=xmin, xmax=xmax, 
                                  ymin=-Inf, ymax=Inf, 
                                  alpha=0.2))+
  geom_errorbar(aes(ymin=lwr_CI, ymax=upr_CI), width=.2, position=position_dodge(.9))+
  geom_line() +
  geom_point(aes(shape = method), fill="white") + 
  scale_shape_manual(values=c(16,21))+
  geom_text(aes(y = 0, label=total_marked), vjust = caribou_pop$yn_l,   size =3.0,fontface='bold', nudge_x = caribou_pop$xn)+
  geom_text(aes(y = 0, label=marked_observed), vjust = caribou_pop$yn_u, size =2.8, nudge_x = caribou_pop$xn )+
  theme_bw()+
  theme(legend.position = "none")+
  labs(x= "Year", y="Caribou abundance")
fig.pop
#Set the width to 600 and maintain the aspect ratio
```
...
wolf_actions<-data.table(getTableQuery("SELECT * FROM wolf_control order by herd_name , year;"))



wolf_actions$herd_name<-gsub("-", " ", wolf_actions$herd_name)
wolf_actions<-wolf_actions[!(herd_name=='Frisby Boulder' & year ==	2004),] #remove the effect of moose reduction in 2003 to allow for a FB 2004 survey
wolf_actions<-wolf_actions[!(herd_name=='Columbia South' & year ==	2004),] #remove the effect of moose reduction
wolf_actions<-wolf_actions[!(herd_name=='Columbia South' & year ==	2002),] #remove the effect of moose reduction
wolf_actions<-wolf_actions[!(herd_name=='Columbia North' & year ==	2004),] #remove the effect of moose reduction
...
#Block level not WGN - only quesnel lake area
data_new<-data_new[!(herd_name == 'Wells Gray North' & year < 1993),]
...
herd.spat<-getSpatialQuery( "Select herd_name, bc_habitat, wkb_geometry from public.bc_caribou_linework_v20200507_shp_core_matrix where herd_name in ('Barkerville', 'Central_Selkirks', 'Columbia_North', 'Columbia_South',  
 'Frisby_Boulder', 'Monashee',  'Narrow_Lake', 'North_Cariboo', 'Purcells_South','Purcell_Central', 'Wells_Gray_North')")
...
herd.bounds<-getSpatialQuery( "Select herd_name, shape from public.bc_caribou_herd_boundary_v20200507 where herd_name in ('Barkerville', 'Central_Selkirks', 'Columbia_North', 'Columbia_South',  
 'Frisby_Boulder', 'Hart_Ranges', 'Monashee',  'Narrow_Lake', 'North_Cariboo', 'Purcells_South', 'Purcell_Central',  'Wells_Gray_North')")
  ...
  geom_sf(data = herd.bounds.all, aes(),fill = "grey50", show.legend=FALSE ) +
   ...
   geom_sf(data = herd.spat[herd.spat$bc_habitat=='Matrix',], aes(fill = "red"), color = NA, show.legend=FALSE )+
    ...
    geom_text_repel(data = herd.bounds, aes(x=X, y=Y, label = herd_name), 
        fontface = "bold", size = 3,nudge_x =  c(-2.3,-2.25,-2.0, -3.7,4.2,-2.6,-3,-2.2,2.1,-2.9,-3,-3), nudge_y = c(-0.75,0,-0.25,0,0.48,-.32,0,0,-0.28,-0.36,-0.2,0))+
...
ggdraw() +
  draw_plot(main) +
  draw_plot(inset, x = 0.65, y = 0.688, width = 0.29, height = 0.28)
```
...
disturb<-data.table(dbGetQuery(conn, paste0("SELECT * from dm_ms.disturbance where scenario in ('", paste(unique(data_new$herd_name), collapse = "', '"),"');")))
...
#data.set<-data.set[!herd_name == 'Monashee',]
...
graph.dist$shp<-'0'
graph.dist[herd_name %in% c('Barkerville', 'Columbia_South',  'Narrow_Lake', 'Purcells_sOuth'), shp:='1',]
graph.dist[herd_name %in% c('Monashee', 'Frisby_Boulder', 'North_Cariboo', 'Wells_Gray_North'), shp:='2',]

out.graph.dist<- ggplot(data = graph.dist, aes(x = year, y =value, color = herd_name, shape = herd_name )) +
  ...
  theme_bw() +
  scale_shape_manual("Subpopulation", values=c(16,17,3,4,15,6,16,17,3,4,15,6))+
  scale_color_discrete("Subpopulation")
out.graph.dist
```
...
matrix.p2<-ggplot(data = data.set, aes(x =matrix.cut80r50.per, y =pop_estimate, color = herd_name )) +  geom_point() + geom_smooth(method = 'lm', se= FALSE) + theme(legend.position = "none")
ggplotly(matrix.p0)
ggplotly(matrix.p1)
ggplotly(matrix.p2)
```
...
```{r, table1}
table1<-data.set[,.(Core = mean(core.area),Core_For = mean(core.area.for), Matrix = mean(matrix.area),Matrix_For = mean(matrix.area.for), r50min = min(core.road50.per),r50max =max(core.road50.per), c80min = min(core.cut80.per), c80min = max(core.cut80.per), c80r50min =min(core.cut80r50.per) ,c80r50max =max(core.cut80r50.per), mat_r50min = min(matrix.road50.per),mat_r50max = max(matrix.road50.per), mat_c80min = min(matrix.cut80.per), mat_c80max = max(matrix.cut80.per), mat_c80r50min =min(matrix.cut80r50.per) ,mat_c80r50max =max(matrix.cut80r50.per)), by = "herd_name"]
table1
```
...
cor.test(data.set$matrix.cut80.per,data.set$core.cut80.per)
cor.test(data.set$matrix.cut40.per,data.set$core.cut40.per)
cor.test(data.set$matrix.cut10_40.per,data.set$core.cut10_40.per)
cor.test(data.set$matrix.road50.per,data.set$core.road50.per)
cor.test(data.set$matrix.road500.per,data.set$core.road500.per)
cor.test(data.set$matrix.cut80r50.per,data.set$core.cut80r50.per)
cor.test(data.set$matrix.cut80r500.per,data.set$core.cut80r500.per)
cor.test(data.set$matrix.cut40r50.per,data.set$core.cut40r50.per)
cor.test(data.set$matrix.cut40r500.per,data.set$core.cut40r500.per)
cor.test(data.set$matrix.c10_40r500.per,data.set$core.c10_40r500.per)
cor.test(data.set$matrix.c10_40r50.per,data.set$core.c10_40r50.per)


cor(data.set[,"core.road500.per"],data.set[,"core.cut40.per"])
cor(data.set[,"core.cut80.per"],data.set[,"core.road500.per"])
cor(data.set[,"matrix.cut80.per"],data.set[,"matrix.road500.per"])
cor(data.set[,"matrix.cut40.per"],data.set[,"matrix.road500.per"])
...
library(nlme)
library(MuMIn)
model.data<-data.set[ ,c("herd_name","herd.size","core.area", "pop_estimate", "pop.0",  "time", "year","core.road50.per","core.road500.per","core.road250.per","core.road750.per","core.cut40.per", "core.cut20.per", "core.cut80.per","core.cut10_40.per", "core.cut40r50.per", "core.cut40r500.per","core.cut80r500.per","core.cut80r50.per","core.cut20r500.per","core.c10_40r500.per","core.c10_40r50.per", "matrix.road50.per","matrix.road250.per", "matrix.road500.per","matrix.road750.per","matrix.cut40.per","matrix.cut80.per","matrix.cut20.per","matrix.cut10_40.per", "matrix.cut40r50.per", "matrix.cut40r500.per","matrix.cut80r500.per","matrix.cut80r50.per","matrix.cut20r500.per","matrix.c10_40r500.per","matrix.c10_40r50.per","total.cut40r50.per","total.cut40.per", "total.cut80.per","total.road50.per","total.road500.per","total.cut10_40.per","total.cut10_40r50.per","total.cut10_40r500.per","total.cut40r50.per", "total.cut40r500.per","total.cut80r500.per","total.cut80r50.per", "total.area")]
...
# Add the null group
model.data[,group:=1]
#model.data<-model.data[!herd_name %in% c("Purcell_Central"),]
#model.data<-model.data[!(herd_name == "Barkerville" & year > 2002),]
#Reporting function
report<-function(x){
par(mfrow=c(2,2))
...
```{r, nonliner}
#NULL
expo.null.1 <- nlme(pop_estimate~exp(N0), fixed =list(N0~1), random= N0~1|herd_name, weights = varExp(form=~herd.size), start =list(fixed=c(N0=4.5)), correlation = corARMA(p=0,q=2,form=~time), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.null.1) 
report(expo.null.1)
...
#Road Models
```{r}
nlmeControl(msMaxIter = 500, msVerbose = TRUE, tolerance = 0.001)
...
model.data[, s.total.road50.per:=scale(total.road50.per, center=TRUE, scale=TRUE)]
start.total.road50.s<-nls(pop_estimate~exp(a+b*s.total.road50.per), start=list(a=5, b=-0.004), data=model.data, nls.control(maxiter=1000))
expo.total.road50.s <- nlme(pop_estimate~ exp(a+b*s.total.road50.per), fixed =list(a~1, b~1), random= a+b~1|herd_name,weights = varExp(form=~herd.size),start =list(fixed=c(a=6, b=-0.4)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.total.road50.s) 

#Core and Matrix road 50
start.road50<-nls(pop_estimate~exp(a+b*core.road50.per+c*matrix.road50.per), start=list(a=6.5, b=-0.3,c=-0.4), data=model.data, nls.control(maxiter=1000))
expo.road50.1 <- nlme(pop_estimate  ~ exp(a+b*core.road50.per+c*matrix.road50.per), fixed =list(a~1,b~1, c~1), weights = varExp(form=~herd.size), random= a+b~1|herd_name, start =list(fixed=c(a= 6.7, b =-0.99, c=-0.01)), data = model.data, method = 'ML', control = c(maxIter = 100000,msMaxIter=10000, tolerance = 0.0001))

start.road50.2<-nls(pop_estimate~exp(a+b*core.road50.per+c*matrix.road50.per), start=list(a=6.5,b=-0.4,c=-0.4), data=model.data, nls.control(maxiter=1000))
expo.road50.2 <- nlme(pop_estimate  ~ exp(a+b*core.road50.per+c*matrix.road50.per), fixed =list(a~1,b~1, c~1), weights = varExp(form=~herd.size), random= a~1|herd_name, start =list(fixed=coef(start.road50.2)), data = model.data, method = 'ML', control = c(maxIter = 100000,msMaxIter=10000, tolerance = 0.0001))
anova(expo.road50.1,expo.road50.2)
...
anova(expo.null.1, expo.total.road50.1,expo.road50.1)
...
#Core and Matrix road 50
start.road50.s<-nls(pop_estimate~exp(a+b*s.core.road50.per+c*s.matrix.road50.per), start=list(a=6.5, b=-0.3,c=-0.4), data=model.data, nls.control(maxiter=1000))
expo.road50.s <- nlme(pop_estimate  ~ exp(a+b*s.core.road50.per+c*s.matrix.road50.per), fixed =list(a~1,b~1, c~1),  random= a+b~1|herd_name, weights = varExp(form=~herd.size), start =list(fixed=coef(start.road50.s)), data = model.data, method = 'REML', control = c(maxIter = 100000))
...
expo.total.road500.1 <- nlme(pop_estimate  ~ exp(a+b*total.road500.per),fixed =list(a~1,b~1), weights= varExp(form=~herd.size), random= a+b~1|herd_name, start =list(fixed=coef(start.total.road500)), data = model.data, method = 'ML', control = c(maxIter = 100000))
...
expo.road500.1 <- nlme(pop_estimate  ~ exp(a+b*core.road500.per+c*matrix.road500.per), fixed =list(a~1,b~1,c~1), weights= varExp(form=~herd.size), random= a+b~1|herd_name, start =list(fixed=c(a=6, b=-0.02, c=-0.02)), data = model.data, method = 'ML', control = c(maxIter = 100000))
...
expo.cut10_40.1 <- nlme(pop_estimate  ~ exp(a+b*core.cut10_40.per+c*matrix.cut10_40.per), fixed =list(a~1, b~1,c~1),  random= a+b~1|herd_name,weights=varExp(form=~herd.size), start =list(fixed=coef(start.cut10_40)), data = model.data, method = 'ML', control = c(maxIter = 100000))
...
expo.cut40.1 <- nlme(pop_estimate  ~ exp(a+b*core.cut40.per+c*matrix.cut40.per), fixed =list(a~1,b~1,c~1), weights=varExp(form=~herd.size), random=a+b~1|herd_name, start =list(fixed=coef(start.cut40)), data = model.data, method = 'ML', control = c(maxIter = 100000))
...
expo.total.cut80.1 <- nlme(pop_estimate~exp(a+b*total.cut80.per), fixed =list(a~1,b~1),  weights=varExp(form=~herd.size), random= a+b~1|herd_name, start =list(fixed=coef(start.total.cut80)), data = model.data, method = 'ML', control = c(maxIter = 100000))
...
expo.cut80.1 <- nlme(pop_estimate~exp(a+b*core.cut80.per+c*matrix.cut80.per), fixed =list(a~1,b~1,c~1), weights=varExp(form=~herd.size), random= a+b~1|herd_name, start =list(fixed=coef(start.cut80)), data = model.data, method = 'ML', control = c(maxIter = 100000))
...
expo.cut80.s <- nlme(pop_estimate~exp(a+b*s.core.cut80.per+c*s.matrix.cut80.per), fixed =list(a~1,b~1,c~1), weights=varExp(form=~herd.size), random= a+b~1|herd_name, start =list(fixed=coef(start.cut80)), data = model.data, method = 'REML', control = c(maxIter = 100000))
...
expo.cut10_40r50.1 <- nlme(pop_estimate  ~ exp(a+b*core.c10_40r50.per+c*matrix.c10_40r50.per), fixed =list(a~1,b~1,c~1), weights= varExp(form=~herd.size), random= a+b~1|herd_name, start =list(fixed=coef(start.cut10_40r50)), data = model.data, method = 'ML', control = c(maxIter = 100000))
...
expo.total.cut10_40r500.1 <- nlme(pop_estimate~exp(a+b*total.cut10_40r500.per), fixed =list(a~1,b~1), weights= varExp(form=~herd.size), random= a+b~1|herd_name, start =list(fixed=coef(start.total.cut10_40r500)), data = model.data, method = 'ML', control = c(maxIter = 100000))
...
expo.cut10_40r500.1 <- nlme(pop_estimate~exp(a+b*core.c10_40r500.per+c*matrix.c10_40r500.per), fixed =list(a~1,b~1, c~1),  weights= varExp(form=~herd.size), random= a+c~1|herd_name, start =list(fixed=coef(start.cut10_40r500)), data = model.data, method = 'ML', control = c(maxIter = 100000, msMaxIter = 500))
...
expo.cut40r50.1 <- nlme(pop_estimate~exp(a+b*core.cut40r50.per+c*matrix.cut40r50.per), fixed =list(a~1,b~1,c~1), weights=varExp(form=~herd.size), random= a+b~1|herd_name, start =list(fixed=coef(start.cut40r50)), data = model.data, method = 'ML', control = c(maxIter = 100000))
...
expo.total.cut40r500.1 <- nlme(pop_estimate~exp(a+b*total.cut40r500.per), fixed =list(a~1,b~1), weights=varExp(form=~herd.size), random= a+b~1|herd_name, start =list(fixed=coef(start.total.cut40r500)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.total.cut40r500.1) 
report(expo.total.cut40r500.1)
#Core and Matrix cut40r500
start.cut40r500<-nls(pop_estimate~exp(a+b*core.cut40r500.per+c*matrix.cut40r500.per), start=list(a=4, b=-0.001, c=-0.002), data=model.data)
expo.cut40r500.1 <- nlme(pop_estimate~exp(a+b*core.cut40r500.per+c*matrix.cut40r500.per), fixed =list(a~1,b~1,c~1),weights=varExp(form=~herd.size), random= a+b~1|herd_name, start =list(fixed=coef(start.cut40r500)), data = model.data, method = 'ML', control = c(maxIter = 100000))
...
start.cut80r50<-nls(pop_estimate~exp(a+b*core.cut80r50.per+c*matrix.cut80r50.per),start=list(a=6, b=-0.001, c=-0.002), data=model.data)
expo.cut80r50.1 <- nlme(pop_estimate~exp(a+b*core.cut80r50.per+c*matrix.cut80r50.per), fixed =list(a~1,b~1,c~1), weights=varExp(form=~herd.size), random= a+b~1|herd_name, start =list(fixed=coef(start.cut80r50)), data = model.data, method = 'ML', control = c(maxIter = 100000))
...
expo.cut80r50.s <- nlme(pop_estimate~exp(a+b*s.core.cut80r50.per+c*s.matrix.cut80r50.per), fixed =list(a~1,b~1,c~1), weights=varExp(form=~herd.size), random= a+b~1|herd_name, start =list(fixed=coef(start.cut80r50)), data = model.data, method = 'REML', control = c(maxIter = 100000))
...
#Summary
```{r}
all<-AICc(expo.null.1, expo.road50.1, expo.total.road50.1,expo.road500.1, expo.total.road500.1,expo.cut10_40.1, expo.total.cut10_40.1, expo.cut40.1, expo.total.cut40.1,expo.cut80.1, expo.total.cut80.1, expo.total.cut10_40r50.1,expo.cut10_40r50.1,expo.total.cut10_40r500.1,expo.cut10_40r500.1,expo.total.cut40r50.1,expo.cut40r50.1,
     expo.total.cut40r500.1,expo.cut40r500.1,expo.total.cut80r50.1,expo.cut80r50.1,expo.total.cut80r500.1,expo.cut80r500.1)
all$delta<-round(qpcR::akaike.weights(all$AIC)$deltaAIC, 3)
all$weights<-round(qpcR::akaike.weights(all$AIC)$weights, 3)
...
summary(update(expo.road50.s, method = 'REML'))
summary(update(expo.cut80r50.s, method = 'REML'))
summary(update(expo.cut80.s, method = 'REML'))
```
...
fig.data2<-rbind(raw.data1,raw.data2 )
...
fig.data2$herd_name<-gsub("_", " ", fig.data2$herd_name)
fig.data$herd_name<-gsub("_", " ", fig.data$herd_name)

fig.5<-ggplot(data = fig.data2, aes(x = R50, y = pop_estimate, colour = factor(herd_name), shape = factor(herd_name))) +
  facet_wrap(~type, nrow =2, ncol=1)+
  geom_point() +
  geom_line(aes(y = pop.pred ))+
  geom_line(data= fig.data, aes(y = pop.pred))+
  labs(x = 'R50 (%)', y ='Caribou abundance', colour = "Herd")+
  theme_bw() +
  scale_color_manual("Subpopulation", values =c("black","#F8766D", "#DE8C00", "#B79F00", "#7CAE00", "#00BA38", "#00C08B", "#00BFC4", "#00B4F0", "#619CFF", "#C77CFF", "#F564E3", "#FF64B0" )) +
  scale_shape_manual("Subpopulation", values=c(NA, 16,17,3,4,15,6,16,17,3,4,15,6))
...
Seip, Dale and Deborah Cichowski. 1996. Population ecology of caribou in British
Columbia. Rangifer, Special Issue No. 9: 73-80. 

found a stabalizing density of 0.04 caribou per km2
## Thresholds
```{r, thresholds}
...
ggplot(data=data.threshold[year > 1985,c("herd_name", "pred", "year")], aes(x=year, y= pred, group = herd_name, color = herd_name)) + geom_line() 
