...
expo.cr.0 <- nlme(pop_estimate ~ expo.model(a,b,c,d, core.cut80r500.per, matrix.cut80r50.per, herd.size), fixed =list(a~1,b~1, c~1, d~1), random= b~1|herd_name, correlation = corAR1(value = 0.5, form = ∼time|herd_name), start =list(fixed=coef(start.cr.0)), data = model.data, method = 'ML', control = c(maxIter = 10000))
...
newdat.cr<-expand.grid(herd_name = 'Barkerville', core.cut40r500.per=seq(from = min(model.data$core.cut40r500.per),
                            to = max(model.data$core.cut40r500.per),
                            by = 0.1),
                    matrix.cut40r500.per= mean(model.data$matrix.cut40r500.per),
                    herd.size =1)
ggplotly(ggplot(model.data, aes(x=core.cut40r500.per, y=pop_estimate, colour=herd_name)) +
  geom_point(size=3) +
  geom_line(aes(y=predict(expo.r.0), group=herd_name)) +
...
gam.c<-gamlss(pop_estimate ~ core.cut80.per  + re(random=~1|herd_name, method = "ML", correlation = corAR1(value =0.8, form=~time|herd_name)), opt="optim",   sigma.formula = ~1 + herd.size , family = LOGNO(), data = model.data, control =gamlss.control(c.crit=0.01, n.cyc=5000,msMaxIter=1000000), method=CG())
plot(gam.c)
summary(gam.c)
...
gam.r<-gamlss(pop_estimate ~ core.road50.per+matrix.road50.per+re(random=~1|herd_name, method = 'ML',  correlation = corAR1(form = ∼time|herd_name)), opt="optim",   sigma.formula = ~1+herd.size, family = LOGNO(), data = model.data, control =gamlss.control(c.crit=0.01, n.cyc=5000,msMaxIter=1000000), method=CG())
summary(gam.r)
plot(gam.r)
...
gam.cr500<-gamlss(pop_estimate ~core.cut80r50.per +re(random=~1|herd_name, method = "ML", correlation = corAR1(value = 0.8, form = ∼time|herd_name)), opt="optim",   sigma.formula = ~1 +herd.size, family = LOGNO(), data = model.data, control =gamlss.control(c.crit=0.01, n.cyc=5000,msMaxIter=1000000), method=CG())
summary(gam.cr500)
