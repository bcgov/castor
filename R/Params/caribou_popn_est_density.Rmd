...
```{r, dist_graph}
one0<-data.set[, c("herd_name", "year","core.cut80.per")]
setnames(one0, "core.cut80.per" , "value")
one0[, dm:='C']
...
one2<-data.set[, c("herd_name", "year","core.cut80r50.per")]
setnames(one2, "core.cut80r50.per" , "value")
one2[, dm:='CR500']

core.graph.dist<-rbind(one0,one2,one3)
core.graph.dist[, hab:='Core']

t0<-data.set[, c("herd_name", "year","matrix.cut80.per")]
setnames(t0, "matrix.cut80.per" , "value")
t0[, dm:='C']
...
t2<-data.set[, c("herd_name", "year","matrix.cut80r50.per")]
setnames(t2, "matrix.cut80r50.per" , "value")
t2[, dm:='CR500']
...
core.p2<-ggplot(data = data.set, aes(x =core.cut80r50.per, y =pop_estimate, color = herd_name,  linetype = as.factor(herd.size)  )) +  geom_point() + geom_smooth(method = 'lm', se= FALSE) + theme(legend.position = "none")
...
matrix.p2<-ggplot(data = data.set, aes(x =matrix.cut80r50.per, y =pop_estimate, color = herd_name,  linetype = as.factor(herd.size)  )) +  geom_point() + geom_smooth(method = 'lm', se= FALSE) + theme(legend.position = "none")
...
cor(data.set[,"core.cut80r50.per"],data.set[,"matrix.cut80r50.per"])
cor(data.set[,"matrix.road50.per"],data.set[,"core.cut40.per"])
cor(data.set[,"matrix.cut80.per"],data.set[,"core.road50.per"])
...
c.0 <- lme(pop_estimate ~ core.cut80.per+matrix.cut80.per, random=~1|herd_name, correlation = corAR1(value = 0.8, form = ∼time|herd_name), data = model.data, method = 'ML', control = c(maxIter = 10000))
acf(residuals(c.0,type="normalized"), lag.max = 5)
summary(c.0) #AIC:1018, Phi:0.96
plot(model.data$pop_estimate, resid(c.0))
abline(0,0)
plot(predict(c.0),model.data$pop_estimate)
abline(0,1, col = 'red')
#Core or matrix slope AIC: 1022
...
abline(0,0) #AIC:1016, PHI:0.96
plot(predict(r.0),model.data$pop_estimate)
abline(0,1, col = 'red')
#Core or matrix slope AIC: 1020

cr500.0 <- lme(pop_estimate ~ core.cut80r50.per+matrix.cut80r50.per, random=~1|herd_name, correlation = corAR1(value = 0.8, form = ∼time|herd_name), data = model.data, method = 'ML', control = c(maxIter = 10000))
...
expo.c.0 <- nlme(pop_estimate  ~ expo.model(a,b,c,d, core.cut80.per,matrix.cut80.per,  herd.size), fixed =list(a~1,b~1,c~1,d~1), random= a~1|herd_name, correlation = corAR1(value = 0.8, form = ∼time|herd_name), start =list(fixed=coef(start.c.0)), data = model.data, method = 'ML', control = c(maxIter = 100000))
...
expo.r.0 <- nlme(pop_estimate ~ expo.model(a,b,c,d, core.road50.per,matrix.road50.per, herd.size), fixed =list(a~1,b~1,c~1, d~1), random=a~1|herd_name, correlation=corAR1(value = 0.5, form = ∼time|herd_name), start =list(fixed=coef(start.r.0)), data = model.data, method = 'ML', control = c(maxIter = 10000))
...
newdat<-expand.grid(herd_name = 'Barkerville', core.road50.per=seq(from = min(model.data$core.road50.per),
                            to = max(model.data$core.road50.per),
                            by = 0.1),
                    matrix.road50.per= mean(model.data$matrix.road50.per),
                    herd.size =0)
newdat2<-expand.grid(herd_name = 'Barkerville', core.road50.per=seq(from = min(model.data$core.road50.per),
                            to = max(model.data$core.road50.per),
                            by = 0.1),
                    matrix.road50.per= mean(model.data$matrix.road50.per),
                    herd.size =1)
ggplotly(ggplot(model.data, aes(x=core.road50.per, y=pop_estimate, colour=herd_name)) +
  geom_point(size=3) +
...
#CUTBLOCKS + ROADS
start.cr.0 <- nls(pop_estimate ~ expo.model(a,b,c,d, core.cut80r50.per, matrix.cut80r50.per, herd.size), start=list(a=5,b=0.1, c = 0.05, d= 1), data= model.data)
expo.cr.0 <- nlme(pop_estimate ~ expo.model(a,b,c,d, core.cut80r50.per, matrix.cut80r50.per, herd.size), fixed =list(a~1,b~1, c~1, d~1), random= a~1|herd_name, correlation = corAR1(value = 0.5, form = ∼time|herd_name), start =list(fixed=coef(start.cr.0)), data = model.data, method = 'ML', control = c(maxIter = 10000))
...
```{r, gams}
#GAMLSS
gam.null<-gamlss(pop_estimate ~ 1 + re(random=~1|herd_name, method = "ML", correlation = corAR1(form=~time|herd_name)), opt="optim",   sigma.formula = ~1+core.area, family = LOGNO(), data = model.data, control =gamlss.control(c.crit=0.001, n.cyc=5000,msMaxIter=1000000), method=CG())
summary(gam.null)
acf(residuals(getSmo(gam.null),type="normalized"), lag.max = 10)

gam.c<-gamlss(pop_estimate ~ re(fixed=~core.cut80.per + core.area, random=~-1+core.cut80.per|herd_name, method = "ML", correlation = corAR1(value =0.8, form=~time|herd_name)), opt="optim",   sigma.formula = ~1+ core.area , family = LOGNO(), data = model.data, control =gamlss.control(c.crit=0.05, n.cyc=5000,msMaxIter=1000000), method=CG())
plot(gam.c)
summary(gam.c)
...
gam.r<-gamlss(pop_estimate ~ re(fixed=~ core.road50.per+matrix.road50.per, random=~ core.road50.per|herd_name, method = 'ML',  correlation = corAR1(form = ∼time|herd_name)), opt="optim",   sigma.formula = ~1+core.area, family = LOGNO(), data = model.data, control =gamlss.control(c.crit=.01, n.cyc=5000,msMaxIter=1000000), method=CG())
summary(gam.r)
plot(predict(gam.r),residuals(gam.r))
abline(0,0, col = 'red')
...
gam.cr500<-gamlss(pop_estimate ~core.cut80r50.per+matrix.cut80r50.per +re(random=~1|herd_name, method = "ML", correlation = corAR1(value = 0.8, form = ∼time|herd_name)), opt="optim",   sigma.formula = ~1 +core.area, family = LOGNO(), data = model.data, control =gamlss.control(c.crit=0.1, n.cyc=5000,msMaxIter=1000000), method=CG())
summary(gam.cr500)
...
test<-model.data[,c('herd_name', 'pop_estimate', 'core.area', 'herd.size')]
test$mu<-predictAll(gam.r)$mu
test$mu.e<-exp(test$mu)
test$sigma<-predictAll(gam.r)$sigma
test$res<-test$mu-log(test$pop_estimate)
plot( test$mu,test$res)
abline(0,0, col ='red')

one<-data.table(v=rlnorm(10000, 3.91, 0.33), type = 'high')
two<-data.table(v=rlnorm(10000, 3.91, 0.17), type = 'low')
histogram.data<-rbindlist(list(one,two))
ggplot(histogram.data, aes(v, group = type)) + 
       geom_histogram(data=subset(histogram.data,type == 'high'),fill = "red", alpha = 0.2) +
geom_histogram(data=subset(histogram.data,type == 'low'),fill = "blue", alpha = 0.2) 
```
