...
herd.bounds<-getSpatialQuery( "Select herd_name, shape from public.bc_caribou_herd_boundary_v20200507 where herd_name in ('Barkerville', 'Wells_Gray_South','Wells_Gray_North','Central_Selkirks','Columbia_North','Columbia_South', 'Groundhog', 'Narrow_Lake', 'North_Cariboo', 'Purcells_South', 'South_Selkirks', 'Hart_Ranges')")
...
disturb<-data.table(dbGetQuery(conn, paste0("SELECT * from dm_all_1040.disturbance where scenario in ('", paste(unique(data_new$herd_name), collapse = "', '"),"');")))
...
data.core<-data.3[critical_hab== 'Core', c("herd_name", "year", "pop_estimate", "lambda", "year.0","pop.0", "lambda.ratio", "lambda.finite", "per.pop", "total_area", "pop_lag", "log.pop", "year_lag", "lambda.dift","lambda.dif", "pop.change","lambda.dift.n0","lambda.dif.n0", "area_for","burn40", "cut20", "cut40", "cut80", "cut10_40", "road50", "road250", "road500", "road750", "c20r50", "c20r250", "c20r500", "c20r750","c40r50", "c40r250", "c40r500", "c40r750","c80r50", "c80r250", "c80r500", "c80r750","c10_40r50","c10_40r500")]
setnames(data.core, c("total_area", "area_for", "burn40","cut20", "cut40", "cut80", "cut10_40", "road50", "road250", "road500", "road750", "c20r50", "c20r250", "c20r500", "c20r750","c40r50", "c40r250", "c40r500", "c40r750","c80r50", "c80r250", "c80r500", "c80r750","c10_40r50","c10_40r500"), 
         c("core.area", "core.area.for", "core.burn40","core.cut20", "core.cut40", "core.cut80", "core.cut10_40","core.road50", "core.road250", "core.road500", "core.road750", "core.c20r50", "core.c20r250", "core.c20r500", "core.c20r750","core.c40r50", "core.c40r250", "core.c40r500", "core.c40r750","core.c80r50", "core.c80r250", "core.c80r500", "core.c80r750","core.c10_40r50","core.c10_40r500"))

data.matrix<-data.3[critical_hab == 'Matrix',c("herd_name", "year","total_area","area_for","burn40", "cut20", "cut40", "cut80", "cut10_40", "road50", "road250", "road500", "road750", "c20r50", "c20r250", "c20r500", "c20r750","c40r50", "c40r250", "c40r500", "c40r750","c80r50", "c80r250", "c80r500", "c80r750","c10_40r50","c10_40r500" )]
setnames(data.matrix, c("total_area", "area_for", "burn40","cut20", "cut40", "cut80", "cut10_40", "road50", "road250", "road500", "road750", "c20r50", "c20r250", "c20r500", "c20r750","c40r50", "c40r250", "c40r500", "c40r750","c80r50", "c80r250", "c80r500", "c80r750","c10_40r50","c10_40r500"), c("matrix.area","matrix.area.for", "matrix.burn40","matrix.cut20", "matrix.cut40", "matrix.cut80", "matrix.cut10_40", "matrix.road50", "matrix.road250", "matrix.road500", "matrix.road750", "matrix.c20r50", "matrix.c20r250", "matrix.c20r500", "matrix.c20r750","matrix.c40r50", "matrix.c40r250", "matrix.c40r500", "matrix.c40r750","matrix.c80r50", "matrix.c80r250", "matrix.c80r500", "matrix.c80r750","matrix.c10_40r50","matrix.c10_40r500"))
...
#Final
data.all<-data.set[, c("herd_name", "year", "pop_estimate", "core.road50.per","core.cut80.per", "core.area", "matrix.road50.per","matrix.cut80.per")]
...
c10.40<-data.set[, c("herd_name", "year","core.cut10_40.per")]
setnames(c10.40, "core.cut10_40.per" , "value")
c10.40[, dm:='C10-40']


r500<-data.set[, c("herd_name", "year","core.road500.per")]
setnames(r500, "core.road500.per" , "value")
r500[, dm:='R500']

c80r500<-data.set[, c("herd_name", "year","core.cut80r500.per")]
setnames(c80r500, "core.cut80r500.per" , "value")
c80r500[, dm:='C80R500']

core.graph.dist<-rbind(c10.40,r500,c80r500)
core.graph.dist[, hab:='Core']

c10.40.m<-data.set[, c("herd_name", "year","matrix.cut10_40.per")]
setnames(c10.40.m, "matrix.cut10_40.per" , "value")
c10.40.m[, dm:='C10-40']

r500.m<-data.set[, c("herd_name", "year","matrix.road500.per")]
setnames(r500.m, "matrix.road500.per" , "value")
r500.m[, dm:='R500']

c80r500.m<-data.set[, c("herd_name", "year","matrix.cut80r500.per")]
setnames(c80r500.m, "matrix.cut80r500.per" , "value")
c80r500.m[, dm:='C80R500']

matrix.graph.dist<-rbind(c10.40.m, r500.m, c80r500.m)
...
graph.dist$dm<- factor(graph.dist$dm, levels = c( 'C10-40','R500','C80R500'))
...
matrix.p2<-ggplot(data = data.set, aes(x =matrix.cut10_40.per, y =pop_estimate, color = herd_name,  linetype = as.factor(herd.size)  )) +  geom_point() + geom_smooth(method = 'lm', se= FALSE) + theme(legend.position = "none")
...
histDist(pop_estimate,family="PO", data=data.set, nbins = 4)
```
...
summary(data.set[,c("core.road50.per","core.cut10_40.per","core.c10_40r500.per")])

summary(data.set[,c("matrix.road50.per","matrix.cut10_40.per","matrix.c10_40r500.per")])

t.test(data.set$matrix.road50.per,data.set$core.road50.per,paired=TRUE,conf.level=0.95)
t.test(data.set$matrix.cut10_40.per,data.set$core.cut10_40.per,paired=TRUE,conf.level=0.95)
t.test(data.set$matrix.c10_40r500.per,data.set$core.c10_40r500.per,paired=TRUE,conf.level=0.95)
```
...
model.data<-data.set[,c("herd_name","herd.size","core.area", "pop_estimate", "pop.den",  "time", "year","core.road50.per","core.road500.per","core.road250.per","core.road750.per","core.cut40.per", "core.cut20.per", "core.cut80.per","core.cut10_40.per", "core.cut40r50.per", "core.cut40r500.per","core.cut80r500.per","core.cut80r50.per","core.cut20r500.per","core.af.cut40.per","core.af.cut20r500.per","core.c10_40r500.per","core.c10_40r50.per", "matrix.road50.per","matrix.road250.per", "matrix.road500.per","matrix.road750.per","matrix.cut40.per","matrix.cut80.per","matrix.cut20.per","matrix.cut10_40.per", "matrix.cut40r50.per", "matrix.cut40r500.per","matrix.cut80r500.per","matrix.cut80r50.per","matrix.cut20r500.per","matrix.c10_40r500.per","matrix.c10_40r50.per","total.cut40r50.per","total.cut40.per", "total.cut80.per","total.road50.per","total.road500.per","total.cut10_40.per","total.cut10_40r50.per","total.cut10_40r500.per","total.cut40r50.per", "total.cut40r500.per","total.cut80r500.per","total.cut80r50.per", "total.area")]
...
report<-function(x){
par(mfrow=c(2,2))
acf(residuals(x,type="normalized"), lag.max = 10) 
...
```{r, nonliner}
#NULL
start.null<-nls(pop_estimate  ~ N0*exp(r*time) ,start=list(N0=400,r=-0.01), data=model.data)
expo.null.1 <- nlme(pop_estimate  ~ N0*exp(r*time), fixed =list(N0~1,r~1), random= N0+r~1|herd_name, weights=varExp(form=~herd.size), start =list(fixed=coef(start.null)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.null.1) #AIC:877 PHI:0.884
report(expo.null.1)
...
```{r}
#Total Road 50
start.total.road50<-nls(pop_estimate  ~ N0*exp((b*total.road50.per)*time) ,start=list(N0=400, b=0.001), data=model.data)
expo.total.road50.1 <- nlme(pop_estimate  ~ N0*exp((b*total.road50.per)*time), fixed =list(N0~1,b~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.total.road50)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.total.road50.1) #AIC:877 PHI:0.884
report(expo.total.road50.1)
#Core and Matrix road 50
start.road50<-nls(pop_estimate  ~ N0*exp((b*core.road50.per+c*matrix.road50.per)*time) ,start=list(N0=400, b=-0.001, c=-0.001), data=model.data)
expo.road50.1 <- nlme(pop_estimate  ~ N0*exp((b*core.road50.per+c*matrix.road50.per)*time), fixed =list(N0~1,b~1,c~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.road50)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.road50.1) #AIC:877 PHI:0.884
report(expo.road50.1)

AICc(expo.road50.1, expo.total.road50.1)
anova(expo.road50.1, expo.total.road50.1)

#Total Road 500
start.total.road500<-nls(pop_estimate  ~ N0*exp((b*total.road500.per)*time) ,start=list(N0=400, b=0.001), data=model.data)
expo.total.road500.1 <- nlme(pop_estimate  ~ N0*exp((b*total.road500.per)*time), fixed =list(N0~1,b~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.total.road500)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.total.road500.1) #AIC:877 PHI:0.884
report(expo.total.road500.1)
#Core and Matrix road 500
start.road500<-nls(pop_estimate  ~ N0*exp((b*core.road500.per+c*matrix.road500.per)*time) ,start=list(N0=400, b=-0.001, c=-0.001), data=model.data)
expo.road500.1 <- nlme(pop_estimate  ~ N0*exp((b*core.road500.per+c*matrix.road500.per)*time), fixed =list(N0~1,b~1,c~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.road500)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.road500.1) #AIC:877 PHI:0.884
report(expo.road500.1)

AICc(expo.road500.1, expo.total.road500.1,expo.road50.1, expo.total.road50.1)
anova(expo.road500.1, expo.total.road500.1)

anova(expo.road50.1, expo.road500.1)
```

#Cutblock models
```{r}
#Total cut10-40
start.total.cut10_40<-nls(pop_estimate  ~ N0*exp((b*total.cut10_40.per)*time) ,start=list(N0=400, b=0.001), data=model.data)
expo.total.cut10_40.1 <- nlme(pop_estimate  ~ N0*exp((b*total.cut10_40.per)*time), fixed =list(N0~1,b~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.total.cut10_40)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.total.cut10_40.1) #AIC:877 PHI:0.884
report(expo.total.cut10_40.1)
#Core and Matrix cut10_40
start.cut10_40<-nls(pop_estimate  ~ N0*exp((b*core.cut10_40.per+c*matrix.cut10_40.per)*time) ,start=list(N0=400, b=-0.001, c=-0.001), data=model.data)
expo.cut10_40.1 <- nlme(pop_estimate  ~ N0*exp((b*core.cut10_40.per+c*matrix.cut10_40.per)*time), fixed =list(N0~1,b~1,c~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.cut10_40)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.cut10_40.1) #AIC:877 PHI:0.884
report(expo.cut10_40.1)

#Cut40
start.total.cut40<-nls(pop_estimate  ~ N0*exp((b*total.cut40.per)*time) ,start=list(N0=400, b=0.001), data=model.data)
expo.total.cut40.1 <- nlme(pop_estimate  ~ N0*exp((b*total.cut40.per)*time), fixed =list(N0~1,b~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.total.cut40)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.total.cut40.1) #AIC:877 PHI:0.884
report(expo.total.cut40.1)
#Core and Matrix cut40
start.cut40<-nls(pop_estimate  ~ N0*exp((b*core.cut40.per+c*matrix.cut40.per)*time) ,start=list(N0=400, b=-0.001, c=-0.001), data=model.data)
expo.cut40.1 <- nlme(pop_estimate  ~ N0*exp((b*core.cut40.per+c*matrix.cut40.per)*time), fixed =list(N0~1,b~1,c~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.cut40)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.cut40.1) #AIC:877 PHI:0.884
report(expo.cut40.1)

#Cut80
start.total.cut80<-nls(pop_estimate  ~ N0*exp((b*total.cut80.per)*time) ,start=list(N0=400, b=-0.001), data=model.data)
expo.total.cut80.1 <- nlme(pop_estimate  ~ N0*exp((b*total.cut80.per)*time), fixed =list(N0~1,b~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.total.cut80)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.total.cut80.1) #AIC:877 PHI:0.884
report(expo.total.cut80.1)
#Core and Matrix cut80
start.cut80<-nls(pop_estimate  ~ N0*exp((b*core.cut80.per+c*matrix.cut80.per)*time) ,start=list(N0=192, b=-0.01, c=-0.01), data=model.data)
expo.cut80.1 <- nlme(pop_estimate  ~ N0*exp((b*core.cut80.per+c*matrix.cut80.per)*time), fixed =list(N0~1,b~1,c~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.cut80)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.cut80.1) #AIC:877 PHI:0.884
report(expo.cut80.1)

AICc(expo.cut10_40.1, expo.total.cut10_40.1, expo.cut40.1, expo.total.cut40.1, expo.cut80.1, expo.total.cut80.1)
```

#Combined models
```{r}
#Cut10_40r50
start.total.cut10_40r50<-nls(pop_estimate  ~ N0*exp((b*total.cut10_40r50.per)*time) ,start=list(N0=400, b=0.001), data=model.data)
expo.total.cut10_40r50.1 <- nlme(pop_estimate  ~ N0*exp((b*total.cut10_40r50.per)*time), fixed =list(N0~1,b~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.total.cut10_40r50)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.total.cut10_40r50.1) #AIC:877 PHI:0.884
report(expo.total.cut10_40r50.1)
#Core and Matrix cut10_40r50
start.cut10_40r50<-nls(pop_estimate  ~ N0*exp((b*core.c10_40r50.per+c*matrix.c10_40r50.per)*time) ,start=list(N0=400, b=-0.001, c=-0.001), data=model.data)
expo.cut10_40r50.1 <- nlme(pop_estimate  ~ N0*exp((b*core.c10_40r50.per+c*matrix.c10_40r50.per)*time), fixed =list(N0~1,b~1,c~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.cut10_40r50)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.cut10_40r50.1) #AIC:877 PHI:0.884
report(expo.cut10_40r50.1)

#Cut10_40r500
start.total.cut10_40r500<-nls(pop_estimate  ~ N0*exp((b*total.cut10_40r500.per)*time) ,start=list(N0=400, b=0.001), data=model.data)
expo.total.cut10_40r500.1 <- nlme(pop_estimate  ~ N0*exp((b*total.cut10_40r500.per)*time), fixed =list(N0~1,b~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.total.cut10_40r500)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.total.cut10_40r500.1) #AIC:877 PHI:0.884
report(expo.total.cut10_40r500.1)
#Core and Matrix cut10_40r500
start.cut10_40r500<-nls(pop_estimate  ~ N0*exp((b*core.c10_40r500.per+c*matrix.c10_40r500.per)*time) ,start=list(N0=400, b=-0.001, c=-0.001), data=model.data)
expo.cut10_40r500.1 <- nlme(pop_estimate  ~ N0*exp((b*core.c10_40r500.per+c*matrix.c10_40r500.per)*time), fixed =list(N0~1,b~1,c~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.cut10_40r500)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.cut10_40r500.1) #AIC:877 PHI:0.884
report(expo.cut10_40r500.1)

#Cut40r50
start.total.cut40r50<-nls(pop_estimate  ~ N0*exp((b*total.cut40r50.per)*time) ,start=list(N0=400, b=0.001), data=model.data)
expo.total.cut40r50.1 <- nlme(pop_estimate  ~ N0*exp((b*total.cut40r50.per)*time), fixed =list(N0~1,b~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.total.cut40r50)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.total.cut40r50.1) #AIC:877 PHI:0.884
report(expo.total.cut40r50.1)
#Core and Matrix cut40r50
start.cut40r50<-nls(pop_estimate  ~ N0*exp((b*core.cut40r50.per+c*matrix.cut40r50.per)*time) ,start=list(N0=400, b=-0.001, c=-0.001), data=model.data)
expo.cut40r50.1 <- nlme(pop_estimate  ~ N0*exp((b*core.cut40r50.per+c*matrix.cut40r50.per)*time), fixed =list(N0~1,b~1,c~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.cut40r50)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.cut40r50.1) #AIC:877 PHI:0.884
report(expo.cut40r50.1)

#Cut40r500
start.total.cut40r500<-nls(pop_estimate  ~ N0*exp((b*total.cut40r500.per)*time) ,start=list(N0=400, b=0.001), data=model.data)
expo.total.cut40r500.1 <- nlme(pop_estimate  ~ N0*exp((b*total.cut40r500.per)*time), fixed =list(N0~1,b~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.total.cut40r500)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.total.cut40r500.1) #AIC:877 PHI:0.884
report(expo.total.cut40r500.1)
#Core and Matrix cut40r500
start.cut40r500<-nls(pop_estimate  ~ N0*exp((b*core.cut40r500.per+c*matrix.cut40r500.per)*time) ,start=list(N0=400, b=-0.001, c=-0.001), data=model.data)
expo.cut40r500.1 <- nlme(pop_estimate  ~ N0*exp((b*core.cut40r500.per+c*matrix.cut40r500.per)*time), fixed =list(N0~1,b~1,c~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.cut40r500)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.cut40r500.1) #AIC:877 PHI:0.884
report(expo.cut40r500.1)

#Cut80r50
start.total.cut80r50<-nls(pop_estimate  ~ N0*exp((b*total.cut80r50.per)*time) ,start=list(N0=400, b=0.001), data=model.data)
expo.total.cut80r50.1 <- nlme(pop_estimate  ~ N0*exp((b*total.cut80r50.per)*time), fixed =list(N0~1,b~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.total.cut80r50)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.total.cut80r50.1) #AIC:877 PHI:0.884
report(expo.total.cut80r50.1)
#Core and Matrix cut80r50
start.cut80r50<-nls(pop_estimate  ~ N0*exp((b*core.cut80r50.per+c*matrix.cut80r50.per)*time) ,start=list(N0=400, b=-0.001, c=-0.001), data=model.data)
expo.cut80r50.1 <- nlme(pop_estimate  ~ N0*exp((b*core.cut80r50.per+c*matrix.cut80r50.per)*time), fixed =list(N0~1,b~1,c~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.cut80r50)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.cut80r50.1) #AIC:877 PHI:0.884
report(expo.cut80r50.1)

#Cut80r500
start.total.cut80r500<-nls(pop_estimate  ~ N0*exp((b*total.cut80r500.per)*time) ,start=list(N0=400, b=0.001), data=model.data)
expo.total.cut80r500.1 <- nlme(pop_estimate  ~ N0*exp((b*total.cut80r500.per)*time), fixed =list(N0~1,b~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.total.cut80r500)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.total.cut80r500.1) #AIC:877 PHI:0.884
report(expo.total.cut80r500.1)
#Core and Matrix cut80r500
start.cut80r500<-nls(pop_estimate  ~ N0*exp((b*core.cut80r500.per+c*matrix.cut80r500.per)*time) ,start=list(N0=400, b=-0.001, c=-0.001), data=model.data)
expo.cut80r500.1 <- nlme(pop_estimate  ~ N0*exp((b*core.cut80r500.per+c*matrix.cut80r500.per)*time), fixed =list(N0~1,b~1,c~1), weights=varExp(form=~herd.size), random= N0+b~1|herd_name, start =list(fixed=coef(start.cut80r500)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.cut80r500.1) #AIC:877 PHI:0.884
report(expo.cut80r500.1)

AICc(expo.total.cut10_40r50.1,expo.cut10_40r50.1,expo.total.cut10_40r500.1,expo.cut10_40r500.1,expo.total.cut40r50.1,expo.cut40r50.1,
...
newdat<-expand.grid(herd_name = 'Barkerville', core.road500.per=seq(from = min(model.data$core.road500.per),
                            to = max(model.data$core.road500.per),
                            by = 0.1),
                    matrix.road500.per= mean(model.data$matrix.road500.per),
                    time =1)
ggplotly(ggplot(model.data[,c("core.road500.per", "pop_estimate", "herd_name", "time")], aes(x=core.road500.per, y=pop_estimate, colour=herd_name)) +
  geom_point(size=3) +
  geom_line(aes(y=predict(expo.road500.1), group=herd_name)) +
  geom_line(data=newdat, aes(y=predict(expo.road500.1, level=0, newdata=newdat) ),size =1.5) 
  ) 

random.effects(expo.road500.1)$a + 192

```
#Summary
```{r}

aic.values<-AICc(expo.null.1,expo.total.cut10_40.1,expo.cut10_40.1,expo.total.road500.1,expo.road500.1,expo.total.cut80r500.1, expo.cut80r500.1)
...
summary(update(expo.total.cut10_40.1, method = 'ML'))
summary(update(expo.cut10_40.1, method = 'ML'))
summary(update(expo.total.road500.1, method = 'ML'))
summary(update(expo.road500.1, method = 'ML'))
summary(update(expo.total.cut40r500.1, method = 'ML'))
summary(update(expo.cut40r500.1, method = 'ML'))

summary(update(expo.null.1, method = 'REML'))
summary(update(expo.total.cut10_40.1, method = 'REML'))
summary(update(expo.cut10_40.1, method = 'REML'))
summary(update(expo.total.road500.1, method = 'REML'))
summary(update(expo.road500.1, method = 'REML'))
summary(update(expo.total.cut80r500.1, method = 'REML'))
summary(update(expo.cut80r500.1, method = 'REML'))
...
data.all[,herd.size:=0]
data.all[core.area >= 224000,herd.size:=1]
data.all$pred<-predict(expo.c.0, data.all,level=1)
data.all<-data.all[!is.na(pred),]

ggplot(data=data.all, aes(x=year, y= pred, group = herd_name, color = herd_name)) + geom_line() 

sum(data.all[year==1990, ]$pred)
```
