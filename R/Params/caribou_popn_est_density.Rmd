...
gam.c<-gamlss(pop_estimate ~ core.cut80.per  +matrix.cut80.per+ re(random=~1|herd_name, method = "ML", correlation = corAR1(value =0.8, form=~time|herd_name)), opt="optim",   sigma.formula = ~1 +core.area , family = LOGNO(), data = model.data, control =gamlss.control(c.crit=0.1, n.cyc=5000,msMaxIter=1000000), method=CG())
plot(gam.c)
summary(gam.c)
...
gam.r<-gamlss(pop_estimate ~ re(fixed=~core.road50.per+matrix.road50.per, random=~core.road50.per|herd_name, method = 'ML',  correlation = corAR1(form = ∼time|herd_name)), opt="optim",   sigma.formula = ~1+core.area, family = LOGNO(), data = model.data, control =gamlss.control(c.crit=0.1, n.cyc=5000,msMaxIter=1000000), method=CG())
summary(gam.r)
plot(gam.r)
