...
model.data$res<-abs(residuals(expo.total.road50.1, type= 'normalized'))**2
lev<- lm(res ~ as.factor(herd_name), data=model.data) 
anova(lev) #f-stat for levenes

anova(expo.null.1, expo.total.road50.1)
...
summary(expo.road50.s) #AIC:877 PHI:0.884
...
anova(expo.null.1, expo.road50.1, expo.total.road50.1)
...
start.road50<-nlsLM(pop_estimate~exp(a+b*core.road500.per+c*matrix.road500.per) ,start=list(a=-3.5, b=-0.001, c= -0.009), data=model.data)
...
expo.total.cut10_40.1 <- nlme(pop_estimate~exp(a+b*total.cut10_40.per), fixed =list(a~1, b~1),  random= a+b~1|herd_name, weights=varExp(form=~pop.0), start =list(fixed=coef(start.total.cut10_40)), data = model.data, method = 'ML', control = c(maxIter = 100000))
...
expo.cut10_40.1 <- nlme(pop_estimate  ~ exp(a+b*core.cut10_40.per+c*matrix.cut10_40.per), fixed =list(a~1, b~1,c~1),  random= a+c~1|herd_name,weights=varPower(form=~pop.0), start =list(fixed=coef(start.cut10_40)), data = model.data, method = 'ML', control = c(maxIter = 100000))
...
start.cut40<-nls(pop_estimate~exp(a+b*core.cut40.per+c*matrix.cut40.per) ,start=list(a=.001, b=-0.001, c =-0.002), data=model.data)
...
summary(expo.cut80.s) #AIC:877 PHI:0.884
report(expo.cut80.s)
model.data$res<-abs(residuals(expo.cut80.s, type= 'normalized'))**2
lev<- lm(res ~ as.factor(herd_name), data=model.data) 
anova(lev) #f-stat for levenes

anova(expo.null.1,expo.cut80.1, expo.total.cut80.1)
...
expo.cut80r50.s <- nlme(pop_estimate~exp(a+b*s.core.cut80r50.per+c*s.matrix.cut80r50.per), fixed =list(a~1,b~1,c~1), weights=varExp(form=~pop.0), random= a+b~1|herd_name, start =list(fixed=coef(start.cut80r50)), data = model.data, method = 'REML', control = c(maxIter = 100000))
...
expo.total.cut80r500.1 <- nlme(pop_estimate~exp(a+b*total.cut80r500.per), fixed =list(a~1,b~1), weights=varExp(form=~pop.0),random= a+b~1|herd_name, start =list(fixed=coef(start.total.cut80r500)), data = model.data, method = 'ML', control = c(maxIter = 100000))
...
start.cut80r500<-nls(pop_estimate~exp(a+b*core.cut80r500.per+c*matrix.cut80r500.per), start=list(a=0.1, b=-0.001, c=-0.0002), data=model.data)
expo.cut80r500.1 <- nlme(pop_estimate~exp(a+b*core.cut80r500.per+c*matrix.cut80r500.per), fixed =list(a~1,b~1,c~1), weights= varExp(form=~pop.0), random= a+b~1|herd_name, start =list(fixed=coef(start.cut80r500)), data = model.data, method = 'ML', control = c(maxIter = 100000))
...
```{r, obs_pred}
ggplot(model.data[,c("core.road50.per", "matrix.road50.per", "pop_estimate", "herd_name", "time", "pop.0")], aes(x=matrix.road50.per, y=pop_estimate, colour=herd_name)) +
  geom_point(size=3) +
  geom_line(aes(y=predict(expo.road50.1, level =1), group=herd_name) )
  #geom_line(data=newdat, aes(y=predict(expo.cut80r500.1, level=0, newdata=newdat) ),size =1.5)
  
par(mfrow=c(2,2))
plot(predict(expo.road50.1, level =1), model.data$pop_estimate)
abline(0,1, col = 'red')
plot(predict(expo.road500.1, level =1), residuals(expo.road500.1, type= 'normalized'))
abline(0,0, col = 'red')
acf(residuals(expo.road500.1,  type= 'normalized'))
hist(residuals(expo.road500.1,  type= 'normalized') , freq = FALSE)
curve(dnorm(x,0, sd(residuals(expo.road500.1,  type= 'normalized'))), add=T)


dtgx<-data.table(cbind(model.data[,c("pop_estimate", "herd_name", "herd.size")],fitted = predict(expo.road50.1)))

dtgx<-dtgx[, herd_name:=lapply(.SD, function(x) { gsub("_", " ", x)}), .SDcols = "herd_name"]

ggplot(data = dtgx, aes(y=pop_estimate, x =fitted, color = herd_name)) +
...
a1<-ggplot(data = model.data[, c("core.road50.per", "pop_estimate", "herd_name")], aes(x = core.road50.per, y = pop_estimate, colour = factor(herd_name))) +
  theme(legend.position = "none")+
    geom_point(size=3) +
    geom_line(aes(y = predict(expo.road50.1) ) )

a2<-ggplot(data = model.data[, c("matrix.road50.per", "pop_estimate", "herd_name")], aes(x = matrix.road50.per, y = pop_estimate, colour = factor(herd_name))) +
    theme(legend.position = "none")+
    geom_point(size=3) +
    geom_line(aes(y = predict(expo.road50.1)))+
    geom_line(aes(y = predict(expo.road50.1, level=0)),col='red' )
