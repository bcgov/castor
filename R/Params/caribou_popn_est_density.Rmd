...
#Core and Matrix cut80r500
start.cut80plusr50<-nls(pop_estimate~exp(a+b*core.cut80.per+c*matrix.road50.per), start=list(a=7, b=-0.01, c=-0.0002), data=model.data)
expo.cut80plusr50.1 <- nlme(pop_estimate~exp(a+b*core.cut80.per+c*matrix.road50.per), fixed =list(a~1,b~1,c~1), weights= varExp(form=~herd.size), random= a+b~1|herd_name, start =list(fixed=coef(start.cut80plusr50)), data = model.data, method = 'ML', control = c(maxIter = 100000))
summary(expo.cut80plusr50.1) #AIC:877 PHI:0.884
report(expo.cut80plusr50.1)
...
abline(0,1, col = 'red')
plot(predict(expo.road500.1, level =1), residuals(expo.road500.1, type= 'normalized'), title= '', ylab = 'Normalized Residuals', xlab = 'Predicted')
abline(0,0, col = 'red')
acf(residuals(expo.road500.1,  type= 'normalized'), main = '')
hist(residuals(expo.road500.1,  type= 'normalized') , freq = FALSE,main = '', xlab = 'Normalized Residuals')
