...
###Grizzly Bear Habitat in GBR North and South
See legal direction and orders (Schedule D). In GBR, 100% of class 1 grizzly bear habitat and 50% of class 2 habitat needs to be protected but these class 1 and class 2 habitats are contained in the WHA's. So Im adding it in manually. This is where I found the information about these habitat types and spatial layers. See: https://www2.gov.bc.ca/gov/content/industry/crown-land-water/land-use-planning/regions/west-coast/great-bear-rainforest/great-bear-rainforest-legal-direction-agreements
...
gbr_bear<-sf::st_read("//spatialfiles2.bcgov/work/FOR/VIC/HTS/ANA/Workarea/ekleynhans/GBRO_ScheduleShapefiles_20160120/GBRSchD_GB_20151105.shp")
gbr_bear_1 <- gbr_bear %>% filter(GB_CLASS==1)
gbr_bear_1$feat_notes<-"Class 1 grizzly bear habitat"
...
(SELECT a.wha_tag  FROM public.wcp_whaply_polygon  a , public.tsa_aac_bounds_gbr b WHERE  ST_INTERSECTS (a.wkb_geometry, b.wkb_geometry)) ORDER BY aprv_date ASC")

gbr_bear_1$ogc_fid<-max(wha_poly$ogc_fid)+1
gbr_bear_1$wha_tag<-"5-800"
gbr_bear_1$aprv_date<-"20160101"
gbr_bear_2 <- gbr_bear %>% filter(GB_CLASS==2)
gbr_bear_2$feat_notes<-"Class 2 grizzly bear habitat"
gbr_bear_2$ogc_fid<-max(wha_poly$ogc_fid)+2
gbr_bear_2$wha_tag<-"5-801"
gbr_bear_2$aprv_date<-"20160101"
gbr_bear_12<- rbind(gbr_bear_1, gbr_bear_2)
names(gbr_bear_12)
gbr_bear_all<- gbr_bear_12 %>% 
  dplyr::select(feat_notes, ogc_fid, wha_tag, aprv_date) %>% 
  group_by (feat_notes, ogc_fid, wha_tag, aprv_date) %>% summarise()

lu<-getSpatialQuery("SELECT *
FROM public.rmp_lu_sp_polygon LIMIT 1") #just a dummy query to get the projection

st_crs(gbr_bear_all)<-st_crs(lu)
setnames(gbr_bear_all, c("geometry"), c("wkb_geometry"))
st_geometry(gbr_bear_all) <- "wkb_geometry"

wha_cond_gbr<-data.table(ogc_fid=character(), harvest=character(), wha_tag=character(), feat_notes=character(), aprv_date=integer())

wha_cond_gbr <- data.frame (matrix (ncol = 5, nrow = 0))
colnames (wha_cond_gbr) <- c ("ogc_fid", "harvest", "wha_tag", "feat_notes", "aprv_date")
wha_cond_gbr[1,1]<-gbr_bear_1$ogc_fid[1]
wha_cond_gbr[2,1]<-gbr_bear_2$ogc_fid[1]
wha_cond_gbr[1,2]<-"NO HARVEST ZONE"
wha_cond_gbr[2,2]<-"CONDITIONAL HARVEST ZONE"
wha_cond_gbr[1,3]<-"5-800"
wha_cond_gbr[2,3]<-"5-801"
wha_cond_gbr[1,4]<-"Class 1 grizzly bear habitat"
wha_cond_gbr[2,4]<-"Class 2 grizzly bear habitat"
wha_cond_gbr[1,5]<-"20160101"
wha_cond_gbr[2,5]<-"20160101"
```
...
plot(wha_poly)
wha_poly_new<-rbind(wha_poly, gbr_bear_all)
plot(wha_poly_new)

#fasterize 
wha.ras.gbr<-fasterize::fasterize(sf= wha_poly_new, raster = ProvRast , field = "ogc_fid")
...
zone_wha<-data.table(zoneid=integer(), type=character(), variable=character(), threshold=numeric(), reference_zone=character(), percentage=numeric(), ndt=integer(),
                     label = character(), multi_condition=character())

#get the ogc_fid from raster
ogc_fids<-data.table(unique(c(t(raster::as.matrix(wha.ras.gbr)))))
...
wha_cond<-rbind(wha_cond, wha_cond_gbr)
...
zone_wha<-data.table(zoneid = 0, type = 'nh', variable = '', threshold = 0, reference_zone='rast.zone_cond_wha_gbr', percentage =0, ndt =0, label = 'wha_no_harv')
#set all to 0 in the raster as no harvest
wha.ras.gbr[wha.ras.gbr[] %in% ogc_fids_noharv] <- 0
...
wha.ras.gbr[wha.ras.gbr[] %in% ogc_fids_rm]<-0 #reassign in the raster
...
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags, "ogc_fid"]
wha.ras.gbr[wha.ras.gbr[] %in% ogc_fids_rm]<-NA
wha_cond<-wha_cond[!(wha_cond$wha_tag %in% wha_tags), ] #remove from table
```
...
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'height', threshold = 5, reference_zone='rast.zone_cond_wha_gbr', percentage =70, ndt =0, label = ogc_fids_rm$wha_tag))
wha_cond<-wha_cond[!(wha_cond$wha_tag %in% wha_tags), ] #remove from table
```
...
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags,c("ogc_fid", "wha_tag") ]
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 1, reference_zone='rast.zone_cond_wha_gbr', percentage =50, ndt =0, label = ogc_fids_rm$wha_tag))
wha_cond<-wha_cond[!(wha_cond$wha_tag %in% wha_tags), ] #remove from table
```
...
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'crownclosure', threshold = 20, reference_zone='rast.zone_cond_wha_gbr', percentage =80, ndt =0, label = ogc_fids_rm$wha_tag))
wha_cond<-wha_cond[!(wha_cond$wha_tag %in% wha_tags), ] #remove from table
```
...
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags, "ogc_fid"]
wha.ras.gbr[wha.ras.gbr[] %in% ogc_fids_rm]<-0 #remove from the raster
wha_cond<-wha_cond[!(wha_cond$wha_tag %in% wha_tags), ] #remove from table
```
...
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags, "ogc_fid"]
wha.ras.gbr[wha.ras.gbr[] %in% ogc_fids_rm]<-0
wha_cond<-wha_cond[!(wha_cond$wha_tag %in% wha_tags), ] #remove from table
```
...
```{r, w5072}
# note Kyle only had 5-072 here. I added the other two
#get the ogc_fids that belong to these.
wha_tags<-c('5-062', '5-072', '5-073')
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags, "ogc_fid"]
wha.ras.gbr[wha.ras.gbr[] %in% ogc_fids_rm]<-NA
wha_cond<-wha_cond[!(wha_cond$wha_tag %in% wha_tags), ] #remove from table
```

### 5-074 to 5-082, 5-084, 5-085 American Badger
Do not construct new roads. Forestry must result in ecological restoration with stocking densities of <75 stems/ha. Do not use pesticides. See http://www.env.gov.bc.ca/wld/documents/wha/TATA_5-074to5-082,5-084,5-085_ord.pdf
```{r, w5074}
#get the ogc_fids that belong to these.
...
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags, c("ogc_fid","wha_tag")]
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 80, reference_zone='rast.zone_cond_wha_gbr', percentage =45, ndt =0, label = ogc_fids_rm$wha_tag))
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 100, reference_zone='rast.zone_cond_wha_gbr', percentage =37, ndt =0, label = ogc_fids_rm$wha_tag))
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 120, reference_zone='rast.zone_cond_wha_gbr', percentage =30, ndt =0, label = ogc_fids_rm$wha_tag))
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 140, reference_zone='rast.zone_cond_wha_gbr', percentage =25, ndt =0, label = ogc_fids_rm$wha_tag))
wha_cond<-wha_cond[!(wha_cond$wha_tag %in% wha_tags), ] #remove from table

wha_tags<-c('5-086') #40% > 70 and 20% > 80
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags, c("ogc_fid","wha_tag")]
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 70, reference_zone='rast.zone_cond_wha_gbr', percentage =40, ndt =0, label = ogc_fids_rm$wha_tag))
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 80, reference_zone='rast.zone_cond_wha_gbr', percentage =20, ndt =0, label = ogc_fids_rm$wha_tag))

wha_tags<-c('5-872','5-873') #25% is left unlogged with an age > 100 years
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags,c("ogc_fid", "wha_tag") ]
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 100, reference_zone='rast.zone_cond_wha_gbr', percentage =25, ndt =0, label = ogc_fids_rm$wha_tag))
wha_cond<-wha_cond[!(wha_cond$wha_tag %in% wha_tags), ] #remove from table
```
...
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags,c("ogc_fid", "wha_tag") ]
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 80, reference_zone='rast.zone_cond_wha_gbr', percentage =66, ndt =0, label = ogc_fids_rm$wha_tag))
wha_cond<-wha_cond[!(wha_cond$wha_tag %in% wha_tags), ] #remove from table
```
...
### 5-753 to 5-758, and 5-760 to 5-764 Sandhill Crane
Do not construct roads. Do not harvest timber. Do not use pesticides. See http://www.env.gov.bc.ca/wld/documents/wha/SACR_Order5-753to758_760to764.pdf
```{r, w5753}
#get the ogc_fids that belong to these.
wha_tags<-c('5-753', '5-754', '5-755', '5-756', '5-757', '5-758', '5-760', '5-761', '5-762', '5-763', '5-764')
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags, "ogc_fid"]
wha.ras.gbr[wha.ras.gbr[] %in% ogc_fids_rm]<-NA
wha_cond<-wha_cond[!(wha_cond$wha_tag %in% wha_tags), ] #remove from table
```
...
### 5-874 to 5-876 to 5-883 American Badger
Do not construct roads or landings. Harvest must result in stocking densities of <75 stems/ha and a target of 20 stem/ha at free-to-grow. Do not use pesticides. Do not construct range developments. See http://www.env.gov.bc.ca/wld/documents/wha/TATA_5-874_876-883_Order.pdf
```{r, w5874}
#get the ogc_fids that belong to these.
wha_tags<-c('5-874', '5-876', '5-877', '5-878', '5-879', '5-880', '5-881', '5-882', '5-883')
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags, "ogc_fid"]
wha.ras.gbr[wha.ras.gbr[] %in% ogc_fids_rm]<-NA
wha_cond<-wha_cond[!(wha_cond$wha_tag %in% wha_tags), ] #remove from table
```

### 5-875  American Badger
No primary forest activities allowed. Do not use pesticides. Do not construct range developments. See http://www.env.gov.bc.ca/wld/documents/wha/TATA_5-875_Order.pdf
```{r, w5875}
#get the ogc_fids that belong to these.
wha_tags<-c('5-875')
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags, "ogc_fid"]
...
### 5-884 to 5-894, 5-896, 5-897  Great Basin Spadefoot
No roads of landings. Stocking densities of <75 stems/ha and a target of 20 stems/ha at free-to-grow. No pesticides. No range developments. No recreation sites, trails, facilities, or structures. Specifically for 5-895 no primary forest activities may occur.
```{r, w5875}
#get the ogc_fids that belong to these.
wha_tags<-c('5-884', '5-885', '5-886', '5-887', '5-888', '5-889', '5-890', '5-891', '5-892', '5-893', '5-894', '5-895', '5-896', '5-897')
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags, "ogc_fid"]
wha.ras.gbr[wha.ras.gbr[] %in% ogc_fids_rm]<-NA
wha_cond<-wha_cond[!(wha_cond$wha_tag %in% wha_tags), ] #remove from table
```

### 5-904, 5-905, 5-911 to 5-916  Data sensitive
Data sensitive
```{r, w5875}
#get the ogc_fids that belong to these.
wha_tags<-c('5-905', '5-905', '5-911', '5-912', '5-913', '5-914', '5-915', '5-916')
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags, "ogc_fid"]
wha.ras.gbr[wha.ras.gbr[] %in% ogc_fids_rm]<-NA
wha_cond<-wha_cond[!(wha_cond$wha_tag %in% wha_tags), ] #remove from table
```
...
### 6-055 Grizzly Bear
http://www.env.gov.bc.ca/wld/documents/wha/URAR_6-055_ord.pdf
No timber harvest or forest health sanitation allowed to occur. 
```{r, 6-055}
#get the ogc_fids that belong to these.
wha_tags<-'6-055'
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags, "ogc_fid"]
...
### 6-058 and 6-059 Coastal Tailed Frog
No harvest within the core area of special management zone also no road construction or stream crossings. see http://www.env.gov.bc.ca/wld/documents/wha/ASTR_6-058-059_ord.pdf
```{r, w6058}
wha_tags<-c('6-058', '6-059') # I broke these two out
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags, "ogc_fid"]
wha.ras.gbr[wha.ras.gbr[] %in% ogc_fids_rm]<-0
wha_cond<-wha_cond[!(wha_cond$wha_tag %in% wha_tags), ] #remove from table
```
...
### 6-153 to 6-156, 6-158, 6-160, 6-161, 6-164, 6-166, 6-170, 6-171, 6-174, 6-177, 6-179 to 6-188, 6-280, 6-281 Marbled Murrelet - North Coast Forest District
see http://www.env.gov.bc.ca/wld/documents/wha/MAMU-6-153var6-281_Order.pdf
Do not construct  or widen roads. Do not harvest timber. Do not use pesticides. Do not establish recreation sites or trails
```{r, w8003}
#get the ogc_fids that belong to these.
wha_tags<-c('6-153','6-154', '6-155', '6-156', '6-158', '6-160', '6-161', '6-164', '6-166', '6-170', '6-171', '6-174', '6-177', '6-179', '6-180', '6-181', '6-182', '6-183', '6-184', '6-185', '6-186', '6-187', '6-188', '6-280', '6-281')
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags, "ogc_fid"]
wha.ras.gbr[wha.ras.gbr[] %in% ogc_fids_rm]<-NA
wha_cond<-wha_cond[!(wha_cond$wha_tag %in% wha_tags), ] #remove from table
```
...
### 6-287 Grizzly bear
To provide functional thermal or security cover to grizzly bears 100% of the forested area of each WHA must be retained in mature and old-growth condition. An allowance of 90% maintained in mature and old-growth may be made to facilitate access, operational saftey, and/or to minimize impacts on adjacent environmental values. See http://www.env.gov.bc.ca/wld/documents/wha/URAR_6-287_ord.pdf
```{r, w6287}
#get the ogc_fids that belong to these.
wha_tags<-c('6-287')
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags, "ogc_fid"]
...
### 6-288 Grizzly bear
To provide functional thermal or security cover to grizzly bears 100% of the forested area of each WHA must be retained in mature and old-growth condition. An allowance of 90% maintained in mature and old-growth may be made to facilitate access, operational saftey, and/or to minimize impacts on adjacent environmental values.  No pesticides may be used. Where ever practicable, 150 m is the min distance between permanent roads and the WHA. See http://www.env.gov.bc.ca/wld/documents/wha/URAR_6-288_ord.pdf
```{r, w6288}
#get the ogc_fids that belong to these.
wha_tags<-c('6-288')
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags, "ogc_fid"]
...
wha_tags<-c('8-232')
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags,c("ogc_fid", "wha_tag") ]
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'height', threshold = 19.5, reference_zone='rast.zone_cond_wha_gbr', percentage =10, ndt =0, label = ogc_fids_rm$wha_tag))
wha_cond<-wha_cond[!(wha_cond$wha_tag %in% wha_tags), ] #remove from table
```
...
# This section of code gives errors related to the wha_tag number. I think this tag number does not exist???  
wha_tags<-c('8-233')
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags,c("ogc_fid", "wha_tag") ]
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'height', threshold = 14, reference_zone='rast.zone_cond_wha_gbr', percentage =30, ndt =0, label = ogc_fids_rm$wha_tag))
wha_cond<-wha_cond[!(wha_cond$wha_tag %in% wha_tags), ] #remove from table
```
...
ogc_fids_rm<-wha_cond[wha_cond$wha_tag %in% wha_tags,c("ogc_fid", "wha_tag") ]
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 40, reference_zone='rast.zone_cond_wha_gbr', percentage =50, ndt =0, label = ogc_fids_rm$wha_tag))
wha_cond<-wha_cond[!(wha_cond$wha_tag %in% wha_tags), ] #remove from table
...
### GBR Grizzly Bear class 2 habitat
From the legal direction and orders (Schedule D) for GBR it says that 50% of class 2 grizzly bear habitat needs to be protected.  See: https://www2.gov.bc.ca/gov/content/industry/crown-land-water/land-use-planning/regions/west-coast/great-bear-rainforest/great-bear-rainforest-legal-direction-agreements
...
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'crownclosure', threshold = 50, reference_zone='rast.zone_cond_wha_gbr', percentage =50, ndt =0, label = ogc_fids_rm$wha_tag))
wha_cond<-wha_cond[!(wha_cond$wha_tag %in% wha_tags), ] #remove from table
```

### Clean up and Commit data
```{r, clean_commit}
writeRaster(wha.ras.gbr, file=paste0(here(), "/wha_gbr.tif"), format="GTiff", overwrite=TRUE)

system("cmd.exe", input = paste0('raster2pgsql -s 3005 -d -I -C -M -N 2147483648  ', here(), '/wha_gbr.tif -t 100x100 rast.zone_cond_wha_gbr |  psql postgres://', keyring::key_get('dbuser', keyring = 'postgreSQL'), ':', keyring::key_get('dbpass', keyring = 'postgreSQL'), '@', keyring::key_get('dbhost', keyring = 'postgreSQL'), ':5432/clus'), show.output.on.console = FALSE, invisible = TRUE)
...
DBI::dbWriteTable(conn, c("public", "zone_wha_gbr"), value= df, row.names = FALSE, overwrite = TRUE) 
```