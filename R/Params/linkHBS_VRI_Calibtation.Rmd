...
calb_data4<-merge(calb_data, pv_timber_mrk2, by = "timber_mark")
calb_data4<-na.omit(calb_data4)

#map the observations?
bec <-get_layer("bec")
ggplot() + geom_sf(data = bec, aes(fill=ZONE), size =0.1)+ geom_point(data=calb_data4, aes(x=X, y=Y), color="red")
```
...
model1 <- lm(obs_vol ~ proj_vol, data=calb_data4)
summary(model1)
temp_var <- predict(model1, data =calb_data4, interval="prediction")
init.data <- cbind(calb_data4, temp_var)
...
ggplot(init.data , aes(proj_vol, obs_vol)) +
  geom_point() +
...
data.76<-rbind(data.table(vol = summed, type = "assume_ind"), data.table(vol = summed.auto, type = "spatial"))
