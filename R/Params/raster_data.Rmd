...
lyr<-getSpatialQuery(paste("SELECT wkb_geometry FROM public.bec_zone limit 1"))

#Make an empty provincial raster aligned with hectares BC
ProvRast <- raster(
  nrows = 15744, ncols = 17216, xmn = 159587.5, xmx = 1881187.5, ymn = 173787.5, ymx = 1748187.5, 
  crs = crs(lyr), resolution = c(100, 100), vals = 0
)
```
...
#get 2023 THLB
layer<-getSpatialQuery(paste("SELECT thlb_fact, shape FROM public.bc_thlb_2022"))
layer.ras <-fasterize::fasterize(sf= layer, raster = ProvRast , field = "thlb_fact")
layer.ras[is.na(layer.ras[])]<-0
writeRaster(layer.ras, file="thlb2022", format="GTiff", overwrite=TRUE)
...
layer<-getSpatialQuery("SELECT harvest_year, shape FROM public.cut_block_all_bc WHERE area_ha >= 0.5 ORDER BY harvest_year")
layer.ras <-fasterize::fasterize(sf= layer, raster = ProvRast , field = "harvest_year")

writeRaster(layer.ras, file="cns_cutblk_year_2023.tif", format="GTiff", overwrite=TRUE)

system("cmd.exe", input = "raster2pgsql -s 3005 -d -I -C -M  C:/Users/KLOCHHEA/clus/R/Params/cns_cutblk_year_2023.tif -t 100x100 rast.cns_cutblk_year_2023 | psql -d clus")

#Harvest locations id
layer<-getSpatialQuery("SELECT objectid, shape FROM public.cut_block_all_bc WHERE data_source <> 'Landsat' ORDER BY objectid")

layer.ras <-fasterize::fasterize(sf= layer, raster = ProvRast , field = "objectid")
layer.ras[is.na(layer.ras[])]<-0
writeRaster(layer.ras, file="cns_cutblk_2023.tif", format="GTiff", overwrite=TRUE)

system("cmd.exe", input = "raster2pgsql -s 3005 -d -I -C -M  C:/Users/KLOCHHEA/clus/R/Params/cns_cutblk_2023.tif -t 100x100 rast.cns_cut_bl_2023 | psql -d clus")
...
```{r, veg_comp, echo = FALSE}
#Harvest locations year
layer<-getSpatialQuery("SELECT feature_id, shape FROM public.veg_comp_lyr_r1_poly2020")

layer.ras <-fasterize::fasterize(sf= layer, raster = ProvRast , field = "feature_id")
rm(layer)
gc()
layer.ras[is.na(layer.ras)]<-0
writeRaster(layer.ras, file="vri2020_id.tif", datatype = 'INT4U', format="GTiff", overwrite=TRUE)

system("cmd.exe", input = "raster2pgsql -s 3005 -d -I -C -M  C:/Users/KLOCHHEA/clus/R/Params/vri2020_id.tif -t 100x100 rast.vri2020_id | psql -d clus")
```
## VEG comp 2021
```{r}
...
writeRaster(layer.ras, file="vri2021_id2.tif", format="GTiff", datatype = 'INT4U', overwrite=TRUE)
system("cmd.exe", input = "raster2pgsql -s 3005 -d -I -C -M  C:/Users/KLOCHHEA/clus/R/Params/vri2021_id.tif -t 100x100 rast.vri2021_id | psql -d clus")
```

## VEG comp 2022
```{r}
layer<-getSpatialQuery("SELECT feature_id, shape FROM vri.veg_comp_lyr_r1_poly2022")

layer.ras <-fasterize::fasterize(sf= layer, raster = ProvRast , field = "feature_id")
rm(layer)
gc()

writeRaster(layer.ras, file="vri2022_id.tif", format="GTiff", datatype = 'INT4U', overwrite=TRUE)

```

## VEG comp 2023
```{r}
layer<-getSpatialQuery("SELECT feature_id, shape FROM vri.veg_comp_lyr_r1_poly2023")

layer.ras <-fasterize::fasterize(sf= layer, raster = ProvRast , field = "feature_id")
rm(layer)
gc()

writeRaster(layer.ras, file="vri2023_id.tif", format="GTiff", datatype = 'INT4U', overwrite=TRUE)
...
# removed roads in 2022; permanent roads are now included in the rast.ce_road_2019 data
...
# all[all[] ==0 ]<-NA
# 
 writeRaster (landusers, file = here::here(), "R/Params/all_perm.tif", format="GTiff", overwrite=TRUE)

system ("cmd.exe", input = paste0('raster2pgsql -s 3005 -d -I -C -M -N 2147483648  ', here::here(), 'R/params/all_perm.tif -t 100x100 rast.perm_dist_no_rds | psql postgres://', keyring::key_get('dbuser', keyring = 'postgreSQL'), ':', keyring::key_get('dbpass', keyring = 'postgreSQL'), '@', keyring::key_get('dbhost', keyring = 'postgreSQL'), ':5432/clus'), show.output.on.console = FALSE, invisible = TRUE)
```


## Dead (Salvage) Volume Raster

```{r, dead volume}

vri <- getSpatialQuery ("SELECT * FROM public.veg_comp_lyr_r1_poly2020")
# dead_stand_volume_125, geometry

# Note: If rasterizing the DEAD_STAND_VOLUME_125 field from the 2020 Veg comp r1 data, check whether it includes inventory data from TFLs (i.e., use FAIBS internal data). Any area where the bclcs_level_1  = 'U' add in the rasterized DEAD_STAND_VOLUME_125 field from the TFL inventory. 

dead.ras <-fasterize::fasterize(sf= vri, raster = ProvRast , field = "dead_stand_volume_125")
rm(vri)
gc()

writeRaster(dead.ras, file="dead_vol_125_2020.tif", format="GTiff", overwrite=TRUE)

system("cmd.exe", input = 'raster2pgsql -s 3005 -d -I -C -M',  here::here(), '/R/Params/dead_vol_125_2020.tif -t 100x100 rast.dead_vol_125_2020 | psql -d clus')
