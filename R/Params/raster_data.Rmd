...
#Harvest locations year
layer<-getSpatialQuery("SELECT feature_id, wkb_geometry FROM public.")

layer.ras <-fasterize::fasterize(sf= layer, raster = ProvRast , field = "feature_id")
writeRaster(layer.ras, file="vri2002_id.tif", format="GTiff", overwrite=TRUE)

system("cmd.exe", input = "raster2pgsql -s 3005 -d -I -C -M  C:/Users/KLOCHHEA/clus/R/Params/vri2002_id.tif -t 100x100 rast.vri2002_id | psql -d clus")
```
...
system ("cmd.exe", input = paste0('raster2pgsql -s 3005 -d -I -C -M -N 2147483648  ', here (), '/R/params/forest_tenure.tif -t 100x100 rast.forest_tenure | psql -d clus -h ', keyring::key_get('dbhost', keyring = 'postgreSQL'), ' -U ',user = keyring::key_get('dbuser', keyring = 'postgreSQL') ,'-W ', password = keyring::key_get('dbpass', keyring = 'postgreSQL')), show.output.on.console = FALSE, invisible = TRUE)
```