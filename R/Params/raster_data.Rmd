...
layer<-getSpatialQuery("SELECT harvestyr, shape FROM public.cut_block_all_bc WHERE areaha >= 0.5 ORDER BY harvestyr")
layer.ras <-fasterize::fasterize(sf= layer, raster = ProvRast , field = "harvestyr")

writeRaster(layer.ras, file="cns_cutblk_year_2022.tif", format="GTiff", overwrite=TRUE)

system("cmd.exe", input = "raster2pgsql -s 3005 -d -I -C -M  C:/Users/KLOCHHEA/clus/R/Params/cns_cutblk_year_2022.tif -t 100x100 rast.cns_cut_bl_year_2022 | psql -d clus")
...
writeRaster(layer.ras, file="cns_cutblk_2022.tif", format="GTiff", overwrite=TRUE)

system("cmd.exe", input = "raster2pgsql -s 3005 -d -I -C -M  C:/Users/KLOCHHEA/clus/R/Params/cns_cutblk_2022.tif -t 100x100 rast.cns_cut_bl_2022 | psql -d clus")
