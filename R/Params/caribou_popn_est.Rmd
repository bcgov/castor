...
plot(data_new$lambda.dift, data_new$lambda.finite)
...
core.dist.graph<-ggplot(data = core.graph.dist, aes(x = year, y =value, color = herd_name )) +
  facet_grid(type~dm, scales='free') +
  geom_point() +
  geom_line() +
  theme(legend.position = "none") 
matrix.dist.graph<-ggplot(data = matrix.graph.dist, aes(x = year, y =value, color = herd_name )) +
  facet_grid(type~dm, scales='free') +
  geom_point() +
  geom_line() +
  theme(legend.position = "none") 

#gridExtra::grid.arrange(core.dist.graph,matrix.dist.graph)
...
m0.e <- lme(lambda.dift ~ 1 , random=~ 1|herd_name, weights = varIdent(form=~1|herd_name), correlation = corAR1(form=~time ), data = model.data, method = 'REML', control = con1)
...
abline(0,0, col = 'red')
cor(fitted(early.seral.per, level=1),model.data$lambda.dift) #0.797
...
abline(0,0, col = 'red')
cor(fitted(early.seral.rate, level =1),model.data$lambda.dift) #0.8767
...
abline(0,0, col = 'red')
cor(fitted(dist.per, level =1),model.data$lambda.dift) #0.809
...
abline(0,0, col = 'red')
cor(fitted(dist.rate, level =1),model.data$lambda.dift) #0.876
...
abline(0,0, col = 'red')
cor(fitted(dist500.rate, level =1),model.data$lambda.dift) #0.86
...
getVarCov(m0.b)
summary(early.seral.per)
getVarCov(early.seral.per)
summary(early.seral.rate)
getVarCov(early.seral.rate)
```

#GAMLSS
To be determined later
```{r, rsf, echo=FALSE, eval=FALE}
gam.dist500.per<-gamlss(l.lambda ~ core.early.seral.rate + re(random=~1|herd_name,method = "REML"), opt="optim",   sigma.formula = ~matrix.early.seral.rate, family = NO(), data = model.data, control =gamlss.control(c.crit = 0.005), method=CG())
