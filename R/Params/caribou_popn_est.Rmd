...
main<-ggplot() +
  ...
  geom_text_repel(data = herd.spat2, aes(x=X, y=Y, label = herd_name2), 
        fontface = "bold", size = 3,nudge_x = c(-4,-2.25,-4,-5,-4,-4,-4,-3,-4,-3.15,-2.25,-3,4), nudge_y = c(-0.15,0,0.25,1.5,-0.25,-0.25,0.25,0,1,0.15,0.05,0,0.75))+
...
#Metrics
data.set[, core.early.seral.rate:=( (core.early.seral-core.early.seral.lag)/core.area)*100]
data.set[, core.dist.rate:= ((core.dist-core.dist.lag)/core.area)*100]
data.set[, core.dist500.rate:= ((core.dist500-core.dist500.lag)/core.area)*100]


##Matrix
data.set[, matrix.early.seral.rate:=( (matrix.early.seral-matrix.early.seral.lag)/matrix.area)*100]
data.set[, matrix.dist.rate:= ((matrix.dist-matrix.dist.lag)/matrix.area)*100]
data.set[, matrix.dist500.rate:=( (matrix.dist500-matrix.dist500.lag)/matrix.area)*100]
...
gam.c<-gamlss(pop.den ~ matrix.early.seral.per + re(random=~1|herd_name, correlation = corAR1(value = 0.8, form = ∼time|herd_name), method = "ML"), opt="optim",   sigma.formula = ~1 + core.early.seral.per, family = GA(), data = model.data, control =gamlss.control(c.crit=0.001, n.cyc=5000,msMaxIter=1000000), method=CG())
summary(gam.c)
...
gam.cr500<-gamlss(pop.den ~ core.dist500.per + re(random=~1|herd_name,correlation = corAR1(value = 0.8, form = ∼time|herd_name), method = "ML"), opt="optim",   sigma.formula = ~1+matrix.dist500.per, family = GA(), data = model.data, control =gamlss.control(c.crit=0.001, n.cyc=5000,msMaxIter=1000000), method=CG())
summary(gam.cr500)

plot(predict(gam.cr500), model.data$log.pop.den) 
abline(0,1, col = 'red')
plot(predict(gam.c), model.data$log.pop.den)
abline(0,1, col = 'red')
