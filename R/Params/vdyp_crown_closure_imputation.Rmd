...
Natural orgin stands do not have a projection of crown closure - VDYP doesn't output crown closure over time. Thus, we will impute its value using the corresponding VRI crown closure estimate within 10 year of the reference time stamp. To do this, a strata based imputation will be used, where only observations with the same leading species group will be used.
...
SELECT t.yc_grp, t.crown_closure,
    (((k.cc - y.cc*1.0)/10)*(t.age - CAST(t.age/10 AS INT)*10))+ y.cc as cc
    FROM (SELECT crown_closure, yc_grp, proj_age_1 as age FROM vdyp_vri2018 
		  where reference_year >= 2008) as t
    LEFT JOIN yc_vdyp_all y 
    ON t.yc_grp = y.yc_grp AND CAST(t.age/10 AS INT)*10 = y.age
    LEFT JOIN yc_vdyp_all k 
