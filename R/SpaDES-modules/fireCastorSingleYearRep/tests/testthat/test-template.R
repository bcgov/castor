...
test_that("test Event1 and Event2.", {
  module <- list("fireCastorSingleYearRep")
  path <- list(
    modulePath = "C:/Users/ekleynha/AppData/Local/Temp/RtmpstFsyC/SpaDES/modules",
    outputPath = file.path(tempdir(), "outputs")
  )
  parameters <- list(
    #.progress = list(type = graphical, interval = 1),
    .globals = list(verbose = FALSE),
    fireCastorSingleYearRep = list(.saveInitialTime = NA)
  )
