...
#    `functionName` is a function's name in your module (e.g., `rsfCLUSEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/KLOCHHEA/clus/R/SpaDES-modules/rsfCLUS/tests/testthat`).
...
test_that("test Event1 and Event2.", {
  ...
  parameters <- list(
    ...
    rsfCLUS = list(.saveInitialTime = NA)
  )
  ...
  if (exists("rsfCLUSEvent1", envir = .GlobalEnv)) {
    simOutput <- rsfCLUSEvent1(mySim)
  } else {
  ...
  if (exists("rsfCLUSEvent2", envir = .GlobalEnv)) {
    simOutput <- rsfCLUSEvent2(mySim)
  } else {
