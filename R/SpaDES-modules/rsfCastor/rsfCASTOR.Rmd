...
This module tracks various Resource Selection Functions (RSF) within a Castor model simulation. 
...
outputDir <- file.path (paste0 (here(), "/R/SpaDES-modules/rsfCASTOR"))
...
cutblockSeqPrepCASTOR = list(nameCutblockRaster ="rast.cns_cut_bl"),
roadCASTOR = list(roadMethod = 'snap', 
                  nameCostSurfaceRas = 'rast.rd_cost_surface', 
                  nameRoads =  'rast.crds_resource'),
rsfCASTOR = list(calculateInterval = 2,
               writeRSFRasters = TRUE,
               criticalHabitatTable = 'public.criticalhabitat')
)

modules <- list ("dataCASTOR", "cutblockSeqPrepCASTOR", "roadCASTOR",  "rsfCASTOR")
...
At each user defined time step, the `rsfCASTOR` module calculates the resource selection function for the user defined species, population and season.
...
* layer := the name of the raster stored in a psql database or the name of the updated variable in `castordb`. 
...
* type :=  declares the type of dynamic variable. A value in this column is needed if the static column = N. The following types are used in the model: 
    ...
    + UP = 'Updatable' in `castordb`, meaning the model 'calls' a variable (e.g., forest stand age) 
...
* sql := the `WHERE` clause of a sql statement describing the RSQLite `castordb` variables used in the dynamic layer, or used to call the 'static' variable from the postgres castordb 
...
The following objects are instantiated via rsfCASTOR:

* pts:= A data.table of X,Y locations - used to find distances; joined to the rsfCovar data.table (below)

* rsfCovar:= A data.table of covariates used to calculate the RSF. Includes the pixelid from the provincial raster so that covariate locations can be spatially identified and is joined to the 'pts' data. This could be uploaded into castordb? Not implemented.
...
* rsf:= A table within the `castordb` RSQLite database that stores the RSF scores for each pixelid.    

# Links to other modules

Requires dataCASTOR to instantiate the `castordb` RSQLite database.
