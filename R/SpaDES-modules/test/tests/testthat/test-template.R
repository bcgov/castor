...
#    `functionName` is a function's name in your module (e.g., `testEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/klochhea/clus/R/SpaDES-modules/test/tests/testthat`).
...
test_that("test Event1 and Event2.", {
  ...
  if (exists("testEvent1", envir = .GlobalEnv)) {
    simOutput <- testEvent1(mySim)
  } else {
  ...
  if (exists("testEvent2", envir = .GlobalEnv)) {
    simOutput <- testEvent2(mySim)
  } else {
