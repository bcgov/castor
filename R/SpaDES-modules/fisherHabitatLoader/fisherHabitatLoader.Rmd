...
## Overview
This module is used to create fisher habitat data that can be loaded into the FLEX2 fisher agent based model. The fisher habitat data is created from forestryCLUS, which updates forest stand attributes over time, and is outputed as a 'stack' of rasters stored in a .tif format. 

## Usage
There are no parameters within this module. It relies on parameters from dataLoaderCLUS (database and boundary info) and growingStockCLUS (time interval). It is meant to be run with forestryCLUS, so that as forest stands are harvested and grow, stand characteristics change and subsequently fisher habitat changes. 
...
scenario = data.table (name = "test",
                       description = "Testing fisher habitat loader.")
