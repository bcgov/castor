...
#    `functionName` is a function's name in your module (e.g., `survivalCLUSEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Work/caribou/clus_github/R/SpaDES-modules/survivalCLUS/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("survivalCLUS")
  path <- list(modulePath = "C:/Work/caribou/clus_github/R/SpaDES-modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    survivalCLUS = list(.saveInitialTime = NA)
  )
  ...
  if (exists("survivalCLUSEvent1", envir = .GlobalEnv)) {
    simOutput <- survivalCLUSEvent1(mySim)
  } else {
  ...
  if (exists("survivalCLUSEvent2", envir = .GlobalEnv)) {
    simOutput <- survivalCLUSEvent2(mySim)
  } else {
