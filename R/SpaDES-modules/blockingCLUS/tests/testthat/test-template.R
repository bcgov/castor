...
#    `functionName` is a function's name in your module (e.g., `blockingCLUSEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/KLOCHHEA/clus/R/SpaDES/modules/blockingCLUS/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("blockingCLUS")
  ...
  parameters <- list(
    ...
    blockingCLUS = list(.saveInitialTime = NA)
  )
  ...
  if (exists("blockingCLUSEvent1", envir = .GlobalEnv)) {
    simOutput <- blockingCLUSEvent1(mySim)
  } else {
  ...
  if (exists("blockingCLUSEvent2", envir = .GlobalEnv)) {
    simOutput <- blockingCLUSEvent2(mySim)
  } else {
