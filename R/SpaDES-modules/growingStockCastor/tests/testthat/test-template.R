...
#    `functionName` is a function's name in your module (e.g., `growingStockCLUSEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/KLOCHHEA/clus/R/SpaDES-modules/growingStockCLUS/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("growingStockCLUS")
  ...
  parameters <- list(
    ...
    growingStockCLUS = list(.saveInitialTime = NA)
  )
  ...
  if (exists("growingStockCLUSEvent1", envir = .GlobalEnv)) {
    simOutput <- growingStockCLUSEvent1(mySim)
  } else {
  ...
  if (exists("growingStockCLUSEvent2", envir = .GlobalEnv)) {
    simOutput <- growingStockCLUSEvent2(mySim)
  } else {
