...
#    `functionName` is a function's name in your module (e.g., `dataLoaderCLUSEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/KLOCHHEA/clus/R/SpaDES-modules/dataLoaderCLUS/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("dataLoaderCLUS")
  ...
  parameters <- list(
    ...
    dataLoaderCLUS = list(.saveInitialTime = NA)
  )
  ...
  if (exists("dataLoaderCLUSEvent1", envir = .GlobalEnv)) {
    simOutput <- dataLoaderCLUSEvent1(mySim)
  } else {
  ...
  if (exists("dataLoaderCLUSEvent2", envir = .GlobalEnv)) {
    simOutput <- dataLoaderCLUSEvent2(mySim)
  } else {
