...
Init <- function (sim) { # this function identifies the GBPUs in the 'study area' creates the survival rate table, calculates survival rate at time = 0, and saves the survival table in the clusdb
  ...
  sim$tableGrizzSurvivalReport <- data.table (dbGetQuery (sim$clusdb, "SELECT SUM (CASE WHEN roadyear IS NOT NULL THEN 1 ELSE 0 END) AS total_roaded, COUNT(*) AS total_area, gbpu_name FROM pixels WHERE gbpu_name IS NOT NULL GROUP BY gbpu_name;"))
