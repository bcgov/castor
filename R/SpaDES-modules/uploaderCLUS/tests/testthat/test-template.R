...
#    `functionName` is a function's name in your module (e.g., `uploaderCLUSEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/klochhea/clus/R/SpaDES-modules/uploaderCLUS/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("uploaderCLUS")
  ...
  parameters <- list(
    ...
    uploaderCLUS = list(.saveInitialTime = NA)
  )
  ...
  if (exists("uploaderCLUSEvent1", envir = .GlobalEnv)) {
    simOutput <- uploaderCLUSEvent1(mySim)
  } else {
  ...
  if (exists("uploaderCLUSEvent2", envir = .GlobalEnv)) {
    simOutput <- uploaderCLUSEvent2(mySim)
  } else {
