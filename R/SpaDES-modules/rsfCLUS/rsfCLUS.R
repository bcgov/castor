...
rsfCLUS.Init <- function(sim) {
  ...
  #Create a concatenated variable 'rsf' that will report the population and season
  ...
  #Loop for getting all the rsf data into the rsf table
  for(layer_name in static_list){
    ...
    layer<-RASTER_CLIP2(srcRaster= layer_name, clipper=P(sim, "dataLoaderCLUS", "nameBoundaryFile"), geom= P(sim, "dataLoaderCLUS", "nameBoundaryGeom"), where_clause =  paste0(P(sim, "dataLoaderCLUS", "nameBoundaryColumn"), " in (''", P(sim, "dataLoaderCLUS", "nameBoundary"),"'')"), conn=conn)
    dbDisconnect(conn)
  ...
  #Set the glm objects for each of the rsf population and season 
  rsf_list<-lapply(as.list(unique(rsf_model_coeff[,"rsf"])$rsf), function(x) {
    rsf_model_coeff[rsf==x, c("beta", "layer")]
  })
  sim$rsfGLM<-lapply(rsf_list, getglmobj)
  
  return(invisible(sim))
}

updateRSFTable<-function(sim){
  dt_list<-unique(rsf_model_coeff[static == 'N' & type == 'DT'])[,c("layer", "sql")]
  if(length(dt_list)>0){
    #TODO:loop through the dt layers that need updating
    dt_layers<-getDistanceToLayers(dt_list)
  }
...
getDistanceToLayers<-function(sim){ #this function takes a sql statement and returns the distance to the result set generated by the sql
  dt_select<-data.table(dbGetQuery(sim$clusdb, paste0("SELECT pixelid FROM pixels WHERE ", sql)))
  dt_select[,field :=0]
  sim$pts<-merge(sim$pts, dt_select, by = 'pixelid', all.x =TRUE)
  test<-RANN::nn2(sim$pts[field==0, 2:3], sim$pts[is.na(field), 2:3], k = 1)
  sim$pts<-pts[is.na(field), dist:=test$nn.dists] #assign the distances
  sim$pts[is.na(dist),]<-0 #those that are selected assign 0
  
  #----Plotting the raster---
  #distRas<-sim$ras 
  #distRas[]<-sim$pts[,'field']
  #distRas[is.na(distRas[])]<-test$nn.dists
  #--------------------------
  
  distTo<-sim$pts[,dist]
  sim$pts[,field:=NULL] #drop the field
  sim$pts[,dist:=NULL] #drop the field
  
  return(invisible(sim))
}

getglmobj <-function(parm_list){
  #Create a fake data.table with 30 vars
