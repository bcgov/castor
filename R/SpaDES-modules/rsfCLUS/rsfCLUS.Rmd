...
rsf_model_coeff<-data.table(population = "DU6", 
                            season ="EW",
                            static = c('Y','Y','Y','Y','N','N','N'),
                            beta = c(-2.562923161, -0.017756076, -0.018701324, -0.022060543, -0.054817853,  -0.154907815, -0.103495695),
                            layer=c('int', "rast.bc_ha_slope", "rast.dt_waterbodies", "rast.dt_watercourses",'cut_1_4','cut_5_10','cut_gt_10'),
                            sql=c('','','','', 'blockid > 0 AND age BETWEEN 1 to 4','blockid > 0 AND age BETWEEN 5 to 10', 'blockid > 0 AND age > 10'),
                            type =c('','','','','DT', 'DT','DT'))

objects <- list(rsf_model_coeffs=rsf_model_coeffs)
paths <- list(
  cachePath = cacheDir,
