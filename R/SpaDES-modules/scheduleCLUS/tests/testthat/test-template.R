...
#    `functionName` is a function's name in your module (e.g., `scheduleCLUSEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/KLOCHHEA/clus/R/SpaDES-modules/scheduleCLUS/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("scheduleCLUS")
  ...
  parameters <- list(
    ...
    scheduleCLUS = list(.saveInitialTime = NA)
  )
  ...
  if (exists("scheduleCLUSEvent1", envir = .GlobalEnv)) {
    simOutput <- scheduleCLUSEvent1(mySim)
  } else {
  ...
  if (exists("scheduleCLUSEvent2", envir = .GlobalEnv)) {
    simOutput <- scheduleCLUSEvent2(mySim)
  } else {
