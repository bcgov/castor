...
The dataCastor preps the database connectivitiy and other objects required to run Castor. A SQLite database is stored in memory called castordb. This database can be saved to file using `saveCastorDB = TRUE` or an existing castordb can be used by specifying the path with `useCastorDB = ".../_castordb.sqlite"`
...
parameters <-  list(
  ...
  dataCastor = list(saveCastorDB = TRUE,
                     randomLandscape = list(100,100,0 + 171000, 100*100 + 171000, 0 + 834000, 100*100+ 834000),
                     randomLandscapeZoneConstraint = 
                       data.table(variable = 'age', threshold = 140, type = 'ge', percentage = 10)
                    )
  )
