...
defineModule(sim, list(
  ...
  parameters = rbind(
    ...
    defineParameter("rasterStack", "character", paste0 (here::here(), "/R/scenarios/test_flex2/test_Williams_Lake_TSA_fisher_habitat.tif"), NA, NA,
                    "Directory where the fisher habitat raster .tif is stored. Used as habitat input to this module."), # create a default somewhere??
...
doEvent.FLEX2 = function (sim, eventTime, eventType) {
  switch(
    ...
    init = {
      sim <- Init (sim)
      sim <- saveAgents (sim)
      sim <- scheduleEvent (sim, time(sim) + P(sim, "timeInterval", "FLEX2"), "FLEX2", "runevents", 19)
      sim <- scheduleEvent (sim, time(sim) + P(sim, "timeInterval", "FLEX2"), "FLEX2", "save", 20)
    },
    # interpolatehabitat = {
    # 
...
Init <- function(sim) {
  ...
  sim$raster.stack <- terra::rast (rasterStack)
  ...
  # dennign habitat
    # get the id for the init denning raster
  ras.den.init.id <- grepl ("init", names (sim$raster.stack)) & grepl ("denning", names (sim$raster.stack))
    # select the init denning raster
  ras.den.init <- sim$raster.stack [[ras.den.init.id]]
    # identify the pixels where there is denning habitat
  ras.den.ids <- ras.den.init * sim$pix.rast
  den.pix <- as.data.table (ras.den.ids [])
  colnames (den.pix) <- "pixelid"
  den.pix <- den.pix [pixelid > 0, ]
  ...
  ras.fisher.pop <- sim$raster.stack [["ras_fisher_pop"]]
  
  ras.fisher.pop.pixid <- ras.fisher.pop * sim$pix.rast
  
  
  fisher.pop.pix <- as.data.table (ras.fisher.pop.pixid [])
  colnames (fisher.pop) <- "pixelid"
  fisher.pop.pix <- fisher.pop.pix [pixelid > 0, ]
...
saveAgents <- function (sim) {
  ...
  new.agents.save <- data.table (n_f_adult = as.numeric (nrow (sim$agents [sex == "F" & age > 1 & !is.na (d2_score), ]) / P(sim, "iterations", "FLEX2")), 
                                 n_f_juv = as.numeric (nrow (sim$agents [sex == "F" & age == 1 & !is.na (d2_score), ]) / P(sim, "iterations", "FLEX2")), 
                                 n_f_disperse = as.numeric (nrow (sim$agents [sex == "F" & age > 0 & !is.na (d2_score), ]) / P(sim, "iterations", "FLEX2")), 
                                 mean_age_f = as.numeric (mean (c (sim$agents [sex == "F", age]) / P(sim, "iterations", "FLEX2"))), 
                                 sd_age_f = as.numeric (sd (c (sim$agents [sex == "F", age]) / P(sim, "iterations", "FLEX2"))), 
  ...
  ras.territories.update [sim$territories$pixelid] <- 1 / P(sim, "iterations", "FLEX2")
