...
#    `functionName` is a function's name in your module (e.g., `roadCLUSEvent1`).
# 2. Copy this file to the tests folder (i.e., `H:/Documents/SpaDESintro/modules/roadCLUS/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("roadCLUS")
  ...
  parameters <- list(
    ...
    roadCLUS = list(.saveInitialTime = NA)
  )
  ...
  if (exists("roadCLUSEvent1", envir = .GlobalEnv)) {
    simOutput <- roadCLUSEvent1(mySim)
  } else {
  ...
  if (exists("roadCLUSEvent2", envir = .GlobalEnv)) {
    simOutput <- roadCLUSEvent2(mySim)
  } else {
