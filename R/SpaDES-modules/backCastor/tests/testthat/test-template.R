...
#    `functionName` is a function's name in your module (e.g., `cutblockSeqPrepCLUSEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/KLOCHHEA/clus/R/SpaDES-modules/cutblockSeqPrepCLUS/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("cutblockSeqPrepCLUS")
  ...
  parameters <- list(
    ...
    cutblockSeqPrepCLUS = list(.saveInitialTime = NA)
  )
  ...
  if (exists("cutblockSeqPrepCLUSEvent1", envir = .GlobalEnv)) {
    simOutput <- cutblockSeqPrepCLUSEvent1(mySim)
  } else {
  ...
  if (exists("cutblockSeqPrepCLUSEvent2", envir = .GlobalEnv)) {
    simOutput <- cutblockSeqPrepCLUSEvent2(mySim)
  } else {
