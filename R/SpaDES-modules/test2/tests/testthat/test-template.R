...
#    `functionName` is a function's name in your module (e.g., `test2Event1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/klochhea/clus/R/SpaDES-modules/test2/tests/testthat`).
...
test_that("test Event1 and Event2.", {
  ...
  if (exists("test2Event1", envir = .GlobalEnv)) {
    simOutput <- test2Event1(mySim)
  } else {
  ...
  if (exists("test2Event2", envir = .GlobalEnv)) {
    simOutput <- test2Event2(mySim)
  } else {
