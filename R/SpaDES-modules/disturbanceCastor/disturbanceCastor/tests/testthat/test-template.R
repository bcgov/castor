...
#    `functionName` is a function's name in your module (e.g., `disturbanceCalcCLUSEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/KLOCHHEA/clus/R/SpaDES-modules/disturbanceCalcCLUS/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("disturbanceCalcCLUS")
  ...
  parameters <- list(
    ...
    disturbanceCalcCLUS = list(.saveInitialTime = NA)
  )
  ...
  if (exists("disturbanceCalcCLUSEvent1", envir = .GlobalEnv)) {
    simOutput <- disturbanceCalcCLUSEvent1(mySim)
  } else {
  ...
  if (exists("disturbanceCalcCLUSEvent2", envir = .GlobalEnv)) {
    simOutput <- disturbanceCalcCLUSEvent2(mySim)
  } else {
