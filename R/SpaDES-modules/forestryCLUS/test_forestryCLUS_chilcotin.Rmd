...
parameters <- list(
  ...
  forestryCLUS = list(harvestBlockPriority = " salvage_vol DESC, age DESC", # query to prioritize blocks within 1km of disturbance, then 2.5km, then 5km, then everything >5km, then oldest first after that =  "CASE WHEN dist < 1000 THEN 3 WHEN dist < 2500 THEN 2 WHEN dist < 5000 THEN 1 ELSE 0 END DESC, age DESC"
...
scenario = data.table (name = "chilcotin_bau_dead_all",
                       description = "Business as usual (BAU). Adjacency = 0m. Priority queue = oldest first (live volume). Salvage dead volume in first 10 years. Minimum volume: Quesnel = 150 m3/year; Williams Lake = 110 m3/year. Even live harvest flow: Quesnel Matrix = 190,000m3/year; Quesnel HEWSR = 0 m3/year; Quesnel LESR = 7,000 m3/year; Quesnel LEWR = 85,000m3/year; Quesnel out = 2,100,000m3/year; Williams Lake HEWSR = 15,000m3/year; Williams Lake LESR = 70,000m3/year; Williams Lake LEWR = 140,000m3/year; Williams Lake Matrix = 140,000m3/year; Williams Lake out = 1,650,000m3/year. Dead volume targets in first two intervals (10 years) only: Quesnel Matrix = 95,000 m3/year; Quesnel HEWSR = 0 m3/year; Quesnel LESR = 3,500 m3/year; Quesnel LEWR = 42,500m3/year; Quesnel out = 1,050,000 m3/year; Williams Lake HEWSR = 7,500 m3/year; Williams Lake LESR = 35,000 m3/year; Williams Lake LEWR = 70,000 m3/year; Williams Lake Matrix = 70,000 m3/year; Williams Lake out = 825,000 m3/year.")



harvestFlow <- rbindlist(list(data.table (compartment ="Quesnel_TSA_HEWSR",
                              ...
                              data.table (compartment ="Quesnel_TSA_LESR",
                              ...
                              data.table (compartment ="Quesnel_TSA_LESR",partition = ' (salvage_vol > 150  and vol/(salvage_vol+vol) <= 0.5)  ', period = rep( seq (from = 1, to = 2, by = 1),1),flow = 17500,partition_type = 'dead'), # 3,500 m3/yr 
                              data.table (compartment ="Quesnel_TSA_LEWR",
                              ...
                              data.table (compartment ="Quesnel_TSA_LEWR",partition = ' (salvage_vol > 150  and vol/(salvage_vol+vol) <= 0.5 ) ', period = rep( seq (from = 1, to = 2, by = 1),1),  flow = 212500, partition_type = 'dead'), # 42,500 m3/yr 
                              data.table (compartment ="Quesnel_TSA_Matrix",
                                          partition = ' (vol > 150 and salvage_vol/(vol + salvage_vol) < 0.5)   ',
                                          period = rep( seq (from = 1, 
                                                           to = 40, 
                                                           by = 1),
                              ...
                              data.table (compartment ="Quesnel_TSA_Matrix",partition = ' (salvage_vol > 150  and vol/(salvage_vol+vol) <= 0.5) ', period = rep( seq (from = 1, to = 2,by = 1),1),  flow = 475000, partition_type = 'dead'), #95,000 m3/yr 
                              data.table (compartment ="Quesnel_TSA_out",
                                         partition = ' (vol > 150 and salvage_vol/(vol + salvage_vol) < 0.5)   ', 
                                          period = rep( seq (from = 1, 
                                                           to = 40, 
                                                           by = 1),
                              ...
                              data.table (compartment ="Quesnel_TSA_out", partition = ' (salvage_vol > 150  and vol/(salvage_vol+vol) <= 0.5) ',  period = rep( seq (from = 1,to = 2, by = 1), 1),  flow = 5250000, partition_type = 'dead'), # 1,050,000 m3/yr 
                              data.table (compartment ="Williams_Lake_TSA_HEWSR",
                                          partition = ' (vol > 110 and salvage_vol/(vol + salvage_vol) < 0.5)   ',
                                          period = rep( seq (from = 1, 
                                                           to = 40, 
                                                           by = 1),
                                                     1), 
                                          flow = 75000,
                                          partition_type = 'live'), # 15,000 m3/yr 
                              data.table (compartment ="Williams_Lake_TSA_HEWSR",partition = ' (salvage_vol > 110  and vol/(salvage_vol+vol) <= 0.5)  ', period = rep( seq (from = 1,to = 2, by = 1), 1), flow = 37500,partition_type = 'dead'), # 7,500 m3/yr 
                              data.table (compartment ="Williams_Lake_TSA_LESR",
                                          partition = ' (vol > 110 and salvage_vol/(vol + salvage_vol) < 0.5)   ',
                                          period = rep( seq (from = 1, 
                                                           to = 40, 
                                                           by = 1),
                                                     1), 
                                          flow = 350000,
                                          partition_type = 'live'), # 70,000 m3/yr 
                              data.table (compartment ="Williams_Lake_TSA_LESR",partition = ' (salvage_vol > 110  and vol/(salvage_vol+vol) <= 0.5)  ', period = rep( seq (from = 1,to = 2,by = 1),1),  flow = 175000,partition_type = 'dead'), # 35,000 m3/yr 
                              data.table (compartment ="Williams_Lake_TSA_LEWR",
                                          partition = ' (vol > 110 and salvage_vol/(vol + salvage_vol) < 0.5)   ',
                                          period = rep( seq (from = 1, 
                                                           to = 40, 
                                                           by = 1),
                             ...
                             data.table (compartment ="Williams_Lake_TSA_LEWR", partition = ' (salvage_vol > 110  and vol/(salvage_vol+vol) <= 0.5) ', period = rep( seq (from = 1, to = 2,by = 1), 1), flow = 350000, partition_type = 'dead'), # 70,000 m3/yr 
                              data.table (compartment ="Williams_Lake_TSA_Matrix",
                                         partition = ' (vol > 110 and salvage_vol/(vol + salvage_vol) < 0.5)   ', 
                                          period = rep( seq (from = 1, 
                                                           to = 40, 
                                                           by = 1),
                                          ...
                                          flow = 700000,
                                          partition_type = 'live'), # 140,000 m3/yr 
                              data.table (compartment ="Williams_Lake_TSA_Matrix",partition = ' (salvage_vol > 110  and vol/(salvage_vol+vol) <= 0.5) ',  period = rep( seq (from = 1,  to = 2, by = 1), 1),  flow = 350000, partition_type = 'dead'), # 70,000 m3/yr 
                              data.table (compartment ="Williams_Lake_TSA_out",
                                         partition = ' (vol > 110 and salvage_vol/(vol + salvage_vol) < 0.5)   ',
                                          period = rep( seq (from = 1, 
                                                           to = 40, 
                                                           by = 1),
                                                      1), 
                                          flow = 8250000,
                                          partition_type = 'live'), # 1,650,000 m3/yr 
                              data.table (compartment ="Williams_Lake_TSA_out", partition = ' (salvage_vol > 110  and vol/(salvage_vol+vol) <= 0.5) ',  period = rep( seq (from = 1, to = 2,  by = 1), 1), flow = 4125000, partition_type = 'dead') # 825,000 m3/yr 
                                         ))
