...
- used oldest first - no details in determination
...
Effective January 31, 2012, the new AAC for TFL 41 is 128 000 cubic metres.


- First, I tested a harvest flow of 150,000m^3^/year (750,000m^3^/5-year); way too high; there is mention of issues with site index in determination; might be cause of why? 50,000; 30,000
...
clusdb <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/TFL41_clusdb.sqlite")) # connext to clusdb -- set the locations of the sqlite
...
parameters <- list(
  ...
  dataLoaderCLUS = list(   dbName='clus',
                           save_clusdb = FALSE,
                           sqlite_dbname = "TFL41",
                           useCLUSdb = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/TFL41_clusdb.sqlite"),
                           #Study Area
  ...
  uploaderCLUS = list(aoiName = 'tfl41', # name of the schema that gets uplaoded to postgres
...
# scenario = data.table (name = "tfl41_bau",
#                        description = "Business-as-usual (BAU). Harvest flow 30,000 m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl41_du7_grp1_nh",
#                        description = "No harvest in group 1 (Itcha, Rainbows, Charlotte, Tweeds, and Telkwa) critical habitat that overlaps with the harvest unit (e.g., TSA or TFL). Harvest flow = 30,000m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl41_du7_grp1_he0d_mlewr12d",
#                        description = "No harvest in group 1 (Itcha, Rainbows, Charlotte, Tweeds, and Telkwa) HEWSR and LESR critical habitat; harvest allowed in LEWR and matrix critical habitat up to 12% disturbance (no buffer) in forested area. Harvest flow = 30,000m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl41_du7_grp1_hi_priority",
#                        description = "No harvest priority stands in high priority zones in group 1 (Itcha, Rainbows, Charlotte, Tweeds, and Telkwa) HEWSR and LESR critical habitat; harvest allowed in high priority zones in LEWR and matrix critical habitat up to 15% disturbance (no buffer) in forested area. Harvest allowed in low and medium priority zones. NOTE: all zones in this unit are high priority matrix. Harvest flow = 30,000m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl41_telkwa_nh",
#                        description = "No harvest in Telkwa critical habitat that overlaps with the harvest unit (e.g., TSA or TFL). Harvest flow = 30,000m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl41_telkwa_he0d_mlewr12d",
#                        description = "No harvest in Telkwa HEWSR and LESR critical habitat; harvest allowed in LEWR and matrix critical habitat up to 12% disturbance (no buffer) in forested area. Harvest flow = 30,000m3/yr. Adjacency = 3m. Oldest first.")

scenario = data.table (name = "tfl41_telkwa_hi_priority",
...
harvestFlow <- rbindlist(list(data.table(compartment ="TFL41",
                                         partition = ' vol > 299 ', 
                                         period = rep( seq (from = 1, # run the 
                                                    ...
                                                    1), 
                                         flow = 150000,
                                         partition_type = 'live') # 30,000m3/yr 
))
