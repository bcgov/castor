...
parameters <- list(
  ...
  dataLoaderCLUS = list( 
                         dbName='castor',
                           #saveCastorDB = TRUE,
                          # sqlite_dbname = "study_area",
                           useCastorDB = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/Revelstoke_TSA_clusdb.sqlite"),
                           #Study Area
                           ...
                           nameYieldsTransitionRaster = 'rast.thlb_2020', #'rast.bc_thlb2018' 
  ...
  forestryCLUS = list( harvestPriority = "age DESC, vol DESC",adjacencyConstraint = TRUE),
  #roadCLUS = list(roadMethod = 'snap', nameCostSurfaceRas = 'rast.rd_cost_surface', nameRoads =  'rast.crds_resource'),
...
modules <- list("dataLoaderCLUS", "growingStockCLUS", "blockingCLUS", "forestryCLUS", "yieldUncertaintyCLUS", "survivalCASTOR", "uploaderCLUS")
...
harvestFlow<- rbindlist(list(data.table(compartment ="Revelstoke_TSA",
                             ...
                             data.table(compartment ="Revelstoke_TSA",
                                     partition = ' (vol > 110) ',
                                     period = rep(seq(from = 3, to=10, by = 1),1), 
                                     flow = 186390,
                                     partition_type = 'live'), 
                             data.table(compartment ="Revelstoke_TSA",
                                     partition = ' (vol > 110) ',
                                     period = rep(seq(from = 11, to=20, by = 1),1), 
                                     flow = 165680,
                                     partition_type = 'live'),
                             data.table(compartment ="Revelstoke_TSA",
                                     partition = ' (vol > 110) ',
                                     period = rep(seq(from = 21, to=30, by = 1),1), 
                                     flow = 144970,
                                     partition_type = 'live'),
                             data.table(compartment ="Revelstoke_TSA",
                                     partition = ' (vol > 110) ',
                                     period = rep(seq(from = 31, to=40, by = 1),1), 
                                     flow = 135900,
                                     partition_type = 'live'),
                             data.table(compartment ="Revelstoke_TSA",
                                     partition = ' (vol > 110) ',
                                     period = rep(seq(from = 41, to=50, by = 1),1), 
                                     flow = 144970,
                                     partition_type = 'live'),
                              data.table(compartment ="Revelstoke_TSA",
                                     partition = ' (vol > 110) ',
                                     period = rep(seq(from = 51, to=60, by = 1),1), 
                                     flow = 165680,
                                     partition_type = 'live'),
                             data.table(compartment ="Revelstoke_TSA",
                                     partition = ' (vol > 110) ',
                                     period = rep(seq(from = 61, to=70, by = 1),1), 
                                     flow = 186390,
                                     partition_type = 'live'),
                             data.table(compartment ="Revelstoke_TSA",
                                     partition = ' (vol > 110) ',
                                     period = rep(seq(from = 71, to=80, by = 1),1), 
                                     flow = 207100,
                                     partition_type = 'live')
))
...
#outputs(mySim) <- data.frame(objectName = c("harvestReport", "growingStockReport", "tableSurvival"))
...
A SQLite db is required (output from dataloaderCLUS). A harvestFlow data.table object that includes the forest management unit (i.e., compartment, aka - 'supply block'), the partition from which the harvest flow applies (e.x., All dead pine); the period at which the flow applies and the amount of volume.
