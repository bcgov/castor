...
parameters <- list(
  ...
  rsfCLUS = list(calculateInterval = 1, criticalHabitatTable = "public.criticalhabitat")
                )

modules <- list("dataLoaderCLUS", "growingStockCLUS", "blockingCLUS", "forestryCLUS", "roadCLUS", "rsfCLUS")

rsf_model_coeff <- data.table(getTableQuery("SELECT * FROM rsf_model_coeff WHERE species = 'caribou' AND population = 'DU8' AND season = 'S'"))#see rsf_model_coeff.rmd 
...
harvestFlow<- data.table(compartment = c('24E','24E','24E','24E','24E','24E','24E','24E','24E','24E'),
                                   ...
                                   year = rep(seq(from = 2018, to=2027, by = 
                                                    1),1), 
                                   flow = 100000)
objects <- list(harvestFlow = harvestFlow, rsf_model_coeff=rsf_model_coeff , scenario = scenario
                )
paths <- list(
  cachePath = cacheDir,
