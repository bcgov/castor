...
parameters <- list(
  ...
  dataLoaderCLUS = list( 
                         dbName='clus',
                           #save_clusdb = TRUE,
                           useCLUSdb = paste0("C:/Users/KLOCHHEA/clus/R/SpaDES-modules/dataLoaderCLUS/Revelstoke_TSA_clusdb.sqlite"),
                           #Study Area
...
scenario = data.table(name="test_adj", description = "checking flow assumptions with adjacency constraint")

harvestFlow<- rbindlist(list(data.table(compartment ="Revelstoke_TSA",
                                     partition = ' vol > 110 ',
                                     year = rep(seq(from = 2018, to=2020, by = 1),1), 
                                     flow = 207100),
                             data.table(compartment ="Revelstoke_TSA",
                                     partition = ' vol > 110 ',
                                     year = rep(seq(from = 2021, to=2030, by = 1),1), 
                                     flow = 186390), 
                             data.table(compartment ="Revelstoke_TSA",
                                     partition = ' vol > 110 ',
                                     year = rep(seq(from = 2031, to=2040, by = 1),1), 
                                     flow = 165680),
                             data.table(compartment ="Revelstoke_TSA",
                                     partition = ' vol > 110 ',
                                     year = rep(seq(from = 2041, to=2050, by = 1),1), 
                                     flow = 144970),
                             data.table(compartment ="Revelstoke_TSA",
                                     partition = ' vol > 110 ',
                                     year = rep(seq(from = 2051, to=2060, by = 1),1), 
                                     flow = 135900),
                             data.table(compartment ="Revelstoke_TSA",
                                     partition = ' vol > 110 ',
                                     year = rep(seq(from = 2061, to=2070, by = 1),1), 
                                     flow = 144970),
                              data.table(compartment ="Revelstoke_TSA",
                                     partition = ' vol > 110 ',
                                     year = rep(seq(from = 2071, to=2080, by = 1),1), 
                                     flow = 165680),
                             data.table(compartment ="Revelstoke_TSA",
                                     partition = ' vol > 110 ',
                                     year = rep(seq(from = 2081, to=2090, by = 1),1), 
                                     flow = 186390),
                             data.table(compartment ="Revelstoke_TSA",
                                     partition = ' vol > 110 ',
                                     year = rep(seq(from = 2091, to=2118, by = 1),1), 
                                     flow = 207100)
))
...
objects <- list(harvestFlow = harvestFlow, patchSizeDist=patchSizeDist, scenario = scenario, calb_ymodel=calb_ymodel)
