...
# Cascadia TSA Kootenay Block Scenarios
...
no harvest in Central Selkirks caribou critical habtiat; to support Central Selkirks herd planning, we run a sceanrio with no harvest in the  Central Selkirks HEWSR and Matrix; assesses teh potential maximum impact of protecting Central Selkirks  only, assuming other herds unprotected
...
# Cascadia TSA Kootenay Block Parameters 
...
## 'harvestFlow'
from 2020 determination: maximum non-declining even-flow was 76,000m^3^/year for 20 years

- First, I tested a harvest flow of 76,000m^3^/year (380,000m^3^/5-year); 100,000, 95,000

- stable flow at 95,000m^3^/year 
...
For cutblock adjacency, in analysis report, allowed maximum 25% height ≤ 2.5 m THLB by landscape unit; I used 3 m adjacncey; thus is a bit conservative; varying this form 20% to 33% had minimal imapct on timber supply

```{r, zone_update}
library (DBI)
# STEP 1: Connect to the clusdb.sqlite database for the AOI
clusdb <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/Cascadia_TKO_TSA_clusdb.sqlite")) # connext to clusdb -- set the locations of the sqlite
...
dbExecute(clusdb, "UPDATE zoneconstraints SET type = 'ge' where reference_zone = 'rast.zone_du9_scenarios' AND zoneid = 62") 
dbExecute(clusdb, "UPDATE zoneconstraints SET percentage = 65 where reference_zone = 'rast.zone_du9_scenarios' AND zoneid = 62")
dbExecute(clusdb, "UPDATE zoneconstraints SET type = '' where reference_zone = 'rast.zone_du9_scenarios' AND zoneid = 33")
...
dbExecute(clusdb, "UPDATE zoneconstraints SET type = 'nh' where reference_zone = 'rast.zone_smc_zones_20210315' AND zoneid in (15,16,17)")
dbDisconnect(clusdb)
```
...
parameters <- list(
  ...
  dataLoaderCLUS = list(   dbName='clus',
                           save_clusdb = FALSE,
                           sqlite_dbname = "Cascadia_TKO_TSA",
                           useCLUSdb = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/Cascadia_TKO_TSA_clusdb.sqlite"),
                           #Study Area
                           nameBoundaryFile = "tsa_aac_bounds",
                           nameBoundaryColumn = "tsa_name",
                           nameBoundary = "Cascadia_TKO_TSA",
  ...
  disturbanceCalcCLUS = list(calculateInterval =  1, # should be 1 if using constraints on 'dist' (disturbance) 
                             ...
                             recovery = 40),
  uploaderCLUS = list(aoiName = 'cascadia_kootenay_block', # name of the schema that gets uplaoded to postgres
...
# scenario = data.table (name = "cascadiakb_bau",
#                        description = "Business-as-usual (BAU). Adjacency = 3m. Oldest first. Harvest flow = 95,000m3/year.")
# scenario = data.table (name = "cascadiakb_centselk_nh",
#                        description = "No harvest in all Central Selkirks critical habitat. Adjacency = 3m. Oldest first. Harvest flow = 95,000m3/year.")
# scenario = data.table (name = "cascadiakb_centselk_ch_he0d_m12d",
 #                       description = "No harvest in Central Selkirks high elevation critical habitat, maximum 12% disturbance  in Central Selkirks matrix critical habitat. Adjacency = 3m. Oldest first. Harvest flow = 95,000m3/year.")
 scenario = data.table (name = "cascadiakb_centselk_scen3a",
                        description = "NNo harvest in old forest, recruitment forest and buffered Central Selkirks core critical habitat areas, as defined by Bevan Ernst, maximum 15% disturbance (unbuffered) in Central Selkirks matrix habitat areas. Adjacency = 3m. Oldest first. Harvest flow = 95,000m3/year.")

#scenario = data.table (name = "cascadiakb_centselk_scen3b",
#                       description = "No harvest in old forest and recruitment forest Central Selkirks core critical habitat areas, as defined by Bevan Ernst, maximum 35% buffered disturbance (unbuffered) in Central Selkirks matrix habitat areas. Adjacency = 3m. Oldest first. Harvest flow = 95,000m3/year.")
...
harvestFlow <- rbindlist(list(data.table(compartment ="Cascadia_TKO_TSA",
                                     partition = ' vol > 150 ', 
                                     period = rep( seq (from = 1, # run the 
                                                ...
                                                1), 
                                     flow = 475000,
                                     partition_type = 'live') #95,000m3/yr 
))
