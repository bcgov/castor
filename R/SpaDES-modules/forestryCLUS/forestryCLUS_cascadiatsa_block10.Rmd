...
# Cascadia TSA - Block 10 Scenarios
...
# Cascadia TSA - Block 10 TSA Parameters 
...
## 'harvestFlow'
On January 23, 2020 the allowable annual cut for the Skeena portion of the Cascadia TSA, which includes blocks 9 to 11, was set at 126,070 cubic metres/year. 

-in TSR base case, used 250m3 as min volume for harvest, which is what i used here

- I started with 100,000, way too high; there may be a THLB or inventory issue here; tried 50,000, 25,000, 20,000

#UPDATE SCENARIO
```{r, zone_update}
#STEP 1: Connect to the clusdb.sqlite database for the AOI
clusdb <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/Cascadia_TSA_Block10_clusdb.sqlite")) # connect to clusdb -- set the locations of the sqlite
...
parameters <- list(
  ...
  dataLoaderCLUS = list( 
                           dbName='clus',
                           save_clusdb = TRUE,
                           sqlite_dbname = "Cascadia_TSA_Block10",
                           useCLUSdb = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/Cascadia_TSA_Block10_clusdb.sqlite"),
                           #Study Area
                           nameBoundaryFile = "tsa_nw_coast_no_tfl",
                           nameBoundaryColumn = "tsa_name",
                           nameBoundary = "Cascadia_TSA_Block10",
  ...
  uploaderCLUS = list(aoiName = 'cascadia_blk10_tsa', # name of the schema that gets uploaded to postgres
...
# scenario = data.table (name = "cascadia_blk10_bau",
#                        description = "Business-as-usual case. Harvest flow = 20,000m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "cascadia_blk10_du7_grp1_nh",
#                        description = "No harvest in group 1 (Itcha, Rainbows, Charlotte, Tweeds, and Telkwa) critical habitat that overlaps with the harvest unit (e.g., TSA or TFL). Harvest flow = 20,000m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "cascadia_blk10_du7_grp1_he0d_mlewr12d",
#                        description = "No harvest in group 1 (Itcha, Rainbows, Charlotte, Tweeds, and Telkwa) HEWSR and LESR critical habitat; harvest allowed in LEWR and matrix critical habitat up to 12% disturbance (no buffer) in forested area. Harvest flow = 20,000m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "cascadia_blk10_du7_grp1_hi_priority",
#                        description = "No harvest priority stands in high priority zones in group 1 (Itcha, Rainbows, Charlotte, Tweeds, and Telkwa) HEWSR and LESR critical habitat; harvest allowed in high priority zones in LEWR and matrix critical habitat up to 15% disturbance (no buffer) in forested area. Harvest allowed in low and medium priority zones. NOTE: all zones in this unit are high priority matrix. Harvest flow = 20,000m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "cascadia_blk10_telkwa_nh",
#                        description = "No harvest in Telkwa critical habitat that overlaps with the harvest unit (e.g., TSA or TFL). Harvest flow = 20,000m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "cascadia_blk10_telkwa_he0d_mlewr12d",
#                        description = "No harvest in Telkwa HEWSR and LESR critical habitat; harvest allowed in LEWR and matrix critical habitat up to 12% disturbance (no buffer) in forested area. Harvest flow = 20,000m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "cascadia_blk10_telkwa_hi_priority",
...
harvestFlow <- rbindlist(list(data.table(compartment ="Cascadia_TSA_Block10",
                                     partition = ' vol > 249 ', 
                                     period = rep( seq (from = 1, # run the 
                                                ...
                                                1), 
                                     flow = 100000,
                                     partition_type = 'live') #20,000m3/yr
))
