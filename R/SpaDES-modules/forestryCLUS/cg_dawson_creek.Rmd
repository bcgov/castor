...
In some situations, scenarios require the zone constraints to be updated in an area of interest (AOI), rather than re-building the clusdb with the addition of a new zone. Note this 'changes' the clusdb permanently and thus may require further updating for new scenarios. Updating clusdb can accomplished with the steps below. 
```{r, zone_update}
#STEP 1: Connect to the clusdb.sqlite database for the AOI
clusdb <- dbConnect(RSQLite::SQLite(), dbname = "C:/Users/klochhea/clus/R/SpaDES-modules/dataLoaderCLUS/TFL48_clusdb.sqlite" ) # connext to clusdb -- set the locations of the sqlite
...
### Remaining CLUS parameters and model run
```{r module_usage}
...
scenario = data.table(name="cg_partner_agreement_nh", description = "Partnership Agreement - projecting no harvesting in management zones outlined in partnership agreement. Adjacency constraint > 3 m.")

harvestFlow<- data.table(compartment ="Dawson_Creek_TSA",
                                     partition = ' vol > 110 ',
                                     year = rep(seq(from = 2018, to=2218, by = 5),1),  flow = 6850000)
