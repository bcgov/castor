...
clusdb <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/Mackenzie_TSA_clusdb.sqlite")) # connext to clusdb -- set the locations of the sqlite
...
- focus on minimizing 'disturabnce; for cariobu, then older, higher vlomue stands
...
- no discussion of this in PDP or determination that I coudl find
...
- In the main portion of the TSA, first, I tested a harvest flow of 1,500,000m^3^/year (7,500,000m^3^/5-year); was acheivle, so tried 1.75 M m3, 1.65 M, 1.70 M m3; slight fall down in last 10 years at 1.7, but kept at this number
...
- in TSR, the annual average was 200, allowign for <200, but I used 150 m3/ha as minimum harvest volume, and assesed what the average was *a priori* to make sure I was at ~200; foudn that with a 1.7 Mm3 flow, for 23 of 40 intervals average <200, 17 of 40 years average less than 195, 13 of 40 years less than 190, and 7 of 40 less than 185; so I am overesriamtign use of low voluem stands much of the time, but on average typiecally not more than by 15m3/ha; lowest average was 180 m3; so I kept as is (any major concerns with this?)

For cutblock adjacency, I used 3 m adjacncey; no real mention fo this in report
...
parameters <- list(
  ...
  disturbanceCalcCLUS = list(calculateInterval = 5, 
                             ...
                             permDisturbanceRaster = "rast.mine_ag_wind_rail",
                             recovery = 40),
  uploaderCLUS = list(aoiName = 'mackenzie_sw_tsa', # name of the schema that gets uplaoded to postgres
                      dbInfo  = list(keyring::key_get("vmdbhost", keyring="postgreSQL"), 
                                     keyring::key_get("vmdbuser", keyring="postgreSQL"), 
                                     keyring::key_get("vmdbpass", keyring="postgreSQL"),  
                                     keyring::key_get("vmdbname", keyring="postgreSQL"))
...
scenario = data.table (name = "mackenzie_bau",
                       description = "Business-as-usual case; sustainable flow = m3/yr .Adjacency was set to 3m.")
