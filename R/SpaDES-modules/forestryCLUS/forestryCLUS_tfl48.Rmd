...
- First, I tested a harvest flow of 1,000,000m^3^/year (5,000,000m^3^/5-year); then 750,000, 600,000, 500,000, 450,000, 400,000, 350,000, 345,000
 - also identified non-declining harvest flow for BAU plus the B2 and B3 areas (new Klinse-za/Twin Sisters Park) of the        partnership agreement areas as protected areas; at 325,000 m^3^/yr
...
# Below sets proposed maximum NRV age targets for LUO for critical habitat overlapping this unit; age threshodl <30 
## Kennedy Siding
...
dbExecute(clusdb, "UPDATE zoneconstraints SET type = 'nh' where reference_zone = 'rast.zone_du7_scenario_20210330' AND (zoneid = 13) OR (zoneid = 19) OR (zoneid = 22) OR (zoneid = 23) OR (zoneid = 24)") 
...
# connect to the db
clusdb <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/TFL48_clusdb.sqlite"))
...
reference_zone = 'rast.zone_cond_bc_moberly_crithab'") 
#  rast.zone_cond_bc_quintette_crithab, rast.zone_cond_bc_burnt_pine_crithab, rast.zone_cond_bc_moberly_crithab, rast.zone_cond_bc_kennedy_siding_crithab
 
 
# for each zone that you want to change:
## calc the total area of the zone 
count <- dbGetQuery(clusdb, "SELECT count() from pixels where zone22 = 4") # rast.zone_cond_bc_quintette_crithab, rast.zone_cond_bc_burnt_pine_crithab, rast.zone_cond_bc_moberly_crithab, rast.zone_cond_bc_kennedy_siding_crithab // 0, 4, 5

## update to the total area in the constraint table for the zone
dbExecute(clusdb, paste0("UPDATE zoneConstraints SET t_area = ", count, " where reference_zone = 'rast.zone_cond_bc_moberly_crithab' AND (zoneid = 4)")) # rast.zone_cond_bc_quintette_crithab, rast.zone_cond_bc_burnt_pine_crithab, rast.zone_cond_bc_moberly_crithab, rast.zone_cond_bc_kennedy_siding_crithab  // 0, 4, 5 

# to check it
zoneconstraints<-dbGetQuery(clusdb, "SELECT * from zoneconstraints where reference_zone = 'rast.zone_cond_bc_moberly_crithab'")                  
...
#                        description = "Business-as-usual (BAU). Harvest flow = 345,000 m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl48_du7_grp2_nh",
#                        description = "No harvest in group 2 (Graham) critical habitat that overlaps with the harvest unit (e.g., TSA or TFL). Harvest flow = 345,000 m3/yr.  Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl48_du7_grp2_he0d_mlewr12d",
#                        description = "No harvest in group 2 (Graham)  HEWSR and LESR critical habitat; harvest allowed in LEWR and matrix critical habitat up to 12% disturbance (no buffer) in forested area. Harvest flow = 345,000 m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl48_du7_grp2_hi_priority",
#                        description = "No harvest priority stands in high priority zones in group 1 (Itcha, Rainbows, Charlotte, Tweeds, and Telkwa) HEWSR and LESR critical habitat; harvest allowed in high priority zones in LEWR and matrix critical habitat up to 15% disturbance (no buffer) in forested area. Harvest allowed in low and medium priority zones. NOTE: all zones in this unit are high priority. Harvest flow = 345,000 m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl48_bau_newpark",
#                        description = "Business-as-usual case with Klinse-za/Twin Sisters park (B2 and B3 areas) from parntership agreement  added as no harvest. Harvest flow = 315,000 m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl48_centgrp_luo_age_le40",
#                        description = "Draft land use order early seral (age) thresholds for central group critical habitat, April 26, 2021. Harvest flow = 315,000 m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl48_centgrp_luo_age_le40_modNRV",
#                        description = "Draft land use order early seral (age) thresholds for central group critical habitat, April 26, 2021. Moderate natural range of variability (NRV) early seral targets. Harvest flow = 315,000 m3/yr. Adjacency = 3m. Oldest first.")


scenario = data.table (name = "tfl48_centgrp_luo_age_le40_maxNRV",
                       description = "Draft land use order early seral (age) thresholds for central group critical habitat, April 26, 2021. Maximum natural range of variability (NRV) early seral targets. Harvest flow = 315,000 m3/yr. Adjacency = 3m. Oldest first.")

# scenario = data.table (name = "tfl48_centgrp_luo_age_le40_maxNRV",
#                        description = "Draft land use order early seral (age) thresholds for central group critical habitat, April 26, 2021. Maximum natural range of variability (NRV) early seral targets. Harvest flow = 315,000 m3/yr. Adjacency = 3m. Oldest first.")

scenario = data.table (name = "tfl48_centgrp_agele30_maxnrv",
...
# scenario = data.table (name = "tfl48_luo_35p_25yo_250cm",
#                        description = "Central group proposed land use order; area less than 35% less than 25 years old or less than 2.5 m tall. Oldest first. Adjacency = 3. Harvest flow 345,000 m3/yr.")
# scenario = data.table (name = "tfl48_luo_35p_25yo",
#                        description = "Central group proposed land use order; area less than 35% less than 25 years old. Oldest first. Adjacency = 3. Harvest flow 345,000 m3/yr.")
# scenario = data.table (name = "tfl48_luo_35p_250cm",
#                        description = "Central group proposed land use order; area less than 35% less than 2.5 m tall. Oldest first. Adjacency = 3. Harvest flow 345,000 m3/yr.")
...
harvestFlow <- rbindlist(list(data.table(compartment ="TFL48",
                                         ...
                                         year = rep( seq (from = 2018, 
                                                          ...
                                                          by = 5),
                                                    1), 
                                         flow = 1575000) # 345,000m3/yr / 1725000 // 315,000m3/yr / 1575000
))
