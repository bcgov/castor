...
No harvest in Columbia North and Columbia South = rast.zone_cond_noharvest_columbia_north_crithab_or_herd,
                                                 rast.zone_cond_noharvest_columbia_south_crithab_or_herd
  
No harvest in Columbia North and Columbia South high elevation critical habitat, maximum 35% buffered disturbance (15% harvest) matrix critical habitat = rast.zone_cond_eccc_columbia_north_crithab_or_herd,
...
## 'harvestFlow'
from 2010 determination: the base case projected a stable harvest level of 88 000 cubic meters per year, which could be maintained for 100 years after which it increased to the long-term harvest level of 101 000 cubic meters per year

- First, I tested a harvest flow of 100,000m^3^/year (500,000m^3^/5-year); 75,000m^3^/year, 50,000m^3^/year, 40,000m^3^/year, 45,000m^3^/year
...
clusdb <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/TFL56_clusdb.sqlite")) # connext to clusdb -- set the locations of the sqlite
...
zoneconstraints<-dbGetQuery(clusdb, "SELECT * from zoneConstraints WHERE reference_zone = 'rast.zone_cond_eccc_columbia_south_crithab_or_herd'")
...
dbExecute(clusdb, "UPDATE zoneconstraints SET type = '' where reference_zone = 'rast.zone_du9_scenarios' AND (zoneid = 4) OR (zoneid = 19) OR (zoneid = 34) OR (zoneid = 61) OR (zoneid = 64) ") 
...
dbExecute(clusdb, "UPDATE zoneconstraints SET type = 'ge' where reference_zone = 'rast.zone_du9_scenarios' AND zoneid = 65") 
dbExecute(clusdb, "UPDATE zoneconstraints SET percentage = 65 where reference_zone = 'rast.zone_du9_scenarios' AND zoneid = 65")
dbExecute(clusdb, "UPDATE zoneconstraints SET type = '' where reference_zone = 'rast.zone_du9_scenarios' AND (zoneid = 4) OR (zoneid = 19) OR (zoneid = 34) OR (zoneid = 61) OR (zoneid = 64) OR (zoneid = 35) OR (zoneid = 49)") 
...
#Below will set Central Rockies  matrix as 35% disturbance (no buffer)
# NO core old, recruit or buffer areas
dbExecute(clusdb, "UPDATE zoneconstraints SET type = '' where reference_zone = 'rast.zone_du9_scenarios' AND (zoneid = 4) OR (zoneid = 5) OR (zoneid = 19) OR (zoneid = 20) OR (zoneid = 34) OR (zoneid = 35) OR (zoneid = 64) OR (zoneid = 65)") 
...
dbExecute(clusdb, "UPDATE zoneconstraints SET type = '' where reference_zone = 'rast.zone_du9_scenarios' AND (zoneid = 5) OR (zoneid = 20) OR (zoneid = 35) OR (zoneid = 61) OR (zoneid = 65)") 
...
dbExecute(clusdb, "UPDATE zoneconstraints SET type = '' where reference_zone = 'rast.zone_du9_scenarios' AND (zoneid = 5) OR (zoneid = 20) OR (zoneid = 35) OR (zoneid = 61) OR (zoneid = 65) OR (zoneid = 34) OR (zoneid = 48)") 
...
dbExecute(clusdb, "UPDATE zoneconstraints SET type = '' where reference_zone = 'rast.zone_smc_zones_20210315' AND (zoneid = 14) OR (zoneid = 47) OR (zoneid = 48) OR (zoneid = 49) OR (zoneid = 50)") 
...
parameters <- list(
  ...
  dataLoaderCLUS = list(   dbName='clus',
                           save_clusdb = FALSE,
                           sqlite_dbname = "TFL56",
                           useCLUSdb = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/TFL56_clusdb.sqlite"),
                           #Study Area
  ...
  uploaderCLUS = list(aoiName = 'tfl56', # name of the schema that gets uplaoded to postgres
...
# scenario = data.table (name = "tfl56_bau",
#                        description = "Business-as-usual (BAU). Harvest flow 45,000 m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl56_columbia_north_nh",
#                        description = "No harvest in Columbia North critical habitat. Harvest flow 45,000 m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl56_columbia_north_ch_he0d_m15d",
#                        description = "No harvest in Columbia North HEWSR critical habitat, 35% buffered disturbance (15% harvest) in matrix critical habitat. Harvest flow 45,000 m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl56_columbia_north_scen3a",
#                        description = "No harvest in old forest, recruitment forest and buffered Columbia North core critical habitat areas, as defined by Bevan Ernst, maximum 35% buffered disturbance (unbuffered) in Columbia North matrix habitat areas. Harvest flow 45,000 m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl56_columbia_north_scen3b",
#                        description = "No harvest in old forest and recruitment forest Columbia North core critical habitat areas, as defined by Bevan Ernst, maximum 35% buffered disturbance (unbuffered) in Columbia North matrix habitat areas. Harvest flow 45,000 m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl56_central_rockies_nh",
#                        description = "No harvest in Central Rockies critical habitat. Harvest flow 45,000 m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl56_central_rockies_ch_he0d_m15d",
#                        description = "No harvest in Central Rockies HEWSR critical habitat, 35% buffered disturbance (15% harvest) in matrix critical habitat. Harvest flow 45,000 m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl56_central_rockies_scen3a3b",
#                        description = "No old forest, recruitment forest or buffered Central Rockies core critical habitat areas, as defined by Bevan Ernst, inteh TSA; maximum 35% buffered disturbance (unbuffered) in Central Rockies matrix habitat areas. Harvest flow 45,000 m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl56_columbia_south_nh",
#                        description = "No harvest in Columbia South critical habitat. Harvest flow 45,000 m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl56_columbia_south_ch_he0d_m15d",
#                        description = "No harvest in Columbia South HEWSR critical habitat, 35% buffered disturbance (15% harvest) in matrix critical habitat. Harvest flow 45,000 m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl56_columbia_south_scen3a",
#                        description = "No harvest in old forest, recruitment forest and buffered Columbia South core critical habitat areas, as defined by Bevan Ernst, maximum 35% buffered disturbance (unbuffered) in Columbia South matrix habitat areas. Harvest flow 45,000 m3/yr. Adjacency = 3m. Oldest first.")

# scenario = data.table (name = "tfl56_columbia_south_scen3b",
#                        description = "No harvest in old forest and recruitment forest Columbia South core critical habitat areas, as defined by Bevan Ernst, maximum 35% buffered disturbance (unbuffered) in Columbia South matrix habitat areas. Harvest flow 45,000 m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl56_columbia_north_he0d",
#                        description = "No harvest in Columbia North high elevation critical habitat; harvest allowed in matrix critical habitat. Harvest flow 45,000 m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl56_columbia_north_he0d_m12d",
#                        description = "No harvest in Columbia North high elevation critical habitat; harvest allowed in matrix critical habitat up to 12% disturbance (no buffer) in forested area. Harvest flow 45,000 m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl56_columbia_north_hi_priority",
#                        description = "No harvest in high priority stands and all priority areas in Columbia North high elevation critical habitat; harvest allowed in matrix all priority areas up to 15% forest disturbance (no buffer) in forested area. Harvest flow 45,000 m3/yr. NOTE: Only high priority areas here. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl56_central_rockies_he0d_m12d",
#                        description = "No harvest in Central Rockies high elevation critical habitat; harvest allowed in matrix critical habitat up to 12% disturbance (no buffer) in forested area. Harvest flow 45,000 m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl56_columbia_south_he0d_m12d",
#                        description = "No harvest in Columbia South high elevation critical habitat; harvest allowed in matrix critical habitat up to 12% disturbance (no buffer) in forested area. Harvest flow 45,000 m3/yr. Adjacency = 3m. Oldest first.")




# BELOW ARE OLD SCENARIOS THAT NEED TO BE RE-RUN WITH NEW MODEL PARAMS
# scenario = data.table (name = "tfl56_colnsfb_nh",
#                        description = "No harvest in Columbia North and Columbia South critical habitat; harvest flow 31,000 m3/yr. Adjacency was set to 3m.")
# scenario = data.table (name = "tfl56_colnsfb_ch_he0d_m15d",
#                        description = "No harvest in Columbia North and Columbia South HEWSR critical habitat, 35% buffered disturbance (15% harvest) in matrix critical habitat; harvest flow 31,000 m3/yr. Adjacency was set to 3m.") # No harvest in CN, CS and FB high elevation critical habitat, maximum 35% buffered disturbance (15% harvest) in CN, CS and FB matrix critical habitat	

# scenario = data.table (name = "tfl56_columbia_north_ch_he10d_m15d",
#                        description = "No harvest in Columbia North high elevation critical habitat areas, maximum 35% buffered disturbance (15% harvest) in Columbia North matrix habitat areas. Harvest flow 45,000 m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "tfl56_columbia_south_ch_he10d_m15d",
...
harvestFlow <- rbindlist(list(data.table(compartment ="TFL56",
                                     partition = ' vol > 150 ', 
                                     period = rep( seq (from = 1, # run the 
                                                ...
                                                1), 
                                     flow = 225000,
                                     partition_type = 'live') #45,000m3/yr 
))
