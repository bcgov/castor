...
## 'harvestFlow'
from 2015 analysis discussion paper:  maximum even flow harvest was 2,000,000m^3^/year; from detemrination, June 3, 2010, the  AAC was set at 485 000 cubic metres 

- First, I tested a harvest flow of 2,000,000m^3^/year (10,000,000m^3^/5-year); was not able to achieve this, so dropped to the harvest flow eventually settling on 1,400,000m^3/yr. This havest rate was only possible with with a minimum harvest volume > 90 and age >= 60. In the report they say that to meet mid-term harvest levels they expect a significant poriton of the harvest will come from stands less than 80yrs. they also provide a figure showing that between 1/4 and 1/3 of the volume harvested will be within the age range of 61 to 80 yrs So setting a minimum age to 60 is probably reasonable for this TSA
...
#                        description = "No harvest in Groundhog critical habitat that overlap with the harvest unit (e.g., TSA or TFL). Adjacency was set to 3m.")
# scenario = data.table (name = "kamloops_groundhog_ch_he0d_m15d",
#                         description = "No harvest in Groundhog high elevation critical habitat, maximum 35% buffered disturbance (15% harvest) in Groundhog matrix critical habitat. Adjacency was set to 3m.")
# scenario = data.table (name = "kamloops_columbia_north_nh",
#                        description = "No harvest in Columbia North critical habitat that overlap with the harvest unit (e.g., TSA or TFL). Adjacency was set to 3m.")
...
harvestFlow <- rbindlist(list(data.table(compartment ="Kamloops_TSA",
                                     ...
                                     year = rep( seq (from = 2018, 
                                                      ...
                                                      by = 5),
                                                1), 
                                     flow = 7000000))) # flow=5750000 worked for adding in the age >=60 # Note in the TSA Discussion Paper (https://www2.gov.bc.ca/assets/gov/farming-natural-resources-and-industry/forestry/stewardship/forest-analysis-inventory/tsr-annual-allowable-cut/kamloops_tsa_discussion_paper.pdf) it looks like a sustainable flow is around 2 000 000 m3/yr. 
