...
## 'harvestFlow'
from 2011 analysis report:  maximum even flow harvest was 2,469,000m^3^/year; from detemrination, January 1, 2013, the  AAC was set at 3 078 405 cubic metres 

- First, I tested a harvest flow of 2,000,000m^3^/year (10,000,000m^3^/5-year); was not able to achieve an even harvest flow, so I dropped to 1,400,000, then 1,200,000 this still did not work so I went down to 1,140,000



why was this model max even flow so low compared to that in the analysis report? maybe adjacney?, maybe min. volume? or maybe harvest queue?
...
dbExecute(clusdb, "UPDATE zoneconstraints SET type = '' where reference_zone = 'rast.zone_cond_reve_area' AND (zoneid = 1) OR (zoneid = 2) OR (zoneid = 3) OR (zoneid = 4) OR (zoneid = 7) OR (zoneid = 8) OR (zoneid = 9) OR (zoneid = 10) OR (zoneid = 13) OR (zoneid = 14) OR (zoneid = 15) OR (zoneid = 16) OR (zoneid = 19) OR (zoneid = 20) OR (zoneid = 21) OR (zoneid = 22)") #This will set Columbia North and South as 'harvestable' areas
...
dbExecute(clusdb, "UPDATE zoneconstraints SET type = 'nh' where reference_zone = 'rast.zone_cond_reve_area' AND (zoneid = 1) OR (zoneid = 7) OR (zoneid = 13) OR (zoneid = 19)") #This will set Columbia North core as no harvest areas 
...
harvestFlow <- rbindlist(list(data.table(compartment ="Okanagan_TSA",
                                     ...
                                     year = rep( seq (from = 2018, 
                                                      ...
                                                      by = 5),
                                                1), 
                                     flow =5700000 ) # last try 7 000 000 and 6 000 000, neither of these worked. 5500000 this one worked .
))
