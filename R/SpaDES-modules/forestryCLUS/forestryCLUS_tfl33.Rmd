...
No harvest in Columbia North, Columbia South and Frisby-Boulder = rast.zone_cond_noharvest_frisby_boulder_crithab_or_herd
...
No harvest in Columbia North high elevation critical habitat, maximum 35% buffered disturbance (15% harvest) matrix critical habitat = rast.zone_cond_eccc_frisby_boulder_crithab_or_herd
...
## 'harvestFlow'
from 2011 determination: 
"...the harvest projection suggests an initial harvest level of 20,250 cubic metres per year starting in 2009 followed by a reduction to 18,100 cubic metres per year in 2019 and a small increase to 18,750 cubic metres per year in 2079." and "...an even-flow over the forecast period of 19 000 cubic metres per year can be achieved."
- started at 19,000m^3^/year (95,000m^3^/5-year); way too high, dropped to 5,000, 2,500


- very low cut levels seem to be due to inventory having few polygons with a site prodcutivity value; harvest in the model is limited to those patches
...
parameters <- list(
  ...
  dataLoaderCLUS = list(   dbName='clus',
                           save_clusdb = FALSE,
                           useCLUSdb = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/TFL33_clusdb.sqlite"),
                           #Study Area
  ...
  disturbanceCalcCLUS = list(calculateInterval = 5, 
                             ...
                             recovery = 40),
  uploaderCLUS = list(aoiName = 'tfl33', # name of the schema that gets uplaoded to postgres
...
# scenario = data.table (name = "tfl33_bau",
#                        description = "Business-as-usual (BAU); harvest flow 2,500 m3/yr. Adjacency was set to 3m.")
# scenario = data.table (name = "tfl33_colnsfb_nh",
#                        description = "No harvest in Frisby-Boulder critical habitat; harvest flow 2,500 m3/yr. Adjacency was set to 3m.")
scenario = data.table (name = "tfl33_colnsfb_ch_he0d_m15d",
                       description = "No harvest in Frisby-Boulder HEWSR critical habitat, 35% buffered disturbance (15% harvest) in matrix critical habitat; harvest flow 2,500 m3/yr. Adjacency was set to 3m.")

harvestFlow <- rbindlist(list(data.table(compartment ="TFL33",
                                         partition = ' vol > 150 ', 
                                         year = rep( seq (from = 2018, 
                                                          to = 2218, 
                                                          by = 5),
                                                    1), 
                                         flow = 12500) # 2,500m3/yr 
))
