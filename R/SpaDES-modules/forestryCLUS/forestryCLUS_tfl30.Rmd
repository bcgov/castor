...
no harvest in Hart Ranges caribou critical habtiat; to support  herd planning, we run a scenario with no harvest in theh HEWSR and Matrix; assesses the potential maximum impact of protecting these herds 

ECCC in Hart Ranges caribou critical habitat; an alterantive to a no harvest scenario, attemps to interpret the federal recovery strategy that stipulates no harvest in high and low elevation critical habitat, and up to 35% disturabnce in matrix habtiat; this is a possible scenario if Canada were to enact an emergency order on Hart Ranges
...
No harvest in Hart Ranges high elevation critical habitat, maximum 35% buffered disturbance (15% harvest) matrix critical habitat = rast.zone_cond_eccc_hart_ranges_crithab_or_herd
...
## 'harvestFlow'
from 2014 determination: 
Three different alternative forecasts are provided. In the base case the initial harvest level is set at 412,500 cubic meters per year for 45 yrs after which it is increased to 537,000 m^3. In the second scenario the even-flow is set at 409,000 m^3/yr and left at that level indefinitely
The third the initial harvest level is 459,000 cubic meters per year and then declines to 368 000 cubic meters per year in ~30 yrs and then increases again to 527,000 m^3/yr.

- started at 410,000m^3/year; was very high, dropped to 200,000, 150,000, 175,000, 185,000, 180,000

- result is much lower than the 2014 flows; maybe an issue with inventory?
...
clusdb <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/TFL30_clusdb.sqlite")) # connext to clusdb -- set the locations of the sqlite
...
zoneconstraints<-dbGetQuery(clusdb, "SELECT * from zoneConstraints WHERE reference_zone = 'rast.zone_cond_bc_hart_ranges_crithab'") 
...
dbExecute(clusdb, "UPDATE zoneconstraints SET percentage = 15 where reference_zone = 'rast.zone_smc_zones_20210315' AND zoneid = 70") 
dbExecute(clusdb, "UPDATE zoneconstraints SET type = '' where reference_zone = 'rast.zone_smc_zones_20210315' AND (zoneid = 67) OR (zoneid = 68) OR (zoneid = 69)") 
...
parameters <- list(
  ...
  dataLoaderCLUS = list(   dbName='clus',
                           save_clusdb = TRUE,
                           sqlite_dbname = "TFL30",
                           useCLUSdb = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/TFL30_clusdb.sqlite"),
                           #Study Area
  ...
  uploaderCLUS = list(aoiName = 'tfl30', # name of the schema that gets uploaded to postgres
...
# rsf_model_coeff[, bounds := 'rast.bc_crithab_and_herd']
 ...
 # scenario = data.table (name = "tfl30_bau",
 #                        description = "Business as usual (BAU). Harvest flow = 180,000m^3^/year. Oldest first priority. Adjacency = 3m.")
# scenario = data.table (name = "tfl30_hart_nh",
#                         description = "No harvest in all Hart Ranges critical habitat. Harvest flow = 180,000m^3^/year. Oldest first priority. Adjacency = 3m.")
# scenario = data.table (name = "tfl30_hart_he0d_m12d",
#                         description = "No harvest in Hart Ranges high elevation critical habitat, maximum 12% buffered disturbance (no buffer) in Hart Ranges matrix critical habitat. Harvest flow = 180,000m^3^/year. Oldest first priority. Adjacency = 3m.")
# scenario = data.table (name = "tfl30_hart_scen3a",
...
# scenario = data.table (name = "tfl30_hart_scen3b",
...
# scenario = data.table (name = "tfl30_hart_hi_med_lo_priority",
#                        description = "No harvest in Hart Ranges high priority stands in high, medium and low priority HEWSR zones; harvest allowed in all matrix critical habitat up to 15% disturbance (no buffer) in forested area. Harvest flow = 180,000m^3^/year. Oldest first priority. Adjacency = 3m.")

scenario = data.table (name = "tfl30_hart_hi_med_priority",
                       description = "No harvest in Hart Ranges high priority stands in high and medium priority HEWSR zones. harvest allowed in low priority HEWSR. Harvest allowed in all matrix critical habitat up to 15% disturbance (no buffer) in forested area. NOTE there is only low priority HEWSR in this TFL. Harvest flow = 180,000m^3^/year. Oldest first priority. Adjacency = 3m.")



harvestFlow <- rbindlist(list(data.table(compartment ="TFL30",
                                     partition = ' vol > 150 ', 
                                     period = rep( seq (from = 1, # run the 
                                                ...
                                                1), 
                                     flow = 900000,
                                     partition_type = 'live') #180,000m3/yr
))
