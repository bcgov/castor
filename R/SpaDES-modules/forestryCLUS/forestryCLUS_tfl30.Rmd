...
Three different alternative forcasts are provided. In the base case the initial hervest level is set at 412,500 cubic meters per year for 45 yrs after which it is inceased to 537,000 m^3. In the second scenario the even-flow is set at 409,000 m^3/yr and left at that level indefinitly
The third the initial harvest level is 459,000 cubic meters per year and then decliens to 368 000 cubic meters per year in ~30 yrs and then increases again to 527,000 m^3/yr.

- started at 409,000m^3/year; way too high, I tried lots of different values but eventually settled on 136,000 m^3/yr. This is very low in comparison to that forcasted in the AAC determination models. Why is it this low? I dont know. Maybe I sholuld play around with age or harvest volume to increase the harvest-flow to a greater value. 
...
# rsf_model_coeff[, bounds := 'rast.bc_crithab_and_herd']
 ...
 #                        description = "No harvest in all Hart Ranges critical habitat. Adjacency was set to 3m.")
 scenario = data.table (name = "tfl30_hart_ch_he0d_m15d",
                        description = "No harvest in Hart Ranges high elevation critical habitat, maximum 35% buffered disturbance (15% harvest) in Hart Ranges matrix critical habitat. Adjacency was set to 3m.")
