...
parameters <- list(
  ...
  dataLoaderCLUS = list( 
                         dbName='clus',
                           ...
                           nameZoneRasters=c("rast.zone_cond_beo", "rast.zone_cond_vqo", "rast.zone_cond_wha", "rast.zone_cond_uwr", "rast.zone_cond_nharv", "rast.zone_cond_fsw", "rast.zone_cond_nh_crit_barkerville_crithab","rast.zone_cond_nh_crit_itcha_ilgachuz_crithab","rast.zone_cond_nh_crit_wells_gray_north_crithab", "rast.zone_cond_nh_crit_charlotte_alplands_crithab", "rast.zone_cond_nh_crit_rainbows_crithab"),
...
#scenario = data.table(name="itchas_eccc_a1p5_r30", description = "ECCC- No harvest in high elevation range or low elevation summer range, 35% disturbance in matrix for itchas. Adjacency constraint > 1.5 m. Recovery > 30 years")
...
#scenario = data.table(name="wells_gray_north_nh_crit_aNA_rNA", description = "No harvest in critical habitat for wells gray north")
...
#scenario = data.table(name="barkerville_eccc_a1p5_r40", description = "ECCC- No harvest in high elevation range or low elevation summer range, 35% disturbance in matrix for barkerville. Adjacency constraint > 1 m. Recovery > 40 years")
...
#scenario = data.table(name="eccc_aNA_r40", description = "ECCC- No harvest in high elevation range or low elevation summer range, 35% disturbance in matrix for itchas, barkerville and wells gray north. Adjacency constraint > 3 m. Recovery > 40 years.")
#scenario = data.table(name="bc_herd_plan_aNA_r40", description = "BC herd plan- No harvest in high elevation range, less than 35% disturbance in low elevation (buffered) and matrix (not buffered) for itchas, barkerville and wells gray north. Recovery > 40 years.")

#scenario = data.table(name="bc_herd_plan_aNA_r30", description = "BC herd plan- No harvest in high elevation range, less than 35% disturbance in low elevation (buffered) and matrix (not buffered) for itchas, barkerville and wells gray north. Recovery > 30 years.")
#scenario = data.table(name="eccc_aNA_r30", description = "ECCC- No harvest in high elevation range or low elevation summer range, 35% disturbance in matrix for itchas, barkerville and wells gray north. Recovery > 30 years.")
...
scenario = data.table(name="nh_all_crit", description = "No harvest in any of the critical habitat types")
...
harvestFlow<- data.table(compartment ="Williams_Lake_TSA",
                                     partition = ' vol > 110 ',
                                     year = rep(seq(from = 2018, to=2218, by = 5),1),  flow = 9300000)
