...
no harvest in Narrow Lake caribou critical habtiat; to support Narrow Lake herd planning, we run a sceanrio with no harvest in the Narrow Lake HEWSR and Matrix; assesses the potential maximum impact of protecting Narrow Lake only

ECCC in Narrow Lake caribou critical habitat; an alterantive to a no harvest scenario, attemps to interpret the federal recovery strategy that stipulates no harvest in high and low elevation critical habitat, and up to 35% disturabnce in matrix habtiat; this is a possible scenario if Canada were to enact an emergency order on Narrow Lake only
...
## 'harvestFlow'
from 2010 determination: AAC was 219,000 cubic metres

- First, I tested a harvest flow of 220,000m^3^/year, 250,000m^3^/year, 300,000m^3^/year, 275,000m^3^/year, 260,000m^3^/year, 270,000m^3^/year, 275,000m^3^/year; back to 270,000m^3^/year

- I used 150 m3/ha as minimum harvest volume;  

- I used 3 m adjacency

#UPDATE SCENARIO
```{r, zone_update}
#STEP 1: Connect to the clusdb.sqlite database for the AOI
clusdb <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/TFL53_clusdb.sqlite")) # connext to clusdb -- set the locations of the sqlite
...
#Below will set  matrix as 12% disturbance (no buffer); there is no core overlap
...
parameters <- list(
  ...
  uploaderCLUS = list(aoiName = 'tfl53', # name of the schema that gets uplaoded to postgres
...
# scenario = data.table (name = "tfl53_bau",
#                        description = "Business-as-usual (BAU). Oldest first. Adjacency = 3m. Even harvest flow = 270,000m3/year")
# scenario = data.table (name = "tfl53_narrow_lake_nh",
#                        description = "No harvest in Narrow Lake critical habitat. Oldest first. Adjacency = 3m. Even harvest flow = 270,000m3/year")
# scenario = data.table (name = "tfl53_narrow_lake_ch_he0d_m15d",
#                        description = "No harvest in Narrow Lake HEWSR, maximum 35% buffered disturbance (15% harvest) in Narrow Lake matrix critical habitat that overlap with the harvest unit. Oldest first. Adjacency = 3m. Even harvest flow = 270,000m3/year.")
# scenario = data.table (name = "tfl52_narrow_lake_scen3a3b",
#                        description = "No haold forest, recruitment forest or buffered Narrow Lake core critical habitat areas, as defined by Bevan Ernst, in TFL 53; maximum 35% buffered disturbance (unbuffered) in Narrow Lake matrix habitat areas. Oldest first. Adjacency = 3m. Even harvest flow = 270,000m3/year.")

scenario = data.table (name = "tfl53_narrow_lake_he0d_m12d",
                       description = "No harvest in Narrow Lake HEWSR, maximum 12% disturbance (unbuffered) in Narrow Lake matrix critical habitat that overlap with the harvest unit. Oldest first. Adjacency = 3m. Even harvest flow = 270,000m3/year.")
