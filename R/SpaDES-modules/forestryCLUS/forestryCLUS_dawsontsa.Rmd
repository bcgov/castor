...
count <- dbGetQuery(clusdb, "SELECT count() from pixels where zone28 = 5") # rast.zone_cond_bc_moberly_crithab, rast.zone_cond_bc_burnt_pine_crithab, rast.zone_cond_bc_quintette_crithab, rast.zone_cond_bc_kennedy_siding_crithab, rast.zone_cond_bc_narraway_crithab

## update to the total area in the constraint table for the zone
dbExecute(clusdb, paste0("UPDATE zoneConstraints SET t_area = ", count, " where reference_zone = 'rast.zone_cond_bc_narraway_crithab' AND (zoneid = 5)")) # rast.zone_cond_bc_moberly_crithab, rast.zone_cond_bc_burnt_pine_crithab, rast.zone_cond_bc_quintette_crithab, rast.zone_cond_bc_kennedy_siding_crithab, rast.zone_cond_bc_kennedy_siding_crithab, rast.zone_cond_bc_narraway_crithab
...
scenario = data.table (name = "dawson_creek_bau_newpark_conif",
                       description = "Business-as-usual case with Klinse-za/Twin Sisters park (B2 and B3 areas) from parntership agreement added as no harvest. Conifer leading stands (deciduous percentage < 20) only. Harvest flow = 1,500,000m3/yr. Adjacency = 3m. Oldest first.")
...
harvestFlow <- rbindlist(list(data.table(compartment ="Dawson_Creek_TSA",
                                     partition = ' vol > 120 AND dec_pcnt < 20 ', #
                                     year = rep( seq (from = 2018,
                                                      to = 2222,
                                                      by = 5),
                                                1),
                                     flow = 5000000) #1,000,000m3/yr / conifer leading stands only / BAU with new protected area
                        ))
