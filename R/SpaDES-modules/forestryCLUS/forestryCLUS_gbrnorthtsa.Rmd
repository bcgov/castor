...
## 'harvestFlow'
No information on the even flow harvest could be found for GBR south, except in the data packages for Kingcome and Strathcona TSA's. However, since GBR south is only a portion of these old TSA's it difficult to work out how to add the harvest flows and soI started with the value for the ACC as the even flow value i.e. 830,500 m^3/year. This value was too high so I dropped it to 700,000m^3/yr but this too was still to high. I then tried 600,000. 

Interestingly, I found by adjusting the volume to 90 cubic meteres per hectare and stand age to 60 I could incease the harvest flow from 600,000m^3/year to 620,000m^3/year. but for the final run and scenarios I decided to use150 m3/ha as minimum harvest volume as this is what was done in most of the other TSA's.
...
# rsf_model_coeff[, bounds := 'rast.bc_crithab_and_herd']
 ...
 scenario = data.table (name = "GBR_north_bau",
