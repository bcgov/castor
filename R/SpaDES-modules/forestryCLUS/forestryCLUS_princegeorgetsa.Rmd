...
BAU plus the new B2 and B3 areas (new Klinse-za/Twin Sisters Park) of the caribou partnership agreement areas as protected areas (rast.central_grp_proposed_luo zones 1 and 2 = no harvest); NOTE; these areas no in the TSA

added proposed LUO's for herds
...
clusdb <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/Prince_George_TSA_clusdb.sqlite")) # Prince_George_North_TSA_clusdb.sqlite  Prince_George_TSA_clusdb.sqlite

#STEP 2: View the constraints available to a zone
zoneconstraints<-dbGetQuery(clusdb, "SELECT * from zoneConstraints WHERE reference_zone = 'rast.zone_cond_bc_kennedy_siding_crithab'") # Note: the reference_zone is the name of the raster in the rast schema. If there is no 'WHERE' clause this may return 1000's of zones; rast.zone_cond_bc_itcha_ilgachuz_crithab rast.zone_cond_bc_tweedsmuir_crithab


# Central Group - Draft Land Use Orders - April 23, 2021
# Below sets proposed maximum NRV age targets for LUO for critical habitat overlapping this unit  with Age <30
## Kennedy Siding
...
# connect to the db
clusdb <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/Prince_George_TSA_clusdb.sqlite"))
...
count <- dbGetQuery(clusdb, "SELECT count() from pixels where zone31 = 5") # 0, 4, 5 

## update to the total area in the constraint table for the zone
dbExecute(clusdb, paste0("UPDATE zoneConstraints SET t_area = ", count, " where reference_zone = 'rast.zone_cond_bc_kennedy_siding_crithab' AND (zoneid = 5)")) # 0, 4, 5 
...
parameters <- list(
  ...
  dataLoaderCLUS = list( 
                           dbName='clus',
                           save_clusdb = TRUE,
                           useCLUSdb = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/Prince_George_TSA_clusdb.sqlite"),  # Prince_George_North_TSA_clusdb.sqlite  Prince_George_TSA_clusdb.sqlite
...
# scenario = data.table (name = "pg_south_centgrp_luo_age_le40_minNRV",
#                        description = "Draft land use order early seral (age) thresholds for central group critical habitat, April 26, 2021. Minimum natural range of variability (NRV) early seral targets. Sustainable flow = 6,000,000m^3^/year. Oldest first priority. Adjacency was set to 3m.")
# scenario = data.table (name = "pg_south_centgrp_luo_age_le40_modNRV",
#                        description = "Draft land use order early seral (age) thresholds for central group critical habitat, April 26, 2021. Moderate natural range of variability (NRV) early seral targets. Sustainable flow = 6,000,000m^3^/year. Oldest first priority. Adjacency was set to 3m.")
# scenario = data.table (name = "pg_south_centgrp_luo_age_le40_maxNRV",
#                        description = "Draft land use order early seral (age) thresholds for central group critical habitat, April 26, 2021. Maximum natural range of variability (NRV) early seral targets. Sustainable flow = 6,000,000m^3^/year. Oldest first priority. Adjacency was set to 3m.")
...
# scenario = data.table (name = "pg_south_centgrp_agele30_maxnrv",
#                        description = "Draft land use order early seral (age) thresholds for central group critical habitat, April 26, 2021. Early seral targets set for maximum natural range of variability (NRV) for each area, using age <30 as early seral. Harvest flow = 6.0 M m3/yr. Adjacency = 3m. Oldest first.")
>>>>>>> 96e1921610ac8cabee10fb84a720bf783fb0bd5d
...
harvestFlow <- rbindlist(list(data.table(compartment ="Prince_George_TSA", # Prince_George_North_TSA   Prince_George_TSA
                                         partition = ' vol > 182 and age >= 75  ', 
                                           year = rep( seq (from = 2018, 
                                                            ...
                                                            by = 5),
                                                      1), 
                                           flow = 30000000) #6,000,000 / 30000000 //  1,200,000 / 6000000 
))
