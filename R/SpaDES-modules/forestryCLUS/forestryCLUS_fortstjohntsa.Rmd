...
parameters <- list(
  ...
  dataLoaderCLUS = list( 
                           dbName='clus',
                           save_clusdb = FALSE,
                           useCLUSdb = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/Fort_St_John_TSA_clusdb.sqlite"), # Fort_St_John_Core_TSA_clusdb.sqlite  Fort_St_John_TSA_clusdb.sqlite
                           #Study Area
...
# scenario = data.table (name = "fsj_nocore_graham_hi_med_lo_priority",
#                        description = "No harvest priority stands in high priority zones in group 2 Graham) HEWSR and LESR critical habitat; harvest allowed in high priority zones in LEWR and matrix critical habitat up to 15% disturbance (no buffer) in forested area. Harvest allowed in low and medium priority zones. NOTE: all zones in this unit are high priority. Harvest flow = 1,550,000m3/yr. Adjacency = 3m. Oldest first.")


harvestFlow <- rbindlist(list(data.table(compartment = "Fort_St_John_TSA", # "Fort_St_John_Core_TSA"  Fort_St_John_TSA
                                     partition = ' vol > 139 ', 
                                     year = rep( seq (from = 2018, 
                                                      to = 2222, 
                                                      by = 5),
                                                1), 
                                     flow = 7750000) #1,550,000m3/yr
))
