...
- harvest priorities in base case of AAC determination were: >50% pine stand first, then open forest stands in open range for ecosystem restoration purposes, and then harvest the oldest stands 

## 'harvestFlow'
from June 2017 determination: AAC was 496,720m^3^/year until 2022 and 424,800m^3^/year thereafter; base case maintained a harvest of 447,158m^3^/year for 100 years, and could be sustained for a 250-year forecast

- First, I tested a harvest flow of 475,000m^3^/year (2,375,000m^3^/5-year), 400,000, 450,000, 425,000, 415,000, 410,000, 405,000

- stable harvest flow at 405,000^3^/5-year

- I used 150 m3/ha as minimum harvest volume, data package used minimum age 60 for pine, 80 for everything else

- For cutblock adjacency, I used 3 m adjacency; this varied adn was not clear in data package
...
clusdb <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/Invermere_TSA_clusdb.sqlite")) # connext to clusdb -- set the locations of the sqlite
...
dbExecute(clusdb, "UPDATE zoneconstraints SET type = '' where reference_zone = 'rast.zone_du9_scenarios' AND zoneid = 62") 
...
dbExecute(clusdb, "UPDATE zoneconstraints SET type = 'ge' where reference_zone = 'rast.zone_du9_scenarios' AND zoneid = 72") 
dbExecute(clusdb, "UPDATE zoneconstraints SET percentage = 65 where reference_zone = 'rast.zone_du9_scenarios' AND zoneid = 72")
dbExecute(clusdb, "UPDATE zoneconstraints SET type = '' where reference_zone = 'rast.zone_du9_scenarios' AND (zoneid = 62) OR (zoneid = 41) OR (zoneid = 55)") 

dbGetQuery(clusdb, "SELECT * from zoneConstraints WHERE reference_zone = 'rast.zone_cond_eccc_purcell_central_crithab_or_herd'")
dbExecute(clusdb, "UPDATE zoneconstraints SET threshold = 0 where reference_zone = 'rast.zone_cond_eccc_purcell_central_crithab_or_herd' AND zoneid = 2") 
...
parameters <- list(
  ...
  dataLoaderCLUS = list(   dbName='clus',
                           ...
                           useCLUSdb = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/Invermere_TSA_clusdb.sqlite"),
                           #Study Area
  ...
  disturbanceCalcCLUS = list(calculateInterval =  1, # should be 1 if using constraints on 'dist' (disturbance) 
                             ...
                             recovery = 40),
  uploaderCLUS = list(aoiName = 'invermere_tsa', # name of the schema that gets uplaoded to postgres
...
# scenario = data.table (name = "invermere_bau",description = "Business-as-usual case. Harvest flow = 382,000m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "invermere_purcell_central_nh", description = "No harvest in Purcell Central critical habitat. Harvest flow = 382,000m3/yr. Adjacency = 3m. Oldest first.")
 scenario = data.table (name = "invermere_purcell_central_ch_he0d_m12d",
                       description = "No harvest in Purcell Central high elevation critical habitat, maximum 12%  disturbance in Purcell Central matrix critical habitat. Harvest flow = 382,000m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "invermere_purcell_central_scen3a",
#                         description = "No harvest in old forest, recruitment forest and buffered Purcell Central core critical habitat areas, as defined by Bevan Ernst, maximum 35% buffered disturbance (unbuffered) in Purcell Central matrix habitat areas. Harvest flow = 405,000m3/yr. Adjacency = 3m. Oldest first.")

#scenario = data.table (name = "invermere_purcell_central_scen3b", description = "No harvest in old forest and recruitment forest Purcell Central core critical habitat areas, as defined by Bevan Ernst, maximum 35% buffered disturbance (unbuffered) in Purcell Central matrix habitat areas. Harvest flow = 405,000m3/yr. Adjacency = 3m. Oldest first.")




harvestFlow <- rbindlist(list(data.table(compartment ="Invermere_TSA",
                                     partition = ' vol > 150 ', 
                                     period = rep( seq (from = 1, # run the 
                                                ...
                                                1), 
                                     flow = 1910000,
                                     partition_type = 'live') #382,000m3/yr 
))
