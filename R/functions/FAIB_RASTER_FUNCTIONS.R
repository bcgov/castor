#----------------------------------------------------------------------------------------------------------------------------------------
#R Package for Accessing PostGIS FAIB Raster functions
...
#-----------------------------------------------------------------------------------------------------------------------------------------
GetRasterLayer_RPOSTGIS<-function(conn, target_rast, rastCol="rast"){library(raster)
  library(rpostgis)
  library(RPostgreSQL)
  library(rgdal)
...
#-----------------------------------------------------------------------------------------------------------------------------------------
GetRasterLayer_FAIB<-function(conn, target_rast){library(raster)
  library(rpostgis)
  library(RPostgreSQL)
  library(rgdal)
...
library(DBI)
library(RPostgreSQL)  
library(sp) #--Needed to plot the rasters
...
geom <- dbGetQuery(pgConn, 'SELECT ST_ASTEXT(ST_TRANSFORM(WKB_GEOMETRY, 4326)) FROM TSA_CLIP WHERE TSA_NUMBER_INT = 27')  #--Single Poly
geom <- dbGetQuery(pgConn, 'SELECT ST_ASTEXT(ST_TRANSFORM(WKB_GEOMETRY, 4326)) FROM TSA_CLIP WHERE TSA_NUMBER_INT = 9')   #--Single Poly
geom <- dbGetQuery(pgConn, 'SELECT ST_ASTEXT(ST_UNION(ST_TRANSFORM(WKB_GEOMETRY, 4326))) FROM TSA_CLIP WHERE TSA_NUMBER_INT = 45')  #--Multiple Polygons


#--Local Connection
pgConn<-GetPostgresConn("postgres", "postgres", "postgres", "localhost")
#--CLUS Connection
#pgConn <- dbConnect("PostgreSQL", dbname = "postgres", host = "localhost", user = "postgres", password = "postgres")

#--Examples RASTER_FROM_VECTOR
ras <- RASTER_FROM_VECTOR(txtPoly,  "TSA_CLIP", whereClause="*", conn=pgConn)
ras <- RASTER_FROM_VECTOR(txtPoly,  "TSA_CLIP", whereClause="*", mask=TRUE, conn=pgConn)
ras <- RASTER_FROM_VECTOR(txtPoly,  "BEC_ZONE_CLIP", whereClause="ZONE IN (''ESSF'') ", vatFld='ZONE', mask=FALSE, conn=pgConn)
ras <- RASTER_FROM_VECTOR(txtPoly,  "BEC_ZONE_CLIP", whereClause="ZONE IN (''ESSF'') ", vatFld='ZONE_ALL', mask=FALSE, conn=pgConn)
ras <- RASTER_FROM_VECTOR(txtPoly,  "BEC_ZONE_CLIP", whereClause="ZONE IN (''ESSF'', ''ICH'') ", vatFld='ZONE_ALL', mask=FALSE, conn=pgConn)
ras <- RASTER_FROM_VECTOR(txtPoly,  "TSA_CLIP", whereClause="*", vatFld='TSA_NAME', mask=FALSE, conn=pgConn)
ras <- RASTER_FROM_VECTOR(txtPoly,  "TSA_CLIP", whereClause="UPPER(TSA_NAME) LIKE ''REV%'' ", vatFld='TSA_NAME', mask=FALSE, conn=pgConn)
ras <- RASTER_FROM_VECTOR(txtPoly,  "TSA_CLIP", whereClause="UPPER(TSA_NAME) LIKE ''REV%'' ", vatFld='TSA_NAME', mask=TRUE, conn=pgConn)
ras <- RASTER_FROM_VECTOR(txtPoly,  "BEC_ZONE_CLIP", whereClause="ZONE IN (''ESSF'') ", vatFld='BEC_KEY', mask=FALSE, conn=pgConn)
ras <- RASTER_FROM_VECTOR(txtPoly,  "BEC_ZONE_CLIP", whereClause="*", vatFld='BEC_KEY', mask=FALSE, conn=pgConn)
ras <- RASTER_FROM_VECTOR(txtPoly,  "BEC_TSA_EXTENT", whereClause="ZONE IN (''ICH'') ", vat='BEC_TSA_EXTENT_RASTER_VAT', vatFld='ZONE', mask=FALSE, conn=pgConn)

#--Examples RASTER_FROM_RASTER
ras <- RASTER_FROM_RASTER(txtPoly,  "TSA_CLIP_RASTER", rastVal="*", mask=FALSE, conn=pgConn)
ras <- RASTER_FROM_RASTER(txtPoly,  "TSA_CLIP_RASTER", rastVal="*", mask=FALSE)
ras <- RASTER_FROM_RASTER(txtPoly,  "BEC_TSA_EXTENT_RASTER", rastVal="*", mask=FALSE, conn=pgConn)
ras <- RASTER_FROM_RASTER(txtPoly,  "BEC_TSA_EXTENT_RASTER", rastVal="6", mask=FALSE, conn=pgConn)
ras <- RASTER_FROM_RASTER(txtPoly,  "BEC_TSA_EXTENT_RASTER", rastVal="6", mask=TRUE, conn=pgConn)
ras <- RASTER_FROM_RASTER(txtPoly,  "BEC_TSA_EXTENT_RASTER", rastVal="4-8", mask=FALSE, conn=pgConn)
ras <- RASTER_FROM_RASTER(txtPoly,  "BEC_TSA_EXTENT_RASTER", rastVal="ZONE IN (''ESSF'') ", rastVAT='BEC_TSA_EXTENT_RASTER_ZONE_VAT', mask=FALSE, conn=pgConn)
ras <- RASTER_FROM_RASTER(txtPoly,  "BEC_TSA_EXTENT_RASTER", rastVal="ZONE IN (''ESSF'', ''ICH'') ", rastVAT='BEC_TSA_EXTENT_RASTER_ZONE_VAT', mask=FALSE, conn=pgConn)
ras <- RASTER_FROM_RASTER(txtPoly,  "TSA_CLIP_RASTER", rastVal="UPPER(TSA_NAME) LIKE ''REV%'' ", rastVAT='TSA_CLIP_NAME_VAT', mask=FALSE, conn=pgConn)
...
ras <- RASTER_FROM_RASTER(geom, "TSA_CLIP_RASTER", rastVal="*", mask=FALSE, conn=pgConn)
ras <- RASTER_FROM_RASTER(geom,  "BEC_TSA_EXTENT_RASTER", rastVal="*", mask=FALSE, conn=pgConn)

#--Examples FC_TO_RASTER
ras <- FC_TO_RASTER('TSA_CLIP', 'TSA_NAME', conn=pgConn)
ras <- FC_TO_RASTER('TSA_CLIP', 'TSA_NUMBER_INT', conn=pgConn)
ras <- FC_TO_RASTER('BEC_ZONE_CLIP', 'ZONE_ALL', conn=pgConn)
ras <- FC_TO_RASTER('BEC_ZONE_CLIP', 'BEC_KEY', conn=pgConn)

#--Examples RASTER_CLIP
ras <- RASTER_CLIP('BEC_TSA_EXTENT_RASTER', geom, conn=pgConn)
ras <- RASTER_CLIP('BEC_TSA_EXTENT_RASTER', txtPoly, conn=pgConn)
