...
locs.caribou.SCEK143.geo <- st_transform (x = locs.caribou.SCEK143, crs = 4326)
...
hr.250.caribou.SCEK143 <- hr.250.caribou.SCEK143 %>%
                          filter (id == 'du6_LateWinter_SCEK143_2015')
hr.250.caribou.SCEK143$hr <- "250"


bb <- attr (locs.caribou.SCEK143, "bb")
...
ggplot () +
  geom_sf (data = hr.750.caribou.SCEK143, linetype = "dotted") +
  geom_sf (data = hr.500.caribou.SCEK143) +
  geom_sf (data = hr.250.caribou.SCEK143, linetype = "dashed") +
  geom_sf (data = locs.caribou.SCEK143.geo) + 
  labs (title = "Estimated home range of caribou SCEK143 in late winter") +
  theme (plot.title = element_text (size = 12)) +
  labs (caption = "Dotted line: h = 750; Solid line: h = 500; Dashed line: h = 250") +
  scalebar (locs.caribou.SCEK143.geo, dist = 1, dd2km = T, model = "WGS84")
  


ggplot () +
  geom_sf (data = locs.caribou.SCEK143.geo) +
  scalebar (locs.caribou.SCEK143.geo, dist = 0.5, dd2km = TRUE, model = 'WGS84')



plot (st_geometry (hr.750.caribou.SCEK143), lty = 2) 
 plot (st_geometry (hr.500.caribou.SCEK143), lty = 1, add = T) 
 plot (st_geometry (hr.250.caribou.SCEK143), lty = 3, add = T)
 plot (st_geometry (locs.caribou.SCEK143), pch = 16, cex = 0.2, add = T)
 scalebar (d = 1000, divs = 2, type = "bar", xy = c (10,10), label = c (0, 1000, 2000))
