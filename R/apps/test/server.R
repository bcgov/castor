...
shinyServer(function(input, output, session) {
  ...
  herd_bound <<- st_zm(getSpatialQuery("SELECT * FROM public.gcbp_carib_polygon WHERE herd_name <> 'NA'"))
  ...
  output$rdTable<-renderTable({
    withProgress(message = 'Running Roads Query', value = 0, {
