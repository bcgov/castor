...
# Reactive Values 
  ...
  drawnPolys<-reactive({
    ...
    if(!is.null(input$map_draw_all_features)){
      ...
      polys<-list()
      #--Testing a function to get the Coords as text constructor for PostGIS
      spTestPoly<-Polygons(
        list(Polygon(cbind(Longitudes[[1]], Latitudes[[1]]))), ID=f$features[[1]]$properties$`_leaflet_id`
      )
      
      
      
      
      #--Mike Fowler Testing Raster Functions
      #drawnPolys()$coordz
      
      #c = spTestPoly@Polygons[[1]]@coords
      ...
      CallPostgresRasterFunction(pgConn, 'VECTORLINKRASTER', 'CLUS_RUNTIME_TESTFROMAPP_VECTORLINK', txtPoly, 'BEC_ZONE_CLIP', "ZONE IN (''ESSF'', ''ICH'')", 'BEC_ZONE_CLIP_RASTER', 'BEC_KEY')
      dbDisconnect(pgConn)
      #print(txtPoly)
      
      #print(polys@Polygons@coords)
      spPolys<-SpatialPolygons(polys)
      
      #proj4string(spPolys)<-"+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"
      proj4string(spPolys)<-"+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"
