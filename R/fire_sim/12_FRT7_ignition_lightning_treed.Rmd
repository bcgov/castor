...
```
For FRT 7 its mean_RH06_RH07

# Now what is the top model for FRT 7?
...
```
The top model is climate1 + prj_g_1 + vegtype
...
dat2<- dat %>% dplyr::filter(frt == 7) %>%
  drop_na(prj_h_1)

table(dat2$vegtype, dat2$fire) # probably good to drop vegtype = D because there are only 4 occurences of a fire ignition and this seems to mess up my model when I include it. 
...
mod.frt.7<-glm(fire ~ climate1 + prj_g_1 + vegtype, data=dat2, family=binomial(link="logit"))

library(caret)
varImp(mod.frt.7)
library(GmAMisc)
modelvalid(dat2, mod.frt.7)

summary(mod.frt.7)
anova(mod.frt.7)
Anova(mod.frt.7, type=3)

# model diagnostic plots
binnedplot (fitted(mod.frt.7), 
            residuals(mod.frt.7), 
            nclass = NULL, 
...
##Partial Residuals
visreg(mod.frt.7, scale="response") # nicer plots than the ones below!

visreg(mod.frt.7, "climate1", by="prj_g_1", scale="response")
visreg(mod.frt.7, "climate1", by="vegtype", scale="response")
```
...
probabilities <- predict(mod.frt.7, type = "response")
...
mod.frt.7<-glm(fire ~ poly(climate1,2) + prj_g_1 + vegtype, data=dat2, family=binomial(link="logit"))
summary(mod.frt.7)
Anova(mod.frt.7)

dat2_b<-dat2 %>% dplyr::select(climate1, prj_g_1)
probabilities <- predict(mod.frt.7, type = "response")
predicted.classes <- ifelse(probabilities > 0.5, "pos", "neg")
...
library(broom)
model.data <- augment(mod.frt.7) %>% 
  mutate(index = 1:n()) 

#The data for the top 3 largest values, according to the Cook’s distance, can be displayed as follow:
model.data %>% 
  dplyr::select(prj_g_1, vegtype, .std.resid, .cooksd) %>%
  top_n(5, .cooksd) 
...
```{r}
mod.frt.7_final<-glm(fire ~ poly(climate1,2) + prj_g_1 + vegtype, data=dat2, family=binomial(link="logit"))
summary(mod.frt.7_final)

### Results of summary
# Call:
# glm(formula = fire ~ poly(climate1, 2) + prj_g_1 + vegtype, family = binomial(link = "logit"), 
#     data = dat2)
# 
# Deviance Residuals: 
#     Min       1Q   Median       3Q      Max  
# -1.2976  -0.6512  -0.5198  -0.3917   2.3810  
# 
# Coefficients:
#                     Estimate Std. Error z value             Pr(>|z|)    
# (Intercept)        -2.967531   0.288116 -10.300 < 0.0000000000000002 ***
# poly(climate1, 2)1 -2.961323   2.580704  -1.147             0.251180    
# poly(climate1, 2)2  7.267196   2.488357   2.920             0.003495 ** 
# prj_g_1             0.008718   0.002308   3.777             0.000158 ***
# vegtypeTC           0.740258   0.232082   3.190             0.001425 ** 
# vegtypeTM           0.458417   0.309143   1.483             0.138111    
# ---
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
# 
# (Dispersion parameter for binomial family taken to be 1)
# 
#     Null deviance: 726.24  on 799  degrees of freedom
# Residual deviance: 687.27  on 794  degrees of freedom
# AIC: 699.27
# 
# Number of Fisher Scoring iterations: 4

# model diagnostic plots
binnedplot (fitted(mod.frt.7_final), 
            residuals(mod.frt.7_final), 
            nclass = NULL, 
            xlab = "Expected Values", 
            ylab = "Average residual", 
            main = paste("Binned Residual Plot - glm"))
...
dat2$resids<-resid(mod.frt.7_final)
...
##Partial Residuals
visreg(mod.frt.7_final, scale="response") # nicer plots than the ones below!
...
top_mod_table_FRT5_light_t_ALL <- data.frame (matrix (ncol = 11, nrow = 0))
colnames (top_mod_table_FRT5_light_t_ALL ) <- c ("CAUSE", "FRT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_climate_1_sqr", "coef_proj_age_1", "coef_vegtypeTC", "coef_vegtypeTM", "AUC")
...
mod.valid <- predict.glm(mod.frt.7_final, newdata=Valid, type="response")
   roc_obj <- roc(Valid[,"fire"], mod.valid, quiet=TRUE)
   mod.auc <- auc(roc_obj)

# create model table (only do this once) and add the relevant data
top_mod_table_NDT1_light_t <- data.frame (matrix (ncol = 11, nrow = 0))
colnames (top_mod_table_NDT1_light_t ) <- c ("CAUSE", "FRT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_climate_1_sqr", "coef_proj_age_1", "coef_vegtypeTC", "coef_vegtypeTM", "AUC")

##Add data for NDT1
top_mod_table_NDT1_light_t[1,1]<-"lightning"
top_mod_table_NDT1_light_t[1,2]<-"FRT7"
top_mod_table_NDT1_light_t[1,3]<-"Y"
top_mod_table_NDT1_light_t[1,4]<-"fire ~ poly(climate1,2) + prj_g_1 + vegtype"
top_mod_table_NDT1_light_t[1,5]<- coef(mod.frt.7_final)[1] #Intercept
top_mod_table_NDT1_light_t[1,6]<- coef(mod.frt.7_final)[2] #Climate variable 1
top_mod_table_NDT1_light_t[1,7]<- coef(mod.frt.7_final)[3] #Climate variable 2
top_mod_table_NDT1_light_t[1,8]<- coef(mod.frt.7_final)[4] #I
top_mod_table_NDT1_light_t[1,9]<- coef(mod.frt.7_final)[5] #co
top_mod_table_NDT1_light_t[1,10]<- coef(mod.frt.7_final)[6] #coe
top_mod_table_NDT1_light_t[1,11]<- mod.auc
...
```{r}
write.csv(top_mod_table_FRT5_light_t_ALL, file="D:\\Fire\\fire_data\\raw_data\\Ignition Models\\Ignition_Model_Results_Lightning_Treed\\top_mod_table_FRT7_light_tree_all.csv")
```
...
#  intercept coef_climate_1 coef_climate_1_sqr coef_proj_age_1 coef_vegtypeTC coef_vegtypeTM       AUC
# -2.967531      -2.961323           7.267196     0.008718437      0.7402582      0.4584168 0.6719222
