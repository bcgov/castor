...
#https://www.google.com/search?q=partial+residual+for+logistic+regression+R&rlz=1C1GCEB_enCA957CA957&oq=partial+residual+for+logistic+regression+R&aqs=chrome..69i57j33i160l3.6682j1j7&sourceid=chrome&ie=UTF-8

#did not seem to work: https://www.rdocumentation.org/packages/asbio/versions/1.6-7/topics/partial.resid.plot
library(asbio)
partial.resid.plot(model.NDT1.S, smooth.span = 0.8, lf.col = 2, sm.col = 4)
asbio::partial.resid.plot(model.NDT1.S)

termplot(model.NDT1.S)
termplot(model.NDT1.S, partial.resid=TRUE, col.res = "purple")

NDT1_spread$resids<-resid(model.NDT1.S)

##
pterms <- predict(model.NDT1.S, type="terms")
pterms[1,] # compare to above; the same

#Create partial residuals
#COuld not get to work: https://www.clayford.net/statistics/partial-residuals-and-the-termplot-function/
partial.residuals <- apply(pterms,2,function(x)x+resid(model.NDT1.S))

par(mfrow=c(3,4))
for(i in 1:11){
  plot(x=model.NDT1.S$model[,(i+1)],y=partial.residuals[,i], col="purple", ylim=c(0,100), xlim=c(0,100))
  abline(lm(partial.residuals[,i] ~ model.NDT1.S$model[,(i+1)]), col="red")
}


termplot(model.NDT1.S, partial.resid=TRUE, col.res = "purple", smooth=panel.smooth)

#Try visreg: visreg is an R package for displaying the results of a fitted model in terms of how a predictor variable x is estimated to affect an outcome y.
#Useful link: https://cran.r-project.org/web/packages/visreg/vignettes/quick-start.html
#Somewhat useful: https://pbreheny.github.io/visreg/
...
Note: the rank deficient warning likely comes from the fact that the water category in vegtype2 and in bclcls_level_5_2 are the same.
...
```{r}
write.csv(top_mod_table_NDT1_spread_poly, file="D:\\Fire\\fire_data\\raw_data\\top_mod_spread_NDT1_light_t_ALL_.csv")
```
...
```{r}
write.csv(top_mod_table_NDT1_spread_poly_means, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT1_spread_poly_means_.csv")
```
...
```{r}
write.csv(top_mod_table_NDT1_spread_poly_sd, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT1_spread_poly_sd_.csv")
```
