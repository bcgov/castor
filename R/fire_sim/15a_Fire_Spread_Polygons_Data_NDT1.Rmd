...
table(spread_poly_NDT1$vegtype2, spread_poly_NDT1$bclcs_level_5_2) #Use landuse and not vegtype?
...
#Run model using dat1
model.NDT1.S<-glm(spread ~ slope + aspect_cos + elev + slope:aspect_cos + slope:elev + dist_infra + roads_km + bclcs_level_5_2 + dist_infra:roads_km + dist_infra:bclcs_level_5_2 + roads_km:bclcs_level_5_2 + vegtype2 + bclcs_level_2, family = binomial, data = dat1)

AIC(model.NDT1.S) #61028.2
summary(model.NDT1.S)
...
#Remove least signiicant
model.NDT1.S<-glm(spread ~ slope + aspect_cos + elev + slope:aspect_cos + slope:elev + dist_infra + roads_km + bclcs_level_5_2 + dist_infra:bclcs_level_5_2 + roads_km:bclcs_level_5_2 + vegtype2 + bclcs_level_2, family = binomial, data = dat1)

AIC(model.NDT1.S) #61026.2
summary(model.NDT1.S)
...
#Try without vegtype since it overlaps with landuse for some categories
model.NDT1.S<-glm(spread ~ slope + aspect_cos + elev + slope:aspect_cos + slope:elev + dist_infra + roads_km + bclcs_level_5_2 + dist_infra:bclcs_level_5_2 + roads_km:bclcs_level_5_2 + bclcs_level_2, family = binomial, data = dat1)

AIC(model.NDT1.S) #61050.0
summary(model.NDT1.S)
...
#Keep vegtype in, but some categories will be NA because covered by other variable
model.NDT1.S<-glm(spread ~ slope + aspect_cos + elev + slope:aspect_cos + slope:elev + dist_infra + roads_km + bclcs_level_5_2 + dist_infra:bclcs_level_5_2 + roads_km:bclcs_level_5_2 + vegtype2 + bclcs_level_2, family = binomial, data = dat1)
...
```{r}
#Remove NAs to ensure all same data used so we can compare AICs
NDT1_spread<-spread_poly_NDT1 %>% drop_na(slope, aspect_cos, elev, dist_infra, roads_km, bclcs_level_5_2, vegtype2, bclcs_level_2) #

#Run Model again with this data; but uses all data here
model.NDT1.S<-glm(spread ~ slope + aspect_cos + elev + slope:aspect_cos + slope:elev + dist_infra + roads_km + bclcs_level_5_2 + dist_infra:bclcs_level_5_2 + roads_km:bclcs_level_5_2 + vegtype2 + bclcs_level_2, family = binomial, data = NDT1_spread)
...
binnedplot (fitted(model.NDT1.S), 
            ...
            ylab = "Average residual", 
            main = paste("Binned Residual Plot - glm"))
#Uneven, with more negatives than positives


#Partial residual plots
#spread ~ slope + aspect_cos + elev + slope:aspect_cos + slope:elev + dist_infra + roads_km + bclcs_level_5_2 + dist_infra:bclcs_level_5_2 + roads_km:bclcs_level_5_2 + vegtype2 + bclcs_level_2
...
visreg(model.NDT1.S, "dist_infra", by="bclcs_level_5_2")
visreg(model.NDT1.S, "dist_infra", by="bclcs_level_5_2", ylim=c(-10,20))
visreg(model.NDT1.S, "roads_km", by="bclcs_level_5_2")
visreg(model.NDT1.S, "roads_km", by="bclcs_level_5_2", ylim=c(-20,20))

visreg(model.NDT1.S, "bclcs_level_5_2", by="roads_km")
visreg(model.NDT1.S, "bclcs_level_5_2", by="roads_km", ylim=c(-5,20))
visreg(model.NDT1.S, "bclcs_level_5_2", by="dist_infra")
visreg(model.NDT1.S, "bclcs_level_5_2", by="dist_infra", ylim=c(-5,20))

visreg(model.NDT1.S, "bclcs_level_2")
visreg(model.NDT1.S, "vegtype2")
visreg(model.NDT1.S, "vegtype2", ylim=c(-20,5))

#Looking at above, I would suggest we do not need interactions with landuse and roads or dist_infra
...
```{r}
#Run Model again with this data; but uses all data here
model.NDT1.S<-glm(spread ~ slope + aspect_cos + elev + slope:aspect_cos + slope:elev + dist_infra + roads_km + bclcs_level_5_2 + vegtype2 + bclcs_level_2, family = binomial, data = NDT1_spread)
...
#Partial residual plots
#spread ~ slope + aspect_cos + elev + slope:aspect_cos + slope:elev + dist_infra + roads_km + bclcs_level_5_2 + vegtype2 + bclcs_level_2
...
visreg(model.NDT1.S, "dist_infra")
visreg(model.NDT1.S, "roads_km")
visreg(model.NDT1.S, "bclcs_level_5_2")
visreg(model.NDT1.S, "bclcs_level_5_2", ylim=c(-5,10))

visreg(model.NDT1.S, "bclcs_level_2")
visreg(model.NDT1.S, "vegtype2")
visreg(model.NDT1.S, "vegtype2", ylim=c(-10,5))
...
#Old table
#Create a new blank table and get AUC too
top_mod_table_NDT1_spread_poly <- data.frame (matrix (ncol = 29, nrow = 0))
colnames (top_mod_table_NDT1_spread_poly ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_slope", "coef_aspect_cos", "coef_elev", "coef_dist_infra", "coef_roads_km", "coef_bclcs_level_5_2OP", "coef_bclcs_level_5_2ROCK", "coef_bclcs_level_5_2SOIL", "coef_bclcs_level_5_2SP", "coef_bclcs_level_5_2UR", "coef_bclcs_level_5_2WATER", "coef_vegtype2OP", "coef_vegtype2RO", "coef_vegtype2S", "coef_vegtype2TB", "coef_vegtype2TC", "coef_vegtype2TM", "coef_vegtype2W", "bclcs_level_2N", "bclcs_level_2T", "bclcs_level_2W", "coef_slope:aspect_cos", "coef_slope:elev", "AUC")
...
#Model   
model.NDT1.S<-glm(spread ~ slope + aspect_cos + elev + slope:aspect_cos + slope:elev + dist_infra + roads_km + bclcs_level_5_2 + vegtype2 + bclcs_level_2, family = binomial, data = dat1) 
...
# create model table (only do this once) and add the relevant data
top_mod_table_NDT1_spread <- data.frame (matrix (ncol = 29, nrow = 0))
colnames (top_mod_table_NDT1_spread ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_slope", "coef_aspect_cos", "coef_elev", "coef_dist_infra", "coef_roads_km", "coef_bclcs_level_5_2OP", "coef_bclcs_level_5_2ROCK", "coef_bclcs_level_5_2SOIL", "coef_bclcs_level_5_2SP", "coef_bclcs_level_5_2UR", "coef_bclcs_level_5_2WATER", "coef_vegtype2OP", "coef_vegtype2RO", "coef_vegtype2S", "coef_vegtype2TB", "coef_vegtype2TC", "coef_vegtype2TM", "coef_vegtype2W", "bclcs_level_2N", "bclcs_level_2T", "bclcs_level_2W", "coef_slope:aspect_cos", "coef_slope:elev", "AUC")
...
top_mod_table_NDT1_spread[1,2]<-"NDT1"
top_mod_table_NDT1_spread[1,3]<-"ALL Treed and not treed"
top_mod_table_NDT1_spread[1,4]<-"spread ~ slope + aspect_cos + elev + slope:aspect_cos + slope:elev + dist_infra + roads_km + bclcs_level_5_2 + vegtype2 + bclcs_level_2" 
top_mod_table_NDT1_spread[1,5]<- coef(model.NDT1.S)[1] #Intercept
top_mod_table_NDT1_spread[1,6]<- coef(model.NDT1.S)[2] #
top_mod_table_NDT1_spread[1,7]<- coef(model.NDT1.S)[3] #
top_mod_table_NDT1_spread[1,8]<- coef(model.NDT1.S)[4] #
...
top_mod_table_NDT1_spread[1,26]<- coef(model.NDT1.S)[22] #
top_mod_table_NDT1_spread[1,27]<- coef(model.NDT1.S)[23] #
top_mod_table_NDT1_spread[1,28]<- coef(model.NDT1.S)[24] #
top_mod_table_NDT1_spread[1,29]<- mod.auc
...
```{r}
write.csv(top_mod_table_NDT1_spread_poly, file="D:\\Fire\\fire_data\\raw_data\\top_mod_NDT1_spread_poly_all_Jan.csv")
```
...
top_mod_table_NDT1_spread_poly_means[1,2]<-"NDT1"
top_mod_table_NDT1_spread_poly_means[1,3]<-"ALL Treed and not treed"
top_mod_table_NDT1_spread_poly_means[1,4]<- "spread ~ slope + aspect_cos + elev + slope:aspect_cos + slope:elev + dist_infra + roads_km + bclcs_level_5_2 + vegtype2 + bclcs_level_2" 
top_mod_table_NDT1_spread_poly_means
```
Save table.

```{r}
write.csv(top_mod_table_NDT1_spread_poly_means, file="D:\\Fire\\fire_data\\raw_data\\top_mod_NDT1_spread_poly_means_Jan.csv")
```
...
top_mod_table_NDT1_spread_poly_sd[1,2]<-"NDT1"
top_mod_table_NDT1_spread_poly_sd[1,3]<-"ALL Treed and not treed"
top_mod_table_NDT1_spread_poly_sd[1,4]<-"spread ~ slope + aspect_cos + elev + slope:aspect_cos + slope:elev + dist_infra + roads_km + bclcs_level_5_2 + vegtype2 + bclcs_level_2" 
top_mod_table_NDT1_spread_poly_sd
```
...
```{r}
write.csv(top_mod_table_NDT1_spread_poly_sd, file="D:\\Fire\\fire_data\\raw_data\\top_mod_NDT1_spread_poly_sd_Jan.csv")
```
