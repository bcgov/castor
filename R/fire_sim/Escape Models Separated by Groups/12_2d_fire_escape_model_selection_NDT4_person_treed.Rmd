---
title: "12_2d_fire_escape_model_selection_NDT4_person_treed"
author: "Cora Skaien"
date: "18/11/2021"
...
#=================================
#  Script Name: 12_2d_fire_escape_model_selection_NDT4_person_treed.R
#  Script Version: 1.0
...
AIC_person_NDT4_escape_treed_climate<-table.glm.climate.simple

AIC_person_NDT4_escape_treed_summary_climate<- AIC_person_NDT4_escape_treed_climate %>%
  group_by(model) %>%
...
AIC_person_NDT4_escape_treed_summary_climate2<- AIC_person_NDT4_escape_treed_summary_climate %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT4_escape_treed_summary_climate2)
```
...
AIC_person_NDT4_escape_treed_VRI<-table.glm.VRI.simple

AIC_person_NDT4_escape_treed_summary_VRI<- AIC_person_NDT4_escape_treed_VRI %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic),
            meanAUC=mean(auc.valid),
            sdAUC=sd(auc.valid),
...
AIC_person_NDT4_escape_treed_summary_VRI2<- AIC_person_NDT4_escape_treed_summary_VRI %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT4_escape_treed_summary_VRI2)
```
...
AIC_person_NDT4_escape_treed_topo<-table.glm.topo.simple

AIC_person_NDT4_escape_treed_summary_topo<- AIC_person_NDT4_escape_treed_topo %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic),
            meanAUC=mean(auc.valid),
            sdAUC=sd(auc.valid),
...
AIC_person_NDT4_escape_treed_summary_topo2<- AIC_person_NDT4_escape_treed_summary_topo %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT4_escape_treed_summary_topo2)
```
...
AIC_person_NDT4_escape_treed_infra<-table.glm.infra.simple

AIC_person_NDT4_escape_treed_summary_infra<- AIC_person_NDT4_escape_treed_infra %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic),
            meanAUC=mean(auc.valid),
            sdAUC=sd(auc.valid),
...
AIC_person_NDT4_escape_treed_summary_infra2<- AIC_person_NDT4_escape_treed_summary_infra %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT4_escape_treed_summary_infra2)
...
```{r}
NDT4_l_models_treed<-rbind(AIC_person_NDT4_escape_treed_summary_climate2, AIC_person_NDT4_escape_treed_summary_VRI2, AIC_person_NDT4_escape_treed_summary_topo2, AIC_person_NDT4_escape_treed_summary_infra2)
NDT4_l_models_treed
NDT4_l_models_treed$NDT<-"NDT4"

write.csv(NDT4_l_models_treed, file="D:\\Fire\\fire_data\\raw_data\\NDT4_person_models_treed_escape.csv")
```
...
Top Models:
1. climate1 + vegtype2
2. proj_age_1 + live_stand_volume_125
3. slope + elevation + wind_atfire + slope:wind_atfire + elevation:wind_atfire
...
```{r}
escape_person_t_NDT4<-subset(Escape_data_person_t, Escape_data_person_t$ntrl_ds=="NDT4") #4639

#Divide data into training and valid
prop<-0.75
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(escape_person_t_NDT4$escape, p = prop,
                                    list = FALSE,
                                    times = 1)
  
   dat1 <- escape_person_t_NDT4[ trainIndex,]
   Valid <- escape_person_t_NDT4[-trainIndex,]

#Run model using dat1
model.NDT4.E<-glm(escape ~ climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + elevation + wind_atfire + slope:wind_atfire + elevation:wind_atfire + dist_mun + dist_dam + dist_nat + dist_mine + roads_km + Tdif_atfire + bclcs_level_5_2  + elevation*climate1 + wind_atfire*climate1 + wind_atfire*vegtype2 + Tdif_atfire*climate1 + Tdif_atfire*wind_atfire +  bclcs_level_5_2*dist_mun +  bclcs_level_5_2*dist_dam + bclcs_level_5_2*dist_pow + bclcs_level_5_2*dist_mine + bclcs_level_5_2*roads_km + dist_mun*roads_km + dist_dam*roads_km + dist_pow*roads_km + dist_mine*roads_km, family = binomial, data = dat1)

AIC(model.NDT4.E) #1908.7
...
model.NDT4.E<-glm(escape ~ climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + elevation + wind_atfire + slope:wind_atfire + elevation:wind_atfire + dist_mun + dist_dam + dist_nat + dist_mine + roads_km + Tdif_atfire + bclcs_level_5_2  + elevation*climate1 + wind_atfire*climate1 + Tdif_atfire*climate1 + Tdif_atfire*wind_atfire +  bclcs_level_5_2*dist_mun + bclcs_level_5_2*dist_mine + bclcs_level_5_2*roads_km + dist_mun*roads_km + dist_dam*roads_km + dist_pow*roads_km + dist_mine*roads_km, family = binomial, data = dat1)

AIC(model.NDT4.E) #1897.7
...
#Remove least significant interaction
model.NDT4.E<-glm(escape ~ climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + elevation + wind_atfire + slope:wind_atfire + elevation:wind_atfire + dist_mun + dist_dam + dist_nat + dist_mine + roads_km + Tdif_atfire + bclcs_level_5_2 + wind_atfire*climate1 + Tdif_atfire*climate1 + Tdif_atfire*wind_atfire + bclcs_level_5_2*dist_mine + bclcs_level_5_2*roads_km + dist_mun*roads_km + dist_dam*roads_km + dist_pow*roads_km + dist_mine*roads_km, family = binomial, data = dat1)

AIC(model.NDT4.E) #1893.2
...
#Remove least significant interaction
model.NDT4.E<-glm(escape ~ climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + elevation + wind_atfire + slope:wind_atfire + elevation:wind_atfire + dist_mun + dist_dam + dist_nat + dist_mine + roads_km + Tdif_atfire + bclcs_level_5_2 + wind_atfire*climate1 + Tdif_atfire*climate1 + Tdif_atfire*wind_atfire + bclcs_level_5_2*dist_mine + dist_mun*roads_km + dist_dam*roads_km + dist_pow*roads_km + dist_mine*roads_km, family = binomial, data = dat1)

AIC(model.NDT4.E) #1890.9
...
#Remove least significant interaction
model.NDT4.E<-glm(escape ~ climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + elevation + wind_atfire + slope:wind_atfire + elevation:wind_atfire + dist_mun + dist_dam + dist_nat + dist_mine + roads_km + Tdif_atfire + bclcs_level_5_2 + wind_atfire*climate1 + Tdif_atfire*climate1 + Tdif_atfire*wind_atfire + bclcs_level_5_2*dist_mine + dist_mun*roads_km + dist_pow*roads_km + dist_mine*roads_km, family = binomial, data = dat1)

AIC(model.NDT4.E) #1889.4
...
#Remove least significant interaction
model.NDT4.E<-glm(escape ~ climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + elevation + wind_atfire + slope:wind_atfire + elevation:wind_atfire + dist_mun + dist_dam + dist_nat + dist_mine + roads_km + Tdif_atfire + bclcs_level_5_2 + wind_atfire*climate1 + Tdif_atfire*climate1 + Tdif_atfire*wind_atfire + dist_mun*roads_km + dist_pow*roads_km + dist_mine*roads_km, family = binomial, data = dat1)

AIC(model.NDT4.E) #1888.1

#Determine AUC of full model
mod.valid <- predict.glm(model.NDT4.E, newdata=Valid, type="response")
   roc_obj <- roc(Valid[,"escape"], mod.valid)
   mod.auc <- auc(roc_obj)
   mod.auc #0.76 --> good fit!
   
Anova(model.NDT4.E, type=3)
#Anova(model.NDT4.E, type=3, singular.ok = TRUE)

#Remove least significant
model.NDT4.E<-glm(escape ~ climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + elevation + wind_atfire + slope:wind_atfire + dist_mun + dist_dam + dist_nat + dist_mine + roads_km + Tdif_atfire + bclcs_level_5_2 + wind_atfire*climate1 + Tdif_atfire*climate1 + Tdif_atfire*wind_atfire + dist_mun*roads_km + dist_pow*roads_km + dist_mine*roads_km, family = binomial, data = dat1)

AIC(model.NDT4.E) #1887.7
...
#Remove least significant
model.NDT4.E<-glm(escape ~ climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + elevation + wind_atfire + slope:wind_atfire + dist_mun + dist_dam + dist_nat + dist_mine + roads_km + Tdif_atfire + bclcs_level_5_2 + wind_atfire*climate1 + Tdif_atfire*climate1 + Tdif_atfire*wind_atfire + dist_pow*roads_km + dist_mine*roads_km, family = binomial, data = dat1)

AIC(model.NDT4.E) #1887.4
...
#Remove least significant
model.NDT4.E<-glm(escape ~ climate1 + vegtype2 + live_stand_volume_125 + slope + wind_atfire + slope:wind_atfire + dist_mun + dist_dam + dist_nat + dist_mine + dist_pow + roads_km + Tdif_atfire + bclcs_level_5_2 + wind_atfire*climate1 + Tdif_atfire*climate1 + Tdif_atfire*wind_atfire + dist_pow*roads_km + dist_mine*roads_km, family = binomial, data = dat1)

AIC(model.NDT4.E) #1884.1
...
NDT4_escape_t<-escape_person_t_NDT4 %>% drop_na(climate1, vegtype2, live_stand_volume_125, dist_dam, dist_nat, dist_mun, dist_mine, slope, wind_atfire, Tdif_atfire, roads_km, bclcs_level_5_2 ) #4387 observations

#Run Model again with this data; but uses all data here
model.NDT4.E<-glm(escape ~ climate1 + vegtype2 + live_stand_volume_125 + slope + wind_atfire + slope:wind_atfire + dist_mun + dist_dam + dist_nat + dist_mine + dist_pow + roads_km + Tdif_atfire + bclcs_level_5_2 + wind_atfire*climate1 + Tdif_atfire*climate1 + Tdif_atfire*wind_atfire + dist_pow*roads_km + dist_mine*roads_km, family = binomial, data = NDT4_escape_t)
...
#Partial Residuals
#climate1 + vegtype2 + live_stand_volume_125 + slope + wind_atfire + slope:wind_atfire + dist_mun + dist_dam + dist_nat + dist_mine + dist_pow + roads_km + Tdif_atfire + bclcs_level_5_2 + wind_atfire*climate1 + Tdif_atfire*climate1 + Tdif_atfire*wind_atfire + dist_pow*roads_km + dist_mine*roads_km

#climate1 + vegtype2 + live_stand_volume_125 + slope + wind_atfire + slope:wind_atfire +  Tdif_atfire  + wind_atfire*climate1 + Tdif_atfire*climate1 + Tdif_atfire*wind_atfire 
visreg(model.NDT4.E, "climate1", by="wind_atfire")
visreg(model.NDT4.E, "climate1", by="Tdif_atfire")

visreg(model.NDT4.E, "vegtype2")
visreg(model.NDT4.E, "live_stand_volume_125")
visreg(model.NDT4.E, "slope", by="wind_atfire")

visreg(model.NDT4.E, "Tdif_atfire", by="climate1")
visreg(model.NDT4.E, "Tdif_atfire", by="wind_atfire")

visreg(model.NDT4.E, "wind_atfire", by="climate1")
visreg(model.NDT4.E, "wind_atfire", by="Tdif_atfire")
visreg(model.NDT4.E, "wind_atfire", by="slope")

#dist_mun + dist_dam + dist_nat + dist_mine + dist_pow + roads_km + bclcs_level_5_2 + dist_pow*roads_km + dist_mine*roads_km
visreg(model.NDT4.E, "dist_mine", by="roads_km")
visreg(model.NDT4.E, "dist_pow", by="roads_km")
visreg(model.NDT4.E, "dist_mun")
visreg(model.NDT4.E, "dist_dam")
visreg(model.NDT4.E, "dist_nat")
visreg(model.NDT4.E, "roads_km", by="dist_mine")
visreg(model.NDT4.E, "roads_km", by="dist_pow")
visreg(model.NDT4.E, "bclcs_level_5_2")
...
top_mod_table_NDT4_person_t_ALL <- data.frame (matrix (ncol = 28, nrow = 0))
colnames (top_mod_table_NDT4_person_t_ALL ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_vegtypeTB", "coef_vegtypeTC", "coef_vegtypeTM", "coef_live_stand_volume_125",  "coef_slope", "coef_wind_atfire", "coef_dist_mun", "coef_dist_dam", "coef_dist_nat", "coef_dist_mine", "coef_dist_pow", "coef_roads_km", "coef_Tdif_atfire", "coef_bclcs_level_5_2OP", "coef_bclcs_level_5_2SP", "coef_slope:wind_atfire", "coef_climate1:wind_atfire", "coef_climate1:Tdif_atfire", "coef_wind_atfire:Tdif_atfire", "coef_dist_pow:roads_km", "coef_dist_mine:roads_km", "AUC")
```
...
top_mod_table_NDT4_person_t <- data.frame (matrix (ncol = 28, nrow = 0))
colnames (top_mod_table_NDT4_person_t ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_vegtypeTB", "coef_vegtypeTC", "coef_vegtypeTM", "coef_live_stand_volume_125",  "coef_slope", "coef_wind_atfire", "coef_dist_mun", "coef_dist_dam", "coef_dist_nat", "coef_dist_mine", "coef_dist_pow", "coef_roads_km", "coef_Tdif_atfire", "coef_bclcs_level_5_2OP", "coef_bclcs_level_5_2SP", "coef_slope:wind_atfire", "coef_climate1:wind_atfire", "coef_climate1:Tdif_atfire", "coef_wind_atfire:Tdif_atfire", "coef_dist_pow:roads_km", "coef_dist_mine:roads_km", "AUC")

##Add data for NDT4
top_mod_table_NDT4_person_t[1,1]<-"person"
top_mod_table_NDT4_person_t[1,2]<-"NDT4"
top_mod_table_NDT4_person_t[1,3]<-"Y"
top_mod_table_NDT4_person_t[1,4]<-"escape ~ climate1 + vegtype2 + live_stand_volume_125 + slope + wind_atfire + slope:wind_atfire + dist_mun + dist_dam + dist_nat + dist_mine + dist_pow + roads_km + Tdif_atfire + bclcs_level_5_2 + wind_atfire*climate1 + Tdif_atfire*climate1 + Tdif_atfire*wind_atfire + dist_pow*roads_km + dist_mine*roads_km" 
top_mod_table_NDT4_person_t[1,5]<- coef(model.NDT4.E)[1] #Intercept
top_mod_table_NDT4_person_t[1,6]<- coef(model.NDT4.E)[2] #C
top_mod_table_NDT4_person_t[1,7]<- coef(model.NDT4.E)[3] #C
top_mod_table_NDT4_person_t[1,8]<- coef(model.NDT4.E)[4] #co
top_mod_table_NDT4_person_t[1,9]<- coef(model.NDT4.E)[5] #co
top_mod_table_NDT4_person_t[1,10]<- coef(model.NDT4.E)[6] #c
top_mod_table_NDT4_person_t[1,11]<- coef(model.NDT4.E)[7] #l
top_mod_table_NDT4_person_t[1,12]<- coef(model.NDT4.E)[8] #co
top_mod_table_NDT4_person_t[1,13]<- coef(model.NDT4.E)[9] #c
top_mod_table_NDT4_person_t[1,14]<- coef(model.NDT4.E)[10] #coeff
top_mod_table_NDT4_person_t[1,15]<- coef(model.NDT4.E)[11] #co
top_mod_table_NDT4_person_t[1,16]<- coef(model.NDT4.E)[12] #coe
top_mod_table_NDT4_person_t[1,17]<- coef(model.NDT4.E)[13] #co
top_mod_table_NDT4_person_t[1,18]<- coef(model.NDT4.E)[14] #co
top_mod_table_NDT4_person_t[1,19]<- coef(model.NDT4.E)[15] #
top_mod_table_NDT4_person_t[1,20]<- coef(model.NDT4.E)[16] # cl
top_mod_table_NDT4_person_t[1,21]<- coef(model.NDT4.E)[17] # c
top_mod_table_NDT4_person_t[1,22]<- coef(model.NDT4.E)[18] #coefficien
top_mod_table_NDT4_person_t[1,23]<- coef(model.NDT4.E)[19] #coefficien
top_mod_table_NDT4_person_t[1,24]<- coef(model.NDT4.E)[20] #coefficien
top_mod_table_NDT4_person_t[1,25]<- coef(model.NDT4.E)[21] #coefficien
top_mod_table_NDT4_person_t[1,26]<- coef(model.NDT4.E)[22] #coefficien
top_mod_table_NDT4_person_t[1,27]<- coef(model.NDT4.E)[23] #coefficien
top_mod_table_NDT4_person_t[1,28]<- mod.auc

top_mod_table_NDT4_person_t_ALL<-rbind(top_mod_table_NDT4_person_t_ALL, top_mod_table_NDT4_person_t)
...
Check.
```{r}
head(top_mod_table_NDT4_person_t_ALL)
...
```{r}
write.csv(top_mod_table_NDT4_person_t_ALL, file="D:\\Fire\\fire_data\\raw_data\\top_mod_escape_NDT4_person_t_ALL.csv")
```
...
```{r}
names(top_mod_table_NDT4_person_t_ALL)

top_mod_table_NDT4_escape_person_t_means<-top_mod_table_NDT4_person_t_ALL %>% summarise_each(funs( mean( .,na.rm = TRUE)))
top_mod_table_NDT4_escape_person_t_means

top_mod_table_NDT4_escape_person_t_means[1,1]<-"person"
top_mod_table_NDT4_escape_person_t_means[1,2]<-"NDT4"
top_mod_table_NDT4_escape_person_t_means[1,3]<-"Treed"
top_mod_table_NDT4_escape_person_t_means[1,4]<- "escape ~ climate1 + vegtype2 + live_stand_volume_125 + slope + wind_atfire + slope:wind_atfire + dist_mun + dist_dam + dist_nat + dist_mine + dist_pow + roads_km + Tdif_atfire + bclcs_level_5_2 + wind_atfire*climate1 + Tdif_atfire*climate1 + Tdif_atfire*wind_atfire + dist_pow*roads_km + dist_mine*roads_km" 
top_mod_table_NDT4_escape_person_t_means
```
Save table.

```{r}
write.csv(top_mod_table_NDT4_escape_person_t_means, file="D:\\Fire\\fire_data\\raw_data\\top_mod_escape_NDT4_person_t_Means.csv")
```

Standard deviation.

```{r}
top_mod_table_NDT4_escape_person_t_sd<-top_mod_table_NDT4_person_t_ALL %>% summarise_each(funs( sd( .,na.rm = TRUE)))
top_mod_table_NDT4_escape_person_t_sd

top_mod_table_NDT4_escape_person_t_sd[1,1]<-"person"
top_mod_table_NDT4_escape_person_t_sd[1,2]<-"NDT4"
top_mod_table_NDT4_escape_person_t_sd[1,3]<-"Treed"
top_mod_table_NDT4_escape_person_t_sd[1,4]<-"escape ~ climate1 + vegtype2 + live_stand_volume_125 + slope + wind_atfire + slope:wind_atfire + dist_mun + dist_dam + dist_nat + dist_mine + dist_pow + roads_km + Tdif_atfire + bclcs_level_5_2 + wind_atfire*climate1 + Tdif_atfire*climate1 + Tdif_atfire*wind_atfire + dist_pow*roads_km + dist_mine*roads_km" 
top_mod_table_NDT4_escape_person_t_sd
```
...
```{r}
write.csv(top_mod_table_NDT4_escape_person_t_sd, file="D:\\Fire\\fire_data\\raw_data\\top_mod_escape_NDT4_person_t_SD.csv")
```
...
```{r}
##Create variable lists to be used in the model loop.
variables_all_NDT4<-c(climate1 = "climate1", climate2 = "climate2", vegtype2 = "vegtype2", live_stand_volume_125 = "live_stand_volume_125", slope = "slope", elevation ="elevation",  dist_dam = "dist_dam", roads_km = "roads_km", bclcs_level_5_2 = "bclcs_level_5_2", wind_atfire = "wind_atfire")
...
#variables_clim_VRI_NDT4<-c(climate1 = "climate1", climate2 = "climate2", vegtype2 = "vegtype2", live_stand_volume_125 = "live_stand_volume_125" )

#Only one climate variable
variables_clim_VRI_NDT4<-c(climate1 = "climate1", vegtype2 = "vegtype2", live_stand_volume_125 = "live_stand_volume_125" )
...
variables_all_infra_landuse_NDT4<-c(dist_mun = "dist_mun", dist_dam = "dist_dam", dist_nat = "dist_nat", dist_mine = "dist_mine", roads_km = "roads_km", bclcs_level_5_2 = "bclcs_level_5_2") #I don't think it makes sense to have interactions between the distance elements, but maybe with land use and each distance element?
...
#Get variables on own
#complete list of models using non-climate vars
mods.me.tmp <- powerSet(inputs.me.NDT4) 
#add climate vars to all of the above
mods.me.NDT4 <- list()
for (i in 1: length(mods.me.tmp)) {
...
#mods.me.NDT4
length(mods.me.NDT4) #1024
mods.me.NDT4[1]
mods.me.NDT4<-mods.me.NDT4[-1]

#complete list of two-way interactions
#mods.twoway.NDT4 <- powerSet(twoway.ints) # Do not run; will take hours if it succeeds at all
...
length(mods.me.clim_VRI_NDT4) #8

#complete list of two-way interactions
mods.twoway.NDT4 <- powerSet(twoway.ints) # 
length(mods.twoway.NDT4) #8
mods.twoway.NDT4
mods.twoway.NDT4<-mods.twoway.NDT4[-1]
...
length(mods.me.clim_VRI_NDT4) #8
length(all.poss.mods.clim_VRI_NDT4) #19
#all.poss.mods.clim_VRI_NDT4


##Check and rid of any duplicated models
duplicated(all.poss.mods.clim_VRI_NDT4) #Need to remove 
#all.poss.mods.clim_VRI_NDT4b<-all.poss.mods.clim_VRI_NDT4[-(65:127)]
#duplicated(all.poss.mods.clim_VRI_NDT4b) 
#length(all.poss.mods.clim_VRI_NDT4b) 

all.poss.mods.infra_landuse_NDT4<-c(1, mods.me.infra_landuse_NDT4, mods.inter.NDT4b)
all.poss.mods.infra_landuse_NDT4[2]
all.poss.mods.infra_landuse_NDT4<-all.poss.mods.infra_landuse_NDT4[-2]

duplicated(all.poss.mods.infra_landuse_NDT4)
...
all.poss.mods.clim_VRI_NDT4<-all.poss.mods.clim_VRI_NDT4[-2]
```
...
#Create empty table
table.glm.clim_VRI.NDT4 <- data.frame (matrix (ncol = 5, nrow = 0))
colnames (table.glm.clim_VRI.NDT4) <- c ("model", "edf", "aic", "auc.valid", "NDT")
...
for (i in 1: length(zones1)){
  print(paste((all.poss.mods.clim_VRI_NDT4[i]), (zones1[h]), sep=" "))
...
mods.fit <- lapply(all.poss.mods.clim_VRI_NDT4, big.mod, df.train=dat1, df.test=Valid)
...
table.glm.clim_VRI.NDT4<-rbind(table.glm.clim_VRI.NDT4, tab.sum.climate)
...
```{r}
head(table.glm.clim_VRI.NDT4)
table(table.glm.clim_VRI.NDT4$model) # 100 per model

AIC_person_NDT4_escape_treed_climate<-table.glm.clim_VRI.NDT4
...
Save.
```{r}
write.csv(AIC_person_NDT4_escape_treed_summary_climate2, file="D:\\Fire\\fire_data\\raw_data\\table.glm.clim_VRI.NDT4_ALL_escape.csv")
```

#Now repeat for all.poss.mods.infra_landuse_NDT4
...
for (i in 1: length(zones1)){
 ...
 # model_dat<- dat2 %>% dplyr::select(escape, escape, variables_all[i])
  model_dat<- dat2 %>% dplyr::select(escape, escape, !!variables_all_infra_landuse_NDT4)
  # Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(model_dat$escape, p = prop,
                                    list = FALSE,
                                    times = 1)
...
AIC_person_NDT4_escape_treed_infra<-table.glm.infra.lu.simple
...
```{r}
write.csv(AIC_person_NDT4_escape_treed_summary_infra2, file="D:\\Fire\\fire_data\\raw_data\\AIC_person_NDT4_escape_treed_summary_infra.csv")
```
