...
Now that the data is prepped from 05_VRI_data_prep, we can move on to the analysis for selecting the best climatic variable(s) for each Natural Disturbance Type (NDT) zone. The loop below will be run first for lightning caused fires, and then for person-caused fires.
...
#Below is not working. Although I can see the table online, I cannot bring it back in. Sometimes the loaded table is empty/blank, and other times it is there but cannot be called on.
