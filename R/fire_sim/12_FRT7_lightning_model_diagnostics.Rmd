...
table(dat7$FWI_veg, dat7$fire)

dat7_2<-dat7 %>% filter(!FWI_veg %in% c("N", "W"))
table(dat7_2$FWI_veg)

dat7_2$FWI_veg <- factor(dat7_2$FWI_veg, levels=c("C-2","C-1","C-3", "D-1/2","M-1/2", "O-1a/b"))

ggplot(data=dat7_2, mapping=aes(x=as.factor(fire), y=climate1))+
  geom_boxplot()

ggplot(data=dat7_2, mapping=aes(x=as.factor(fire), y=elevatn))+
  geom_boxplot()

cor.test(dat7_2$climate1, dat7_2$elevatn) # elevation and climate are correlated

head(dat7_2)

table(is.na(dat7_2$elevatn))

mod.frt.5<-glm(fire ~ climate1 + FWI_veg + elevatn, data=dat7_2, family=binomial(link="logit"))
...
mod.frt.5<-glm(fire ~ climate1 + FWI_veg, data=dat7_2, family=binomial(link="logit"))
summary(mod.frt.5)
Anova(mod.frt.5, type=3)
...
dat2_b<-dat7_2%>% dplyr::select(fire, elevatn, climate1)
...
colnames (top_mod_table_FRT10_lightningALL ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1", "coef_FWI_veg_C-1","coef_FWI_veg_C-3", "coef_FWI_veg_D-1/2", "coef_FWI_veg_M-1/2",  "coef_FWI_veg_O-1a/b", "AUC")

dat7_2$fire_veg<-paste(dat7_2$fire, dat7_2$FWI_veg)
```
...
#Model   
mod<-glm(fire ~ climate1 + FWI_veg, data=dat1, family=binomial(link="logit"))
...
colnames (top_mod_table_FRT10_lightningL ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1", "coef_FWI_veg_C-1","coef_FWI_veg_C-3", "coef_FWI_veg_D-1/2", "coef_FWI_veg_M-1/2",  "coef_FWI_veg_O-1a/b", "AUC")


##Add data for NDT1
top_mod_table_FRT10_lightningL[1,1]<-"lightning"
top_mod_table_FRT10_lightningL[1,2]<-"FRT7"
top_mod_table_FRT10_lightningL[1,3]<-"fire ~ climate1 + FWI_veg"
...
FRT10_summary_table_mean$Model_terms<-"fire ~ climate1 + FWI_veg"
