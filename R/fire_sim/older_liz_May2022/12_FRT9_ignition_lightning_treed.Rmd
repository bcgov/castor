...
```
For FRT 9 its mean_Tmax04_Tmax05_Tmax06_Tmax07_Tmax08_Tmax9
...
```
climate1 + prj_h_1 + prj_g_1 + dm_h_bc
...
dat2<- dat %>% dplyr::filter(frt == 9) %>%
  drop_na(prj_h_1)

table(dat2$vegtype, dat2$fire) # hmmm there are no D, TB or TM ignition occurences. Only TC... Guess that explains why vegetation type does not show up. Guess ill run the model with TC only.
...
mod.frt.9<-glm(fire ~ climate1 + prj_h_1 + prj_g_1 + dm_h_bc, data=dat2, family=binomial(link="logit"))
...
summary(mod.frt.9)
anova(mod.frt.9)
Anova(mod.frt.9, type=3)

# model diagnostic plots
binnedplot (fitted(mod.frt.9), 
            residuals(mod.frt.9), 
            nclass = NULL, 
...
##Partial Residuals
visreg(mod.frt.9, scale="response") # nicer plots than the ones below!

visreg(mod.frt.9, "climate1", by="prj_g_1", scale="response")
visreg(mod.frt.9, "climate1", by="prj_h_1", scale="response")
visreg(mod.frt.9, "climate1", by="dm_h_bc", scale="response")
visreg(mod.frt.9, "prj_g_1", by="prj_h_1", scale="response")
```
...
probabilities <- predict(mod.frt.9, type = "response")
...
# all variables should be approximatly linear which is mostly true except for maybe climate1? Im going to try to fix this by lookign at polynomials of climate1. Note that q, I(q^2) and I(q^3) will be correlated and correlated variables can cause problems. The use of poly() lets you avoid this by producing orthogonal polynomials, therefore I’m going to use poly().

mod.frt.9_1<-glm(fire ~ climate1 + prj_g_1 + poly(prj_h_1,1) + dm_h_bc, data=dat2, family=binomial(link="logit"))
mod.frt.9_2<-glm(fire ~ climate1 + prj_g_1 + poly(prj_h_1,2) + dm_h_bc, data=dat2, family=binomial(link="logit"))
mod.frt.9_3<-glm(fire ~ climate1 + prj_g_1 + poly(prj_h_1,3) + dm_h_bc, data=dat2, family=binomial(link="logit"))
mod.frt.9_4<-glm(fire ~ climate1 + prj_g_1 + poly(prj_h_1,4) + dm_h_bc, data=dat2, family=binomial(link="logit"))
anova(mod.frt.9_1,mod.frt.9_2,mod.frt.9_3,mod.frt.9_4, test="Chisq")
summary(mod.frt.9)
Anova(mod.frt.9)

dat2_b<-dat2 %>% dplyr::select(prj_h_1, prj_g_1)
probabilities <- predict(mod.frt.9_1, type = "response")
predicted.classes <- ifelse(probabilities > 0.5, "pos", "neg")
...
# looks like the polynomial is not really helping. Not sure how to deal with this??
mod.frt.9<-glm(fire ~ climate1 + prj_g_1 + prj_h_1 + dm_h_bc, data=dat2, family=binomial(link="logit"))
...
library(broom)
model.data <- augment(mod.frt.9) %>% 
  mutate(index = 1:n()) 

#The data for the top 3 largest values, according to the Cook’s distance, can be displayed as follow:
model.data %>% 
  dplyr::select(prj_g_1,climate1, .std.resid, .cooksd) %>%
  top_n(5, .cooksd) 
...
```{r}
mod.frt.9_final<-glm(fire ~ climate1 + prj_g_1 + prj_h_1 + dm_h_bc, data=dat2, family=binomial(link="logit"))
summary(mod.frt.9_final)

### Results of summary
# Call:
# glm(formula = fire ~ climate1 + prj_g_1 + prj_h_1 + dm_h_bc, 
#     family = binomial(link = "logit"), data = dat2)
# 
# Deviance Residuals: 
#     Min       1Q   Median       3Q      Max  
# -1.2024  -0.6559  -0.5002  -0.3373   2.4835  
# 
# Coefficients:
#              Estimate Std. Error z value Pr(>|z|)   
# (Intercept) -1.640867   3.801123  -0.432  0.66597   
# climate1     0.064706   0.164285   0.394  0.69368   
# prj_g_1      0.004868   0.002562   1.900  0.05743 . 
# prj_h_1      0.064098   0.024601   2.606  0.00917 **
# dm_h_bc     -0.002102   0.001505  -1.397  0.16248   
# ---
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
# 
# (Dispersion parameter for binomial family taken to be 1)
# 
#     Null deviance: 333.75  on 367  degrees of freedom
# Residual deviance: 308.45  on 363  degrees of freedom
# AIC: 318.45
# 
# Number of Fisher Scoring iterations: 5

# model diagnostic plots
binnedplot (fitted(mod.frt.9_final), 
            residuals(mod.frt.9_final), 
            nclass = NULL, 
            xlab = "Expected Values", 
            ylab = "Average residual", 
            main = paste("Binned Residual Plot - glm"))
...
dat2$resids<-resid(mod.frt.9_final)
...
##Partial Residuals
visreg(mod.frt.9_final, scale="response") # nicer plots than the ones below!
...
top_mod_table_FRT5_light_t_ALL <- data.frame (matrix (ncol = 10, nrow = 0))
colnames (top_mod_table_FRT5_light_t_ALL ) <- c ("CAUSE", "FRT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_proj_age_1", "coef_proj_height_1", "coef_elevation", "AUC")
...
mod.valid <- predict.glm(mod.frt.9_final, newdata=Valid, type="response")
   roc_obj <- roc(Valid[,"fire"], mod.valid, quiet=TRUE)
   mod.auc <- auc(roc_obj)

# create model table (only do this once) and add the relevant data
top_mod_table_NDT1_light_t <- data.frame (matrix (ncol = 10, nrow = 0))
colnames (top_mod_table_NDT1_light_t ) <- c ("CAUSE", "FRT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_proj_age_1", "coef_proj_height_1", "coef_elevation", "AUC")

##Add data for NDT1
top_mod_table_NDT1_light_t[1,1]<-"lightning"
top_mod_table_NDT1_light_t[1,2]<-"FRT9"
top_mod_table_NDT1_light_t[1,3]<-"Y"
top_mod_table_NDT1_light_t[1,4]<-"fire ~ climate1 + prj_g_1 + prj_h_1 + dm_h_bc"
top_mod_table_NDT1_light_t[1,5]<- coef(mod.frt.9_final)[1] #Intercept
top_mod_table_NDT1_light_t[1,6]<- coef(mod.frt.9_final)[2] #Climate variable 1
top_mod_table_NDT1_light_t[1,7]<- coef(mod.frt.9_final)[3] #Climate variable 2
top_mod_table_NDT1_light_t[1,8]<- coef(mod.frt.9_final)[4] #I
top_mod_table_NDT1_light_t[1,9]<- coef(mod.frt.9_final)[5] #co
top_mod_table_NDT1_light_t[1,10]<- mod.auc
...
```{r}
write.csv(top_mod_table_FRT5_light_t_ALL, file="D:\\Fire\\fire_data\\raw_data\\Ignition Models\\Ignition_Model_Results_Lightning_Treed\\top_mod_table_FRT9_light_tree_all.csv")
```
...
FRT5_summary_table_mean<- top_mod_table_FRT5_light_t_ALL %>% summarize_if(is.numeric,mean)
  ...
  # intercept coef_climate_1 coef_proj_age_1 coef_proj_height_1 coef_elevation       AUC
  # -1.640867     0.06470642      0.00486784         0.06409836   -0.002101855 0.7111667
