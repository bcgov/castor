---
title: "13_2d_fire_spread_size_model_selection_person_treed_NDT4"
author: "Cora Skaien"
date: "06/12/2021"
...
#=================================
#  Script Name: 13_2d_fire_spread_size_model_selection_person_treed_NDT4.R
#  Script Version: 1.0
...
variables_all_infra_landuse_NDT4<-c(dist_mun = "dist_mun", dist_dam = "dist_dam", dist_nat = "dist_nat", dist_mine = "dist_mine", roads_km = "roads_km", bclcs_level_5_2 = "bclcs_level_5_2")
...
for (i in 1:(length(inputs.me.infra_landuse_NDT4)-1)) {
  for (j in (i+1):length(inputs.me.infra_landuse_NDT4)) {
     twoway.ints.in <- cbind(twoway.ints.in, paste(inputs.me.infra_landuse_NDT4[i], inputs.me.infra_landuse_NDT4[j], sep=":"))
  }
}
twoway.ints.in
...
#Get variables on own
#complete list of models using non-climate vars
mods.me.tmp.in <- powerSet(variables_all_infra_landuse_NDT4) 
#add climate vars to all of the above
mods.me.infra_landuse_NDT4 <- list()
for (i in 1: length(mods.me.tmp.in)) {
   mods.me.infra_landuse_NDT4[[i]] <- c(mods.me.tmp.in[[i]])
}
...
#complete list of two-way interactions
#mods.twoway.in.NDT4b <- powerSet(twoway.in.ints.in) #
#Or use subset of interactions
mods.twoway.in.NDT4b <- powerSet(twoway.ints.in_dist) 
length(mods.twoway.in.NDT4b) #512
#mods.twoway.in.NDT4b
mods.twoway.in.NDT4b[1]
mods.twoway.in.NDT4b<-mods.twoway.in.NDT4b[-1]

#Finding models in mods.me that accommodate/allow interaction terms in each mods.twoway.in to be added
mods.inter.NDT4b.in <- list()
counter <- 0
for (i in 1: length(mods.twoway.in.NDT4b)) {
   s1 <- unique(unlist( strsplit(mods.twoway.in.NDT4b[[i]], split=':', fixed=TRUE) ) )
   for (j in 1: length(mods.me.infra_landuse_NDT4)) {
      if (all(s1 %in% mods.me.infra_landuse_NDT4[[j]])==TRUE) {
        counter <- counter + 1
        both <-  c(mods.me.infra_landuse_NDT4[[j]], mods.twoway.in.NDT4b[[i]])
        mods.inter.NDT4b.in[[counter]] <- both
      }
   }
}

length(mods.inter.NDT4b.in) #1365
#mods.inter.NDT4b.in

#####2. Infrastructure and land use
all.poss.mods.infra_landuse<-c(1,mods.me.infra_landuse_NDT4, mods.inter.NDT4b.in)
...
for (h in 1:length(zones1)) {
  dat2<- subset(spread_data_person_t, spread_data_person_t$ntrl_ds=="NDT4")
...
AIC_person_NDT4_spread_treed_climate<-table.glm.climate.simple.spread

AIC_person_NDT4_spread_treed_summary_climate<- AIC_person_NDT4_spread_treed_climate %>%
  group_by(model) %>%
...
AIC_person_NDT4_spread_treed_summary_climate2<- AIC_person_NDT4_spread_treed_summary_climate %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT4_spread_treed_summary_climate2)
```
...
AIC_person_NDT4_spread_treed_climate_gamma<-table.glm.climate.simple.spread.gamma

AIC_person_NDT4_spread_treed_summary_climate_gamma<- AIC_person_NDT4_spread_treed_climate_gamma %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic, na.rm=TRUE),
            meanR2=mean(Model.R2, na.rm=TRUE),
            sdR2=sd(Model.R2, na.rm=TRUE),
...
AIC_person_NDT4_spread_treed_summary_climate_gamma2<- AIC_person_NDT4_spread_treed_summary_climate_gamma %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT4_spread_treed_summary_climate_gamma2)
```
...
AIC_person_NDT4_spread_treed_VRI<-table.glm.VRI.simple.spread

AIC_person_NDT4_spread_treed_summary_VRI<- AIC_person_NDT4_spread_treed_VRI %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic, na.rm=TRUE),
            meanR2=mean(Model.R2, na.rm=TRUE),
            meanAdjustR2=mean(Adjusted.R2, na.rm=TRUE),
...
AIC_person_NDT4_spread_treed_summary_VRI2<- AIC_person_NDT4_spread_treed_summary_VRI %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT4_spread_treed_summary_VRI2)
```
...
AIC_person_NDT4_spread_treed_topo<-table.glm.topo.simple.spread

AIC_person_NDT4_spread_treed_summary_topo<- AIC_person_NDT4_spread_treed_topo %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic, na.rm=TRUE),
            meanR2=mean(Model.R2, na.rm=TRUE),
            meanAdjustR2=mean(Adjusted.R2, na.rm=TRUE),
...
AIC_person_NDT4_spread_treed_summary_topo2<- AIC_person_NDT4_spread_treed_summary_topo %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT4_spread_treed_summary_topo2)
```
...
AIC_person_NDT4_spread_treed_infra<-table.glm.infra.simple.spread

AIC_person_NDT4_spread_treed_summary_infra<- AIC_person_NDT4_spread_treed_infra %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic, na.rm=TRUE),
            meanR2=mean(Model.R2, na.rm=TRUE),
            meanAdjustR2=mean(Adjusted.R2, na.rm=TRUE),
...
AIC_person_NDT4_spread_treed_summary_infra2<- AIC_person_NDT4_spread_treed_summary_infra %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT4_spread_treed_summary_infra2)
```

Combine together
```{r}
NDT4_l_treed_spread<-rbind(AIC_person_NDT4_spread_treed_summary_climate2, AIC_person_NDT4_spread_treed_summary_infra2, AIC_person_NDT4_spread_treed_summary_topo2, AIC_person_NDT4_spread_treed_summary_VRI2)
...
Save data.
```{r}
write.csv(NDT4_l_treed_spread, file="D:\\Fire\\fire_data\\raw_data\\NDT4_person_models_treed_spread_Nov.csv")
```
...
```{r}
spread_person_t_NDT4<-subset(spread_data_person_t, spread_data_person_t$ntrl_ds=="NDT4")

prop<-0.75

#Partition data into training and validation
 trainIndex <- createDataPartition(spread_person_t_NDT4$vegtype2, p = prop,
                                    list = FALSE,
                                    times = 1)
  
   dat1 <- spread_person_t_NDT4[ trainIndex,]
   Valid <- spread_person_t_NDT4[-trainIndex,]
  
#Run model using dat1
model.NDT4<-lm(LNsize_ha ~ proj_age_1 + live_stand_volume_125 + dist_mun + dist_mine + roads_km + slope + elevation + slope:elevation + climate1 + climate2 + wind_atfire + climate1:proj_height_1 + climate1:proj_age_1 + climate1:live_stand_volume_125 + climate1:elevation + climate1:wind_atfire + slope:wind_atfire + elevation:wind_atfire + vegtype2:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire + Tdif_atfire:live_stand_volume_125, data = dat1)

AIC(model.NDT4) #1261.8
summary(model.NDT4)$r.squared #0.14
summary(model.NDT4)$adj.r.squared #0.07

predictions <- model.NDT4 %>% predict(Valid)
R2(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.04
RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE) #1.82
MAE(predictions, Valid$LNsize_ha, na.rm=TRUE) #1.30
...
#Remove least sig
model.NDT4<-lm(LNsize_ha ~ proj_age_1 + live_stand_volume_125 + dist_mun + dist_mine + roads_km + slope + elevation + slope:elevation + climate1 + climate2 + wind_atfire + climate1:proj_height_1 + climate1:proj_age_1 + climate1:live_stand_volume_125 + climate1:wind_atfire + slope:wind_atfire + elevation:wind_atfire + vegtype2:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire + Tdif_atfire:live_stand_volume_125, data = dat1)

AIC(model.NDT4) #1259.8
summary(model.NDT4)$r.squared #0.14
summary(model.NDT4)$adj.r.squared #0.08
...
#Remove least sig
model.NDT4<-lm(LNsize_ha ~ proj_age_1 + live_stand_volume_125 + dist_mun + dist_mine + roads_km + slope + elevation + slope:elevation + climate1 + climate2 + wind_atfire + climate1:proj_height_1 + climate1:proj_age_1 + climate1:live_stand_volume_125 + climate1:wind_atfire + slope:wind_atfire + elevation:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire + Tdif_atfire:live_stand_volume_125, data = dat1)

AIC(model.NDT4) #1254.99
summary(model.NDT4)$r.squared #0.14
summary(model.NDT4)$adj.r.squared #0.08

predictions <- model.NDT4 %>% predict(Valid)
R2(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.04
RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE) #1.81
...
AIC(model.NDT4) #1252.99
summary(model.NDT4)$r.squared #0.14
summary(model.NDT4)$adj.r.squared #0.09
...
#Remove least sig
model.NDT4<-lm(LNsize_ha ~ proj_age_1 + live_stand_volume_125 + dist_mun + dist_mine + roads_km + slope + elevation + slope:elevation + climate1 + climate2 + wind_atfire + climate1:proj_height_1 + climate1:live_stand_volume_125 + climate1:wind_atfire + elevation:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire + Tdif_atfire:live_stand_volume_125, data = dat1)

AIC(model.NDT4) #1251.6
summary(model.NDT4)$r.squared #0.14
summary(model.NDT4)$adj.r.squared #0.09

predictions <- model.NDT4 %>% predict(Valid)
R2(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.04
RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE) #1.79
...
#Remove least sig
model.NDT4<-lm(LNsize_ha ~ proj_age_1 + live_stand_volume_125 + dist_mun + dist_mine + roads_km + slope + elevation + slope:elevation + climate1 + climate2 + wind_atfire + climate1:live_stand_volume_125 + climate1:wind_atfire + elevation:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire + Tdif_atfire:live_stand_volume_125, data = dat1)

AIC(model.NDT4) #1250.6
summary(model.NDT4)$r.squared #0.14
summary(model.NDT4)$adj.r.squared #0.09

predictions <- model.NDT4 %>% predict(Valid)
R2(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.04
RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE) #1.80
MAE(predictions, Valid$LNsize_ha, na.rm=TRUE) #1.27
...
#Remove least sig
model.NDT4<-lm(LNsize_ha ~ proj_age_1 + live_stand_volume_125 + dist_mun + roads_km + slope + elevation + slope:elevation + climate1 + climate2 + wind_atfire + climate1:live_stand_volume_125 + climate1:wind_atfire + elevation:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire + Tdif_atfire:live_stand_volume_125, data = dat1)

AIC(model.NDT4) #1249.1
summary(model.NDT4)$r.squared #0.13
summary(model.NDT4)$adj.r.squared #0.09
...
#Remove least sig
model.NDT4<-lm(LNsize_ha ~ proj_age_1 + live_stand_volume_125 + dist_mun + roads_km + slope + elevation + slope:elevation + climate1 + climate2 + wind_atfire + climate1:wind_atfire + elevation:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire + Tdif_atfire:live_stand_volume_125, data = dat1)

AIC(model.NDT4) #1248.5
summary(model.NDT4)$r.squared #0.13
summary(model.NDT4)$adj.r.squared #0.09
...
#Remove least sig
model.NDT4<-lm(LNsize_ha ~ proj_age_1 + live_stand_volume_125 + dist_mun + slope + elevation + slope:elevation + climate1 + climate2 + wind_atfire + climate1:wind_atfire + elevation:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire + Tdif_atfire:live_stand_volume_125, data = dat1)

AIC(model.NDT4) #1247.9
summary(model.NDT4)$r.squared #0.13
summary(model.NDT4)$adj.r.squared #0.09
...
NDT4_spread_size_t<-spread_person_t_NDT4 %>% drop_na(proj_age_1 , live_stand_volume_125 , dist_mun , roads_km , slope , climate1 , climate2 , wind_atfire, elevation, Tdif_atfire)

#Run Model again with this data; but uses all data here
model.NDT4.Spread<-lm(LNsize_ha ~ proj_age_1 + live_stand_volume_125 + dist_mun + roads_km + slope + elevation + slope:elevation + climate1 + climate2 + wind_atfire + climate1:wind_atfire + elevation:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire + Tdif_atfire:live_stand_volume_125, data = NDT4_spread_size_t)
...
#Partial Residuals
#LNsize_ha ~ proj_age_1 + live_stand_volume_125 + dist_mun + roads_km + slope + elevation + slope:elevation + climate1 + climate2 + wind_atfire + climate1:wind_atfire + elevation:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire + Tdif_atfire:live_stand_volume_125
visreg(model.NDT4.Spread, "proj_age_1")
visreg(model.NDT4.Spread, "live_stand_volume_125", by="Tdif_atfire")
visreg(model.NDT4.Spread, "dist_mun")
visreg(model.NDT4.Spread, "roads_km")

visreg(model.NDT4.Spread, "slope", by= "elevation")
visreg(model.NDT4.Spread, "elevation", by="slope")
visreg(model.NDT4.Spread, "elevation", by="wind_atfire")

#climate1 + climate2 + wind_atfire + climate1:wind_atfire + elevation:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire + Tdif_atfire:live_stand_volume_125
visreg(model.NDT4.Spread, "climate1", by="wind_atfire")
visreg(model.NDT4.Spread, "climate2")

visreg(model.NDT4.Spread, "wind_atfire", by="elevation")
visreg(model.NDT4.Spread, "wind_atfire", by="Tdif_atfire")

visreg(model.NDT4.Spread, "Tdif_atfire", by="wind_atfire")
visreg(model.NDT4.Spread, "Tdif_atfire", by="live_stand_volume_125")
...
top_mod_table_NDT4_person_t_ALL <- data.frame (matrix (ncol = 26, nrow = 0))
colnames (top_mod_table_NDT4_person_t_ALL ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_proj_age_1", "coef_live_stand_volume_125", "coef_dist_mun", "coef_roads_km",  "coef_slope", "coef_elevation", "coef_climate_1", "coef_climate_2", "coef_wind_atfire", "coef_Tdif_atfire", "coef_slope:elevation", "coef_climate1:wind_atfire", "coef_elevation:wind_atfire", "coef_wind_atfire:Tdif_atfire", "coef_live_stand_volume_125:Tdif_atfire", "AIC", "Model.R2", "Valid.R2", "Valid.RSME", "Valid.MAE", "Adjusted.R2")
...
#Model   
model.NDT4.Spread<-lm(LNsize_ha ~ proj_age_1 + live_stand_volume_125 + dist_mun + roads_km + slope + elevation + slope:elevation + climate1 + climate2 + wind_atfire + climate1:wind_atfire + elevation:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire + Tdif_atfire:live_stand_volume_125, data = dat1) 

predictions <- model.NDT4.Spread %>% predict(Valid)

# create model table (only do this once) and add the relevant data
top_mod_table_NDT4_person_t <- data.frame (matrix (ncol = 26, nrow = 0))
...
top_mod_table_NDT4_person_t[1,4]<-"LNsize_ha ~ proj_age_1 + live_stand_volume_125 + dist_mun + roads_km + slope + elevation + slope:elevation + climate1 + climate2 + wind_atfire + climate1:wind_atfire + elevation:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire + Tdif_atfire:live_stand_volume_125" 
top_mod_table_NDT4_person_t[1,5]<- coef(model.NDT4.Spread)[1] #Intercept
top_mod_table_NDT4_person_t[1,6]<- coef(model.NDT4.Spread)[2] #C
top_mod_table_NDT4_person_t[1,7]<- coef(model.NDT4.Spread)[3] #C
top_mod_table_NDT4_person_t[1,8]<- coef(model.NDT4.Spread)[4] #co
top_mod_table_NDT4_person_t[1,9]<- coef(model.NDT4.Spread)[5] #co
top_mod_table_NDT4_person_t[1,10]<- coef(model.NDT4.Spread)[6] #c
top_mod_table_NDT4_person_t[1,11]<- coef(model.NDT4.Spread)[7] #l
top_mod_table_NDT4_person_t[1,12]<- coef(model.NDT4.Spread)[8] #co
top_mod_table_NDT4_person_t[1,13]<- coef(model.NDT4.Spread)[9] #c
top_mod_table_NDT4_person_t[1,14]<- coef(model.NDT4.Spread)[10] #coeff
top_mod_table_NDT4_person_t[1,15]<- coef(model.NDT4.Spread)[11] #co
top_mod_table_NDT4_person_t[1,16]<- coef(model.NDT4.Spread)[12] #coe
top_mod_table_NDT4_person_t[1,17]<- coef(model.NDT4.Spread)[13] #co
top_mod_table_NDT4_person_t[1,18]<- coef(model.NDT4.Spread)[14] #co
top_mod_table_NDT4_person_t[1,19]<- coef(model.NDT4.Spread)[15] #
top_mod_table_NDT4_person_t[1,20]<- coef(model.NDT4.Spread)[16] # cl
top_mod_table_NDT4_person_t[1,21]<- AIC(model.NDT4.Spread)
top_mod_table_NDT4_person_t[1,22]<- summary(model.NDT4.Spread)$r.squared
top_mod_table_NDT4_person_t[1,23]<- R2(predictions, Valid$LNsize_ha, na.rm=TRUE)
top_mod_table_NDT4_person_t[1,24]<- RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE)
top_mod_table_NDT4_person_t[1,25]<- MAE(predictions, Valid$LNsize_ha, na.rm=TRUE)
top_mod_table_NDT4_person_t[1,26]<- summary(model.NDT4.Spread)$adj.r.squared
...
```{r}
write.csv(top_mod_table_NDT4_person_t_ALL, file="D:\\Fire\\fire_data\\raw_data\\top_mod_model.NDT4.Spread_firesize_person_t.csv")
```
...
top_mod_table_NDT4_model.NDT4.Spreadperson_t_means<-top_mod_table_NDT4_person_t_ALL %>% summarise_each(funs( mean( .,na.rm = TRUE)))
top_mod_table_NDT4_model.NDT4.Spreadperson_t_means

top_mod_table_NDT4_model.NDT4.Spreadperson_t_means[1,1]<-"person"
top_mod_table_NDT4_model.NDT4.Spreadperson_t_means[1,2]<-"NDT4"
top_mod_table_NDT4_model.NDT4.Spreadperson_t_means[1,3]<-"Treed"
top_mod_table_NDT4_model.NDT4.Spreadperson_t_means[1,4]<- "LNsize_ha ~ proj_age_1 + live_stand_volume_125 + dist_mun + roads_km + slope + elevation + slope:elevation + climate1 + climate2 + wind_atfire + climate1:wind_atfire + elevation:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire + Tdif_atfire:live_stand_volume_125" 
top_mod_table_NDT4_model.NDT4.Spreadperson_t_means
```
Save table.

```{r}
write.csv(top_mod_table_NDT4_model.NDT4.Spreadperson_t_means, file="D:\\Fire\\fire_data\\raw_data\\top_mod_model.NDT4.Spread_firesize_person_t_Means.csv")
```

Standard deviation.

```{r}
top_mod_table_NDT4_model.NDT4.Spreadperson_t_sd<-top_mod_table_NDT4_person_t_ALL %>% summarise_each(funs( sd( .,na.rm = TRUE)))
top_mod_table_NDT4_model.NDT4.Spreadperson_t_sd

top_mod_table_NDT4_model.NDT4.Spreadperson_t_sd[1,1]<-"person"
top_mod_table_NDT4_model.NDT4.Spreadperson_t_sd[1,2]<-"NDT4"
top_mod_table_NDT4_model.NDT4.Spreadperson_t_sd[1,3]<-"Treed"
top_mod_table_NDT4_model.NDT4.Spreadperson_t_sd[1,4]<-"LNsize_ha ~ proj_age_1 + live_stand_volume_125 + dist_mun + roads_km + slope + elevation + slope:elevation + climate1 + climate2 + wind_atfire + climate1:wind_atfire + elevation:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire + Tdif_atfire:live_stand_volume_125" 
top_mod_table_NDT4_model.NDT4.Spreadperson_t_sd
```
...
```{r}
write.csv(top_mod_table_NDT4_model.NDT4.Spreadperson_t_sd, file="D:\\Fire\\fire_data\\raw_data\\top_mod_model.NDT4.Spread_firesize_person_t_SD.csv")
```
