...
Combine together
#Note: no climate model will available because climate1 is the only variable for that model
```{r}
NDT5_l_treed_spread<-rbind(AIC_lightning_NDT5_spread_treed_summary_infra2, AIC_lightning_NDT5_spread_treed_summary_topo2, AIC_lightning_NDT5_spread_treed_summary_VRI2)
...
2. dist_mun + dist_dam + dist_mine + bclcs_level_5_2 + dist_mun:bclcs_level_5_2 + dist_dam:bclcs_level_5_2
3. slope
4. climate1 

##################### Begin Manual Exploration ##################
Begin manual exploration from here. We will use the above variables, plus additional interactions: wind_atfire + climate1:live_stand_volume_125 + climate1:wind_atfire + slope:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire + Tdif_atfire:live_stand_volume_125


```{r}
spread_lightning_t_NDT5<-subset(spread_data_lightning_t, spread_data_lightning_t$ntrl_ds=="NDT5") #19 observations...

#Given how few observations there are, we can have very few variables in the model
...
#Run model using dat1: Note, this first model already has too many variables - hoping it can be informative though
model.NDT5<-lm(LNsize_ha ~ live_stand_volume_125 + dist_mun + roads_km + bclcs_level_5_2 + climate1 + wind_atfire + climate1:live_stand_volume_125  + climate1:wind_atfire + slope + slope:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire + Tdif_atfire:live_stand_volume_125, data = dat1)

AIC(model.NDT5) #Did not converge... 

#Remove interactions
model.NDT5<-lm(LNsize_ha ~ live_stand_volume_125 + dist_mun + roads_km + bclcs_level_5_2 + climate1 + wind_atfire + slope + Tdif_atfire, data = dat1)

AIC(model.NDT5) #47.7
summary(model.NDT5)$r.squared #0.89
summary(model.NDT5)$adj.r.squared #0.66

predictions <- model.NDT5 %>% predict(Valid)
R2(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.69
RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.75
MAE(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.62
...
#Remove least sig
model.NDT5<-lm(LNsize_ha ~ live_stand_volume_125 + dist_mun + roads_km + bclcs_level_5_2 + climate1 + wind_atfire + slope, data = dat1)

AIC(model.NDT5) #45.7
summary(model.NDT5)$r.squared #0.89
summary(model.NDT5)$adj.r.squared #0.72

predictions <- model.NDT5 %>% predict(Valid)
R2(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.69
RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.75
MAE(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.63
...
#Remove least sig
model.NDT5<-lm(LNsize_ha ~ live_stand_volume_125 + dist_mun + roads_km + bclcs_level_5_2 + climate1 + slope, data = dat1)

AIC(model.NDT5) #43.7
summary(model.NDT5)$r.squared #0.88
summary(model.NDT5)$adj.r.squared #0.77

predictions <- model.NDT5 %>% predict(Valid)
R2(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.73
RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.76
MAE(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.65
...
#Remove least sig
model.NDT5<-lm(LNsize_ha ~ live_stand_volume_125 + dist_mun + bclcs_level_5_2 + slope, data = dat1)

AIC(model.NDT5) #41.8
summary(model.NDT5)$r.squared #0.88
summary(model.NDT5)$adj.r.squared #0.80

predictions <- model.NDT5 %>% predict(Valid)
R2(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.72
RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.79
MAE(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.68
...
#Remove least sig
model.NDT5<-lm(LNsize_ha ~ live_stand_volume_125 + dist_mun + bclcs_level_5_2, data = dat1)

AIC(model.NDT5) #40.1
summary(model.NDT5)$r.squared #0.88
summary(model.NDT5)$adj.r.squared #0.82

predictions <- model.NDT5 %>% predict(Valid)
R2(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.61
RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.91
MAE(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.78
...
NDT5_spread_size_t<-spread_lightning_t_NDT5 %>% drop_na( live_stand_volume_125 , dist_mun, slope, bclcs_level_5_2) #17 observations

#Run Model again with this data; but uses all data here
model.NDT5.Spread<-lm(LNsize_ha ~ live_stand_volume_125 + dist_mun + bclcs_level_5_2 + slope, data = NDT5_spread_size_t)

AIC(model.NDT5.Spread) #49.4
summary(model.NDT5.Spread)$r.squared #0.87
summary(model.NDT5.Spread)$adj.r.squared #0.82
...
top_mod_table_NDT5_lightning_t_ALL <- data.frame (matrix (ncol = 16, nrow = 0))
colnames (top_mod_table_NDT5_lightning_t_ALL ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_live_stand_volume_125", "coef_dist_mun", "coef_bclcs_level_5_2OP", "coef_bclcs_level_5_2SP", "coef_slope", "AIC", "Model.R2", "Valid.R2", "Valid.RSME", "Valid.MAE", "Adjusted.R2")
...
prop<-0.75
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(NDT5_spread_size_t$bclcs_level_5_2, p = prop,
                                    list = FALSE,
                                    times = 1)
...
#Model   
model.NDT5.Spread<-lm(LNsize_ha ~ live_stand_volume_125 + dist_mun + bclcs_level_5_2 + slope, data = dat1) 

predictions <- model.NDT5.Spread %>% predict(Valid)

# create model table (only do this once) and add the relevant data
top_mod_table_NDT5_lightning_t <- data.frame (matrix (ncol = 16, nrow = 0))
colnames (top_mod_table_NDT5_lightning_t ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_live_stand_volume_125", "coef_dist_mun", "coef_bclcs_level_5_2OP", "coef_bclcs_level_5_2SP", "coef_slope", "AIC", "Model.R2", "Valid.R2", "Valid.RSME", "Valid.MAE", "Adjusted.R2")

##Add data for NDT5
top_mod_table_NDT5_lightning_t[1,1]<-"lightning"
top_mod_table_NDT5_lightning_t[1,2]<-"NDT5"
top_mod_table_NDT5_lightning_t[1,3]<-"Y"
top_mod_table_NDT5_lightning_t[1,4]<-"LNsize_ha ~ live_stand_volume_125 + dist_mun + bclcs_level_5_2 + slope" 
...
top_mod_table_NDT5_lightning_t[1,11]<- AIC(model.NDT5.Spread)
top_mod_table_NDT5_lightning_t[1,12]<- summary(model.NDT5.Spread)$r.squared
top_mod_table_NDT5_lightning_t[1,13]<- R2(predictions, Valid$LNsize_ha, na.rm=TRUE)
top_mod_table_NDT5_lightning_t[1,14]<- RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE)
top_mod_table_NDT5_lightning_t[1,15]<- MAE(predictions, Valid$LNsize_ha, na.rm=TRUE)
top_mod_table_NDT5_lightning_t[1,16]<- summary(model.NDT5.Spread)$adj.r.squared
...
top_mod_table_NDT5_model.NDT5.Spreadlightning_t_means[1,4]<- "LNsize_ha ~ live_stand_volume_125 + dist_mun + bclcs_level_5_2 + slope" 
top_mod_table_NDT5_model.NDT5.Spreadlightning_t_means
```
Save table.
...
top_mod_table_NDT5_model.NDT5.Spreadlightning_t_sd[1,4]<-"LNsize_ha ~ live_stand_volume_125 + dist_mun + bclcs_level_5_2 + slope" 
top_mod_table_NDT5_model.NDT5.Spreadlightning_t_sd
```
