...
With a second subset of data, using average or max values for temperature in month of fire performed far better. As self: how will future projections be modelled? Will they be modelled by year total, or will they be modelled by month within year and brought together? Using tave_09, will model by year; if use Tmax_atfire, then can model by month. Which is preferred? Using temperature at time of fire creates a better model.
...
```{r}
table(NDT1_escape_t$bclcs_level_2)
...
top_mod_table_NDT1 <- data.frame (matrix (ncol = 41, nrow = 0))
...
top_mod_table_NDT1[1,35]<- coef(model.NDT1.E)[31] #coefficient 
top_mod_table_NDT1[1,36]<- coef(model.NDT1.E)[32] #coefficient 
top_mod_table_NDT1[1,37]<- coef(model.NDT1.E)[33] #coefficient 
top_mod_table_NDT1[1,38]<- coef(model.NDT1.E)[34] #coefficient 
top_mod_table_NDT1[1,39]<- coef(model.NDT1.E)[35] #coefficient 
top_mod_table_NDT1[1,40]<- coef(model.NDT1.E)[36] #coefficient 
top_mod_table_NDT1[1,41]<- mod.auc
...
```{r}
#Remove NAs to ensure all same data used so we can compare AICs
NDT1_escape_t2<-escape_all_t_NDT1 %>% drop_na(Tmax_atfire, ppt_atfire, mdc_atfire, vegtype2, live_stand_volume_125, dist_infra, bclcs_level_2, slope, aspect, elevation, wind_atfire, Tdif_atfire, roads_km, fire_cs) #4238 observations
table(NDT1_escape_t2$vegtype2)

#Run Model again with this data; but uses all data here
model.NDT1.E2<-glm(escape ~ Tmax_atfire + ppt_atfire + vegtype2 + mdc_atfire + ppt_atfire:mdc_atfire + live_stand_volume_125 + bclcs_level_2 + live_stand_volume_125:bclcs_level_2 + slope + aspect + elevation + wind_atfire + slope:aspect + dist_infra + roads_km + dist_infra:roads_km + wind_atfire + Tdif_atfire  + elevation*ppt_atfire + Tdif_atfire*Tmax_atfire + Tdif_atfire*ppt_atfire + fire_cs + fire_cs*dist_infra + fire_cs*roads_km + fire_cs*elevation + fire_cs*Tdif_atfire + bclcs_level_2*Tmax_atfire, family = binomial, data = NDT1_escape_t2)
...
NDT1_escape_t2$resids<-resid(model.NDT1.E2)
...
binnedplot (NDT1_escape_t2$Tmax_atfire, 
            NDT1_escape_t2$resids, 
            nclass = NULL, 
            xlab = "", 
            ylab = "Average residual", 
...
visreg(model.NDT1.E2, "Tmax_atfire", by="bclcs_level_2")
visreg(model.NDT1.E2, "Tmax_atfire", by="Tdif_atfire")
...
visreg(model.NDT1.E2, "vegtype2", ylim=c(-40,40))

visreg(model.NDT1.E2, "mdc_atfire", by="ppt_atfire")
visreg(model.NDT1.E2, "Tdif_atfire", by="Tmax_atfire")
...
visreg(model.NDT1.E2, "live_stand_volume_125", by="bclcs_level_2", overlay=TRUE)
...
#Create a new blank table and get AUC too
top_mod_table_NDT1_ALL2 <- data.frame (matrix (ncol = 41, nrow = 0))
colnames (top_mod_table_NDT1_ALL2 ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_Tmax_atfire", "coef_ppt_atfire", "coef_vegtypeOP", "coef_vegtypeRO", "coef_vegtypeS", "coef_vegtypeTB", "coef_vegtypeTC", "coef_vegtypeTM", "coef_vegtypeUR", "coef_mdc_atfire", "coef_live_stand_volume_125", "coef_bclcs_level_2N", "coef_bclcs_level_2T", "coef_slope", "coef_aspect", "coef_elevation","coef_wind_atfire", "coef_dist_infra", "coef_roads_km", "coef_Tdifatfire", "coef_fire_csPerson", "coef_ppt_atfire:mdc_atfire", "coef_live_stand_volume_125:bclcs_level_2N", "coef_live_stand_volume_125:bclcs_level_2T", "coef_slope:aspect", "coef_dist_infra:roads_km", "coef_ppt_atfire:elevation", "coef_Tmax_atfire:Tdif_atfire", "coef_ppt_atfire:Tdif_atfire", "coef_dist_infra:fire_csPerson", "coef_roads_km:fire_csPerson", "coef_elevation:fire_csPerson", "coef_Tdif_atfire:fire_csPerson", "coef_Tmax_atfire:bclcs_level_2N", "coef_Tmax_atfire:bclcs_level_2T", "AUC")
```
...
top_mod_table_NDT1[1,34]<- coef(model.NDT1.E2)[30] #coefficient 
top_mod_table_NDT1[1,35]<- coef(model.NDT1.E2)[31] #coefficient 
top_mod_table_NDT1[1,36]<- coef(model.NDT1.E2)[32] #coefficient 
top_mod_table_NDT1[1,37]<- coef(model.NDT1.E2)[33] #coefficient 
top_mod_table_NDT1[1,38]<- coef(model.NDT1.E2)[34] #coefficient 
top_mod_table_NDT1[1,39]<- coef(model.NDT1.E2)[35] #coefficient 
top_mod_table_NDT1[1,40]<- coef(model.NDT1.E2)[36] #coefficient 
top_mod_table_NDT1[1,41]<- mod.auc
...
```{r}
#Create a new blank table and get AUC too
top_mod_table_NDT1_ALL_v2 <- data.frame (matrix (ncol = 41, nrow = 0))
colnames (top_mod_table_NDT1_ALL_v2 ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_climate_2", "coef_vegtypeOP", "coef_vegtypeRO", "coef_vegtypeS", "coef_vegtypeTB", "coef_vegtypeTC", "coef_vegtypeTM", "coef_vegtypeUR", "coef_mdc_atfire", "coef_live_stand_volume_125", "coef_bclcs_level_2N", "coef_bclcs_level_2T", "coef_slope", "coef_aspect", "coef_elevation","coef_wind_atfire", "coef_dist_infra", "coef_roads_km", "coef_Tdifatfire", "coef_fire_csPerson", "coef_climate2:mdc_atfire", "coef_live_stand_volume_125:bclcs_level_2N", "coef_live_stand_volume_125:bclcs_level_2T", "coef_slope:aspect", "coef_dist_infra:roads_km", "coef_climate2:elevation", "coef_climate1:Tdif_atfire", "coef_climate2:Tdif_atfire", "coef_dist_infra:fire_csPerson", "coef_roads_km:fire_csPerson", "coef_elevation:fire_csPerson", "coef_Tdif_atfire:fire_csPerson", "coef_climate1:bclcs_level_2N", "coef_climate1:bclcs_level_2T", "AUC")
```

```{r}
dat1 <- subset(NDT1_escape_t, NDT1_escape_t$fire_yr<2018) #3712
Valid <- subset(NDT1_escape_t, NDT1_escape_t$fire_yr>2017) #526
...
top_mod_table_NDT1[1,35]<- coef(model.NDT1.E.v2)[31] #coefficient 
top_mod_table_NDT1[1,36]<- coef(model.NDT1.E.v2)[32] #coefficient 
top_mod_table_NDT1[1,37]<- coef(model.NDT1.E.v2)[33] #coefficient 
top_mod_table_NDT1[1,38]<- coef(model.NDT1.E.v2)[34] #coefficient 
top_mod_table_NDT1[1,39]<- coef(model.NDT1.E.v2)[35] #coefficient 
top_mod_table_NDT1[1,40]<- coef(model.NDT1.E.v2)[36] #coefficient 
top_mod_table_NDT1[1,41]<- mod.auc
...
View.
```{r}
head(top_mod_table_NDT1_ALL_v2) #AUC 0.66 this way
```
...
```{r}
dat1 <- subset(NDT1_escape_t, NDT1_escape_t$fire_yr<2017) #3466
Valid <- subset(NDT1_escape_t, NDT1_escape_t$fire_yr>2016) #772
...
View.
```{r}
head(top_mod_table_NDT1_ALL_v2) #New AUC 0.67, not much better, but not awful
```
