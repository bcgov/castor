...
AIC(model.NDT2) #5375.2 (5390.42 on round2)
...
#Remove least significant 
model.NDT2<-glm(fire_pres ~ climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + dist_mun + dist_dam + dist_nat + dist_pow + dist_mine + roads_km + dist_mun:roads_km + dist_dam:roads_km + dist_pow:roads_km + dist_mine:roads_km + elevation:climate1 + bclcs_level_5_2, family = binomial, data = dat1) 

AIC(model.NDT2) #(5388.42 on round2)
...
#Remove least significant 
model.NDT2<-glm(fire_pres ~ climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + dist_mun + dist_dam + dist_nat + dist_pow + dist_mine + roads_km + dist_mun:roads_km + dist_pow:roads_km + dist_mine:roads_km + elevation:climate1 + bclcs_level_5_2, family = binomial, data = dat1) 

AIC(model.NDT2) #(5386.5 on round2)
...
#Remove least significant 
model.NDT2<-glm(fire_pres ~ climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + dist_mun + dist_dam + dist_nat + dist_pow + dist_mine + roads_km + dist_pow:roads_km + dist_mine:roads_km + elevation:climate1 + bclcs_level_5_2, family = binomial, data = dat1) 

AIC(model.NDT2) #(5385.7 on round2)
...
AIC(model.NDT2) #7237.3

model.NDT2<-glm(fire_pres ~ climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + dist_mun + dist_dam + dist_nat + dist_pow + dist_mine + roads_km + dist_pow:roads_km + dist_mine:roads_km + elevation:climate1 + bclcs_level_5_2, family = binomial, data = NDT2_ignition_t) 

AIC(model.NDT2) #7237.2 #Seems equally good. Run 500 times and see if better than first run before correction.
...
#Partial Residuals
#climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + dist_mun + dist_dam + dist_nat + dist_pow + dist_mine + roads_km + dist_pow:roads_km + dist_mine:roads_km + elevation:climate1 + bclcs_level_5_2
...
top_mod_table_NDT2_person_t[1,4]<-"fire_pres ~ climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + dist_mun + dist_dam + dist_nat + dist_pow + dist_mine + roads_km + dist_pow:roads_km + dist_mine:roads_km + elevation:climate1 + bclcs_level_5_2" 
...
top_mod_table_NDT2_person_t[1,28]<- mod.auc
...
top_mod_table_NDT2_ignite_person_means[1,4]<- "fire_pres ~ climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + dist_mun + dist_dam + dist_nat + dist_pow + dist_mine + roads_km + dist_pow:roads_km + dist_mine:roads_km + elevation:climate1 + bclcs_level_5_2" 
top_mod_table_NDT2_ignite_person_means
```
Save table.
...
top_mod_table_NDT2_ignite_person_sd[1,4]<-"fire_pres ~ climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + dist_mun + dist_dam + dist_nat + dist_pow + dist_mine + roads_km + dist_pow:roads_km + dist_mine:roads_km + elevation:climate1 + bclcs_level_5_2" 
top_mod_table_NDT2_ignite_person_sd
```
...
#Repeat above for reduced variables
...
#Run model using dat1
model.NDT2<-glm(fire_pres ~ climate1 + vegtype2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:elevation + aspect_cos:elevation + dist_infra + roads_km + dist_infra:roads_km + elevation:climate1, family = binomial, data = dat1) 

AIC(model.NDT2) #5858.95
...
#Remove least significant interaction
model.NDT2<-glm(fire_pres ~ climate1 + vegtype2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:elevation + aspect_cos:elevation + dist_infra + roads_km  + elevation:climate1, family = binomial, data = dat1) 

AIC(model.NDT2) #5857.1
...
#Remove least significant interaction
model.NDT2<-glm(fire_pres ~ climate1 + vegtype2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + aspect_cos:elevation + dist_infra + roads_km  + elevation:climate1, family = binomial, data = dat1) 

AIC(model.NDT2) #5855.2
...
#Remove least significant
model.NDT2<-glm(fire_pres ~ climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + aspect_cos:elevation + dist_infra + roads_km  + elevation:climate1, family = binomial, data = dat1) 

AIC(model.NDT2) #5854.6
...
```{r}
#Remove NAs to ensure all same data used so we can compare AICs
NDT2_ignition_t<-ignition_person_t_NDT2 %>% drop_na(climate1, vegtype2, live_stand_volume_125, proj_age_1, dist_infra, roads_km, slope, elevation, aspect_cos)

#Run Model again with this data; but uses all data here
model.NDT2<-glm(fire_pres ~ climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + aspect_cos:elevation + dist_infra + roads_km  + elevation:climate1, family = binomial, data = NDT2_ignition_t) 
...
#Partial Residuals
#climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + aspect_cos:elevation + dist_infra + roads_km  + elevation:climate1
...
#Create a new blank table and get AUC too
top_mod_table_NDT2_person_t_ALL <- data.frame (matrix (ncol = 20, nrow = 0))
colnames (top_mod_table_NDT2_person_t_ALL ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_vegtype2OP", "coef_vegtype2TB", "coef_vegtype2TC", "coef_vegtype2TM", "coef_proj_age_1", "coef_live_stand_volume_125", "coef_slope", "coef_aspect_cos", "coef_elevation", "coef_dist_infra", "coef_roads_km", "coef_aspect_cos:elevation", "coef_climate1:elevation", "AUC")
```
Let's run it 500 times to get good mean values.
...
# create model table (only do this once) and add the relevant data
top_mod_table_NDT2_person_t <- data.frame (matrix (ncol = 20, nrow = 0))
colnames (top_mod_table_NDT2_person_t ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_vegtype2OP", "coef_vegtype2TB", "coef_vegtype2TC", "coef_vegtype2TM", "coef_proj_age_1", "coef_live_stand_volume_125", "coef_slope", "coef_aspect_cos", "coef_elevation", "coef_dist_infra", "coef_roads_km", "coef_aspect_cos:elevation", "coef_climate1:elevation", "AUC")
...
top_mod_table_NDT2_person_t[1,2]<-"NDT2"
top_mod_table_NDT2_person_t[1,3]<-"Y"
top_mod_table_NDT2_person_t[1,4]<-"fire_pres ~ climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + aspect_cos:elevation + dist_infra + roads_km  + elevation:climate1" 
top_mod_table_NDT2_person_t[1,5]<- coef(model.NDT2)[1] #Intercept
top_mod_table_NDT2_person_t[1,6]<- coef(model.NDT2)[2] #Climate variable 1
top_mod_table_NDT2_person_t[1,7]<- coef(model.NDT2)[3] #Climate variable 2
top_mod_table_NDT2_person_t[1,8]<- coef(model.NDT2)[4] #
...
top_mod_table_NDT2_person_t[1,18]<- coef(model.NDT2)[14]
top_mod_table_NDT2_person_t[1,19]<- coef(model.NDT2)[15]
top_mod_table_NDT2_person_t[1,20]<- mod.auc
...
top_mod_table_NDT2_ignite_person_means[1,2]<-"NDT2"
top_mod_table_NDT2_ignite_person_means[1,3]<-"Treed"
top_mod_table_NDT2_ignite_person_means[1,4]<- "fire_pres ~ climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + aspect_cos:elevation + dist_infra + roads_km  + elevation:climate1" 
top_mod_table_NDT2_ignite_person_means
```
Save table.

```{r}
write.csv(top_mod_table_NDT2_ignite_person_means, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT2_ignite_person_means_Jan.csv")
```
...
top_mod_table_NDT2_ignite_person_sd[1,2]<-"NDT2"
top_mod_table_NDT2_ignite_person_sd[1,3]<-"Treed"
top_mod_table_NDT2_ignite_person_sd[1,4]<-"fire_pres ~ climate1 + vegtype2 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + aspect_cos:elevation + dist_infra + roads_km  + elevation:climate1" 
top_mod_table_NDT2_ignite_person_sd
```
...
```{r}
write.csv(top_mod_table_NDT2_ignite_person_sd, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT2_ignite_person_sd_Jan.csv")
```
