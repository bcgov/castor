...
mod_frt_10a<- glm(fire ~ climate1 + elevatn + FWI_veg + road_dist_m + infr_dist, data=dat9, family=binomial(link="logit"))
Anova(mod_frt_10a, type=3)
mod_frt9b<- glm(fire ~ climate1 + elevatn + FWI_veg + infr_dist, data=dat9, family=binomial(link="logit"))
anova(mod_frt_10a,mod_frt9b, test="Chisq") # removing infrastructure is fine
Anova(mod_frt9b, type=3)

mod_frt9c<-glm(fire ~ climate1 + FWI_veg + infr_dist, data=dat9, family=binomial(link="logit"))
anova(mod_frt9b,mod_frt9c, test="Chisq") # removing roads is bad
Anova(mod_frt9c, type=3)
...
##Partial Residuals
visreg(mod_frt9c, scale="response") # nicer plots than the ones below!
...
dat2_b<-dat9 %>% dplyr::select(fire, climate1, infr_dist)
...
model.data %>% 
  dplyr::select(climate1, FWI_veg, infr_dist,.std.resid, .cooksd) %>%
  top_n(5, .cooksd) 
...
```{r}
mod_frt9<- glm(fire ~ climate1 + infr_dist + FWI_veg, data=dat9, family=binomial(link="logit"))
...
#Model   
mod<-glm(fire ~ climate1 + infr_dist + FWI_veg, data=dat1, family=binomial(link="logit"))
...
top_mod_table_FRT5_person[1,3]<-"fire ~ climate1 + infr_dist + FWI_veg"
