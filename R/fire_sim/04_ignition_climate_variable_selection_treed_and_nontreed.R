...
table(fire_veg_data2$bclcs_level_2, fire_veg_data2$fire_cs) # T=treed, N =  non-treed and L = 
...
ignition_pres_abs4<- fire_veg_data2
...
check %>% print(n=200) # hmm there are some NA's in the 0 column. I should probably correct that.
...
# I should probably have replace = false but there are a few rows where there are more fire ignitions in that subzone than randomly sampled locations which is causing issues with this code. For now Ill leave it like this.
  sampled_df<- df2 %>% 
    ...
    dplyr::select(-data) %>%
    unnest(samp) %>%
    dplyr::select(fire_yr, zone, subzone, feature_id:mdc_09)
...
# Correlation between total precipitation and MDC. This is also pretty correlated with values between -0.77 and -0.18
dist.cut.corr <- dat [c (29:33, 34:38)]
corr <- round (cor (dist.cut.corr), 3)
ggcorrplot (corr, type = "lower", lab = TRUE, tl.cex = 10,  lab_size = 3,
            title = "Correlation between total precipitation and MDC")

# Correlation between Tmax and total precipitation. The correlation is very low, as would be expected (-0.02 to -0.47) 
