...
# 1  glm1                                     ppt09 47.85970 2.057281
# 2  glm2                        ppt09 + proj_age_1 46.10887 0.306443
# 3  glm3                     ppt09 + proj_height_1 45.80242 0.000000
# 4  glm4             ppt09 + live_stand_volume_125 46.96887 1.166448
# 5  glm5 ppt09 + live_stand_volume_125 +proj_age_1 47.26550 1.463078
# 6  glm6        ppt09 + proj_age_1 + proj_height_1 47.26550 1.463078


# glm5: ppt09 + live_stand_volume_125 + proj_height_1


####################################################
##### TOP MODEL####

bg_t2<-bg_t %>% drop_na(live_stand_volume_125, proj_age_1)
...
binnedplot (bg_t2$live_stand_volume_125, 
            bg_t2$resids, 
            nclass = NULL, 
...
plot(bg_t2$live_stand_volume_125,bg_t2$fire_pres)
curve(invlogit(coef(glm_best)[1] +coef(glm_best)[2]*mean(bg_t2$ppt09) + coef(glm_best)[3]*x + coef(glm_best)[4]*mean(bg_t2$proj_age_1)), add=TRUE)

plot(bg_t2$proj_age_1,bg_t2$fire_pres)
curve(invlogit(coef(glm_best)[1] +coef(glm_best)[2]*mean(bg_t2$ppt09) + coef(glm_best)[3]*mean(bg_t2$live_stand_volume_125) + coef(glm_best)[4]*x), add=TRUE)

plot(bg_t2$ppt09,bg_t2$fire_pres)
curve(invlogit(coef(glm_best)[1] +coef(glm_best)[2]*x + coef(glm_best)[3]*mean(bg_t2$live_stand_volume_125) + coef(glm_best)[4]*mean(bg_t2$proj_age_1)), add=TRUE)


# create model table (only do this once) and add the relevant data
top_mod_table <- data.frame (matrix (ncol = 7, nrow = 0))
colnames (top_mod_table) <- c ("ZONE", "TREED", "Model_terms", "intercept", "coef_climate", "coef_live_stand_vol", "coef_height")
top_mod_table[1,1]<-"BG"
top_mod_table[1,2]<-"Y"
top_mod_table[1,3]<-"fire_pres ~ ppt09 + live_stand_volume_125 + proj_height_1"
top_mod_table[1,4]<- coef(glm_best)[1]
top_mod_table[1,5]<- coef(glm_best)[2]
top_mod_table[1,6]<- coef(glm_best)[3]
top_mod_table[1,7]<- coef(glm_best)[4]
...
# examining  relationship between top climatic variable and ignition for bg  
p <- ggplot(bwbs, aes(tave07, as.numeric(fire_pres))) +
  geom_smooth(method="glm", formula=y~x,
              method.args=list(family="binomial"),
              alpha=0.3, size=1) +
  geom_point(position=position_jitter(height=0.03, width=0)) +
...
p <- ggplot(bwbs, aes(ppt07, as.numeric(fire_pres))) +
  geom_smooth(method="glm", formula=y~x,
              method.args=list(family="binomial"),
              alpha=0.3, size=1) +
  geom_point(position=position_jitter(height=0.03, width=0)) +
...
p <- ggplot(bwbs_t, aes(tave07, as.numeric(fire_pres))) +
  geom_smooth(method="glm", formula=y~x,
              method.args=list(family="binomial"),
              alpha=0.3, size=1) +
  geom_point(position=position_jitter(height=0.03, width=0)) +
...
p <- ggplot(bwbs_t, aes(ppt07, as.numeric(fire_pres))) +
  geom_smooth(method="glm", formula=y~x,
              method.args=list(family="binomial"),
              alpha=0.3, size=1) +
  geom_point(position=position_jitter(height=0.03, width=0)) +
...
# note stand volume and height are correlated. I should maybe not run these model combinations
...
i=2
table.glm[i,1]<-"glm2"
table.glm[i,2]<-"tave07 + ppt07 + proj_age_1"
table.glm[i,3]<-AIC(glm2)
...
i=3
table.glm[i,1]<-paste0("glm",i)
table.glm[i,2]<-"tave07 + ppt07 + proj_height_1"
table.glm[i,3]<-AIC(glm3)
...
i=4
table.glm[i,1]<-paste0("glm",i)
table.glm[i,2]<-"tave07 + ppt07 + live_stand_volume_125"
table.glm[i,3]<-AIC(glm4)
...
i=5
table.glm[i,1]<-paste0("glm",i)
table.glm[i,2]<-"tave07 + ppt07 + live_stand_volume_125 + proj_height_1"
table.glm[i,3]<-AIC(glm5)
...
i=6
table.glm[i,1]<-paste0("glm",i)
table.glm[i,2]<-"tave07 + ppt07 + live_stand_volume_125 +proj_age_1"
...
i=7
table.glm[i,1]<-paste0("glm",i)
table.glm[i,2]<-"tave07 + ppt07 + proj_age_1 + proj_height_1"
...
i=8
table.glm[i,1]<-paste0("glm",i)
table.glm[i,2]<-"tave07 + ppt07 + proj_age_1 + proj_height_1 + live_stand_volume_125"
...
bg_t2<-bg_t %>% drop_na(live_stand_volume_125, proj_height_1)
...
plot(bg_t2$live_stand_volume_125,bg_t2$fire_pres)
curve(invlogit(coef(glm_best)[1] +coef(glm_best)[2]*mean(bg_t2$ppt09) + coef(glm_best)[3]*x + coef(glm_best)[4]*mean(bg_t2$proj_height_1)), add=TRUE)

plot(bg_t2$proj_height_1,bg_t2$fire_pres)
curve(invlogit(coef(glm_best)[1] +coef(glm_best)[2]*mean(bg_t2$ppt09) + coef(glm_best)[3]*mean(bg_t2$live_stand_volume_125) + coef(glm_best)[4]*x), add=TRUE)

plot(bg_t2$ppt09,bg_t2$fire_pres)
curve(invlogit(coef(glm_best)[1] +coef(glm_best)[2]*x + coef(glm_best)[3]*mean(bg_t2$live_stand_volume_125) + coef(glm_best)[4]*mean(bg_t2$proj_height_1)), add=TRUE)
