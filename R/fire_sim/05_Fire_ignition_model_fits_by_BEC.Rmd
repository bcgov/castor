...
table(dat$bclcs_level_2) ##What is L? It is Land, exclude and do not use.

dat_t<- dat %>% dplyr::filter(bclcs_level_2=="T")
dat_nt<- dat %>% dplyr::filter(bclcs_level_2=="N")
dat_l<- dat %>% dplyr::filter(bclcs_level_2=="L")

table(dat$vegtype)
table(dat_t$vegtype) #either disturbed, open, treed broadleaf, treed conifer, or treed mixed broadleaf and conifer
table(dat_nt$vegtype) #either disturbed, open or shrub
...
##Note that n tree areas also have tree attributes
hist(dat_nt$proj_height_1)
hist(dat_nt$proj_age_1)
hist(dat_nt$live_stand_volume_125) #Pretty much all NAs. Can likely assume 0.
dat_nt$proj_height_1 #Also many NAs. 
dat_nt$proj_age_1 # Also many NAs. 

hist(dat_l$proj_height_1)
hist(dat_l$proj_age_1)
hist(dat_l$live_stand_volume_125)
## This is ok. Still exclude from models, because majority of polygon will not be treed

```


When doing the below analyses, it was noticed that disturbed areas always get eliminated when NAs are removed from the final data used for the model. Here, we investigate what variables are missing.

```{r}
dat_Disturbed<-subset(dat, dat$vegtype=="D")
head(dat_Disturbed)
...
dat <- within(dat, live_stand_volume_125[is.na(live_stand_volume_125) & vegtype == 'D'] <- 0)
...
glm13<- glm(fire_pres ~ scale(mean_tave06_tave07_tave08) +
             scale(live_stand_volume_125)* 
             vegtype,
           data= bg_t,
           family = binomial,
...
glm14<- glm(fire_pres ~ scale(mean_tave06_tave07_tave08) +
             scale(live_stand_volume_125)*vegtype + 
              scale(proj_age_1),
           data= bg_t,
           family = binomial,
...
i=14
table.glm[i,1]<-paste0("glm",i)
table.glm[i,2]<-"mean_tave06_tave07_tave08 + live_stand_volume_125*vegtype + proj_age_1"
table.glm[i,3]<-AICc(glm14)

glm15<- glm(fire_pres ~ scale(mean_tave06_tave07_tave08) +
              scale(proj_height_1) +
              scale(proj_age_1) +
              scale(live_stand_volume_125) +
             vegtype,
...
i=15
table.glm[i,1]<-paste0("glm",i)
table.glm[i,2]<-"mean_tave06_tave07_tave08 + proj_height_1 + proj_age_1 + live_stand_volume_125 + vegtype"
table.glm[i,3]<-AICc(glm15)

glm16<- glm(fire_pres ~ scale(mean_tave06_tave07_tave08) +
              scale(proj_height_1) +
              scale(proj_age_1) +
              scale(live_stand_volume_125) +
             vegtype,
...
# mean_tave06_tave07_tave08 + proj_height_1 + proj_age_1 + live_stand_volume_125 + vegtype
# mean_tave06_tave07_tave08 + proj_height_1 + proj_age_1 + live_stand_volume_125 * vegtype
##Use the one without the interaction because simpler and do not have NA for one coefficient
...
bwbs_t2<-bwbs_t %>% drop_na(live_stand_volume_125, proj_height_1, proj_age_1)
...
top_mod_table[2,2]<-"Y"
top_mod_table[2,3]<-"fire_pres ~ mean_tave06_tave07_tave08 + proj_height_1 + proj_age_1 + live_stand_volume_125 + vegtype"
