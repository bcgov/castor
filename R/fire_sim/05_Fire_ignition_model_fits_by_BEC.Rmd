...
##For this zone, vegtype and bclcs_level_4 are identical
...
top_mod_table ##Determine if better way to represent categories for bclcs_level_4, and if each climate variable should have its own column
...
table(bwbs_t$vegtype) # includes disturbed, so use this insead of bclcs_level4
table(bwbs_t$bclcs_level_4)
...
#top_mod_table[2,11]<- coef(glm_best)[4] #coefficient bclcs_level_4D
#top_mod_table[2,12]<- coef(glm_best)[4] #coefficient bclcs_level_4OP
#top_mod_table[2,13]<- coef(glm_best)[4] #coefficient bclcs_level_4S
top_mod_table[2,14]<- coef(glm_best)[4] #coefficient bclcs_level_4TB
top_mod_table[2,15]<- coef(glm_best)[5] #coefficient bclcs_level_4TC
top_mod_table[2,16]<- coef(glm_best)[6] #coefficient bclcs_level_4TM
top_mod_table
```
...
top_mod_table[3,2]<-"Y"
top_mod_table[3,3]<-"fire_pres ~ tave08 * ppt08 + live_stand_volume_125 + bclcs_level_4"
...
# Examining the relationship between some stand level variables. Volume and height are fairly correlated but age and volume are not. However, none of the variables are more correlated than 0.7. So Ill try all combinations
ggscatter(sbs, x = "live_stand_volume_125", y = "proj_age_1", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "kendall",
          xlab = "live stand volume", ylab = "Stand age")

ggscatter(sbs, x = "live_stand_volume_125", y = "proj_height_1", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "kendall",
          xlab = "live stand volume", ylab = "Stand height")

ggscatter(sbs, x = "proj_age_1", y = "proj_height_1", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "kendall",
          xlab = "proj_age_1", ylab = "Stand height")

hist(sbs$proj_age_1, breaks=30)
hist(sbs$live_stand_volume_125, breaks=30)
...
sbs_red<-sbs %>% drop_na(live_stand_volume_125, proj_height_1, vegtype)
...
top_mod_table[5,14]<- coef(glm_best)[7] #coefficient vegtypeTB
top_mod_table[5,15]<- coef(glm_best)[8] #coefficient vegtypeTC
top_mod_table[5,16]<- coef(glm_best)[9] #coefficient vegtypeTM
top_mod_table
