...
BEC zones still need to do: CWH, ICH, IDF, IMA, MH, MS, PP, SWB
...
p <- ggplot(ICH, aes(mean_tave06_tave07_tave08, as.numeric(fire_pres))) +
  geom_smooth(method="glm", formula=y~x,
              method.args=list(family="binomial"),
              alpha=0.3, size=1) +
  geom_point(position=position_jitter(height=0.03, width=0)) +
...
p <- ggplot(ICH_t, aes(mean_tave06_tave07_tave08, as.numeric(fire_pres))) +
  geom_smooth(method="glm", formula=y~x,
              method.args=list(family="binomial"),
              alpha=0.3, size=1) +
  geom_point(position=position_jitter(height=0.03, width=0)) +
...
hist(ICH_t$mean_tave06_tave07_tave08)
...
table(ICH_t$vegtype) # includes disturbed, so use this insead of bclcs_level4
table(ICH_t$bclcs_level_4)
...
ICH_t2<-ICH_t %>% drop_na(live_stand_volume_125)
...
binnedplot (ICH_t2$mean_tave06_tave07_tave08, 
            ICH_t2$resids, 
            nclass = NULL, 
            xlab = "", 
            ylab = "Average residual", 
