...
#for(i in select[1:length(select)]){
  ...
  #year.list1<-unique(sapply(strsplit(year.list, "-"),"[",1))
  years<-c("200001", "200501", "201001")#, "201501", "202001", "202501", "203001", "203501", "204001", "204501", "205001", "205501", "206001", "206501", "207001", "207501", "208001", "208501", "209001", "209501", "210001")
  #years<-year.list1[grep(paste(toMatchyears, collapse="|"), year.list1)]
  ...
  for(run in runs){
    ...
    for(element in elements){
      ...
      ref.files.element <- ref.files[grep(element, ref.files)]
      ...
      for(m in 1:12){
        ...
        r <- temp[[which(substr(names(temp),7,8)==monthcodes[m])]] # get all Januaries or Februaries etc
        if(element=="pr") r <- r*86400*monthdays[m] else r <- r-273.15  #convert units to /month (86400 seconds / day) and to degrees C
        #temp.rast <- if(m==1) r else brick(c(temp.rast, r))
        delta.r<- r-ref.rast
        names(delta.r) <- names(r)
      }
      
      
      writeRaster(delta.r, paste("D:\\Fire\\fire_data\\raw_data\\Future_climate\\outputs\\MPI-ESM1-2-HR_delta_values", element,"_", gcm, "_",years[1],"_",years[3], "_month_",monthcodes[m], ".nc", sep=""), format="CDF", overwrite=TRUE)
      print(element)
      }
