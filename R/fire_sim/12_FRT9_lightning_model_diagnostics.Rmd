...
#Checking model fits 
```{r}
dat<- dat_lightning %>% dplyr::select(fire_yr, ig_mnth, frt, fire, elevatn, climate1, FWI_veg)

table(dat$frt, dat$FWI_veg, dat$fire)

dat9<- dat %>% dplyr::filter(frt == 9) 
table(dat9$FWI_veg, dat9$fire)

dat9$FWI_veg[dat9$FWI_veg=="M-3"]<-"O-1a/b"
#dat9$FWI_veg[dat9$FWI_veg=="S-2"]<-"M-1/2"
#dat9_2<-dat9 %>% filter(FWI_veg!="N")
table(dat9$FWI_veg)

dat9$FWI_veg <- factor(dat9$FWI_veg, levels=c("C-3","C-1","C-2","C-7", "M-1/2", "N", "O-1a/b"))

ggplot(data=dat9, mapping=aes(x=as.factor(fire), y=climate1))+
  geom_boxplot()

ggplot(data=dat9, mapping=aes(x=as.factor(fire), y=elevatn))+
  geom_boxplot()

cor.test(dat9$climate1, dat9$elevatn)

mod.frt.9<-glm(fire ~ climate1 + FWI_veg + elevatn, data=dat9, family=binomial(link="logit"))

# looking at difference in link function
modclim_logit<-glm(fire ~ climate1, data=dat9, family=binomial(link="logit"))
modclim_cloglog<-glm(fire ~ climate1, data=dat9, family=binomial(link="cloglog"))
...
visreg(mod.frt.9, "climate1", by="elevatn", scale="response")
visreg(mod.frt.9, "climate1", by="FWI_veg", scale="response")
visreg(mod.frt.9, "elevatn", by="FWI_veg", scale="response")
visreg(mod.frt.9, "climate1", by="FWI_veg", overlay=TRUE, scale="response")
visreg(mod.frt.9, "climate1", by="elevatn", overlay=TRUE, scale="response")
...
dat2_b<-dat9%>% dplyr::select(fire, elevatn, climate1)
...
colnames (top_mod_table_FRT10_lightningALL ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1", "coef_FWI_veg_C-1","coef_FWI_veg_C-2", "coef_FWI_veg_C-7", "coef_FWI_veg_M-1/2", "coef_FWI_veg_N", "coef_FWI_veg_O-1a/b", "coef_elevatn", "AUC")

dat9$fire_veg<-paste(dat9$fire, dat9$FWI_veg)
```
...
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(dat9$fire_veg, p = prop,
                                    list = FALSE,
                                    times = 1)
...
colnames (top_mod_table_FRT10_lightningL ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1", "coef_FWI_veg_C-1","coef_FWI_veg_C-2", "coef_FWI_veg_C-7", "coef_FWI_veg_M-1/2", "coef_FWI_veg_N", "coef_FWI_veg_O-1a/b", "coef_elevatn", "AUC")


##Add data for NDT1
top_mod_table_FRT10_lightningL[1,1]<-"lightning"
top_mod_table_FRT10_lightningL[1,2]<-"FRT9"
top_mod_table_FRT10_lightningL[1,3]<-"fire ~ climate1 + FWI_veg + elevatn"
...
FRT10_summary_table_mean$FRT<-9
FRT10_summary_table_mean$intercept_veg<-"C-3"
FRT10_summary_table_mean$Model_terms<-"fire ~ climate1 + FWI_veg + elevatn"
...
```{r}
write.csv(FRT10_summary_table_mean, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_FRT9_lightning.csv")
```
