...
#Checking model fits 
```{r}
dat<- dat_lightning %>% dplyr::select(fire_yr, ig_mnth, frt, fire, dm_h_bc, climate1, fwveg)

table(dat$frt, dat$fwveg, dat$fire)

dat9<- dat_lightning %>% dplyr::filter(frt == 9) 
table(dat9$fwveg, dat9$fire)

dat9$fwveg[dat9$fwveg=="M-3"]<-"O-1a/b"
#dat9$fwveg[dat9$fwveg=="S-2"]<-"M-1/2"
#dat9_2<-dat9 %>% filter(fwveg!="N")
table(dat9$fwveg)

dat9_a<-dat9 %>% drop_na(fwveg)

dat9$fwveg <- factor(dat9$fwveg, levels=c("C-3","C-1","C-2","C-7", "M-1/2", "O-1a/b"))
...
mod.frt.9<-glm(fire ~ climate1 + fwveg, data=dat9_a, family=binomial(link="logit"))
...
mod.frt.9b<-glm(fire ~ climate1, data=dat9_a, family=binomial(link="logit"))
summary(mod.frt.9b)
Anova(mod.frt.9b, type=3)
anova(mod.frt.9,mod.frt.9b, test="Chisq") 

# end model:
mod.frt.9<-glm(fire ~ climate1, data=dat9, family=binomial(link="logit"))
...
dat2_b<-dat9%>% dplyr::select(fire, climate1)
...
top_mod_table_FRT10_lightningALL <- data.frame (matrix (ncol = 6, nrow = 0))
colnames (top_mod_table_FRT10_lightningALL ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1", "AUC")

dat9$fire_veg<-paste(dat9$fire, dat9$fwveg)
```
...
#Model   
mod<-glm(fire ~ climate1, data=dat1, family=binomial(link="logit"))
...
top_mod_table_FRT10_lightningL <- data.frame (matrix (ncol = 6, nrow = 0))
colnames (top_mod_table_FRT10_lightningL ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1", "AUC")


##Add data for NDT1
top_mod_table_FRT10_lightningL[1,1]<-"lightning"
top_mod_table_FRT10_lightningL[1,2]<-"FRT9"
top_mod_table_FRT10_lightningL[1,3]<-"fire ~ climate1"
top_mod_table_FRT10_lightningL[1,4]<- coef(mod)[1] #Intercept
top_mod_table_FRT10_lightningL[1,5]<- coef(mod)[2] #Climate variable
top_mod_table_FRT10_lightningL[1,6]<- mod.auc
...
FRT10_summary_table_mean$Model_terms<-"fire ~ climate1"
