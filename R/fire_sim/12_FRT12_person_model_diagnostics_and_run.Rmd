...
dat12$FWI_veg[dat12$FWI_veg=="M-3"]<-"M-1/2"
# going to combine slash categories because S-2 only has 6 obs.
dat12$FWI_veg[dat12$FWI_veg=="S-2"]<-"S-1"

# also remove water and N
dat12<- dat12 %>% filter(!FWI_veg %in%  c("N", "W"))
table(dat12$FWI_veg)
...
ggplot(data=dat12, mapping=aes(x=as.factor(fire), y=log(rd_dst_+1)))+
  geom_boxplot()


mod_frt_12a<- glm(fire ~ climate1 + elevatn + FWI_veg + log(rd_dst_+1) + log(infr_ds+1), data=dat12, family=binomial(link="logit"))

mod_frt12b<- glm(fire ~ climate1 + elevatn + FWI_veg + log(rd_dst_+1), data=dat12, family=binomial(link="logit"))
anova(mod_frt_12a,mod_frt12b, test="Chisq") 

mod_frt12c<- glm(fire ~ climate1 + elevatn + FWI_veg + log(infr_ds+1), data=dat12, family=binomial(link="logit"))
anova(mod_frt_12a,mod_frt12c, test="Chisq") 

mod_frt_12d<- glm(fire ~ climate1 + FWI_veg + log(rd_dst_+1) + log(infr_ds+1), data=dat12, family=binomial(link="logit"))
anova(mod_frt_12a,mod_frt_12d, test="Chisq") 


# best model is mod_frt_12a It has the lowest AIC (AIC=6080)

dat12$log_road_dist<-log(dat12$rd_dst_+1)
dat12$log_inf_dist<-log(dat12$infr_ds+1)

mod_frt12<- glm(fire ~ climate1 + elevatn + FWI_veg + log_road_dist + log_inf_dist, data=dat12, family=binomial(link="logit"))

summary(mod_frt12)
Anova(mod_frt12, type=3)
...
##Partial Residuals
visreg(mod_frt12, scale="response") # nicer plots than the ones below!
...
probabilities <- predict(mod_frt12, type = "response")
...
model.data %>% 
  filter(abs(.std.resid) > 2.9) # there are no data points with a standardized residual of greater than 3. So we are all good.
...
```{r}
mod_frt12<- glm(fire ~ climate1 + elevatn + log_road_dist + log_inf_dist + FWI_veg, data=dat12_2, family=binomial(link="logit"))

summary(mod_frt12)

#Create a new blank table and get AUC too
top_mod_table_FRT5_person_All <- data.frame (matrix (ncol = 16, nrow = 0))
colnames (top_mod_table_FRT5_person_All ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1", "coef_elevatn", "coef_log_road_dist", "coef_log_inf_dist", "coef_FWI_veg_C-3", "coef_FWI_veg_C-5", "coef_FWI_veg_C-7", "coef_FWI_veg_D-1/2", "coef_FWI_veg_M-1/2","coef_FWI_veg_O-1a/b", "coef_FWI_veg_S-1",  "AUC")
...
colnames (top_mod_table_FRT5_person ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1", "coef_elevatn", "coef_log_road_dist", "coef_log_inf_dist", "coef_FWI_veg_C-3", "coef_FWI_veg_C-5", "coef_FWI_veg_C-7", "coef_FWI_veg_D-1/2", "coef_FWI_veg_M-1/2","coef_FWI_veg_O-1a/b", "coef_FWI_veg_S-1",  "AUC")
