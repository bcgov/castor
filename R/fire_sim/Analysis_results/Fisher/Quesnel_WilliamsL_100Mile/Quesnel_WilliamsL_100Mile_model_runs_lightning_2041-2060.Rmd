...
future_clim_2 <- future_clim_2 %>%
  mutate(climate2_2050 =if_else(frt==10, mean_PPT07_PPT08_PPT09,
                                    if_else(frt==11, mean_PPT07_PPT08_PPT09,
                                    if_else(frt==13, as.numeric(PPT07),
                                            if_else(frt==15, mean_PPT06_PPT07_PPT08, NA_real_)))))

future_clim_3<-future_clim_2 %>% dplyr::select(idno, climate1_2050, climate2_2050)
...
dat2<- dat2 %>% filter(frt %in% c(10,12,13,14,15)) %>%
  filter(FWI_veg !="W") %>%
  filter(!bclcs_level_5 %in% c("GL", "LA"))
...
frt10$logit_P<- model_coef_table[[2]] + 
  ...
  model_coef_table[[4]]*frt10$climate2_2050 +
...
frt13$logit_P<- model_coef_table[[2]] + 
  ...
  model_coef_table[[4]]*frt13$climate2_2050 +
...
frt15$logit_P<- model_coef_table[[2]] + 
  model_coef_table[[3]]*frt15$climate2_2050 +
...
names(no_ignition)
no_ignition<-no_ignition %>% dplyr::select(idno:climate2_2050, geom)
no_ignition$logit_P<-0
no_ignition$prob_ignition<-0

frt_all2<-rbind(frtall, no_ignition)

st_write(frt_all2, "C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\Probability_of_lightning_Ignition_Quesnell_WilliamsL_100Mile_allFRT_800m_2050.gpkg", delete_layer=TRUE, driver="GPKG")
```
