...
no_ignition<-dat %>% filter(FWI_veg %in% c("N", "W"))

dat<- dat %>% filter(!FWI_veg %in% c("N", "W"))
...
table(frt10$FWI_veg)
frt10$FWI_veg[frt10$FWI_veg=="S-1"]<-"S-2"
frt10$FWI_veg[frt10$FWI_veg=="C-2"]<-"C-3"
...
frt10$logit_P<- model_coef_table[[2]] + 
  ...
  model_coef_table[[12]]*frt10$veg_S2 +
...
frt12$logit_P<- model_coef_table[[2]] + 
  ...
  model_coef_table[[13]]*frt12$veg_S1 +
  model_coef_table[[14]]*frt12$veg_S2 + 
  model_coef_table[[15]]*frt12$Elevation
...
frt13$FWI_veg[frt13$FWI_veg=="C-4"]<-"C-3"
...
frt14$FWI_veg[frt14$FWI_veg=="S-2"]<-"S-1"
...
table(frt15$FWI_veg)
frt15$FWI_veg[frt15$FWI_veg=="C-3"]<-"C-7" # this is not great!
...
frt15$logit_P<- model_coef_table[[2]] + 
  ...
  model_coef_table[[9]]*frt15$Elevation
...
names(frt_all)
frtall<-frt_all %>% select(idno:climate2, logit_P:geom)
names(no_ignition)
no_ignition<-no_ignition %>% select(idno:climate2, geom)
no_ignition$logit_P<-0
no_ignition$prob_ignition<-0

frt_all2<-rbind(frtall, no_ignition)

st_write(frt_all2, "C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\Probability_of_lightning_Ignition_Quesnell_WilliamsL_100Mile_allFRT_800m.gpkg", delete_layer=TRUE, driver="GPKG")
...
writeRaster(maxint, file=" C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\prob_of_lighting_ignition.tif", format="GTiff", overwrite=TRUE)
