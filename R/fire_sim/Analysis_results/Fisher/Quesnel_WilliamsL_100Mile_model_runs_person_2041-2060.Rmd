...
sf::st_write(dat2, dsn = "C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\Quesnel_WilliamsL_100Mile_points_with_rds_infrastructure.gpkg", delete_layer=TRUE)
...
future_clim<-read.csv("D:\\Fire\\fire_data\\raw_data\\ClimateBC_Data\\climate_BC_input\\Quesnel_WilliamsL_100Mile_8GCMs_ensemble_ssp370_2041MP.csv")
...
# human caused fires
future_clim$mean_PPT06_PPT07 <-future_clim$PPT06 + future_clim$PPT07

future_clim$mean_Tave04_Tave05_Tave06_Tave07_Tave08_Tave09_Tave10<-(future_clim$Tave04 + future_clim$Tave05 + future_clim$Tave06 + future_clim$Tave07 +future_clim$Tave08 + future_clim$Tave09 + future_clim$Tave10)/7

future_clim$mean_PPT06_PPT07_PPT08_PPT09 <- (future_clim$PPT06 + future_clim$PPT07 + future_clim$PPT08 + future_clim$PPT09)/4

future_clim$mean_Tave08_Tave09_Tave10<-(future_clim$Tave08 + future_clim$Tave09 + future_clim$Tave10)/3

future_clim$mean_Tmax04_Tmax05_Tmax06_Tmax07_Tmax08_Tmax09_Tmax10<-(future_clim$Tmax04 + future_clim$Tmax05 + future_clim$Tmax06 + future_clim$Tmax07 +future_clim$Tmax08 + future_clim$Tmax09 + future_clim$Tmax10)/7
...
future_clim_2<- future_clim %>% dplyr::select(ID1, ID2, Latitude, Longitude, Elevation, Tmax01: PPT12, mean_PPT06_PPT07:mean_PPT07_PPT08_PPT09) %>%
  rename(idno=ID1,
         frt=ID2)

table(is.na(future_clim_2$frt))
future_clim_2$climate1_2050<-NA
...
# #Repeat for climate 2
 future_clim_2$climate2_2050<-"NA"
# 
# #Perform mutate to get the applicable variable for each row
future_clim_2<-future_clim_2 %>%
    mutate(climate2_2050 = case_when(
                                frt == "13" ~ mean_PPT07_PPT08_PPT09,
                                frt == "15" ~ mean_PPT07_PPT08_PPT09,
                               TRUE ~ NA_real_))


table(future_clim_2$idno)

future_clim_dat<-future_clim_2 %>% dplyr::select(idno, climate1_2050, climate2_2050)

dat_future<-left_join(dat, future_clim_dat)
```
...
dat_future$veg_C2 <- ifelse(dat_future$FWI_veg == 'C-2', 1, 0)
dat_future$veg_C3 <- ifelse(dat_future$FWI_veg == 'C-3', 1, 0)
dat_future$veg_C4 <- ifelse(dat_future$FWI_veg == 'C-4', 1, 0)
dat_future$veg_C5 <- ifelse(dat_future$FWI_veg == 'C-5', 1, 0)
dat_future$veg_C7 <- ifelse(dat_future$FWI_veg == 'C-7', 1, 0)
dat_future$veg_D12 <- ifelse(dat_future$FWI_veg == 'D-1/2', 1, 0)
dat_future$veg_M12 <- ifelse(dat_future$FWI_veg == 'M-1/2', 1, 0)
dat_future$veg_M3 <- ifelse(dat_future$FWI_veg == 'M-3', 1, 0)
dat_future$veg_N <- ifelse(dat_future$FWI_veg == 'N', 1, 0)
dat_future$veg_O1ab <- ifelse(dat_future$FWI_veg == 'O-1a/b', 1, 0)
dat_future$veg_S1 <- ifelse(dat_future$FWI_veg == 'S-1', 1, 0)
dat_future$veg_S2 <- ifelse(dat_future$FWI_veg == 'S-2', 1, 0)
dat_future$veg_S3 <- ifelse(dat_future$FWI_veg == 'S-3', 1, 0)
dat_future$veg_W <- ifelse(dat_future$FWI_veg == 'W', 1, 0)

names(dat_future)
table(dat_future$frt, dat_future$FWI_veg)
dat_future$log_dist_roads_m<-log(dat_future$dist_roads_m + 1)
dat_future$log_dist_infr_m<-log(dat_future$dist_infr_m + 1)

no_ignition<-dat_future %>% 
...
dat_future2<- dat_future %>% filter(frt %in% c(10,12,13,14,15)) %>%
  filter(FWI_veg !="W") %>%
  filter(!bclcs_level_5 %in% c("GL", "LA"))
table(dat_future2$bclcs_level_5)
...
```{r}
frt10<- dat_future2 %>% filter(frt==10) 
table(frt10$FWI_veg)
...
frt10$logit_P<- model_coef_table[[2]] + 
  model_coef_table[[3]]*frt10$climate1_2050 +
...
# FRT 12
```{r}
frt12<- dat_future2 %>% filter(frt==12)
...
frt12$logit_P<- model_coef_table[[2]] + 
  model_coef_table[[3]]*frt12$climate1_2050 +
...
# FRT 13
```{r}
frt13<- dat_future2 %>% filter(frt==13)
table(frt13$FWI_veg)
...
frt13$logit_P<- model_coef_table[[2]] + 
  model_coef_table[[3]]*frt13$climate1_2050 +
...
frt14<- dat_future2 %>% filter(frt==14)
table(frt14$FWI_veg)
#NOTE C2 is the intercept
...
frt14$logit_P<- model_coef_table[[2]] + 
  model_coef_table[[3]]*frt14$climate1_2050 +
...
frt15<- dat_future2 %>% filter(frt==15)
table(frt15$FWI_veg)
...
frt15$logit_P<- model_coef_table[[2]] + 
  model_coef_table[[3]]*frt15$climate1_2050 +
...
st_write(frt_all, "C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\Probability_of_person_Ignition_Quesnell_WilliamsL_100Mile_allFRT_800m_future2050.gpkg", delete_layer=TRUE, driver="GPKG")
```
...
writeRaster(maxint, file=" C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\Probability_of_Person_Ignition_Quesnell_WilliamsL_100Mile_allFRT_800m_future2050.tif", format="GTiff", overwrite=TRUE)
```


```{r}
person.ignit<-raster(" C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\Probability_of_Person_Ignition_Quesnell_WilliamsL_100Mile_allFRT_800m_future2050.tif")

lightning.ignit<-raster(" C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\Probability_of_lightning_Ignition_Quesnell_WilliamsL_100Mile_allFRT_800m_2050.tif")
...
writeRaster(total.ignit, file=" C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\prob_of_lightning_and_person_ignition_2050.tif", format="GTiff", overwrite=TRUE)
