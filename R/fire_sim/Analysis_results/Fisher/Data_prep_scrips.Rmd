...
```{r cars}
study_area<-getSpatialQuery("SELECT * FROM public.tsa_boundaries")

#st_write(study_area, "C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\Skeena_Omineca_Thompson_Okanagan\\Kyle_fisher_study_area.gpkg", delete_layer=TRUE, driver="GPKG")

plot(study_area)

# change grid size to 100 when this code works
x<-st_make_grid(study_area, cellsize=c(400,400), crs=3005)
x2<- st_centroid(x)
...
export_dat<- x4 %>% rename(ID1 = idno,
                           ...
                           lon = longitude,
                           el = dem) %>%
  dplyr::select(ID1, ID2, lat, lon, el)

table(is.na(export_dat$el))
export_dat$el[is.na(export_dat$el)]<-"."


#max(export_dat$lat)

write.csv(export_dat, "C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\bc_dem_frt.csv", row.names = FALSE)
...
## 2041_2070 Not sure if this works. Testing it now
inputFile = '/C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\bc_dem_frt.csv' 
outputFile = '/C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\bc_dem_frt_clim_2041_2070.csv'
yearPeriod = '/8GCMs_ensemble_ssp370_2041-2070.gcm'
system2(exe,args= c('/M', yearPeriod, inputFile, outputFile))

####################
```

Import data and pull out important fields
```{r}
x4_clim_dem<-read.csv("C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\bc_dem_frt_clim.csv")



x4_clim_dem$Tmax02<-as.numeric(x4_clim_dem$Tmax02)
x4_clim_dem$Elevation<-as.numeric(x4_clim_dem$Elevation)
x4_clim_dem<-x4_clim_dem %>% filter(Tmax02>-100)

dim(x4_clim_dem)

table(is.na(x4_clim_dem$Elevation))
...
x4_clim_dem$mean_Tave05_Tave06_Tave07_Tave08 <- (x4_clim_dem$Tave05 + x4_clim_dem$Tave06 + x4_clim_dem$Tave07 + x4_clim_dem$Tave08)/4
x4_clim_dem$mean_PPT05_PPT06_PPT07_PPT08<-(x4_clim_dem$PPT05 + x4_clim_dem$PPT06 + x4_clim_dem$PPT07 + x4_clim_dem$PPT08)/4

x4_clim_dem$meanRH05_RH06_RH07<-(x4_clim_dem$RH05 + x4_clim_dem$RH06 + x4_clim_dem$RH07)/3
...
x4_clim_dem$mean_Tmax07_Tmax08_Tmax09<-(x4_clim_dem$Tmax07 + x4_clim_dem$Tmax08 + x4_clim_dem$Tmax09)/3
...
x4_clim_dem$mean_PPT06_PPT07 <-(x4_clim_dem$PPT06 + x4_clim_dem$PPT07)/2

x4_clim_dem$mean_Tave04_Tave05_Tave06_Tave07_Tave08_Tave09_Tave10<-(x4_clim_dem$Tave04 + x4_clim_dem$Tave05 + x4_clim_dem$Tave06 + x4_clim_dem$Tave07 +x4_clim_dem$Tave08 + x4_clim_dem$Tave09 + x4_clim_dem$Tave10)/7

x4_clim_dem$mean_PPT06_PPT07_PPT08_PPT09 <- (x4_clim_dem$PPT06 + x4_clim_dem$PPT07 + x4_clim_dem$PPT08 + x4_clim_dem$PPT09)/4

x4_clim_dem$mean_Tave08_Tave09_Tave10<-(x4_clim_dem$Tave08 + x4_clim_dem$Tave09 + x4_clim_dem$Tave10)/3

x4_clim_dem$mean_Tmax04_Tmax05_Tmax06_Tmax07_Tmax08_Tmax09_Tmax10<-(x4_clim_dem$Tmax04 + x4_clim_dem$Tmax05 + x4_clim_dem$Tmax06 + x4_clim_dem$Tmax07 +x4_clim_dem$Tmax08 + x4_clim_dem$Tmax09 + x4_clim_dem$Tmax10)/7
...
x4_clim_dem_2<- x4_clim_dem %>% dplyr::select(ID1, ID2, Latitude, Longitude, Elevation, Tmax05,Tave07, PPT07, mean_Tave05_Tave06_Tave07_Tave08:mean_Tmax04_Tmax05_Tmax06_Tmax07_Tmax08_Tmax09_Tmax10) %>%
  rename(idno=ID1,
         frt=ID2)
...
# note the elevation seems to have been messed up. Im not sure why
my.sf.point<-my.sf.point %>% select(!Elevation)
x<-read.csv("C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\bc_dem_frt.csv")

x<- x %>% select(ID1, ID2, el) %>% rename(idno=ID1,
                                          frt=ID2)

my.sf.point.joined<-left_join(my.sf.point, x)
head(my.sf.point.joined)

st_write(my.sf.point.joined, "C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\BC_data.gpkg", delete_layer=TRUE, driver="GPKG")

```
Now in QGIS join the above layer to the VRI.


```{r}
dat<- st_read("C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\BC_data_veg.gpkg")

rm(pointCoordinates,ras.frt, rasStack,rasValue2,test, x,x2,x3,x3_dem)

table(dat$idno)

#my.sf.point<-st_read( "C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\Quesnel_WilliamsLake_100Mile.gpkg")


#do this part if dat is a multiploygon. But if its a point dont worry about this
dat<-st_drop_geometry(dat)

names(dat)
dat2<-dat %>% dplyr::select(idno, frt, inventory_standard_cd: proj_height_2)

dat3<-left_join(my.sf.point, dat2)
...
Now save it
```{r}
st_write(fire_all1, "C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\BC_Elev_clim_veg.gpkg", delete_layer=TRUE, driver="GPKG")

```


###########################################################
#### join slope, aspect, road, infrastructure, and wind rasters
###########################################################
...
rasStack = stack(DEM_slope, DEM_aspect, roads_dist, dist_rail, dist_power, dist_oil, dist_mines, dist_urban)

##Try this first
test<-cbind(fire_all1, st_coordinates(fire_all1))
head(test)
...
fire_all1_slope_aspect<-cbind(fire_all1, rasValue2)
head(fire_all1_slope_aspect)
crs(fire_all1_slope_aspect)
...
rm(rasStack, rasValue2)
rasStack<- stack(summer_wind_raster, spring_wind_raster)
rasValue2=raster::extract(rasStack, pointCoordinates)
fire_all1_slope_aspect_wind<-cbind(fire_all1_slope_aspect, rasValue2)

# get distance to closest infrastructure
fire_all1_slope_aspect_wind$dist_roads_m<-fire_all1_slope_aspect_wind$dist_roads*100
fire_all1_slope_aspect_wind$dist_mines_m<-fire_all1_slope_aspect_wind$dist_mines*100
fire_all1_slope_aspect_wind$dist_oil_m<-fire_all1_slope_aspect_wind$dist_oil*100
fire_all1_slope_aspect_wind$dist_power_m<-fire_all1_slope_aspect_wind$dist_power*100
fire_all1_slope_aspect_wind$dist_rail_m<-fire_all1_slope_aspect_wind$dist_rail*100
fire_all1_slope_aspect_wind$dist_urban_m<-fire_all1_slope_aspect_wind$dist_urban*100

fire_all1_slope_aspect_wind$dist_infrastructure_m<-0


fire_all1_slope_aspect_wind$dist_infrastructure_m<-
  ifelse(fire_all1_slope_aspect_wind$dist_rail_m < fire_all1_slope_aspect_wind$dist_power_m, fire_all1_slope_aspect_wind$dist_rail_m, fire_all1_slope_aspect_wind$dist_power_m)

fire_all1_slope_aspect_wind$dist_infrastructure_m<-
  ifelse(fire_all1_slope_aspect_wind$dist_oil_m < fire_all1_slope_aspect_wind$dist_infrastructure_m, fire_all1_slope_aspect_wind$dist_oil_m, fire_all1_slope_aspect_wind$dist_infrastructure_m)

fire_all1_slope_aspect_wind$dist_infrastructure_m<-
  ifelse(fire_all1_slope_aspect_wind$dist_mines_m < fire_all1_slope_aspect_wind$dist_infrastructure_m, fire_all1_slope_aspect_wind$dist_mines_m, fire_all1_slope_aspect_wind$dist_infrastructure_m)

fire_all1_slope_aspect_wind$dist_infrastructure_m<-
  ifelse(fire_all1_slope_aspect_wind$dist_urban_m < fire_all1_slope_aspect_wind$dist_infrastructure_m, fire_all1_slope_aspect_wind$dist_urban_m, fire_all1_slope_aspect_wind$dist_infrastructure_m)

fire_all1_slope_aspect_wind2<- fire_all1_slope_aspect_wind  %>% dplyr::select(idno:mean_Tmax04_Tmax05_Tmax06_Tmax07_Tmax08_Tmax09_Tmax10, bclcs_level_5, FWI_veg:aspect_ha_bc_3005, wind_summer_clipped_224,wind_spring_raster_224, dist_infrastructure_m, dist_roads_m, geom) %>% 
  rename(win_spr = wind_spring_raster_224,
         win_sum = wind_summer_clipped_224,
         slope = slope_ha_bc_3005,
         aspect = aspect_ha_bc_3005)

st_write(fire_all1_slope_aspect_wind2, "C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\BC_Elev_clim_veg_rd_infr_wind.gpkg", delete_layer=TRUE, driver="GPKG")
