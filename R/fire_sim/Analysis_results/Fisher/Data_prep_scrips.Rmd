...
#Elevation
DEM <- raster("T:\\FOR\\VIC\\HTS\\ANA\\PROJECTS\\CLUS\\Data\\dem\\all_bc\\dem.tif")
plot(DEM)
...
```{r cars}
study_area<-getSpatialQuery("SELECT * FROM public.fisher_zones")

#st_write(study_area, "C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\Skeena_Omineca_Thompson_Okanagan\\Kyle_fisher_study_area.gpkg", delete_layer=TRUE, driver="GPKG")
...
write.csv(export_dat, "C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\Skeena_Omineca_Thompson_Okanagan\\Fisher_study_area.csv", row.names = FALSE)
...
## 2021
inputFile = '/C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\Skeena_Omineca_Thompson_Okanagan\\Fisher_study_area.csv' 
outputFile = '/C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\Skeena_Omineca_Thompson_Okanagan\\Fisher_study_area_out.csv'
yearPeriod = '/Year_2021.ann'
system2(exe,args= c('/M', yearPeriod, inputFile, outputFile))


x4_clim_dem<-read.csv("C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\Skeena_Omineca_Thompson_Okanagan\\Fisher_study_area_out.csv")
...
#If you need to open the table listing the best climate
...
climate_variables_person<-read.csv("C:/Work/caribou/clus_data/Fire/Fire_sim_data/data/climate_AIC_results_person_FRT_summary.csv")
...
x4_clim_dem$RH05_RH06_RH07<-x4_clim_dem$RH05 + x4_clim_dem$RH06 + x4_clim_dem$RH07

# human caused fires
x4_clim_dem$mean_PPT06_PPT07 <-x4_clim_dem$PPT06 + x4_clim_dem$PPT07
...
x4_clim_dem_2<- x4_clim_dem %>% dplyr::select(ID1, ID2, Latitude, Longitude, Elevation, Tmax01: PPT12, mean_Tave07_Tave08_Tave09:mean_Tmax04_Tmax05_Tmax06_Tmax07_Tmax08_Tmax09_Tmax10) %>%
  rename(idno=ID1,
         frt=ID2)
...
st_write(my.sf.point, "C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\Skeena_Omineca_Thompson_Okanagan\\Fisher_study_area.gpkg", delete_layer=TRUE, driver="GPKG")
...
```{r}
dat<- st_read("C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\Skeena_Omineca_Thompson_Okanagan\\Fisher_study_area_veg.gpkg")

rm(pointCoordinates,ras.frt, rasStack,rasValue2,test, x,x2,x3,x3_dem)

table(dat$idno)

#my.sf.point<-st_read( "C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\Quesnel_WilliamsLake_100Mile.gpkg")
...
Now save it
```{r}
st_write(fire_all1, "C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\Skeena_Omineca_Thompson_Okanagan\\Fisher_study_area_veg_FWI.gpkg", delete_layer=TRUE, driver="GPKG")
