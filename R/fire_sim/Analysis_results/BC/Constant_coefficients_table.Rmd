...
frt_const<-frt_all %>% dplyr::select(idno, frt, logit_P_lightning_coef:logit_P_spread_coef) %>%
...
# save it!
```{r}
st_write(frt_const, "C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\estimated_coefficients\\BC_ignit_escape_spread_final_constant_coefficients.gpkg", delete_layer=TRUE, driver="GPKG")
```


```{r}
frt_const<- frt_const %>% st_drop_geometry()

frt_coefficients<-left_join(frt_var, frt_const)

st_write(frt_coefficients, "C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\estimated_coefficients\\BC_ignit_escape_spread_final_all_coefficients_2021.gpkg", delete_layer=TRUE, driver="GPKG")
```
