...
dat2<- dat %>% select(idno, frt, frt_escape, FWI_veg, bclcs_level_5)
names(dat2)
...
clim2044<-read.csv("C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\Future_climate_bc\\bc_dem_frt_8GCMs_ensemble_ssp370_2039M.csv")

clim2049<-read.csv("C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\Future_climate_bc\\bc_dem_frt_8GCMs_ensemble_ssp370_2039M.csv")
```
...
###################################################
clim<-clim %>% select(idno, frt, mean_Tave05_Tave06_Tave07_Tave08,
                      meanRH05_RH06_RH07,
                      Tmax05,
                      Tave07,
...
no_ignition<-no_ignition %>% select(idno, frt, FWI_veg, varying_coef_lightning :varying_coef_spread)

frt_all2<-frt_all %>% select(idno, frt, FWI_veg, varying_coef_lightning :varying_coef_spread)
...
# save it!
```{r}
st_write(frt_all2, "C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\estimated_coefficients\\BC_ignit_escape_spread_varying_coefficient_2049.gpkg", delete_layer=TRUE, driver="GPKG")
```
