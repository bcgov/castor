...
Now save it
```{r}
st_write(fire_all1, "C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\BC_Elev_veg.gpkg", delete_layer=TRUE, driver="GPKG")
...
st_write(fire_all1_slope_aspect_wind, "C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\BC_Elev_veg_rd_infr_wind.gpkg", delete_layer=TRUE, driver="GPKG")
...
```{r}
x4<-read.csv("C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\bc_dem_frt_clim2.csv")
head(fire_all1_slope_aspect_wind)

x5<-x4 %>% dplyr::select(ID1:Longitude, Tmax01:PPT12, RH01:RH12) %>%
  dplyr::rename(idno=ID1,
         frt=ID2)

dat_final<-left_join(fire_all1_slope_aspect_wind, x5)
...
st_write(dat_final, "C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\BC_all_dat_400m_final.gpkg", delete_layer=TRUE, driver="GPKG")
