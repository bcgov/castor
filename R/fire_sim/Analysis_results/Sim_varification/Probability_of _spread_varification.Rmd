...
hist(x$rast_spread_2022)

bins<-quantile(x$rast_spread_2022, probs = seq(0, 1, 1/10), na.rm=TRUE)

# 
# y<-x %>% mutate(x_bin = ntile(layer, n=10))
# table(y$x_bin)

y_alt<-x %>% mutate(x_bin = cut(rast_spread_2022, breaks=10))
...
```{r}
fire_bounds_current<-try(
  bcdc_query_geodata("WHSE_LAND_AND_NATURAL_RESOURCE.PROT_CURRENT_FIRE_POLYS_SP") %>% collect()
)

table(fire_bounds_current$FIRE_YEAR)

names(fire_bounds_current)
hist(fire_bounds_current$FIRE_SIZE_HECTARES)
fire_bounds_current2<-fire_bounds_current %>% filter(FIRE_SIZE_HECTARES>10)
dim(fire_bounds_current)
dim(fire_bounds_current2)
```


```{r}
foo_sp<-as(fire_bounds_current2,"Spatial")
...
ggplot() +
 geom_raster(data = spread_pts_df , aes(x = x, y = y, fill = rast_spread_2022)) +
...
y_alt5<-x_frt5 %>% mutate(x_bin = cut(rast_spread_2022, breaks=10))
...
y_alt7<-x_frt7 %>% mutate(x_bin = cut(rast_spread_2022, breaks=10))
...
y_alt7<-x_frt9 %>% mutate(x_bin = cut(rast_spread_2022, breaks=10))
...
y_alt7<-x_frt10 %>% mutate(x_bin = cut(rast_spread_2022, breaks=10))
...
y_alt7<-x_frt11 %>% mutate(x_bin = cut(rast_spread_2022, breaks=10))
...
y_alt7<-x_frt12 %>% mutate(x_bin = cut(rast_spread_2022, breaks=10))
...
y_alt7<-x_frt13 %>% mutate(x_bin = cut(rast_spread_2022, breaks=10))
...
y_alt7<-x_frt14 %>% mutate(x_bin = cut(rast_spread_2022, breaks=10))
...
y_alt7<-x_frt15 %>% mutate(x_bin = cut(rast_spread_2022, breaks=10))
