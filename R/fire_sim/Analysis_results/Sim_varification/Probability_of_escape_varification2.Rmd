...
summary(ignit_escape$CURRENT_SIZE)
```
...
```{r}
lightning_ignition<-raster(" C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\rasters\\Prob_lightning_2021.tif")
human_ignition<-raster(" C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\rasters\\Prob_person_2021.tif")
...
ggplot() +
 geom_raster(data = escape_pts_df , aes(x = x, y = y, fill = Prob_spread_2021 )) +
  geom_point(data = pointCoordinates, aes(x = test.X, y = test.Y))
#####
```
...
ggplot(data=x2) +    
   geom_histogram(aes(x=size *16), fill = "blue") 
ggplot(data=x2) +    
   geom_histogram(aes(x=CURRENT_SIZE), fill = "green") 
