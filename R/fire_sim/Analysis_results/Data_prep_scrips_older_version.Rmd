...
#### **NOTE I now do the climate extraction at the end and then join it back**
...
x4_clim_dem_2<- x3_dem
hist(x4_clim_dem_2$dem)
rm(x3_dem)
gc()
...
# 
st_write(my.sf.point, "C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\BC_data2.gpkg", delete_layer=TRUE, driver="GPKG")
...
```{r}
dat<- st_read("C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\BC_data_veg_VRI2001.gpkg")
head(dat)
names(dat)

dat1<- st_read("C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\BC_data_veg_VRI2003.gpkg")
dat2<- st_read("C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\BC_data_veg_VRI2004.gpkg")
dat3<- st_read("C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\BC_data_veg_VRI2005.gpkg")

dat_all<-rbind(rbind(rbind(dat, dat1), dat2), dat3)
dim(dat_all)
dat_all$idno[duplicated(dat_all$idno)] # no duplicates good
...
names(dat_all)
dat_all2<-dat_all %>% dplyr::select(idno, frt,el, inventory_standard_cd: proj_height_1, geom)
...
Now save it
```{r}
st_write(fire_all1, "C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\BC_Elev_veg_2001_joined.gpkg", delete_layer=TRUE, driver="GPKG")
...
fire_all1_slope_aspect_wind2<- fire_all1_slope_aspect_wind  %>% dplyr::select(idno:el, bclcs_level_4, bclcs_level_5, FWI_veg:aspect_ha_bc_3005, wind_summer_clipped_224,wind_spring_raster_224, dist_infrastructure_m, dist_roads_m, geom) %>% 
...
```{r}
st_write(fire_all1_slope_aspect_wind2, "C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\BC_Elev_veg_rd_infr_wind_2001.gpkg", delete_layer=TRUE, driver="GPKG")
...
fire_all1_slope_aspect_wind<-st_read("C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\BC_Elev_veg_rd_infr_wind.gpkg")
x3<- st_transform(fire_all1_slope_aspect_wind, crs = "+proj=longlat +datum=NAD83 / BC Albers +no_defs")
...
## 2021
inputFile = '/C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\bc_dem_frt.csv' 
outputFile = '/C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\bc_dem_frt_clim2.csv'
yearPeriod = '/Year_2021.ann'
system2(exe,args= c('/M', yearPeriod, inputFile, outputFile))
...
now join climate data back to fire_all1_slope_aspect_wind and save the whole file.
Then run the model for spread for the whole of BC and generate a spread probability map.

Repeat the end step for escape!


```{r}
x4<-read.csv("C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\bc_dem_frt_Year_2001M.csv")
head(fire_all1_slope_aspect_wind2)

x5<-x4 %>% dplyr::select(ID1:Longitude, Tmax01:Tmax12, Tave01:PPT12, RH01:RH12) %>%
  dplyr::rename(idno=ID1,
         frt=ID2)

dat_final<-left_join(fire_all1_slope_aspect_wind2, x5)

names(dat_final)


dat_final<- dat_final  %>% dplyr::select(idno:el, bclcs_level_1:bclcs_level_5, land_cover_class_cd_1:yearsSinceDisturbance, FWI_veg:aspect_ha_bc_3005, wind_summer_clipped_224:geom) %>% 
  rename(win_spr = wind_spring_raster_224,
         win_sum = wind_summer_clipped_224,
         slope = slope_ha_bc_3005,
         aspect = aspect_ha_bc_3005,
...
st_write(dat_final, "C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\BC\\BC_all_dat_400m_final_2001.gpkg", delete_layer=TRUE, driver="GPKG")
