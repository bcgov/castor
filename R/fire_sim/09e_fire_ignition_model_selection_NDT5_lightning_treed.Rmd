...
#=================================
#  Script Name: 09e_fire_ignition_model_selection_NDT5_lightning_treed.R
#  Script Version: 1.0
...
#Run model using dat1
model.NDT5<-glm(fire_pres ~ climate1 + climate2 + vegtype2 + proj_height_1 + live_stand_volume_125 + proj_age_1 + climate1:proj_age_1 + climate2:proj_height_1 + climate2:vegtype2 + dist_mun + dist_dam + dist_mine + dist_pow + bclcs_level_5_2 + dist_dam:bclcs_level_5_2 + dist_mine:bclcs_level_5_2 + slope + elevation + slope:elevation + elevation*climate1, family = binomial, data = dat1)

AIC(model.NDT5) #264.6
...
AIC(model.NDT5) #265.9

#Determine AUC of full model
mod.valid <- predict.glm(model.NDT5, newdata=Valid, type="response")
   roc_obj <- roc(Valid[,"fire_pres"], mod.valid)
   mod.auc <- auc(roc_obj)
   mod.auc #0.87 --> really good!
   
Anova(model.NDT5, type=3)
Anova(model.NDT5, type=3, singular.ok = TRUE) 

#Least sig
model.NDT5<-glm(fire_pres ~ climate1 + climate2 + proj_height_1 + live_stand_volume_125 + climate2:proj_height_1 + dist_dam + dist_mine + bclcs_level_5_2 + dist_mine:bclcs_level_5_2 + elevation, family = binomial, data = dat1)

AIC(model.NDT5) #270.98 - worse!
...
NDT5_t<-dat_lightning_t_NDT5 %>% drop_na(climate1, climate2, proj_height_1, live_stand_volume_125, dist_mine, dist_dam, vegtype2, slope, aspect, elevation, bclcs_level_5_2, elevation)

#Run Model again with this data; but uses all data here
model.NDT5<-glm(fire_pres ~ climate1 + climate2 + proj_height_1 + live_stand_volume_125 + climate2:proj_height_1 + dist_dam + dist_mine + bclcs_level_5_2 + dist_dam:bclcs_level_5_2 + dist_mine:bclcs_level_5_2 + elevation, family = binomial, data = NDT5_t)

AIC(model.NDT5) #381.95
...
NDT5_t$resids<-resid(model.NDT5)

binnedplot (NDT5_t$live_stand_volume_125, 
            NDT5_t$resids, 
            nclass = NULL, 
...
#Partial Residuals
#climate1 + climate2 + proj_height_1 + live_stand_volume_125 + climate2:proj_height_1 + dist_dam + dist_mine + bclcs_level_5_2 + dist_dam:bclcs_level_5_2 + dist_mine:bclcs_level_5_2 + elevation
visreg(model.NDT5, "climate1")
visreg(model.NDT5, "climate2", by="proj_height_1")

visreg(model.NDT5, "proj_height_1", by="climate2")
visreg(model.NDT5, "live_stand_volume_125")

visreg(model.NDT5, "elevation")

visreg(model.NDT5, "dist_dam", by="bclcs_level_5_2", overlay=TRUE, ylim=c(-30,30))
visreg(model.NDT5, "dist_mine", by="bclcs_level_5_2", overlay=TRUE, ylim=c(-30,30))

visreg(model.NDT5, "bclcs_level_5_2", by="dist_mine")
visreg(model.NDT5, "bclcs_level_5_2", by="dist_mine", ylim=c(-40,40))
visreg(model.NDT5, "bclcs_level_5_2", by="dist_dam")
visreg(model.NDT5, "bclcs_level_5_2", by="dist_dam", ylim=c(-40,40))
...
top_mod_table_NDT5_light_t_ALL <- data.frame (matrix (ncol = 20, nrow = 0))
colnames (top_mod_table_NDT5_light_t_ALL ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_climate_2", "coef_stand_height", "coef_live_stand_volume_125",  "coef_dist_dam", "coef_dist_mine", "coef_bclcs_level_5_2OP", "coef_bclcs_level_5_2SP", "coef_elevation", "climate2:proj_height_1", "dist_dam:bclcs_level_5_2OP", "dist_dam:bclcs_level_5_2SP", "dist_mine:bclcs_level_5_2OP", "dist_mine:bclcs_level_5_2SP", "AUC")
```
...
prop<-0.75
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(NDT5_t$fire_veg, p = prop,
                                    list = FALSE,
                                    times = 1)
  
   dat1 <- NDT5_t[ trainIndex,]
   Valid <- NDT5_t[-trainIndex,]
...
top_mod_table_NDT5_light_t <- data.frame (matrix (ncol = 20, nrow = 0))
colnames (top_mod_table_NDT5_light_t ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_climate_2", "coef_stand_height", "coef_live_stand_volume_125",  "coef_dist_dam", "coef_dist_mine", "coef_bclcs_level_5_2OP", "coef_bclcs_level_5_2SP", "coef_elevation", "climate2:proj_height_1", "dist_dam:bclcs_level_5_2OP", "dist_dam:bclcs_level_5_2SP", "dist_mine:bclcs_level_5_2OP", "dist_mine:bclcs_level_5_2SP", "AUC")

##Add data for NDT5
top_mod_table_NDT5_light_t[1,1]<-"lightning"
top_mod_table_NDT5_light_t[1,2]<-"NDT5"
top_mod_table_NDT5_light_t[1,3]<-"Y"
top_mod_table_NDT5_light_t[1,4]<-"fire_pres ~ climate1 + climate2 + proj_height_1 + live_stand_volume_125 + climate2:proj_height_1 + dist_dam + dist_mine + bclcs_level_5_2 + dist_dam:bclcs_level_5_2 + dist_mine:bclcs_level_5_2 + elevation" 
...
top_mod_table_NDT5_light_t[1,8]<- coef(model.NDT5)[4] #proj height
top_mod_table_NDT5_light_t[1,9]<- coef(model.NDT5)[5] #coefficient live stand volume
top_mod_table_NDT5_light_t[1,10]<- coef(model.NDT5)[6] #coefficient dist_dam
top_mod_table_NDT5_light_t[1,11]<- coef(model.NDT5)[7] #coefficient dist_mine
top_mod_table_NDT5_light_t[1,12]<- coef(model.NDT5)[8] #coefficient bclcs_level_5_2OP
top_mod_table_NDT5_light_t[1,13]<- coef(model.NDT5)[9] #coef bclcs_level_5_2SP
top_mod_table_NDT5_light_t[1,14]<- coef(model.NDT5)[10] #coef elevation
top_mod_table_NDT5_light_t[1,15]<- coef(model.NDT5)[11] #climate2:proj_height_1
top_mod_table_NDT5_light_t[1,16]<- coef(model.NDT5)[12] #coefficient dist_dam:bclcs_level_5_2OP
top_mod_table_NDT5_light_t[1,17]<- coef(model.NDT5)[13] #coefficient dist_dam:bclcs_level_5_2SP
top_mod_table_NDT5_light_t[1,18]<- coef(model.NDT5)[14] #coefficient dist_mine:bclcs_level_5_2OP
top_mod_table_NDT5_light_t[1,19]<- coef(model.NDT5)[15] #coefficient dist_mine:bclcs_level_5_2SP
top_mod_table_NDT5_light_t[1,20]<- mod.auc
...
```{r}
write.csv(top_mod_table_NDT5_light_t_ALL, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT5_light_t_ALL.csv")
...
mean(top_mod_table_NDT5_light_t_ALL$AUC) #0.87

# create model table (only do this once) and add the relevant data
top_mod_table_NDT5_light_t_Means <- data.frame (matrix (ncol = 20, nrow = 0))
colnames (top_mod_table_NDT5_light_t_Means ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_climate_2", "coef_stand_height", "coef_live_stand_volume_125",  "coef_dist_dam", "coef_dist_mine", "coef_bclcs_level_5_2OP", "coef_bclcs_level_5_2SP", "coef_elevation", "climate2:proj_height_1", "dist_dam:bclcs_level_5_2OP", "dist_dam:bclcs_level_5_2SP", "dist_mine:bclcs_level_5_2OP", "dist_mine:bclcs_level_5_2SP", "AUC")
...
top_mod_table_NDT5_light_t_Means[1,4]<-"fire_pres ~ climate1 + climate2 + proj_height_1 + live_stand_volume_125 + climate2:proj_height_1 + dist_dam + dist_mine + bclcs_level_5_2 + dist_dam:bclcs_level_5_2 + dist_mine:bclcs_level_5_2 + elevation" 
...
top_mod_table_NDT5_light_t_Means[1,8]<- mean(top_mod_table_NDT5_light_t_ALL$coef_stand_height, na.rm=TRUE) #I
top_mod_table_NDT5_light_t_Means[1,9]<- mean(top_mod_table_NDT5_light_t_ALL$coef_live_stand_volume_125, na.rm=TRUE)
top_mod_table_NDT5_light_t_Means[1,10]<- mean(top_mod_table_NDT5_light_t_ALL$coef_dist_dam, na.rm=TRUE) 
top_mod_table_NDT5_light_t_Means[1,11]<- mean(top_mod_table_NDT5_light_t_ALL$coef_dist_mine, na.rm=TRUE) 
top_mod_table_NDT5_light_t_Means[1,12]<- mean(top_mod_table_NDT5_light_t_ALL$coef_bclcs_level_5_2OP, na.rm=TRUE) 
top_mod_table_NDT5_light_t_Means[1,13]<- mean(top_mod_table_NDT5_light_t_ALL$coef_bclcs_level_5_2SP, na.rm=TRUE) #
top_mod_table_NDT5_light_t_Means[1,14]<- mean(top_mod_table_NDT5_light_t_ALL$coef_elevation, na.rm=TRUE) #
top_mod_table_NDT5_light_t_Means[1,15]<- mean(top_mod_table_NDT5_light_t_ALL$`climate2:proj_height_1`, na.rm=TRUE) 
top_mod_table_NDT5_light_t_Means[1,16]<- mean(top_mod_table_NDT5_light_t_ALL$`dist_dam:bclcs_level_5_2OP`, na.rm=TRUE) 
top_mod_table_NDT5_light_t_Means[1,17]<- mean(top_mod_table_NDT5_light_t_ALL$`dist_dam:bclcs_level_5_2SP`, na.rm=TRUE)#
top_mod_table_NDT5_light_t_Means[1,18]<- mean(top_mod_table_NDT5_light_t_ALL$`dist_mine:bclcs_level_5_2OP`, na.rm=TRUE) 
top_mod_table_NDT5_light_t_Means[1,19]<- mean(top_mod_table_NDT5_light_t_ALL$`dist_mine:bclcs_level_5_2SP`, na.rm=TRUE) 
top_mod_table_NDT5_light_t_Means[1,20]<- mean(top_mod_table_NDT5_light_t_ALL$AUC, na.rm=TRUE)
...
```{r}
write.csv(top_mod_table_NDT5_light_t_Means, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT5_light_t_Means.csv")
```


Get Standard deviation (SD) values.

```{r}
# create model table (only do this once) and add the relevant data
top_mod_table_NDT5_light_t_SD <- data.frame (matrix (ncol = 20, nrow = 0))
colnames (top_mod_table_NDT5_light_t_SD ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_climate_2", "coef_stand_height", "coef_live_stand_volume_125",  "coef_dist_dam", "coef_dist_mine", "coef_bclcs_level_5_2OP", "coef_bclcs_level_5_2SP", "coef_elevation", "climate2:proj_height_1", "dist_dam:bclcs_level_5_2OP", "dist_dam:bclcs_level_5_2SP", "dist_mine:bclcs_level_5_2OP", "dist_mine:bclcs_level_5_2SP", "AUC")
...
top_mod_table_NDT5_light_t_SD[1,4]<-"fire_pres ~ climate1 + climate2 + proj_height_1 + live_stand_volume_125 + climate2:proj_height_1 + dist_dam + dist_mine + bclcs_level_5_2 + dist_dam:bclcs_level_5_2 + dist_mine:bclcs_level_5_2 + elevation" 
...
top_mod_table_NDT5_light_t_SD[1,8]<- sd(top_mod_table_NDT5_light_t_ALL$coef_stand_height, na.rm=TRUE) #I
top_mod_table_NDT5_light_t_SD[1,9]<- sd(top_mod_table_NDT5_light_t_ALL$coef_live_stand_volume_125, na.rm=TRUE)
top_mod_table_NDT5_light_t_SD[1,10]<- sd(top_mod_table_NDT5_light_t_ALL$coef_dist_dam, na.rm=TRUE) 
top_mod_table_NDT5_light_t_SD[1,11]<- sd(top_mod_table_NDT5_light_t_ALL$coef_dist_mine, na.rm=TRUE) 
top_mod_table_NDT5_light_t_SD[1,12]<- sd(top_mod_table_NDT5_light_t_ALL$coef_bclcs_level_5_2OP, na.rm=TRUE) 
top_mod_table_NDT5_light_t_SD[1,13]<- sd(top_mod_table_NDT5_light_t_ALL$coef_bclcs_level_5_2SP, na.rm=TRUE) #
top_mod_table_NDT5_light_t_SD[1,14]<- sd(top_mod_table_NDT5_light_t_ALL$coef_elevation, na.rm=TRUE) #
top_mod_table_NDT5_light_t_SD[1,15]<- sd(top_mod_table_NDT5_light_t_ALL$`climate2:proj_height_1`, na.rm=TRUE) 
top_mod_table_NDT5_light_t_SD[1,16]<- sd(top_mod_table_NDT5_light_t_ALL$`dist_dam:bclcs_level_5_2OP`, na.rm=TRUE) 
top_mod_table_NDT5_light_t_SD[1,17]<- sd(top_mod_table_NDT5_light_t_ALL$`dist_dam:bclcs_level_5_2SP`, na.rm=TRUE)#
top_mod_table_NDT5_light_t_SD[1,18]<- sd(top_mod_table_NDT5_light_t_ALL$`dist_mine:bclcs_level_5_2OP`, na.rm=TRUE) 
top_mod_table_NDT5_light_t_SD[1,19]<- sd(top_mod_table_NDT5_light_t_ALL$`dist_mine:bclcs_level_5_2SP`, na.rm=TRUE) 
top_mod_table_NDT5_light_t_SD[1,20]<- sd(top_mod_table_NDT5_light_t_ALL$AUC, na.rm=TRUE)
...
```{r}
write.csv(top_mod_table_NDT5_light_t_SD, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT5_light_t_SD.csv")
```
...
#Investigate vegtype groups
table(dat_lightning_t_NDT5$vegtype2) #only one TB; rest TC. Remove TB.
dat_lightning_t_NDT5<-subset(dat_lightning_t_NDT5, dat_lightning_t_NDT5$vegtype2=="TC")
...
```{r}
#Run model using dat1
model.NDT5<-glm(fire_pres ~ climate1 + climate2  + proj_height_1 + live_stand_volume_125 + proj_age_1 + climate1:proj_age_1 + climate2:proj_height_1 + dist_infra + slope + elevation + slope:elevation + elevation*climate1 + elevation*climate2, family = binomial, data = dat1)

AIC(model.NDT5) #271.0

#Determine AUC of full model
mod.valid <- predict.glm(model.NDT5, newdata=Valid, type="response")
   roc_obj <- roc(Valid[,"fire_pres"], mod.valid)
   mod.auc <- auc(roc_obj)
   mod.auc #0.81 --> really good!
   
Anova(model.NDT5, type=3)
#Anova(model.NDT5, type=3, singular.ok = TRUE) 

#Remove least significant
model.NDT5<-glm(fire_pres ~ climate1 + climate2  + proj_height_1 + live_stand_volume_125 + proj_age_1 + climate1:proj_age_1 + climate2:proj_height_1 + dist_infra + slope + elevation + elevation*climate1 + elevation*climate2, family = binomial, data = dat1)

AIC(model.NDT5) #269.1
...
#Remove least significant
model.NDT5<-glm(fire_pres ~ climate1 + climate2  + proj_height_1 + live_stand_volume_125 + proj_age_1 + climate2:proj_height_1 + dist_infra + slope + elevation + elevation*climate1 + elevation*climate2, family = binomial, data = dat1)

AIC(model.NDT5) #267.7

#Determine AUC of full model
mod.valid <- predict.glm(model.NDT5, newdata=Valid, type="response")
   roc_obj <- roc(Valid[,"fire_pres"], mod.valid)
   mod.auc <- auc(roc_obj)
   mod.auc #0.80 --> really good!
   
Anova(model.NDT5, type=3)
#Anova(model.NDT5, type=3, singular.ok = TRUE) 

#Remove least significant
model.NDT5<-glm(fire_pres ~ climate1 + climate2  + proj_height_1 + live_stand_volume_125 + proj_age_1 + climate2:proj_height_1 + dist_infra + slope + elevation + elevation*climate1, family = binomial, data = dat1)

AIC(model.NDT5) #266.6
...
#Remove least significant
model.NDT5<-glm(fire_pres ~ climate1 + climate2  + proj_height_1 + live_stand_volume_125 + proj_age_1 + climate2:proj_height_1 + dist_infra + slope + elevation, family = binomial, data = dat1)

AIC(model.NDT5) #265.8
...
#Remove least significant
model.NDT5<-glm(fire_pres ~ climate1 + climate2  + proj_height_1 + live_stand_volume_125 + proj_age_1 + climate2:proj_height_1 + dist_infra + elevation, family = binomial, data = dat1)

AIC(model.NDT5) #265.3
...
#Remove least significant
model.NDT5<-glm(fire_pres ~ climate1 + climate2  + proj_height_1 + live_stand_volume_125 + climate2:proj_height_1 + dist_infra + elevation, family = binomial, data = dat1)

AIC(model.NDT5) #264.8
...
```{r}
#Remove NAs to ensure all same data used so we can compare AICs
NDT5_t<-dat_lightning_t_NDT5 %>% drop_na(climate1, climate2, proj_height_1, live_stand_volume_125, dist_infra, elevation)

#Run Model again with this data; but uses all data here
model.NDT5<-glm(fire_pres ~ climate1 + climate2  + proj_height_1 + live_stand_volume_125 + climate2:proj_height_1 + dist_infra + elevation, family = binomial, data = NDT5_t)

AIC(model.NDT5) #374.6
...
#Partial Residuals
#climate1 + climate2  + proj_height_1 + live_stand_volume_125 + climate2:proj_height_1 + dist_infra + elevation
visreg(model.NDT5, "climate1")
visreg(model.NDT5, "climate2", by="proj_height_1")
...
#Create a new blank table and get AUC too
top_mod_table_NDT5_light_t_ALL <- data.frame (matrix (ncol = 13, nrow = 0))
colnames (top_mod_table_NDT5_light_t_ALL ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_climate_2", "coef_proj_height_1", "coef_live_stand_volume_125",  "coef_dist_infra", "coef_elevation", "climate2:proj_height_1", "AUC")
```
...
# create model table (only do this once) and add the relevant data
top_mod_table_NDT5_light_t <- data.frame (matrix (ncol = 13, nrow = 0))
colnames (top_mod_table_NDT5_light_t ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_climate_2", "coef_proj_height_1", "coef_live_stand_volume_125",  "coef_dist_infra", "coef_elevation", "climate2:proj_height_1", "AUC")

##Add data for NDT5
top_mod_table_NDT5_light_t[1,1]<-"lightning"
top_mod_table_NDT5_light_t[1,2]<-"NDT5"
top_mod_table_NDT5_light_t[1,3]<-"Y"
top_mod_table_NDT5_light_t[1,4]<-"fire_pres ~ climate1 + climate2  + proj_height_1 + live_stand_volume_125 + climate2:proj_height_1 + dist_infra + elevation" 
top_mod_table_NDT5_light_t[1,5]<- coef(model.NDT5)[1] #Intercept
top_mod_table_NDT5_light_t[1,6]<- coef(model.NDT5)[2] #Climate variable 1
top_mod_table_NDT5_light_t[1,7]<- coef(model.NDT5)[3] #Climate variable 2
top_mod_table_NDT5_light_t[1,8]<- coef(model.NDT5)[4] #proj height
top_mod_table_NDT5_light_t[1,9]<- coef(model.NDT5)[5] #coefficient live stand volume
top_mod_table_NDT5_light_t[1,10]<- coef(model.NDT5)[6] #coefficient di
top_mod_table_NDT5_light_t[1,11]<- coef(model.NDT5)[7] #coefficient d
top_mod_table_NDT5_light_t[1,12]<- coef(model.NDT5)[8] #coefficient
top_mod_table_NDT5_light_t[1,13]<- mod.auc
...
```{r}
write.csv(top_mod_table_NDT5_light_t_ALL, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT5_light_t_ALL_Jan.csv")
...
top_mod_table_NDT5_light_t_Means<-top_mod_table_NDT5_light_t_ALL %>% summarise_each(funs( mean( .,na.rm = TRUE)))
top_mod_table_NDT5_light_t_Means

top_mod_table_NDT5_light_t_Means[1,1]<-"lightning"
top_mod_table_NDT5_light_t_Means[1,2]<-"NDT5"
top_mod_table_NDT5_light_t_Means[1,3]<-"Treed"
top_mod_table_NDT5_light_t_Means[1,4]<- "fire_pres ~ climate1 + climate2  + proj_height_1 + live_stand_volume_125 + climate2:proj_height_1 + dist_infra + elevation" 
top_mod_table_NDT5_light_t_Means
```
Save table.

```{r}
write.csv(top_mod_table_NDT5_light_t_Means, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT5_light_t_Means_Jan.csv")
```

Standard deviation.

```{r}
top_mod_table_NDT5_light_t_SD<-top_mod_table_NDT5_light_t_ALL %>% summarise_each(funs( sd( .,na.rm = TRUE)))
top_mod_table_NDT5_light_t_SD

top_mod_table_NDT5_light_t_SD[1,1]<-"lightning"
top_mod_table_NDT5_light_t_SD[1,2]<-"NDT5"
top_mod_table_NDT5_light_t_SD[1,3]<-"Treed"
top_mod_table_NDT5_light_t_SD[1,4]<-"fire_pres ~ climate1 + climate2  + proj_height_1 + live_stand_volume_125 + climate2:proj_height_1 + dist_infra + elevation" 
top_mod_table_NDT5_light_t_SD
```
...
```{r}
write.csv(top_mod_table_NDT5_light_t_SD, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT5_light_t_SD_Jan.csv")
```
