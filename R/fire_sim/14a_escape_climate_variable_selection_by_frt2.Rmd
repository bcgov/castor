...
escape<-read.csv(dat, "C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\escape_data.csv")

#("C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\Fire_escape_data_all.gpkg")

table(is.na(escape$frt)) # wierd there are  points that have no frt assigned to them.Not I tried joining them to frt again and I still have 13 missing frt info. I looked at them in QGIS and decided that the following assignments were correct
# 
# escape$frt[escape$idno==15511]<-13
# escape$frt[escape$idno==31609]<-13
# escape$frt[escape$idno==15214]<-13
# escape$frt[escape$idno==15507]<-13
# escape$frt[escape$idno==10828]<-13
# escape$frt[escape$idno==119]<-14
# escape$frt[escape$idno==20067]<-14
# escape$frt[escape$idno==965]<-14
# escape$frt[escape$idno==22140]<-14
# escape$frt[escape$idno==26529]<-14
# escape$frt[escape$idno==32226]<-14
# escape$frt[escape$idno==27182]<-14
# escape$frt[escape$idno==719]<-14
# escape$frt[escape$idno==14884]<-14
# escape$frt[escape$idno==23161]<-14
# escape$frt[escape$idno==20054]<-14
# escape$frt[escape$idno==21163]<-14
# escape$frt[escape$idno==1832]<-14
# escape$frt[escape$idno==31495]<-14
# 
# escape$frt[is.na(escape$frt)]<-15
escape$frt[escape$frt==3]<-5

escape$frt[escape$frt==9]<-11
table(escape$frt)
...
ggplot() + 
  geom_line(data = escape2_summary13, mapping = aes(x = fire_yr, y = meanTmax07))+
  geom_line(data = escape2_summary13, aes(x=fire_yr, y=no_fires/20, col='red'))+
  facet_grid(~frt) +
...
escape2_summary<- escape2 %>%
  group_by(fire_yr, frt, escape) %>%
  summarize(no_fires = n())

escape2_summary<-as.data.frame(escape2_summary)

ggplot(escape2_summary, aes(x = fire_yr, y = no_fires, color = as.factor(escape))) +
  geom_point() +
  geom_line() +
  facet_grid(~frt)

# there are some NA values which is really weird.
escape2[frt %in% c(12, 14) & fire_yr %in% c(2016, 2017), CURRENT]
# ok it turns out there are actually NA in the fire size column. Ill remove those.
...
ignit_summary<- escape2 %>%
  group_by(ig_mnth, frt) %>%
  dplyr::summarize(Area=sum(CURRENT))
...
plot(escape2$CURRENT, escape2$CMI07)

ggplot() + 
  geom_line(data = escape2_summary13, mapping = aes(x = fire_yr, y = meanCMI07, color = FIRE_CA))+
  facet_grid(~frt) +
  scale_x_continuous(breaks = seq(from = 2001, to = 2022, by =10 ))+
...
ggplot(escape2_summary13, aes(meanCMI07, no_fires, color=FIRE_CA)) + 
  geom_point() +
  geom_smooth(method=lm, formula =  y ~ poly(x, 2))  +facet_grid(~frt) + 
  ylim(0,500)

hist(as.numeric(escape2$ig_mnth))

escape2_9<-escape2 %>% filter(CURRENT>1, ig_mnth=="07")

ggplot(escape2_9, aes(CMI07, CURRENT)) + 
  geom_point() +
  geom_smooth(method=lm, formula =  y ~ poly(x, 2)) +
  facet_grid(~frt)

ggplot(escape2_9, aes(Tmax07, CURRENT)) + 
  geom_point() +
  geom_smooth(method=lm, formula =  y ~ poly(x, 2)) +
  facet_grid(~frt)

ggplot(escape2_9, aes(PPT07, CURRENT)) + 
  geom_point() +
  geom_smooth(method=lm, formula =  y ~ poly(x, 2)) +
  facet_grid(~frt)
...
# Tave
tave<-escape2_summary %>% dplyr::select(frt, Tave02,Tave03,Tave04,Tave05,Tave06,Tave07, Tave08, Tave09, Tave10, Tave11)
...
# Tmax
Tmax<-escape2_summary %>% dplyr::select(frt, Tmax02,Tmax03,Tmax04,Tmax05,Tmax06,Tmax07, Tmax08, Tmax09, Tmax10, Tmax11)
...
# PPT
PPT<-escape2_summary %>% dplyr::select(frt, PPT02,PPT03,PPT04,PPT05,PPT06,PPT07, PPT08, PPT09, PPT10, PPT11)
...
# RH
RH<-escape2_summary %>% dplyr::select(frt, RH02,RH03,RH04,RH05,RH06,RH07, RH08, RH09, RH10, RH11)
...
# MDC
MDC_<-escape2_summary %>% dplyr::select(frt, MDC_02,MDC_03,MDC_04,MDC_05,MDC_06,MDC_07, MDC_08, MDC_09, MDC_10, MDC_11)
...
# CMI
CMI<-escape2_summary %>% dplyr::select(frt, CMI02,CMI03,CMI04,CMI05,CMI06,CMI07, CMI08, CMI09, CMI10, CMI11)

CMIx<-pivot_longer(CMI, cols=2:11, names_to="Month", values_to="CMI")
CMIx$CMI<- as.numeric(CMIx$CMI)

p<-ggplot(data=CMIx, aes(x=Month, y=CMI, group= frt)) +
  geom_line(aes(colour=frt)) +
  geom_point(aes(colour=frt)) 
p
...
dist.cut.corr <- escape2 [,c("Tmax02", "Tmax03", "Tmax04", "Tmax05", "Tmax06", "Tmax07", "Tmax08", "Tmax09", "Tmax10", "CMI02", "CMI03", "CMI04", "CMI05", "CMI06", "CMI07", "CMI08", "CMI09", "CMI10")]

corr <- round (cor (as.data.frame(dist.cut.corr)), 2)

ggcorrplot (corr, type = "lower", lab = TRUE, tl.cex = 10,  lab_size = 3,
            title = "Correlation between maximum temperature and CMI")


dist.cut.corr <- escape2 [,c (19:28, 39:48)]
...
escape2$tot_spring_PPT<-(escape2$PPT03+ escape2$PPT04 + escape2$PPT05)/3
escape2$tot_summer_PPT<-(escape2$PPT06+ escape2$PPT07 + escape2$PPT08)/3

escape2$spring_Tmax<-(escape2$Tmax03+ escape2$Tmax04 + escape2$Tmax05)/3
escape2$summer_Tmax<-(escape2$Tmax06+ escape2$Tmax07 + escape2$Tmax08)/3

escape2$spring_Tave<-(escape2$Tave03+ escape2$Tave04 + escape2$Tave05)/3
escape2$summer_Tave<-(escape2$Tave06+ escape2$Tave07 + escape2$Tave08)/3

escape2$tot_spring_CMD<-(escape2$CMD03+ escape2$CMD04 + escape2$CMD05)/3
escape2$tot_summer_CMD<-(escape2$CMD06+ escape2$CMD07 + escape2$CMD08)/3

escape2$tot_spring_CMI<-(escape2$CMI03+ escape2$CMI04 + escape2$CMI05)/3
escape2$tot_summer_CMI<-(escape2$CMI06+ escape2$CMI07 + escape2$CMI08)/3


escape2$mean_Tmax03_Tmax04<- (escape2$Tmax03+ escape2$Tmax04)/2
...
escape2$mean_Tmax03_Tmax04_Tmax05_Tmax06<- (escape2$Tmax03 + escape2$Tmax04+ escape2$Tmax05 + escape2$Tmax06)/4
...
escape2$mean_Tmax03_Tmax04_Tmax05_Tmax06_Tmax07<- (escape2$Tmax03 + escape2$Tmax04 + escape2$Tmax05+ escape2$Tmax06 + escape2$Tmax07)/5
...
escape2$mean_Tmax03_Tmax04_Tmax05_Tmax06_Tmax07_Tmax08<- (escape2$Tmax03 + escape2$Tmax04 + escape2$Tmax05 + escape2$Tmax06 + escape2$Tmax07+ escape2$Tmax08)/6
...
escape2$mean_Tave03_Tave04<- (escape2$Tave03+ escape2$Tave04)/2
...
escape2$mean_Tave03_Tave04_Tave05_Tave06<- (escape2$Tave03 + escape2$Tave04+ escape2$Tave05 + escape2$Tave06)/4
...
escape2$mean_Tave03_Tave04_Tave05_Tave06_Tave07<- (escape2$Tave03 + escape2$Tave04 + escape2$Tave05+ escape2$Tave06 + escape2$Tave07)/5
...
escape2$mean_Tave03_Tave04_Tave05_Tave06_Tave07_Tave08<- (escape2$Tave03 + escape2$Tave04 + escape2$Tave05 + escape2$Tave06 + escape2$Tave07+ escape2$Tave08)/6
...
#### PPT
escape2$mean_PPT03_PPT04<- (escape2$PPT03+ escape2$PPT04)/2
...
escape2$mean_PPT03_PPT04_PPT05_PPT06<- (escape2$PPT03 + escape2$PPT04+ escape2$PPT05 + escape2$PPT06)/4
...
escape2$mean_PPT03_PPT04_PPT05_PPT06_PPT07<- (escape2$PPT03 + escape2$PPT04 + escape2$PPT05+ escape2$PPT06 + escape2$PPT07)/5
...
escape2$mean_PPT03_PPT04_PPT05_PPT06_PPT07_PPT08<- (escape2$PPT03 + escape2$PPT04 + escape2$PPT05 + escape2$PPT06 + escape2$PPT07+ escape2$PPT08)/6
...
#### RH
escape2$mean_RH03_RH04<- (escape2$RH03+ escape2$RH04)/2
...
escape2$mean_RH03_RH04_RH05_RH06<- (escape2$RH03 + escape2$RH04+ escape2$RH05 + escape2$RH06)/4
...
escape2$mean_RH03_RH04_RH05_RH06_RH07<- (escape2$RH03 + escape2$RH04 + escape2$RH05+ escape2$RH06 + escape2$RH07)/5
...
escape2$mean_RH03_RH04_RH05_RH06_RH07_RH08<- (escape2$RH03 + escape2$RH04 + escape2$RH05 + escape2$RH06 + escape2$RH07+ escape2$RH08)/6
...
### MDC
escape2$mean_MDC_03_MDC_04<- (escape2$MDC_03+ escape2$MDC_04)/2
...
escape2$mean_MDC_03_MDC_04_MDC_05_MDC_06<- (escape2$MDC_03 + escape2$MDC_04+ escape2$MDC_05 + escape2$MDC_06)/4
...
escape2$mean_MDC_03_MDC_04_MDC_05_MDC_06_MDC_07<- (escape2$MDC_03 + escape2$MDC_04 + escape2$MDC_05+ escape2$MDC_06 + escape2$MDC_07)/5
...
escape2$mean_MDC_03_MDC_04_MDC_05_MDC_06_MDC_07_MDC_08<- (escape2$MDC_03 + escape2$MDC_04 + escape2$MDC_05 + escape2$MDC_06 + escape2$MDC_07+ escape2$MDC_08)/6
...
#### CMI
escape2$mean_CMI03_CMI04<- (escape2$CMI03+ escape2$CMI04)/2
...
escape2$mean_CMI03_CMI04_CMI05_CMI06<- (escape2$CMI03 + escape2$CMI04+ escape2$CMI05 + escape2$CMI06)/4
...
escape2$mean_CMI03_CMI04_CMI05_CMI06_CMI07<- (escape2$CMI03 + escape2$CMI04 + escape2$CMI05+ escape2$CMI06 + escape2$CMI07)/5
...
escape2$mean_CMI03_CMI04_CMI05_CMI06_CMI07_CMI08<- (escape2$CMI03 + escape2$CMI04 + escape2$CMI05 + escape2$CMI06 + escape2$CMI07+ escape2$CMI08)/6
...
#### CMD
escape2$mean_CMD03_CMD04<- (escape2$CMD03+ escape2$CMD04)/2
escape2$mean_CMD04_CMD05<- (escape2$CMD04+ escape2$CMD05)/2
escape2$mean_CMD05_CMD06<- (escape2$CMD05+ escape2$CMD06)/2
escape2$mean_CMD06_CMD07<- (escape2$CMD06+ escape2$CMD07)/2
escape2$mean_CMD07_CMD08<- (escape2$CMD07+ escape2$CMD08)/2
escape2$mean_CMD08_CMD09<- (escape2$CMD08+ escape2$CMD09)/2
escape2$mean_CMD09_CMD10<- (escape2$CMD09+ escape2$CMD10)/2

escape2$mean_CMD04_CMD05_CMD06<- (escape2$CMD04+ escape2$CMD05 + escape2$CMD06)/3
escape2$mean_CMD05_CMD06_CMD07<- (escape2$CMD05+ escape2$CMD06 + escape2$CMD07)/3
escape2$mean_CMD07_CMD08_CMD09<- (escape2$CMD07+ escape2$CMD08 + escape2$CMD09)/3
escape2$mean_CMD08_CMD09_CMD10<- (escape2$CMD08+ escape2$CMD09 + escape2$CMD10)/3

escape2$mean_CMD03_CMD04_CMD05_CMD06<- (escape2$CMD03 + escape2$CMD04+ escape2$CMD05 + escape2$CMD06)/4
escape2$mean_CMD04_CMD05_CMD06_CMD07<- (escape2$CMD04 + escape2$CMD05+ escape2$CMD06 + escape2$CMD07)/4
escape2$mean_CMD05_CMD06_CMD07_CMD08<- (escape2$CMD05 + escape2$CMD06+ escape2$CMD07 + escape2$CMD08)/4
escape2$mean_CMD06_CMD07_CMD08_CMD09<- (escape2$CMD06 + escape2$CMD07+ escape2$CMD08 + escape2$CMD09)/4
escape2$mean_CMD07_CMD08_CMD09_CMD10<- (escape2$CMD07 + escape2$CMD08+ escape2$CMD09 + escape2$CMD10)/4

escape2$mean_CMD03_CMD04_CMD05_CMD06_CMD07<- (escape2$CMD03 + escape2$CMD04 + escape2$CMD05+ escape2$CMD06 + escape2$CMD07)/5
escape2$mean_CMD04_CMD05_CMD06_CMD07_CMD08<- (escape2$CMD04 + escape2$CMD05 + escape2$CMD06+ escape2$CMD07 + escape2$CMD08)/5
escape2$mean_CMD05_CMD06_CMD07_CMD08_CMD09<- (escape2$CMD05 + escape2$CMD06 + escape2$CMD07+ escape2$CMD08 + escape2$CMD09)/5
escape2$mean_CMD06_CMD07_CMD08_CMD09_CMD10<- ( escape2$CMD06 + escape2$CMD07+ escape2$CMD08 + escape2$CMD09 + escape2$CMD10)/5

escape2$mean_CMD03_CMD04_CMD05_CMD06_CMD07_CMD08<- (escape2$CMD03 + escape2$CMD04 + escape2$CMD05 + escape2$CMD06 + escape2$CMD07+ escape2$CMD08)/6
escape2$mean_CMD04_CMD05_CMD06_CMD07_CMD08_CMD9<- (escape2$CMD04 + escape2$CMD05 + escape2$CMD06 + escape2$CMD07+ escape2$CMD08 + escape2$CMD09)/6
escape2$mean_CMD05_CMD06_CMD07_CMD08_CMD09_CMD10<- (escape2$CMD05 + escape2$CMD06 + escape2$CMD07+ escape2$CMD08 + escape2$CMD09 + escape2$CMD10)/6
...
variables<- c("spring_Tmax",
 ...
 variables1<-c("spring_Tmax","summer_Tmax", "Tmax04","Tmax05", "Tmax06", "Tmax07", "Tmax08", 
              "mean_Tmax04_Tmax05","mean_Tmax05_Tmax06", "mean_Tmax06_Tmax07", "mean_Tmax07_Tmax08",
               ...
               "spring_Tave","summer_Tave","Tave04","Tave05", "Tave06", "Tave07", "Tave08", 
...
variables2<-c("tot_spring_PPT","tot_summer_PPT","PPT04","PPT05", "PPT06", "PPT07", "PPT08", 
              ...
              "tot_spring_PPT","tot_summer_PPT","PPT04","PPT05", "PPT06", "PPT07", "PPT08", 
              "mean_PPT04_PPT05","mean_PPT05_PPT06", "mean_PPT06_PPT07", "mean_PPT07_PPT08",
               "mean_PPT04_PPT05_PPT06", "mean_PPT05_PPT06_PPT07", "mean_PPT04_PPT05_PPT06_PPT07", "mean_PPT05_PPT06_PPT07_PPT08") 
...
unique(escape2$frt)
str(escape2$frt)
escape2$frt<-as.factor(escape2$frt)
...
for (h in 1:length(zones)) {
  ...
  dat2<- escape2 %>% dplyr::filter(frt ==zones[h])
...
dat_0<-dat2 %>% filter(CURRENT<=1) 
dat_0$escaped<-0
dt_0 = sort(sample(nrow(dat_0), nrow(dat_0)*prop))
train_0<-dat_0[dt_0,]
test_0<-dat_0[-dt_0,]

dat_1<-dat2 %>% filter(CURRENT>1)
dat_1$escaped<-1
...
```{r}
write.csv(aic_bec_lightning_summary2, file="C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\tmp\\escape_climate_AIC_results.csv")
```
