---
title: "13_2e_fire_spread_size_model_selection_person_treed_NDT5"
author: "Cora Skaien"
date: "07/12/2021"
...
#=================================
#  Script Name: 13_2e_fire_spread_size_model_selection_person_treed_NDT5.R
#  Script Version: 1.0
...
table(spread_data_person_t$vegtype2, spread_data_person_t$ntrl_ds)
```


##There are ZERO data points for NDT5 for person-caused fires within treed polygons. Do not need to run below code.
...
variables_all_infra_landuse_NDT5<-c(dist_mun = "dist_mun", dist_dam = "dist_dam", dist_nat = "dist_nat", dist_mine = "dist_mine", roads_km = "roads_km", bclcs_level_5_2 = "bclcs_level_5_2")
...
for (i in 1:(length(inputs.me.infra_landuse_NDT5)-1)) {
  for (j in (i+1):length(inputs.me.infra_landuse_NDT5)) {
     twoway.ints.in <- cbind(twoway.ints.in, paste(inputs.me.infra_landuse_NDT5[i], inputs.me.infra_landuse_NDT5[j], sep=":"))
  }
}
twoway.ints.in
...
#Get variables on own
#complete list of models using non-climate vars
mods.me.tmp.in <- powerSet(variables_all_infra_landuse_NDT5) 
#add climate vars to all of the above
mods.me.infra_landuse_NDT5 <- list()
for (i in 1: length(mods.me.tmp.in)) {
   mods.me.infra_landuse_NDT5[[i]] <- c(mods.me.tmp.in[[i]])
}
...
#complete list of two-way interactions
#mods.twoway.in.NDT5b <- powerSet(twoway.in.ints.in) #
#Or use subset of interactions
mods.twoway.in.NDT5b <- powerSet(twoway.ints.in_dist) 
length(mods.twoway.in.NDT5b) #512
#mods.twoway.in.NDT5b
mods.twoway.in.NDT5b[1]
mods.twoway.in.NDT5b<-mods.twoway.in.NDT5b[-1]

#Finding models in mods.me that accommodate/allow interaction terms in each mods.twoway.in to be added
mods.inter.NDT5b.in <- list()
counter <- 0
for (i in 1: length(mods.twoway.in.NDT5b)) {
   s1 <- unique(unlist( strsplit(mods.twoway.in.NDT5b[[i]], split=':', fixed=TRUE) ) )
   for (j in 1: length(mods.me.infra_landuse_NDT5)) {
      if (all(s1 %in% mods.me.infra_landuse_NDT5[[j]])==TRUE) {
        counter <- counter + 1
        both <-  c(mods.me.infra_landuse_NDT5[[j]], mods.twoway.in.NDT5b[[i]])
        mods.inter.NDT5b.in[[counter]] <- both
      }
   }
}

length(mods.inter.NDT5b.in) #1365
#mods.inter.NDT5b.in

#####2. Infrastructure and land use
all.poss.mods.infra_landuse<-c(1,mods.me.infra_landuse_NDT5, mods.inter.NDT5b.in)
...
for (h in 1:length(zones1)) {
  dat2<- subset(spread_data_person_t, spread_data_person_t$ntrl_ds=="NDT5")
...
AIC_person_NDT5_spread_treed_climate<-table.glm.climate.simple.spread

AIC_person_NDT5_spread_treed_summary_climate<- AIC_person_NDT5_spread_treed_climate %>%
  group_by(model) %>%
...
AIC_person_NDT5_spread_treed_summary_climate2<- AIC_person_NDT5_spread_treed_summary_climate %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT5_spread_treed_summary_climate2)
```
...
AIC_person_NDT5_spread_treed_climate_gamma<-table.glm.climate.simple.spread.gamma

AIC_person_NDT5_spread_treed_summary_climate_gamma<- AIC_person_NDT5_spread_treed_climate_gamma %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic, na.rm=TRUE),
            meanR2=mean(Model.R2, na.rm=TRUE),
            sdR2=sd(Model.R2, na.rm=TRUE),
...
AIC_person_NDT5_spread_treed_summary_climate_gamma2<- AIC_person_NDT5_spread_treed_summary_climate_gamma %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT5_spread_treed_summary_climate_gamma2)
```
...
AIC_person_NDT5_spread_treed_VRI<-table.glm.VRI.simple.spread

AIC_person_NDT5_spread_treed_summary_VRI<- AIC_person_NDT5_spread_treed_VRI %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic, na.rm=TRUE),
            meanR2=mean(Model.R2, na.rm=TRUE),
            meanAdjustR2=mean(Adjusted.R2, na.rm=TRUE),
...
AIC_person_NDT5_spread_treed_summary_VRI2<- AIC_person_NDT5_spread_treed_summary_VRI %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT5_spread_treed_summary_VRI2)
```
...
AIC_person_NDT5_spread_treed_topo<-table.glm.topo.simple.spread

AIC_person_NDT5_spread_treed_summary_topo<- AIC_person_NDT5_spread_treed_topo %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic, na.rm=TRUE),
            meanR2=mean(Model.R2, na.rm=TRUE),
            meanAdjustR2=mean(Adjusted.R2, na.rm=TRUE),
...
AIC_person_NDT5_spread_treed_summary_topo2<- AIC_person_NDT5_spread_treed_summary_topo %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT5_spread_treed_summary_topo2)
```
...
AIC_person_NDT5_spread_treed_infra<-table.glm.infra.simple.spread

AIC_person_NDT5_spread_treed_summary_infra<- AIC_person_NDT5_spread_treed_infra %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic, na.rm=TRUE),
            meanR2=mean(Model.R2, na.rm=TRUE),
            meanAdjustR2=mean(Adjusted.R2, na.rm=TRUE),
...
AIC_person_NDT5_spread_treed_summary_infra2<- AIC_person_NDT5_spread_treed_summary_infra %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT5_spread_treed_summary_infra2)
```

Combine together
```{r}
NDT5_l_treed_spread<-rbind(AIC_person_NDT5_spread_treed_summary_climate2, AIC_person_NDT5_spread_treed_summary_infra2, AIC_person_NDT5_spread_treed_summary_topo2, AIC_person_NDT5_spread_treed_summary_VRI2)

NDT5_l_treed_spread$NDT<-"NDT5"

head(NDT5_l_treed_spread)
```

Save data.
```{r}
write.csv(NDT5_l_treed_spread, file="D:\\Fire\\fire_data\\raw_data\\NDT5_person_models_treed_spread_Nov.csv")
```
...
```{r}
spread_person_t_NDT5<-subset(spread_data_person_t, spread_data_person_t$ntrl_ds=="NDT5")

prop<-0.75

#Partition data into training and validation
 trainIndex <- createDataPartition(spread_person_t_NDT5$vegtype2, p = prop,
                                    list = FALSE,
                                    times = 1)
  
   dat1 <- spread_person_t_NDT5[ trainIndex,]
   Valid <- spread_person_t_NDT5[-trainIndex,]
  
#Run model using dat1
model.NDT5<-lm(LNsize_ha ~ proj_height_1 + live_stand_volume_125 + dist_mun + dist_mine + roads_km + bclcs_level_5_2 + dist_mun:roads_km + roads_km:bclcs_level_5_2 + slope + aspect + slope:aspect + climate1 + vegtype2 + wind_atfire + climate1:proj_height_1 + climate1:proj_age_1 + climate1:live_stand_volume_125 + climate1:elevation + climate1:wind_atfire + slope:wind_atfire + elevation:wind_atfire + vegtype2:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire + Tdif_atfire:live_stand_volume_125, data = dat1)

AIC(model.NDT5) #1636.9
summary(model.NDT5)$r.squared #0.24
summary(model.NDT5)$adj.r.squared #0.18

predictions <- model.NDT5 %>% predict(Valid)
...
NDT5_spread_size_t<-spread_person_t_NDT5 %>% drop_na(proj_height_1 , live_stand_volume_125 , dist_mine , roads_km , bclcs_level_5_2, slope , aspect , climate1 , vegtype2 , wind_atfire, elevation)

#Run Model again with this data; but uses all data here
model.NDT5.Spread<-lm(LNsize_ha ~ proj_height_1 + live_stand_volume_125  + dist_mine + roads_km + bclcs_level_5_2 + roads_km:bclcs_level_5_2 + slope + aspect + climate1 + vegtype2 + wind_atfire  + elevation + climate1:elevation + elevation:wind_atfire + vegtype2:wind_atfire, data = NDT5_spread_size_t)

#Anova(model.NDT5.Spread, type=3)
Anova(model.NDT5.Spread, type=3, singular.ok = TRUE)

# model diagnostic plots
binnedplot (fitted(model.NDT5.Spread), 
            residuals(model.NDT5.Spread), 
            nclass = NULL, 
...
NDT5_spread_size_t$resids<-resid(model.NDT5.Spread)

binnedplot (NDT5_spread_size_t$live_stand_volume_125, 
            NDT5_spread_size_t$resids, 
            nclass = NULL, 
            xlab = "live stand volume", 
            ylab = "Average residual", 
            main = paste("Binned Residual Plot - glm", i))

binnedplot (NDT5_spread_size_t$climate1, 
            NDT5_spread_size_t$resids, 
            nclass = NULL, 
            xlab = "", 
            ylab = "Average residual", 
...
visreg(model.NDT5.Spread, "proj_height_1")
visreg(model.NDT5.Spread, "live_stand_volume_125")

visreg(model.NDT5.Spread, "dist_mine")
visreg(model.NDT5.Spread, "roads_km", by="bclcs_level_5_2", overlay=TRUE)
visreg(model.NDT5.Spread, "bclcs_level_5_2", by="roads_km")

visreg(model.NDT5.Spread, "slope")
visreg(model.NDT5.Spread, "aspect")
visreg(model.NDT5.Spread, "elevation", by="climate1")
visreg(model.NDT5.Spread, "elevation", by="wind_atfire")
#climate1 + vegtype2 + wind_atfire  + elevation + climate1:elevation + elevation:wind_atfire + vegtype2:wind_atfire

visreg(model.NDT5.Spread, "climate1", by="elevation")
visreg(model.NDT5.Spread, "vegtype2", by="wind_atfire")

visreg(model.NDT5.Spread, "wind_atfire", by="vegtype2", overlay=TRUE)
visreg(model.NDT5.Spread, "wind_atfire", by="elevation")

```
We should repeat the above several times and take the mean of the coefficients.

```{r}
summary(model.NDT5.Spread)

#Create a new blank table and get AUC too
top_mod_table_NDT5_person_t_ALL <- data.frame (matrix (ncol = 30, nrow = 0))
colnames (top_mod_table_NDT5_person_t_ALL ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_proj_height_1", "coef_live_stand_volume_125", "coef_dist_mine", "coef_roads_km", "coef_bclcs_level_5_2OP", "coef_bclcs_level_5_2SP", "coef_slope", "coef_aspect", "coef_climate_1", "coef_vegtypeTC", "coef_vegtypeTM", "coef_wind_atfire", "coef_elevation", "coef_roads_km:bclcs_level_5_2OP", "coef_roads_km:bclcs_level_5_2SP", "coef_climate1:elevation", "coef_wind_atfire:elevation", "coef_vegtype2TC:wind_atfire", "coef_vegtype2TM:wind_atfire", "AIC", "Model.R2", "Valid.R2", "Valid.RSME", "Valid.MAE", "Adjusted.R2")
...
prop<-0.75
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(NDT5_spread_size_t$vegtype2, p = prop,
                                    list = FALSE,
                                    times = 1)
  
   dat1 <- NDT5_spread_size_t[ trainIndex,]
   Valid <- NDT5_spread_size_t[-trainIndex,]
   
#Model   
model.NDT5.Spread<-lm(LNsize_ha ~ proj_height_1 + live_stand_volume_125  + dist_mine + roads_km + bclcs_level_5_2 + roads_km:bclcs_level_5_2 + slope + aspect + climate1 + vegtype2 + wind_atfire + elevation + climate1:elevation + elevation:wind_atfire + vegtype2:wind_atfire, data = dat1) 

predictions <- model.NDT5.Spread %>% predict(Valid)

# create model table (only do this once) and add the relevant data
top_mod_table_NDT5_person_t <- data.frame (matrix (ncol = 30, nrow = 0))
colnames (top_mod_table_NDT5_person_t ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_proj_height_1", "coef_live_stand_volume_125", "coef_dist_mine", "coef_roads_km", "coef_bclcs_level_5_2OP", "coef_bclcs_level_5_2SP", "coef_slope", "coef_aspect", "coef_climate_1", "coef_vegtypeTC", "coef_vegtypeTM", "coef_wind_atfire", "coef_elevation", "coef_roads_km:bclcs_level_5_2OP", "coef_roads_km:bclcs_level_5_2SP", "coef_climate1:elevation", "coef_wind_atfire:elevation", "coef_vegtype2TC:wind_atfire", "coef_vegtype2TM:wind_atfire", "AIC", "Model.R2", "Valid.R2", "Valid.RSME", "Valid.MAE", "Adjusted.R2")

##Add data for NDT5
top_mod_table_NDT5_person_t[1,1]<-"person"
top_mod_table_NDT5_person_t[1,2]<-"NDT5"
top_mod_table_NDT5_person_t[1,3]<-"Y"
top_mod_table_NDT5_person_t[1,4]<-"LNsize_ha ~ proj_height_1 + live_stand_volume_125  + dist_mine + roads_km + bclcs_level_5_2 + roads_km:bclcs_level_5_2 + slope + aspect + climate1 + vegtype2 + wind_atfire + elevation + climate1:elevation + elevation:wind_atfire + vegtype2:wind_atfire" 
top_mod_table_NDT5_person_t[1,5]<- coef(model.NDT5.Spread)[1] #Intercept
top_mod_table_NDT5_person_t[1,6]<- coef(model.NDT5.Spread)[2] #C
top_mod_table_NDT5_person_t[1,7]<- coef(model.NDT5.Spread)[3] #C
top_mod_table_NDT5_person_t[1,8]<- coef(model.NDT5.Spread)[4] #co
top_mod_table_NDT5_person_t[1,9]<- coef(model.NDT5.Spread)[5] #co
top_mod_table_NDT5_person_t[1,10]<- coef(model.NDT5.Spread)[6] #c
top_mod_table_NDT5_person_t[1,11]<- coef(model.NDT5.Spread)[7] #l
top_mod_table_NDT5_person_t[1,12]<- coef(model.NDT5.Spread)[8] #co
top_mod_table_NDT5_person_t[1,13]<- coef(model.NDT5.Spread)[9] #c
top_mod_table_NDT5_person_t[1,14]<- coef(model.NDT5.Spread)[10] #coeff
top_mod_table_NDT5_person_t[1,15]<- coef(model.NDT5.Spread)[11] #co
top_mod_table_NDT5_person_t[1,16]<- coef(model.NDT5.Spread)[12] #coe
top_mod_table_NDT5_person_t[1,17]<- coef(model.NDT5.Spread)[13] #co
top_mod_table_NDT5_person_t[1,18]<- coef(model.NDT5.Spread)[14] #co
top_mod_table_NDT5_person_t[1,19]<- coef(model.NDT5.Spread)[15] #
top_mod_table_NDT5_person_t[1,20]<- coef(model.NDT5.Spread)[16] # cl
top_mod_table_NDT5_person_t[1,21]<- coef(model.NDT5.Spread)[17] # c
top_mod_table_NDT5_person_t[1,22]<- coef(model.NDT5.Spread)[18] #coefficien
top_mod_table_NDT5_person_t[1,23]<- coef(model.NDT5.Spread)[19] #coefficien
top_mod_table_NDT5_person_t[1,24]<- coef(model.NDT5.Spread)[20] #coefficien
top_mod_table_NDT5_person_t[1,25]<- AIC(model.NDT5.Spread)
top_mod_table_NDT5_person_t[1,26]<- summary(model.NDT5.Spread)$r.squared
top_mod_table_NDT5_person_t[1,27]<- R2(predictions, Valid$LNsize_ha, na.rm=TRUE)
top_mod_table_NDT5_person_t[1,28]<- RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE)
top_mod_table_NDT5_person_t[1,29]<- MAE(predictions, Valid$LNsize_ha, na.rm=TRUE)
top_mod_table_NDT5_person_t[1,30]<- summary(model.NDT5.Spread)$adj.r.squared
...
```{r}
write.csv(top_mod_table_NDT5_person_t_ALL, file="D:\\Fire\\fire_data\\raw_data\\top_mod_model.NDT5.Spread_firesize_person_t.csv")
```
...
top_mod_table_NDT5_model.NDT5.Spreadperson_t_means<-top_mod_table_NDT5_person_t_ALL %>% summarise_each(funs( mean( .,na.rm = TRUE)))
top_mod_table_NDT5_model.NDT5.Spreadperson_t_means

top_mod_table_NDT5_model.NDT5.Spreadperson_t_means[1,1]<-"person"
top_mod_table_NDT5_model.NDT5.Spreadperson_t_means[1,2]<-"NDT5"
top_mod_table_NDT5_model.NDT5.Spreadperson_t_means[1,3]<-"Treed"
top_mod_table_NDT5_model.NDT5.Spreadperson_t_means[1,4]<- "LNsize_ha ~ proj_height_1 + live_stand_volume_125  + dist_mine + roads_km + bclcs_level_5_2 + roads_km:bclcs_level_5_2 + slope + aspect + climate1 + vegtype2 + wind_atfire + elevation + climate1:elevation + elevation:wind_atfire + vegtype2:wind_atfire" 
top_mod_table_NDT5_model.NDT5.Spreadperson_t_means
```
Save table.

```{r}
write.csv(top_mod_table_NDT5_model.NDT5.Spreadperson_t_means, file="D:\\Fire\\fire_data\\raw_data\\top_mod_model.NDT5.Spread_firesize_person_t_Means.csv")
```

Standard deviation.

```{r}
top_mod_table_NDT5_model.NDT5.Spreadperson_t_sd<-top_mod_table_NDT5_person_t_ALL %>% summarise_each(funs( sd( .,na.rm = TRUE)))
top_mod_table_NDT5_model.NDT5.Spreadperson_t_sd

top_mod_table_NDT5_model.NDT5.Spreadperson_t_sd[1,1]<-"person"
top_mod_table_NDT5_model.NDT5.Spreadperson_t_sd[1,2]<-"NDT5"
top_mod_table_NDT5_model.NDT5.Spreadperson_t_sd[1,3]<-"Treed"
top_mod_table_NDT5_model.NDT5.Spreadperson_t_sd[1,4]<-"LNsize_ha ~ proj_height_1 + live_stand_volume_125  + dist_mine + roads_km + bclcs_level_5_2 + roads_km:bclcs_level_5_2 + slope + aspect + climate1 + vegtype2 + wind_atfire + elevation + climate1:elevation + elevation:wind_atfire + vegtype2:wind_atfire" 
top_mod_table_NDT5_model.NDT5.Spreadperson_t_sd
```
...
```{r}
write.csv(top_mod_table_NDT5_model.NDT5.Spreadperson_t_sd, file="D:\\Fire\\fire_data\\raw_data\\top_mod_model.NDT5.Spread_firesize_person_t_SD.csv")
```
