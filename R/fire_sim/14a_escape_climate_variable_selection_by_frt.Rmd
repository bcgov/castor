...
escape10<- escape2 %>% filter(Cluster==10)
table(escape10$FIRE_YEAR, escape10$ig_mnth)
...
dist.cut.corr <- escape2 [c (77:83, 97:103)]
...
dist.cut.corr <- escape2 [c (77:83, 107:113)]
corr <- round (cor (dist.cut.corr), 3)
ggcorrplot (corr, type = "lower", lab = TRUE, tl.cex = 10,  lab_size = 3,
            title = "Correlation between maximum temperature and RH") # a little correlated 0.62

dist.cut.corr <- escape2 [c (77:83, 127:133)]
corr <- round (cor (dist.cut.corr), 3)
ggcorrplot (corr, type = "lower", lab = TRUE, tl.cex = 10,  lab_size = 3,
            title = "Correlation between maximum temperature and CMI") # very corrleated 0.8

dist.cut.corr <- escape2 [c (97:103, 107:113)]
corr <- round (cor (dist.cut.corr), 3)

ggcorrplot (corr, type = "lower", lab = TRUE, tl.cex = 10,  lab_size = 3,
            title = "Correlation between PPT and RH") # not really correlated

dist.cut.corr <- escape2 [c (97:103, 127:133)]
corr <- round (cor (dist.cut.corr), 3)

ggcorrplot (corr, type = "lower", lab = TRUE, tl.cex = 10,  lab_size = 3,
            title = "Correlation between PPT and CMI") # not really correlated
...
```{r}
write.csv(aic_bec_lightning_summary2, file="C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\tmp\\spread_climate_AIC_results_Cluster.csv")
```
