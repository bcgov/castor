...
Save the DEM layer file. This save will take several hours since there is over 1,000,000 data points, so wait until further steps are complete if that is an option. Note: when I attempted to save these, it crashed after 10 hours. Avoid saving at this time.
...
head(spread_500_DEM_buf) #Note, now these are polygon features instead of point features because we have buffers.
```

Perform st_intersection between the roads layer and our buffered layer to get roads clipped to the area of the polygons.

```{r}
spread_150_roads_buffers<-st_intersection(spread_150_DEM_buf, roads_layer)
head(spread_150_roads_buffers)


#Check what the distribution of shape length is
hist(spread_150_roads_buffers$shape_length)
mean(spread_150_roads_buffers$shape_length) #Mean shape length is 548 m

#Repeat for 500 m
spread_500_roads_buffers<-st_intersection(spread_500_DEM_buf, roads_layer)
head(spread_500_roads_buffers)


#Check what the distribution of shape length is
hist(spread_500_roads_buffers$shape_length)
mean(spread_500_roads_buffers$shape_length) #Mean shape length is 548 m
...
spread_150_roads_buffers_df<-as.data.frame(spread_150_roads_buffers)

road_sums_df_150<- spread_150_roads_buffers_df %>%
  group_by(id) %>% #Check what ID is needed!! May be OBJECTID? Needs to be different for each point.
  summarize(
...
str(road_sums_df_150)#Check how many rows there are in the data and compare to our initial data
#181,356
str(spread_150_roads_buffers) # 274594 obs
...
spread_500_roads_buffers_df<-as.data.frame(spread_500_roads_buffers)

road_sums_df_500<- spread_500_roads_buffers_df %>%
  group_by(id) %>% #Check what ID is needed!! May be OBJECTID? Needs to be different for each point.
  summarize(
    shape_length = sum(shape_length)
  )

table(road_sums_df_500$id)
...
spread_150_DEM_roads<- merge(spread_150_roads_buffers, road_sums_df_150, by = 'id', all=TRUE)
...
#spread_150_DEM_roads_sf<-st_as_sf(spread_150_DEM_roads)

#Replace NAs in shape_length column with 0s
spread_150_DEM_roads$shape_length[is.na(spread_150_DEM_roads$shape_length)] <- 0
head(spread_150_DEM_roads) #We can see that NAs have not been switched to 0

###500 m buffers
crs(road_sums_df_500) #Check if it has a coordinate system - no (as expected)

spread_500_DEM_roads<- merge(spread_500_roads_buffers, road_sums_df_500, by = 'id', all=TRUE)
...
mean(spread_150_DEM_roads$shape_length) #Mean is 3841.987 m of road per km square
max(spread_150_DEM_roads$shape_length) #Max is 207,868.7 m of road per km square. This seems impossible.

##Convert to kms worth of road
spread_150_DEM_roads$shape_length_km<-(spread_150_DEM_roads$shape_length/1000)
hist(spread_150_DEM_roads$shape_length_km, n=100)
mean(spread_150_DEM_roads$shape_length_km) #Mean is 3.8 km of road per km square
max(spread_150_DEM_roads$shape_length_km) #max is 207 km of road per km square... seems unlikely!

spread_150_DEM_roads_2<-subset(spread_150_DEM_roads, spread_150_DEM_roads$shape_length_km < 100)
hist(spread_150_DEM_roads_2$shape_length_km, n=100) #Is outlier removed? New max is 138 if subset at the 200 km level.
max(spread_150_DEM_roads_2$shape_length_km)
str(spread_150_DEM_roads_2) #274573 observations -- only lose 21 observations and likely more accurate
str(spread_150_DEM_roads) #274594 observations

##500 m buffer
hist(spread_500_DEM_roads$shape_length)
mean(spread_500_DEM_roads$shape_length) #Mean is 3841.987 m of road per km square
max(spread_500_DEM_roads$shape_length) #Max is 207,868.7 m of road per km square. This seems impossible.
...
```{r}
st_write(spread_150_DEM_roads_2, dsn = "D:\\Fire\\fire_data\\raw_data\\ClimateBC_Data\\spread_150_DEM_roads.shp", delete_layer=TRUE)
...
```{r}
##Extract Coordinates
test<-cbind(spread_150_DEM_roads_2, st_coordinates(spread_150_DEM_roads_2))
head(test)
...
str(spread_150_DEM_roads_2)# 274573 values

#Append new information
spread_150_DEM_roads_wind<-cbind(spread_150_DEM_roads_2, rasValue3)
head(spread_150_DEM_roads_wind)
crs(spread_150_DEM_roads_wind)
```


Now, select all those fires that are >1 ha (i.e., those that escaped)
