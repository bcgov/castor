...
#Considerations
Consider quadratic terms for continuous variables and including the untransformed data and the quadratic term (X + X^2) in the model.
...
Combine all four datasets together with new method.

```{r}
#First ensure all same columns
names(spread_data_lightning_t)
names(spread_data_lightning_nt)
names(spread_data_person_t)
names(spread_data_person_nt)

#Remove mdc_atfire_2 and fire_zone
spread_data_lightning_t<-spread_data_lightning_t[-132]
spread_data_lightning_t<-spread_data_lightning_t[-132]
spread_data_lightning_nt<-spread_data_lightning_nt[-132]
spread_data_lightning_nt<-spread_data_lightning_nt[-132]

#Remove escape10 and size_ha_x100
spread_data_lightning_t<-spread_data_lightning_t[-149]
spread_data_lightning_t<-spread_data_lightning_t[-145]
spread_data_lightning_nt<-spread_data_lightning_nt[-149]
spread_data_lightning_nt<-spread_data_lightning_nt[-145]

spread_data_ALL<-rbind(spread_data_lightning_t, spread_data_lightning_nt, spread_data_person_t, spread_data_person_nt)
```

```{r}
write.csv(spread_data_ALL, file="D:\\Fire\\fire_data\\raw_data\\spread_data_ALL_Dec.csv")

```

#Continue work with combined data

```{r}
spread_data_ALL<- read.csv(file="D:\\Fire\\fire_data\\raw_data\\spread_data_ALL_Dec.csv")
```
...
```{r}
#################################
#### Running simple linear model: all lightning, person, treed and non treed
#################################
# create loop to do variable selection of climate data
unique(spread_data_ALL$ntrl_ds)
...
for (h in 1:length(zones_spread)) {
  dat2<- spread_data_ALL %>% dplyr::filter(ntrl_ds ==zones_spread[h])
...
## Create empty vector
spread_data_ALL$climate1<-0
head(spread_data_ALL)

spread_data_ALL<-spread_data_ALL %>%
    mutate(climate1 = case_when(ntrl_ds_numeric == 1 ~ tmax08, # NDT1
                                ntrl_ds_numeric == 2 ~ tave09, #NDT2
                                ntrl_ds_numeric == 3 ~ tmax06, #NDT3
                                ntrl_ds_numeric == 4 ~ tmax05, # NDT4
                                ntrl_ds_numeric == 5 ~ tave08, # NDT5
                                TRUE ~ NA_real_))

#Repeat for climate2
spread_data_ALL$ppt05<-as.numeric(spread_data_ALL$ppt05)
spread_data_ALL$ppt06<-as.numeric(spread_data_ALL$ppt06)
spread_data_ALL$ppt08<-as.numeric(spread_data_ALL$ppt08)
spread_data_ALL$ppt09<-as.numeric(spread_data_ALL$ppt09)

spread_data_ALL$climate2<-0
head(spread_data_ALL)
...
spread_data_ALL<-spread_data_ALL %>%
    mutate(climate2 = case_when(ntrl_ds_numeric == 1 ~ ppt08, # NDT1
                                ...
                                ntrl_ds_numeric == 4 ~ ppt05, # NDT4
                                ntrl_ds_numeric == 5 ~ ppt08, # NDT5
                                TRUE ~ NA_real_))

```

#Repeat above for all NDTs together 
...
#################################
# create loop to do variable selection of climate data
spread_data_ALL$ONE<-c(1)
ONE<-spread_data_ALL$ONE

filenames<-list()

#Begin loop
for (h in 1) {
  dat2<- spread_data_ALL
...
table.glm.climate.simple[1,1]<-"ALLNDT"
table.glm.climate.simple[1,2]<-"intercept"
table.glm.climate.simple[1,3]<-extractAIC(model1)[2]


for (i in 1: length(variables)){
  print(paste((variables[i]), sep=" "))
  ...
  table.glm.climate.simple[i+1,1]<-"ALLNDT"
  table.glm.climate.simple[i+1,2]<-variables[i]
  table.glm.climate.simple[i+1,3]<-extractAIC(model1)[2]
...
for (i in 1: length(variables1)){
  print(paste((variables1[i]), variables2[i], sep=" "))
  model_dat<- dat2 %>% dplyr::select(LNsize_ha, variables1[i], variables2[i])
  
  model2 <- lm (LNsize_ha ~ . ,
                 data=model_dat)
  
  table.glm.climate.simple[(i+length(variables))+1,1]<-"ALLNDT"
  table.glm.climate.simple[(i+length(variables))+1,2]<-paste0(variables1[i],"+", variables2[i])
  table.glm.climate.simple[(i+length(variables))+1,3]<-extractAIC(model2)[2]
...
for (i in 1: length(variables1)){
  print(paste((variables1[i]), "x",variables2[i], sep=" "))
  ...
  table.glm.climate.simple[(i+length(variables) +length(variables1) + 1),1]<-"ALLNDT"
  table.glm.climate.simple[(i+length(variables) +length(variables1) + 1),2]<-paste0(variables1[i],"x", variables2[i])
  table.glm.climate.simple[(i+length(variables) +length(variables1) + 1),3]<-extractAIC(model2)[2]
...
#assign file names to the work
nam1<-paste("AIC","run",h,sep="_") #defining the name
assign(nam1,table.glm.climate.simple)
filenames<-append(filenames,nam1)
}
...
aic_bec_spread_summary_2<- aic_bec_spread %>%
  group_by(Variable) 

aic_bec_spread_summary_2b<- aic_bec_spread_summary_2 %>%
  mutate(deltaAIC=AIC-min(AIC))

#Above not working at the moment, so do manually
min(aic_bec_spread_summary_2$AIC) #8627.998
aic_bec_spread_summary_2$minAIC<-min(aic_bec_spread_summary_2$AIC)
aic_bec_spread_summary_2$minAIC

aic_bec_spread_summary_2$deltaAIC<-aic_bec_spread_summary_2$AIC -aic_bec_spread_summary_2$minAIC

aic_bec_spread_summary_2b<-aic_bec_spread_summary_2
aic_bec_spread_summary_2b

```
Save to computer.
```{r}
write.csv(aic_bec_spread_summary_2b, file="D:\\Fire\\fire_data\\raw_data\\aic_bec_spread_climate_summary_sizeha_ALLNDT.csv")
```
...
#Ensure that dist_any contains the relevant values and not all

```{r}
spread_data_ALL$dist_any_<-0
spread_data_ALL$dist_any<-0

spread_data_ALL$dist_any_<-
  ifelse(spread_data_ALL$dist_mine<spread_data_ALL$dist_pow, spread_data_ALL$dist_mine, spread_data_ALL$dist_pow)
str(spread_data_ALL$dist_any_)

spread_data_ALL$dist_any<-
  ifelse (spread_data_ALL$dist_any_<spread_data_ALL$dist_mun, spread_data_ALL$dist_any_, spread_data_ALL$dist_mun)
str(spread_data_ALL$dist_any)

spread_data_ALL$dist_any_<-
  ifelse (spread_data_ALL$dist_any<spread_data_ALL$dist_dam, spread_data_ALL$dist_any, spread_data_ALL$dist_dam)
str(spread_data_ALL$dist_any_)

spread_data_ALL$dist_infra<-spread_data_ALL$dist_any_
hist(spread_data_ALL$dist_infra)
```

Save data.
```{r}
write.csv(spread_data_ALL, file="D:\\Fire\\fire_data\\raw_data\\spread_data_ALL_Dec2021.csv")
```




########### IF Not Separated by NDT ##############
#Rerun above but not by NDT
...
# create loop to do variable selection of climate data
filenames<-list()
spread_data_ALL$ALL<-1

#Begin loop
for (h in 1:length(ALL)) {
  dat2<- spread_data_ALL
  
#Create frame of AIC table
# summary table
table.glm.climate.simple <- data.frame (matrix (ncol = 2, nrow = 0))
colnames (table.glm.climate.simple) <- c ("Variable", "AIC")
...
table.glm.climate.simple[1,1]<-"intercept"
table.glm.climate.simple[1,2]<-extractAIC(model1)[2]
...
for (i in 1: length(variables)){
  ...
  table.glm.climate.simple[i+1,1]<-variables[i]
  table.glm.climate.simple[i+1,2]<-extractAIC(model1)[2]
...
for (i in 1: length(variables1)){
  ...
  table.glm.climate.simple[(i+length(variables))+1,1]<-paste0(variables1[i],"+", variables2[i])
  table.glm.climate.simple[(i+length(variables))+1,2]<-extractAIC(model2)[2]
...
for (i in 1: length(variables1)){
  ...
  table.glm.climate.simple[(i+length(variables) +length(variables1) + 1),1]<-paste0(variables1[i],"x", variables2[i])
  table.glm.climate.simple[(i+length(variables) +length(variables1) + 1),2]<-extractAIC(model2)[2]
...
Save to computer.
```{r}
write.csv(aic_bec_spread_summary2, file="D:\\Fire\\fire_data\\raw_data\\aic_bec_spread_climate_summary_sizeha_ALLNDTs.csv")
```
...
spread_data_ALL<-spread_data_ALL %>%
    mutate(climate1 = case_when(ntrl_ds_numeric == 1 ~ mean_tmax06_tmax07_tmax08_tmax09, # NDT1
                                ntrl_ds_numeric == 2 ~ mean_tmax06_tmax07_tmax08_tmax09, #NDT2
                                ntrl_ds_numeric == 3 ~ mean_tmax06_tmax07_tmax08_tmax09, #NDT3
                                ntrl_ds_numeric == 4 ~ mean_tmax06_tmax07_tmax08_tmax09, # NDT4
                                ntrl_ds_numeric == 5 ~ mean_tmax06_tmax07_tmax08_tmax09, # NDT5
                                TRUE ~ NA_real_))

```
No climate2 needed; if still in, then remove

```{r}
names(spread_data_ALL)
spread_data_ALL<-spread_data_ALL[-137]
```

#Add quadratic terms for variables of interest

#Inspect relationships with key variables
Start with Climate1. The results here show that the relationship with climate 1 is indeed linear; no quadratic term needed.
```{r}
ggplot (spread_data_ALL, aes (x = LNsize_ha, y = climate1, colour = factor(ntrl_ds))) +
  geom_point () +
  geom_smooth (se=F) +
  labs (title = "Effects Climate1 by NDT",
        x = "LN(fire size)",
        y = "Climate1")  +
  scale_colour_discrete (name = "NDT", labels = c("NDT1", "NDT2", "NDT3", "NDT4", "NDT5"))
```

For climate2, also look predominately linear, with a slight curve perhaps for NDT1 (hump)

```{r}
ggplot (spread_data_ALL, aes (x = LNsize_ha, y = climate2, colour = factor(ntrl_ds))) +
  geom_point () +
  geom_smooth (se=F) +
  labs (title = "Effects Climate2 by NDT",
        x = "LN(fire size)",
        y = "Climate2")  +
  scale_colour_discrete (name = "NDT", labels = c("NDT1", "NDT2", "NDT3", "NDT4", "NDT5"))
```

MDC_atfire mostly linear, except there is a lag at the start wherein it remains flat until a fire size of ~ ln(3). Add quadratic term.

```{r}
ggplot (spread_data_ALL, aes (x = LNsize_ha, y = mdc_atfire, colour = factor(ntrl_ds))) +
  geom_point () +
  geom_smooth (se=F) +
  labs (title = "Effects mdc_atfire by NDT",
        x = "LN(fire size)",
        y = "mdc_atfire")  +
  scale_colour_discrete (name = "NDT", labels = c("NDT1", "NDT2", "NDT3", "NDT4", "NDT5"))
```
...
```{r}
spread_data_ALL$mdc_atfire2<-spread_data_ALL$mdc_atfire^2
spread_data_ALL$mdc_atfire2
```


Projected height looks pretty linear for all categories

```{r}
ggplot (spread_data_ALL, aes (x = LNsize_ha, y = proj_height_1, colour = factor(ntrl_ds))) +
  geom_point () +
  geom_smooth (se=F) +
  labs (title = "Effects proj_height_1 by NDT",
        x = "LN(fire size)",
        y = "proj_height_1")  +
  scale_colour_discrete (name = "NDT", labels = c("NDT1", "NDT2", "NDT3", "NDT4", "NDT5"))
```

Projected age also looks linear.
```{r}
ggplot (spread_data_ALL, aes (x = LNsize_ha, y = proj_age_1, colour = factor(ntrl_ds))) +
  geom_point () +
  geom_smooth (se=F) +
  labs (title = "Effects proj_age_1 by NDT",
        x = "LN(fire size)",
        y = "proj_age_1")  +
  scale_colour_discrete (name = "NDT", labels = c("NDT1", "NDT2", "NDT3", "NDT4", "NDT5"))
```
...
```{r}
ggplot (spread_data_ALL, aes (x = LNsize_ha, y = live_stand_volume_125, colour = factor(ntrl_ds))) +
  geom_point () +
  geom_smooth (se=F) +
  labs (title = "Effects live_stand_volume_125 by NDT",
        x = "LN(fire size)",
        y = "live_stand_volume_125")  +
  scale_colour_discrete (name = "NDT", labels = c("NDT1", "NDT2", "NDT3", "NDT4", "NDT5"))
```

The relationship with slope is non-linear in most. 

```{r}
ggplot (spread_data_ALL, aes (x = LNsize_ha, y = slope, colour = factor(ntrl_ds))) +
  geom_point () +
  geom_smooth (se=F) +
  labs (title = "Effects slope by NDT",
        x = "LN(fire size)",
        y = "slope")  +
  scale_colour_discrete (name = "NDT", labels = c("NDT1", "NDT2", "NDT3", "NDT4", "NDT5"))
```
...
```{r}
spread_data_ALL$slope2<-spread_data_ALL$slope^2
```

Aspect linear.

```{r}
ggplot (spread_data_ALL, aes (x = LNsize_ha, y = aspect, colour = factor(ntrl_ds))) +
  geom_point () +
  geom_smooth (se=F) +
  labs (title = "Effects aspect by NDT",
        x = "LN(fire size)",
        y = "aspect")  +
  scale_colour_discrete (name = "NDT", labels = c("NDT1", "NDT2", "NDT3", "NDT4", "NDT5"))
```
...
```{r}
ggplot (spread_data_ALL, aes (x = LNsize_ha, y = elevation, colour = factor(ntrl_ds))) +
  geom_point () +
  geom_smooth (se=F) +
  labs (title = "Effects elevation by NDT",
        x = "LN(fire size)",
        y = "elevation")  +
  scale_colour_discrete (name = "NDT", labels = c("NDT1", "NDT2", "NDT3", "NDT4", "NDT5"))
```

Linear except in NDT5, then very unusual curve.

```{r}
ggplot (spread_data_ALL, aes (x = LNsize_ha, y = dist_infra, colour = factor(ntrl_ds))) +
  geom_point () +
  geom_smooth (se=F) +
  labs (title = "Effects dist_infra by NDT",
        x = "LN(fire size)",
        y = "dist_infra")  +
  scale_colour_discrete (name = "NDT", labels = c("NDT1", "NDT2", "NDT3", "NDT4", "NDT5"))
```
...
```{r}
spread_data_ALL$dist_infra2<-spread_data_ALL$dist_infra^2
```

Wind linear.

```{r}
ggplot (spread_data_ALL, aes (x = LNsize_ha, y = wind_atfire, colour = factor(ntrl_ds))) +
  geom_point () +
  geom_smooth (se=F) +
  labs (title = "Effects wind_atfire by NDT",
        x = "LN(fire size)",
        y = "wind_atfire")  +
  scale_colour_discrete (name = "NDT", labels = c("NDT1", "NDT2", "NDT3", "NDT4", "NDT5"))
```

Roads linear, but definitely some outliers. 

```{r}
ggplot (spread_data_ALL, aes (x = LNsize_ha, y = roads_km, colour = factor(ntrl_ds))) +
  geom_point () +
  geom_smooth (se=F) +
  labs (title = "Effects roads_km by NDT",
        x = "LN(fire size)",
        y = "roads_km")  +
  scale_colour_discrete (name = "NDT", labels = c("NDT1", "NDT2", "NDT3", "NDT4", "NDT5"))
...
```{r}
ggplot (spread_data_ALL, aes (x = LNsize_ha, y = Tdif_atfire, colour = factor(ntrl_ds))) +
  geom_point () +
  geom_smooth (se=F) +
  labs (title = "Effects Tdif_atfire by NDT",
        x = "LN(fire size)",
        y = "Tdif_atfire")  +
  scale_colour_discrete (name = "NDT", labels = c("NDT1", "NDT2", "NDT3", "NDT4", "NDT5"))
```
...
Save data.
```{r}
write.csv(spread_data_ALL, file="D:\\Fire\\fire_data\\raw_data\\spread_data_ALL_Dec2021_CombinedNDT.csv")
```
