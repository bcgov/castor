...
```{r}
unknown_ignition_weather_veg

unknown_ignition_weather_veg$yearHarvest<-as.numeric(as.character(stringi::stri_sub(unknown_ignition_weather_veg$harvest_date,1,4)))
unknown_ignition_weather_veg<- unknown_ignition_weather_veg %>% dplyr::mutate(yearsSinceHarvest = FIRE_YEAR - yearHarvest)

unknown_ignition_weather_veg$earliest_nonlogging_dist_type_2<-gsub('[" "]', '', unknown_ignition_weather_veg$earliest_nonlogging_dist_type) # removing some weird characters
table(unknown_ignition_weather_veg$earliest_nonlogging_dist_type)
table(unknown_ignition_weather_veg$earliest_nonlogging_dist_type_2) # better

unknown_ignition_weather_veg$earliest_nonlogging_dist_type<-unknown_ignition_weather_veg$earliest_nonlogging_dist_type_2

unknown_ignition_weather_veg$earliest_nonlogging_dist_type_2<- as.factor(unknown_ignition_weather_veg$earliest_nonlogging_dist_type_2)
levels(unknown_ignition_weather_veg$earliest_nonlogging_dist_type_2)<-list(burn="B",burn="BE", burn="BR", burn="BW", burn="NB",drought="D",  disease="DF", disease="DMP", disease="DS", disease="DSG", insect="I", insect="IBB", insect="IBD", insect="IBF", insect="IBM", insect="IBS", flooding="NF", slide="NS", windthrow="NW", planting="P" ) # amalgamate some categories
table(unknown_ignition_weather_veg$earliest_nonlogging_dist_type_2)

unknown_ignition_weather_veg$earliest_nonlogging_dist_date
unknown_ignition_weather_veg$yearDisturb<-as.numeric(as.character(stringi::stri_sub(unknown_ignition_weather_veg$earliest_nonlogging_dist_date,1,4)))

unknown_ignition_weather_veg$yearsSinceDisturbance<- ifelse (unknown_ignition_weather_veg$FIRE_YEAR >= unknown_ignition_weather_veg$yearDisturb, unknown_ignition_weather_veg$FIRE_YEAR - unknown_ignition_weather_veg$yearDisturb, NA)
table(unknown_ignition_weather_veg$yearsSinceDisturbance)
...
unknown_ignition_weather_veg<- unknown_ignition_weather_veg %>%
...
table(unknown_ignition_weather_veg$dominant_conifer)

unknown_ignition_weather_veg<-unknown_ignition_weather_veg %>% mutate(conifer_pct_cover_total=rowSums(across(c(pct1:pct6)), na.rm=TRUE))

# the above calculation puts zeros in rows with NA's so Im changing them back to NA
unknown_ignition_weather_veg$conifer_pct_cover_total<- ifelse(is.na(unknown_ignition_weather_veg$pct1) & is.na(unknown_ignition_weather_veg$pct2) & is.na(unknown_ignition_weather_veg$pct3) & is.na(unknown_ignition_weather_veg$pct4) & is.na(unknown_ignition_weather_veg$pct5) & is.na(unknown_ignition_weather_veg$pct6), NA, unknown_ignition_weather_veg$conifer_pct_cover_total)
...
```{r}
unknown_ignition_weather_veg$FWI_veg<-NA

# not vegetated and logged
unknown_ignition_weather_veg1<- unknown_ignition_weather_veg %>%
...
# note the NA values in the unknown_ignition_weather_veg table were causing ifelse statements to do weird things. This seems better. Hopefully its correct.

#table(unknown_ignition_weather_veg1$FWI_veg)
...
#```{r}
# recently burned
#unknown_ignition_weather_veg1<- unknown_ignition_weather_veg1 %>%
#  mutate(FWI_veg = case_when(
...
table(is.na(unknown_ignition_weather_veg1$FWI_veg))
table(unknown_ignition_weather_veg1$FWI_veg)

unknown_ignition_weather_veg2<-st_drop_geometry(unknown_ignition_weather_veg1)

st_write(unknown_ignition_weather_veg1, "C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\unknown_ignition_weather_veg.shp")

write.csv(unknown_ignition_weather_veg2, "C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\unknown_ignition_weather_veg.csv")

unknown_ignition_weather_veg3<- left_join(unknown_ignition_weather_veg1,st_drop_geometry(x))
...
unknown_ignition_weather_veg4<-unknown_ignition_weather_veg3 %>%
  rename(fire_no = FIRE_NUMBER,
    fire_yr=FIRE_YEAR,
       IGNITIO = IGNITION_DATE,
       fire_cs=FIRE_CAUSE, 
       FIRE_LA=FIRE_LABEL,
       fir_typ=FIRE_TYPE,
       LATITUD = LATITUDE,
       LONGITU = LONGITUDE, 
       size_ha = CURRENT_SIZE,
       slope = slope_ha_bc_3005,
       aspect = aspect_ha_bc_3005,
       elevatn = dem_ha_bc,
       infr_ds = dist_infrastructure_m, 
       rd_dst_ = dist_roads_m,
    geom = geometry) %>%
       select(fire_no, fire_yr, IGNITIO, fire_cs, FIRE_LA, ZONE, FIRE_ID, fir_typ, LATITUD, LONGITU, size_ha, ig_mnth, Cluster, idno, slope, aspect, elevatn, Tmax02:MDC_11, infr_ds, rd_dst_, win_sum, win_spg, inventory_standard_cd:FWI_veg)


fire_escapes3 <- fire_escapes2  %>% select(fire_no, fire_yr, IGNITIO, fire_cs, FIRE_LA, ZONE, FIRE_ID, fir_typ, LATITUD, LONGITU, size_ha, ig_mnth, Cluster, idno, slope, aspect, elevatn, Tmax02:MDC_11, infr_ds, rd_dst_, win_sum, win_spg, inventory_standard_cd:live_stand_volume_175, yearHarvest:FWI_veg)

dim(fire_escapes3)

fire_escapes_final<- rbind(fire_escapes3, unknown_ignition_weather_veg4)

st_write(fire_escapes_final, "C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\Fire_escape_data_all.gpkg")
