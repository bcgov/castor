...
Get X andY columns before lose geometry.
```{r}
test<-cbind(fire_spread_veg_data_B, st_coordinates(fire_spread_veg_data_B))
head(test)

test<-st_coordinates(fire_spread_veg_data_B)
head(test)
```
...
table(fire_spread_veg_data_df$bclcs_level_5, fire_spread_veg_data_df$fire_yr)
...
Because files are too big to save, save as chunks so can bring back in and append later if needed.

```{r}
fire_spread_veg_data_df_set1<-fire_spread_veg_data_df[1:250000,]
fire_spread_veg_data_df_set2<-fire_spread_veg_data_df[250001:500000,]
fire_spread_veg_data_df_set3<-fire_spread_veg_data_df[500001:750000,]
fire_spread_veg_data_df_set4<-fire_spread_veg_data_df[750001:1000000,]
fire_spread_veg_data_df_set5<-fire_spread_veg_data_df[1000001:1246218,]
...
```{r}
write.csv(fire_spread_veg_data_df_set1, file = "D:\\Fire\\fire_data\\raw_data\\Historical_Fire_Perimiter_polygons\\fire_spread_veg_data_set1.csv")

write.csv(fire_spread_veg_data_df_set2, file = "D:\\Fire\\fire_data\\raw_data\\Historical_Fire_Perimiter_polygons\\fire_spread_veg_data_set2.csv")

write.csv(fire_spread_veg_data_df_set3, file = "D:\\Fire\\fire_data\\raw_data\\Historical_Fire_Perimiter_polygons\\fire_spread_veg_data_set3.csv")

write.csv(fire_spread_veg_data_df_set4, file = "D:\\Fire\\fire_data\\raw_data\\Historical_Fire_Perimiter_polygons\\fire_spread_veg_data_set4.csv")

write.csv(fire_spread_veg_data_df_set5, file = "D:\\Fire\\fire_data\\raw_data\\Historical_Fire_Perimiter_polygons\\fire_spread_veg_data_set5.csv")

```

#Now if the data gets lost from the R environment, it can be brought in in chunks and re-appended

############################### COMPLETE ######################

Move on to the 15_Fire_Spread_Polygons_Data series.

Note: As of February 13, 2022, I suggest using all NDTs together in file 15f_Fire_Spread_Polygons_Data_ALLNDTS as opposed to having a different model for each NDT.
