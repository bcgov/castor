...
(SELECT feature_id, bclcs_level_2, bclcs_level_3, bclcs_level_4, bclcs_level_5,
  ...
  (SELECT wkb_geometry, idno, fire_yr, fire_cs, fir_typ, size_ha, fire, zone,
   ...
   mdc_09, slp_h_b, aspct__ from dc_data_DEM_5x where fire_yr = '2002') as fire where st_contains
...
##############
CREATE TABLE fire_veg_2020_C AS
(SELECT feature_id, bclcs_level_2, bclcs_level_3, bclcs_level_4, bclcs_level_5,
  harvest_date,
  fire.idno, fire.fire_yr, fire.fire_cs, fire.fir_typ, fire.size_ha, fire.fire,
  fire.zone, fire.subzone, fire.ntrl_ds, fire.tmax05, fire.tmax06, fire.tmax07,
...
# live_stand_volume_125 does not exist
 ...
 CREATE TABLE fire_veg_2007_C AS
...
##############################
 CREATE TABLE fire_veg_2008_C AS
 (SELECT feature_id, bclcs_level_2, bclcs_level_3, bclcs_level_4, bclcs_level_5,
...
# (SELECT wkb_geometry from dc_data_DEM_5x_5x where fire_yr = '2008') as fire where st_contains
...
fire_veg_2003 <- sf::st_read  (dsn = conn, # connKyle
                               query = "SELECT * FROM public.fire_veg_2003_C")
fire_veg_2004 <- sf::st_read  (dsn = conn, # connKyle
                               query = "SELECT * FROM public.fire_veg_2004_C")
fire_veg_2005 <- sf::st_read  (dsn = conn, # connKyle
                               query = "SELECT * FROM public.fire_veg_2005_C")
fire_veg_2006 <- sf::st_read  (dsn = conn, # connKyle
                               query = "SELECT * FROM public.fire_veg_2006_C")
fire_veg_2007 <- sf::st_read  (dsn = conn, # connKyle
                               query = "SELECT * FROM public.fire_veg_2007_C")
fire_veg_2008 <- sf::st_read  (dsn = conn, # connKyle
                               query = "SELECT * FROM public.fire_veg_2008_C")
fire_veg_2009 <- sf::st_read  (dsn = conn, # connKyle
                               query = "SELECT * FROM public.fire_veg_2009_C")
fire_veg_2010 <- sf::st_read  (dsn = conn, # connKyle
                               query = "SELECT * FROM public.fire_veg_2010_C")
fire_veg_2011 <- sf::st_read  (dsn = conn, # connKyle
                               query = "SELECT * FROM public.fire_veg_2011_C")
fire_veg_2012 <- sf::st_read  (dsn = conn, # connKyle
                               query = "SELECT * FROM public.fire_veg_2012_C")
fire_veg_2013 <- sf::st_read  (dsn = conn, # connKyle
                               query = "SELECT * FROM public.fire_veg_2013_C")
fire_veg_2014 <- sf::st_read  (dsn = conn, # connKyle
                               query = "SELECT * FROM public.fire_veg_2014_C")
fire_veg_2015 <- sf::st_read  (dsn = conn, # connKyle
                               query = "SELECT * FROM public.fire_veg_2015_C")
fire_veg_2016 <- sf::st_read  (dsn = conn, # connKyle
                               query = "SELECT * FROM public.fire_veg_2016_C")
fire_veg_2017 <- sf::st_read  (dsn = conn, # connKyle
                               query = "SELECT * FROM public.fire_veg_2017_C")
fire_veg_2018 <- sf::st_read  (dsn = conn, # connKyle
                               query = "SELECT * FROM public.fire_veg_2018_C")
fire_veg_2019 <- sf::st_read  (dsn = conn, # connKyle
                               query = "SELECT * FROM public.fire_veg_2019_C")
fire_veg_2020 <- sf::st_read  (dsn = conn, # connKyle
                               query = "SELECT * FROM public.fire_veg_2020_C")
...
```{r}
st_write(fire_veg_data_B, dsn = "D:\\Fire\\fire_data\\raw_data\\ClimateBC_Data\\fire_ignitions_veg_climate_ALL_5x_NoW.shp", delete_layer=TRUE) #File very large > 2 GB, so having saving issues. Perhaps now is a good time to separate the file into lightning, person caused, etc.
```
...
##Lightning caused
pre<- fire_veg_data_lightning_NA_5x %>%
  filter(fire==1) %>%
...
pre_checkpre<- fire_veg_data_lightning_NA_5x %>%
  filter(fire==0) %>%
  dplyr::select(fire_yr, fire, zone, bclcs_level_2) %>%
  group_by(fire_yr, zone, bclcs_level_2) %>%
  summarize(abs_n=n())
...
abs_match <- fire_veg_data_lightning_NA_5x %>%
  filter(fire == 0) %>%
...
pre1<- fire_veg_data_lightning_NA_5x %>%
  filter(fire==1)
...
##Person caused
pre<- fire_veg_data_person_NA_5x %>%
  filter(fire==1) %>%
  dplyr::select(fire_yr, fire, zone, bclcs_level_2) %>%
  group_by(fire_yr, zone, bclcs_level_2) %>%
  summarize(fire_n=n())
...
#pre_checkpre<- fire_veg_data_person_NA_5x %>%
#  filter(fire==0) %>%
#  dplyr::select(fire_yr, fire, zone, bclcs_level_2) %>%
#  group_by(fire_yr, zone, bclcs_level_2) %>%
#  summarize(abs_n=n())
...
abs_match <- fire_veg_data_person_NA_5x %>%
  filter(fire == 0) %>%
  group_by(fire_yr, zone, bclcs_level_2) %>%   # prep for work by yr and veg type
  nest() %>%              # --> one row per yr and vegtype
  ungroup()
...
# joining my subsampled absence data back to the fire ignition presence data
pre1<- fire_veg_data_person_NA_5x %>%
  filter(fire==1)
dim(sampled_df) # 22715 rows
dim(pre1) #  15228 rows
