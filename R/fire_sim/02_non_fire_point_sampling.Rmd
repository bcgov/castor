...
##Or from local device; e.g. below
 fire.ignt.frt <-st_read(dsn="C:\\Work\\caribou\\castor\\R\\fire_sim\\tmp\\bc_fire_ignition_clipped.shp")
# 
# head(fire.ignt.frt)
...
fire.ignt.frt2 <- fire.ignt.frt %>%
  dplyr::select("id", "FIRE_NUMBER", "FIRE_YEAR", "IGNITION_DATE", "FIRE_CAUSE", "FIRE_LABEL", "FIRE_CENTRE", "ZONE", "FIRE_ID", "FIRE_TYPE", "LATITUDE", "LONGITUDE","CURRENT_SIZE","ig_mnth", "Cluster","geometry")
...
for (i in 1:length(years)) {
  ...
  colnames (all_sample_points) <- c ("id", "FIRE_NUMBER", "FIRE_YEAR", "IGNITION_DATE", "FIRE_CAUSE", "FIRE_LABEL", "FIRE_CENTRE", "ZONE", "FIRE_ID", "FIRE_TYPE", "LATITUDE", "LONGITUDE","CURRENT_SIZE", "ig_mnth", "Cluster", "fire","geometry")
  ...
  for (j in 1:length(frt_clusters)) {
    ...
    sample_size<-dim(foo_ignit_small)[1]*10 # although Ill run the models with 5x the number of ignition points here Ill sample more locations so that each vegetation type is well covered. 
...
st_write(samp_locations_sf, dsn = "C:\\Work\\caribou\\castor\\R\\fire_sim\\tmp\\ignition_samp_locations_5x.shp", delete_dsn = TRUE, overwrite = TRUE)
