...
for (i in 1:length(years)) {
  print(years[i])
  foo<- fire_igni_bec %>% filter(fire_year==years[i])
  ...
  for (j in 1:length(beczone)) {
    ...
    sample.p.trans <- st_transform(samp_joined, crs = geo.prj)
...
# when I try to run these locations in climateBC it says there are locations that are beyond the bounds. I think there are locations in the ocean between vancouver island and the mainland and I think there are causing the issue. So ill clip these things out.

forest_tenure<-getSpatialQuery("SELECT tsa_number, tsnmbrdscr, geom FROM public.forest_tenure")

forest_tenure<-st_transform(forest_tenure, 3005)
#plot(bec[, "zone"]) # check we got the whole province

tsa <-forest_tenure %>% 
  filter (tsnmbrdscr != 'Queen Charlotte Timber Supply Area') %>%
  filter(tsnmbrdscr != 'North Island Timber Supply Area') %>%
  filter(tsnmbrdscr != 'Arrowsmith Timber Supply Area') %>%
  filter(tsnmbrdscr != 'Pacific Timber Supply Area')

sample_locations<-samp_locations_sf[tsa,] # clipping out the above TSA's in the hope this solves the problem with extracting data from climateBC

for (i in 1: length(years)) {
  dat<- sample_locations %>% filter(fire_year==years[i])
  ...
  write.csv(sample.pts, file = paste0(the_dir, "\\", basename(nam1)))
}
...
ggplot() +
  geom_sf(data=sbs) +
  geom_sf(data=fire.ignition_sbs) # good they line up
