...
dat11$fwveg[dat11$fwveg=="S1"]<-"N" 
dat11<- dat11 %>% filter(!fwveg %in%  c( "W", "N"))

table(dat11$fwveg)

dat11$fwveg <- factor(dat11$fwveg, levels=c("C-3","C-1","C-2","C-5","C-7","D-1/2", "M-1/2", "O-1a/b"))
...
cor.test(dat11$climate1, dat11$dm_h_bc) # note climate1 and elevation are correlated

table(is.na(dat11$climate1))
table(is.na(dat11$dm_h_bc))
table(is.na(dat11$fwveg))
dat11b<-dat11 %>% drop_na(fwveg)

mod.frt.11<-glm(fire ~ climate1 + fwveg + dm_h_bc, data=dat11b, family=binomial(link="logit"))
summary(mod.frt.11)
Anova(mod.frt.11, type=3)

mod.frt.11b<-glm(fire ~ climate1 + dm_h_bc, data=dat11b, family=binomial(link="logit"))
summary(mod.frt.11b)
Anova(mod.frt.11b, type=3)
anova(mod.frt.11, mod.frt.11b,test="Chisq")



mod.frt.11c<-glm(fire ~ climate1 + dm_h_bc, data=dat11, family=binomial(link="logit"))
Anova(mod.frt.11c, type=3)

mod.frt.11d<-glm(fire ~ climate1, data=dat11, family=binomial(link="logit"))
Anova(mod.frt.11d, type=3)

# model diagnostic plots
binnedplot (fitted(mod.frt.11d), 
            residuals(mod.frt.11d), 
            nclass = NULL, 
...
dat11$resids<-resid(mod.frt.11d)
...
##Partial Residuals
visreg(mod.frt.11d, scale="response") # nicer plots than the ones below!
...
dat2_b<-dat11b%>% dplyr::select(fire, climate1)
