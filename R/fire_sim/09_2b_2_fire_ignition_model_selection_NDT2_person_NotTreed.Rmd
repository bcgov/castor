---
title: "09_2b_2_fire_ignition_model_selection_NDT2_person_NotTreed"
author: "Cora Skaien"
date: "12/11/2021"
...
#=================================
#  Script Name: 09_2b_2_fire_ignition_model_selection_NDT2_person_NotTreed.R
#  Script Version: 1.0
...
AIC_person_NDT2_fire_pres_NotTreed_climate<-table.glm.climate.simple

AIC_person_NDT2_fire_pres_NotTreed_summary_climate<- AIC_person_NDT2_fire_pres_NotTreed_climate %>%
  group_by(model) %>%
...
AIC_person_NDT2_fire_pres_NotTreed_summary_climate2<- AIC_person_NDT2_fire_pres_NotTreed_summary_climate %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT2_fire_pres_NotTreed_summary_climate2)
```
...
AIC_person_NDT2_fire_pres_NotTreed_topo<-table.glm.topo.simple

AIC_person_NDT2_fire_pres_NotTreed_summary_topo<- AIC_person_NDT2_fire_pres_NotTreed_topo %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic),
            meanAUC=mean(auc.valid),
            sdAUC=sd(auc.valid),
...
AIC_person_NDT2_fire_pres_NotTreed_summary_topo2<- AIC_person_NDT2_fire_pres_NotTreed_summary_topo %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT2_fire_pres_NotTreed_summary_topo2)
```
...
AIC_person_NDT2_fire_pres_NotTreed_infra<-table.glm.infra.simple

AIC_person_NDT2_fire_pres_NotTreed_summary_infra<- AIC_person_NDT2_fire_pres_NotTreed_infra %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic),
            meanAUC=mean(auc.valid),
            sdAUC=sd(auc.valid),
...
AIC_person_NDT2_fire_pres_NotTreed_summary_infra2<- AIC_person_NDT2_fire_pres_NotTreed_summary_infra %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT2_fire_pres_NotTreed_summary_infra2)
...
```{r}
NDT2_l_models_NotTreed<-rbind(AIC_person_NDT2_fire_pres_NotTreed_summary_climate2, AIC_person_NDT2_fire_pres_NotTreed_summary_topo2, AIC_person_NDT2_fire_pres_NotTreed_summary_infra2)
NDT2_l_models_NotTreed
NDT2_l_models_NotTreed$NDT<-"NDT2"

write.csv(NDT2_l_models_NotTreed, file="D:\\Fire\\fire_data\\raw_data\\NDT2_person_models_NotTreed_ignition.csv")
```
...
3. dist_mun + dist_dam + dist_nat + dist_mine + roads_km + dist_mun:roads_km + dist_dam:roads_km + dist_nat:roads_km + dist_mine:roads_km
...
```{r}
ignition_person_nt_NDT2<-subset(dat_person_nt, dat_person_nt$ntrl_ds=="NDT2")
table(ignition_person_nt_NDT2$vegtype2)

#Divide data into training and valid
prop<-0.75
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(ignition_person_nt_NDT2$fire_pres, p = prop,
                                    list = FALSE,
                                    times = 1)
  
   dat1 <- ignition_person_nt_NDT2[ trainIndex,]
   Valid <- ignition_person_nt_NDT2[-trainIndex,]

#Run model using dat1
model.NDT2<-glm(fire_pres ~ climate1 + vegtype2 + slope + aspect_cos + elevation + slope:elevation + dist_mun + dist_dam + dist_nat + dist_mine + roads_km + dist_mun:roads_km + dist_dam:roads_km + dist_nat:roads_km + dist_mine:roads_km + bclcs_level_5_2 + elevation:climate1 + bclcs_level_5_2:roads_km, family = binomial, data = dat1)

AIC(model.NDT2) #1374.13
...
model.NDT2<-glm(fire_pres ~ climate1 + vegtype2 + slope + aspect_cos + elevation + slope:elevation + dist_mun + dist_dam + dist_nat + dist_mine + roads_km + dist_mun:roads_km + dist_dam:roads_km + dist_nat:roads_km + dist_mine:roads_km + bclcs_level_5_2 + bclcs_level_5_2:roads_km, family = binomial, data = dat1)

AIC(model.NDT2) #1372.14
...
model.NDT2<-glm(fire_pres ~ climate1 + vegtype2 + slope + elevation + slope:elevation + dist_mun + dist_dam + dist_nat + dist_mine + roads_km + dist_mun:roads_km + dist_dam:roads_km + dist_nat:roads_km + dist_mine:roads_km + bclcs_level_5_2 + bclcs_level_5_2:roads_km, family = binomial, data = dat1)

AIC(model.NDT2) #1370.4
...
#Remove least significant
model.NDT2<-glm(fire_pres ~ climate1 + vegtype2 + slope + elevation + slope:elevation + dist_mun + dist_dam + dist_nat + dist_mine + roads_km + dist_mun:roads_km + dist_nat:roads_km + dist_mine:roads_km + bclcs_level_5_2 + bclcs_level_5_2:roads_km, family = binomial, data = dat1)

AIC(model.NDT2) #1370.2
...
#Remove least significant
model.NDT2<-glm(fire_pres ~ climate1 + vegtype2 + slope + elevation + slope:elevation + dist_mun + dist_nat + dist_mine + roads_km + dist_mun:roads_km + dist_nat:roads_km + dist_mine:roads_km + bclcs_level_5_2 + bclcs_level_5_2:roads_km, family = binomial, data = dat1)

AIC(model.NDT2) #1368.2
...
NDT2_ignition_nt<-ignition_person_nt_NDT2 %>% drop_na(climate1, vegtype2, dist_mun, dist_nat, dist_mine, roads_km, slope, elevation, bclcs_level_5_2)

#Run Model again with this data; but uses all data here
model.NDT2<-glm(fire_pres ~ climate1 + vegtype2 + slope + elevation + slope:elevation + dist_mun + dist_nat + dist_mine + roads_km + dist_mun:roads_km + dist_nat:roads_km + dist_mine:roads_km + bclcs_level_5_2 + bclcs_level_5_2:roads_km, family = binomial, data = NDT2_ignition_nt) 
...
NDT2_ignition_nt$resids<-resid(model.NDT2)

binnedplot (NDT2_ignition_nt$live_stand_volume_125, 
            NDT2_ignition_nt$resids, 
            nclass = NULL, 
            xlab = "live stand volume", 
            ylab = "Average residual", 
            main = paste("Binned Residual Plot - glm", i))

binnedplot (NDT2_ignition_nt$climate1, 
            NDT2_ignition_nt$resids, 
            nclass = NULL, 
            xlab = "", 
            ylab = "Average residual", 
...
#Partial Residuals
library(visreg)
#climate1 + vegtype2 + slope + elevation + slope:elevation + dist_mun + dist_nat + dist_mine + roads_km + dist_mun:roads_km + dist_nat:roads_km + dist_mine:roads_km + bclcs_level_5_2 + bclcs_level_5_2:roads_km
...
#dist_mun + dist_nat + dist_mine + roads_km + dist_mun:roads_km + dist_nat:roads_km + dist_mine:roads_km + bclcs_level_5_2 + bclcs_level_5_2:roads_km
visreg(model.NDT2, "dist_mun", by="roads_km")
visreg(model.NDT2, "dist_nat", by="roads_km")
visreg(model.NDT2, "dist_mine", by="roads_km")

visreg(model.NDT2, "roads_km", by="dist_mun")
visreg(model.NDT2, "roads_km", by="dist_nat")
visreg(model.NDT2, "roads_km", by="dist_mine")
visreg(model.NDT2, "roads_km", by="bclcs_level_5_2")
visreg(model.NDT2, "roads_km", by="bclcs_level_5_2", overlay=TRUE)

visreg(model.NDT2, "bclcs_level_5_2", by="roads_km")
...
top_mod_table_NDT2_person_t_ALL <- data.frame (matrix (ncol = 23, nrow = 0))
colnames (top_mod_table_NDT2_person_t_ALL ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_vegtype2OP", "coef_vegtype2S", "coef_slope", "coef_elevation", "coef_dist_mun", "coef_dist_nat", "coef_dist_mine", "coef_roads_km", "coef_bclcs_level_5_2OP", "coef_bclcs_level_5_2SP", "coef_slope:elevation", "coef_dist_mun:roads_km", "coef_dist_nat:roads_km", "coef_dist_mine:roads_km", "coef_roads_km:bclcs_level_5_2OP", "coef_roads_km:bclcs_level_5_2SP", "AUC")
```
...
prop<-0.75
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(NDT2_ignition_nt$fire_veg, p = prop,
                                    list = FALSE,
                                    times = 1)
  
   dat1 <- NDT2_ignition_nt[ trainIndex,]
   Valid <- NDT2_ignition_nt[-trainIndex,]
...
top_mod_table_NDT2_person_t <- data.frame (matrix (ncol = 23, nrow = 0))
colnames (top_mod_table_NDT2_person_t ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_vegtype2OP", "coef_vegtype2S", "coef_slope", "coef_elevation", "coef_dist_mun", "coef_dist_nat", "coef_dist_mine", "coef_roads_km", "coef_bclcs_level_5_2OP", "coef_bclcs_level_5_2SP", "coef_slope:elevation", "coef_dist_mun:roads_km", "coef_dist_nat:roads_km", "coef_dist_mine:roads_km", "coef_roads_km:bclcs_level_5_2OP", "coef_roads_km:bclcs_level_5_2SP", "AUC")

##Add data for NDT2
top_mod_table_NDT2_person_t[1,1]<-"person"
top_mod_table_NDT2_person_t[1,2]<-"NDT2"
top_mod_table_NDT2_person_t[1,3]<-"N"
top_mod_table_NDT2_person_t[1,4]<-"fire_pres ~ climate1 + vegtype2 + slope + elevation + slope:elevation + dist_mun + dist_nat + dist_mine + roads_km + dist_mun:roads_km + dist_nat:roads_km + dist_mine:roads_km + bclcs_level_5_2 + bclcs_level_5_2:roads_km" 
...
top_mod_table_NDT2_person_t[1,23]<- mod.auc
...
mean(top_mod_table_NDT2_person_t_ALL$AUC) #0.82

top_mod_table_NDT2_ignite_person_NT_means<-top_mod_table_NDT2_person_t_ALL %>% summarise_each(funs( mean( .,na.rm = TRUE)))
top_mod_table_NDT2_ignite_person_NT_means

top_mod_table_NDT2_ignite_person_NT_means[1,1]<-"Person"
top_mod_table_NDT2_ignite_person_NT_means[1,2]<-"NDT2"
top_mod_table_NDT2_ignite_person_NT_means[1,3]<-"Not Treed"
top_mod_table_NDT2_ignite_person_NT_means[1,4]<- "fire_pres ~ climate1 + vegtype2 + slope + elevation + slope:elevation + dist_mun + dist_nat + dist_mine + roads_km + dist_mun:roads_km + dist_nat:roads_km + dist_mine:roads_km + bclcs_level_5_2 + bclcs_level_5_2:roads_km" 
top_mod_table_NDT2_ignite_person_NT_means
```
Save table.

```{r}
write.csv(top_mod_table_NDT2_ignite_person_NT_means, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT2_ignite_person_NT_means.csv")
```

Standard deviation.

```{r}
top_mod_table_NDT2_ignite_person_NT_sd<-top_mod_table_NDT2_person_t_ALL %>% summarise_each(funs( sd( .,na.rm = TRUE)))
top_mod_table_NDT2_ignite_person_NT_sd

top_mod_table_NDT2_ignite_person_NT_sd[1,1]<-"Person"
top_mod_table_NDT2_ignite_person_NT_sd[1,2]<-"NDT2"
top_mod_table_NDT2_ignite_person_NT_sd[1,3]<-"Not Treed"
top_mod_table_NDT2_ignite_person_NT_sd[1,4]<-"fire_pres ~ climate1 + vegtype2 + slope + elevation + slope:elevation + dist_mun + dist_nat + dist_mine + roads_km + dist_mun:roads_km + dist_nat:roads_km + dist_mine:roads_km + bclcs_level_5_2 + bclcs_level_5_2:roads_km" 
top_mod_table_NDT2_ignite_person_NT_sd
```
...
```{r}
write.csv(top_mod_table_NDT2_ignite_person_NT_sd, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT2_ignite_person_NT_sd.csv")
```
...
#Run model using dat1
model.NDT2<-glm(fire_pres ~ climate1 + vegtype2 + slope + aspect_cos + elevation + slope:elevation + dist_infra + roads_km + dist_infra:roads_km + elevation:climate1, family = binomial, data = dat1)

AIC(model.NDT2) #1416.4
...
#Remove least significant
model.NDT2<-glm(fire_pres ~ climate1 + vegtype2 + slope + aspect_cos + elevation + slope:elevation + dist_infra + roads_km + dist_infra:roads_km, family = binomial, data = dat1)

AIC(model.NDT2) #1414.4
...
```{r}
#Remove NAs to ensure all same data used so we can compare AICs
NDT2_ignition_nt<-ignition_person_nt_NDT2 %>% drop_na(climate1, vegtype2, dist_infra, roads_km, slope, elevation, aspect_cos)

#Run Model again with this data; but uses all data here
model.NDT2<-glm(fire_pres ~ climate1 + vegtype2 + slope + aspect_cos + elevation + slope:elevation + dist_infra + roads_km + dist_infra:roads_km, family = binomial, data = NDT2_ignition_nt) 
...
#Partial Residuals
library(visreg)
#climate1 + vegtype2 + slope + aspect_cos + elevation + slope:elevation + dist_infra + roads_km + dist_infra:roads_km
...
visreg(model.NDT2, "dist_infra", by="roads_km")
visreg(model.NDT2, "roads_km", by="dist_infra")
...
#Create a new blank table and get AUC too
top_mod_table_NDT2_person_t_ALL <- data.frame (matrix (ncol = 16, nrow = 0))
colnames (top_mod_table_NDT2_person_t_ALL ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_vegtype2OP", "coef_vegtype2S", "coef_slope", "coef_aspect_cos", "coef_elevation", "coef_dist_infra", "coef_roads_km", "coef_slope:elevation", "coef_dist_infra:roads_km", "AUC")
```
Let's run it 500 times to get good mean values.
...
# create model table (only do this once) and add the relevant data
top_mod_table_NDT2_person_t <- data.frame (matrix (ncol = 16, nrow = 0))
colnames (top_mod_table_NDT2_person_t ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_vegtype2OP", "coef_vegtype2S", "coef_slope", "coef_aspect_cos", "coef_elevation", "coef_dist_infra", "coef_roads_km", "coef_slope:elevation", "coef_dist_infra:roads_km", "AUC")

##Add data for NDT2
top_mod_table_NDT2_person_t[1,1]<-"person"
top_mod_table_NDT2_person_t[1,2]<-"NDT2"
top_mod_table_NDT2_person_t[1,3]<-"N"
top_mod_table_NDT2_person_t[1,4]<-"fire_pres ~ climate1 + vegtype2 + slope + aspect_cos + elevation + slope:elevation + dist_infra + roads_km + dist_infra:roads_km" 
top_mod_table_NDT2_person_t[1,5]<- coef(model.NDT2)[1] #Intercept
top_mod_table_NDT2_person_t[1,6]<- coef(model.NDT2)[2] #Climate variable 1
top_mod_table_NDT2_person_t[1,7]<- coef(model.NDT2)[3] #Climate variable 2
top_mod_table_NDT2_person_t[1,8]<- coef(model.NDT2)[4] #
...
top_mod_table_NDT2_person_t[1,14]<- coef(model.NDT2)[10]
top_mod_table_NDT2_person_t[1,15]<- coef(model.NDT2)[11]
top_mod_table_NDT2_person_t[1,16]<- mod.auc
...
```{r}
write.csv(top_mod_table_NDT2_person_t_ALL, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT2_person_NT_ALL_Jan.csv")
```
...
top_mod_table_NDT2_ignite_person_NT_means[1,2]<-"NDT2"
top_mod_table_NDT2_ignite_person_NT_means[1,3]<-"Not Treed"
top_mod_table_NDT2_ignite_person_NT_means[1,4]<- "fire_pres ~ climate1 + vegtype2 + slope + aspect_cos + elevation + slope:elevation + dist_infra + roads_km + dist_infra:roads_km" 
top_mod_table_NDT2_ignite_person_NT_means
```
Save table.

```{r}
write.csv(top_mod_table_NDT2_ignite_person_NT_means, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT2_ignite_person_NT_means_Jan.csv")
```
...
top_mod_table_NDT2_ignite_person_NT_sd[1,2]<-"NDT2"
top_mod_table_NDT2_ignite_person_NT_sd[1,3]<-"Not Treed"
top_mod_table_NDT2_ignite_person_NT_sd[1,4]<-"fire_pres ~ climate1 + vegtype2 + slope + aspect_cos + elevation + slope:elevation + dist_infra + roads_km + dist_infra:roads_km" 
top_mod_table_NDT2_ignite_person_NT_sd
```
...
```{r}
write.csv(top_mod_table_NDT2_ignite_person_NT_sd, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT2_ignite_person_NT_sd_Jan.csv")
```
