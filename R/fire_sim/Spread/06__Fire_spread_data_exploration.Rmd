...
frt5<-fire_spread %>% filter(frt==5 & !fwveg %in% c("N", "W"))
...
frt14$scale_climate1<-(frt14$climate1-mean(frt14$climate1))/sd(frt14$climate1)
...
model4 <- glmer (pttype ~ poly(scale_climate1,2) + scale_climate2 + scale_slope + as.factor(fwveg) + log_roads + log_infra + scale_dist_ignit + (1|fire_yr) + (1|fire_yr:fire_label),
               data=frt14,
               family = binomial (link = "logit"))

ss <- getME(model4,c("theta","fixef"))
m4 <- update(model4,start=ss,control=glmerControl(optCtrl=list(maxfun=2e4))) # BIC 164056.46 
summary(m4)
car::Anova(m4, type=3)
visreg(m4)
