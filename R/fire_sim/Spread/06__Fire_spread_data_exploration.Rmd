...
model12 <- glmer (pttype ~ scale_climate1 + scale_climate1 * veg_cat + scale_age +scale_slope + aspect_cardinal + scale_pct_conifer + scale_roads + (1|fire_label),
               data=frt10,
               family = binomial (link = "logit"))
...
visreg(model15)
ss <- getME(model15,c("theta","fixef"))
m2 <- update(model15,start=ss,control=glmerControl(optCtrl=list(maxfun=2e6)))
...
plot(predict(model16),residuals(model16))
abline(h=0,lty=2,col="grey")
lines(lowess(predict(model16),residuals(model16)),col="black",lwd=2)
...
```{r}
saveRDS(m2, "C:/Work/caribou/castor/R/fire_sim/tmp/frt10.rds")
```
