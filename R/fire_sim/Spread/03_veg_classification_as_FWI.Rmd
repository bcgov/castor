...
fire_veg_sf<- fire_veg_sf %>% dplyr::mutate(yearsSinceHarvest = year - yearHarvest)
...
fire_veg_sf$yearsSinceDisturbance<- ifelse (fire_veg_sf$year > fire_veg_sf$yearDisturb, fire_veg_sf$year - fire_veg_sf$yearDisturb, NA)
table(fire_veg_sf$yearsSinceDisturbance)
...
```{r}
fire_veg_sf$FWI_veg<-NA

#Now coding the VEGETATED areas

# recently burned
fire_all1<- fire_veg_sf %>%
...
y<-fire_all1 %>% filter(pttype==1) %>% filter(FWI_veg %in% c("N", "W")) # Note there are a whole bunch of ignition points that have FWI_veg categoriezed as N. This is a problem because N means no fuel. I tried to deal with this by buffering the points 50m then 150m 
table(y$year, y$FWI_veg)
...
table(y$FWI_veg, y$frt)

st_write(fire_all1, "C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\spread\\Fire_spread_data_all_veg.gpkg", delete_layer=TRUE, driver="GPKG")
