...
escape2_summary13<- escape2 %>%
  dplyr::filter(ig_mnth=="07") %>%
  group_by(FIRE_YEAR, Cluster) %>%
  summarize(meanPPT07=mean(PPT07),
                   no_fires = n())

ggplot() + 
  geom_line(data = escape2_summary13, mapping = aes(x = FIRE_YEAR, y = meanPPT07))+
  geom_line(data = escape2_summary13, aes(x=FIRE_YEAR, y=no_fires/20, col='red'))+
  facet_grid(~Cluster) +
  scale_x_continuous(breaks = seq(from = 2001, to = 2022, by =10 ))+
  scale_y_continuous( name = expression("Average July Tmax (black)"),
...
dat_0<-dat2 %>% filter(CURRENT_SIZE<=1.5) 
dat_0$escaped<-0
dt_0 = sort(sample(nrow(dat_0), nrow(dat_0)*prop))
train_0<-dat_0[dt_0,]
test_0<-dat_0[-dt_0,]

dat_1<-dat2 %>% filter(CURRENT_SIZE>1.5)
dat_1$escaped<-1
...
```{r}
write.csv(aic_bec_lightning_summary2, file="C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\tmp\\escape_greaterThan1.5_climate_AIC_results.csv")
```
