...
To do this I can just regress the probability of escape on the probability of spread at the same locations i.e. locations where ignition occured. 
...
ignit_pts <- rasterToPoints(spread_crop, spatial = TRUE)
...
ggplot() +
 geom_raster(data = ignit_pts_df , aes(x = x, y = y, fill = rast_spread_400m_2)) +
...
ggplot() +               
  geom_density(data = x2, aes(x=CURRENT_SIZE), 
             fill = "dark green")+
  geom_density(data = x2, aes(x=size*16), 
             fill = "blue")+
  labs(x = "X-Data", y = "Y-Data")+
  ggtitle("Combined Plot")

x3<-cbind(x2$start_points, x2$CURRENT_SIZE)

p3 <- ggplot(x2, aes(x=CURRENT_SIZE)) + 
  geom_density() +
  geom_density(data=ignit_subset_10$CURRENT_SIZE, fill="purple")

p4<-ggplot(ignit_subset_10, aes(x=CURRENT_SIZE)) +
  geom_density()

p3 + p4
# Add mean line
p3 + geom_vline(aes(xintercept=(ignit_subset_10$CURRENT_SIZE)), color="blue", linetype="dashed", size=1)

hist(x$x) # gives distribution of fire sizes
...
ggplot() +
 geom_raster(data = escape_pts_df , aes(x = x, y = y, fill = rast_escape_400m)) +
  geom_point(data = pointCoordinates, aes(x = test.X, y = test.Y))
#####
```
...
out[[i]]  <- spreadWithEscape(a, start = ignition.pts, escapeProb = escape.ras , spreadProb = spread.ras)
