...
message("Downloading climate data from climateBC ...")  
    ...
    gcm_ts <- gcm_ts_input(dbCon, bbox = thebb, gcm = (P(sim, "gcm", "fireCastor")), # c("ACCESS-ESM1-5", "EC-Earth3", "MPI-ESM1-2-HR"), 
 ...
 pts<-as.data.frame(mySim$pts[,c("x", "y", "pixelid")])
  ...
  clim_dat<-df_list %>% reduce(full_join, by='pixelid')
...
distProcess <- function(sim) {
  ...
  if (suppliedElsewhere(sim$probFireRasts)) {
    ...
    print(dim(probfire))
   print(sim$probFireRasts$prob_ignition_spread)
