...
defineModule(sim, list(
  ...
  reqdPkgs = list("here","data.table", "raster", "SpaDES.tools", "tidyr", "climRdev", "pool", "ClimateNAr"),
  parameters = rbind(
...
doEvent.fireCastor = function(sim, eventTime, eventType, debug = FALSE){
  switch(
    ...
    init = {
      ...
      sim <- scheduleEvent(sim, time(sim), "fireCastor", "determineFuelTypes", 11)
      sim <- scheduleEvent(sim, time(sim), "fireCastor", "calcProbabilityFire", 12)
...
getVegVariables = {
  if(nrow(dbGetQuery(sim$castordb, "SELECT * FROM sqlite_master WHERE type = 'table' and name ='fueltype'")) == 0){
        message('need to creating fueltypes table')
...
getClimateVariables <- function(sim) {
 ...
 #write the points to file so that we can use Tongli's Climate BC program to grab climate for our year of interest
  ...
  if (time(sim)*sim$updateInterval + P(sim, "simStartYear", "fireCastor") < 2022){
  ...
  outFile = paste0(here::here(), '\\R\\SpaDES-modules\\fireCastor\\outputs\\', scenario$name, time(sim)*sim$updateInterval + P(sim, "simStartYear", "fireCastor"),'_M.csv')
  outputFile = gsub("/", "\\\\",outFile)
  
  #system2(exe,args= c('/M', paste0("/",period), paste0("/",inputFile), paste0("/",outputFile)))
  } else {
    period = paste0(P(sim, "gcm", "fireCastor"), '@', time(sim)*sim$updateInterval + P(sim, "simStartYear", "fireCastor"), '.gcm') 
    inFile =  paste0(here::here(), "\\R\\SpaDES-modules\\fireCastor\\inputs\\", scenario$name,"_sample_pts.csv")
    inputFile = gsub("/","\\\\", inFile)
  ...
  climate<-read.csv(outputFile)
    ...
    outputFile = paste0(here::here(), '/R/SpaDES-modules/fireCastor/outputs/',"Year_test_", time(sim)*sim$updateInterval + P(sim, "simStartYear", "fireCastor"),'_M.csv')
  ...
  message("calculate monthly drought code")
    ...
    for (j in 1 : length(Lf)) {
      ...
      climate2$MDC_01<-15 # the MDC value for Feb This assumes that the ground is saturated at the start of the season. Maybe not true for all locations... may need to think about this a little more.
...
#    model_coef_table_spread<-read.csv("C:\\Work\\caribou\\castor\\R\\fire_sim\\Analysis_results\\BC\\Coefficient_tables\\top_mod_table_frt15_spread.csv")
  ...
  print("sim$probFireRasts[!is.na(prob_ignition_spread),]")
...
distProcess <- function(sim) {
  ...
  if (suppliedElsewhere(sim$probFireRasts)) {
    ...
    print(dim(probfire))
   print(!is.na(sim$probFireRasts$prob_ignition_spread))
    ...
    #print(plot(sim$spreadRas))
    
    #for (i in 1:numberFireReps) {
    message("get fire ignition locations")
    
    no_ignitions<-(round(rgamma(1, shape=sim$fit_g$estimate[1], rate=sim$fit_g$estimate[2])))#*sim$updateInterval
    message("Number of ignitions:")
    print(no_ignitions)#*sim$updateInterval)
    

    no_ignitions<-ifelse(no_ignitions#*sim$updateInterval 
                         < (sim$min_ignit#*sim$updateInterval
                            ), (sim$min_ignit#*sim$updateInterval
                                ), no_ignitions#*sim$updateInterval
                         )
    
    no_starts_sample<-sim$max_ignit*2#*sim$updateInterval
    
   # print(sim$max_ignit*sim$updateInterval*2)
    ...
    starts<-sample(sim$probFireRasts$pixelid, no_starts_sample, replace=FALSE)
    ...
    message("updating fueltype table")
    ...
    dbBegin(sim$castordb)
    rs<-dbSendQuery(sim$castordb,qry , out[, "pixels"])
    dbClearResult(rs)
    dbCommit(sim$castordb)
