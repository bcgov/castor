...
fire.igni.frt<- st_intersection(fire.ignition_sf, frt_sf) # ST_Intersects is a function that takes two geometries and returns true if any part of those geometries is shared between the 2
...
table(fire.igni.frt$fire_year)
```

Write fire.igni.frt to file because it takes so long to make.

```{r}
st_write(fire.igni.frt, overwrite = TRUE,  dsn="C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\fire_ignition_hist\\fire_ignit_by_frt.shp", delete_dsn = TRUE)
...
fire_igni_frt<- st_read (dsn = conn, 
          layer = c ("public", "fire_ignit_by_frt"))
dbDisconnect (conn)

##Or from local device; e.g. below
# fire_igni_bec <-st.read(dsn="C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\fire_ignition_hist\\fire_ignit_by_frt.shp")
...
st_crs(fire_igni_frt1) #Retrieve coordinate reference system to check
fire_igni_frt2<-as.data.frame(fire_igni_frt1)
...
fire_igni_frt_new<-fire.igni.frt %>% dplyr::select(ogc_fid: fire_type, size_ha: wkb_geometry)
...
for (i in 1:length(years)) {
  print(years[i])
  foo<- fire_igni_frt_new %>% filter(fire_year==years[i])
  foo_ignit_sf<- st_as_sf(foo)
  
  all_sample_points <- data.frame (matrix (ncol = 13, nrow = 0)) # add 'data' to the points
  colnames (all_sample_points) <- c ("ogc_fid", "fire_no", "fire_year", "ign_date", "fire_cause", "fire_id", "fire_type", "latitude", "longitude","size_ha", "Cluster", "fire", "wkb_geometry")
  ...
  for (j in 1:length(frt_clusters)) {
    ...
    foo.ignit.buffered<-foo.ignit.buffered %>% 
      dplyr::select(ogc_fid, fire_no, fire_id, Cluster, wkb_geometry)
    ...
    samp_joined_new<- samp_joined %>% 
      ...
      dplyr::select(ogc_fid, fire_no, fire_year, ign_date, fire_cause, fire_id, fire_type, latitude, longitude, size_ha, Cluster, wkb_geometry, fire)
...
table(samp_locations_sf$Cluster)
...
st_write(samp_locations_sf, dsn = "D:\\Fire\\fire_data\\raw_data\\ClimateBC_Data\\samp_locations_fire_all_5x_frt.shp", delete_dsn = TRUE, overwrite = TRUE)
...
```{r}
samp_locations_sf <-st_read(dsn = "D:\\Fire\\fire_data\\raw_data\\ClimateBC_Data\\samp_locations_fire_all_5x_frt.shp")
head(samp_locations_sf) #Unfortunately these are not the most updated points... where did they go?
...
file.list1<-list.files("D:\\Fire\\fire_data\\raw_data\\ClimateBC_Data\\output_frt", pattern="sampled.points.all", all.files=FALSE, full.names=FALSE)
y1<-gsub(".csv","",file.list1)
the_dir <- "D:\\Fire\\fire_data\\raw_data\\ClimateBC_Data\\output_frt"
...
You may wish to collect more months data for calculations later.

```{r}
climate_data<-st_read(dsn = "D:\\Fire\\fire_data\\raw_data\\ClimateBC_Data\\DC_data_5x.shp")
head(climate_data)
str(climate_data)
samp_locations_sf2<-samp_locations_sf[c(1,32,33,35)]
...
for (i in 1: length(y1)){
  ...
  x<-eval(as.name(y1[i])) %>% 
    rename(YEAR=ID2) %>%
    dplyr::select(ID1, YEAR,Latitude, Longitude, Tmax01:Tmax12, Tave01:Tave12, PPT01:PPT12)
...
DC.ignitions1<- DC.ignitions %>% rename(idno=ID1,
                                        fire_year=YEAR) 
climate_data$idno <- as.factor(as.character(climate_data$idno))
climate_data$fire_yr <- as.numeric(as.character(climate_data$fire_yr))

# Now join DC.ignitions back with the original fire ignition dataset
ignition_weather<-left_join(DC.ignitions1, climate_data)
head(ignition_weather) #Lat -Longs match
dim(ignition_weather) 
st_crs(ignition_weather) #Answer NA
head(ignition_weather) #Note, there are 2 Lat/Long columns: ensure that they are the same; otherwise, you may be using the incorrect climate csvs that were manually created.
...
#Ensure you check the Lat and long between the different columns; if they differ, then something has gone wrong.
...
st_write (obj = ignition_weather_crs, 
          dsn = conn, 
          layer = c ("public", "DC_data_ALL_5x_frt"))
...
st_write(ignition_weather_crs, dsn = "D:\\Fire\\fire_data\\raw_data\\ClimateBC_Data\\DC_data_5x_AllMonths_frt.shp", delete_layer=TRUE)
