---
title: "09_2d_fire_ignition_model_selection_NDT4_person_treed"
author: "Cora Skaien"
date: "08/11/2021"
...
#=================================
#  Script Name: 09_2d_fire_ignition_model_selection_NDT4_person_treed.R
#  Script Version: 1.0
...
AIC_person_NDT4_fire_pres_treed_climate<-table.glm.climate.simple

AIC_person_NDT4_fire_pres_treed_summary_climate<- AIC_person_NDT4_fire_pres_treed_climate %>%
  group_by(model) %>%
...
AIC_person_NDT4_fire_pres_treed_summary_climate2<- AIC_person_NDT4_fire_pres_treed_summary_climate %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT4_fire_pres_treed_summary_climate2)
```
...
AIC_person_NDT4_fire_pres_treed_VRI<-table.glm.VRI.simple

AIC_person_NDT4_fire_pres_treed_summary_VRI<- AIC_person_NDT4_fire_pres_treed_VRI %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic),
            meanAUC=mean(auc.valid),
            sdAUC=sd(auc.valid),
...
AIC_person_NDT4_fire_pres_treed_summary_VRI2<- AIC_person_NDT4_fire_pres_treed_summary_VRI %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT4_fire_pres_treed_summary_VRI2)
```
...
AIC_person_NDT4_fire_pres_treed_topo<-table.glm.topo.simple

AIC_person_NDT4_fire_pres_treed_summary_topo<- AIC_person_NDT4_fire_pres_treed_topo %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic),
            meanAUC=mean(auc.valid),
            sdAUC=sd(auc.valid),
...
AIC_person_NDT4_fire_pres_treed_summary_topo2<- AIC_person_NDT4_fire_pres_treed_summary_topo %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT4_fire_pres_treed_summary_topo2)
```
...
AIC_person_NDT4_fire_pres_treed_infra<-table.glm.infra.simple

AIC_person_NDT4_fire_pres_treed_summary_infra<- AIC_person_NDT4_fire_pres_treed_infra %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic),
            meanAUC=mean(auc.valid),
            sdAUC=sd(auc.valid),
...
AIC_person_NDT4_fire_pres_treed_summary_infra2<- AIC_person_NDT4_fire_pres_treed_summary_infra %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT4_fire_pres_treed_summary_infra2)
...
```{r}
NDT4_l_models_treed<-rbind(AIC_person_NDT4_fire_pres_treed_summary_climate2, AIC_person_NDT4_fire_pres_treed_summary_VRI2, AIC_person_NDT4_fire_pres_treed_summary_topo2, AIC_person_NDT4_fire_pres_treed_summary_infra2)
NDT4_l_models_treed
NDT4_l_models_treed$NDT<-"NDT4"

write.csv(NDT4_l_models_treed, file="D:\\Fire\\fire_data\\raw_data\\NDT4_person_models_treed_ignition.csv")
```
...
4. dist_mun + dist_dam + dist_nat + dist_pow + dist_mine + roads_km + dist_mun:roads_km + dist_dam:roads_km
...
```{r}
ignition_person_t_NDT4<-subset(dat_person_t, dat_person_t$ntrl_ds=="NDT4")

#Divide data into training and valid
prop<-0.75
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(ignition_person_t_NDT4$fire_pres, p = prop,
                                    list = FALSE,
                                    times = 1)
  
   dat1 <- ignition_person_t_NDT4[ trainIndex,]
   Valid <- ignition_person_t_NDT4[-trainIndex,]

#Run model using dat1
model.NDT4<-glm(fire_pres ~ climate1 + climate2 + vegtype2 + climate1:climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + aspect_cos:elevation + dist_mun + dist_dam + dist_nat + dist_pow + dist_mine + roads_km + dist_mun:roads_km + dist_dam:roads_km + elevation*climate1 + elevation*climate2 + bclcs_level_5_2 + bclcs_level_5_2:roads_km, family = binomial, data = dat1)

AIC(model.NDT4) #11217.6
...
model.NDT4<-glm(fire_pres ~ climate1 + climate2 + vegtype2 + climate1:climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + aspect_cos:elevation + dist_mun + dist_dam + dist_nat + dist_pow + dist_mine + roads_km + dist_mun:roads_km + elevation*climate1 + elevation*climate2 + bclcs_level_5_2 + bclcs_level_5_2:roads_km, family = binomial, data = dat1)

AIC(model.NDT4) #11215.9
...
model.NDT4<-glm(fire_pres ~ climate1 + climate2 + vegtype2 + climate1:climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + aspect_cos:elevation + dist_mun + dist_nat + dist_pow + dist_mine + roads_km + dist_mun:roads_km + elevation*climate1 + elevation*climate2 + bclcs_level_5_2, family = binomial, data = dat1)

AIC(model.NDT4) #11211.3
...
#Remove least significant
model.NDT4<-glm(fire_pres ~ climate1 + climate2 + vegtype2 + climate1:climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + dist_mun + dist_nat + dist_mine + roads_km + dist_mun:roads_km + elevation*climate1 + elevation*climate2 + bclcs_level_5_2, family = binomial, data = dat1)

AIC(model.NDT4) #11209.8
...
#Remove least significant
model.NDT4<-glm(fire_pres ~ climate1 + climate2 + climate1:climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + dist_mun + dist_nat + dist_mine + roads_km + dist_mun:roads_km + elevation*climate1 + elevation*climate2 + bclcs_level_5_2, family = binomial, data = dat1)

AIC(model.NDT4) #11208.2
...
NDT4_ignition_t<-ignition_person_t_NDT4 %>% drop_na(climate1, climate2, live_stand_volume_125, proj_height_1, proj_age_1, dist_mun, dist_nat, dist_mine, roads_km, bclcs_level_5_2, slope, elevation, aspect_cos)

#Run Model again with this data; but uses all data here
model.NDT4<-glm(fire_pres ~ climate1 + climate2 + climate1:climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + dist_mun + dist_nat + dist_mine + roads_km + dist_mun:roads_km + elevation*climate1 + elevation*climate2 + bclcs_level_5_2, family = binomial, data = NDT4_ignition_t) 
...
NDT4_ignition_t$resids<-resid(model.NDT4)

binnedplot (NDT4_ignition_t$climate1, 
            NDT4_ignition_t$resids, 
            nclass = NULL, 
            xlab = "", 
            ylab = "Average residual", 
            main = paste("Binned Residual Plot - glm", i))
...
#Partial Residuals
#climate1 + climate2 + climate1:climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + dist_mun + dist_nat + dist_mine + roads_km + dist_mun:roads_km + elevation*climate1 + elevation*climate2 + bclcs_level_5_2
...
visreg(model.NDT4, "proj_height_1")
visreg(model.NDT4, "proj_age_1")
visreg(model.NDT4, "live_stand_volume_125")

visreg(model.NDT4, "slope", by="elevation")
visreg(model.NDT4, "slope", by="aspect_cos")
visreg(model.NDT4, "aspect_cos", by="slope")
...
top_mod_table_NDT4_person_t_ALL <- data.frame (matrix (ncol = 27, nrow = 0))
colnames (top_mod_table_NDT4_person_t_ALL ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_climate_2", "coef_proj_height_1", "coef_proj_age_1", "coef_live_stand_volume_125", "coef_slope", "coef_aspect_cos", "coef_elevation", "coef_dist_mun", "coef_dist_nat", "coef_dist_mine", "coef_roads_km", "coef_bclcs_level_5_2DE", "coef_bclcs_level_5_2OP", "coef_bclcs_level_5_2SP", "coef_climate1:climate2", "coef_slope:aspect_cos", "coef_slope:elevation", "coef_dist_mun:roads_km", "coef_climate1:elevation", "coef_climate2:elevation", "AUC")
```
...
prop<-0.75
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(NDT4_ignition_t$fire_veg, p = prop,
                                    list = FALSE,
                                    times = 1)
  
   dat1 <- NDT4_ignition_t[ trainIndex,]
   Valid <- NDT4_ignition_t[-trainIndex,]
...
top_mod_table_NDT4_person_t <- data.frame (matrix (ncol = 27, nrow = 0))
...
top_mod_table_NDT4_person_t[1,4]<-"fire_pres ~ climate1 + climate2 + climate1:climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + dist_mun + dist_nat + dist_mine + roads_km + dist_mun:roads_km + elevation*climate1 + elevation*climate2 + bclcs_level_5_2" 
...
top_mod_table_NDT4_person_t[1,26]<- coef(model.NDT4)[22]
top_mod_table_NDT4_person_t[1,27]<- mod.auc
...
top_mod_table_NDT4_ignite_person_means<-top_mod_table_NDT4_person_t_ALL %>% summarise_each(funs( mean( .,na.rm = TRUE)))
top_mod_table_NDT4_ignite_person_means

top_mod_table_NDT4_ignite_person_means[1,1]<-"person"
top_mod_table_NDT4_ignite_person_means[1,2]<-"NDT4"
top_mod_table_NDT4_ignite_person_means[1,3]<-"Treed"
top_mod_table_NDT4_ignite_person_means[1,4]<- "fire_pres ~ climate1 + climate2 + climate1:climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + dist_mun + dist_nat + dist_mine + roads_km + dist_mun:roads_km + elevation*climate1 + elevation*climate2 + bclcs_level_5_2" 
top_mod_table_NDT4_ignite_person_means
```
Save table.

```{r}
write.csv(top_mod_table_NDT4_ignite_person_means, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT4_ignite_person_means.csv")
```

Standard deviation.

```{r}
top_mod_table_NDT4_ignite_person_sd<-top_mod_table_NDT4_person_t_ALL %>% summarise_each(funs( sd( .,na.rm = TRUE)))
top_mod_table_NDT4_ignite_person_sd

top_mod_table_NDT4_ignite_person_sd[1,1]<-"person"
top_mod_table_NDT4_ignite_person_sd[1,2]<-"NDT4"
top_mod_table_NDT4_ignite_person_sd[1,3]<-"Treed"
top_mod_table_NDT4_ignite_person_sd[1,4]<-"fire_pres ~ climate1 + climate2 + climate1:climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + dist_mun + dist_nat + dist_mine + roads_km + dist_mun:roads_km + elevation*climate1 + elevation*climate2 + bclcs_level_5_2" 
top_mod_table_NDT4_ignite_person_sd
```
...
```{r}
write.csv(top_mod_table_NDT4_ignite_person_sd, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT4_ignite_person_sd.csv")
```
...
#Run model using dat1
model.NDT4<-glm(fire_pres ~ climate1 + climate2 + vegtype2 + climate1:climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + aspect_cos:elevation + dist_infra + roads_km + dist_infra:roads_km + elevation*climate1 + elevation*climate2, family = binomial, data = dat1)

AIC(model.NDT4) #12361.9
...
#Remove least significant interaction
model.NDT4<-glm(fire_pres ~ climate1 + climate2 + vegtype2 + climate1:climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + dist_infra + roads_km + dist_infra:roads_km + elevation*climate1 + elevation*climate2, family = binomial, data = dat1)

AIC(model.NDT4) #12360.2
...
#Remove least significant interaction
model.NDT4<-glm(fire_pres ~ climate1 + climate2 + vegtype2 + climate1:climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:elevation + dist_infra + roads_km + dist_infra:roads_km + elevation*climate1 + elevation*climate2, family = binomial, data = dat1)

AIC(model.NDT4) #12359.8
...
```{r}
#Remove NAs to ensure all same data used so we can compare AICs
NDT4_ignition_t<-ignition_person_t_NDT4 %>% drop_na(climate1, climate2, live_stand_volume_125, proj_height_1, proj_age_1, dist_infra, roads_km, slope, elevation, aspect_cos)

table(NDT4_ignition_t$vegtype2) #9 open

#Run Model again with this data; but uses all data here
model.NDT4<-glm(fire_pres ~ climate1 + climate2 + vegtype2 + climate1:climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:elevation + dist_infra + roads_km + dist_infra:roads_km + elevation*climate1 + elevation*climate2, family = binomial, data = NDT4_ignition_t) 
...
#Partial Residuals
#climate1 + climate2 + vegtype2 + climate1:climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:elevation + dist_infra + roads_km + dist_infra:roads_km + elevation*climate1 + elevation*climate2
...
# create model table (only do this once) and add the relevant data
top_mod_table_NDT4_person_t <- data.frame (matrix (ncol = 25, nrow = 0))
colnames (top_mod_table_NDT4_person_t ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_climate_2", "coef_vegtype2OP", "coef_vegtype2TB", "coef_vegtype2TC", "coef_vegtype2TM", "coef_proj_height_1", "coef_proj_age_1", "coef_live_stand_volume_125", "coef_slope", "coef_aspect_cos", "coef_elevation", "coef_dist_infra", "coef_roads_km", "coef_climate1:climate2",  "coef_slope:elevation", "coef_dist_infra:roads_km", "coef_climate1:elevation", "coef_climate2:elevation", "AUC")
...
top_mod_table_NDT4_person_t[1,2]<-"NDT4"
top_mod_table_NDT4_person_t[1,3]<-"Y"
top_mod_table_NDT4_person_t[1,4]<-"fire_pres ~ climate1 + climate2 + vegtype2 + climate1:climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:elevation + dist_infra + roads_km + dist_infra:roads_km + elevation*climate1 + elevation*climate2" 
top_mod_table_NDT4_person_t[1,5]<- coef(model.NDT4)[1] #Intercept
top_mod_table_NDT4_person_t[1,6]<- coef(model.NDT4)[2] #Climate variable 1
top_mod_table_NDT4_person_t[1,7]<- coef(model.NDT4)[3] #Climate variable 2
top_mod_table_NDT4_person_t[1,8]<- coef(model.NDT4)[4] #
...
top_mod_table_NDT4_person_t[1,23]<- coef(model.NDT4)[19]
top_mod_table_NDT4_person_t[1,24]<- coef(model.NDT4)[20]
top_mod_table_NDT4_person_t[1,25]<- mod.auc
...
top_mod_table_NDT4_ignite_person_means[1,2]<-"NDT4"
top_mod_table_NDT4_ignite_person_means[1,3]<-"Treed"
top_mod_table_NDT4_ignite_person_means[1,4]<- "fire_pres ~ climate1 + climate2 + vegtype2 + climate1:climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:elevation + dist_infra + roads_km + dist_infra:roads_km + elevation*climate1 + elevation*climate2" 
top_mod_table_NDT4_ignite_person_means
```
Save table.

```{r}
write.csv(top_mod_table_NDT4_ignite_person_means, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT4_ignite_person_means_Jan.csv")
```
...
top_mod_table_NDT4_ignite_person_sd[1,2]<-"NDT4"
top_mod_table_NDT4_ignite_person_sd[1,3]<-"Treed"
top_mod_table_NDT4_ignite_person_sd[1,4]<-"fire_pres ~ climate1 + climate2 + vegtype2 + climate1:climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:elevation + dist_infra + roads_km + dist_infra:roads_km + elevation*climate1 + elevation*climate2" 
top_mod_table_NDT4_ignite_person_sd
```
...
```{r}
write.csv(top_mod_table_NDT4_ignite_person_sd, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT4_ignite_person_sd_Jan.csv")
```
