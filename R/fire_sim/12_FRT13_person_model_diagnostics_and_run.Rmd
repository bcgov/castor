...
dat13$FWI_veg[dat13$FWI_veg=="C-1"]<-"C-3" # C_1 and C-3 are apparently similar in spread rates so will change C-1 to C-3
dat13<- dat13 %>% filter(!FWI_veg %in%  c("N", "W"))
...
ggplot(data=dat13, mapping=aes(x=as.factor(fire), y=log(rd_dst_+1)))+
  geom_boxplot()

ggplot(data=dat13, mapping=aes(x=as.factor(fire), y=log(infr_ds+1)))+
  geom_boxplot()



mod_frt_13a<- glm(fire ~ climate1 + climate2 + elevatn + FWI_veg + rd_dst_ + infr_ds, data=dat13, family=binomial(link="logit"))

mod_frt13b<- glm(fire ~ climate1 + climate2 + elevatn + FWI_veg + rd_dst_, data=dat13, family=binomial(link="logit"))
anova(mod_frt_13a,mod_frt13b, test="Chisq") 

mod_frt13c<- glm(fire ~ climate1 + climate2 + elevatn + FWI_veg + infr_ds, data=dat13, family=binomial(link="logit"))
anova(mod_frt_13a,mod_frt13c, test="Chisq") 

mod_frt_13d<- glm(fire ~ climate1 + climate2 + FWI_veg + rd_dst_ + infr_ds, data=dat13, family=binomial(link="logit"))
anova(mod_frt_13a,mod_frt_13d, test="Chisq") 

mod_frt_13a_log<- glm(fire ~ climate1 + elevatn + FWI_veg + log(rd_dst_+1) + log(infr_ds+1), data=dat13, family=binomial(link="logit"))

# best model is mod_frt_5a_log. It has the lowest AIC 

dat13$log_road_dist<-log(dat13$rd_dst_+1)
dat13$log_inf_dist<-log(dat13$infr_ds+1)
...
colnames (top_mod_table_FRT5_person_All ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1", "coef_climate_2", "coef_elevatn", "coef_log_road_dist", "coef_log_inf_dist", "coef_FWI_veg_C-3", "coef_FWI_veg_C-5", "coef_FWI_veg_C-7", "coef_FWI_veg_D-1/2", "coef_FWI_veg_M-1/2", "coef_FWI_veg_M-3","coef_FWI_veg_O-1a/b", "coef_FWI_veg_S-1", "coef_FWI_veg_S-2", "coef_FWI_veg_S-3",  "AUC")
...
colnames (top_mod_table_FRT5_person ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1", "coef_climate_2", "coef_elevatn", "coef_log_road_dist", "coef_log_inf_dist", "coef_FWI_veg_C-3", "coef_FWI_veg_C-5", "coef_FWI_veg_C-7", "coef_FWI_veg_D-1/2", "coef_FWI_veg_M-1/2", "coef_FWI_veg_M-3","coef_FWI_veg_O-1a/b", "coef_FWI_veg_S-1", "coef_FWI_veg_S-2", "coef_FWI_veg_S-3",  "AUC")
