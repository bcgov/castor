...
mod_frt_13a<- glm(fire ~ climate1 + climate2 + elevatn + FWI_veg + road_dist_m + infr_dist, data=dat13, family=binomial(link="logit"))
Anova(mod_frt_13a, type=3)

mod_frt13b<- glm(fire ~ climate1 + climate2 + elevatn + FWI_veg + road_dist_m, data=dat13, family=binomial(link="logit"))
anova(mod_frt_13a,mod_frt13b, test="Chisq") 

mod_frt13c<- glm(fire ~ climate1 + climate2 + elevatn + FWI_veg + infr_dist, data=dat13, family=binomial(link="logit"))
anova(mod_frt_13a,mod_frt13c, test="Chisq") 

mod_frt_13d<- glm(fire ~ climate1 + climate2 + FWI_veg + road_dist_m + infr_dist, data=dat13, family=binomial(link="logit"))
anova(mod_frt_13a,mod_frt_13d, test="Chisq") 

mod_frt_13a_log<- glm(fire ~ climate1 + elevatn + FWI_veg + log(road_dist_m+1) + log(infr_dist+1), data=dat13, family=binomial(link="logit"))
...
```{r}
mod_frt13<- glm(fire ~ climate1 + climate2 + elevatn + log_road_dist + log_inf_dist + FWI_veg, data=dat13_2, family=binomial(link="logit"))
...
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(dat13_2$fire_veg, p = prop,
                                    list = FALSE,
                                    times = 1)
  
   dat1 <- dat13_2[ trainIndex,]
   Valid <- dat13_2[-trainIndex,]
