...
# FRT 12
dat13<- dat_human %>% dplyr::filter(frt == 13) 
...
ggplot(data=dat13, mapping=aes(x=as.factor(fire), y=dm_h_bc))+
  geom_boxplot()

ggplot(data=dat13, mapping=aes(x=as.factor(fire), y=log(road_dist_m+1)))+
  geom_boxplot()

ggplot(data=dat13, mapping=aes(x=as.factor(fire), y=log(dist_infrastructure_m+1)))+
  geom_boxplot()

cor.test(dat13$dm_h_bc, dat13$climate1) # correlation is -0.73
cor.test(dat13$road_dist_m, dat13$dist_infrastructure_m)

mod_frt_13a<- glm(fire ~ climate1 + fwveg + road_dist_m + dist_infrastructure_m, data=dat13, family=binomial(link="logit"))
Anova(mod_frt_13a, type=3)

dat13$log_road_dist<-log(dat13$road_dist_m+1)
dat13$log_inf_dist<-log(dat13$dist_infrastructure_m+1)

mod_frt_13b<- glm(fire ~ climate1 + fwveg + log_road_dist + dist_infrastructure_m, data=dat13, family=binomial(link="logit"))

mod_frt_13c<- glm(fire ~ climate1 + fwveg + log_road_dist + log_inf_dist, data=dat13, family=binomial(link="logit"))

summary(mod_frt_13c)
Anova(mod_frt_13c, type=3)

#checking whether the model is over or under dispersed
z <- glm(fire ~ climate1 + fwveg + log_road_dist + log_inf_dist, family = quasibinomial(link="logit"), data = dat13)
summary(z)
anova(z, test = "F")
...
binnedplot (fitted(mod_frt_13c), 
            residuals(mod_frt_13c), 
            nclass = NULL, 
...
dat13$resids<-resid(mod_frt_13c)
...
##Partial Residuals
visreg(mod_frt_13c, scale="response") # nicer plots than the ones below!
...
dat2_b<-dat13 %>% dplyr::select(fire, climate1, log_road_dist, log_inf_dist)
probabilities <- predict(mod_frt_13c, type = "response")
...
plot(mod_frt_13c, which = 4, id.n = 3)
...
library(broom)
model.data <- augment(mod_frt_13c) %>% 
  mutate(index = 1:n()) 
...
```{r}
mod_frt13<- glm(fire ~ climate1 + log_road_dist + log_inf_dist + fwveg, data=dat13, family=binomial(link="logit"))
...
#Model   
mod<-glm(fire ~ climate1 + log_road_dist + log_inf_dist + fwveg, data=dat1, family=binomial(link="logit"))
...
top_mod_table_FRT5_person[1,3]<-"fire ~ climate1 + log_road_dist + log_inf_dist + fwveg"
...
FRT5_summary_table_mean$frt<-"13"
