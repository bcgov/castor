...
## Simulate
The following simulates the two fire regime models indpendantly for each year. Thus, in years where the simulation results in very large fire, the next proceeding year will still use the landcover estimates that actually occured.
```{r}
...
ggplot(data=runs.bounds) + geom_line(aes(x = year, y = nf_med),col = 'blue', lwd =1.5)+ geom_line(aes(x = year, y = nf_low), col = 'blue', linetype = "dashed")+ geom_line(aes(x = year, y = nf_up), col = 'blue', linetype = "dashed") + geom_line(data = obs.occ, aes(x=FIRE_YEAR, y = nfires), color = "red") + xlab("Year") + ylab("Count of Fires (>1 ha)")
