---
title: "fire_occurrence"
output: html_document
date: '2023-09-11'
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
source(here::here("R/functions/R_Postgres.R"))
library(data.table)
library(sf)
library(tidyverse)
library(glmmTMB)
library(DHARMa)
library(sdmTMB)
```

In this script we model the occurrence of lightning cause wildfires. This includes the number of fires and where they occur. to do this we rely on the historical incidents data reported in the data catalog of BC. The number of fires within a 10 km grid are estimated along with various predictor variables that relate to landcover and climate.The landcover data comes from the VRI using the previous year of the fire as the estimate of the amount of landcover types within the 10 km voxel.

## Data
```{r}
data.lightning<-readRDS("data.lightning.rds")
occ.model.data<-data.lightning[flam.layer >0,][!is.na(CMI3yr),][,dCMI3yr:=CMI-(CMI3yr/3)][, coast:=0][frt==15, coast:=1][is.na(dec.layer), dec.layer:=0][is.na(mixed.layer), mixed.layer:=0][is.na(open.layer), open.layer:=0][,flammable:=con + mix + dec + young + veg][flammable >0, ][, year:= as.factor(FIRE_YEAR)]
```

## Model
```{r}

l12_nb_3yr <- glmmTMB(count ~ 1 + CMI + CMI3yr, offset = log(flammable), family = nbinom2(link = "log"), ziformula = ~0, data=occ.model.data)

simulationOutput <- simulateResiduals(fittedModel = l12_nb_3yr)
plot(simulationOutput)

occ.model.data$pred_mu<-data.table(mu= predict(l12_nb_3yr, newdata = occ.model.data[, c("flam.layer","flammable", "CMI", "CMI3yr")][,FIRE_YEAR := NA][,frt := NA], type="response"))
ggplot2::ggplot(data=occ.model.data[ FIRE_YEAR %in% c( 2012, 2016, 2017, 2018, 2020, 2022)], aes(x=x, y=y, color = pred_mu)) + facet_wrap(~FIRE_YEAR) + geom_point()
occ.model.data[, sim:= rnbinom(n = 1, size = 0.251, mu =pred_mu )]
 
sim1=simulate(l12_nb_3yr, nsim = 1)
Simdat=occ.model.data[!is.na(CMI3yr),]
Simdat$count=sim1[[1]]
Simdat=transform(Simdat,  
            type="simulated")
occ.model.data$type = "observed"  
Dat=rbind(occ.model.data[!is.na(CMI3yr),], Simdat) 
ggplot(Dat[, sum(count), by = c("FIRE_YEAR", "type")],  aes(x=FIRE_YEAR, y =V1, colour=type))+geom_line()
ggplot(Dat,  aes(x=CMI, y =as.factor(count), colour=type))+geom_boxplot() + facet_grid(~type)

spde <- make_mesh(occ.model.data[,c("x", "y")], xy_cols = c("x", "y"), cutoff = 10)

m0 <- sdmTMB(data = occ.model.data, count ~ 1 + (1|year) , offset = log(occ.model.data$flammable), family = nbinom2(), time = "year",  mesh = spde, spatial = F, spatiotemporal = FALSE, reml=F)
testm0<-predict(m0)
cor(exp(testm0$est), testm0$count/testm0$flammable)

m0_gmrf <- sdmTMB(data = occ.model.data, count ~ 1 + (1|year) , offset = log(occ.model.data$flammable), family = nbinom2(), time = "year",  mesh = spde, spatial = T, spatiotemporal = FALSE, reml=F)

m1 <- sdmTMB(data = occ.model.data, count ~ 1 + CMI_MIN + (1|year) , offset = log(occ.model.data$flammable), family = nbinom2(), time = "year",  mesh = spde, spatial = T, spatiotemporal = FALSE, reml=F)


m2 <- sdmTMB(data = occ.model.data, count ~ 1 + CMI_MIN  + dCMI3yr + (1|year), offset = log(occ.model.data$flammable), family = nbinom2(), time = "year",  mesh = spde, spatial = T, spatiotemporal = FALSE)

m3 <- sdmTMB(data = occ.model.data, count ~ 1 + CMI_MIN + dCMI3yr + as.factor(frt) + (1|year), offset = log(occ.model.data$flammable), family = nbinom2(), time = "year",  mesh = spde, spatial = T, spatiotemporal = FALSE)

##Vegetation typing
m4 <- sdmTMB(data = occ.model.data, count ~ 1 + CMI_MIN + dCMI3yr + as.factor(frt) + log(con+1) + log(young+1) + log(veg+1) + log(dec+1) + log(mix+1) + (1|year), offset = log(occ.model.data$flammable), family = nbinom2(), time = "year", mesh = spde, spatial = T, spatiotemporal = FALSE)

m5 <- sdmTMB(data = occ.model.data, count ~ 1 + CMI_MIN + dCMI3yr + as.factor(frt) + log(con+1) + log(young+1) + (1|year), offset = log(occ.model.data$flammable), family = nbinom2(), time = "year", mesh = spde, spatial = T, spatiotemporal = FALSE, reml=T)


m6 <- sdmTMB(data = occ.model.data, count ~  1 + CMI_MIN + dCMI3yr + as.factor(frt) + log(con+1) + log(young+1) + (1|year), offset = log(occ.model.data$flammable), family = nbinom2(), time = "year", mesh = spde, spatial = F, spatiotemporal = FALSE)

#test99<-predict(m6)
#test99<-test99[, estres:=exp(est)]
#test99<-test99[, mu1:= exp(-13.15)]
#test99<-test99[, mu2:= mu1*(1 + exp(m6$model$par[["log_ratio_mix"]]))]
#test99<-test99[, est9:= ((1-plogis(m6$model$par[["logit_p_mix"]]))*mu1 + plogis(m6$model$par[["logit_p_mix"]])*mu2)*flammable]

AIC(m0, m1, m2, m3, m4, m5,m6)
saveRDS(m5, "C:/Users/klochhea/castor/R/fire_sim/number_of_ignitions/m5.rds")

```


## Diagnostics
```{r}
occ.model.data$resids <- residuals(m5) # randomized quantile residuals
qqnorm(occ.model.data$resids)
qqline(occ.model.data$resids)

sim1<-simulate(m5, nsim=100, re_form = NULL)
#sim1<-simulate(m4, nsim=1000)
Simdat<-occ.model.data
simdata<-rbindlist(lapply(seq(1,100,1), function(rep_sim){
  Simdat$count<-sim1[,rep_sim]
  Simdat=transform(Simdat,  
            type=paste0("simulated_rep",rep_sim))
}))

occ.model.data$type = "observed"  
Dat=rbind(occ.model.data, simdata) 
data_sim_ag<-Dat[, sum(count), by = c("FIRE_YEAR", "type", "frt")]
#data_sim_ag[type != 'observed', type := 'simulated']
ggplot()+
  geom_boxplot(data=data_sim_ag[type != 'observed' & frt %in% c(14,15,13,12,11,10)],  aes(x=as.factor(FIRE_YEAR), y =V1)) +
  geom_point(data=data_sim_ag[type == 'observed'& frt %in% c(14,15,13,12,11,10)],  aes(x=as.factor(FIRE_YEAR), y =V1, color = 'red')) +
  facet_wrap(~frt, ncol =2, scales = "free") + ylab("Number of fires (>1 ha)")+ xlab("Year")+ theme(legend.position="none")

data.count.pred<-predict(m5 )
data.count.pred$est2<-exp(data.count.pred$est)

number<-data.count.pred[, .(nfires = sum(count)), by = c('FIRE_YEAR', 'frt')]
test_fo<-merge(data.count.pred[,.(pred = sum(est2)), by = c("FIRE_YEAR", 'frt')], number, by = c("FIRE_YEAR", 'frt'))


ggplot2::ggplot(data=test_fo, aes(x = pred, y = nfires)) + geom_point() + geom_smooth(method = "lm", se = T) + geom_abline(slope=1, intercept=0, col = 'black') +
   facet_wrap(~frt, ncol =3, scales = "free")+ ylab("Observed")+ xlab("Predicted")+ theme(legend.position="none")
if(FALSE){
  
nd <- data.frame(
    dCMI3yr = seq(min(occ.model.data$dCMI3yr), 
      max(occ.model.data$dCMI3yr), length.out = 100),  
    con = mean(occ.model.data$con), CMI_MIN = mean(occ.model.data$CMI_MIN), young = mean(occ.model.data$young), frt = 14,
    year = as.factor(2020), coast =0, flammable = mean(occ.model.data$flammable), x = mean(occ.model.data$x), y = mean(occ.model.data$y))

p <- predict(m5, newdata = nd, se_fit = T, re_form = T)

ggplot(p, aes(dCMI3yr, exp(est), 
              ymin = exp(est - 1.96 * est_se), 
              ymax = exp(est + 1.96 * est_se))) +
    geom_line() + geom_ribbon(alpha = 0.4)

}
```


#### APPENDIX: Gaussian Markov Random Fields
```{r}
#Some functions to use later
# Matern correlation
cMatern <- function(h, nu, kappa) {
  ifelse(h > 0, besselK(h * kappa, nu) * (h * kappa)^nu / 
    (gamma(nu) * 2^(nu - 1)), 1)
}

#create some data such that its a single line of 250 points each 1/25 away from each other
loc <- 0:249 / 25 
#Calculate the adjacency matrix
mdist <- as.matrix(dist(loc))
d <- dim(goshawkNestingHabitat)
mdist <-as.matrix(dist(matrix(values(goshawkNestingHabitat,), d[1], d[2], byrow=TRUE)))

#parameters to show sensitivity
nu <- c(0.5, 1, 2.5, 3, 4.5)
range <- c(1,2,3,4)
params <- cbind(nu = rep(nu, length(range)), range = rep(range, each = length(nu)))

# Sample error five times - white noise gaussian standard normal ~ N(0, 1)
set.seed(123)
z <- matrix(rnorm(nrow(mdist) * 5), ncol = 5)

# Compute the correlated samples
# Scenarios (i.e., different set of parameters)
yy <- lapply(1:nrow(params), function(j) { 
  param <- c(params[j, 1], sqrt(8 * params[j, 1]) / params[j, 2], 
    params[j, 2])
  v <- cMatern(mdist, param[1], param[2])

  # fix the diagonal to avoid numerical issues
  diag(v) <- 1 + 1e-9 

  # Parameter scenario and computed sample
  return(list(params = param, y = crossprod(chol(v), z)))
})

#Conclusion: the nu parameter adds inflections, the range parameter add smoothness
```
