...
end) as fuel_type FROM veg_comp_lyr_r1_poly{year_vri} where bclcs_level_1 = 'V' AND bclcs_level_2 = 'N' AND harvest_date is null; ")))
```
...
end) as fuel_type FROM veg_comp_lyr_r1_poly{year_vri} where bclcs_level_4 is null and bclcs_level_4 in ('101', '102', '103', '104', '105', '106', '107', '108', '109', '110', 
...
end) as fuel_type FROM veg_comp_lyr_r1_poly{year_vri} where bclcs_level_1 = 'U'")))
```
...
ft_vri$dups = duplicated(ft_vri, by = key(ft_vri));
...
### Spatial
```{r, spatial}
vri<- getSpatialQuery(glue::glue("SELECT feature_id, {geom_column} FROM veg_comp_lyr_r1_poly{year_vri} "))
vri_out<-merge(vri, ft_vri_out, by = "feature_id")
st_write(vri_out, paste0("C:/Users/klochhea/castor/vri",year_vri,"_ft.gpkg"))
```
