...
data.size<-data.size[,flammable:=con + mix + dec + young + veg][flammable >0, ][!is.na(CURRENT_SIZE),][flammable >0,][,dCMI3yr:=CMI - CMI3yr]

m4<-readRDS("C:/Users/klochhea/castor/R/fire_sim/number_of_ignitions/m4.rds")

#get the random spatial effect
data.size<-predict(m4, newdata = data.size)
...
fit.model.size<-model.size[,c("CURRENT_SIZE", "size","IGNITION_DATE", "CMI", "dCMI3yr", "haz.layer", "DD18_sm", "TEMP_MAX", "FIRE_CENTRE", "nfires", "duration", "CMI05","CMI06", "CMI07", "CMI08", "con", "dec", "mix", "young", "veg", "PPT_sm", "est", "frt", "ndt")][size>0, ]
```
...
## Alternative model(s)
```{r}
n1_wei<-gamlss(formula = size ~ 1 + CMI05 + CMI06 + CMI07 + CMI08 + as.factor(frt)+ log(young +1) + log(con +1) + est + log(veg +1) + log(dec +1) + log(mix +1)+ PPT_sm + nfires + TEMP_MAX,
           mu.link = "log",
           sigma.formula = ~ 1 + CMI05 + CMI06 + CMI07 + CMI08 + as.factor(frt)+ as.factor(FIRE_CENTRE)+ log(mix +1) + log(young +1) + log(con +1) + est + log(veg +1) + log(dec +1) + PPT_sm + nfires + TEMP_MAX,
           family = WEI3(),
...
library(gamlss.mx)
set.seed(46)
nm1_wei<-gamlssMX(formula = size ~  1  + as.factor(ndt) + log(young +1) + log(con +1) + est,
           K=2,
           sigma.formula = ~ 1,
           family = list(WEI3(), WEI3()),
           data = na.omit(fit.model.size))
