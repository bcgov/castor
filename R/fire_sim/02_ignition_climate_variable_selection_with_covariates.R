...
# Loosely following the methods of Marchal et al. (2017) Ecography (https://onlinelibrary.wiley.com/doi/full/10.1111/ecog.01849) Supporting Information Appendix 1 I try to figure out which is the best climate variable or climate variables to include in my model. Initially I ran models only including climate variables (e.g. logb(p/1-p) = B0 + B1x1 or logb(p/1-p) = B0 + B1x1 + B2x2 or logb(p/1-p) = B0 + B1x1 + B2x2 + B1x1:B2x2. But some of the models turned out to be pretty junk. So now Ill run these models including the vegetation variables (vegtype, proj_age_1, live_stand_volume_125) in addition to the climate variables. I chose to do it this way because I think these are the important vegetation variables that I will include in all BEC zone models but which climate variable is likely to vary between BEC zones. So, I will run full models to get out the best climate variable instead of very simple models. Then for each model I will extract the AIC as a means for comparison. I also calculate the AUC by splitting the data into a training and validation data set. Finally I repeat the analysis calculating the AIC and AUC using traing and validation data sets 100 times taking the average of both the AIC and AUC values. These are the values that I spit out into a csv file so that I can examine which climate variable is best for each BEC zone. 
...
model_dat<- dat2 %>% dplyr::select(fire_pres, vegtype, proj_age_1, live_stand_volume_125)
...
model1 <- glm (fire_pres ~ vegtype + log(proj_age_1) + live_stand_volume_125 ,
               data=dat1,
               family = binomial (link = "logit"))

table.glm.climate.simple[1,1]<-zones[h]
table.glm.climate.simple[1,2]<-"no climate"
...
for (i in 1: length(variables)){
  ...
  model_dat<- dat2 %>% dplyr::select(fire_pres, fire_yr, vegtype, proj_age_1, live_stand_volume_125, variables[i])
  model_dat$proj_age_1<-log(model_dat$proj_age_1)
...
for (i in 1: length(variables1)){
  ...
  model_dat<- dat2 %>% dplyr::select(fire_pres, vegtype, proj_age_1, live_stand_volume_125, variables1[i], variables2[i])
  model_dat$proj_age_1<-log(model_dat$proj_age_1)
  # Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(model_dat$fire_pres, p = prop,
                                    list = FALSE,
...
for (i in 1: length(variables1)){
  ...
  model_dat<- dat2 %>% dplyr::select(fire_pres,vegtype, proj_age_1, live_stand_volume_125, variables1[i], variables2[i])
  model_dat$interaction <- model_dat[,5]*model_dat[,6]
  model_dat$proj_age_1<-log(model_dat$proj_age_1)
  # Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(model_dat$fire_pres, p = prop,
                                    list = FALSE,
