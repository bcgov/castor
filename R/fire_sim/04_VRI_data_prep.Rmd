...
(SELECT feature_id, bclcs_level_2, bclcs_level_3, bclcs_level_4, bclcs_level_5,
  ...
  (SELECT wkb_geometry, idno, fire_yr, fire_cs, fir_typ, size_ha, fire, zone,
   ...
   mdc_09, slp_h_b, aspct__ from dc_data_DEM where fire_yr = '2002') as fire where st_contains
...
(SELECT feature_id, bclcs_level_2, bclcs_level_3, bclcs_level_4, bclcs_level_5,
  ...
  (SELECT wkb_geometry, idno, fire_yr, fire_cs, fir_typ, size_ha, fire, zone,
   subzone, ntrl_ds, tmax05, tmax06, tmax07, tmax08, tmax09, tave05, tave06, tave07,
   tave08, tave09, ppt05, ppt06, ppt07, ppt08, ppt09, mdc_05, mdc_06, mdc_07, mdc_08,
   mdc_09, slp_h_b, aspct__ from dc_data_DEM where fire_yr = '2020') as fire where st_contains
...
# live_stand_volume_125 does not exist
 ...
 (SELECT feature_id, bclcs_lv_2, bclcs_lv_3, bclcs_lv_4, bclcs_lv_5,
  ...
  (SELECT wkb_geometry, idno, fire_yr, fire_cs, fir_typ, size_ha, fire, zone,
   subzone, ntrl_ds, tmax05, tmax06, tmax07, tmax08, tmax09, tave05, tave06, tave07,
   tave08, tave09, ppt05, ppt06, ppt07, ppt08, ppt09, mdc_05, mdc_06, mdc_07, mdc_08,
   mdc_09, slp_h_b, aspct__ from dc_data_DEM where fire_yr = '2007') as fire where st_contains
...
##############################
 ...
 (SELECT feature_id, bclcs_level_2, bclcs_level_3, bclcs_level_4, bclcs_level_5,
   ...
   (SELECT wkb_geometry, idno, fire_yr, fire_cs, fir_typ, size_ha, fire, zone,
    subzone, ntrl_ds, tmax05, tmax06, tmax07, tmax08, tmax09, tave05, tave06, tave07,
    tave08, tave09, ppt05, ppt06, ppt07, ppt08, ppt09, mdc_05, mdc_06, mdc_07, mdc_08,
    mdc_09, slp_h_b, aspct__ from dc_data_DEM where fire_yr = '2008') as fire where st_contains
...
ADD COLUMN live_stand_volume_125 double precision;
 ...
 (SELECT wkb_geometry from dc_data_dem where fire_yr = '2008') as fire where st_contains
...
```{r}
#st_write(fire_veg_data_B, dsn = "D:\\Fire\\fire_data\\raw_data\\ClimateBC_Data\\fire_ignitions_veg_climate_ALL.shp", delete_layer=TRUE) #File very large > 2 GB, so having saving issues. Perhaps now is a good time to separate the file into lightning, person caused, etc.
...
fire_veg_data_lightning<-ignition_pres_abs4 %>% filter(fire_cs=="Lightning")
table(fire_veg_data_lightning$fire_cs) #15228
fire_veg_data_person<-ignition_pres_abs4 %>% filter(fire_cs=="Person")
table(fire_veg_data_person$fire_cs) #21373
fire_veg_data_NA<-ignition_pres_abs4 %>% filter(fire_cs=="NA")
table(fire_veg_data_NA$fire_cs) #351829
fire_veg_data_unknown<-ignition_pres_abs4 %>% filter(fire_cs=="Unknown")
table(fire_veg_data_unknown$fire_cs) #964

fire_veg_data_lightning_NA<-rbind(fire_veg_data_lightning, fire_veg_data_NA)
table(fire_veg_data_lightning_NA$fire_cs)

fire_veg_data_person_NA<-rbind(fire_veg_data_person, fire_veg_data_NA)
table(fire_veg_data_person_NA$fire_cs)

##Write individual files
st_write(fire_veg_data_lightning, dsn = "D:\\Fire\\fire_data\\raw_data\\Fire_climate_DEM_VRI_shape_files\\fire_veg_dem_lightning.shp", delete_layer=TRUE)

st_write(fire_veg_data_person, dsn = "D:\\Fire\\fire_data\\raw_data\\Fire_climate_DEM_VRI_shape_files\\fire_veg_dem_person.shp", delete_layer=TRUE)

st_write(fire_veg_data_NA, dsn = "D:\\Fire\\fire_data\\raw_data\\Fire_climate_DEM_VRI_shape_files\\fire_veg_dem_NA.shp", delete_layer=TRUE) #Get error

##Write aggregated files for lightning and person caused
st_write(fire_veg_data_lightning_NA, dsn = "D:\\Fire\\fire_data\\raw_data\\Fire_climate_DEM_VRI_shape_files\\fire_veg_dem_lightning_NA.shp", delete_layer=TRUE) #Get error

st_write(fire_veg_data_person_NA, dsn = "D:\\Fire\\fire_data\\raw_data\\Fire_climate_DEM_VRI_shape_files\\fire_veg_dem_person_NA.shp", delete_layer=TRUE) #Get error
...
keep(fire_veg_data_B, ignition_pres_abs4, fire_veg_data_lightning, fire_veg_data_person, fire_veg_data_NA, fire_veg_data_lightning_NA, fire_veg_data_person_NA, pre, pre_checkpre) #shows you which variables will be removed

keep(fire_veg_data_B, ignition_pres_abs4, fire_veg_data_lightning, fire_veg_data_person, fire_veg_data_NA, fire_veg_data_lightning_NA, fire_veg_data_person_NA, pre, pre_checkpre, sure = TRUE) # setting sure to TRUE removes variables not in the list
...
Let's sub-sample the NAs here before saving
...
##Do separately for lightning and person caused fires


##Lightning caused
pre<- fire_veg_data_lightning_NA %>%
  filter(fire==1) %>%
  dplyr::select(fire_yr, fire, zone, bclcs_level_2) %>%
  group_by(fire_yr, zone, bclcs_level_2) %>%
  summarize(fire_n=n())

##Below code with a large number of random points may take a few hours to run
pre_checkpre<- fire_veg_data_lightning_NA %>%
  filter(fire==0) %>%
  dplyr::select(fire_yr, fire, zone, bclcs_level_2) %>%
  group_by(fire_yr, zone, bclcs_level_2) %>%
  summarize(abs_n=n())
...
abs_match <- fire_veg_data_lightning_NA %>%
  filter(fire == 0) %>%
...
pre1<- fire_veg_data_lightning_NA %>%
  filter(fire==1)
...
##Person caused
pre<- fire_veg_data_person_NA %>%
  filter(fire==1) %>%
  dplyr::select(fire_yr, fire, zone, bclcs_level_2) %>%
  group_by(fire_yr, zone, bclcs_level_2) %>%
  summarize(fire_n=n())

#See if we can use results from pre_checkpre from the lightning file for this one too since they are the same NAs. Might not work because of initial naming differences?
#pre_checkpre<- fire_veg_data_person_NA %>%
#  filter(fire==0) %>%
#  dplyr::select(fire_yr, fire, zone, bclcs_level_2) %>%
#  group_by(fire_yr, zone, bclcs_level_2) %>%
#  summarize(abs_n=n())
...
abs_match <- fire_veg_data_person_NA %>%
  filter(fire == 0) %>%
  group_by(fire_yr, zone, bclcs_level_2) %>%   # prep for work by yr and veg type
  nest() %>%              # --> one row per yr and vegtype
  ungroup()
