...
#=================================
#  Script Name: 04_Road_Density_Data_prep.R
#  Script Version: 1.0
#  Script Purpose: Determine the road density (km/km-square) for each datapoint.
...
```
```{r}
conn <- DBI::dbConnect (dbDriver ("RPostgres::Postgres()"), 
                        host = keyring::key_get('dbhost', keyring = 'postgreSQL'), 
                        dbname = keyring::key_get('dbname', keyring = 'postgreSQL'), 
                        port = '5432',
                        user = keyring::key_get('dbuser', keyring = 'postgreSQL'),
...
conn <- DBI::dbConnect (dbDriver ("RPostgreSQL::PostgreSQL("), 
                        host = keyring::key_get('dbhost', keyring = 'postgreSQL'), 
                        dbname = keyring::key_get('dbname', keyring = 'postgreSQL'), 
                        port = '5432',
                        user = keyring::key_get('dbuser', keyring = 'postgreSQL'),
...
```{r}
sample_locations_DEM <- st_read(dsn = "D:\\Fire\\fire_data\\raw_data\\ClimateBC_Data\\DC_data_DEM_5x_AllMonths.shp")
...
st_write(sample_locations_DEM_roads_2, dsn = "D:\\Fire\\fire_data\\raw_data\\ClimateBC_Data\\Data_climate_DEM_roads_.shp", delete_layer=TRUE)
