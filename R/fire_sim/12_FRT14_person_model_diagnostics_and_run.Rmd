...
# FRT 12
dat14<- dat_human %>% dplyr::filter(frt == 14) 
table(dat14$fwveg, dat14$fire)

dat14$fwveg[dat14$fwveg=="C-1"]<-"C-2" 
...
ggplot(data=dat14, mapping=aes(x=as.factor(fire), y=log(dist_infrastructure_m+1)))+
  geom_boxplot()

cor.test(dat14$dm_h_bc, dat14$climate1) # 0.88 HIGHLY CORRELATED
cor.test(dat14$road_dist_m, dat14$dist_infrastructure_m)

mod_frt_14a_log<- glm(fire ~ climate1 + fwveg + log(road_dist_m+1) + log(dist_infrastructure_m+1), data=dat14, family=binomial(link="logit"))
Anova(mod_frt_14a_log, type=3)

dat14$log_road_dist<-log(dat14$road_dist_m+1)
dat14$log_inf_dist<-log(dat14$dist_infrastructure_m+1)

mod_frt14<- glm(fire ~ climate1 + fwveg + log_road_dist + log_inf_dist, data=dat14, family=binomial(link="logit"))
...
z <- glm(fire ~ climate1 + fwveg + log_road_dist + log_inf_dist, family = quasibinomial(link="logit"), data = dat14)
summary(z)
...
```{r}
mod_frt14<- glm(fire ~ climate1 + log_road_dist + log_inf_dist + fwveg, data=dat14, family=binomial(link="logit"))
...
FRT5_summary_table_mean$veg_intercept<-"C-7"
FRT5_summary_table_mean$frt<-14
