...
dat14$FWI_veg[dat14$FWI_veg=="M-3"]<-"M-1/2" # C_1 and C-3 are apparently similar in spread rates so will change C-1 to C-3

dat14<- dat14 %>% filter(!FWI_veg %in%  c("N", "W"))
...
ggplot(data=dat14, mapping=aes(x=as.factor(fire), y=log(rd_dst_+1)))+
  geom_boxplot()

ggplot(data=dat14, mapping=aes(x=as.factor(fire), y=log(infr_ds+1)))+
  geom_boxplot()


mod_frt_14a_log<- glm(fire ~ climate1 + elevatn + FWI_veg + log(rd_dst_+1) + log(infr_ds+1), data=dat14, family=binomial(link="logit"))

mod_frt14b_log<- glm(fire ~ climate1 + elevatn + FWI_veg + log(rd_dst_+1), data=dat14, family=binomial(link="logit"))
anova(mod_frt_14a_log,mod_frt14b_log, test="Chisq") 

mod_frt_14c_log<- glm(fire ~ climate1 + elevatn + FWI_veg + log(infr_ds+1), data=dat14, family=binomial(link="logit"))
anova(mod_frt_14a_log,mod_frt_14c_log, test="Chisq") 

mod_frt_14d_log<- glm(fire ~ climate1 + FWI_veg + log(rd_dst_+1) + log(infr_ds+1), data=dat14, family=binomial(link="logit"))
anova(mod_frt_14a_log,mod_frt_14d_log, test="Chisq") 


dat14$log_road_dist<-log(dat14$rd_dst_+1)
dat14$log_inf_dist<-log(dat14$infr_ds+1)

mod_frt14<- glm(fire ~ climate1 + elevatn + FWI_veg + log_road_dist + log_inf_dist, data=dat14, family=binomial(link="logit"))
...
z <- glm(fire ~ climate1 + elevatn + FWI_veg + log_road_dist + log_inf_dist, family = quasibinomial(link="logit"), data = dat14)
summary(z)
...
dat2_b<-dat14 %>% dplyr::select(fire, elevatn, climate1, log_road_dist, log_inf_dist)
...
model.data %>% 
  dplyr::select(climate1,  elevatn, log_road_dist, log_inf_dist,.std.resid, .cooksd) %>%
  top_n(5, .cooksd) 
...
```{r}
mod_frt14<- glm(fire ~ climate1 + elevatn + log_road_dist + log_inf_dist + FWI_veg, data=dat14, family=binomial(link="logit"))
