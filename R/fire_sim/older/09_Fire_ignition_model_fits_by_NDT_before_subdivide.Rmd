...
###############DATA PREP COMPLETE###################
Because of how much code is used per model, we will make separate files for each NDT. Move on to 091_Fire_ignition_model_fits_NDT4.
...
#May need to divide data into training and valid
prop<-0.75
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
...
Rerun with na.omit and check diagnostics.


```{r}

#Remove NAs to ensure all same data used so we can compare AICs
NDT4_t<-dat1 %>% drop_na(climate1, climate2, proj_height_1, live_stand_volume_125, dist_mun,  dist_dam, dist_nat, dist_pow, vegtype2, bclcs_level_5_2)
