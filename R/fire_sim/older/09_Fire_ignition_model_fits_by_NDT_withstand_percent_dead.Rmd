...
2. proj_height_1 + live_stand_volume_125 + stand_percentage_dead
...
```{r}
##Create variable lists to be used in the model loop.
variables_all_NDT4<-c(climate1 = "climate1", climate2 = "climate2", vegtype2 = "vegtype2", proj_height_1 = "proj_height_1", live_stand_volume_125 = "live_stand_volume_125", stand_percentage_dead = "stand_percentage_dead", slope = "slope", aspect = "aspect", elevation ="elevation", dist_mun = "dist_mun", dist_dam = "dist_dam", dist_nat = "dist_nat", dist_pow = "dist_pow", dist_any = "dist_any", bclcs_level_5_2 = "bclcs_level_5_2")
...
```{r}
all.poss.mods.clim.vegtype2[10]
...
for (i in 1: length(mods.twoway.NDT4)) {
   ...
   for (j in 1: length(mods.me.NDT4)) {
      if (all(s1 %in% mods.me.NDT4[[j]])==TRUE) {
        counter <- counter + 1
        both <-  c(mods.me.NDT4[[j]], mods.twoway.NDT4[[i]])
...
all.poss.mods.NDT4<-c(1, mods.me.NDT4, mods.inter.NDT4)
all.poss.mods.NDT4
all.poss.mods.NDT4<-all.poss.mods.NDT4[-2] #Use this line only if there is an odd character(0) added to list
...
NDT4_t2<-NDT4_t %>% drop_na(live_stand_volume_125, vegtype2, climate1, climate2, proj_height_1, stand_percentage_dead, slope, aspect, elevation, dist_mun, dist_dam, dist_nat, dist_pow, dist_mine, dist_any)
# When remove these, we go from 7288 variables down to 1378 variables... so not ideal...
...
#The cause for most data missing is percent stand dead... hard to include.
