...
5. stand_percentage_dead (note: missing for 2007 and 2008)
6. vegtype2 (with additional categories)
7. slope
8. aspect (cos)
9. elevation
10. heatload
11. Various distance to infrastructure variables (dist_mun, dist_dam, dist_nat, dist_pow, dist_mine, dist_any) - no interactions
...
3. Road density (to be added to final investigated model)
...
variables_all<-c(climate1 = "climate1", climate2 = "climate2", proj_height_1 = "proj_height_1", proj_age_1 = "proj_age_1", live_stand_volume_125 = "live_stand_volume_125", stand_percentage_dead = "stand_percentage_dead", slope = "slope", aspect = "aspect", elevation ="elevation", vegtype2 = "vegtype2", dist_mun = "dist_mun", dist_dam = "dist_dam", dist_nat = "dist_nat", dist_pow = "dist_pow", dist_mine = "dist_mine", dist_any = "dist_any") #heatload="heatload",

variables_all_c1<-c(climate1 = "climate1", proj_height_1 = "proj_height_1", proj_age_1 = "proj_age_1", live_stand_volume_125 = "live_stand_volume_125", stand_percentage_dead = "stand_percentage_dead", slope = "slope", aspect = "aspect", elevation ="elevation", vegtype2 = "vegtype2", dist_mun = "dist_mun", dist_dam = "dist_dam", dist_nat = "dist_nat", dist_pow = "dist_pow", dist_mine = "dist_mine", dist_any = "dist_any") #heatload="heatload",
...
vars.oth<-c("proj_height_1", "proj_age_1", "live_stand_volume_125", "stand_percentage_dead") 
...
#########4. Now for distance to infrasructure data, get all possible two-way interactions
...
With the big.mod option, currently the code is not saving all 100 runs, so just run onces instead or figure out how to fix that in the code.
...
```{r}
#terms in each model
x1d <- unlist(sapply(mods.fit, '[', 1))
x1d
#Aic for models
x3d <- matrix(unlist(sapply(mods.fit, '[', 3)), ncol=2, byrow=TRUE)
x3d
#auc from validation data
x4d <- unlist(sapply(mods.fit, '[', 4))
x4d
#combining all as df
tab.sum.infra <- cbind.data.frame(model=x1d, edf=x3d[,1], aic=x3d[,2], auc.valid=x4d)
tab.sum.infra$NDT<-c("NDT4")
tab.sum.infra$delta.aic<- tab.sum.infra$aic - (min(tab.sum.infra$aic))
tab.sum.infra
```
...
```{r}
NDT4_l_models<-rbind(tab.sum.climate, tab.sum.VRI, tab.sum.topo, tab.sum.infra)
NDT4_l_models

write.csv(NDT4_l_models, file="D:\\Fire\\fire_data\\raw_data\\NDT4_lightning_models_Sept.csv")
```
...
colnames (table.glm.climate.simple) <- c ("model", "edf", "aic", "auc.valid", "NDT","delta.aic")
...
#n<-length(filenames)
#AIC_lightning_NDT1_treed<-mkFrameList(n)
head(table.glm.climate.simple)
table(table.glm.climate.simple$model) # We somehow get 2500-5000 per model! When we expected 100... what is happening? (or 200 for those repeated)

AIC_lightning_NDT1_treed<-table.glm.climate.simple

AIC_lightning_NDT1_treed_summary<- AIC_lightning_NDT1_treed %>%
  group_by(model) %>%
...
AIC_lightning_NDT1_treed_summary2<- AIC_lightning_NDT1_treed_summary %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_lightning_NDT1_treed_summary2)
```
...
```{r}
NDT1_l_models<-rbind(tab.sum.climate, tab.sum.VRI, tab.sum.topo, tab.sum.infra)
NDT1_l_models
