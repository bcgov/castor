...
tmax_fc_raster_c<- tmax_fc_raster-273.15
...
# rasterVis plot
tmax_fc_01 <- raster(tmax_fc_raster_resampled, layer=48)
plt <- levelplot(tmax_fc_01, margin = F,  pretty=TRUE,
                 main="January temperature")
plt + layer(sp.lines(prov.bnd, col="black", lwd=1.0)) 


# EXTRACT FUTURE CLIMATE DATA AT TARGET POINTS
tmax_fc_pts <- extract(tmax_fc_raster_resampled, sppts, method="simple")
class(tmax_fc_pts)

tmax_fc_pts_df <- as.data.frame(tmax_fc_pts)
tmax_fc_pts_df<- cbind(as.data.frame(sppts)[2:3], tmax_fc_pts_df)
...
ref_future_tmax_pts<- left_join(tmax_ref_pts_df,tmax_fc_pts_df)
...
-ref_future_tmax_pts$X1985.01.15[1]+ref_future_tmax_pts$X2000.01.16[1]
ref_future_tmax_pts$delta_tmax_1_2000[1]

summary_future_clim<- ref_future_tmax_pts %>%
  dplyr::select(delta_tmax_1_2000:delta_tmax_12_2004) 
