...
Because there are few variables to explore for NDT5, we will step to manual model investigation utilizing the above variables and relevant interactions. + Tdif_atfire:climate1 + Tdif_atfire:climate2 + Tdif_atfire:wind_atfire + Tdif_atfire

#Next investigation

```{r}
escape_lightning_t_NDT5<-subset(Escape_data_lightning_t, Escape_data_lightning_t$ntrl_ds=="NDT5") #119 observations
...
model.NDT5.E<-glm(escape ~ climate1 + climate2 + climate1:climate2 + live_stand_volume_125 + dist_dam + bclcs_level_5_2 + dist_dam:bclcs_level_5_2 + roads_km + wind_atfire + Tdif_atfire:climate1 + Tdif_atfire:climate2 + Tdif_atfire:wind_atfire + Tdif_atfire, family = binomial, data = dat1)

AIC(model.NDT5.E) #64.96
...
model.NDT5.E<-glm(escape ~ climate1 + climate2 + climate1:climate2 + live_stand_volume_125 + dist_dam + bclcs_level_5_2 + dist_dam:bclcs_level_5_2 + roads_km + wind_atfire + Tdif_atfire:climate2 + Tdif_atfire:wind_atfire + Tdif_atfire, family = binomial, data = dat1)

AIC(model.NDT5.E) #63.2
...
#Remove least significant variables
model.NDT5.E<-glm(escape ~ climate1 + climate2 + live_stand_volume_125 + dist_dam + bclcs_level_5_2 + dist_dam:bclcs_level_5_2 + roads_km + wind_atfire + Tdif_atfire:climate2 + Tdif_atfire:wind_atfire + Tdif_atfire, family = binomial, data = dat1)

AIC(model.NDT5.E) #61.8

#Determine AUC of full model
mod.valid <- predict.glm(model.NDT5.E, newdata=Valid, type="response")
   roc_obj <- roc(Valid[,"escape"], mod.valid)
   mod.auc <- auc(roc_obj)
   mod.auc #0.45--> huge drop
   
Anova(model.NDT5.E, type=3)
#Anova(model.NDT5.E, type=3, singular.ok = TRUE)

#Remove least significant variables
model.NDT5.E<-glm(escape ~ climate2 + live_stand_volume_125 + dist_dam + bclcs_level_5_2 + dist_dam:bclcs_level_5_2 + roads_km + wind_atfire + Tdif_atfire:climate2 + Tdif_atfire:wind_atfire + Tdif_atfire, family = binomial, data = dat1)

AIC(model.NDT5.E) #59.8
...
#Compare to top model before Tdif_atfire
model.NDT5.E<-glm(escape ~ climate1 + climate2 + climate1:climate2 + live_stand_volume_125, family = binomial, data = dat1)

AIC(model.NDT5.E) #60.0
...
#Try with this top model plus Tdif
model.NDT5.E<-glm(escape ~ climate1 + climate2 + climate1:climate2 + live_stand_volume_125 + Tdif_atfire:wind_atfire + Tdif_atfire, family = binomial, data = dat1)

AIC(model.NDT5.E) #61.2
...
NDT5_escape_t<-escape_lightning_t_NDT5 %>% drop_na(climate2, live_stand_volume_125, dist_dam, bclcs_level_5_2, roads_km, wind_atfire, Tdif_atfire) #102 observations; small sample size

#Run Model again with this data; but uses all data here
model.NDT5.E<-glm(escape ~ climate2 + live_stand_volume_125 + dist_dam + bclcs_level_5_2 + dist_dam:bclcs_level_5_2 + roads_km + wind_atfire + Tdif_atfire:climate2 + Tdif_atfire:wind_atfire + Tdif_atfire, family = binomial, data = NDT5_escape_t)
...
# climate2 + live_stand_volume_125 + dist_dam + bclcs_level_5_2 + dist_dam:bclcs_level_5_2 + roads_km + wind_atfire + Tdif_atfire:climate2 + Tdif_atfire:wind_atfire + Tdif_atfire
visreg(model.NDT5.E, "climate2", by="Tdif_atfire")
visreg(model.NDT5.E, "Tdif_atfire", by="climate2")
visreg(model.NDT5.E, "Tdif_atfire", by="wind_atfire")
visreg(model.NDT5.E, "wind_atfire", by="Tdif_atfire")

visreg(model.NDT5.E, "live_stand_volume_125")

visreg(model.NDT5.E, "dist_dam", by="bclcs_level_5_2", overlay=TRUE)
visreg(model.NDT5.E, "dist_dam", by="bclcs_level_5_2", overlay=TRUE, ylim=c(-10,10))
visreg(model.NDT5.E, "bclcs_level_5_2", by="dist_dam")
visreg(model.NDT5.E, "bclcs_level_5_2", by="dist_dam", ylim=c(-10,10))
...
top_mod_table_NDT5_light_t_ALL <- data.frame (matrix (ncol = 18, nrow = 0))
colnames (top_mod_table_NDT5_light_t_ALL ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_2", "coef_live_stand_volume_125", "coef_dist_dam", "coef_bclcs_level_5_2OP", "coef_bclcs_level_5_2SP", "coef_roads_km", "coef_wind_atfire", "coef_Tdif_atfire", "coef_dist_dam:bclcs_level_5_2OP", "coef_dist_dam:bclcs_level_5_2SP", "coef_climate2:Tdif_atfire", "coef_wind_atfire:Tdif_atfire", "AUC")
```
...
top_mod_table_NDT5_light_t <- data.frame (matrix (ncol = 18, nrow = 0))
...
top_mod_table_NDT5_light_t[1,4]<-"escape ~ climate2 + live_stand_volume_125 + dist_dam + bclcs_level_5_2 + dist_dam:bclcs_level_5_2 + roads_km + wind_atfire + Tdif_atfire:climate2 + Tdif_atfire:wind_atfire + Tdif_atfire" 
top_mod_table_NDT5_light_t[1,5]<- coef(model.NDT5.E)[1] #Intercept
top_mod_table_NDT5_light_t[1,6]<- coef(model.NDT5.E)[2] #Climate variable 2
top_mod_table_NDT5_light_t[1,7]<- coef(model.NDT5.E)[3] #Climate live_stand_volume_125
top_mod_table_NDT5_light_t[1,8]<- coef(model.NDT5.E)[4] #coefficient dist_dam 
top_mod_table_NDT5_light_t[1,9]<- coef(model.NDT5.E)[5] #coefficient bclcs_level_5_2OP
top_mod_table_NDT5_light_t[1,10]<- coef(model.NDT5.E)[6] #coefficient bclcs_level_5_2SP
top_mod_table_NDT5_light_t[1,11]<- coef(model.NDT5.E)[7] #coefficient roads_km
top_mod_table_NDT5_light_t[1,12]<- coef(model.NDT5.E)[8] #coefficient wind_atfire
top_mod_table_NDT5_light_t[1,13]<- coef(model.NDT5.E)[9] #coefficient Tdif_atfire
top_mod_table_NDT5_light_t[1,14]<- coef(model.NDT5.E)[10] #coefficient dist_dam:bclcs_level_5_2OP
top_mod_table_NDT5_light_t[1,15]<- coef(model.NDT5.E)[11] #coefficient dist_dam:bclcs_level_5_2SP
top_mod_table_NDT5_light_t[1,16]<- coef(model.NDT5.E)[12] #coefficient climate2:Tdif_atfire 
top_mod_table_NDT5_light_t[1,17]<- coef(model.NDT5.E)[13] #coefficient wind_atfire:Tdif_atfire
top_mod_table_NDT5_light_t[1,18]<- mod.auc
...
mean(top_mod_table_NDT5_light_t_ALL$AUC) #0.61 --> way worse than original 0.67 without Tdif
```

BELOW NOT UPDATED.
```{r}
# create model table (only do this once) and add the relevant data
top_mod_table_NDT5_light_t_Means <- data.frame (matrix (ncol = 18, nrow = 0))
...
top_mod_table_NDT5_light_t_Means[1,4]<-"escape ~ climate2 + live_stand_volume_125 + dist_dam + bclcs_level_5_2 + dist_dam:bclcs_level_5_2 + roads_km + wind_atfire + Tdif_atfire:climate2 + Tdif_atfire:wind_atfire + Tdif_atfire" 
...
```{r}
# create model table (only do this once) and add the relevant data
top_mod_table_NDT5_light_t_SD <- data.frame (matrix (ncol = 18, nrow = 0))
...
top_mod_table_NDT5_light_t_SD[1,4]<-"escape ~ climate2 + live_stand_volume_125 + dist_dam + bclcs_level_5_2 + dist_dam:bclcs_level_5_2 + roads_km + wind_atfire + Tdif_atfire:climate2 + Tdif_atfire:wind_atfire + Tdif_atfire" 
...
We will revery back to the last model.
...
top_mod_table_NDT5_light_t[1,5]<- coef(model.NDT5.E)[1] #Intercept
top_mod_table_NDT5_light_t[1,6]<- coef(model.NDT5.E)[2] #Climate variable 1
top_mod_table_NDT5_light_t[1,7]<- coef(model.NDT5.E)[3] #Climate variable 2
top_mod_table_NDT5_light_t[1,8]<- coef(model.NDT5.E)[4] #coefficient live_stand_volume_125
top_mod_table_NDT5_light_t[1,9]<- coef(model.NDT5.E)[5] #coefficient climate1:climate2
