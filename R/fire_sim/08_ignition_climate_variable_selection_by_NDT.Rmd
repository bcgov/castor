---
title: "07_ignition_climate_variable_selection"
...
#=================================
#  Script Name: 08_ignition_climate_variable_selection.R
#  Script Version: 1.0
...
Now we must bring in the data that we created at the end of 07_VRI_data_prep. We need only the lightning + NA and the person + NA data sets here. We also need the fire ignition data from the end of our first file (01_fire_ignition_data_prep).
...
dat_lightning <- sf::st_read  (dsn = connKyle, # connKyle
                               query = "SELECT * FROM public.dat_lightning_for_analysis_")
...
dat_person <- sf::st_read  (dsn = connKyle, # connKyle
                               query = "SELECT * FROM public.dat_person_for_analysis_")
dbDisconnect (connKyle)
...
Can bring in from local machine.
```{r}
dat_lightning<-read.csv(file ="D:\\Fire\\fire_data\\raw_data\\Fire_climate_DEM_VRI_shape_files\\dat_lightning_for_analysis_Oct.csv")
head(dat_lightning)

dat_person<-read.csv(file ="D:\\Fire\\fire_data\\raw_data\\Fire_climate_DEM_VRI_shape_files\\dat_person_for_analysis_Oct.csv")
head(dat_person)
...
Now that the data is prepped from 07_VRI_data_prep, we can move on to the analysis for selecting the best climatic variable(s) for each Natural Disturbance Type (NDT) zone. The loop below will be run first for lightning caused fires, and then for person-caused fires.
...
dist.cut.corr <- dat_lightning [c (21:25, 36:40)]
...
dist.cut.corr <- dat_lightning [c (31:35, 36:40)]
corr <- round (cor (dist.cut.corr), 3)
ggcorrplot (corr, type = "lower", lab = TRUE, tl.cex = 10,  lab_size = 3,
...
dist.cut.corr <- dat_lightning [c (21:25, 31:35)]
corr <- round (cor (dist.cut.corr), 3)
ggcorrplot (corr, type = "lower", lab = TRUE, tl.cex = 10,  lab_size = 3,
...
dist.cut.corr <- dat_lightning [c (25:29, 30:34)]
corr <- round (cor (dist.cut.corr), 3)
ggcorrplot (corr, type = "lower", lab = TRUE, tl.cex = 10,  lab_size = 3,
...
table(dat_lightning$ign_month)
head(dat_lightning) 

#Ensure all months available in data for comparisons
dat_lightning$tave_dif_01_02<-(dat_lightning$tave02 - dat_lightning$tave01)
dat_lightning$tave_dif_02_03<-(dat_lightning$tave03 - dat_lightning$tave02)
dat_lightning$tave_dif_03_04<-(dat_lightning$tave04 - dat_lightning$tave03)
dat_lightning$tave_dif_04_05<-(dat_lightning$tave05 - dat_lightning$tave04)
dat_lightning$tave_dif_05_06<-(dat_lightning$tave06 - dat_lightning$tave05)
dat_lightning$tave_dif_06_07<-(dat_lightning$tave07 - dat_lightning$tave06)
dat_lightning$tave_dif_07_08<-(dat_lightning$tave08 - dat_lightning$tave07)
dat_lightning$tave_dif_08_09<-(dat_lightning$tave09 - dat_lightning$tave08)
dat_lightning$tave_dif_09_10<-(dat_lightning$tave10 - dat_lightning$tave09)
dat_lightning$tave_dif_10_11<-(dat_lightning$tave11 - dat_lightning$tave10)
dat_lightning$tave_dif_11_12<-(dat_lightning$tave12 - dat_lightning$tave11)

#Create a variable that is the mean of the main months for which fires occur
dat_lightning$tave_dif_mean<-((dat_lightning$tave_dif_05_06 + dat_lightning$tave_dif_06_07 + dat_lightning$tave_dif_07_08 + dat_lightning$tave_dif_08_09)/4)

#Compare to the mean for each individually
mean(dat_lightning$tave_dif_05_06) #3.79
mean(dat_lightning$tave_dif_06_07) #3.004
mean(dat_lightning$tave_dif_07_08) #-0.852
mean(dat_lightning$tave_dif_08_09)  #-4.6906
dat_lightning$tave_dif_mean2<-((dat_lightning$tave_dif_05_06 + dat_lightning$tave_dif_06_07 + dat_lightning$tave_dif_07_08)/3)
mean(dat_lightning$tave_dif_mean2)

#Create a variable for the max difference for which fires most often occur
library(matrixStats)
dat_lightning$tave_dif_max<- rowMaxs(as.matrix(dat_lightning[,c(117,118,119,120)])) #select columns for dat_lightning$tave_dif_05_06, dat_lightning$tave_dif_06_07, dat_lightning$tave_dif_07_08, dat_lightning$tave_dif_08_09)

head(dat_lightning)
hist(dat_lightning$tave_dif_04_05)
hist(dat_lightning$tave_dif_05_06)
hist(dat_lightning$tave_dif_06_07)
hist(dat_lightning$tave_dif_07_08)
hist(dat_lightning$tave_dif_08_09)
table(dat_lightning$ign_month) 

#Assign values by month fire occurred, with the TRUE value assigning to the locations where fire did not occur
dat_lightning$Tdif_atfire<-0
dat_lightning$ign_month<-as.numeric(dat_lightning$ign_month)
head(dat_lightning)
dat_lightning_2<-dat_lightning %>%
    mutate(Tdif_atfire = case_when(ign_month == 5 ~ tave_dif_04_05,
                                  ign_month == 6 ~ tave_dif_05_06,
                                  ign_month == 7 ~ tave_dif_06_07,
                                  ign_month == 8 ~ tave_dif_07_08,
                                  ign_month == 9 ~ tave_dif_08_09,
                                  ign_month == 10 ~ tave_dif_09_10,
                                  ign_month == 11 ~ tave_dif_10_11,
                                  ign_month == 12 ~ tave_dif_11_12,
                                  ign_month == 4 ~ tave_dif_03_04,
                                  ign_month == 3 ~ tave_dif_02_03,
                                  ign_month == 2 ~ tave_dif_01_02,
                                  ign_month == 1 ~ tave_dif_01_02,
...
dat_lightning_2$Tdif_atfireB<-0
dat_lightning_2$ign_month<-as.numeric(dat_lightning_2$ign_month)
head(dat_lightning_2)
dat_lightning_2<-dat_lightning_2 %>%
    mutate(Tdif_atfireB = case_when(ign_month == 5 ~ tave_dif_04_05,
                                  ign_month == 6 ~ tave_dif_05_06,
                                  ign_month == 7 ~ tave_dif_06_07,
                                  ign_month == 8 ~ tave_dif_07_08,
                                  ign_month == 9 ~ tave_dif_08_09,
...
head(dat_lightning_2)
hist(dat_lightning_2$Tdif_atfireB) #Mostly clustered at slightly about 2

#Make a second, similar variable in which the TRUE statement represents the max difference instead of the mean
dat_lightning_2$Tdif_atfire2<-0
head(dat_lightning_2)
dat_lightning_2<-dat_lightning_2 %>%
    mutate(Tdif_atfire2 = case_when(ign_month == 5 ~ tave_dif_04_05,
                                  ign_month == 6 ~ tave_dif_05_06,
                                  ign_month == 7 ~ tave_dif_06_07,
                                  ign_month == 8 ~ tave_dif_07_08,
                                  ign_month == 9 ~ tave_dif_08_09,
...
head(dat_lightning_2)
hist(dat_lightning_2$Tdif_atfire)

dat_lightning<-dat_lightning_2
...
```{r}
dat_lightning$T_atfire<-0
dat_lightning$ign_month<-as.numeric(dat_lightning$ign_month)
head(dat_lightning)
dat_lightning_<-dat_lightning %>%
    mutate(T_atfire = case_when(ign_month == 5 ~ tave_05,
                                  ign_month == 6 ~ tave_06,
                                  ign_month == 7 ~ tave_07,
                                  ign_month == 8 ~ tave_08,
                                  ign_month == 9 ~ tave_09,
                                  ign_month == 10 ~ tave_09,
                                  ign_month == 11 ~ tave_09,
                                  ign_month == 4 ~ tave_05,
                                  ign_month == 3 ~ tave_05,
                                  ign_month == 2 ~ tave_05,
                                  ign_month == 1 ~ tave_05,
...
dat_lightning_$fire_pres<-dat_lightning_$fire
...
```{r}
dat_lightning_$mdc_atfire_2<-0
head(dat_lightning_)
dat_lightning_<-dat_lightning_ %>%
    mutate(mdc_atfire_2 = case_when(ign_month == 5 ~ mdc_05,
                                  ign_month == 6 ~ mdc_06,
                                  ign_month == 7 ~ mdc_07,
                                  ign_month == 8 ~ mdc_08,
                                  ign_month == 9 ~ mdc_09,
...
p <- ggplot(dat_lightning_, aes(mdc_atfire_2, as.numeric(fire_pres))) +
  geom_smooth(method="glm", formula=y~x,
              method.args=list(family="binomial"),
              alpha=0.3, size=1) +
  geom_point(position=position_jitter(height=0.03, width=0)) +
...
```{r}
#create new column
dat_lightning_$fire_zone<-paste(dat_lightning_$fire_pres, dat_lightning_$ntrl_ds)
str(dat_lightning_$fire_zone)
dat_lightning_$fire_zone<-as.factor(dat_lightning_$fire_zone)
table(dat_lightning_$fire_zone)
head(dat_lightning_)
```
...
```{r}
#Check months for fire
table(dat_person$ign_month)
head(dat_person) 

#Without all months, we can only use the below comparisons. 
dat_person$tave_dif_01_02<-(dat_person$tave02 - dat_person$tave01)
dat_person$tave_dif_02_03<-(dat_person$tave03 - dat_person$tave02)
dat_person$tave_dif_03_04<-(dat_person$tave04 - dat_person$tave03)
dat_person$tave_dif_04_05<-(dat_person$tave05 - dat_person$tave04)
dat_person$tave_dif_05_06<-(dat_person$tave06 - dat_person$tave05)
dat_person$tave_dif_06_07<-(dat_person$tave07 - dat_person$tave06)
dat_person$tave_dif_07_08<-(dat_person$tave08 - dat_person$tave07)
dat_person$tave_dif_08_09<-(dat_person$tave09 - dat_person$tave08)
dat_person$tave_dif_09_10<-(dat_person$tave10 - dat_person$tave09)
dat_person$tave_dif_10_11<-(dat_person$tave11 - dat_person$tave10)
dat_person$tave_dif_11_12<-(dat_person$tave12 - dat_person$tave11)

dat_person$tave_dif_mean<-((dat_person$tave_dif_05_06 + dat_person$tave_dif_06_07 + dat_person$tave_dif_07_08 + dat_person$tave_dif_08_09)/4)

#Compare to the mean for each individually
mean(dat_person$tave_dif_05_06) #3.56
mean(dat_person$tave_dif_06_07) #3.06
mean(dat_person$tave_dif_07_08) #-0.63
mean(dat_person$tave_dif_08_09)  #-4.27
dat_person$tave_dif_mean2<-((dat_person$tave_dif_05_06 + dat_person$tave_dif_06_07 + dat_person$tave_dif_07_08)/3)
mean(dat_person$tave_dif_mean2)

#Create a variable for the max difference for which fires most often occur
library(matrixStats)
dat_person$tave_dif_max<- rowMaxs(as.matrix(dat_person[,c(117,118,119,120)])) #select columns for dat_person$tave_dif_05_06, dat_person$tave_dif_06_07, dat_person$tave_dif_07_08, dat_person$tave_dif_08_09)

head(dat_person)
hist(dat_person$tave_dif_04_05)
hist(dat_person$tave_dif_05_06)
hist(dat_person$tave_dif_06_07)
hist(dat_person$tave_dif_07_08)
hist(dat_person$tave_dif_08_09)
table(dat_person$ign_month) 

#Assign values by month fire occurred, with the TRUE value assigning to the locations where fire did not occur
dat_person$Tdif_atfire<-0
dat_person$ign_month<-as.numeric(dat_person$ign_month)
head(dat_person)
dat_person_2<-dat_person %>%
    mutate(Tdif_atfire = case_when(ign_month == 5 ~ tave_dif_04_05,
                                  ign_month == 6 ~ tave_dif_05_06,
                                  ign_month == 7 ~ tave_dif_06_07,
                                  ign_month == 8 ~ tave_dif_07_08,
...
#try without 08_09 since temperature change goes in opposite direction
dat_person_2$Tdif_atfireB<-0
dat_person_2$ign_month<-as.numeric(dat_person_2$ign_month)
head(dat_person_2)
dat_person_2<-dat_person_2 %>%
    mutate(Tdif_atfireB = case_when(ign_month == 5 ~ tave_dif_04_05,
                                  ign_month == 6 ~ tave_dif_05_06,
                                  ign_month == 7 ~ tave_dif_06_07,
                                  ign_month == 8 ~ tave_dif_07_08,
                                  ...
                                  TRUE ~ tave_dif_mean2)) #Those areas without fire will receive the mean value for the season. 
head(dat_person_2)
hist(dat_person_2$Tdif_atfireB) #Mostly clustered at slightly about 2

#Make a second, similar variable in which the TRUE statement represents the max difference instead of the mean
dat_person_2$Tdif_atfire2<-0
head(dat_person_2)
dat_person_2<-dat_person_2 %>%
    mutate(Tdif_atfire2 = case_when(ign_month == 5 ~ tave_dif_04_05,
                                  ign_month == 6 ~ tave_dif_05_06,
                                  ...
                                  ign_month == 2 ~ tave_dif_01_02,
                                  ign_month == 1 ~ tave_dif_01_02,
                                  TRUE ~ tave_dif_max)) #Those areas without fire will receive the mean value for the season. These MDC values may be most accurately used for the escape and spread data, wherein we only assess those areas where fires began.

head(dat_person_2)
hist(dat_person_2$Tdif_atfire)

dat_person<-dat_person_2
...
head(dat_person_)
dat_person_$fire_pres<-dat_person_$fire
...
```{r}
#create new column
dat_person_$fire_zone<-paste(dat_person_$fire_pres, dat_person_$ntrl_ds)
str(dat_person_$fire_zone)
dat_person_$fire_zone<-as.factor(dat_person_$fire_zone)
table(dat_person_$fire_zone)
head(dat_person_)
```
...
table(dat_lightning_$fire_yr, dat_lightning_$fire_pres)
table(dat_lightning_$fire_yr, dat_lightning_$fire_cs, dat_lightning_$zone)
table(dat_lightning_$fire_yr, dat_lightning_$fire_cs, dat_lightning_$ntrl_ds)
...
table(dat_person_$fire_yr, dat_person_$fire_pres)
table(dat_person_$fire_yr, dat_person_$fire_cs, dat_person_$zone)
...
We have two main options: separate by BEC zone (14), or separate by Natural Disturbance Type (NDT). The initial code, now saved in the "old" folder, assesses by BEC zones independently. Here, we will run by NDT to have fewer models for the first iteration of this project.

#Natural Disturbance Types, as defined in the Forest Practices Code Biodiversity Guidebook:
...
For specifics of which BEC zones and subzones fall into which NDT, see https://testwww.for.gov.bc.ca/tasb/legsregs/fpc/fpcguide/biodiv/app3.htm

```{r}
# create loop to do variable selection of climate data
unique(dat_lightning_$ntrl_ds)
str(dat_lightning_$ntrl_ds)
dat_lightning_$ntrl_ds<-as.factor(dat_lightning_$ntrl_ds)
zones<- c("NDT1", "NDT2", "NDT3", "NDT4", "NDT5")
...
for (h in 1:length(zones)) {
  dat2<- dat_lightning_ %>% dplyr::filter(ntrl_ds ==zones[h])
