...
table(dat_lightning$ign_month) # captures most fires, but does miss the 659 fires in month 5 (since we do not currently have month)

#FOR NOW USE 05_06 for 04_05, BUT NEEDS UPDATING!
...
dat_lightning_2<-dat_lightning %>%
    mutate(Tdif_atfire = case_when(ign_month == 5 ~ tave_dif_04_05,
                                  ...
                                  TRUE ~ tave_dif_mean)) #Those areas without fire will receive the mean value for the season. These MDC values may be most accurately used for the escape and spread data, wherein we only assess those areas where fires began.
...
table(dat_person$ign_month) # captures most fires, but does miss the 659 fires in month 5 (since we do not currently have month)
