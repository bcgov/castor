...
Bring in data points for which we want data for slope and aspect (i.e, sample locations). This is the data that we uploaded at the end of the 02_climate_data_prep_all. If you are continuing from this file immediately, you may not need to bring it back in again. 
...
sample_locations2 <- sf::st_read  (dsn = connKyle, # connKyle
                               query = "SELECT * FROM public.DC_data_5x")
...
head(rasValue2)
str(rasValue2) #274594 values
str(sample_locations2)#274594 values
...
sample_locations_DEM<- sample_locations_DEM %>% rename(TRASP=rasValue3)
hist(sample_locations_DEM$TRASP)

```

Save this file and use for next R Markdown file (04_vri_data_prep). Make sure you save it to the local machine via OSGeo4W for use in code in the next file.

```{r}

st_write(sample_locations_DEM, dsn = "D:\\Fire\\fire_data\\raw_data\\ClimateBC_Data\\DC_data_DEM_5x.shp", delete_layer=TRUE)

#Also write to local machine on OSGeo4W
#ogr2ogr -f "PostgreSQL" PG:"host=localhost user=postgres dbname=postgres password=postgres port=5432" D:\\Fire\\fire_data\\raw_data\\ClimateBC_Data\\DC_data_DEM_5x.shp -overwrite -a_srs EPSG:3005 -progress --config PG_USE_COPY YES -nlt PROMOTE_TO_MULTI

##Below needs: (1) update to relevant credentials and (2) then enter into the OSGeo4W command line and hit enter. 
#ogr2ogr -f PostgreSQL PG:"host=DC052586 user= dbname=clus password= port=5432" D:\\Fire\\fire_data\\raw_data\\ClimateBC_Data\\DC_data_DEM_5x.shp -overwrite -a_srs EPSG:3005 -progress --config PG_USE_COPY YES -nlt PROMOTE_TO_MULTI
...
############## Now move on to file 04_Road_density_data_prep#############
