...
table(is.na(fire_veg_sf$bec_bec_zone_code_code))
...
table(fire_veg_sf$bec_bec_subzone)
...
fire_veg_sf$proj_proj_height_1_1<- round(fire_veg_sf$proj_proj_height_1_1,1)
fire_veg_sf$bec_bec_zone_code_code<- (as.character(fire_veg_sf$bec_bec_zone_code_code))
fire_veg_sf$bec_bec_subzone<- as.character(fire_veg_sf$bec_bec_subzone)
...
fire_veg_sf<-rbind(fire_veg_1, fire_veg_2)
...
To save time and effort in coding I create a list of conifer and deciduous species and the bec_bec_subzone codes that equate to wet areas. 
```{r}
...
veg2$crown_closure<-as.numeric(veg2$crown_closure)
  ...
  veg2[bclcs_level_1=="V" & bclcs_level_2=="N" & is.na(yearHarvest) & (non_productive_cd == 60 |non_productive_cd == 62 | non_productive_cd == 63), fwveg:="O-1a/b"]
  ...
  veg2[bclcs_level_1=="V" & bclcs_level_2=="T" & species_pct_1 > 79 & dominant_conifer %in% c("F","FD","FDC","FDI") & bec_zone_code %in% c("CWH", "MH", "CDF") & proj_height_1 %between% c(4, 12) & (is.na(yearHarvest) | (yearsSinceHarvest>6)) & crown_closure >= 55, fwveg:="C-3"]
  veg2[bclcs_level_1=="V" & bclcs_level_2=="T" & species_pct_1> 79 & species_cd_1 %in% c("F","FD","FDC","FDI") & bec_zone_code =="ICH" & bec_subzone %in% wet & proj_height_1 %between% c(4, 12) & crown_closure > 55  & (is.na(yearHarvest) | (yearsSinceHarvest>6)), fwveg:="C-3"]
  veg2[bclcs_level_1=="V" & bclcs_level_2=="T" & species_pct_1> 79 & species_cd_1 %in% c("F","FD","FDC","FDI") & proj_height_1 %between% c(4,12) & crown_closure > 55 & stand_percentage_dead > 34  & (is.na(yearHarvest) | (yearsSinceHarvest>6)), fwveg:="C-4"]
  veg2[bclcs_level_1=="V" & bclcs_level_2=="T" & species_pct_1> 79 & species_cd_1 %in% c("F","FD","FDC","FDI") & proj_height_1 %between% c(4,12) & crown_closure > 55  & (is.na(yearHarvest) | (yearsSinceHarvest>6)), fwveg:="C-3"]
  ...
  veg2[bclcs_level_1=="V" & bclcs_level_2=="T" & species_pct_1> 79 & species_cd_1 %in% c("F","FD","FDC","FDI") & bec_zone_code %in% c("CWH", "MH", "CDF") & crown_closure %between% c(26,55), fwveg:="C-5"]
  veg2[bclcs_level_1=="V" & bclcs_level_2=="T" & species_pct_1> 79 & species_cd_1 %in% c("F","FD","FDC","FDI") & bec_zone_code=="ICH" & bec_subzone %in% wet & crown_closure %between% c(26, 55), fwveg:="C-5"]
  veg2[bclcs_level_1=="V" & bclcs_level_2=="T" & species_pct_1> 79 & species_cd_1 %in% c("F","FD","FDC","FDI") & crown_closure %between% c(26, 55), fwveg:="C-7"]
...
y<-veg2 %>% filter(fire==1) %>% filter(fwveg %in% c("N")) # Note there are a whole bunch of ignition points that have FWI_veg categoriezed as N. This is a problem because N means no fuel. I tried to deal with this by buffering the points 50m then 150m. Buffering did not seem to help.
table(y$fire_yr)
...
st_write(veg2, "C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\Fire_data_all_veg2.gpkg", delete_layer=TRUE, driver="GPKG")
...
veg3<-veg2
veg2[Cluster==3, Cluster:=5]

frt5<-veg2[Cluster==5 & !fwveg %in% c("W", "S-1","S-2", "N"),]
frt5[fwveg == "M-3", fwveg:="M-1/2"]
table(frt5$fwveg, frt5$fire)

z <- glm(fire ~ fwveg, family = binomial(link="logit"), data = frt5, na.action = na.exclude)
z_null<-glm(fire ~ 1, family = binomial(link="logit"), data=frt5)
...
################################
```{r}
frt7<-veg2[Cluster==7 & !fwveg %in% c("W", "N"),]
table(frt7$fwveg, frt7$fire)
...
z <- glm(fire ~ fwveg, family = binomial(link="logit"), data = frt7, na.action = na.exclude)
...
################################
```{r}
frt9<-veg2[Cluster==9 & !fwveg %in% c("W", "N"),]
table(frt9$fwveg, frt9$fire)


z <- glm(fire ~ fwveg, family = binomial(link="logit"), data = frt9, na.action = na.exclude)
...
################################
```{r}
frt10<-veg2[Cluster==10 & !fwveg %in% c("W", "N"),]
table(frt10$fwveg, frt10$fire)
...
z <- glm(fire ~ fwveg, family = binomial(link="logit"), data = frt10, na.action = na.exclude)
...
################################
```{r}
frt11<-veg2[Cluster==11 & !fwveg %in% c("W", "N", "S-1"),]
table(frt11$fwveg, frt11$fire)

# combine some vegetation types that dont have much data
z <- glm(fire ~ fwveg, family = binomial(link="logit"), data = frt11, na.action = na.exclude)
...
################################
```{r}
frt12<-veg2[Cluster==12 & !fwveg %in% c("W", "N"),]
table(frt12$fwveg, frt12$fire)
...
z <- glm(fire ~ fwveg, family = binomial(link="logit"), data = frt12, na.action = na.exclude)
...
################################
```{r}
frt13<-veg2[Cluster==13 & !fwveg %in% c("W", "N"),]
table(frt13$fwveg, frt13$fire)
...
z <- glm(fire ~ fwveg, family = binomial(link="logit"), data = frt13, na.action = na.exclude)
...
################################
```{r}
frt14<-veg2[Cluster==14 & !fwveg %in% c("W", "N", "C-4"),]
table(frt14$fwveg, frt14$fire)
...
z <- glm(fire ~ fwveg, family = binomial(link="logit"), data = frt14, na.action = na.exclude)
...
################################
```{r}
frt15<-veg2[Cluster==15 & !fwveg %in% c("W", "N"),]
table(frt15$fwveg, frt15$fire)

# combine some vegetation types that dont have much data
frt15[fwveg=="S-3", fwveg:="S-1"]
frt15[fwveg=="S-2", fwveg:="S-1"]

z <- glm(fire ~ fwveg, family = binomial(link="logit"), data = frt15, na.action = na.exclude)
