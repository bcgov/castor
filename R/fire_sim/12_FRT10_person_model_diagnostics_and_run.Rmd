...
dat10$FWI_veg[dat10$FWI_veg=="C-5"]<-"C-7" # seems like C5 maybe most similar to C7
# tehre are only 9 observations in the C-2 fuel type. From figures on pg 6 of perrakis it seems like C-2 is most similar to C-3 so although not ideal Ill lump these

dat10$FWI_veg[dat10$FWI_veg=="C-2"]<-"C-3"

# also going to combine D-1/2 (Green or Leafless Aspen or Deciduous Shrub) with M-3 (Dead Balsam Fir/Mixedwood – Leafless) because there are few observations in these categories and them seem similar. Maybe Ill also put the slash (S-1 and S-2 into one of these categories. 
dat10$FWI_veg[dat10$FWI_veg=="M-3"]<-"D-1/2"
dat10$FWI_veg[dat10$FWI_veg=="S-1"]<-"D-1/2"
dat10$FWI_veg[dat10$FWI_veg=="S-2"]<-"D-1/2"
table(dat10$FWI_veg)

# remove observations that fall in water or over non-vegetation areas (N). Will fill these back in at the end as a zero
dat10<- dat10 %>% filter(!FWI_veg %in%  c("N", "W"))
table(dat10$FWI_veg)
...
mod_frt_10a<- glm(fire ~ climate1 + elevatn + FWI_veg + rd_dst_ + infr_ds, data=dat10, family=binomial(link="logit"))
mod_frt10b<- glm(fire ~ climate1 + elevatn + FWI_veg + rd_dst_, data=dat10, family=binomial(link="logit"))
anova(mod_frt_10a,mod_frt10b, test="Chisq") # removing infrastructure is fine

mod_frt10c<- glm(fire ~ climate1 + elevatn + FWI_veg + infr_ds, data=dat10, family=binomial(link="logit"))
anova(mod_frt_10a,mod_frt10c, test="Chisq") # removing roads is bad

mod_frt10d<- glm(fire ~ climate1 + FWI_veg + rd_dst_, data=dat10, family=binomial(link="logit"))
anova(mod_frt10b,mod_frt10d, test="Chisq") # removing elevation is also fine

mod_frt_10a_log<- glm(fire ~ climate1 + elevatn + FWI_veg + log(rd_dst_+1) + log(infr_ds+1), data=dat10, family=binomial(link="logit"))

mod_frt10b_log<- glm(fire ~ climate1 + elevatn + FWI_veg + log(rd_dst_+1), data=dat10, family=binomial(link="logit"))
anova(mod_frt_10a_log,mod_frt10b_log, test="Chisq")

mod_frt_10c_log<- glm(fire ~ climate1 + elevatn + FWI_veg + log(infr_ds+1), data=dat10, family=binomial(link="logit"))
anova(mod_frt_10a_log,mod_frt_10c_log, test="Chisq")

mod_frt10d_log<- glm(fire ~ climate1 + FWI_veg + log(rd_dst_+1), data=dat10, family=binomial(link="logit"))
anova(mod_frt10b_log,mod_frt10d_log, test="Chisq") # this is the best model, both according to AIC and comparing the fits of two models



dat10$log_road_dist<-log(dat10$rd_dst_+1)

#### TOP MODEL
mod_frt10<- glm(fire ~ climate1 + FWI_veg + log_road_dist, data=dat10, family=binomial(link="logit"))

summary(mod_frt10)
Anova(mod_frt10, type=3)

# just checking whetehr a quasibinomial is better. The dispersion should be close to 1, which it is. So the below is not neccessary! Will stick with regular binomial. Got this from Dolph Schluter's R tips page (Check assumptions of models in the Fit a generalized linear model section) https://www.zoology.ubc.ca/~schluter/R/Model.html
z <- glm(fire ~ climate1 + FWI_veg + log_road_dist, family = quasibinomial(link="logit"), data = dat10)
summary(z)
anova(z, test = "F")

# model diagnostic plots
# below model looks way better with log road dist. It had some structure with just road dist
binnedplot (fitted(mod_frt10), 
            residuals(mod_frt10), 
            nclass = NULL, 
...
binnedplot (dat10$log_road_dist, 
            dat10$resids, 
            nclass = NULL, 
            xlab = "road dist", 
            ylab = "Average residual", 
...
##Partial Residuals
visreg(mod_frt10, scale="response") # nicer plots than the ones below!
...
dat2_b<-dat10 %>% dplyr::select(fire, climate1, log_road_dist)
probabilities <- predict(mod_frt10, type = "response")
...
library(broom)
model.data <- augment(mod_frt10) %>% 
  mutate(index = 1:n()) 

#The data for the top 3 largest values, according to the Cook’s distance, can be displayed as follow:
model.data %>% 
  dplyr::select(climate1, FWI_veg, log_road_dist,.std.resid, .cooksd) %>%
  top_n(5, .cooksd) 
...
```{r}
mod_frt10<- glm(fire ~ climate1 + log_road_dist + FWI_veg, data=dat10, family=binomial(link="logit"))

summary(mod_frt10)

#Create a new blank table and get AUC too
top_mod_table_FRT5_person_All <- data.frame (matrix (ncol = 10, nrow = 0))
colnames (top_mod_table_FRT5_person_All ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1", "coef_log_road_dist", "coef_FWI_veg_C-5", "coef_FWI_veg_C-7", "coef_FWI_veg_D-1/2", "AUC")
...
#Model   
mod<-glm(fire ~ climate1 + log_road_dist + FWI_veg, data=dat1, family=binomial(link="logit"))
...
top_mod_table_FRT5_person <- data.frame (matrix (ncol = 10, nrow = 0))
colnames (top_mod_table_FRT5_person ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1", "coef_log_road_dist", "coef_FWI_veg_C-5", "coef_FWI_veg_C-7", "coef_FWI_veg_D-1/2", "AUC")
...
top_mod_table_FRT5_person[1,10]<- mod.auc
...
write.csv(AIC_lightning_NDT1_t_ignitereed_summary_climate2, file="C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\data\\Ignition_lightning_results\\FRT_5_7_9_10_11_12_14_person_models.csv")
