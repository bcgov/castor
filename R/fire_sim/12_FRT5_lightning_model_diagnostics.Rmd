...
table(dat5$fwveg, dat5$fire)

frt5<-veg2[Cluster==5 & !fwveg %in% c("W", "S-1","S-2", "N"),]


dat5_2<-dat5 %>% filter(!fwveg %in% c("N", "W","S-1","S-2"))
table(dat5_2$fwveg)

dat5_2$fwveg <- factor(dat5_2$fwveg, levels=c("C-1","C-2","C-3","C-7", "D-1/2","M-1/2", "O-1a/b"))

ggplot(data=dat5_2, mapping=aes(x=as.factor(fire), y=climate1))+
  geom_boxplot()

ggplot(data=dat5_2, mapping=aes(x=as.factor(fire), y=dm_h_bc))+
  geom_boxplot()

cor.test(dat5$climate1, dat5$dm_h_bc) # elevation and climate are correlated
cor.test(dat5$climate2, dat5$dm_h_bc)
cor.test(dat5$climate1, dat5$climate2)

head(dat5_2)

table(is.na(dat5_2$dm_h_bc))
dat5_3<-dat5_2 %>% drop_na(dm_h_bc)

mod.frt.5a<-glm(fire ~ climate1 + climate2 + fwveg + dm_h_bc, data=dat5_3, family=binomial(link="logit"))

mod.frt.5b<-glm(fire ~ climate2 + fwveg + dm_h_bc, data=dat5_3, family=binomial(link="logit"))

mod.frt.5c<-glm(fire ~ climate1 + climate2 + fwveg, data=dat5_3, family=binomial(link="logit"))

# keeping elevation and throwing out climate1 is best. Since I have climate2 ill do this.

mod.frt.5<-glm(fire ~ climate2 + fwveg + climate1, data=dat5_3, family=binomial(link="logit"))

cor(model.matrix(mod.frt.5)[,-1])

hist(dat5_3$climate2)
hist(dat5_3$climate1)
summary(mod.frt.5)
...
visreg(mod.frt.5, "climate2", by="climate1", scale="response")
visreg(mod.frt.5, "climate1", by="fwveg", scale="response")
visreg(mod.frt.5, "climate2", by="fwveg", scale="response")
...
dat2_b<-dat5_3%>% dplyr::select(fire, climate1, climate2)
...
model.data %>% 
  dplyr::select(climate2,  climate1, fwveg,.std.resid, .cooksd) %>%
  top_n(5, .cooksd) 
...
colnames (top_mod_table_FRT10_lightningALL ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1","coef_climate_2", "coef_fwveg_C-2","coef_fwveg_C-3", "coef_fwveg_C-7", "coef_fwveg_D-1/2","coef_fwveg_M-1/2",  "coef_fwveg_O-1a/b", "AUC")

dat5_2$fire_veg<-paste(dat5_2$fire, dat5_2$fwveg)
```
...
#Model   
mod<-glm(fire ~ climate1 + climate2 + fwveg, data=dat1, family=binomial(link="logit"))
...
colnames (top_mod_table_FRT10_lightningL ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1","coef_climate_2", "coef_fwveg_C-2","coef_fwveg_C-3", "coef_fwveg_C-7", "coef_fwveg_D-1/2","coef_fwveg_M-1/2",  "coef_fwveg_O-1a/b", "AUC")


##Add data for NDT1
top_mod_table_FRT10_lightningL[1,1]<-"lightning"
top_mod_table_FRT10_lightningL[1,2]<-"FRT5"
top_mod_table_FRT10_lightningL[1,3]<-"fire ~ climate1 + climate2 + fwveg"
...
FRT10_summary_table_mean$Model_terms<-"fire ~ climate1 + climate2 + fwveg"
