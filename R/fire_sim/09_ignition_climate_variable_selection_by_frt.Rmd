...
```{r}
fire_ignitions<-st_read("C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\tmp\\Fire_data_all_with_veg.shp") 
...
dat_lightning<-st_set_geometry(fire_ignitions,NULL)

dist.cut.corr <- dat_lightning [c (21:28, 61:68)]
...
dist.cut.corr <- dat_lightning [c (41:48, 61:68)]
corr <- round (cor (dist.cut.corr), 3)
ggcorrplot (corr, type = "lower", lab = TRUE, tl.cex = 10,  lab_size = 3,
...
dist.cut.corr <- dat_lightning [c (21:28, 41:48)]
corr <- round (cor (dist.cut.corr), 3)
ggcorrplot (corr, type = "lower", lab = TRUE, tl.cex = 10,  lab_size = 3,
...
dist.cut.corr <- dat_lightning [c (31:38, 41:48)]
corr <- round (cor (dist.cut.corr), 3)
ggcorrplot (corr, type = "lower", lab = TRUE, tl.cex = 10,  lab_size = 3,
...
dist.cut.corr <- dat_person [c (21:28, 61:68)]
corr <- round (cor (dist.cut.corr), 3)
ggcorrplot (corr, type = "lower", lab = TRUE, tl.cex = 10,  lab_size = 3,
            title = "Correlation between maximum temperature and MDC")
...
dim(sampled_df) # 
dim(pre1) #  15228 rows
sampled_df<-sampled_df %>% dplyr::select(fire_yr:bclc__2, idno:mean_MDC_03_MDC_04_MDC_05_MDC_06_MDC_07_MDC_08_MDC_09_MDC_10_MDC_11)
...
dat_lightning$tot_spring_PPT<-dat_lightning$Tmax03+ dat_lightning$Tmax04 + dat_lightning$Tmax05
dat_lightning$tot_summer_PPT<-dat_lightning$Tmax06+ dat_lightning$Tmax07 + dat_lightning$Tmax08
```
...
variables<- c(#"Tmax03",
 ...
 variables1<-c("Tmax04", "Tmax05", "Tmax06", "Tmax07", "Tmax08", "Tmax09", "mean_Tmax03_Tmax04_Tmax05", "mean_Tmax06_Tmax07_Tmax08", 
               "Tave04","Tave05", "Tave06", "Tave07", "Tave08", "Tave09", "mean_Tave03_Tave04_Tave05", "mean_Tave06_Tave07_Tave08")
 
variables2<-c("PPT04","PPT05", "PPT06", "PPT07", "PPT08", "PPT09", "mean_PPT03_PPT04_PPT05", "mean_PPT06_PPT07_PPT08",
              "PPT04","PPT05", "PPT06", "PPT07", "PPT08", "PPT09",  "mean_PPT03_PPT04_PPT05", "mean_PPT06_PPT07_PPT08") 
...
table(is.na(aic_bec_lightning2$AIC))
...
dim(sampled_df) # 
dim(pre1) #  15228 rows
sampled_df<-sampled_df %>% dplyr::select(fire_yr,frt, idno:mean_MDC_03_MDC_04_MDC_05_MDC_06_MDC_07_MDC_08_MDC_09_MDC_10_MDC_11)
...
variables<- c("Tmax03",
 ...
 variables1<-c("Tmax03","Tmax04", "Tmax05", "Tmax06", "Tmax07", "Tmax08", "Tmax09","Tmax10","Tmax11", 
               "Tave03","Tave04","Tave05", "Tave06", "Tave07", "Tave08", "Tave09", "Tave10","Tave11")
 
variables2<-c("PPT03","PPT04","PPT05", "PPT06", "PPT07", "PPT08", "PPT09","PPT10","PPT11",
              "PPT03","PPT04","PPT05", "PPT06", "PPT07", "PPT08", "PPT09","PPT10", "PPT11") 
# precipitation and MDC and temperature and MDC are quite correlated so I'm leaving this combination of variables out. 
...
# lets look at fit of the Valid (validation) dataset
dat_test$model1_predict <- predict.glm(model1,newdata = dat_test,type="response")
roc_obj <- roc(dat_test$fire, dat_test$model1_predict)
auc(roc_obj)
table.glm.climate.simple[1,4]<-auc(roc_obj)
...
for (i in 1: length(variables)){
  ...
  # lets look at fit of the Valid (validation) dataset
  dat_test1$model1_predict <- predict.glm(model1,newdata = dat_test1,type="response")
  roc_obj <- roc(dat_test1$fire, dat_test1$model1_predict)
  auc(roc_obj)
  table.glm.climate.simple[i+1,4]<-auc(roc_obj)
