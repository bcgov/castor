...
fire_veg_02_07[duplicated(fire_veg_2002$idno),]$fire_yr # there should be no DUPLICATES

st_crs(fire_veg_02_07)
...
fire_veg_02_11<-rbind(rbind(rbind(rbind(fire_veg_2008_joinedVRI2009, fire_veg_2009), fire_veg_2010), fire_veg_2011),fire_veg_02_07)

fire_veg_02_11[duplicated(fire_veg_02_11$idno),]$fire_yr

# having problems with combining the files and Im not sure why.
# when I try to combine the files its giving me problems so some of them I join manually and then it seems to work. WEIRD! 
y3<-y1[16:24]
file.list<-y3
mkFrameList <- function(nfiles) {
  d <- lapply(seq_len(nfiles),function(i) {
    eval(parse(text=file.list[i]))
  })
  do.call(rbind,d)
}
n1<-length(y3)
fire_veg13_20<-mkFrameList(n1)

fire_veg13_20[duplicated(fire_veg13_20$idno),]$fire_yr
table(fire_veg13_20$fire_yr)
dim(fire_veg13_20)


fire_veg_2012_2<-rbind(fire_veg_2012_joinedVRI2013, fire_veg_2012)

# note I messed up dead_stand_volume. i meant to sample dead_stand_volume_175 but I by mistake sampled dead_stand_volume_125 for all years except 2021. 
fire_veg_2012_3<-fire_veg_2012_2 %>% select(!dead_stand_volume_125)
fire_veg_02_11a<-fire_veg_02_11 %>% select(!dead_stand_volume_175)
fire_veg13_20a<-fire_veg13_20 %>% select(!dead_stand_volume_125)
fire_veg_2021a<-fire_veg_2021 %>% select(!dead_stand_volume_175)

fire_veg<-rbind(rbind(rbind(fire_veg_02_11a, fire_veg_2012_3),fire_veg13_20a),fire_veg_2021a)
