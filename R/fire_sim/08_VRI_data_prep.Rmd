...
```{r}
file.list1<-list.files("C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\QGIS_analysis\\For_analysis_2022_08_23", pattern="fire_veg", all.files=FALSE, full.names=FALSE)
y1<-gsub(".gpkg","",file.list1)
the_dir <- "C:\\Work\\caribou\\clus_data\\Fire\\Fire_sim_data\\QGIS_analysis\\For_analysis_2022_08_23"
...
n<-length(y2)
fire_veg_02_07<-mkFrameList(n) 
fire_veg_02_07[duplicated(fire_veg_02_07$idno),] # there should be no DUPLICATES

# remove(age_1:height_2 columns) because they dont seem to be in later VRI's
fire_veg_02_07$dead_stand_volume_175<-NA
fire_veg_2008_joinedVRI2009$whole_stem_biomass_per_ha<-NA
fire_veg_2008_joinedVRI2009$branch_biomass_per_ha<-NA
fire_veg_2008_joinedVRI2009$foliage_biomass_per_ha<-NA
fire_veg_2008_joinedVRI2009$bark_biomass_per_ha<-NA
...
y3<-y1[10:13]
y3<-c(y1[c(10, 12:17, 19:26)], "fire_veg_02_06")
mkFrameList <- function(nfiles) {
  d <- lapply(seq_len(nfiles),function(i) {
