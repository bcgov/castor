---
title: "06a_Fire ignition model fit by BEC zone"
...
## Pull in the data for both lightning and person caused fires. 
...
This will be done separatey for trees and non-treed areas. 

First, let's do this for treed areas.


```{r}
#Select the zones for which we have the best climate variables for
...
zones1<-c("ESSF", "MH", "CMA", "MS", "SBPS", "BWBS", "SBS", "SWB") #"CDF", "BAFA", "IDF"
zones2<-c("ICH", "CWH",   "PP",  "IMA",  "BG") #"CDF", "BAFA", "IDF"
...
variables_02<-c("climate1", "climate1", "climate1", "climate1", "climate1")
variables<-c("proj_height_1", "proj_age_1", "live_stand_volume_125", "slope", "aspect") #remove vegtype

##For demo
variables_02<-c("climate1", "climate1", "climate1", "climate1", "climate1", "climate1")
variables<-c("proj_height_1", "proj_age_1", "live_stand_volume_125", "slope", "aspect", "vegtype") #remove vegtype

##vegtype issue is that training and validation data sets have different levels. Find a way to subset these to ensure that levels of each level are included in both data sets.
#https://rdrr.io/cran/caret/man/createDataPartition.html



variable_climate<-c("climate1")
variable_projH<-c("proj_height_1")
variable_projA<-c("proj_age_1")
variable_vol<-c("live_stand_volume_125")
variable_slope<-c("slope")
variable_aspect<-c("aspect")
variable_veg<-c("vegtype")
...
for (i in 1: length(variables_02)){
  ...
  model_dat<- dat2 %>% dplyr::select(fire_pres, variables_02[i], variables[i])
  # Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(model_dat$fire_pres, p = prop,
                                    list = FALSE,
                                    times = 1)
