...
#FRT 9
p_9 <- ggplot(dat_lightning %>% filter(frt=="9"), aes(PPT09
, as.numeric(fire))) +
  geom_smooth(method="glm", formula=y~x,
              method.args=list(family="binomial"),
              alpha=0.3, size=1) +
...
#FRT 10
p_10_1 <- ggplot(dat_lightning %>% filter(frt=="10"), aes(Tave08, as.numeric(fire))) +
  geom_smooth(method="glm", formula=y~x,
              method.args=list(family="binomial"),
              alpha=0.3, size=1) +
  geom_point(position=position_jitter(height=0.03, width=0)) +
...
# generally there is no change in the impact of the climate variable over time. 
...
table(dat_lightning$bclcs_level_2) ##What is L? It is Land, exclude and do not use or include it with not treed
