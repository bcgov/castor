...
ggplot(data=dat15, mapping=aes(x=as.factor(fire), y=log(infr_dist+1)))+
  geom_boxplot()

table(is.na(dat15$elevatn))

cor.test(dat15$climate1, dat15$elevatn) #0.72
cor.test(dat15$climate2, dat15$elevatn) #0.14
...
table(is.na(dat15$infr_dist))
table(is.na(dat15$FWI_veg))

mod_frt_15a<- glm(fire ~ climate1 + climate2 + FWI_veg + road_dist_m + infr_dist, data=dat15, family=binomial(link="logit"))
Anova(mod_frt_15a, type=3)

mod_frt15b<- glm(fire ~ climate1 + climate2 + FWI_veg + road_dist_m, data=dat15, family=binomial(link="logit"))

mod_frt15c<- glm(fire ~ climate1 + climate2 + FWI_veg + infr_dist, data=dat15, family=binomial(link="logit"))



mod_frt_15a_log<- glm(fire ~ climate1 + climate2 + FWI_veg + log(road_dist_m+1) + log(infr_dist+1), data=dat15, family=binomial(link="logit"))

mod_frt15b_log<- glm(fire ~ climate1 +climate2 + FWI_veg + log(road_dist_m+1), data=dat15, family=binomial(link="logit"))
anova(mod_frt_15a_log,mod_frt15b_log, test="Chisq") 

mod_frt_15c_log<- glm(fire ~ climate1 +climate2 + FWI_veg + log(infr_dist+1), data=dat15, family=binomial(link="logit"))
...
dat15$log_inf_dist<-log(dat15$infr_dist+1)
mod_frt15<- glm(fire ~ 1 + climate1 + climate2 + FWI_veg + log_road_dist + log_inf_dist, data=dat15, family=binomial(link="logit"))
...
mod_frt15<- glm(fire ~ climate1 + climate2 + FWI_veg + log_road_dist + log_inf_dist, data=dat15, family=binomial(link="logit"))
...
mod_frt15<- glm(fire ~ climate1 + climate2 + log_road_dist + log_inf_dist + as.factor(FWI_veg), data=dat15, family=binomial(link="logit"))
...
dat15$fire_veg<-paste(dat15$fire, dat15$FWI_veg)
```
...
#Model   
mod<-glm(fire ~ climate1 + climate2 + log_road_dist + log_inf_dist + FWI_veg, data=dat1, family=binomial(link="logit"))
...
top_mod_table_FRT5_person[1,3]<-"fire ~ climate1 + climate2 + log_road_dist + inf_dist + FWI_veg"
