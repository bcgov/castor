...
# FRT 12
dat15<- dat_person %>% dplyr::filter(frt == 15) 
table(dat15$FWI_veg)

# there are only 3 observations in the C1 fuel type
dat15$FWI_veg[dat15$FWI_veg=="S-1"]<-"S-3"
dat15$FWI_veg[dat15$FWI_veg=="S-2"]<-"S-3"# C_1 and C-3 are apparently similar in spread rates so will change C-1 to C-3
 # remove water and non-veg type locations

dat15<- dat15 %>% filter(!FWI_veg %in%  c("N", "W"))
...
ggplot(data=dat15, mapping=aes(x=as.factor(fire), y=elevatn))+
  geom_boxplot()

ggplot(data=dat15, mapping=aes(x=as.factor(fire), y=log(rd_dst_+1)))+
  geom_boxplot()

ggplot(data=dat15, mapping=aes(x=as.factor(fire), y=log(infr_ds+1)))+
  geom_boxplot()

table(is.na(dat15$elevatn))

dat15_2<- dat15 %>% filter(!is.na(elevatn))
table(is.na(dat15_2$elevatn))


mod_frt_15a<- glm(fire ~ climate1 + climate2 + elevatn + FWI_veg + rd_dst_ + infr_ds, data=dat15_2, family=binomial(link="logit"))

mod_frt15b<- glm(fire ~ climate1 + climate2 + elevatn + FWI_veg + rd_dst_, data=dat15_2, family=binomial(link="logit"))

mod_frt15c<- glm(fire ~ climate1 + climate2 + elevatn + FWI_veg + infr_ds, data=dat15_2, family=binomial(link="logit"))

mod_frt_15a_log<- glm(fire ~ climate1 + climate2 + elevatn + FWI_veg + log(rd_dst_+1) + log(infr_ds+1), data=dat15_2, family=binomial(link="logit"))

mod_frt15b_log<- glm(fire ~ climate1 +climate2 + elevatn + FWI_veg + log(rd_dst_+1), data=dat15_2, family=binomial(link="logit"))

mod_frt_15c_log<- glm(fire ~ climate1 +climate2 + elevatn + FWI_veg + log(infr_ds+1), data=dat15_2, family=binomial(link="logit"))

# best model is mod_frt_5a_log. It has the lowest AIC (AIC=138.1)

dat15_2$log_road_dist<-log(dat15_2$rd_dst_+1)
dat15_2$log_inf_dist<-log(dat15_2$infr_ds+1)
mod_frt15<- glm(fire ~ climate1 + climate2 + elevatn + FWI_veg + log_road_dist + log_inf_dist, data=dat15_2, family=binomial(link="logit"))


summary(mod_frt15)
Anova(mod_frt_15a_log, type=3)
...
dat15_2$resids<-resid(mod_frt15)

binnedplot (dat15_2$climate1, 
            dat15_2$resids, 
            nclass = NULL, 
            xlab = "climate 1", 
            ylab = "Average residual", 
...
dat2_b<-dat15_2 %>% dplyr::select(fire, elevatn, climate1, climate2, log_road_dist, log_inf_dist)
...
dat15_3<- dat15_2 %>% filter(resids<3)

mod_frt15<- glm(fire ~ climate1 + climate2 + elevatn + FWI_veg + log_road_dist + log_inf_dist, data=dat15_3, family=binomial(link="logit"))
...
mod_frt15<- glm(fire ~ climate1 + climate2 + elevatn + log_road_dist + log_inf_dist + FWI_veg, data=dat15_3, family=binomial(link="logit"))

summary(mod_frt15)

#Create a new blank table and get AUC too
top_mod_table_FRT5_person_All <- data.frame (matrix (ncol = 15, nrow = 0))
colnames (top_mod_table_FRT5_person_All ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1", "coef_climate2", "coef_elevatn", "coef_log_road_dist", "coef_log_inf_dist", "coef_FWI_veg_C-5", "coef_FWI_veg_D-1/2", "coef_FWI_veg_M-1/2", "coef_FWI_veg_O-1a/b", "coef_FWI_veg_S-3", "AUC")

dat15_3$fire_veg<-paste(dat15_3$fire, dat15_3$FWI_veg)
```
...
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(dat15_3$fire_veg, p = prop,
                                    list = FALSE,
                                    times = 1)
  
   dat1 <- dat15_3[ trainIndex,]
   Valid <- dat15_3[-trainIndex,]
...
colnames (top_mod_table_FRT5_person ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1", "coef_climate2", "coef_elevatn", "coef_log_road_dist", "coef_inf_dist", "coef_FWI_veg_C-5", "coef_FWI_veg_D-1/2", "coef_FWI_veg_M-1/2", "coef_FWI_veg_O-1a/b", "coef_FWI_veg_S-3", "AUC")

##Add data for NDT1
top_mod_table_FRT5_person[1,1]<-"person"
top_mod_table_FRT5_person[1,2]<-"FRT14"
top_mod_table_FRT5_person[1,3]<-"fire ~ climate1 + elevatn + log_road_dist + inf_dist + FWI_veg"
