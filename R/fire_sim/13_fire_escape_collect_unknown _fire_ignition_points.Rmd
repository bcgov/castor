...
fire_all<- fire_all %>% dplyr::mutate(yearsSinceHarvest = FIRE_YEAR - yearHarvest)
...
fire_all$yearsSinceDisturbance<- ifelse (fire_all$FIRE_YEAR > fire_all$yearDisturb, fire_all$FIRE_YEAR - fire_all$yearDisturb, NA)
...
fire_all_reduced<-fire_all1 %>% dplyr::select(FIRE_YEAR:FIRE_CAUSE, LATITUDE:CURRENT_SIZE, ig_mnth:dist_roads_m, dist_infrastructure_m:idno, Tmax02:RH11, CMI02:CMI11, MDC_02:MDC_11, FWI_veg, conifer_pct_cover_total, yearHarvest:yearsSinceDisturbance)

table(is.na(fire_all_reduced$FWI_veg))

fire_all_reduced<-fire_all_reduced %>% drop_na(FWI_veg)

```


```{r}
st_write(fire_all_reduced, "C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\escape_weather_veg.gpkg", overwrite=TRUE)

write.csv(fire_all2, "C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\unknown_ignition_weather_veg.csv")
```


```{r}
unknown<- st_read("C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\unknown_ignition_weather_veg.gpkg")

#fire_escapes3<- fire_all1
fire_escapes3<-st_read("C:\\Work\\caribou\\castor_data\\Fire\\Fire_sim_data\\data\\Fire_data_all_veg2.gpkg")
