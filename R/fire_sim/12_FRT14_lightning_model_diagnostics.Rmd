...
table(dat14$fwveg, dat14$fire)

dat14<-dat14 %>% filter(!fwveg %in% c("W", "N"))
dat14$fwveg[dat14$fwveg=="M-3"]<-"M-1/2" 
dat14$fwveg <- factor(dat14$fwveg, levels=c("C-7","C-2","C-3","C-5","D-1/2", "M-1/2", "O-1a/b", "S-1"))
...
ggplot(data=dat14, mapping=aes(x=as.factor(fire), y=dm_h_bc))+
  geom_boxplot()

cor.test(dat14$dm_h_bc, dat14$climate1)

mod.frt.14<-glm(fire ~ climate1 + fwveg + dm_h_bc, data=dat14, family=binomial(link="logit"))
Anova(mod.frt.14, type=3)

mod.frt.14b<-glm(fire ~ climate1 + dm_h_bc, data=dat14, family=binomial(link="logit"))
Anova(mod.frt.14b, type=3)

# model diagnostic plots
binnedplot (fitted(mod.frt.14b), 
            residuals(mod.frt.14b), 
            nclass = NULL, 
...
dat14$resids<-resid(mod.frt.14b)
...
##Partial Residuals
visreg(mod.frt.14b, scale="response") # nicer plots than the ones below!

visreg(mod.frt.14b, "climate1", by="dm_h_bc", scale="response")
...
dat2_b<-dat14%>% dplyr::select(fire, climate1, dm_h_bc)
probabilities <- predict(mod.frt.14b, type = "response")
...
plot(mod.frt.14b, which = 6, id.n = 3)
...
top_mod_table_FRT12_lightningALL <- data.frame (matrix (ncol = 7, nrow = 0))
colnames (top_mod_table_FRT12_lightningALL ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept",
                                                   "coef_climate_1", 
                                                   "coef_dm_h_bc",
                                                   "AUC")

dat14$fire_veg<-paste(dat14$fire, dat14$fwveg)
```
...
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(dat14$fire, p = prop,
                                    list = FALSE,
                                    times = 1)
...
#Model   
mod<-glm(fire ~ climate1 + dm_h_bc, data=dat1, family=binomial(link="logit"))
...
top_mod_table_FRT12_lightningL <- data.frame (matrix (ncol = 7, nrow = 0))
...
top_mod_table_FRT12_lightningL[1,3]<-"fire ~ climate1 + dm_h_bc"
top_mod_table_FRT12_lightningL[1,4]<- coef(mod)[1] #Intercept
top_mod_table_FRT12_lightningL[1,5]<- coef(mod)[2] #Climate variable 1
top_mod_table_FRT12_lightningL[1,6]<- coef(mod)[3]
top_mod_table_FRT12_lightningL[1,7]<- mod.auc
...
FRT12_summary_table_mean$Model_terms<-"fire ~ climate1 + dm_h_bc"
