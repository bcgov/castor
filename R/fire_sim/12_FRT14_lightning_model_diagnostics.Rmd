...
dat14<- dat %>% dplyr::filter(frt == 14) 
table(dat14$FWI_veg)

dat14<-dat14 %>% filter(FWI_veg!="N")
...
#dat14$FWI_veg[dat14$FWI_veg=="C-4"]<-"C-2"
## changing M-3 to M-1/2 because there are only 6 occurences of M-3 so its hard to run a model on this.
#dat14$FWI_veg[dat14$FWI_veg=="M-3"]<-"M-1/2"
table(dat14$FWI_veg, dat14$fire)
...
table(is.na(dat14$fire))
mod.frt.14<-glm(fire ~ climate1 + FWI_veg + elevatn, data=dat14, family=binomial(link="logit"))

# looking at difference in link function
modclim_logit<-glm(fire ~ climate1, data=dat14, family=binomial(link="logit"))
modclim_cloglog<-glm(fire ~ climate1, data=dat14, family=binomial(link="cloglog"))
...
plot(x=dat14$climate1, y=dat14$fire)
lines(mydata$climate1, Pred_logit, col="red")
lines(mydata$climate1, Pred_cloglog, col="blue")

dat14$FWI_veg<- as.factor(dat14$FWI_veg)


mod.frt.14<-glm(fire ~ climate1 + FWI_veg + elevatn, data=dat14, family=binomial(link="logit"))
cor(model.matrix(mod.frt.14)[,-1])

# note that climate1 and elevation are correlated. Im going to have to remove one of these variables
mod.frt.14a<-glm(fire ~ climate1 + FWI_veg, data=dat14, family=binomial(link="logit"))
mod.frt.14b<-glm(fire ~ FWI_veg + elevatn, data=dat14, family=binomial(link="logit"))

# removing either climate1 or elecation results in the same AIC so Ill get rid of elevation and stick with climate
mod.frt.14<-glm(fire ~ climate1 + FWI_veg, data=dat14, family=binomial(link="logit"))

hist(dat14$climate1)
hist(dat14$elevatn)
summary(mod.frt.14)

Anova(mod.frt.14, type=3)

# model diagnostic plots
binnedplot (fitted(mod.frt.14), 
            residuals(mod.frt.14), 
            nclass = NULL, 
...
##Partial Residuals
visreg(mod.frt.14, scale="response") # nicer plots than the ones below!

visreg(mod.frt.14, "climate1", by="elevatn", scale="response")
visreg(mod.frt.14, "climate1", by="climate2", scale="response")
visreg(mod.frt.14, "climate1", by="FWI_veg", scale="response")
visreg(mod.frt.14, "elevatn", by="FWI_veg", scale="response")
visreg(mod.frt.14, "elevatn", by="climate2", scale="response")
visreg(mod.frt.14, "climate2", by="FWI_veg", scale="response")
visreg(mod.frt.14, "climate1", by="FWI_veg", overlay=TRUE, scale="response")
visreg(mod.frt.14, "climate1", by="elevatn", overlay=TRUE, scale="response")
...
dat2_b<-dat14%>% dplyr::select(fire, climate1)
probabilities <- predict(mod.frt.14, type = "response")
...
plot(mod.frt.14, which = 6, id.n = 3)
...
library(broom)
model.data <- augment(mod.frt.14) %>% 
  mutate(index = 1:n()) 

#The data for the top 3 largest values, according to the Cook’s distance, can be displayed as follow:
model.data %>% 
  dplyr::select(climate1, FWI_veg,.std.resid, .cooksd) %>%
  top_n(5, .cooksd) 
...
top_mod_table_FRT12_lightningALL <- data.frame (matrix (ncol = 14, nrow = 0))
colnames (top_mod_table_FRT12_lightningALL ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1", "coef_FWI_veg_C-3",  "coef_FWI_veg_C-5",  "coef_FWI_veg_C-7", "coef_FWI_veg_D-1/2", "coef_FWI_veg_M-1/2", "coef_FWI_veg_M-3", "coef_FWI_veg_O-1a/b", "coef_FWI_veg_S-1", "AUC")
...
top_mod_table_FRT12_lightningL <- data.frame (matrix (ncol = 14, nrow = 0))
colnames (top_mod_table_FRT12_lightningL ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1", "coef_FWI_veg_C-3",  "coef_FWI_veg_C-5",  "coef_FWI_veg_C-7", "coef_FWI_veg_D-1/2", "coef_FWI_veg_M-1/2", "coef_FWI_veg_M-3", "coef_FWI_veg_O-1a/b", "coef_FWI_veg_S-1", "AUC")


##Add data for NDT1
top_mod_table_FRT12_lightningL[1,1]<-"lightning"
top_mod_table_FRT12_lightningL[1,2]<-"FRT14"
top_mod_table_FRT12_lightningL[1,3]<-"fire ~ climate1 + FWI_veg"
...
top_mod_table_FRT12_lightningL[1,13]<- coef(mod)[10]
top_mod_table_FRT12_lightningL[1,14]<- mod.auc
...
FRT12_summary_table_mean$FRT<-14
FRT12_summary_table_mean$Model_terms<-"fire ~ climate1 + FWI_veg"
...
```{r}
write.csv(FRT12_summary_table_mean, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_FRT14_lightning.csv")
```
