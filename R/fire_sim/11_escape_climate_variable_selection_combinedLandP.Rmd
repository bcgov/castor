...
Escape_data_lightning[setdiff(names(Escape_data_person), names(Escape_data_lightning))] <- NA
Escape_data_person[setdiff(names(Escape_data_lightning), names(Escape_data_person))] <- NA

Escape_data_ALL_LandP<- rbind(Escape_data_lightning, Escape_data_person) #Combine all groups!
...
```{r}
table(Escape_data_ALL_LandP$fire_yr, Escape_data_ALL_LandP$escape)
table(Escape_data_ALL_LandP$fire_yr, Escape_data_ALL_LandP$escape, Escape_data_ALL_LandP$zone)

table(Escape_data_ALL_LandP$fire_yr, Escape_data_ALL_LandP$escape, Escape_data_ALL_LandP$ntrl_ds)

str(Escape_data_ALL_LandP)
...
for (h in 1:length(zones_escape)) {
  dat2<- Escape_data_ALL_LandP %>% dplyr::filter(ntrl_ds ==zones_escape[h])
...
```{r}
#Save AIC table
write.csv(aic_bec_escape_summary2, file="D:\\Fire\\fire_data\\raw_data\\ClimateBC_Data\\escape_climate_AIC_results_allData.csv")
```

Top model for NDTs 1,2,3 and 5 is tave09xppt09; For NDT4 it is mdc_06. Let's use all these variables in model selection?

```{r}
#Save csv
write.csv(Escape_data_ALL_LandP, file="D:\\Fire\\fire_data\\raw_data\\ClimateBC_Data\\Escape_data_ALL_LandP_df.csv")
...
```{r}
table(Escape_data_ALL_LandP$fire_yr, Escape_data_ALL_LandP$escape, Escape_data_ALL_LandP$zone)
escaped_table_BEC<-table(Escape_data_ALL_LandP$fire_yr, Escape_data_ALL_LandP$escape, Escape_data_ALL_LandP$zone)
