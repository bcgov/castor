...
dat10$fwveg[dat10$fwveg=="M-3"]<-"M-1/2"
dat10$fwveg[dat10$fwveg=="S-2"]<-"S-1"
dat10$fwveg[dat10$fwveg=="S-3"]<-"S-1"
dat10<-dat10 %>% filter(!fwveg %in% c("N", "W"))
table(dat10$fwveg)

ggplot(data=dat10, mapping=aes(x=as.factor(fire), y=climate1))+
  geom_boxplot()

ggplot(data=dat10, mapping=aes(x=as.factor(fire), y=climate2))+
  geom_boxplot()
ggplot(data=dat10, mapping=aes(x=as.factor(fire), y=dm_h_bc))+
  geom_boxplot()

cor.test(dat10$climate1, dat10$dm_h_bc)
cor.test(dat10$climate2, dat10$dm_h_bc)
cor.test(dat10$climate1, dat10$climate2)

dat10$fwveg<- as.factor(dat10$fwveg)

mod.frt.10<-glm(fire ~ climate1 + climate2 + fwveg + dm_h_bc, data=dat10, family=binomial(link="logit"))
...
mod.frt.10<-glm(fire ~ climate1 + climate2 + dm_h_bc, data=dat10, family=binomial(link="logit"))
Anova(mod.frt.10, type=3)
...
visreg(mod.frt.10, "dm_h_bc", by="climate2", scale="response")
visreg(mod.frt.10, "climate1", by="dm_h_bc", overlay=TRUE, scale="response")
...
dat2_b<-dat10%>% dplyr::select(fire, dm_h_bc, climate1, climate2)
...
model.data %>% 
  dplyr::select(climate1,climate2,  dm_h_bc,.std.resid, .cooksd) %>%
  top_n(5, .cooksd) 
...
top_mod_table_FRT10_lightningALL <- data.frame (matrix (ncol = 8, nrow = 0))
colnames (top_mod_table_FRT10_lightningALL ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1", "coef_climate_2", "coef_dm_h_bc", "AUC")

dat10$fire_veg<-paste(dat10$fire, dat10$fwveg)
```
...
#Model   
mod<-glm(fire ~ climate1 + climate2 + dm_h_bc, data=dat1, family=binomial(link="logit"))
...
top_mod_table_FRT10_lightningL <- data.frame (matrix (ncol = 8, nrow = 0))
colnames (top_mod_table_FRT10_lightningL ) <- 
  c ("CAUSE", "FRT",  "Model_terms", "intercept","coef_climate_1", "coef_climate_2", "coef_dm_h_bc", "AUC")


##Add data for NDT1
top_mod_table_FRT10_lightningL[1,1]<-"lightning"
top_mod_table_FRT10_lightningL[1,2]<-"FRT10"
top_mod_table_FRT10_lightningL[1,3]<-"fire ~ climate1 + climate2 + dm_h_bc"
...
top_mod_table_FRT10_lightningL[1,8]<-  mod.auc
...
FRT10_summary_table_mean$Model_terms<-"fire ~ climate1 + climate2 + dm_h_bc"
