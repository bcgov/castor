...
dat10<- dat %>% dplyr::filter(frt == 10) 
table(dat10$FWI_veg)

dat10_2<-dat10 %>% filter(FWI_veg!="S-2")
table(dat10_2$FWI_veg)



ggplot(data=dat10_2, mapping=aes(x=as.factor(fire), y=climate1))+
  geom_boxplot()

ggplot(data=dat10_2, mapping=aes(x=as.factor(fire), y=elevatn))+
  geom_boxplot()

mod.frt.10<-glm(fire ~ climate1 +climate2 + FWI_veg + elevatn, data=dat10_2, family=binomial(link="logit"))

# looking at difference in link function
modclim_logit<-glm(fire ~ climate1, data=dat10_2, family=binomial(link="logit"))
modclim_cloglog<-glm(fire ~ climate1, data=dat10_2, family=binomial(link="cloglog"))
...
plot(x=dat10_2$climate1, y=dat10_2$fire)
lines(mydata$climate1, Pred_logit, col="red")
lines(mydata$climate1, Pred_cloglog, col="blue")

dat10_2$FWI_veg<- as.factor(dat10_2$FWI_veg)


mod.frt.10<-glm(fire ~ climate1 + climate2 + FWI_veg + elevatn, data=dat10_2, family=binomial(link="logit"))
cor(model.matrix(mod.frt.10)[,-1])

hist(dat10_2$climate1)
hist(dat10_2$elevatn)
summary(mod.frt.10)
...
dat10_2$resids<-resid(mod.frt.10)

binnedplot (dat10_2$climate1, 
            dat10_2$resids, 
            nclass = NULL, 
            xlab = "climate1", 
            ylab = "Average residual", 
...
visreg(mod.frt.10, "climate1", by="elevatn", scale="response")
visreg(mod.frt.10, "climate1", by="climate2", scale="response")
visreg(mod.frt.10, "climate1", by="FWI_veg", scale="response")
visreg(mod.frt.10, "elevatn", by="FWI_veg", scale="response")
visreg(mod.frt.10, "elevatn", by="climate2", scale="response")
visreg(mod.frt.10, "climate2", by="FWI_veg", scale="response")
visreg(mod.frt.10, "climate1", by="FWI_veg", overlay=TRUE, scale="response")
visreg(mod.frt.10, "climate1", by="elevatn", overlay=TRUE, scale="response")
...
dat2_b<-dat10_2%>% dplyr::select(fire, elevatn, climate1, climate2)
...
model.data %>% 
  dplyr::select(climate1,climate2,  elevatn, FWI_veg,.std.resid, .cooksd) %>%
  top_n(5, .cooksd) 
...
top_mod_table_FRT10_lightningALL <- data.frame (matrix (ncol = 16, nrow = 0))
colnames (top_mod_table_FRT10_lightningALL ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1", "coef_climate_2", "coef_FWI_veg_C-3",  "coef_FWI_veg_C-5",  "coef_FWI_veg_C-7", "coef_FWI_veg_D-1/2", "coef_FWI_veg_M-1/2", "coef_FWI_veg_M-3", "coef_FWI_veg_N", "coef_FWI_veg_O-1a/b","coef_elevatn", "AUC")

dat10_2$fire_veg<-paste(dat10_2$fire, dat10_2$FWI_veg)
```
...
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(dat10_2$fire_veg, p = prop,
                                    list = FALSE,
                                    times = 1)
  
   dat1 <- dat10_2[ trainIndex,]
   Valid <- dat10_2[-trainIndex,]
...
top_mod_table_FRT10_lightningL <- data.frame (matrix (ncol = 16, nrow = 0))
colnames (top_mod_table_FRT10_lightningL ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1", "coef_climate_2", "coef_FWI_veg_C-3",  "coef_FWI_veg_C-5",  "coef_FWI_veg_C-7", "coef_FWI_veg_D-1/2", "coef_FWI_veg_M-1/2", "coef_FWI_veg_M-3", "coef_FWI_veg_N", "coef_FWI_veg_O-1a/b","coef_elevatn", "AUC")


##Add data for NDT1
top_mod_table_FRT10_lightningL[1,1]<-"lightning"
top_mod_table_FRT10_lightningL[1,2]<-"FRT10"
top_mod_table_FRT10_lightningL[1,3]<-"fire ~ climate1 + climate2 + FWI_veg + elevatn"
...
top_mod_table_FRT10_lightningL[1,13]<- coef(mod)[10] #pr
top_mod_table_FRT10_lightningL[1,14]<- coef(mod)[11] #coeffic
top_mod_table_FRT10_lightningL[1,15]<- coef(mod)[12] #coeffic
top_mod_table_FRT10_lightningL[1,16]<- mod.auc
...
```{r}
write.csv(top_mod_table_FRT10_lightningALL, file="D:\\Fire\\fire_data\\raw_data\\Ignition Models\\Ignition_Model_Results_Lightning_Treed\\top_mod_table_FRT5_light_tree_all.csv")
```
...
FRT10_summary_table_mean$FRT<-10
...
```{r}
write.csv(FRT10_summary_table_mean, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_FRT10_lightning.csv")
```
