...
Escape_data_ALL<-Escape_data_ALL %>% st_drop_geometry()
...
Escape_data_ALL$mean_PPT05_PPT06<- (Escape_data_ALL$PPT05 + Escape_data_ALL$PPT06)/2
...
Escape_data_ALL2<-Escape_data_ALL %>% filter(!FWI_veg %in% c("W"))
...
variables_all<-c(climate1 = "climate1", climate2 = "climate2", slope = "slope", elevatn ="elevatn", infr_dist = "infr_dist", road_dist_m = "road_dist_m", win_spg = "win_spg", win_sum = "win_sum", FWI_veg = "FWI_veg" ) 

vars.clim.vegtype2<-c("climate1", "FWI_veg")
vars.clim.vegtype2b<-c("climate1", "climate2", "FWI_veg")
vars.topo<-c("slope", "elevatn", "win_spg", "win_sum")
vars.infra<-c("infr_dist", "road_dist_m")
...
```{r}
zones1<-c(5, 7, 9, 11, 13, 14) #Do one zone at a time
prop<-0.75
Escape_data_ALL2$veg_escape<-paste(Escape_data_ALL2$escape10, Escape_data_ALL2$FWI_veg)
...
#Create empty table
zones1<-c(10, 12, 15) #Do one zone at a time
...
# NOTE I HAVE NOT RE_DONE THIS ANALYSIS FOR FRT 5, 7, 9, 11. MUST REDO!!
#### FRT 5
```{r}
...
model_ft5<-glm(escape10 ~ climate1 + FWI_veg + slope + elevatn + win_spg + road_dist_m +infr_dist + slope:elevatn, family = binomial, data = dat1)
...
#Remove least term ~ climate1:climate2
model_ft9c<-glm(escape10 ~ climate1 + FWI_veg  + slope + elevatn + rd_dst_ + fire_cs, family = binomial, data = dat1)

AIC(model_ft9c) #103.0267
...
#Remove least term ~ wind_atfire
model_ft9d<-glm(escape10 ~ climate1 + FWI_veg  + slope + rd_dst_ + fire_cs, family = binomial, data = dat1)

AIC(model_ft9d) #101.3001
...
#Remove least term ~ slope
model_ft9e<-glm(escape10 ~ climate1 + as.factor(FWI_veg)  + slope + rd_dst_, family = binomial, data = dat1)

AIC(model_ft9e) #100.7179
...
#######################
#Remove least term ~ slope
model_ft9e<-glm(escape10 ~ climate1 + as.factor(FWI_veg)  + slope + rd_dst_, family = binomial, data = escape_frt9)
...
#Create a new blank table and get AUC too
top_mod_table_FRT9_ALL <- data.frame (matrix (ncol = 11, nrow = 0))
...
#Model   
model.FRT9<-glm(escape10 ~ climate1 + as.factor(FWI_veg)  + slope + rd_dst_, family = binomial, data = dat1) 
...
# create model table (only do this once) and add the relevant data
top_mod_table_FRT9 <- data.frame (matrix (ncol = 11, nrow = 0))
...
top_mod_table_FRT9[1,2]<-"escape ~ climate1 + FWI_veg + slope + rd_dst" 
...
top_mod_table_FRT9[1,6]<- coef(model.FRT9)[4] #coefficient 
top_mod_table_FRT9[1,7]<- coef(model.FRT9)[5] #coefficient 
top_mod_table_FRT9[1,8]<- coef(model.FRT9)[6] #coefficient
top_mod_table_FRT9[1,9]<- coef(model.FRT9)[7] #
top_mod_table_FRT9[1,10]<- coef(model.FRT9)[8] #coefficient 
top_mod_table_FRT9[1,11]<- mod.auc
...
summary_top_mod_table_FRT9_ALL$Model_terms<-"escape ~ climate1 + FWI_veg + slope + rd_dst" 
...
escape_frt10<- Escape_data_ALL2 %>% filter(Cluster==10)
table(escape_frt10$FWI_veg)
escape_frt10$FWI_veg[escape_frt10$FWI_veg=="S-1"]<-"S-2"
escape_frt10$FWI_veg[escape_frt10$FWI_veg=="M-3"]<-"O-1a/b"
...
table(is.na(escape_frt10$escape10))

cor.test(escape_frt10$road_dist_m, escape_frt10$infr_dist)
cor.test(escape_frt10$climate1, escape_frt10$climate2)
cor.test(escape_frt10$climate1, escape_frt10$elevatn)
cor.test(escape_frt10$elevatn, escape_frt10$climate2)


#Run model using dat1
model_ft10<-glm(escape10 ~ climate1 + climate2 + FWI_veg  + slope + elevatn + road_dist_m +infr_dist + win_sum, family = binomial, data = escape_frt10)

AIC(model_ft10) #572.1238
Anova(model_ft10, type=3) 

#Remove least significant interaction
model_ft10b<-glm(escape10 ~ climate1 + climate2 + FWI_veg  + slope + elevatn + road_dist_m +infr_dist, family = binomial, data = escape_frt10)

AIC(model_ft10b) #570.2661
Anova(model_ft10b, type=3) 

#Remove least term ~ elevatn
model_ft10c<-glm(escape10 ~ climate1 + climate2 + FWI_veg  + slope + road_dist_m +infr_dist, family = binomial, data = escape_frt10)

AIC(model_ft10c) #568.6153
Anova(model_ft10c, type=3) 

#Remove least term ~ elevatn
model_ft10d<-glm(escape10 ~ climate1 + climate2  + slope + road_dist_m +infr_dist, family = binomial, data = escape_frt10)

AIC(model_ft10d) #558.4926
Anova(model_ft10d, type=3) 


#Remove least term ~ elevatn
model_ft10d<-glm(escape10 ~ climate1 + climate2  + road_dist_m +infr_dist, family = binomial, data = escape_frt10)

AIC(model_ft10d) #559.0828
Anova(model_ft10d, type=3) 
...
escape_frt10$log_road<-log(escape_frt10$road_dist_m+1)
model_ft10e<-glm(escape10 ~ climate1 + climate2  + log_road +infr_dist, family = binomial, data = escape_frt10)

hist(escape_frt10$climate2)
hist(escape_frt10$slope)
hist(log(escape_frt10$road_dist_m+1))
hist(escape_frt10$infr_dist)

table(escape_frt10$escape10) #0 = 948 and 1 = 267 not bad actually.

# model diagnostic plots
binnedplot (fitted(model_ft10e), 
            residuals(model_ft10e), 
            nclass = NULL, 
            xlab = "Expected Values", 
            ylab = "Average residual")


escape_frt10$resids<-resid(model_ft10e)

binnedplot (escape_frt10$climate1, 
            escape_frt10$resids,  
            nclass = NULL)

binnedplot (escape_frt10$climate2, 
            escape_frt10$resids,  
            nclass = NULL)

binnedplot (escape_frt10$infr_dist, 
            escape_frt10$resids,  
            nclass = NULL)

binnedplot (log(escape_frt10$road_dist_m+1), 
            escape_frt10$resids,  
            nclass = NULL)
...
prop<-0.75
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(escape_frt10$escape10, p = prop,
                                    list = FALSE,
                                    times = 1)
...
#Model   
model.FRT10<-glm(escape10 ~ climate1 + climate2  + log_road +infr_dist, family = binomial, data = dat1)
...
top_mod_table_FRT10[1,2]<-"escape ~ climate1 + climate2  + log_road_dist + infra_dist" 
...
summary_top_mod_table_FRT10_ALL$Model_terms<-"escape ~ climate1 + climate2  + log_road_dist + infra_dist" 
...
#### FRT 11
```{r}
escape_frt11<- Escape_data_ALL2 %>% filter(Cluster==11)
table(escape_frt11$FWI_veg)
escape_frt11<- escape_frt11 %>% filter(FWI_veg!="N")
escape_frt11$FWI_veg[escape_frt11$FWI_veg=="O-1a/b"]<-"C-3"
#escape_frt11$FWI_veg[escape_frt11$FWI_veg=="S-2"]<-"M-1/2"
...
table(is.na(escape_frt11$wind_atfire))
table(is.na(escape_frt11$rd_dst_))
table(is.na(escape_frt11$slope))
table(is.na(escape_frt11$fire_cs))
table(is.na(escape_frt11$escape10))

#Divide data into training and valid
prop<-0.75
  trainIndex <- createDataPartition(escape_frt11$veg_escape, p = prop,
                                    list = FALSE,
                                    times = 1)
...
#Run model using dat1
model_ft11<-glm(escape10 ~ climate1 + FWI_veg  + elevatn + elevatn:wind_atfire + rd_dst_ + wind_atfire + fire_cs, family = binomial, data = escape_frt11)

AIC(model_ft11) #235.5254
Anova(model_ft11, type=3) 

#Remove least significant interaction
model_ft11b<-glm(escape10 ~ climate1 + FWI_veg  + elevatn + rd_dst_ + wind_atfire + fire_cs, family = binomial, data = escape_frt11)


AIC(model_ft11b) #233.645
Anova(model_ft11b, type=3) 

#Remove least term ~ FWI_veg
model_ft11c<-glm(escape10 ~ climate1  + elevatn + rd_dst_ + wind_atfire + fire_cs, family = binomial, data = escape_frt11)

AIC(model_ft11c) #223.6864
Anova(model_ft11c, type=3) 

#Remove least term ~ elevatn
model_ft11d<-glm(escape10 ~ climate1 + rd_dst_ + wind_atfire + fire_cs, family = binomial, data = escape_frt11)

AIC(model_ft11d) #221.7887
Anova(model_ft11d, type=3) 


#Remove least term ~ wind
model_ft11d<-glm(escape10 ~ climate1 + rd_dst_ + fire_cs, family = binomial, data = escape_frt11)

AIC(model_ft11d) #220.151
Anova(model_ft11d, type=3) 
...
#######################
#Remove least term ~ slope
model_ft11e<-glm(escape10 ~ climate1 + rd_dst_ + fire_cs, family = binomial, data = escape_frt11)

mod.valid <- predict.glm(model_ft11e, newdata=Valid, type="response")
   roc_obj <- roc(Valid[,"escape10"], mod.valid)
   mod.auc <- auc(roc_obj)
   mod.auc #0.7308 

hist(escape_frt11$climate1)
hist(escape_frt11$rd_dst_)

table(escape_frt11$escape10) #0 = 948 and 1 = 267 not bad actually.
table(escape_frt11$FWI_veg)
...
binnedplot (escape_frt11$rd_dst_, 
            escape_frt11$resids,  
            nclass = NULL)
...
#Model   
model.FRT11<-glm(escape10 ~ climate1 + rd_dst_ + fire_cs, family = binomial, data = dat1)
...
top_mod_table_FRT11[1,2]<-"escape ~ climate1 + rd_dst_ + fire_cs" 
...
top_mod_table_FRT11[1,6]<- coef(model.FRT11)[4] #coefficient 
top_mod_table_FRT11[1,7]<- coef(model.FRT11)[5] #coefficient 
...
summary_top_mod_table_FRT11_ALL$Model_terms<-"escape ~ climate2 + FWI_veg + slope + rd_dst" 
...
#### FRT 12
```{r}
escape_frt12<- Escape_data_ALL2 %>% filter(Cluster==12)
table(Escape_data_ALL2$Cluster)
dim(escape_frt12)
table(escape_frt12$escape10,escape_frt12$FWI_veg)
escape_frt12$FWI_veg[escape_frt12$FWI_veg=="C-4"]<-"C-2"
escape_frt12$FWI_veg[escape_frt12$FWI_veg=="S-3"]<-"M-1/2"
escape_frt12$FWI_veg[escape_frt12$FWI_veg=="S-1"]<-"M-1/2"
...
table(is.na(escape_frt12$fire_cs))
table(is.na(escape_frt12$escape10))

cor.test(escape_frt12$road_dist_m, escape_frt12$infr_dist)
cor.test(escape_frt12$climate1, escape_frt12$climate2)
cor.test(escape_frt12$climate1, escape_frt12$elevatn) # these are correlated!
...
model_ft12<-glm(escape10 ~ climate1 + climate2 + FWI_veg  + slope + elevatn + win_sum + log_road + log_infr, family = binomial, data = escape_frt12)
#2776

model_ft12<-glm(escape10 ~ climate2 + FWI_veg  + slope + elevatn + win_sum + log_road + log_infr, family = binomial, data = escape_frt12)
#2777

model_ft12<-glm(escape10 ~ climate1 + climate2 + FWI_veg  + slope + win_sum + log_road + log_infr, family = binomial, data = escape_frt12)
#2784

#keep elecation throw out climate1


model_ft12<-glm(escape10 ~ climate1 + climate2 + FWI_veg  + slope + win_sum  + log_road + log_infr, family = binomial, data = escape_frt12)

AIC(model_ft12) #2755.106
Anova(model_ft12, type=3) 

#Remove least significant interaction
model_ft12b<-glm(escape10 ~ climate2 + FWI_veg  + slope + win_sum  + log_road + log_infr, family = binomial, data = escape_frt12)

AIC(model_ft12b) #2753.483
...
#######################
#Remove least term ~ slope
model_ft12e<-glm(escape10 ~ climate2 + FWI_veg  + slope + win_sum  + log_road + log_infr, family = binomial, data = escape_frt12)

hist(escape_frt12$climate2)
hist(escape_frt12$log_road)

table(escape_frt12$escape10) #0 = 4359 and 1 = 456 not bad actually.
...
binnedplot (escape_frt12$log_road, 
            escape_frt12$resids,  
            nclass = NULL)

binnedplot (escape_frt12$slope, 
            escape_frt12$resids,  
            nclass = NULL)

binnedplot (escape_frt12$elevatn, 
            escape_frt12$resids,  
            nclass = NULL)
...
#Model   
model.FRT12<-glm(escape10 ~ climate2 + FWI_veg  + slope + win_sum  + log_road + log_infr, family = binomial, data = dat1)
...
top_mod_table_FRT12[1,2]<-"escape ~ climate2 + FWI_veg  + slope + win_sum  + log_road + log_infr" 
...
top_mod_table_FRT12[1,17]<- coef(model.FRT12)[15] #coefficient 
...
summary_top_mod_table_FRT12_ALL$Model_terms<-"climate2 + FWI_veg  + slope + win_sum  + log_road + log_infr" 
...
#### FRT 13
```{r}
escape_frt13<- Escape_data_ALL2 %>% filter(Cluster==13)
table(escape_frt13$FWI_veg, escape_frt13$escape10)
escape_frt13$FWI_veg[escape_frt13$FWI_veg=="M-3"]<-"O-1a/b"
escape_frt13$FWI_veg[escape_frt13$FWI_veg=="C-4"]<-"C-2"
escape_frt13$FWI_veg[escape_frt13$FWI_veg=="S-2"]<-"M-1/2"
...
table(is.na(escape_frt13$wind_atfire))
table(is.na(escape_frt13$rd_dst_))
table(is.na(escape_frt13$slope))
table(is.na(escape_frt13$fire_cs))
table(is.na(escape_frt13$escape10))

cor.test(escape_frt13$road_dist_m, escape_frt13$infr_dist)
cor.test(escape_frt13$climate1, escape_frt13$climate2)
cor.test(escape_frt13$climate1, escape_frt13$elevatn) # these are correlated!
...
#Run model using dat1
model_ft13<-glm(escape10 ~ climate1 + FWI_veg  + slope + elevatn + win_sum + log_road + log_infr, family = binomial, data = escape_frt13)

AIC(model_ft13) #5758.033
Anova(model_ft13, type=3) 

#Remove least significant interaction
model_ft13b<-glm(escape10 ~ climate1 + FWI_veg  + slope + win_sum + log_road + log_infr, family = binomial, data = escape_frt13)
anova(model_ft13, model_ft13b, test="Chisq")

AIC(model_ft13b) #5757.604
Anova(model_ft13b, type=3) 
...
#######################
#Remove least term ~ slope
model_ft13e<-glm(escape10 ~ climate1 + FWI_veg  + slope + elevatn + win_sum + log_road + log_infr, family = binomial, data = escape_frt13)

hist(escape_frt13$climate1)
hist(escape_frt13$log_road)

table(escape_frt13$escape10) #0 = 948 and 1 = 267 not bad actually.
...
#Model   
model.FRT13<-glm(escape10 ~ climate1 + FWI_veg  + slope + elevatn + win_sum + log_road + log_infr, family = binomial, data = dat1)
...
top_mod_table_FRT13[1,2]<-"escape10 ~ climate1 + FWI_veg  + slope + elevatn + win_sum + log_road + log_infr" 
...
summary_top_mod_table_FRT13_ALL$Model_terms<-"escape10 ~ climate1 + FWI_veg  + slope + elevatn + win_sum + log_road + log_infr" 
...
#### FRT 14
```{r}
escape_frt14<- Escape_data_ALL2 %>% filter(Cluster==14)
table(escape_frt14$FWI_veg, escape_frt14$escape10)
...
cor.test(escape_frt14$climate1, escape_frt14$climate2)
...
table(is.na(escape_frt14$rd_dst_))
table(is.na(escape_frt14$slope))
table(is.na(escape_frt14$fire_cs))
table(is.na(escape_frt14$escape10))

escape_frt14<-escape_frt14 %>% filter(!is.na(FWI_veg))

#Run model using dat1
model_ft14<-glm(escape10 ~ climate1 + FWI_veg + elevatn + slope + win_sum + log_road + log_infr, family = binomial, data = escape_frt14)

AIC(model_ft14) #3737.464
Anova(model_ft14, type=3) 

#Remove least significant interaction
model_ft14b<-glm(escape10 ~ climate1 + FWI_veg + elevatn + win_sum + log_road + log_infr, family = binomial, data = escape_frt14)

AIC(model_ft14b) #3739.792
Anova(model_ft14b, type=3) 
anova(model_ft14, model_ft14b, test="Chisq")

#######################
### TOP MODEL
#######################
#Remove least term ~ slope
model_ft14e<-glm(escape10 ~ climate1 + FWI_veg + elevatn + win_sum + log_road + log_infr, family = binomial, data = escape_frt14)

hist(escape_frt14$climate1)
hist(escape_frt14$elevatn)
hist(escape_frt14$log_road)

table(escape_frt14$escape10) #0 = 9123 and 1 = 556 not bad actually.
...
#Model   
model.FRT14<-glm(escape10 ~ climate1 + FWI_veg + elevatn + win_sum + log_road + log_infr, family = binomial, data = dat1)
...
top_mod_table_FRT14[1,2]<-"escape10 ~ climate1 + FWI_veg + elevatn + win_sum + log_road + log_infr" 
...
summary_top_mod_table_FRT14_ALL$Model_terms<-"escape10 ~ climate1 + FWI_veg + elevatn + win_sum + log_road + log_infr" 
...
#### FRT 15
```{r}
escape_frt15_2<- Escape_data_ALL2 %>% filter(Cluster==15)
table(escape_frt15_2$FWI_veg)
escape_frt15_2$FWI_veg[escape_frt15_2$FWI_veg=="S-1"]<-"S-3"
escape_frt15_2$FWI_veg[escape_frt15_2$FWI_veg=="C-7"]<-"C-5"
escape_frt15_2$FWI_veg[escape_frt15_2$FWI_veg=="O-1a/b"]<-"C-3"
escape_frt15_2$log_road<-log(escape_frt15_2$road_dist_m+1)
escape_frt15_2$log_infr<- log(escape_frt15_2$infr_dist+1)



frt_15_0<-escape_frt15_2 %>% filter(escape10==0)
frt_15_1<-escape_frt15_2 %>% filter(escape10==1)
...
escape_frt15$veg_escape<-paste(escape_frt15$fire, escape_frt15$FWI_veg)

cor.test(escape_frt15$road_dist_m, escape_frt15$infr_dist)
cor.test(escape_frt15$climate1, escape_frt15$climate2)
cor.test(escape_frt15$climate1, escape_frt15$elevatn) # these are correlated!
cor.test(escape_frt15$elevatn, escape_frt15$slope)
cor.test(escape_frt15$win_sum, escape_frt15$win_spg) # very correlated
...
table(is.na(escape_frt15$escape10))

#escape_frt15_2<- escape_frt15 %>% filter(!is.na(elevatn))
#escape_frt15_2<- escape_frt15 %>% filter(!is.na(win_sum))

#Run model using dat1
model_ft15<-glm(escape10 ~ climate1 + climate2 + FWI_veg + slope + win_sum + log_road + log_infr, family = binomial, data = escape_frt15_2)

AIC(model_ft15) #1060.374
Anova(model_ft15, type=3) 

#Remove least significant interaction
model_ft15b<-glm(escape10 ~ elevatn + climate2 + FWI_veg + slope + win_sum + log_road + log_infr, family = binomial, data = escape_frt15_2)
...
model_ft15c<-glm(escape10 ~ climate1 + climate2 + FWI_veg + slope + win_sum + log_road + log_infr, family = binomial, data = escape_frt15)

AIC(model_ft15c) #1060.374
Anova(model_ft15c, type=3) 

#rm 
model_ft15d<-glm(escape10 ~ climate1 + climate2 + FWI_veg + slope + log_road + log_infr, family = binomial, data = escape_frt15)

AIC(model_ft15d) #1058.55
Anova(model_ft15d, type=3) 

model_ft15e<-glm(escape10 ~ climate1 + climate2 + FWI_veg + slope + log_infr, family = binomial, data = escape_frt15)

AIC(model_ft15e) #1058.55
Anova(model_ft15e, type=3) 

model_ft15f<-glm(escape10 ~ climate2 + FWI_veg + slope + log_infr, family = binomial, data = escape_frt15)

AIC(model_ft15f) #1058.55
Anova(model_ft15f, type=3) 
...
#######################
#Remove least term ~ slope
model_ft15e<-glm(escape10 ~ climate2 + FWI_veg + slope + log_infr, family = binomial, data = escape_frt15_2)

hist(escape_frt15$slope)
hist(escape_frt15$climate2)
hist(escape_frt15$log_infr)

table(escape_frt15$escape10) #0 = 948 and 1 = 267 not bad actually.
table(escape_frt15$FWI_veg, escape_frt15$escape10)
...
trainIndex <- createDataPartition(escape_frt15_2$escape10, p = prop,
                                    list = FALSE,
                                    times = 1)
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  dat1 <- escape_frt15_2[ trainIndex,] # note only 6% of obs are 1's so Im going to sample to get my sample size up to 30% 1's.
   # in total I need 1589 1's
   dat1_1<-dat1 %>% filter(escape10==1)
   dat1_0<-dat1 %>% filter(escape10==0)
   dat1_1_samp<- sample_n(dat1_1, round(length(dat1$escape10)*0.1,0), replace = TRUE)
   
   dat<-rbind(dat1_0, dat1_1_samp)
   Valid <- escape_frt15_2[-trainIndex,]
   
#Model   
model.FRT15<-glm(escape10 ~ climate2 + FWI_veg + slope + log_infr, family = binomial, data = dat)
...
frac_ones_pop<-length(escape_frt15_2$escape10[escape_frt15_2$escape10==1])/(length(escape_frt15_2$escape10))
frac_ones_samp<-length(dat$escape10[dat$escape10==1])/(length(dat$escape10))
...
summary_top_mod_table_FRT15_ALL$Model_terms<-"escape ~ climate2 + FWI_veg + slope + log_infr" 
