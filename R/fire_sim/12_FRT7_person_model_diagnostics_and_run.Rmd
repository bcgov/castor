...
```{r}
# FRT 7
dat7<- dat_person %>% dplyr::filter(frt == 7) 
table(dat7$FWI_veg)

dat7<-dat7 %>% filter(FWI_veg!="W")

ggplot(data=dat7, mapping=aes(x=as.factor(fire), y=climate1))+
  geom_boxplot()

ggplot(data=dat7, mapping=aes(x=as.factor(fire), y=elevatn))+
  geom_boxplot()

cor.test(dat7$climate1, dat7$elevatn) # elevation and climate are highly correlated!!!
cor.test(dat7$road_dist_m, dat7$infr_dist)


hist(log(dat7$road_dist_m+1))
hist(log(dat7$infr_dist +1))

table(is.na(dat7$climate1))
table(is.na(dat7$elevatn))

mod_frt_7a<- glm(fire ~ climate1 + elevatn + FWI_veg + log(road_dist_m+1) + infr_dist, data=dat7, family=binomial(link="logit"))

mod_frt_7b<- glm(fire ~ climate1 + FWI_veg + log(road_dist_m+1) + infr_dist, data=dat7, family=binomial(link="logit"))

mod_frt_7c_log<- glm(fire ~ climate1 + FWI_veg + log(road_dist_m+1) + log(infr_dist+1), data=dat7, family=binomial(link="logit"))

dat7$log_road_dist<-log(dat7$road_dist_m+1)
#dat7$log_infr_dist<-log(dat7$infr_dist+1)

mod_frt7<- glm(fire ~ climate1 + FWI_veg + log_road_dist + infr_dist, data=dat7, family=binomial(link="logit"))
...
dat2_b<-dat7 %>% dplyr::select(fire, climate1, log_road_dist, infr_dist)
...
model.data %>% 
  dplyr::select(climate1, log_road_dist, infr_dist,.std.resid, .cooksd) %>%
  top_n(5, .cooksd) 
...
dat7$fire_veg<-paste(dat7$fire, dat7$FWI_veg)
```
...
#Model   
mod<-glm(fire ~ climate1 + FWI_veg + log_road_dist + infr_dist, data=dat1, family=binomial(link="logit"))
...
top_mod_table_FRT5_person[1,3]<-"fire ~ climate1 + FWI_veg + log_road_dist + infr_dist"
...
top_mod_table_FRT5_person[1,13]<- coef(mod)[10] #co
top_mod_table_FRT5_person[1,14]<- coef(mod)[11] #Climate variable 2
top_mod_table_FRT5_person[1,15]<- coef(mod)[12] #I
