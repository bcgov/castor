...
dat15<- dat15 %>% dplyr::filter(FWI_veg != "N") 
dat15$FWI_veg[dat15$FWI_veg=="S-1"]<-"S-3"
dim(dat15)

dat15 %>% filter(is.na(FWI_veg))
dat15 %>% filter(is.na(elevatn))
dat15 %>% filter(is.na(climate1))

dat15<- dat15%>% drop_na(FWI_veg)
 
# put in average elevation for the locations with M-1/2 vegetation type
 elevM12<-dat15 %>% filter(FWI_veg =="M-1/2") %>%
   filter(!is.na(elevatn)) %>%
   summarize(elev_mean=mean(elevatn))
 
 elevC5<-dat15 %>% filter(FWI_veg =="C-5") %>%
   filter(!is.na(elevatn)) %>%
   summarize(elev_mean=mean(elevatn))
 
 dat15$elevatn[is.na(dat15$elevatn) & dat15$FWI_veg=="M-1/2"]<-580.5608
 dat15$elevatn[is.na(dat15$elevatn) & dat15$FWI_veg=="C-5"]<-536.4029
...
mod.frt.10<-glm(fire ~ climate1 +climate2 + FWI_veg + elevatn, data=dat15, family=binomial(link="logit"))

# looking at difference in link function
modclim_logit<-glm(fire ~ climate1, data=dat15, family=binomial(link="logit"))
modclim_cloglog<-glm(fire ~ climate1, data=dat15, family=binomial(link="cloglog"))

summary(dat15)
mydata<-data.frame(climate1=seq(from=7, to=20, by=0.5))
...
plot(x=dat15$climate1, y=dat15$fire)
lines(mydata$climate1, Pred_logit, col="red")
lines(mydata$climate1, Pred_cloglog, col="blue")

dat15$FWI_veg<- as.factor(dat15$FWI_veg)


mod.frt.10<-glm(fire ~ climate1 + climate2 + FWI_veg + elevatn, data=dat15, family=binomial(link="logit"))

# hmm note elevation and climate 1 are quite correlated -0.75. I should probably leave out one of them in my model
cor(model.matrix(mod.frt.10)[,-1])

mod.frt.clim1<-glm(fire ~climate1 + climate2 + FWI_veg, data=dat15, family=binomial(link="logit"))
# AIC = 2965

 mod.frt.elev<-glm(fire ~climate2 + FWI_veg + elevatn, data=dat15, family=binomial(link="logit"))
 # AIC = 2844
 
 # Ill remove climate1 and keep in elevation


hist(dat15$climate2)
hist(dat15$elevatn)
summary(mod.frt.elev)

Anova(mod.frt.elev, type=3)

# model diagnostic plots
binnedplot (fitted(mod.frt.elev), 
            residuals(mod.frt.elev), 
            nclass = NULL, 
...
dat15$resids<-resid(mod.frt.elev)
...
##Partial Residuals
visreg(mod.frt.elev, scale="response") # nicer plots than the ones below!

visreg(mod.frt.elev, "climate2", by="elevatn", scale="response")
visreg(mod.frt.elev, "climate2", by="FWI_veg", scale="response")
visreg(mod.frt.elev, "elevatn", by="FWI_veg", scale="response")
...
dat2_b<-dat15%>% dplyr::select(fire, elevatn, climate2)
...
plot(mod.frt.elev, which = 6, id.n = 3)
...
top_mod_table_FRT12_lightningALL <- data.frame (matrix (ncol = 12, nrow = 0))
colnames (top_mod_table_FRT12_lightningALL ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_2",  "coef_FWI_veg_C-5",  "coef_FWI_veg_C-7", "coef_FWI_veg_D-1/2", "coef_FWI_veg_M-1/2", "coef_FWI_veg_S-3","coef_elevatn", "AUC")
...
top_mod_table_FRT12_lightningL <- data.frame (matrix (ncol = 12, nrow = 0))
colnames (top_mod_table_FRT12_lightningL ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_2",  "coef_FWI_veg_C-5",  "coef_FWI_veg_C-7", "coef_FWI_veg_D-1/2", "coef_FWI_veg_M-1/2", "coef_FWI_veg_S-3","coef_elevatn", "AUC")


##Add data for NDT1
top_mod_table_FRT12_lightningL[1,1]<-"lightning"
top_mod_table_FRT12_lightningL[1,2]<-"FRT15"
top_mod_table_FRT12_lightningL[1,3]<-"fire ~ climate2 + FWI_veg + elevatn"
...
top_mod_table_FRT12_lightningL[1,12]<- mod.auc 
...
FRT12_summary_table_mean$Model_terms<-"fire ~ climate2 + FWI_veg + elevatn"
