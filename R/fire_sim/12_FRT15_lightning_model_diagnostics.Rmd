...
dat15<- dat15 %>% dplyr::filter(FWI_veg != "W") 
...
# put in average elevation for the locations with M-1/2 vegetation type
 ...
 elevC3<-dat15 %>% filter(FWI_veg =="C-3") %>%
   filter(!is.na(elevatn)) %>%
   summarize(elev_mean=mean(as.numeric(elevatn)))
 
 dat15$elevatn[is.na(dat15$elevatn) & dat15$FWI_veg=="M-1/2"]<-elevM12
 dat15$elevatn[is.na(dat15$elevatn) & dat15$FWI_veg=="C-5"]<-elevC5
 dat15$elevatn[is.na(dat15$elevatn) & dat15$FWI_veg=="C-3"]<-elevC3
 
 dat15$elevatn<-as.numeric(dat15$elevatn)
