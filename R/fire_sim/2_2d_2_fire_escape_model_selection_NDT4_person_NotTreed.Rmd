---
title: "12_2d_2_fire_escape_model_selection_NDT4_person_NotTreed"
author: "Cora Skaien"
date: "19/11/2021"
...
#=================================
#  Script Name: 12_2d_2_fire_escape_model_selection_NDT4_person_NotTreed.R
#  Script Version: 1.0
...
Select NDT: NDT4
- NDT4 has two climate variables; NDTs 2-5 have only 1
...
AIC_person_NDT4_escape_ntreed_climate<-table.glm.climate.simple

AIC_person_NDT4_escape_ntreed_summary_climate<- AIC_person_NDT4_escape_ntreed_climate %>%
  group_by(model) %>%
...
AIC_person_NDT4_escape_ntreed_summary_climate2<- AIC_person_NDT4_escape_ntreed_summary_climate %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT4_escape_ntreed_summary_climate2)
```
...
AIC_person_NDT4_escape_ntreed_ntopo<-table.glm.topo.simple

AIC_person_NDT4_escape_ntreed_summary_ntopo<- AIC_person_NDT4_escape_ntreed_ntopo %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic),
            meanAUC=mean(auc.valid),
            sdAUC=sd(auc.valid),
...
AIC_person_NDT4_escape_ntreed_summary_ntopo2<- AIC_person_NDT4_escape_ntreed_summary_ntopo %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT4_escape_ntreed_summary_ntopo2)
```
...
AIC_person_NDT4_escape_ntreed_infra<-table.glm.infra.simple

AIC_person_NDT4_escape_ntreed_summary_infra<- AIC_person_NDT4_escape_ntreed_infra %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic),
            meanAUC=mean(auc.valid),
            sdAUC=sd(auc.valid),
...
AIC_person_NDT4_escape_ntreed_summary_infra2<- AIC_person_NDT4_escape_ntreed_summary_infra %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT4_escape_ntreed_summary_infra2)
...
```{r}
NDT4_l_models_ntreed<-rbind(AIC_person_NDT4_escape_ntreed_summary_climate2, AIC_person_NDT4_escape_ntreed_summary_ntopo2, AIC_person_NDT4_escape_ntreed_summary_infra2)
NDT4_l_models_ntreed
NDT4_l_models_ntreed$NDT<-"NDT4"

write.csv(NDT4_l_models_ntreed, file="D:\\Fire\\fire_data\\raw_data\\NDT4_person_models_notrees_escape.csv")
```
...
Top Models:
1. climate1 + vegtype2
2. slope + elevation + wind_atfire + slope:wind_atfire
3. dist_mun + dist_nat + dist_mine + roads_km
...
```{r}
escape_person_nt_NDT4<-subset(Escape_data_person_nt, Escape_data_person_nt$ntrl_ds=="NDT4") #3008 observations

#Divide data into training and valid
prop<-0.75
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(escape_person_nt_NDT4$escape, p = prop,
                                    list = FALSE,
                                    times = 1)
  
   dat1 <- escape_person_nt_NDT4[ trainIndex,]
   Valid <- escape_person_nt_NDT4[-trainIndex,]

#Run model using dat1
#Start without additional interactions
model.NDT4.E<-glm(escape ~ climate1 + vegtype2 + slope + elevation + wind_atfire + slope:wind_atfire + dist_mun + dist_nat + dist_mine + roads_km +  bclcs_level_5_2 + Tdif_atfire + elevation*climate1 + wind_atfire*climate1 + wind_atfire*vegtype2 + Tdif_atfire*climate1 + Tdif_atfire*wind_atfire + bclcs_level_5_2*dist_mun + bclcs_level_5_2*dist_mine + bclcs_level_5_2*dist_nat + bclcs_level_5_2*roads_km +  dist_mun*roads_km + dist_mine*roads_km + dist_nat:roads_km, family = binomial, data = dat1)

AIC(model.NDT4.E) #1990.4
...
model.NDT4.E<-glm(escape ~ climate1 + vegtype2 + slope + elevation + wind_atfire + slope:wind_atfire + dist_mun + dist_nat + dist_mine + roads_km +  bclcs_level_5_2 + Tdif_atfire + elevation*climate1 + wind_atfire*climate1 + wind_atfire*vegtype2 + Tdif_atfire*climate1 + bclcs_level_5_2*dist_mun + bclcs_level_5_2*dist_mine + bclcs_level_5_2*roads_km +  dist_mun*roads_km + dist_mine*roads_km + dist_nat:roads_km, family = binomial, data = dat1)

AIC(model.NDT4.E) #1984.5
...
#Remove least significant
model.NDT4.E<-glm(escape ~ climate1 + vegtype2 + slope + elevation + wind_atfire + slope:wind_atfire + dist_mun + dist_nat + dist_mine + roads_km +  bclcs_level_5_2 + Tdif_atfire + elevation*climate1 + wind_atfire*climate1 + wind_atfire*vegtype2 + Tdif_atfire*climate1 + bclcs_level_5_2*dist_mun + bclcs_level_5_2*dist_mine +  dist_mun*roads_km + dist_nat:roads_km, family = binomial, data = dat1)

AIC(model.NDT4.E) #1978.99
...
#Remove least significant
model.NDT4.E<-glm(escape ~ climate1 + vegtype2 + slope + elevation + wind_atfire + slope:wind_atfire + dist_mun + dist_nat + dist_mine + roads_km +  bclcs_level_5_2 + Tdif_atfire + wind_atfire*climate1 + wind_atfire*vegtype2 + Tdif_atfire*climate1 + bclcs_level_5_2*dist_mun + bclcs_level_5_2*dist_mine +  dist_mun*roads_km + dist_nat:roads_km, family = binomial, data = dat1)

AIC(model.NDT4.E) #1977.2
...
#Remove least significant
model.NDT4.E<-glm(escape ~ climate1 + vegtype2 + slope + elevation + wind_atfire + slope:wind_atfire + dist_mun + dist_nat + dist_mine + roads_km +  bclcs_level_5_2 + Tdif_atfire + wind_atfire*climate1  + Tdif_atfire*climate1 + bclcs_level_5_2*dist_mun + bclcs_level_5_2*dist_mine +  dist_mun*roads_km + dist_nat:roads_km, family = binomial, data = dat1)

AIC(model.NDT4.E) #1974.7
...
#Remove least significant
model.NDT4.E<-glm(escape ~ climate1 + vegtype2 + slope + elevation + wind_atfire + slope:wind_atfire + dist_mun + dist_nat + dist_mine + roads_km +  bclcs_level_5_2 + Tdif_atfire + Tdif_atfire*climate1 + bclcs_level_5_2*dist_mun + bclcs_level_5_2*dist_mine +  dist_mun*roads_km + dist_nat:roads_km, family = binomial, data = dat1)

AIC(model.NDT4.E) #1973.2
...
#Remove least significant
model.NDT4.E<-glm(escape ~ climate1 + vegtype2 + slope + elevation + wind_atfire + slope:wind_atfire + dist_mun + dist_nat + dist_mine + roads_km +  bclcs_level_5_2 + Tdif_atfire + Tdif_atfire*climate1 + bclcs_level_5_2*dist_mun + bclcs_level_5_2*dist_mine + dist_nat:roads_km, family = binomial, data = dat1)

AIC(model.NDT4.E) #1971.9
...
#Remove least significant
model.NDT4.E<-glm(escape ~ climate1 + vegtype2 + slope + elevation + wind_atfire + slope:wind_atfire + dist_mun + dist_nat + dist_mine + roads_km +  bclcs_level_5_2 + Tdif_atfire + Tdif_atfire*climate1 + bclcs_level_5_2*dist_mun + dist_nat:roads_km, family = binomial, data = dat1)

AIC(model.NDT4.E) #1970.0
...
#Remove least significant
model.NDT4.E<-glm(escape ~ climate1 + vegtype2 + slope + elevation + wind_atfire + slope:wind_atfire + dist_mun + dist_nat + dist_mine + roads_km +  bclcs_level_5_2 + Tdif_atfire + Tdif_atfire*climate1 + bclcs_level_5_2*dist_mun, family = binomial, data = dat1)
...
#Remove least significant
model.NDT4.E<-glm(escape ~ climate1 + vegtype2 + slope + elevation + wind_atfire + slope:wind_atfire + dist_mun + dist_mine + roads_km +  bclcs_level_5_2 + Tdif_atfire + Tdif_atfire*climate1 + bclcs_level_5_2*dist_mun, family = binomial, data = dat1)

AIC(model.NDT4.E) #1969.9
...
#Remove least significant
model.NDT4.E<-glm(escape ~ climate1 + slope + elevation + wind_atfire + slope:wind_atfire + dist_mun + dist_mine + roads_km +  bclcs_level_5_2 + Tdif_atfire + Tdif_atfire*climate1 + bclcs_level_5_2*dist_mun, family = binomial, data = dat1)

AIC(model.NDT4.E) #1968.6
...
#Remove least significant
model.NDT4.E<-glm(escape ~ climate1 + slope  + wind_atfire + slope:wind_atfire + dist_mun + dist_mine + roads_km +  bclcs_level_5_2 + Tdif_atfire + Tdif_atfire*climate1 + bclcs_level_5_2*dist_mun, family = binomial, data = dat1)

AIC(model.NDT4.E) #1966.7
...
NDT4_escape_nt<-escape_person_nt_NDT4 %>% drop_na(climate1, slope, wind_atfire, Tdif_atfire, dist_mun, dist_mine, roads_km, bclcs_level_5_2) #3007 observations

#Run Model again with this data; but uses all data here
model.NDT4.E<-glm(escape ~ climate1 + slope  + wind_atfire + slope:wind_atfire + dist_mun + dist_mine + roads_km +  bclcs_level_5_2 + Tdif_atfire + Tdif_atfire*climate1 + bclcs_level_5_2*dist_mun, family = binomial, data = NDT4_escape_nt)
...
binnedplot (NDT4_escape_nt$live_stand_volume_125, 
...
#Partial Residuals
#climate1 + slope  + wind_atfire + slope:wind_atfire + dist_mun + dist_mine + roads_km +  bclcs_level_5_2 + Tdif_atfire + Tdif_atfire*climate1 + bclcs_level_5_2*dist_mun
...
#dist_mun + dist_mine + roads_km +  bclcs_level_5_2 + bclcs_level_5_2*dist_mun
visreg(model.NDT4.E, "bclcs_level_5_2", by="dist_mun")
visreg(model.NDT4.E, "dist_mun", by="bclcs_level_5_2", overlay=TRUE)
visreg(model.NDT4.E, "dist_mine")
...
top_mod_ntable_NDT4_person_nt_ALL <- data.frame (matrix (ncol = 19, nrow = 0))
colnames (top_mod_ntable_NDT4_person_nt_ALL ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_slope", "coef_wind_atfire", "coef_dist_mun", "coef_dist_mine", "coef_roads_km", "coef_bclcs_level_5_2OP", "coef_bclcs_level_5_2SP", "coef_Tdif_atfire", "coef_slope:wind_atfire", "coef_climate1:Tdif_atfire", "coef_dist_mun:bclcs_level_5_2OP", "coef_dist_mun:bclcs_level_5_2SP", "AUC")

NDT4_escape_nt$escape_land<-paste(NDT4_escape_nt$escape, NDT4_escape_nt$bclcs_level_5_2)
```
...
prop<-0.75
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(NDT4_escape_nt$escape_land, p = prop,
                                    list = FALSE,
                                    times = 1)
...
top_mod_ntable_NDT4_person_nt <- data.frame (matrix (ncol = 19, nrow = 0))
colnames (top_mod_ntable_NDT4_person_nt ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_slope", "coef_wind_atfire", "coef_dist_mun", "coef_dist_mine", "coef_roads_km", "coef_bclcs_level_5_2OP", "coef_bclcs_level_5_2SP", "coef_Tdif_atfire", "coef_slope:wind_atfire", "coef_climate1:Tdif_atfire", "coef_dist_mun:bclcs_level_5_2OP", "coef_dist_mun:bclcs_level_5_2SP", "AUC")

##Add data for NDT4
top_mod_ntable_NDT4_person_nt[1,1]<-"person"
top_mod_ntable_NDT4_person_nt[1,2]<-"NDT4"
top_mod_ntable_NDT4_person_nt[1,3]<-"N"
top_mod_ntable_NDT4_person_nt[1,4]<-"escape ~ climate1 + slope  + wind_atfire + slope:wind_atfire + dist_mun + dist_mine + roads_km +  bclcs_level_5_2 + Tdif_atfire + Tdif_atfire*climate1 + bclcs_level_5_2*dist_mun" 
top_mod_ntable_NDT4_person_nt[1,5]<- coef(model.NDT4.E)[1] #Intercept
top_mod_ntable_NDT4_person_nt[1,6]<- coef(model.NDT4.E)[2] #C
top_mod_ntable_NDT4_person_nt[1,7]<- coef(model.NDT4.E)[3] #C
top_mod_ntable_NDT4_person_nt[1,8]<- coef(model.NDT4.E)[4] #co
top_mod_ntable_NDT4_person_nt[1,9]<- coef(model.NDT4.E)[5] #co
top_mod_ntable_NDT4_person_nt[1,10]<- coef(model.NDT4.E)[6] #c
top_mod_ntable_NDT4_person_nt[1,11]<- coef(model.NDT4.E)[7] #l
top_mod_ntable_NDT4_person_nt[1,12]<- coef(model.NDT4.E)[8] #co
top_mod_ntable_NDT4_person_nt[1,13]<- coef(model.NDT4.E)[9] #c
top_mod_ntable_NDT4_person_nt[1,14]<- coef(model.NDT4.E)[10] #coeff
top_mod_ntable_NDT4_person_nt[1,15]<- coef(model.NDT4.E)[11] #coeff
top_mod_ntable_NDT4_person_nt[1,16]<- coef(model.NDT4.E)[12] #coeff
top_mod_ntable_NDT4_person_nt[1,17]<- coef(model.NDT4.E)[13] #coeff
top_mod_ntable_NDT4_person_nt[1,18]<- coef(model.NDT4.E)[14] #coeff
top_mod_ntable_NDT4_person_nt[1,19]<- mod.auc

top_mod_ntable_NDT4_person_nt_ALL<-rbind(top_mod_ntable_NDT4_person_nt_ALL, top_mod_ntable_NDT4_person_nt)
...
Check.
```{r}
head(top_mod_ntable_NDT4_person_nt_ALL)
...
```{r}
write.csv(top_mod_ntable_NDT4_person_nt_ALL, file="D:\\Fire\\fire_data\\raw_data\\top_mod_escape_NDT4_person_nt_ALL.csv")
```
...
```{r}
names(top_mod_ntable_NDT4_person_nt_ALL)

top_mod_ntable_NDT4_escape_person_nt_means<-top_mod_ntable_NDT4_person_nt_ALL %>% summarise_each(funs( mean( .,na.rm = TRUE)))
top_mod_ntable_NDT4_escape_person_nt_means

top_mod_ntable_NDT4_escape_person_nt_means[1,1]<-"person"
top_mod_ntable_NDT4_escape_person_nt_means[1,2]<-"NDT4"
top_mod_ntable_NDT4_escape_person_nt_means[1,3]<-"Not Treed"
top_mod_ntable_NDT4_escape_person_nt_means[1,4]<- "escape ~ climate1 + slope  + wind_atfire + slope:wind_atfire + dist_mun + dist_mine + roads_km +  bclcs_level_5_2 + Tdif_atfire + Tdif_atfire*climate1 + bclcs_level_5_2*dist_mun" 
top_mod_ntable_NDT4_escape_person_nt_means
```
Save table.

```{r}
write.csv(top_mod_ntable_NDT4_escape_person_nt_means, file="D:\\Fire\\fire_data\\raw_data\\top_mod_escape_NDT4_person_nt_Means.csv")
```

Standard deviation.

```{r}
top_mod_ntable_NDT4_escape_person_nt_sd<-top_mod_ntable_NDT4_person_nt_ALL %>% summarise_each(funs( sd( .,na.rm = TRUE)))
top_mod_ntable_NDT4_escape_person_nt_sd

top_mod_ntable_NDT4_escape_person_nt_sd[1,1]<-"person"
top_mod_ntable_NDT4_escape_person_nt_sd[1,2]<-"NDT4"
top_mod_ntable_NDT4_escape_person_nt_sd[1,3]<-"Not Treed"
top_mod_ntable_NDT4_escape_person_nt_sd[1,4]<-"escape ~ climate1 + slope  + wind_atfire + slope:wind_atfire + dist_mun + dist_mine + roads_km +  bclcs_level_5_2 + Tdif_atfire + Tdif_atfire*climate1 + bclcs_level_5_2*dist_mun" 
top_mod_ntable_NDT4_escape_person_nt_sd
```
...
```{r}
write.csv(top_mod_ntable_NDT4_escape_person_nt_sd, file="D:\\Fire\\fire_data\\raw_data\\top_mod_escape_NDT4_person_nt_SD.csv")
```
