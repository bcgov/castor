...
The final dataset used to fit the statistical equations can be found in postgres (KyleCLUS) under the name public.fire_ignitions_veg_climate_cleaned
...
For projecting fire ignitions forward I need predicted data collected for the future. For vegetation data I hope to use the CLUS model which pulls volume, height and age data from tipsy curves and I should be able to access this data during a CLUS run. Need to speak to Kyle about this. Also need to determine accounting for when land might be harvested, changing the land type once again.
