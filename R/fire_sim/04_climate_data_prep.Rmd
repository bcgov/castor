...
```{r}
sample_locations_DEM <-st_read(dsn = "C:\\Work\\caribou\\clus\\R\\fire_sim\\tmp\\DEM_5x_AllMonths_frt.shp")
...
# Here we have a choice you can either manually extract the points as follows or use the R code below to extract them
...
setwd("D:/ClimateBC721"); # set the ClimateBC root directory as the working directory
exe <- "ClimateBC_v7.21.exe"
...
# note there may be no data for 2021 in this case you will need to get the climate data off the future projections. Im not sure how to do this through R code so I did it manually using the 13 GCM ensemble and ssp370.
##2021
...
file.list1<-list.files("C:\\Work\\caribou\\clus\\R\\fire_sim\\tmp\\output", pattern="samp.points", all.files=FALSE, full.names=FALSE)
...
#############################################
    # note if I want to find MDC10 Im giong to have to change this code. I tried putting a list of mnths e.g. c("04", "05", "06",..."10") but this did not work with also changing j from 1:length(mnths) because there is part of the code where it uses information from the month before i.e. j-1 and I could not quickly think of a solution, so I left it. 
...
for (i in 1: length(y1)){
  ...
  x<-eval(as.name(y1[i])) %>% 
    rename(YEAR=ID2) %>%
    dplyr::select(ID1, YEAR,Latitude, Longitude, Tmax04:Tmax10, Tave04:Tave10, PPT04:PPT10, RH04:RH10)
...
```{r}
table(ignition_weather_crs$FIRE_YEAR, ignition_weather_crs$FIRE_CAUSE)

st_write(ignition_weather_crs, dsn = "C:\\Work\\caribou\\clus\\R\\fire_sim\\tmp\\samp_locations_fire_all_5x.shp", delete_layer=TRUE)
