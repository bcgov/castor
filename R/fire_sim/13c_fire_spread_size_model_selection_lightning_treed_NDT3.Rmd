---
title: "13c_fire_spread_size_model_selection_lightning_treed_NDT3"
author: "Cora Skaien"
date: "01/12/2021"
...
#=================================
#  Script Name: 13c_fire_spread_size_model_selection_lightning_treed_NDT3.R
#  Script Version: 1.0
...
Only has climate1, no climate2. Use all.poss.mods.clim.vegtype. NDT1,3, 4 and 5 only have one climate variables. NDT3 has two.
...
AIC_lightning_NDT3_spread_treed_climate<-table.glm.climate.simple.spread

AIC_lightning_NDT3_spread_treed_summary_climate<- AIC_lightning_NDT3_spread_treed_climate %>%
  group_by(model) %>%
...
AIC_lightning_NDT3_spread_treed_summary_climate2<- AIC_lightning_NDT3_spread_treed_summary_climate %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_lightning_NDT3_spread_treed_summary_climate2)
```
...
AIC_lightning_NDT3_spread_treed_climate_gamma<-table.glm.climate.simple.spread.gamma

AIC_lightning_NDT3_spread_treed_summary_climate_gamma<- AIC_lightning_NDT3_spread_treed_climate_gamma %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic, na.rm=TRUE),
            meanR2=mean(Model.R2, na.rm=TRUE),
            sdR2=sd(Model.R2, na.rm=TRUE),
...
AIC_lightning_NDT3_spread_treed_summary_climate_gamma2<- AIC_lightning_NDT3_spread_treed_summary_climate_gamma %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_lightning_NDT3_spread_treed_summary_climate_gamma2)
```
...
AIC_lightning_NDT3_spread_treed_VRI<-table.glm.VRI.simple.spread
...
AIC_lightning_NDT3_spread_treed_topo<-table.glm.topo.simple.spread
...
AIC_lightning_NDT3_spread_treed_infra<-table.glm.infra.simple.spread
...
Combine together
```{r}
NDT3_l_treed_spread<-rbind(AIC_lightning_NDT3_spread_treed_summary_climate2, AIC_lightning_NDT3_spread_treed_summary_infra2, AIC_lightning_NDT3_spread_treed_summary_topo2, AIC_lightning_NDT3_spread_treed_summary_VRI2)
...
Save data.
```{r}
write.csv(NDT3_l_treed_spread, file="D:\\Fire\\fire_data\\raw_data\\NDT3_lightning_models_treed_spread_Nov.csv")
```
...
2. dist_mun + dist_dam + dist_nat + dist_mine + bclcs_level_5_2 + dist_mine:bclcs_level_5_2
3. slope + aspect + slope:aspect
4. climate1 + climate2 + vegtype2

Note: R-square values were all very low, all <0.05, and adjusted Rsquare all < 0.04.


##################### Begin Manual Exploration ##################
Begin manual exploration from here. We will use the above variables, plus additional interactions: wind_atfire + climate1:proj_height_1 + climate1:proj_age_1 + climate1:live_stand_volume_125 + elevation +climate1:elevation + climate1:wind_atfire + slope:wind_atfire + elevation:wind_atfire + vegtype2:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire + Tdif_atfire:live_stand_volume_125


```{r}
spread_lightning_t_NDT3<-subset(spread_data_lightning_t, spread_data_lightning_t$ntrl_ds=="NDT3")

prop<-0.75

#Partition data into training and validation
 trainIndex <- createDataPartition(spread_lightning_t_NDT3$escape10, p = prop,
                                    list = FALSE,
                                    times = 1)
  
   dat1 <- spread_lightning_t_NDT3[ trainIndex,]
   Valid <- spread_lightning_t_NDT3[-trainIndex,]
  
#Run model using dat1
model.NDT3<-lm(LNsize_ha ~ proj_height_1 + proj_age_1 + live_stand_volume_125 + dist_mun + dist_dam + dist_nat + dist_mine + bclcs_level_5_2 + dist_mine:bclcs_level_5_2 + slope + aspect + slope:aspect + climate1 + climate2 + vegtype2 + wind_atfire + climate1:proj_height_1 + climate1:proj_age_1 + climate1:live_stand_volume_125 + elevation + climate1:elevation + climate1:wind_atfire + slope:wind_atfire + elevation:wind_atfire + vegtype2:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire + Tdif_atfire:live_stand_volume_125, data = dat1)

AIC(model.NDT3) #1451.6
summary(model.NDT3)$r.squared #0.27
summary(model.NDT3)$adj.r.squared #0.17

predictions <- model.NDT3 %>% predict(Valid)
R2(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.055
RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE) #6.73
MAE(predictions, Valid$LNsize_ha, na.rm=TRUE) #2.88
...
#Remove least sig
model.NDT3<-lm(LNsize_ha ~ proj_height_1 + proj_age_1 + live_stand_volume_125 + dist_mun + dist_dam + dist_nat + dist_mine + bclcs_level_5_2 + dist_mine:bclcs_level_5_2 + slope + aspect + slope:aspect + climate1 + climate2 + vegtype2 + wind_atfire + climate1:proj_height_1 + climate1:proj_age_1 + climate1:live_stand_volume_125 + elevation + climate1:elevation + climate1:wind_atfire + slope:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire + Tdif_atfire:live_stand_volume_125, data = dat1)

AIC(model.NDT3) #1444.4
summary(model.NDT3)$r.squared #0.27
summary(model.NDT3)$adj.r.squared #0.18

predictions <- model.NDT3 %>% predict(Valid)
R2(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.16
RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE) #2.52
MAE(predictions, Valid$LNsize_ha, na.rm=TRUE) #2.03
...
#Remove least sig
model.NDT3<-lm(LNsize_ha ~ proj_height_1 + proj_age_1 + live_stand_volume_125 + dist_mun + dist_dam + dist_nat + dist_mine + bclcs_level_5_2 + dist_mine:bclcs_level_5_2 + slope + aspect + slope:aspect + climate1 + climate2 + vegtype2 + wind_atfire + climate1:proj_height_1 + climate1:proj_age_1 + climate1:live_stand_volume_125 + elevation + climate1:elevation + slope:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire, data = dat1)

AIC(model.NDT3) #1440.7
summary(model.NDT3)$r.squared #0.27
summary(model.NDT3)$adj.r.squared #0.19

predictions <- model.NDT3 %>% predict(Valid)
R2(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.16
RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE) #2.51
MAE(predictions, Valid$LNsize_ha, na.rm=TRUE) #2.03
...
#Remove least sig
model.NDT3<-lm(LNsize_ha ~ proj_height_1 + proj_age_1 + live_stand_volume_125 + dist_mun + dist_dam + dist_nat + dist_mine + bclcs_level_5_2 + dist_mine:bclcs_level_5_2 + slope + aspect + slope:aspect + climate1 + climate2 + vegtype2 + wind_atfire + climate1:proj_height_1 + climate1:proj_age_1 + climate1:live_stand_volume_125 + elevation + slope:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire, data = dat1)

AIC(model.NDT3) #1439.2
summary(model.NDT3)$r.squared #0.27
summary(model.NDT3)$adj.r.squared #0.19

predictions <- model.NDT3 %>% predict(Valid)
R2(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.17
RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE) #2.49
MAE(predictions, Valid$LNsize_ha, na.rm=TRUE) #2.03
...
#Remove least sig
model.NDT3<-lm(LNsize_ha ~ proj_height_1 + proj_age_1 + live_stand_volume_125 + dist_mun + dist_dam + dist_nat + dist_mine + bclcs_level_5_2 + dist_mine:bclcs_level_5_2 + slope + aspect + slope:aspect + climate1 + climate2 + vegtype2 + wind_atfire + climate1:live_stand_volume_125 + elevation + slope:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire, data = dat1)

AIC(model.NDT3) #1438.3
summary(model.NDT3)$r.squared #0.26
summary(model.NDT3)$adj.r.squared #0.19

predictions <- model.NDT3 %>% predict(Valid)
R2(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.16
RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE) #2.50
MAE(predictions, Valid$LNsize_ha, na.rm=TRUE) #2.04
...
#Remove least sig
model.NDT3<-lm(LNsize_ha ~ proj_height_1 + proj_age_1 + live_stand_volume_125 + dist_mun + dist_dam + dist_nat + dist_mine + bclcs_level_5_2 + slope + aspect + slope:aspect + climate1 + climate2 + vegtype2 + wind_atfire + climate1:live_stand_volume_125 + elevation + slope:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire, data = dat1)

AIC(model.NDT3) #1437.2
summary(model.NDT3)$r.squared #0.25
summary(model.NDT3)$adj.r.squared #0.19
...
#Remove least sig
model.NDT3<-lm(LNsize_ha ~ proj_height_1 + proj_age_1 + live_stand_volume_125 + dist_mun + dist_nat + dist_mine + slope + aspect + slope:aspect + climate1 + climate2 + vegtype2 + wind_atfire + climate1:live_stand_volume_125 + elevation + slope:wind_atfire + Tdif_atfire + Tdif_atfire:wind_atfire, data = dat1)

AIC(model.NDT3) #1432.3
summary(model.NDT3)$r.squared #0.25
summary(model.NDT3)$adj.r.squared #0.19
...
RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE) #2.52
MAE(predictions, Valid$LNsize_ha, na.rm=TRUE) #2.06
...
#Remove least sig
model.NDT3<-lm(LNsize_ha ~ proj_height_1 + proj_age_1 + live_stand_volume_125 + dist_mun + dist_nat + dist_mine + slope + aspect + slope:aspect + climate1 + climate2 + vegtype2 + wind_atfire + climate1:live_stand_volume_125 + elevation + slope:wind_atfire + Tdif_atfire, data = dat1)

AIC(model.NDT3) #1431.7
summary(model.NDT3)$r.squared #0.25
summary(model.NDT3)$adj.r.squared #0.19
...
R2(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.15
RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE) #2.52
MAE(predictions, Valid$LNsize_ha, na.rm=TRUE) #2.05
...
#Remove least sig
model.NDT3<-lm(LNsize_ha ~ proj_height_1 + proj_age_1 + live_stand_volume_125 + dist_mun + dist_nat + dist_mine + slope + aspect + slope:aspect + climate1 + climate2 + vegtype2 + wind_atfire + climate1:live_stand_volume_125 + elevation + slope:wind_atfire, data = dat1)

AIC(model.NDT3) #1429.8
summary(model.NDT3)$r.squared #0.24
summary(model.NDT3)$adj.r.squared #0.19

predictions <- model.NDT3 %>% predict(Valid)
R2(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.15
RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE) #2.53
MAE(predictions, Valid$LNsize_ha, na.rm=TRUE) #2.05
...
#Remove least sig
model.NDT3<-lm(LNsize_ha ~ proj_height_1 + proj_age_1 + live_stand_volume_125 + dist_mun + dist_nat + slope + aspect + slope:aspect + climate1 + climate2 + vegtype2 + wind_atfire + climate1:live_stand_volume_125 + elevation + slope:wind_atfire, data = dat1)

AIC(model.NDT3) #1429.3
summary(model.NDT3)$r.squared #0.24
summary(model.NDT3)$adj.r.squared #0.19

predictions <- model.NDT3 %>% predict(Valid)
R2(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.14
RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE) #2.55
MAE(predictions, Valid$LNsize_ha, na.rm=TRUE) #2.08
...
#Remove least sig
model.NDT3<-lm(LNsize_ha ~ proj_age_1 + live_stand_volume_125 + dist_mun + dist_nat + slope + aspect + slope:aspect + climate1 + climate2 + vegtype2 + wind_atfire + climate1:live_stand_volume_125 + elevation + slope:wind_atfire, data = dat1)

AIC(model.NDT3) #1428.7
summary(model.NDT3)$r.squared #0.24
summary(model.NDT3)$adj.r.squared #0.19

predictions <- model.NDT3 %>% predict(Valid)
R2(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.13
RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE) #2.57
MAE(predictions, Valid$LNsize_ha, na.rm=TRUE) #2.08
...
#Remove least sig
model.NDT3<-lm(LNsize_ha ~ proj_age_1 + live_stand_volume_125 + dist_mun + dist_nat + slope + aspect + slope:aspect + climate1 + climate2 + vegtype2 + wind_atfire + elevation + slope:wind_atfire, data = dat1)

AIC(model.NDT3) #1428.0
summary(model.NDT3)$r.squared #0.23
summary(model.NDT3)$adj.r.squared #0.19

predictions <- model.NDT3 %>% predict(Valid)
R2(predictions, Valid$LNsize_ha, na.rm=TRUE) #0.14
RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE) #2.53
MAE(predictions, Valid$LNsize_ha, na.rm=TRUE) #2.07
...
NDT3_spread_size_t<-spread_lightning_t_NDT3 %>% drop_na(proj_age_1, live_stand_volume_125 , dist_mun, dist_nat , slope , aspect , climate1 , climate2, vegtype2 , wind_atfire, elevation)

#Run Model again with this data; but uses all data here
model.NDT3.Spread<-lm(LNsize_ha ~ proj_age_1 + live_stand_volume_125 + dist_mun + dist_nat + slope + aspect + slope:aspect + climate1 + climate2 + vegtype2 + wind_atfire + elevation + slope:wind_atfire, data = NDT3_spread_size_t)
...
#Partial Residuals
#proj_age_1 + live_stand_volume_125 + dist_mun + dist_nat + slope + aspect + slope:aspect + climate1 + climate2 + vegtype2 + wind_atfire + elevation + slope:wind_atfire
visreg(model.NDT3.Spread, "proj_age_1")
visreg(model.NDT3.Spread, "live_stand_volume_125")

visreg(model.NDT3.Spread, "dist_mun")
visreg(model.NDT3.Spread, "dist_nat")

visreg(model.NDT3.Spread, "slope", by="aspect")
visreg(model.NDT3.Spread, "slope", by="wind_atfire")
visreg(model.NDT3.Spread, "aspect", by="slope")
visreg(model.NDT3.Spread, "elevation")

#climate1 + climate2 + vegtype2 + wind_atfire + elevation + slope:wind_atfire

visreg(model.NDT3.Spread, "climate1")
visreg(model.NDT3.Spread, "climate2")
visreg(model.NDT3.Spread, "vegtype2")
visreg(model.NDT3.Spread, "wind_atfire", by="slope")
...
top_mod_table_NDT3_lightning_t_ALL <- data.frame (matrix (ncol = 27, nrow = 0))
colnames (top_mod_table_NDT3_lightning_t_ALL ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_proj_age_1", "coef_live_stand_volume_125", "coef_dist_mun", "coef_dist_nat", "coef_slope", "coef_aspect", "coef_climate_1", "coef_climate_2", "coef_vegtypeOP", "coef_vegtypeTB", "coef_vegtypeTC", "coef_vegtypeTM", "coef_wind_atfire", "coef_elevation",  "coef_slope:aspect", "coef_slope:wind_atfire", "AIC", "Model.R2", "Valid.R2", "Valid.RSME", "Valid.MAE", "Adjusted.R2")
...
#Model   
model.NDT3.Spread<-lm(LNsize_ha ~ proj_age_1 + live_stand_volume_125 + dist_mun + dist_nat + slope + aspect + slope:aspect + climate1 + climate2 + vegtype2 + wind_atfire + elevation + slope:wind_atfire, data = dat1) 

predictions <- model.NDT3.Spread %>% predict(Valid)

# create model table (only do this once) and add the relevant data
top_mod_table_NDT3_lightning_t <- data.frame (matrix (ncol = 27, nrow = 0))
colnames (top_mod_table_NDT3_lightning_t ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_proj_age_1", "coef_live_stand_volume_125", "coef_dist_mun", "coef_dist_nat", "coef_slope", "coef_aspect", "coef_climate_1", "coef_climate_2", "coef_vegtypeOP", "coef_vegtypeTB", "coef_vegtypeTC", "coef_vegtypeTM", "coef_wind_atfire", "coef_elevation",  "coef_slope:aspect", "coef_slope:wind_atfire", "AIC", "Model.R2", "Valid.R2", "Valid.RSME", "Valid.MAE", "Adjusted.R2")

##Add data for NDT3
top_mod_table_NDT3_lightning_t[1,1]<-"lightning"
top_mod_table_NDT3_lightning_t[1,2]<-"NDT3"
top_mod_table_NDT3_lightning_t[1,3]<-"Y"
top_mod_table_NDT3_lightning_t[1,4]<-"LNsize_ha ~ proj_age_1 + live_stand_volume_125 + dist_mun + dist_nat + slope + aspect + slope:aspect + climate1 + climate2 + vegtype2 + wind_atfire + elevation + slope:wind_atfire" 
top_mod_table_NDT3_lightning_t[1,5]<- coef(model.NDT3.Spread)[1] #Intercept
top_mod_table_NDT3_lightning_t[1,6]<- coef(model.NDT3.Spread)[2] #C
top_mod_table_NDT3_lightning_t[1,7]<- coef(model.NDT3.Spread)[3] #C
top_mod_table_NDT3_lightning_t[1,8]<- coef(model.NDT3.Spread)[4] #co
top_mod_table_NDT3_lightning_t[1,9]<- coef(model.NDT3.Spread)[5] #co
top_mod_table_NDT3_lightning_t[1,10]<- coef(model.NDT3.Spread)[6] #c
top_mod_table_NDT3_lightning_t[1,11]<- coef(model.NDT3.Spread)[7] #l
top_mod_table_NDT3_lightning_t[1,12]<- coef(model.NDT3.Spread)[8] #co
top_mod_table_NDT3_lightning_t[1,13]<- coef(model.NDT3.Spread)[9] #c
top_mod_table_NDT3_lightning_t[1,14]<- coef(model.NDT3.Spread)[10] #coeff
top_mod_table_NDT3_lightning_t[1,15]<- coef(model.NDT3.Spread)[11] #co
top_mod_table_NDT3_lightning_t[1,16]<- coef(model.NDT3.Spread)[12] #coe
top_mod_table_NDT3_lightning_t[1,17]<- coef(model.NDT3.Spread)[13] #co
top_mod_table_NDT3_lightning_t[1,18]<- coef(model.NDT3.Spread)[14] #co
top_mod_table_NDT3_lightning_t[1,19]<- coef(model.NDT3.Spread)[15] #
top_mod_table_NDT3_lightning_t[1,20]<- coef(model.NDT3.Spread)[16] # cl
top_mod_table_NDT3_lightning_t[1,21]<- coef(model.NDT3.Spread)[17] # c
top_mod_table_NDT3_lightning_t[1,22]<- AIC(model.NDT3.Spread)
top_mod_table_NDT3_lightning_t[1,23]<- summary(model.NDT3.Spread)$r.squared
top_mod_table_NDT3_lightning_t[1,24]<- R2(predictions, Valid$LNsize_ha, na.rm=TRUE)
top_mod_table_NDT3_lightning_t[1,25]<- RMSE(predictions, Valid$LNsize_ha, na.rm=TRUE)
top_mod_table_NDT3_lightning_t[1,26]<- MAE(predictions, Valid$LNsize_ha, na.rm=TRUE)
top_mod_table_NDT3_lightning_t[1,27]<- summary(model.NDT3.Spread)$adj.r.squared

top_mod_table_NDT3_lightning_t_ALL<-rbind(top_mod_table_NDT3_lightning_t_ALL, top_mod_table_NDT3_lightning_t)
...
Check.
```{r}
head(top_mod_table_NDT3_lightning_t_ALL)
...
```{r}
write.csv(top_mod_table_NDT3_lightning_t_ALL, file="D:\\Fire\\fire_data\\raw_data\\top_mod_model.NDT3.Spread_firesize_lightning_t.csv")
```
...
```{r}
names(top_mod_table_NDT3_lightning_t_ALL)

top_mod_table_NDT3_model.NDT3.Spreadlightning_t_means<-top_mod_table_NDT3_lightning_t_ALL %>% summarise_each(funs( mean( .,na.rm = TRUE)))
top_mod_table_NDT3_model.NDT3.Spreadlightning_t_means

top_mod_table_NDT3_model.NDT3.Spreadlightning_t_means[1,1]<-"lightning"
top_mod_table_NDT3_model.NDT3.Spreadlightning_t_means[1,2]<-"NDT3"
top_mod_table_NDT3_model.NDT3.Spreadlightning_t_means[1,3]<-"Treed"
top_mod_table_NDT3_model.NDT3.Spreadlightning_t_means[1,4]<- "LNsize_ha ~ proj_age_1 + live_stand_volume_125 + dist_mun + dist_nat + slope + aspect + slope:aspect + climate1 + climate2 + vegtype2 + wind_atfire + elevation + slope:wind_atfire" 
top_mod_table_NDT3_model.NDT3.Spreadlightning_t_means
```
Save table.

```{r}
write.csv(top_mod_table_NDT3_model.NDT3.Spreadlightning_t_means, file="D:\\Fire\\fire_data\\raw_data\\top_mod_model.NDT3.Spread_firesize_lightning_t_Means.csv")
```

Standard deviation.

```{r}
top_mod_table_NDT3_model.NDT3.Spreadlightning_t_sd<-top_mod_table_NDT3_lightning_t_ALL %>% summarise_each(funs( sd( .,na.rm = TRUE)))
top_mod_table_NDT3_model.NDT3.Spreadlightning_t_sd

top_mod_table_NDT3_model.NDT3.Spreadlightning_t_sd[1,1]<-"lightning"
top_mod_table_NDT3_model.NDT3.Spreadlightning_t_sd[1,2]<-"NDT3"
top_mod_table_NDT3_model.NDT3.Spreadlightning_t_sd[1,3]<-"Treed"
top_mod_table_NDT3_model.NDT3.Spreadlightning_t_sd[1,4]<-"LNsize_ha ~ proj_age_1 + live_stand_volume_125 + dist_mun + dist_nat + slope + aspect + slope:aspect + climate1 + climate2 + vegtype2 + wind_atfire + elevation + slope:wind_atfire" 
top_mod_table_NDT3_model.NDT3.Spreadlightning_t_sd
```
...
```{r}
write.csv(top_mod_table_NDT3_model.NDT3.Spreadlightning_t_sd, file="D:\\Fire\\fire_data\\raw_data\\top_mod_model.NDT3.Spread_firesize_lightning_t_SD.csv")
```
