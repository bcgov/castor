---
title: "12e_2_fire_escape_model_selection_NDT5_NotTreed"
author: "Cora Skaien"
date: "07/10/2021"
...
#=================================
#  Script Name: 12e_2_fire_escape_model_selection_NDT5_NotTreed.R
#  Script Version: 1.0
...
length(mods.twoway.NDT5b) #512
#mods.twoway.NDT5b
mods.twoway.NDT5b[1]
mods.twoway.NDT5b<-mods.twoway.NDT5b[-1]
...
all.poss.mods.infra<-c(1, mods.inter.NDT5b) 
...
#Because we have such low sample size, we cannot assess the full suite of categories. Vegtype will need to be assessed separately.
all.poss.mods.clim.vegtype2
all.poss.mods.clim.vegtype2b

#Proportion also needs to be reduced so that we have presences and absences in both categories
Run the model fewer than 100 times given small sample size as the loop often fails with different ways the data gets subset.
...
Now that we have run the model 10 times, we want the average AIC and AUC for each variable combination.

```{r}
head(table.glm.climate.simple)
table(table.glm.climate.simple$model) # 100 per model

AIC_lightning_NDT5_escape_nottreed_climate<-table.glm.climate.simple

AIC_lightning_NDT5_escape_nottreed_summary_climate<- AIC_lightning_NDT5_escape_nottreed_climate %>%
  group_by(model) %>%
...
AIC_lightning_NDT5_escape_nottreed_summary_climate2<- AIC_lightning_NDT5_escape_nottreed_summary_climate %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_lightning_NDT5_escape_nottreed_summary_climate2)
```

#Now repeat for topography. 
Change the number of iterations to fewer than 100 to increase chances of below working; with each subsetting of data, there can be data errors, and with such a small sample size, this often makes the loop fail.
...
AIC_lightning_NDT5_escape_nottreed_topo<-table.glm.topo.simple

AIC_lightning_NDT5_escape_nottreed_summary_topo<- AIC_lightning_NDT5_escape_nottreed_topo %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic),
            meanAUC=mean(auc.valid),
            sdAUC=sd(auc.valid),
...
AIC_lightning_NDT5_escape_nottreed_summary_topo2<- AIC_lightning_NDT5_escape_nottreed_summary_topo %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_lightning_NDT5_escape_nottreed_summary_topo2)
```
...
table(table.glm.infra.simple$model) # 10 per model; here, successfully ran 6 times

AIC_lightning_NDT5_escape_nottreed_infra<-table.glm.infra.simple

AIC_lightning_NDT5_escape_nottreed_summary_infra<- AIC_lightning_NDT5_escape_nottreed_infra %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic),
            meanAUC=mean(auc.valid),
            sdAUC=sd(auc.valid),
...
AIC_lightning_NDT5_escape_nottreed_summary_infra2<- AIC_lightning_NDT5_escape_nottreed_summary_infra %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_lightning_NDT5_escape_nottreed_summary_infra2)
...
```{r}
NDT5_l_models_nottreed<-rbind(AIC_lightning_NDT5_escape_nottreed_summary_climate2, AIC_lightning_NDT5_escape_nottreed_summary_topo2, AIC_lightning_NDT5_escape_nottreed_summary_infra2)
NDT5_l_models_nottreed
NDT5_l_models_nottreed$NDT<-"NDT5"

write.csv(NDT5_l_models_nottreed, file="D:\\Fire\\fire_data\\raw_data\\NDT5_lightning_models_nottreed_escape.csv")
```
...
2. slope + aspect + elevation + wind_atfire + aspect:wind_atfire #AUC < 0.5! Performed worse than by chance...
3.dist_dam + dist_mine + roads_km + dist_mine:roads_km

Tdif_atfire:climate1  + Tdif_atfire:wind_atfire + Tdif_atfire + Tdif_atfire:climate2
...
```{r}
escape_lightning_nt_NDT5<-subset(Escape_data_lightning_nt, Escape_data_lightning_nt$ntrl_ds=="NDT5")

table(escape_lightning_nt_NDT5$vegtype2) #27 observations... yikes. Not much to work with.
table(escape_lightning_nt_NDT5$escape) #23 did not escape, 4 did
table(escape_lightning_nt_NDT5$vegtype2, escape_lightning_nt_NDT5$escape)
table(escape_lightning_nt_NDT5$climate1)
table(escape_lightning_nt_NDT5$climate2)
```

The sample size is too small to be able to run the loops prior. Instead, we will manually explore.

```{r}
#Divide data into training and valid
prop<-0.70
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(escape_lightning_nt_NDT5$escape, p = prop,
                                    list = FALSE,
                                    times = 1)
  
   dat1 <- escape_lightning_nt_NDT5[ trainIndex,]
   Valid <- escape_lightning_nt_NDT5[-trainIndex,]

#Run model using dat1
model.NDT5.E<-glm(escape ~ climate1 + climate2 + climate1:climate2 + slope + aspect + elevation + wind_atfire + aspect:wind_atfire + dist_dam + dist_mine + roads_km + dist_mine:roads_km, family = binomial, data = dat1)
...
#overfit... will not work. Look at results and pick and choose and compare.

#create logical model
model.NDT5.E<-glm(escape ~ climate1 + climate2 + climate1:climate2 + slope + aspect + elevation + wind_atfire + dist_dam + dist_mine + roads_km + dist_mun, family = binomial, data = dat1)
...
Anova(model.NDT5.E, type=3) #Still does not work; no convergence

#create logical model
model.NDT5.E<-glm(escape ~ climate1 + climate2 + slope + elevation + wind_atfire + dist_dam + dist_mine + roads_km + dist_mun, family = binomial, data = dat1)

AIC(model.NDT5.E) #236.26
...
#create logical model
model.NDT5.E<-glm(escape ~ climate1 + climate2 + slope + elevation + wind_atfire +  roads_km + dist_mun, family = binomial, data = dat1)

AIC(model.NDT5.E) #24.8
...
Anova(model.NDT5.E, type=3) #Converges, but too many variables

#create logical model
model.NDT5.E<-glm(escape ~ climate1 + climate2 + slope + elevation + wind_atfire +  roads_km, family = binomial, data = dat1)

AIC(model.NDT5.E) #22.8
...
#create logical model
model.NDT5.E<-glm(escape ~ climate1 + climate2 + climate1:climate2, family = binomial, data = dat1)
...
#create logical model
model.NDT5.E<-glm(escape ~ climate1 + climate2, family = binomial, data = dat1)

AIC(model.NDT5.E) #17.9
...
Anova(model.NDT5.E, type=3) #Converges,

#create logical model
model.NDT5.E<-glm(escape ~ climate1, family = binomial, data = dat1)

AIC(model.NDT5.E) #15.9
...
#create logical model
model.NDT5.E<-glm(escape ~ climate1 + roads_km, family = binomial, data = dat1)

AIC(model.NDT5.E) #17.2
...
#create logical model
model.NDT5.E<-glm(escape ~ climate1 + dist_dam, family = binomial, data = dat1)

AIC(model.NDT5.E) #17.8
...
##Compare with Tdif with new subset
model.NDT5.E<-glm(escape ~ climate1, family = binomial, data = dat1)

AIC(model.NDT5.E) #18.5
...
# Try with tdif
model.NDT5.E<-glm(escape ~ Tdif_atfire, family = binomial, data = dat1)

AIC(model.NDT5.E) #20.5
...
#
model.NDT5.E<-glm(escape ~ climate2, family = binomial, data = dat1)

AIC(model.NDT5.E) #20.0
...
#
model.NDT5.E<-glm(escape ~ Tdif_atfire + climate2, family = binomial, data = dat1)
...
NDT5_escape_nt<-escape_lightning_nt_NDT5 %>% drop_na(climate1)

#Run Model again with this data; but uses all data here
model.NDT5.E<-glm(escape ~ climate1, family = binomial, data = NDT5_escape_nt)

Anova(model.NDT5.E, type=3) #climate1*elevation REALLY not significant here!
#Anova(model.NDT5.E, type=3, singular.ok = TRUE)
...
NDT5_escape_nt$resids<-resid(model.NDT5.E)


binnedplot (NDT5_escape_nt$climate1, 
            NDT5_escape_nt$resids, 
            nclass = NULL, 
            xlab = "", 
            ylab = "Average residual", 
            main = paste("Binned Residual Plot - glm", i))

# Diagnostic plots look... not great
visreg(model.NDT5.E, "climate1")
```
...
```{r}
#Remove NAs to ensure all same data used so we can compare AICs
NDT5_escape_nt<-escape_lightning_nt_NDT5 %>% drop_na(Tdif_atfire)

#Run Model again with this data; but uses all data here
model.NDT5.E<-glm(escape ~ Tdif_atfire, family = binomial, data = NDT5_escape_nt)
...
Repeat with climate1 and climate2


```{r}
#Remove NAs to ensure all same data used so we can compare AICs
NDT5_escape_nt<-escape_lightning_nt_NDT5 %>% drop_na(climate1, climate2)

#Run Model again with this data; but uses all data here
model.NDT5.E<-glm(escape ~ climate1 +climate2, family = binomial, data = NDT5_escape_nt)
...
#First we will use climate1

```{r}
summary(model.NDT5.E)

#Create a new blank table and get AUC too
top_mod_table_NDT5_light_nt_ALL <- data.frame (matrix (ncol = 7, nrow = 0))
colnames (top_mod_table_NDT5_light_nt_ALL ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1","AUC")
```

Let's run it 500 times to get good mean values. Run the below as many times as possible to get at least 100 successful runs appended to the table.
...
prop<-0.5
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(NDT5_escape_nt$escape, p = prop,
                                    list = FALSE,
                                    times = 1)
  
   dat1 <- NDT5_escape_nt[ trainIndex,]
   Valid <- NDT5_escape_nt[-trainIndex,]
   
#Model   
model.NDT5.E<-glm(escape ~ climate1, data = dat1) 
...
top_mod_table_NDT5_light_nt <- data.frame (matrix (ncol = 7, nrow = 0))
colnames (top_mod_table_NDT5_light_nt ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "AUC")

##Add data for NDT5
top_mod_table_NDT5_light_nt[1,1]<-"lightning"
top_mod_table_NDT5_light_nt[1,2]<-"NDT5"
top_mod_table_NDT5_light_nt[1,3]<-"N"
top_mod_table_NDT5_light_nt[1,4]<-"escape ~ climate" 
top_mod_table_NDT5_light_nt[1,5]<- coef(model.NDT5.E)[1] #Intercept
top_mod_table_NDT5_light_nt[1,6]<- coef(model.NDT5.E)[2] #Climate variable 1
top_mod_table_NDT5_light_nt[1,7]<- mod.auc
...
```{r}
write.csv(top_mod_table_NDT5_light_nt_ALL, file="D:\\Fire\\fire_data\\raw_data\\top_mod_escape_NDT5_light_nt_ALL.csv")
```
...
mean(top_mod_table_NDT5_light_nt_ALL$AUC) #0.72

# create model table (only do this once) and add the relevant data
top_mod_table_NDT5_light_nt_Means <- data.frame (matrix (ncol = 7, nrow = 0))
colnames (top_mod_table_NDT5_light_nt_Means ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1","AUC")
...
top_mod_table_NDT5_light_nt_Means[1,4]<-"escape ~ climate1" 
top_mod_table_NDT5_light_nt_Means[1,5]<- mean(top_mod_table_NDT5_light_nt_ALL$intercept) #Intercept
top_mod_table_NDT5_light_nt_Means[1,6]<- mean(top_mod_table_NDT5_light_nt_ALL$coef_climate_1, na.rm=TRUE) #Climate variable 1
top_mod_table_NDT5_light_nt_Means[1,7]<- mean(top_mod_table_NDT5_light_nt_ALL$AUC, na.rm=TRUE)
...
```{r}
write.csv(top_mod_table_NDT5_light_nt_Means, file="D:\\Fire\\fire_data\\raw_data\\top_mod_escape_NDT5_light_nt_Means.csv")
```
...
```{r}
# create model table (only do this once) and add the relevant data
top_mod_table_NDT5_light_nt_SD <- data.frame (matrix (ncol = 7, nrow = 0))
colnames (top_mod_table_NDT5_light_nt_SD ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1","AUC")
...
top_mod_table_NDT5_light_nt_SD[1,4]<-"escape ~ climate1" 
top_mod_table_NDT5_light_nt_SD[1,5]<- sd(top_mod_table_NDT5_light_nt_ALL$intercept) #Intercept
top_mod_table_NDT5_light_nt_SD[1,6]<- sd(top_mod_table_NDT5_light_nt_ALL$coef_climate_1, na.rm=TRUE) #Climate variable 1
top_mod_table_NDT5_light_nt_SD[1,7]<- sd(top_mod_table_NDT5_light_nt_ALL$AUC, na.rm=TRUE)
...
```{r}
write.csv(top_mod_table_NDT5_light_nt_SD, file="D:\\Fire\\fire_data\\raw_data\\top_mod_escape_NDT5_light_nt_SD.csv")
```


#Now try with climate1 and climate2

```{r}
summary(model.NDT5.E)

#Create a new blank table and get AUC too
top_mod_table_NDT5_light_nt_ALL <- data.frame (matrix (ncol = 8, nrow = 0))
colnames (top_mod_table_NDT5_light_nt_ALL ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_climate_2", "AUC")
```
...
#Model   
model.NDT5.E<-glm(escape ~ climate1 + climate2, data = dat1) 
...
# create model table (only do this once) and add the relevant data
top_mod_table_NDT5_light_nt <- data.frame (matrix (ncol = 8, nrow = 0))
colnames (top_mod_table_NDT5_light_nt ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_climate_2", "AUC")

##Add data for NDT5
top_mod_table_NDT5_light_nt[1,1]<-"lightning"
top_mod_table_NDT5_light_nt[1,2]<-"NDT5"
top_mod_table_NDT5_light_nt[1,3]<-"N"
top_mod_table_NDT5_light_nt[1,4]<-"escape ~ climate1 + climate2" 
top_mod_table_NDT5_light_nt[1,5]<- coef(model.NDT5.E)[1] #Intercept
top_mod_table_NDT5_light_nt[1,6]<- coef(model.NDT5.E)[2] #Climate variable 1
top_mod_table_NDT5_light_nt[1,7]<- coef(model.NDT5.E)[3] #Climate variable 2
top_mod_table_NDT5_light_nt[1,8]<- mod.auc
...
```{r}
write.csv(top_mod_table_NDT5_light_nt_ALL, file="D:\\Fire\\fire_data\\raw_data\\top_mod_escape_NDT5_light_nt_ALL_.csv")
```
...
```{r}
names(top_mod_table_NDT5_light_nt_ALL)
mean(top_mod_table_NDT5_light_nt_ALL$AUC) #0.67

# create model table (only do this once) and add the relevant data
top_mod_table_NDT5_light_nt_Means <- data.frame (matrix (ncol = 8, nrow = 0))
colnames (top_mod_table_NDT5_light_nt_Means ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_climate_2", "AUC")
...
top_mod_table_NDT5_light_nt_Means[1,5]<- mean(top_mod_table_NDT5_light_nt_ALL$intercept) #Intercept
top_mod_table_NDT5_light_nt_Means[1,6]<- mean(top_mod_table_NDT5_light_nt_ALL$coef_climate_1, na.rm=TRUE) #Climate variable 1
top_mod_table_NDT5_light_nt_Means[1,7]<- mean(top_mod_table_NDT5_light_nt_ALL$coef_climate_2, na.rm=TRUE) #Climate variable 1
top_mod_table_NDT5_light_nt_Means[1,8]<- mean(top_mod_table_NDT5_light_nt_ALL$AUC, na.rm=TRUE)
...
```{r}
# create model table (only do this once) and add the relevant data
top_mod_table_NDT5_light_nt_SD <- data.frame (matrix (ncol = 8, nrow = 0))
colnames (top_mod_table_NDT5_light_nt_SD ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_climate_2", "AUC")
...
top_mod_table_NDT5_light_nt_SD[1,5]<- sd(top_mod_table_NDT5_light_nt_ALL$intercept) #Intercept
top_mod_table_NDT5_light_nt_SD[1,6]<- sd(top_mod_table_NDT5_light_nt_ALL$coef_climate_1, na.rm=TRUE) #Climate variable 1
top_mod_table_NDT5_light_nt_SD[1,7]<- sd(top_mod_table_NDT5_light_nt_ALL$coef_climate_2, na.rm=TRUE) #Climate variable 1
top_mod_table_NDT5_light_nt_SD[1,8]<- sd(top_mod_table_NDT5_light_nt_ALL$AUC, na.rm=TRUE)
