...
# there are only 4 observations in the C7 fuel type so since this fiel type has fire spread rates similar to C3 I will change all C1 observations to C3
#dat5$FWI_veg[dat5$FWI_veg=="C-1"]<-"C-3" # lumping C-1 and C-3 because they have very similar ROS (See pg 9 of Perrakis)
dat5$FWI_veg[dat5$FWI_veg=="C-7"]<-"C-3" # seems like C7 maybe most similar to C3 or C5 but since I have no C5 ill make it C3.
...
mod_frt_5a<- glm(fire ~ climate1 + elevatn + FWI_veg + rd_dst_ + infr_ds, data=dat5, family=binomial(link="logit"))

mod_frt_5b<- glm(fire ~ climate1 + elevatn + FWI_veg + rd_dst_, data=dat5, family=binomial(link="logit"))

mod_frt_5b_log<- glm(fire ~ climate1 + elevatn + FWI_veg + log(rd_dst_+1), data=dat5, family=binomial(link="logit"))


mod_frt_5c<- glm(fire ~ climate1 + elevatn + FWI_veg + infr_ds, data=dat5, family=binomial(link="logit"))

mod_frt_5c_log<- glm(fire ~ climate1 + elevatn + FWI_veg + log(infr_ds+1), data=dat5, family=binomial(link="logit"))


anova(mod_frt_5a, mod_frt_5b)

anova(mod_frt_5a, mod_frt_5c)

# best model is mod_frt_5b_log. It has the lowest AIC (AIC=419)

dat5$log_road_dist<-log(dat5$rd_dst_+1)

mod_frt5<- glm(fire ~ climate1 + elevatn + FWI_veg + log_road_dist, data=dat5, family=binomial(link="logit"))
...
dat2_b<-dat5 %>% dplyr::select(fire, elevatn, climate1, log_road_dist)
...
model.data %>% 
  dplyr::select(climate1,  elevatn, FWI_veg, log_road_dist,.std.resid, .cooksd) %>%
  top_n(5, .cooksd) 
...
```{r}
mod_frt5<- glm(fire ~ climate1 + elevatn + log_road_dist + FWI_veg, data=dat5, family=binomial(link="logit"))

summary(mod_frt5)

#Create a new blank table and get AUC too
top_mod_table_FRT5_person_All <- data.frame (matrix (ncol = 13, nrow = 0))
colnames (top_mod_table_FRT5_person_All ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1", "coef_elevatn", "coef_log_road_dist", "coef_FWI_veg_C-2",  "coef_FWI_veg_C-3", "coef_FWI_veg_D-1/2", "coef_FWI_veg_M-1/2", "coef_FWI_veg_O-1a/b", "AUC")
...
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(dat5$fire_veg, p = prop,
                                    list = FALSE,
                                    times = 1)
...
top_mod_table_FRT5_person <- data.frame (matrix (ncol = 13, nrow = 0))
colnames (top_mod_table_FRT5_person ) <- c ("CAUSE", "FRT",  "Model_terms", "intercept", "coef_climate_1", "coef_elevatn", "coef_log_road_dist", "coef_FWI_veg_C-2",  "coef_FWI_veg_C-3", "coef_FWI_veg_D-1/2", "coef_FWI_veg_M-1/2", "coef_FWI_veg_1a/b", "AUC")

##Add data for NDT1
top_mod_table_FRT5_person[1,1]<-"person"
top_mod_table_FRT5_person[1,2]<-"FRT5"
top_mod_table_FRT5_person[1,3]<-"fire ~ climate1 + elevatn + log_road_dist + FWI_veg"
...
top_mod_table_FRT5_person[1,13]<- mod.auc
