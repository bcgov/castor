---
title: "09_2e_fire_ignition_model_selection_NDT5_person_treed"
author: "Cora Skaien"
date: "08/11/2021"
...
#=================================
#  Script Name: 09_2e_fire_ignition_model_selection_NDT5_person_treed.R
#  Script Version: 1.0
...
AIC_person_NDT5_fire_pres_treed_climate<-table.glm.climate.simple

AIC_person_NDT5_fire_pres_treed_summary_climate<- AIC_person_NDT5_fire_pres_treed_climate %>%
  group_by(model) %>%
...
AIC_person_NDT5_fire_pres_treed_summary_climate2<- AIC_person_NDT5_fire_pres_treed_summary_climate %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT5_fire_pres_treed_summary_climate2)
```
...
AIC_person_NDT5_fire_pres_treed_VRI<-table.glm.VRI.simple

AIC_person_NDT5_fire_pres_treed_summary_VRI<- AIC_person_NDT5_fire_pres_treed_VRI %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic),
            meanAUC=mean(auc.valid),
            sdAUC=sd(auc.valid),
...
AIC_person_NDT5_fire_pres_treed_summary_VRI2<- AIC_person_NDT5_fire_pres_treed_summary_VRI %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT5_fire_pres_treed_summary_VRI2)
```
...
AIC_person_NDT5_fire_pres_treed_topo<-table.glm.topo.simple

AIC_person_NDT5_fire_pres_treed_summary_topo<- AIC_person_NDT5_fire_pres_treed_topo %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic),
            meanAUC=mean(auc.valid),
            sdAUC=sd(auc.valid),
...
AIC_person_NDT5_fire_pres_treed_summary_topo2<- AIC_person_NDT5_fire_pres_treed_summary_topo %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT5_fire_pres_treed_summary_topo2)
```
...
AIC_person_NDT5_fire_pres_treed_infra<-table.glm.infra.simple

AIC_person_NDT5_fire_pres_treed_summary_infra<- AIC_person_NDT5_fire_pres_treed_infra %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic),
            meanAUC=mean(auc.valid),
            sdAUC=sd(auc.valid),
...
AIC_person_NDT5_fire_pres_treed_summary_infra2<- AIC_person_NDT5_fire_pres_treed_summary_infra %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT5_fire_pres_treed_summary_infra2)
...
```{r}
NDT5_l_models_treed<-rbind(AIC_person_NDT5_fire_pres_treed_summary_climate2, AIC_person_NDT5_fire_pres_treed_summary_VRI2, AIC_person_NDT5_fire_pres_treed_summary_topo2, AIC_person_NDT5_fire_pres_treed_summary_infra2)
NDT5_l_models_treed
NDT5_l_models_treed$NDT<-"NDT5"

write.csv(NDT5_l_models_treed, file="D:\\Fire\\fire_data\\raw_data\\NDT5_person_models_treed_ignition.csv")
```
...
Top Models:
1. climate1 + climate2 + vegtype2 + climate1:vegtype2 + climate2:vegtype2
2. proj_height_1 + proj_age_1 + live_stand_volume_125
3. slope + elevation
4. dist_mun + dist_dam + dist_nat + roads_km + dist_mun:roads_km
...
```{r}
ignition_person_t_NDT5<-subset(dat_person_t, dat_person_t$ntrl_ds=="NDT5")
#Only 150 observations for analysis, likely too many starting terms!

table(ignition_person_t_NDT5$vegtype2) #3 Tb, 147 TC. Remove vegtype from model
hist(ignition_person_t_NDT5$roads_km)
ignition_person_t_NDT5$roads_km #Mostly 0s

#Divide data into training and valid
prop<-0.75
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(ignition_person_t_NDT5$fire_pres, p = prop,
                                    list = FALSE,
                                    times = 1)
  
   dat1 <- ignition_person_t_NDT5[ trainIndex,]
   Valid <- ignition_person_t_NDT5[-trainIndex,]

#Run model using dat1
model.NDT5.E<-glm(fire_pres ~ climate1 + climate2 + vegtype2 + climate1:vegtype2 + climate2:vegtype2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + elevation + dist_mun + dist_dam + dist_nat + roads_km + dist_mun:roads_km + bclcs_level_5_2 + elevation*climate1 + elevation*climate2 + bclcs_level_5_2:roads_km, family = binomial, data = dat1)

#vegtype issue; remove
model.NDT5.E<-glm(fire_pres ~ climate1 + climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + elevation + dist_mun + dist_dam + dist_nat + roads_km + dist_mun:roads_km + bclcs_level_5_2 + elevation*climate1 + elevation*climate2 + bclcs_level_5_2:roads_km, family = binomial, data = dat1) #did not converge
...
#Remove least sig
model.NDT5.E<-glm(fire_pres ~ climate1 + climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + elevation + dist_mun + dist_dam + dist_nat + roads_km + dist_mun:roads_km + bclcs_level_5_2 + elevation*climate1 + elevation*climate2, family = binomial, data = dat1) #did not converge
...
#Too many variables; try two groups with highest AUC first and then add some others in later to test
model.NDT5.E<-glm(fire_pres ~ climate1 + climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + elevation + dist_mun + dist_dam + dist_nat + roads_km + dist_mun:roads_km + bclcs_level_5_2 + elevation*climate1 + elevation*climate2, family = binomial, data = dat1) #did not converge
...
#Remove proj height and age
model.NDT5.E<-glm(fire_pres ~ climate1 + climate2+ live_stand_volume_125 + elevation + dist_mun + dist_dam + dist_nat + roads_km + dist_mun:roads_km + bclcs_level_5_2 + elevation*climate1 + elevation*climate2, family = binomial, data = dat1) #did not converge

AIC(model.NDT5.E) #54.5
...
#Compare to just infrastructure model plus climate
model.NDT5.E<-glm(fire_pres ~ climate1 + climate2 + dist_mun + dist_dam + dist_nat + roads_km + dist_mun:roads_km + bclcs_level_5_2, family = binomial, data = dat1) #did not converge

AIC(model.NDT5.E) #90.8

#Determine AUC of full model
mod.valid <- predict.glm(model.NDT5.E, newdata=Valid, type="response")
   roc_obj <- roc(Valid[,"fire_pres"], mod.valid)
   mod.auc <- auc(roc_obj)
   mod.auc #0.94 --> insane predictive power
   
Anova(model.NDT5.E, type=3)
Anova(model.NDT5.E, type=3, singular.ok = TRUE)

#Remove least sig
model.NDT5.E<-glm(fire_pres ~ climate1 + climate2 + dist_mun + dist_dam + dist_nat + roads_km + dist_mun:roads_km, family = binomial, data = dat1) #did not converge

AIC(model.NDT5.E) #87.14

#Determine AUC of full model
mod.valid <- predict.glm(model.NDT5.E, newdata=Valid, type="response")
   roc_obj <- roc(Valid[,"fire_pres"], mod.valid)
   mod.auc <- auc(roc_obj)
   mod.auc #0.93 --> insane predictive power
   
Anova(model.NDT5.E, type=3)
Anova(model.NDT5.E, type=3, singular.ok = TRUE)

#Remove least sig
model.NDT5.E<-glm(fire_pres ~ climate1 + climate2 + dist_mun + dist_dam + roads_km + dist_mun:roads_km, family = binomial, data = dat1) #did not converge

AIC(model.NDT5.E) #85.8

#Determine AUC of full model
mod.valid <- predict.glm(model.NDT5.E, newdata=Valid, type="response")
   roc_obj <- roc(Valid[,"fire_pres"], mod.valid)
   mod.auc <- auc(roc_obj)
   mod.auc #0.92 --> insane predictive power
   
Anova(model.NDT5.E, type=3)
Anova(model.NDT5.E, type=3, singular.ok = TRUE)

#Remove least sig
model.NDT5.E<-glm(fire_pres ~ climate1 + climate2 + dist_mun + roads_km + dist_mun:roads_km, family = binomial, data = dat1) #did not converge

AIC(model.NDT5.E) #84.99
...
#Despite not being the highest AIC, it does have one of the highest AUCs. With only 150 total data points (and thus 114 for model fitting), the earlier models are at risk of overfitting. A model with fewer variables is better.

#Try without roads given low variaiton
model.NDT5.E<-glm(fire_pres ~ climate1 + climate2 + dist_mun, family = binomial, data = dat1) #did not converge

AIC(model.NDT5.E) #103.1
...
NDT5_ignition_t<-ignition_person_t_NDT5 %>% drop_na(climate1, climate2, dist_mun, roads_km)

#Run Model again with this data; but uses all data here
model.NDT5<-glm(fire_pres ~ climate1 + climate2 + dist_mun + roads_km + dist_mun:roads_km, family = binomial, data = NDT5_ignition_t) 

Anova(model.NDT5, type=3) #glm.fit: fitted probabilities numerically 0 or 1 occurred 
...
NDT5_ignition_t$resids<-resid(model.NDT5)

binnedplot (NDT5_ignition_t$climate1, 
            NDT5_ignition_t$resids, 
            nclass = NULL, 
            xlab = "live stand volume", 
            ylab = "Average residual", 
            main = paste("Binned Residual Plot - glm", i))

binnedplot (NDT5_ignition_t$climate2, 
            NDT5_ignition_t$resids, 
            nclass = NULL, 
            xlab = "", 
            ylab = "Average residual", 
...
#Partial Residuals
#climate1 + climate2 + dist_mun + roads_km + dist_mun:roads_km
visreg(model.NDT5, "climate1")
visreg(model.NDT5, "climate2")

visreg(model.NDT5, "dist_mun", by="roads_km")
visreg(model.NDT5, "roads_km", by="dist_mun")
visreg(model.NDT5, "roads_km", by="dist_mun", ylim=c(-1000,1000))

#Messy; run without roads
model.NDT5<-glm(fire_pres ~ climate1 + climate2 + dist_mun, family = binomial, data = NDT5_ignition_t) 
...
top_mod_table_NDT5_person_t_ALL <- data.frame (matrix (ncol = 9, nrow = 0))
colnames (top_mod_table_NDT5_person_t_ALL ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_climate_2",  "coef_dist_mun", "AUC")
```
...
prop<-0.75
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(NDT5_ignition_t$fire_veg, p = prop,
                                    list = FALSE,
                                    times = 1)
  
   dat1 <- NDT5_ignition_t[ trainIndex,]
   Valid <- NDT5_ignition_t[-trainIndex,]
   
#Model   
model.NDT5<-glm(fire_pres ~ climate1 + climate2 + dist_mun, family = binomial, data = dat1) 
...
top_mod_table_NDT5_person_t <- data.frame (matrix (ncol = 9, nrow = 0))
colnames (top_mod_table_NDT5_person_t ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_climate_2",  "coef_dist_mun", "AUC")

##Add data for NDT5
top_mod_table_NDT5_person_t[1,1]<-"person"
top_mod_table_NDT5_person_t[1,2]<-"NDT5"
top_mod_table_NDT5_person_t[1,3]<-"Y"
top_mod_table_NDT5_person_t[1,4]<-"fire_pres ~ climate1 + climate2 + dist_mun" 
...
top_mod_table_NDT5_person_t[1,8]<- coef(model.NDT5)[4] #dist_mun
top_mod_table_NDT5_person_t[1,9]<- mod.auc
...
top_mod_table_NDT5_ignite_person_means<-top_mod_table_NDT5_person_t_ALL %>% summarise_each(funs( mean( .,na.rm = TRUE)))
top_mod_table_NDT5_ignite_person_means

top_mod_table_NDT5_ignite_person_means[1,1]<-"person"
top_mod_table_NDT5_ignite_person_means[1,2]<-"NDT5"
top_mod_table_NDT5_ignite_person_means[1,3]<-"Treed"
top_mod_table_NDT5_ignite_person_means[1,4]<- "fire_pres ~ climate1 + climate2 + dist_mun" 
top_mod_table_NDT5_ignite_person_means
```
Save table.

```{r}
write.csv(top_mod_table_NDT5_ignite_person_means, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT5_ignite_person_means.csv")
```

Standard deviation.

```{r}
top_mod_table_NDT5_ignite_person_sd<-top_mod_table_NDT5_person_t_ALL %>% summarise_each(funs( sd( .,na.rm = TRUE)))
top_mod_table_NDT5_ignite_person_sd

top_mod_table_NDT5_ignite_person_sd[1,1]<-"person"
top_mod_table_NDT5_ignite_person_sd[1,2]<-"NDT5"
top_mod_table_NDT5_ignite_person_sd[1,3]<-"Treed"
top_mod_table_NDT5_ignite_person_sd[1,4]<-"fire_pres ~ climate1 + climate2 + dist_mun" 
top_mod_table_NDT5_ignite_person_sd
```
...
```{r}
write.csv(top_mod_table_NDT5_ignite_person_sd, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT5_ignite_person_sd.csv")
```
...
table(ignition_person_t_NDT5$vegtype2) #1 Tb, 147 TC. Remove vegtype from model
hist(ignition_person_t_NDT5$roads_km)
ignition_person_t_NDT5$roads_km #Mostly 0s
...
#Run model using dat1
model.NDT5.E<-glm(fire_pres ~ climate1 + climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + elevation + dist_infra + roads_km + dist_infra:roads_km  + elevation*climate1 + elevation*climate2, family = binomial, data = dat1) #

AIC(model.NDT5.E) #80.9
...
model.NDT5.E<-glm(fire_pres ~ climate1 + climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + elevation + dist_infra + roads_km + dist_infra:roads_km + elevation*climate2, family = binomial, data = dat1) #

AIC(model.NDT5.E) #79.4
...
#Remove least significant
model.NDT5.E<-glm(fire_pres ~ climate1 + climate2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + elevation + dist_infra + roads_km + dist_infra:roads_km, family = binomial, data = dat1) #

AIC(model.NDT5.E) #78.4
...
#Remove least significant
model.NDT5.E<-glm(fire_pres ~ climate1 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + elevation + dist_infra + roads_km + dist_infra:roads_km, family = binomial, data = dat1) #

AIC(model.NDT5.E) #76.97
...
```{r}
#Remove NAs to ensure all same data used so we can compare AICs
NDT5_ignition_t<-ignition_person_t_NDT5 %>% drop_na(climate1, climate2, dist_infra, roads_km, proj_height_1, proj_age_1, live_stand_volume_125)

#Run Model again with this data; but uses all data here
model.NDT5<-glm(fire_pres ~ climate1 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + elevation + dist_infra + roads_km + dist_infra:roads_km, family = binomial, data = NDT5_ignition_t) 

AIC(model.NDT5) #96.8

model.NDT5<-glm(fire_pres ~ climate1 + proj_height_1 + proj_age_1 + live_stand_volume_125 + elevation + dist_infra + roads_km + dist_infra:roads_km, family = binomial, data = NDT5_ignition_t) 
...
#Partial Residuals
#climate1 + proj_height_1 + proj_age_1 + live_stand_volume_125 + elevation + dist_infra + roads_km + dist_infra:roads_km
visreg(model.NDT5, "climate1")

visreg(model.NDT5, "proj_height_1")
visreg(model.NDT5, "proj_age_1")
visreg(model.NDT5, "live_stand_volume_125")

visreg(model.NDT5, "elevation")

visreg(model.NDT5, "dist_infra", by="roads_km")
visreg(model.NDT5, "roads_km", by="dist_infra")

#Messy; run without roads
model.NDT5<-glm(fire_pres ~ climate1 + proj_height_1 + proj_age_1 + live_stand_volume_125 + elevation + dist_infra + dist_infra, family = binomial, data = NDT5_ignition_t) 
...
#Create a new blank table and get AUC too
top_mod_table_NDT5_person_t_ALL <- data.frame (matrix (ncol = 12, nrow = 0))
colnames (top_mod_table_NDT5_person_t_ALL ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_proj_height_1", "coef_proj_age_1", "coef_live_stand_volume_125", "coef_elevation", "coef_dist_infra", "AUC")
```
...
#Model   
model.NDT5<-glm(fire_pres ~ climate1 + proj_height_1 + proj_age_1 + live_stand_volume_125 + elevation + dist_infra + dist_infra, family = binomial, data = dat1) 
...
# create model table (only do this once) and add the relevant data
top_mod_table_NDT5_person_t <- data.frame (matrix (ncol = 12, nrow = 0))
colnames (top_mod_table_NDT5_person_t ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_proj_height_1", "coef_proj_age_1", "coef_live_stand_volume_125", "coef_elevation", "coef_dist_infra", "AUC")

##Add data for NDT5
top_mod_table_NDT5_person_t[1,1]<-"person"
top_mod_table_NDT5_person_t[1,2]<-"NDT5"
top_mod_table_NDT5_person_t[1,3]<-"Y"
top_mod_table_NDT5_person_t[1,4]<-"fire_pres ~ climate1 + proj_height_1 + proj_age_1 + live_stand_volume_125 + elevation + dist_infra + dist_infra" 
top_mod_table_NDT5_person_t[1,5]<- coef(model.NDT5)[1] #Intercept
top_mod_table_NDT5_person_t[1,6]<- coef(model.NDT5)[2] #Climate variable 1
top_mod_table_NDT5_person_t[1,7]<- coef(model.NDT5)[3] #Cli
top_mod_table_NDT5_person_t[1,8]<- coef(model.NDT5)[4] #d
top_mod_table_NDT5_person_t[1,9]<- coef(model.NDT5)[5] #d
top_mod_table_NDT5_person_t[1,10]<- coef(model.NDT5)[6] #d
top_mod_table_NDT5_person_t[1,11]<- coef(model.NDT5)[7] #d
top_mod_table_NDT5_person_t[1,12]<- mod.auc
...
top_mod_table_NDT5_ignite_person_means[1,2]<-"NDT5"
top_mod_table_NDT5_ignite_person_means[1,3]<-"Treed"
top_mod_table_NDT5_ignite_person_means[1,4]<- "fire_pres ~ climate1 + proj_height_1 + proj_age_1 + live_stand_volume_125 + elevation + dist_infra + dist_infra" 
top_mod_table_NDT5_ignite_person_means
```
Save table.

```{r}
write.csv(top_mod_table_NDT5_ignite_person_means, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT5_ignite_person_means_Jan.csv")
```
...
top_mod_table_NDT5_ignite_person_sd[1,2]<-"NDT5"
top_mod_table_NDT5_ignite_person_sd[1,3]<-"Treed"
top_mod_table_NDT5_ignite_person_sd[1,4]<-"fire_pres ~ climate1 + proj_height_1 + proj_age_1 + live_stand_volume_125 + elevation + dist_infra + dist_infra" 
top_mod_table_NDT5_ignite_person_sd
```
...
```{r}
write.csv(top_mod_table_NDT5_ignite_person_sd, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT5_ignite_person_sd_Jan.csv")
```
