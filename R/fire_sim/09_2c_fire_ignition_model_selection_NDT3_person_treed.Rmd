---
title: "09_2c_fire_ignition_model_selection_NDT3_person_treed"
author: "Cora Skaien"
date: "08/11/2021"
...
#=================================
#  Script Name: 09_2c_fire_ignition_model_selection_NDT3_person_treed.R
#  Script Version: 1.0
...
AIC_person_NDT3_fire_pres_treed_climate<-table.glm.climate.simple

AIC_person_NDT3_fire_pres_treed_summary_climate<- AIC_person_NDT3_fire_pres_treed_climate %>%
  group_by(model) %>%
...
AIC_person_NDT3_fire_pres_treed_summary_climate2<- AIC_person_NDT3_fire_pres_treed_summary_climate %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT3_fire_pres_treed_summary_climate2)
```
...
AIC_person_NDT3_fire_pres_treed_VRI<-table.glm.VRI.simple

AIC_person_NDT3_fire_pres_treed_summary_VRI<- AIC_person_NDT3_fire_pres_treed_VRI %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic),
            meanAUC=mean(auc.valid),
            sdAUC=sd(auc.valid),
...
AIC_person_NDT3_fire_pres_treed_summary_VRI2<- AIC_person_NDT3_fire_pres_treed_summary_VRI %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT3_fire_pres_treed_summary_VRI2)
```
...
AIC_person_NDT3_fire_pres_treed_topo<-table.glm.topo.simple

AIC_person_NDT3_fire_pres_treed_summary_topo<- AIC_person_NDT3_fire_pres_treed_topo %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic),
            meanAUC=mean(auc.valid),
            sdAUC=sd(auc.valid),
...
AIC_person_NDT3_fire_pres_treed_summary_topo2<- AIC_person_NDT3_fire_pres_treed_summary_topo %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT3_fire_pres_treed_summary_topo2)
```
...
AIC_person_NDT3_fire_pres_treed_infra<-table.glm.infra.simple

AIC_person_NDT3_fire_pres_treed_summary_infra<- AIC_person_NDT3_fire_pres_treed_infra %>%
  group_by(model) %>%
  summarise(meanAIC=mean(aic),
            meanAUC=mean(auc.valid),
            sdAUC=sd(auc.valid),
...
AIC_person_NDT3_fire_pres_treed_summary_infra2<- AIC_person_NDT3_fire_pres_treed_summary_infra %>%
  mutate(deltaAIC=meanAIC-min(meanAIC))

head(AIC_person_NDT3_fire_pres_treed_summary_infra2)
...
```{r}
NDT3_l_models_treed<-rbind(AIC_person_NDT3_fire_pres_treed_summary_climate2, AIC_person_NDT3_fire_pres_treed_summary_VRI2, AIC_person_NDT3_fire_pres_treed_summary_topo2, AIC_person_NDT3_fire_pres_treed_summary_infra2)
NDT3_l_models_treed
NDT3_l_models_treed$NDT<-"NDT3"

write.csv(NDT3_l_models_treed, file="D:\\Fire\\fire_data\\raw_data\\NDT3_person_models_treed_ignition.csv")
```
...
3. slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + aspect_cos:elevation
4. dist_mun + dist_dam + dist_nat + dist_pow + dist_mine + roads_km + dist_dam:roads_km + dist_pow:roads_km + dist_mine:roads_km
...
```{r}
ignition_person_t_NDT3<-subset(dat_person_t, dat_person_t$ntrl_ds=="NDT3")

#Divide data into training and valid
prop<-0.75
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(ignition_person_t_NDT3$fire_veg, p = prop,
                                    list = FALSE,
                                    times = 1)
  
   dat1 <- ignition_person_t_NDT3[ trainIndex,]
   Valid <- ignition_person_t_NDT3[-trainIndex,]

#Run model using dat1
model.NDT3<-glm(fire_pres ~ climate1 + vegtype2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + aspect_cos:elevation + dist_mun + dist_dam + dist_nat + dist_pow + dist_mine + roads_km + dist_dam:roads_km + dist_pow:roads_km + dist_mine:roads_km + elevation:climate1 + bclcs_level_5_2 + bclcs_level_5_2:roads_km, family = binomial, data = dat1) 

AIC(model.NDT3) #9114.4
...
model.NDT3<-glm(fire_pres ~ climate1 + vegtype2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + aspect_cos:elevation + dist_mun + dist_dam + dist_nat + dist_pow + dist_mine + roads_km + dist_dam:roads_km + dist_mine:roads_km + elevation:climate1 + bclcs_level_5_2 + bclcs_level_5_2:roads_km, family = binomial, data = dat1) 

AIC(model.NDT3) #9113.5
...
model.NDT3<-glm(fire_pres ~ climate1 + vegtype2 + proj_height_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + aspect_cos:elevation + dist_mun + dist_dam + dist_nat + dist_pow + dist_mine + roads_km + dist_dam:roads_km + dist_mine:roads_km + elevation:climate1 + bclcs_level_5_2 + bclcs_level_5_2:roads_km, family = binomial, data = dat1) 

AIC(model.NDT3) #9111.7
...
#Remove least significant
model.NDT3<-glm(fire_pres ~ climate1 + vegtype2 + proj_height_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + aspect_cos:elevation + dist_mun + dist_dam + dist_nat + dist_mine + roads_km + dist_dam:roads_km + dist_mine:roads_km + elevation:climate1 + bclcs_level_5_2 + bclcs_level_5_2:roads_km, family = binomial, data = dat1) 

AIC(model.NDT3) #9110.5
...
NDT3_ignition_t<-ignition_person_t_NDT3 %>% drop_na(climate1, vegtype2, live_stand_volume_125, proj_height_1, dist_mun, dist_dam, dist_nat, dist_mine, roads_km, bclcs_level_5_2, slope, elevation, aspect_cos)

#Run Model again with this data; but uses all data here
model.NDT3<-glm(fire_pres ~ climate1 + vegtype2 + proj_height_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + aspect_cos:elevation + dist_mun + dist_dam + dist_nat + dist_mine + roads_km + dist_dam:roads_km + dist_mine:roads_km + elevation:climate1 + bclcs_level_5_2 + bclcs_level_5_2:roads_km, family = binomial, data = NDT3_ignition_t) 
...
NDT3_ignition_t$resids<-resid(model.NDT3)

binnedplot (NDT3_ignition_t$climate1, 
            NDT3_ignition_t$resids, 
            nclass = NULL, 
            xlab = "", 
            ylab = "Average residual", 
            main = paste("Binned Residual Plot - glm", i))
...
#Partial Residuals
#climate1 + vegtype2 + proj_height_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + aspect_cos:elevation + dist_mun + dist_dam + dist_nat + dist_mine + roads_km + dist_dam:roads_km + dist_mine:roads_km + elevation:climate1 + bclcs_level_5_2 + bclcs_level_5_2:roads_km

visreg(model.NDT3, "climate1", by="elevation")

visreg(model.NDT3, "vegtype2")

visreg(model.NDT3, "proj_height_1")
visreg(model.NDT3, "live_stand_volume_125")

visreg(model.NDT3, "slope", by="elevation")
visreg(model.NDT3, "slope", by="aspect_cos")
visreg(model.NDT3, "aspect_cos", by="elevation")
visreg(model.NDT3, "aspect_cos", by="slope")
visreg(model.NDT3, "elevation", by="aspect_cos")
visreg(model.NDT3, "elevation", by="slope")
visreg(model.NDT3, "elevation", by="climate1")

visreg(model.NDT3, "dist_mun")
visreg(model.NDT3, "dist_dam", by="roads_km")
visreg(model.NDT3, "dist_mine", by="roads_km")
visreg(model.NDT3, "dist_nat")

visreg(model.NDT3, "roads_km", by="dist_dam")
visreg(model.NDT3, "roads_km", by="dist_mine")
visreg(model.NDT3, "roads_km", by="bclcs_level_5_2", overlay=TRUE)

visreg(model.NDT3, "bclcs_level_5_2", by="roads_km")
```
...
top_mod_table_NDT3_person_t_ALL <- data.frame (matrix (ncol = 30, nrow = 0))
colnames (top_mod_table_NDT3_person_t_ALL ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_vegtype2TB", "coef_vegtype2TC", "coef_vegtype2TM", "coef_proj_height_1", "coef_live_stand_volume_125", "coef_slope", "coef_aspect_cos", "coef_elevation", "coef_dist_mun", "coef_dist_dam", "coef_dist_nat", "coef_dist_mine", "coef_roads_km", "coef_bclcs_level_5_2OP", "coef_bclcs_level_5_2SP", "coef_slope:aspect_cos", "coef_slope:elevation", "coef_aspect_cos:elevation", "coef_dist_dam:roads_km", "coef_dist_mine:roads_km", "coef_climate1:elevation", "coef_roads_km:bclcs_level_5_2OP", "coef_roads_km:bclcs_level_5_2SP", "AUC")
```
...
prop<-0.75
# Creating training and testing datasets so that I can get a measure of how well the model actually predicts the data e.g. AUG
  trainIndex <- createDataPartition(NDT3_ignition_t$fire_veg, p = prop,
                                    list = FALSE,
                                    times = 1)
  
   dat1 <- NDT3_ignition_t[ trainIndex,]
   Valid <- NDT3_ignition_t[-trainIndex,]
...
top_mod_table_NDT3_person_t <- data.frame (matrix (ncol = 30, nrow = 0))
...
top_mod_table_NDT3_person_t[1,4]<-"fire_pres ~ climate1 + vegtype2 + proj_height_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + aspect_cos:elevation + dist_mun + dist_dam + dist_nat + dist_mine + roads_km + dist_dam:roads_km + dist_mine:roads_km + elevation:climate1 + bclcs_level_5_2 + bclcs_level_5_2:roads_km" 
top_mod_table_NDT3_person_t[1,5]<- coef(model.NDT3)[1] #Intercept
top_mod_table_NDT3_person_t[1,6]<- coef(model.NDT3)[2] #Climate variable 1
top_mod_table_NDT3_person_t[1,7]<- coef(model.NDT3)[3] #Climate variable 2
top_mod_table_NDT3_person_t[1,8]<- coef(model.NDT3)[4] #
top_mod_table_NDT3_person_t[1,9]<- coef(model.NDT3)[5]
top_mod_table_NDT3_person_t[1,10]<- coef(model.NDT3)[6]
top_mod_table_NDT3_person_t[1,11]<- coef(model.NDT3)[7]
top_mod_table_NDT3_person_t[1,12]<- coef(model.NDT3)[8]
top_mod_table_NDT3_person_t[1,13]<- coef(model.NDT3)[9]
top_mod_table_NDT3_person_t[1,14]<- coef(model.NDT3)[10]
top_mod_table_NDT3_person_t[1,15]<- coef(model.NDT3)[11]
top_mod_table_NDT3_person_t[1,16]<- coef(model.NDT3)[12]
top_mod_table_NDT3_person_t[1,17]<- coef(model.NDT3)[13]
top_mod_table_NDT3_person_t[1,18]<- coef(model.NDT3)[14]
top_mod_table_NDT3_person_t[1,19]<- coef(model.NDT3)[15]
top_mod_table_NDT3_person_t[1,20]<- coef(model.NDT3)[16]
top_mod_table_NDT3_person_t[1,21]<- coef(model.NDT3)[17]
top_mod_table_NDT3_person_t[1,22]<- coef(model.NDT3)[18]
top_mod_table_NDT3_person_t[1,23]<- coef(model.NDT3)[19]
top_mod_table_NDT3_person_t[1,24]<- coef(model.NDT3)[20]
top_mod_table_NDT3_person_t[1,25]<- coef(model.NDT3)[21]
top_mod_table_NDT3_person_t[1,26]<- coef(model.NDT3)[22]
top_mod_table_NDT3_person_t[1,27]<- coef(model.NDT3)[23]
top_mod_table_NDT3_person_t[1,28]<- coef(model.NDT3)[24]
top_mod_table_NDT3_person_t[1,29]<- coef(model.NDT3)[25]
top_mod_table_NDT3_person_t[1,30]<- mod.auc
...
```{r}
write.csv(top_mod_table_NDT3_person_t_ALL, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT3_person_t_ALL.csv")
```
...
top_mod_table_NDT3_ignite_person_means<-top_mod_table_NDT3_person_t_ALL %>% summarise_each(funs( mean( .,na.rm = TRUE)))
top_mod_table_NDT3_ignite_person_means

top_mod_table_NDT3_ignite_person_means[1,1]<-"person"
top_mod_table_NDT3_ignite_person_means[1,2]<-"NDT3"
top_mod_table_NDT3_ignite_person_means[1,3]<-"Treed"
top_mod_table_NDT3_ignite_person_means[1,4]<- "fire_pres ~ climate1 + vegtype2 + proj_height_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + aspect_cos:elevation + dist_mun + dist_dam + dist_nat + dist_mine + roads_km + dist_dam:roads_km + dist_mine:roads_km + elevation:climate1 + bclcs_level_5_2 + bclcs_level_5_2:roads_km" 
top_mod_table_NDT3_ignite_person_means
```
Save table.

```{r}
write.csv(top_mod_table_NDT3_ignite_person_means, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT3_ignite_person_means.csv")
```

Standard deviation.

```{r}
top_mod_table_NDT3_ignite_person_sd<-top_mod_table_NDT3_person_t_ALL %>% summarise_each(funs( sd( .,na.rm = TRUE)))
top_mod_table_NDT3_ignite_person_sd

top_mod_table_NDT3_ignite_person_sd[1,1]<-"person"
top_mod_table_NDT3_ignite_person_sd[1,2]<-"NDT3"
top_mod_table_NDT3_ignite_person_sd[1,3]<-"Treed"
top_mod_table_NDT3_ignite_person_sd[1,4]<-"fire_pres ~ climate1 + vegtype2 + proj_height_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + aspect_cos:elevation + dist_mun + dist_dam + dist_nat + dist_mine + roads_km + dist_dam:roads_km + dist_mine:roads_km + elevation:climate1 + bclcs_level_5_2 + bclcs_level_5_2:roads_km" 
top_mod_table_NDT3_ignite_person_sd
```
...
```{r}
write.csv(top_mod_table_NDT3_ignite_person_sd, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT3_ignite_person_sd.csv")
```


#Repeat process with reduced variables
...
#Run model using dat1
model.NDT3<-glm(fire_pres ~ climate1 + vegtype2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + aspect_cos:elevation + dist_infra + roads_km + dist_infra:roads_km +  elevation:climate1, family = binomial, data = dat1) 

AIC(model.NDT3) #10520.5
...
#Remove least significant interaction
model.NDT3<-glm(fire_pres ~ climate1 + vegtype2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + aspect_cos:elevation + dist_infra + roads_km +  elevation:climate1, family = binomial, data = dat1) 

AIC(model.NDT3) #10518.9
...
#Remove least significant 
model.NDT3<-glm(fire_pres ~ climate1 + vegtype2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:aspect_cos + slope:elevation + dist_infra + roads_km +  elevation:climate1, family = binomial, data = dat1) 

AIC(model.NDT3) #10517.5
...
#Remove least significant 
model.NDT3<-glm(fire_pres ~ climate1 + vegtype2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:elevation + dist_infra + roads_km +  elevation:climate1, family = binomial, data = dat1) 

AIC(model.NDT3) #10516.7
...
```{r}
#Remove NAs to ensure all same data used so we can compare AICs
NDT3_ignition_t<-ignition_person_t_NDT3 %>% drop_na(climate1, vegtype2, live_stand_volume_125, proj_height_1, proj_age_1, dist_infra, roads_km, slope, elevation, aspect_cos)

table(NDT3_ignition_t$vegtype2)
NDT3_ignition_t<-subset(NDT3_ignition_t, NDT3_ignition_t$vegtype2!="OP")

#Run Model again with this data; but uses all data here
model.NDT3<-glm(fire_pres ~ climate1 + vegtype2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:elevation + dist_infra + roads_km + elevation:climate1, family = binomial, data = NDT3_ignition_t) 
...
#Partial Residuals
#climate1 + vegtype2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:elevation + dist_infra + roads_km + elevation:climate1

visreg(model.NDT3, "climate1", by="elevation")
visreg(model.NDT3, "vegtype2")

visreg(model.NDT3, "proj_height_1")
visreg(model.NDT3, "proj_age_1")
visreg(model.NDT3, "live_stand_volume_125")

visreg(model.NDT3, "slope", by="elevation")
visreg(model.NDT3, "aspect_cos")
visreg(model.NDT3, "elevation", by="slope")
visreg(model.NDT3, "elevation", by="climate1")

visreg(model.NDT3, "dist_infra")
visreg(model.NDT3, "roads_km")
...
#Create a new blank table and get AUC too
top_mod_table_NDT3_person_t_ALL <- data.frame (matrix (ncol = 20, nrow = 0))
colnames (top_mod_table_NDT3_person_t_ALL ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_vegtype2TB", "coef_vegtype2TC", "coef_vegtype2TM", "coef_proj_height_1", "coef_proj_age_1", "coef_live_stand_volume_125", "coef_slope", "coef_aspect_cos", "coef_elevation", "coef_dist_infra",  "coef_roads_km", "coef_slope:elevation", "coef_climate1:elevation", "AUC")
```
Let's run it 500 times to get good mean values.
...
# create model table (only do this once) and add the relevant data
top_mod_table_NDT3_person_t <- data.frame (matrix (ncol = 20, nrow = 0))
colnames (top_mod_table_NDT3_person_t ) <- c ("CAUSE", "NDT", "TREED", "Model_terms", "intercept", "coef_climate_1", "coef_vegtype2TB", "coef_vegtype2TC", "coef_vegtype2TM", "coef_proj_height_1", "coef_proj_age_1", "coef_live_stand_volume_125", "coef_slope", "coef_aspect_cos", "coef_elevation", "coef_dist_infra",  "coef_roads_km", "coef_slope:elevation", "coef_climate1:elevation", "AUC")
...
top_mod_table_NDT3_person_t[1,2]<-"NDT3"
top_mod_table_NDT3_person_t[1,3]<-"Y"
top_mod_table_NDT3_person_t[1,4]<-"fire_pres ~ climate1 + vegtype2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:elevation + dist_infra + roads_km + elevation:climate1" 
top_mod_table_NDT3_person_t[1,5]<- coef(model.NDT3)[1] #Intercept
top_mod_table_NDT3_person_t[1,6]<- coef(model.NDT3)[2] #Climate variable 1
top_mod_table_NDT3_person_t[1,7]<- coef(model.NDT3)[3] #Climate variable 2
top_mod_table_NDT3_person_t[1,8]<- coef(model.NDT3)[4] #
...
top_mod_table_NDT3_person_t[1,18]<- coef(model.NDT3)[14]
top_mod_table_NDT3_person_t[1,19]<- coef(model.NDT3)[15]
top_mod_table_NDT3_person_t[1,20]<- mod.auc
...
```{r}
write.csv(top_mod_table_NDT3_person_t_ALL, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT3_person_t_ALL_Jan.csv")
```
...
top_mod_table_NDT3_ignite_person_means[1,2]<-"NDT3"
top_mod_table_NDT3_ignite_person_means[1,3]<-"Treed"
top_mod_table_NDT3_ignite_person_means[1,4]<- "fire_pres ~ climate1 + vegtype2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:elevation + dist_infra + roads_km + elevation:climate1" 
top_mod_table_NDT3_ignite_person_means
```
Save table.

```{r}
write.csv(top_mod_table_NDT3_ignite_person_means, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT3_ignite_person_means_Jan.csv")
```
...
top_mod_table_NDT3_ignite_person_sd[1,2]<-"NDT3"
top_mod_table_NDT3_ignite_person_sd[1,3]<-"Treed"
top_mod_table_NDT3_ignite_person_sd[1,4]<-"fire_pres ~ climate1 + vegtype2 + proj_height_1 + proj_age_1 + live_stand_volume_125 + slope + aspect_cos + elevation + slope:elevation + dist_infra + roads_km + elevation:climate1" 
top_mod_table_NDT3_ignite_person_sd
```
...
```{r}
write.csv(top_mod_table_NDT3_ignite_person_sd, file="D:\\Fire\\fire_data\\raw_data\\top_mod_table_NDT3_ignite_person_sd_Jan.csv")
```
