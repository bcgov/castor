...
### Summarize fire and cut into 5 years intervals
...
data.moose.density <- data.table (data.moose.density)
data.lme <- data.moose.density [, .(adj_density_est_km2, wmu, survey_year, wmu_area_ha,
...
```{r areas to densities, eval = T, warning = F}

data.lme [, prop_wetland := wetland_area_ha / wmu_area_ha]
data.lme [, prop_lake := lake_area_ha / wmu_area_ha]
data.lme [, prop_river := river_area_ha / wmu_area_ha]
data.lme [, prop_shrub := shrub_area_ha / wmu_area_ha]
data.lme [, prop_cut_1_5 := Cut_1_5_area_ha / wmu_area_ha]
data.lme [, prop_cut_6_10 := Cut_6_10_area_ha / wmu_area_ha]
data.lme [, prop_cut_11_20 := Cut_11_20_area_ha / wmu_area_ha]
data.lme [, prop_cut_21_30 := Cut_21_30_area_ha / wmu_area_ha]                        
data.lme [, prop_fire_1_5 := Fire_1_5_area_ha / wmu_area_ha]                        
data.lme [, prop_fire_6_10 := Fire_6_10_area_ha / wmu_area_ha]                        
data.lme [, prop_fire_11_15 := Fire_11_15_area_ha / wmu_area_ha]                        
data.lme [, prop_fire_16_20 := Fire_16_20_area_ha / wmu_area_ha]   
data.lme [, prop_fire_21_25 := Fire_21_25_area_ha / wmu_area_ha]                        
data.lme [, prop_fire_26_30 := Fire_26_30_area_ha / wmu_area_ha]                        
data.lme [, prop_fir := Fir_area_ha / wmu_area_ha]                        
data.lme [, prop_populus := Populus_area_ha / wmu_area_ha]                        
data.lme [, prop_birch := Birch_area_ha / wmu_area_ha]                        
data.lme [, prop_dougfir := DougFir_area_ha / wmu_area_ha]                        
data.lme [, prop_larch := Larch_area_ha / wmu_area_ha]                        
data.lme [, prop_pine := Pine_area_ha / wmu_area_ha]                        
data.lme [, prop_spruce := Spruce_area_ha / wmu_area_ha]                        
data.lme [, prop_cedar := Cedar_Hemlock_area_ha / wmu_area_ha]                        
data.lme [, dens_road_loose := road_loose_km / (wmu_area_ha / 100)]                        
data.lme [, dens_road_paved := road_paved_km / (wmu_area_ha / 100)]                        
data.lme [, dens_road_rough := road_rough_km / (wmu_area_ha / 100)]                        
data.lme [, dens_road_overgrown := road_overgrown_km / (wmu_area_ha / 100)]                        
...
dotchart (data.lme$prop_wetland)
dotchart (data.lme$prop_lake )
dotchart (data.lme$prop_river ) # wmu 5-02D has double river area than other units
...
dotchart (data.lme$prop_shrub ) # 5-13C had significantly more shrubs
dotchart (data.lme$site_index_mean)
dotchart (data.lme$site_index_sd)
dotchart (data.lme$age_mean)
dotchart (data.lme$age_sd)
dotchart (data.lme$prop_cut_1_5)
dotchart (data.lme$prop_cut_6_10 )
dotchart (data.lme$prop_cut_11_20 )
dotchart (data.lme$prop_cut_21_30 )
dotchart (data.lme$prop_fire_1_5)
dotchart (data.lme$prop_fire_6_10 )
dotchart (data.lme$prop_fire_11_15)
dotchart (data.lme$prop_fire_16_20 )
dotchart (data.lme$prop_fire_21_25 )
dotchart (data.lme$prop_fire_26_30 )
dotchart (data.lme$prop_fir)
dotchart (data.lme$prop_populus )
dotchart (data.lme$prop_birch) # a few zero's.....
dotchart (data.lme$prop_dougfir)
dotchart (data.lme$prop_larch) # problematic; lots of zeros
dotchart (data.lme$prop_pine)
dotchart (data.lme$prop_spruce)
dotchart (data.lme$prop_cedar) # a few zero's.....
dotchart (data.lme$dens_road_loose)                          
dotchart (data.lme$dens_road_paved)                          
dotchart (data.lme$dens_road_overgrown)    # lots of zero's                      
dotchart (data.lme$dens_road_rough )                          

# dropping the landcover covairates - most are problematic
# dropping bec covariates; soem problematic, correalted with cliamte anyway

# combining wet area types (river, wetland, lake)
data.lme [, prop_wet := prop_wetland + prop_lake + prop_river]

# combining forest cutblock ages (cut 1 to 10, cut 11-30, fire 1 to 15, fire 16 to 30)
data.lme [, prop_cut_1_10 := prop_cut_1_5 + prop_cut_6_10]
data.lme [, prop_cut_11_30 := prop_cut_11_20 + prop_cut_21_30]
data.lme [, prop_fire_1_15 := prop_fire_1_5 + prop_fire_6_10 + prop_fire_11_15]
data.lme [, prop_fire_16_30 := prop_fire_16_20 + prop_fire_21_25 + prop_fire_26_30]

# combining road types
data.lme [, dens_road_unpaved := dens_road_loose + dens_road_rough]

# start with:
# mat_mean, pas_ann_mean, prop_wet, prop_shrub, site_index_mean, site_index_sd, age_mean, age_sd, 
# prop_cut_1_10, prop_cut_11_30, prop_fire_1_15, prop_fire_16_30, prop_populus, prop_pine,
# prop_spruce, prop_dougfir, dens_road_paved, dens_road_unpaved
...
```{r, simple GLM model with VIF, eval = T, warning = F}
glm.moose.density <- glm (adj_density_est_km2 ~ mat_mean + pas_ann_mean + prop_wet + prop_shrub +
                                                site_index_mean + site_index_sd + age_mean +
                                                age_sd + prop_cut_1_10 + prop_cut_11_30 +
                                                prop_fire_1_15 + prop_fire_16_30 + prop_populus +
                                                prop_pine + prop_spruce + prop_dougfir +
                                                dens_road_paved + dens_road_unpaved, 
                            data = data.lme,
...
glm.moose.density <- glm (adj_density_est_km2 ~ pas_ann_mean + prop_wet + prop_shrub +
                                                # mat_mean + 
                                                site_index_mean + site_index_sd + age_mean +
                                                age_sd + prop_cut_1_10 + prop_cut_11_30 +
                                                prop_fire_1_15 + prop_fire_16_30 + prop_populus +
...
glm.moose.density <- glm (adj_density_est_km2 ~ pas_ann_mean + prop_wet + prop_shrub +
                                                # mat_mean + 
                                                site_index_mean + site_index_sd + 
                                                # age_mean + age_sd + 
                                                prop_cut_1_10 + prop_cut_11_30 +
                                                prop_fire_1_15 + prop_fire_16_30 + prop_populus +
                                                prop_pine + prop_spruce + prop_dougfir +
                                                dens_road_paved + dens_road_unpaved, 
                            data = data.lme,
...
```{r, random effects model; check for heterogeneity, normality and independence fo residuals, eval = T,  warning = F }
require (lme4)
require (ggplot2)
require (lattice)

# Fit the LME model
lme.moose.density <- glmer (adj_density_est_km2 ~ pas_ann_mean + prop_wet + prop_shrub +
                                                  prop_cut_1_10 + prop_cut_11_30 +
                                                  prop_fire_1_15 + prop_fire_16_30 + prop_populus +
                                                  prop_pine + prop_spruce + prop_dougfir +
                                                  dens_road_paved + dens_road_unpaved +
...
# 5-02B in 1994 is an outlier; removed this data point from analysis
...
qqnorm (residuals (lme.moose.density)) # some deviation at high values
qqline (residuals (lme.moose.density)) 




# removed 5-02-D 1999 because seems to be a problematic outlier
data.lme.final <- data.lme.final [!18]
...
ggplot (data.lme.final, aes(lme.residuals)) + # normality of residuals
        geom_histogram (binwidth = 0.05) # there is some skewness here, but not terrible
plot (density (data.lme.final$lme.residuals))
shapiro.test(data.lme.final$lme.residuals) # p-value = 0.2132, so is normal statistically

qqnorm (residuals (lme.moose.density)) 
qqline (residuals (lme.moose.density)) 
...
ggplot (data.lme.final, aes(x = prop_wet, y = lme.residuals)) +
  geom_point () +
  geom_point (aes (y = lme.residuals), shape = 1) +
  geom_text (aes (label = paste (wmu, survey_year, sep = "_")), hjust = 0, vjust = 0, 
                   check_overlap = T,
...
ggplot (data.lme.final, aes(x = prop_shrub, y = lme.residuals)) +
  geom_point () +
  geom_point (aes (y = lme.residuals), shape = 1) +
  geom_text (aes (label = paste (wmu, survey_year, sep = "_")), hjust = 0, vjust = 0, 
                   check_overlap = T,
...
ggplot (data.lme.final, aes(x = prop_cut_1_10, y = lme.residuals)) +
  geom_point () +
  geom_point (aes (y = lme.residuals), shape = 1) +
  geom_text (aes (label = paste (wmu, survey_year, sep = "_")), hjust = 0, vjust = 0, 
                   check_overlap = T,
...
ggplot (data.lme.final, aes(x = prop_cut_11_30, y = lme.residuals)) +
  geom_point () +
  geom_point (aes (y = lme.residuals), shape = 1) +
  geom_text (aes (label = paste (wmu, survey_year, sep = "_")), hjust = 0, vjust = 0, 
                   check_overlap = T,
...
ggplot (data.lme.final, aes(x = prop_fire_1_15, y = lme.residuals)) +
  geom_point () +
  geom_point (aes (y = lme.residuals), shape = 1) +
  geom_text (aes (label = paste (wmu, survey_year, sep = "_")), hjust = 0, vjust = 0, 
                   check_overlap = T,
...
ggplot (data.lme.final, aes(x = prop_fire_16_30, y = lme.residuals)) +
  geom_point () +
  geom_point (aes (y = lme.residuals), shape = 1) +
  geom_text (aes (label = paste (wmu, survey_year, sep = "_")), hjust = 0, vjust = 0, 
                   check_overlap = T,
...
ggplot (data.lme.final, aes(x = prop_populus, y = lme.residuals)) +
  geom_point () +
  geom_point (aes (y = lme.residuals), shape = 1) +
  geom_text (aes (label = paste (wmu, survey_year, sep = "_")), hjust = 0, vjust = 0, 
                   check_overlap = T,
...
ggplot (data.lme.final, aes(x = prop_pine, y = lme.residuals)) +
  geom_point () +
  geom_point (aes (y = lme.residuals), shape = 1) +
  geom_text (aes (label = paste (wmu, survey_year, sep = "_")), hjust = 0, vjust = 0, 
                   check_overlap = T,
...
ggplot (data.lme.final, aes(x = prop_spruce, y = lme.residuals)) +
  geom_point () +
  geom_point (aes (y = lme.residuals), shape = 1) +
  geom_text (aes (label = paste (wmu, survey_year, sep = "_")), hjust = 0, vjust = 0, 
                   check_overlap = T,
...
ggplot (data.lme.final, aes(x = prop_dougfir, y = lme.residuals)) +
  geom_point () +
  geom_point (aes (y = lme.residuals), shape = 1) +
  geom_text (aes (label = paste (wmu, survey_year, sep = "_")), hjust = 0, vjust = 0, 
                   check_overlap = T,
...
ggplot (data.lme.final, aes(x = dens_road_paved, y = lme.residuals)) +
  geom_point () +
  geom_point (aes (y = lme.residuals), shape = 1) +
  geom_text (aes (label = paste (wmu, survey_year, sep = "_")), hjust = 0, vjust = 0, 
                   check_overlap = T,
...
ggplot (data.lme.final, aes(x = dens_road_unpaved, y = lme.residuals)) +
  geom_point () +
  geom_point (aes (y = lme.residuals), shape = 1) +
  geom_text (aes (label = paste (wmu, survey_year, sep = "_")), hjust = 0, vjust = 0, 
                   check_overlap = T,
...
The top-ranked model had an AIC weight (AIC~w~) of 0.526, and included covariates for proportion of area of *Populus* leading tree species, proportion of area of rivers, proportion of area burnt 16 to 30 years ago, proportion of area cut 1 to 10 and 11 to 30 years ago, and desnity of paved and unpaved roads, and had an *R^2^* value of 0.830 (Table xx). The second ranked-model (AIC~w~ = 0.267; *R^2^* = 0.830) included these same covariates and  proportion of area burnt 1 to 15 years ago. The third ranked-model (AIC~w~ = 0.207; *R^2^* = 0.828) included these same covariates as the top model and  proportion of area that is Douglas fir leading. 


```{r, model selection, eval = T, warning = F}
require (lme4)
require (MuMIn)
require (cAIC4)
require (kableExtra)

# NOTE: # 5-02B in 1994 and 5-02-D 1999 seemed to be a problematic outliers; removed these data points from analysis, ABOVE

# removed 5-12-B 2002 because model had trouble predicting and produced non-normal residuals
data.lme.final <- data.lme.final [!30]
data.lme.save <- data.lme.final [, c (1:4, 6, 63, 65:66, 78, 80, 82:83, 86, 90:94)]
write.csv (data.lme.save, "C:\\Work\\caribou\\clus_github\\R\\moose\\data_moose_density_chilcotin.csv")

## GLOBAL MODEL ##
lme.moose.density.global <- glmer (adj_density_est_km2 ~ pas_ann_mean + 
                                                          # prop_wet + 
                                                          prop_wetland + 
                                                          prop_river +
                                                          prop_shrub +
                                                          prop_cut_1_10 + prop_cut_11_30 +
                                                          prop_fire_1_15 + prop_fire_16_30 + 
                                                          prop_populus +
                                                          prop_pine + prop_spruce + prop_dougfir +
                                                          dens_road_paved + dens_road_unpaved +
                                                          (1 | wmu), 
                                  ...
                                  REML = FALSE,
                                  na.action = "na.fail")
summary (lme.moose.density.global)
## Use MuMin 'dredge' function to fit combos of global model ##
table.dredge <- dredge (lme.moose.density.global,
                        beta = "none",
                        rank = "AICc",
                        subset = ("dens_road_paved" & "dens_road_unpaved")) # subset to make sure paved and unpaved road types are considered in models together (i.e., not considrred independently)
table.dredge.aic.2 <- subset (table.dredge, delta < 2) # subset of top models

table.aic <- data.frame ()
table.aic [1, 1] <- "Density Paved Road, Density Unpaved Roads, Proportion Cutblocks 1 to 10 years old, Proportion Cutblocks 11 to 30 years old, Proportion Burnt 16 to 30 years old, Proportion Populus Spp., Proportion River Area"
table.aic [2, 1] <- "Density Paved Road, Density Unpaved Roads, Proportion Cutblocks 1 to 10 years old, Proportion Cutblocks 11 to 30 years old, Proportion Burnt 1 to 15 years old, Proportion Burnt 16 to 30 years old, Proportion Populus Spp., Proportion River Area"
table.aic [3, 1] <- "Density Paved Road, Density Unpaved Roads, Proportion Cutblocks 1 to 10 years old, Proportion Cutblocks 11 to 30 years old, Proportion Burnt 16 to 30 years old, Proportion Populus Spp., Proportion River Area, Proportion Douglas Fir"
colnames (table.aic) <- "Model Covariates"

table.aic.2 <- dplyr::select (table.dredge.aic.2,
                             "AICc", "delta", "weight")
colnames (table.aic.2) <- c ("AICc", "delta AIC", "AIC weight")

table.aic <- dplyr::bind_cols (table.aic, table.aic.2)

table.aic [1, 5] <- r.squaredGLMM (glmer (adj_density_est_km2 ~ dens_road_paved + 
                                                                dens_road_unpaved +
                                                                prop_cut_1_10 + prop_cut_11_30 + 
                                                                prop_fire_16_30 +
                                                                prop_populus +
...
table.aic [2, 5] <- r.squaredGLMM (glmer (adj_density_est_km2 ~ dens_road_paved + 
                                                                dens_road_unpaved +
                                                                prop_cut_1_10 + prop_cut_11_30 + 
                                                                prop_fire_1_15 + prop_fire_16_30 +
                                                                prop_populus +
...
table.aic [3, 5] <- r.squaredGLMM (glmer (adj_density_est_km2 ~ dens_road_paved + 
                                                                dens_road_unpaved +
                                                                prop_cut_1_10 + prop_cut_11_30 + 
                                                                prop_fire_16_30 +
                                                                prop_populus +
                                            ...
                                            REML = FALSE,
                                            verbose = T)) [1,1]
table.aic [, c (2:5)] <- round (table.aic [, c (2:5)], digits = 3)
colnames (table.aic) [5] <- "R-squared"

kable (table.aic,
       caption = "Table xx. Moose density model selection using Akaike Information Criteria (AIC).") %>%
  kable_styling (position = "left",
...
### averaged top model coefficients


```{r, model coefficient average}
model.avg.lme.moose.density <- model.avg (table.dredge, subset = delta < 2)
summary (model.avg.lme.moose.density)
...
kable (table.avg.coeff.est,
       caption = "Table xx. Averaged moose density model coefficient values for the Chilcotin region of British Columbia.") %>%
  kable_styling (position = "left",
                 bootstrap_options = c("striped", "hover", "condensed"),
                 fixed_thead = T,
                 full_width = F)
...
data.lme.final$lme.predicted <- predict (model.avg (table.dredge, subset = delta < 2, evaluate = F))
data.lme.final$lme.residuals <- data.lme.final$adj_density_est_km2 - data.lme.final$lme.predicted
...
plot (density (data.lme.final$lme.residuals)) # there is some skewness here
shapiro.test(data.lme.final$lme.residuals) # p-value = 0.44, so is not normal statistically

qqnorm (data.lme.final$lme.residuals) # some deviation in upper quantile; 5-04 1994 could be the problem
qqline (data.lme.final$lme.residuals) 

# WMU's 5-12-B 2002, 5-04 1994 and 5-02-D 2014 appear to be the problems; these are under-predicted by the model
# started by removing 5-12-B 2002; seems to be the worst offender
...
ggplot (data.lme.final, aes(x = prop_river, y = lme.residuals)) +
  geom_point () +
  geom_point (aes (y = lme.residuals), shape = 1) +
  geom_text (aes (label = paste (wmu, survey_year, sep = "_")), hjust = 0, vjust = 0, 
                   check_overlap = T,
