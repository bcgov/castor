...
bark.beetles.m.1999 <- st_transform (bark.beetles.m.1999, crs = 3005)
wmu.bark.beetles.m.1999 <- sf::st_intersection (bark.beetles.m.1999, poly.wmu ["WILDLIFE_MGMT_UNIT_ID"])
  
table.wmu.bark.beetles.m.1999 <- as.data.table (wmu.bark.beetles.m.1999)  
  
table.wmu.bark.beetles.m.1999.sum <- table.wmu.bark.beetles.m.1999 %>% 
                                          group_by (WILDLIFE_MGMT_UNIT_ID) %>%
                                          summarise (beetle_1999_m_area_ha = sum (POLY_AREA))
names (table.wmu.bark.beetles.m.1999.sum) [names (table.wmu.bark.beetles.m.1999.sum) == "WILDLIFE_MGMT_UNIT_ID"] <- "wmu"

## these next steps left-join the fire data to wmu data ##
setkey (as.data.table (table.wmu.bark.beetles.m.1999.sum), wmu)
setkey (wmu.habitat, wmu)
wmu.habitat <- merge (wmu.habitat, table.wmu.bark.beetles.m.1999.sum, all.x = TRUE)
wmu.habitat [is.na (wmu.habitat)] <- 0
rm (bark.beetles.m.1999, wmu.bark.beetles.m.1999, table.wmu.bark.beetles.m.1999, table.wmu.bark.beetles.m.1999.sum, bark.beetles.m.1999)
gc ()
