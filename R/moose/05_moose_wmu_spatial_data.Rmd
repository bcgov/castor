...
poly.wmu.gmz7Pe <- poly.wmu [poly.wmu$GAME_MANAGEMENT_ZONE_ID == "7Pe" ,]
poly.wmu.gmz7Pe <- poly.wmu.gmz7Pe ["WILDLIFE_MGMT_UNIT_ID"] # drop the data fields we don't need
poly.vri.clean.gmz7Pe <- sf::st_intersection (poly.vri.clean, poly.wmu.gmz7Pe) # intersect the VRI with WMU data
poly.vri.clean.gmz7Pe <- poly.vri.clean.gmz7Pe %>% 
                            mutate (area_m2 = st_area (.) %>% # calculate area of each polygon in m2
                            as.numeric ())
tab.poly.vri.gmz7Pe <- as.data.table (poly.vri.clean.gmz7Pe) 
tab.poly.vri.gmz7Pe <- tab.poly.vri.gmz7Pe [, "SHAPE" := NULL] 
names (tab.poly.vri.gmz7Pe) [names (tab.poly.vri.gmz7Pe) == "WILDLIFE_MGMT_UNIT_ID"] <- "wmu"
tab.poly.vri.gmz7Pe$area_ha <- tab.poly.vri.gmz7Pe$area_m2 / 10000 # convert m^2 to ha
tab.poly.vri <- rbind (tab.poly.vri, tab.poly.vri.gmz7Pe)
rm (poly.wmu.gmz7Pe, poly.vri.clean.gmz7Pe, tab.poly.vri.gmz7Pe)
gc ()
