...
##-----------------------
## look at actual fire number versus the average for Itcha ILgachuz

fire_area<-fire.bound %>%
  filter(HERD_NAME=="Telkwa",habitat=="Matrix")
fire_area$type<-"orig"


fire_ave_area<-df3 %>%
  select(mean_area_m2,Herd_names,habitat,Year_end) %>%
  filter(Herd_names=="Telkwa",habitat=="Matrix")
...
Itcha<-rbind(fire_area,fire_ave_area2)

ggplot(Itcha, aes(x=year, y=area_m2,colour=type))+
  geom_point(size=3,alpha=0.5) +
  geom_line(alpha=0.2,size=1) +
  ggtitle("Telkwa-matrix")

plot2<-ggplot(fire_ave_area, aes(x=Year_end, y=mean_area_m2))+
  geom_point(size=3,alpha=0.5) +
  geom_line(alpha=0.2,size=1)
