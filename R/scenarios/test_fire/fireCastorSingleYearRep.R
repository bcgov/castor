...
doEvent.fireCastor = function(sim, eventTime, eventType, debug = FALSE){
  switch(
    ...
    init = {
      ...
      sim <- scheduleEvent(sim, time(sim), "fireCastor", "calculateProbEscapeSpread", 12)
      sim <- scheduleEvent(sim, time(sim), "fireCastor", "burnRepitions", 13)
...
calculateProbEscapeSpread = {
  ...
  sim <- scheduleEvent(sim, time(sim) + P(sim, "calculateInterval", "fireCastor") , "fireCastor", "calculateProbEscapeSpread", 12)
},

burnRepitions = {
  sim<-burnReps(sim)
  sim <- scheduleEvent(sim, time(sim) + P(sim, "calculateInterval", "fireCastor"), "fireCastor", "burnRepitions", 15)
},
...
Init <- function(sim) {
  ...
  sim$perFireReport<-data.table(timeperiod= integer(), rep = integer(), ignition_location=integer(), pixelid10km = integer(), areaburned_estimated = numeric())
  
  sim$firedisturbanceTable<-data.table(scenario = scenario$name, reps = as.numeric(0), pixelid = sim$pts$pixelid, numberTimesBurned = as.numeric(0))
  
  sim$fireReport<-data.table(timeperiod= integer(), rep = integer(), numberstarts = integer(), totalareaburned = numeric(), thlbburned = numeric())
...
#  prov_cmi<-data.table(getTableQuery(paste0("SELECT * FROM ",P(sim, "prov_cmi_table","fireCastor"), " WHERE gcm= ",P(sim, "gcm", "fireCastor"), "AND ssp=", P(sim, "ssp", "fireCastor"), "and period=", time(sim)*P(sim, "calculateInterval", "fireCastor") + P(sim, "simStartYear", "fireCastor") , " AND run =", P(sim, "ssp", "fireCastor"), ";")))
 ...
 downdat<-downdat[ ,est:= exp(-17.0 -0.0576*cmi_min-0.124*(cmi-cmi3yr/3)-0.363*avgCMIProv  -0.979*frt5 -0.841*frt7 -1.55*frt9  -1.55*frt10  -1.03*frt11  -1.09*frt12 -1.34*frt13  -0.876*frt14  -2.36*frt15+ 0.495*log(con + 1) + 0.0606 *log(young + 1) -0.0256 *log(dec + 1) +est_rf  + log(flammable) )]
 
 downdat[, mu1:= 2.158738 - 0.001108 * PPT_sm - 0.011496 * cmi + -0.719612 * est  -0.020594 * log(con + 1)][, sigma1:=  1.087][ , mu2:= 2.645616 -0.001222*PPT_sm + 0.049921 * cmi +1.918825 * est -0.209590*log(con + 1) ][, sigma2:= 0.27]
  
 downdat[, pi2:=1/(1+exp(-1*(-0.1759469+ -1.374135*frt5-0.6081503*frt7-2.698864*frt9 -1.824072*frt10 -3.028758*frt11  -1.234629*frt12-1.540873*frt13-0.842797*frt14  -1.334035*frt15+ 0.6835479*avgCMIProv+0.1055167*TEMP_MAX )))][,pi1:=1-pi2]
...
print(fire.size)
    ...
    tempperFireReport<-data.table(timeperiod= integer(),rep = integer(),ignition_location=integer(), pixelid10km = integer(), areaburned_estimated = numeric())
    ...
    sim$firedisturbanceTable[pixelid %in% out$pixels, numberTimesBurned := numberTimesBurned + 1]
    sim$firedisturbanceTable[, reps := reps + 1]
    ...
    tempperFireReport<-data.table(timeperiod = time(sim), rep = rep,ignition_location=fire.size$ignit_location, pixelid10km = fire.size$pixelid10km, areaburned_estimated = fire.size$fire.size)
    ...
    tempperFireReport<-data.table(timeperiod = time(sim), rep = rep, ignition_location="NA", pixelid10km = "NA", areaburned_estimated = totalareaburned)
    
    }
    
    sim$perFireReport<-rbindlist(list(sim$perFireReport,tempperFireReport ))
    
    sim$fireReport<-rbindlist(list(sim$fireReport, data.table(timeperiod= time(sim), rep=rep, numberstarts = numberstarts, totalareaburned = totalareaburned, thlbburned = thlbburned)))
...
savefirerast<-function(sim){
  ...
  message("saving fire related rasters")
