...
db <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/scenarios/comparison_stsm/stsm_compare_noroads_noblocks_castordb.sqlite"))
...
yield.trans <- yields [yieldid > 0]
max.yield.trans <- yield.trans [yield.trans [, .I [which.max(mai)], by = yieldid]$V1]
yield.counts.trans <- pixels  [, .(count = .N, count_thlb = sum (thlb, na.rm = T)), by = yieldid_trans]
yield.counts.trans <- merge  (yield.counts.trans, max.yield.trans, by.x = "yieldid_trans", by.y = "yieldid")
yield.counts.trans [, total_vol := (mai*count_thlb)]
lrsy.trans <- sum (yield.counts.trans$total_vol) # 235,930 m3
...
scenario = data.table (name = "oldest_decadal_noroads_noblocks_constraint",
                       description = "Priority queue = oldest first. Adjacency constraint = None. No roads or blocks. Maximum even live harvest flow: 108,500 m3/year. Minimum harvest age = 80 and minimum harvest volume = 150")
