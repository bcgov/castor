...
```{r, harvest fig}
schema<-"whitesail_morice_patchworks"
conn <- dbConnect (RPostgreSQL::PostgreSQL(),
                  ...
                  dbname = keyring::key_get("vmdbname", keyring="postgreSQL"),
				          port = "5432")
data.harvest <- data.table(dbGetQuery (conn, glue::glue("SELECT * FROM {schema}.harvest where scenario in ('bau_rr21', 'gfa_mod_rr21', 'hbea_mod_rr21', 'no_harv_wha_v1_rr21', 'smz_wetlands_mod_rr21', 'steventon_modelled_mod_rr21', 'steventon_observed_mod_rr21', 'conditional_rr21', 'alternative_smz_rr21')") ))
data.volumebyarea<-data.table(dbGetQuery (conn, glue::glue("SELECT * FROM {schema}.volumebyarea where scenario in ('bau_rr21', 'gfa_mod_rr21', 'hbea_mod_rr21', 'no_harv_wha_v1_rr21', 'smz_wetlands_mod_rr21', 'steventon_modelled_mod_rr21', 'steventon_observed_mod_rr21', 'conditional_rr21', 'alternative_smz_rr21')") ))
data.disturb <- data.table(dbGetQuery (conn, glue::glue("SELECT scenario, compartment, timeperiod, critical_hab, total_area, cut40, road50, c40r50, c40r500 FROM {schema}.disturbance where scenario in ('bau_rr21', 'gfa_mod_rr21', 'hbea_mod_rr21', 'no_harv_wha_v1_rr21', 'smz_wetlands_mod_rr21', 'steventon_modelled_mod_rr21', 'steventon_observed_mod_rr21', 'conditional_rr21', 'alternative_smz_rr21')")) )
data.seral <- data.table(dbGetQuery (conn, glue::glue("SELECT * FROM {schema}.survival where scenario in ('bau_rr21', 'gfa_mod_rr21', 'hbea_mod_rr21', 'no_harv_wha_v1_rr21', 'smz_wetlands_mod_rr21', 'steventon_modelled_mod_rr21', 'steventon_observed_mod_rr21', 'conditional_rr21', 'alternative_smz_rr21')")))
dbDisconnect (conn)
...
plot.seral<- ggplot (data.seral[timeperiod <= 50, ], 
  ...
  plot.survival<- ggplot (data.seral[timeperiod <= 50, ], 
                               aes (x = timeperiod, y = survival_rate, 
                                    color = scenario, linetype = scenario)) +
                            geom_line (position = position_dodge (width = 1), size = 1) +
  facet_wrap (. ~herd_bounds, ncol = 2, scales="free_y") + geom_hline(yintercept = 0.85) +
