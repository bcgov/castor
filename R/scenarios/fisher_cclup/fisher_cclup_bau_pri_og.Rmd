...
#remove the es and caribou to get early seral risk
```{r}
clusdb <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/scenarios/fisher_cclup/rcb_castordb.sqlite"))
#remove scenario 2a
dbExecute(clusdb, "UPDATE zoneconstraints SET type = 'ge' WHERE reference_zone = 'rast.zone_wha_2021' AND zoneid in (3781,3780,3779,263);") 
#dbExecute(clusdb, "UPDATE zoneconstraints SET type = '' WHERE reference_zone = 'rast.zone_chilcotin_scenario2';") don't need this since its not in the list

dbExecute(clusdb, "UPDATE zoneconstraints SET type = '' WHERE type = 'le' and reference_zone = 'rast.zone_cond_beo_pg_cclup';") 
dbDisconnect(clusdb)
```
...
scenario = data.table(name="esr_bau_pri_og_adj", description = "business as usual: existing legal constraints and no harvesting in priority old growth deferral areas (TAP)")
