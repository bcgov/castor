...
---
title: "Database creation for williams lake Analyses Version 2.0"
...
#Caribou scenario 2a
```{r, Scenario_2a, eval=FALSE}
require (DBI)
source (paste0(here::here(), "/R/functions/R_Postgres.R"))

#STEP 1: Connect to the clusdb.sqlite database for the AOI
clusdb <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/scenarios/fisher_cclup/rcb_castordb.sqlite"))
...
VALUES 
(5,'rast.zone_chilcotin_scenario2','zone11',0,'age',141,'ge',25,NULL,NULL,776877,0,250),
(5,'rast.zone_chilcotin_scenario2','zone11',0,'age',121,'ge',30,NULL,NULL,776877,0,250),
(5,'rast.zone_chilcotin_scenario2','zone11',0,'age',101,'ge',37,NULL,NULL,776877,0,250),
(5,'rast.zone_chilcotin_scenario2','zone11',0,'age',81,'ge',45,NULL,NULL,776877,0,250),
(6,'rast.zone_chilcotin_scenario2','zone11',0,'age',141,'ge',25,NULL,NULL,891930,0,250),
(6,'rast.zone_chilcotin_scenario2','zone11',0,'age',121,'ge',30,NULL,NULL,891930,0,250),
(6,'rast.zone_chilcotin_scenario2','zone11',0,'age',101,'ge',37,NULL,NULL,891930,0,250),
(6,'rast.zone_chilcotin_scenario2','zone11',0,'age',81,'ge',45,NULL,NULL,891930,0,250);")
}

dbDisconnect(clusdb)

```
#Legalize es in intermediate and high
...
#Assign es to low using intermediate
```{r, change_interm, eval = FALSE}
clusdb <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/scenarios/fisher_cclup/rcb_castordb.sqlite"))
change<-readRDS(paste0(here::here(), "/R/scenarios/fisher_cclup/lowToInt.rds"))
areas<-unique(dbGetQuery(clusdb, paste0("Select zoneid, t_area from zoneconstraints where reference_zone = 'rast.zone_cond_beo_pg_cclup' and zoneid in (",paste(unique(change$id_beo), sep = " ", collapse = ","),")")))

params<-merge(change, areas, by.x = "id_beo", by.y = "zoneid", all.y=T)
params[, reference_zone:= 'rast.zone_cond_beo_pg_cclup'][, zone_column:= 'zone8'][,variable :='age'][,threshold :=40][,type:='le'][,start:=0][,stop:=250]

dbBegin(clusdb)
    rs<-dbSendQuery(clusdb, "INSERT into zoneconstraints (zoneid, reference_zone, zone_column, ndt, variable, threshold, type, percentage, t_area, start, stop) values (:id_beo, :reference_zone, :zone_column, :ndt, :variable, :threshold,:type, :target, :t_area, :start, :stop) ;", params)
dbClearResult(rs)
dbCommit(clusdb)
dbDisconnect(clusdb)
```

#Upgrade low to intermediate
```{r, upgrade_low, eval = FALSE}
clusdb <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/scenarios/fisher_cclup/rcb_castordb.sqlite"))
mat<-readRDS(paste0(here::here(), "/R/scenarios/fisher_cclup/lowToInt_Mature.rds"))
mat<-mat[,c("id_beo", "legal_ma_1")]
dbBegin(clusdb)
    rs<-dbSendQuery(clusdb,"UPDATE zoneConstraints set percentage = :legal_ma_1 where zoneid = :id_beo and threshold < 140 and threshold > 40 and reference_zone = 'rast.zone_cond_beo_pg_cclup';", mat)
dbClearResult(rs)
dbCommit(clusdb)

old<-readRDS(paste0(here::here(), "/R/scenarios/fisher_cclup/lowToInt_Old.rds"))
old<-old[,c("id_beo", "legal_ol_1")]
old_layer<-data.table(dbGetQuery(clusdb, paste0("select * from zoneConstraints where reference_zone = 'rast.zone_cond_beo_pg_cclup' and zoneid in(",paste(old$id_beo, collapse = ",", sep = ""),")")))
threshold<-old_layer[, max(threshold), by = zoneid]

old2<-merge(old, threshold, by.x = "id_beo", by.y = "zoneid")

dbBegin(clusdb)
    rs<-dbSendQuery(clusdb,"UPDATE zoneConstraints set percentage = :legal_ol_1 where zoneid = :id_beo and threshold = :V1  and reference_zone = 'rast.zone_cond_beo_pg_cclup';", old2)
dbClearResult(rs)
dbCommit(clusdb)
...
setPaths(modulePath = here::here("R/SpaDES-modules"),
         outputPath = here::here("R/scenarios/fisher_cclup"))
...
parameters <-  list(
  ...
  dataCastor = list (dbName = 'clus', # name of the postgres database
                         #saveCastorDB = TRUE,
                         useCastorDB = paste0(here::here(), "/R/scenarios/fisher_cclup/rcb_castordb.sqlite"),
                         sqlite_dbname = "rcb",
                         nameBoundaryFile = "tsa", # name of the (TSA) boundary data in postgres
                         nameBoundaryColumn = "tsnmbrdscr", 
                         nameBoundary = c("Quesnel_TSA", "Williams_Lake_TSA","100_Mile_House_TSA"),  
  ...
  earlySeralRiskCastor = list(becRaster = "rast.zone_cond_beo_pg_cclup"),
   uploadCastor = list(aoiName = 'fisher_rcb', 
...
scenario = data.table(name="esr_bau_adj_og_caribou_upgrade_low", description = "business as usual: existing legal constraints and no harvesting in priority old growth deferral areas (TAP) and caribou scenario 2a (no harvest in core, conditional harvest in matrix) and also legalizing early seral and updgrading all targets in low to intermediate")
...
harvestFlow <- rbindlist(list( data.table(compartment ="Williams_Lake_TSA",
                                         partition = ' vol >= 80 and age >= 60 ', 
                                         period = rep( seq (from = 1, # run the 
                                                      to = 40, 
                                                      by = 1),
                                                    1), 
                                         flow = 8100000, #bau 12400000 
                                         partition_type = 'live'),
                              data.table(compartment ="Onehundred_Mile_House_TSA",
                                         partition = ' vol > 100 and age > 80 ', 
                                         period = rep( seq (from = 1, # run the 
                                                      to = 40, 
                                                      by = 1),
                                                    1),
                                         flow =  5500000, #bau harvest 7195000
                                         partition_type = 'live'),
                              data.table(compartment ="Quesnel_TSA",
                                         partition = ' vol >= 110 and age >= 60 ', 
                                         period = rep( seq (from = 1, # run the 
                                                      to = 40, 
                                                      by = 1),
                                                    1), 
