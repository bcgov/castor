...
scenario = data.table(name="bau", description = "business as usual - with old growth deferrals")
...
if(FALSE){
harvestFlow <- data.table(compartment ="Quesnel_TSA",partition = ' vol > 120 ', period = rep( seq (from = 1, to = 40,  by = 1),1),  flow = 8650000, #9500000 #8650000--good #8750000 too high non -adj
 partition_type = 'live')
}
harvestFlow <- data.table(compartment ="Quesnel_TSA",
                                         ...
                                         period = rep( seq (from = 1, # run the 
                                                      ...
                                                      by = 1),
                                                    1), 
                                         flow = 7310000,  #high - 7350000 #Low 7050000
                                         partition_type = 'live')
objects <- list(scenario = scenario, harvestFlow = harvestFlow)
inputs <- list()
mySim <- simInit(times = times, params = parameters, modules = modules,
                 objects = objects)
outputs(mySim) <- data.frame (objectName = c("harvestReport","growingStockReport", "zoneManagement", "earlySeralRiskReport"))
