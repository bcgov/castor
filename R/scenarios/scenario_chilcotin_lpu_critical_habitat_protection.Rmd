---
title: "Chilcotin Cariobu Local Population Unit (LPU) Canada critical habitat scenario."
...
# Scenario Background
This habitat protection scenario tests the influence of 



# Scenario "Study Area" 

CHilctoin LPU - set these Params in dataloader?
 - This LPU consists of the Itcha-Ilgachuz, Rainbows and Charlotte Alplands cariobu herds (need to 
   load LPU boudnary into clusdb)
 - The Chilcotin LPU overlaps the following timber supply areas (TSAs) and timber supply blocks 
   (TSBs):
    - GBR North TSA
    - GBR South TSA, TSB# V 47C
    - Prince George TSA, TSB# 24D
    - Quesnel TSA, TSB# 26B and 26A
    - Williams Lake TSA, TSB# 29A, 29B, 29C, 29E and 29I
 - The overlap with the Prince George TSA is minimal and most of the GBR North TSA that overlaps the 
   LPU is in Tweedsmuir Park, therefore, the timber 

critical habtit as defined in fereal recoveyr stratgye for southern moutnain cariob 

Types of protections (from Leo DeGroot/ federal recovery strategy):

```{r, create the protection type data.table}
library (data.table)
library (kableExtra)
tab_protection_type <- data.table (type = c('High Elevation All Seasons', 'Low Elevation Summer 
                                            Range', 'Low Elevation Winter Range', 'Matrix Type 1', 
                                            'Matrix Type 2'),
                                   constraint = c('No Harvest', 'No Harvest', 'No Harvest', ' Less 
                                                  than 35% forest age less than 40 years old', 'Less 
                                                  than 35% forest age less than 40 years old')
                                   )
