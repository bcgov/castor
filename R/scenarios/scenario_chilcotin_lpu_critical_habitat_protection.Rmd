...
* matrix
Each of these types corresponds to a specific defintion of how it is protected, based on its understood importance to cariobu popaultion sustainability. They are ntended to be consistent with the federal recopvery strategy for southern mountain caribou ([Environment Canada 2014](https://www.registrelep-sararegistry.gc.ca/virtual_sara/files/plans/rs_woodland_caribou_bois_s_mtn_pop_0114_e.pdf)), although the protection designations for some types are ambiguous within this plan.     

HEWR and HESR critical habitat types are typically considered the most improtant habitat for cariobu (often referred to as 'core' habitat). They typically cover areas heavily and regularly used by caribou, and existing wildlfie habtiat areas (WHAs) and ungulate winter ranges (UWRs).  
...
The priority proposed recovery actions for the Itcha-Ilgachuz herd are to protect LEWR and LESR critical habitat from further timber harvesting and development (cite herd plan). However, currently, 97% of the Itcha-Ilgachuz herd’s LEWR is available for timber harvesting (cite herd plan), and thus there is uncertainty and concern over how these proposed actions may influence forest harvest and the AAC in the Williams Lake and Quesnel TSA's. In addition, predator removal has been proposed for the Chilcotin LPU, but it's long-term effectiveness as a recovery action depends on effective habitat protection.   


```{r, create the protection type data.table}
library (data.table)
library (kableExtra)
tab_protection_type <- data.table (type = c ('High Elevation Summer', 'High Elevation Winter', 
                                             'Low Elevation Summer Range', 
                                             'Low Elevation Winter Range', 'Matrix'),
                                   constraint = c ('No Harvest', 'No Harvest', 'No Harvest', 
                                                   'Less than 35% forest age less than 40 years old', 
                                                  'Less than 35% forest age less than 40 years old')
                                   )
...
- short-term recommended recovery actions for Itcha-Ilgachuz,  relevant to the current cariobu and land use simualtor (CLUS) model, are to:
  - reduce or halt timber harvesting in Low Elevation Winter Range, Low Elevation  Summer Range and 
    Matrix habitat until the disturbance threshold is at or below the federal target of 35%
