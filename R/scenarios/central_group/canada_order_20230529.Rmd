---
title: "Canada Emergency Protection Order Scenario for Central Group Caribou Recovery Planning"
author: ""
date: "29 May 2023"
output:
  html_document: 
...
rast.zone_cond_bc_burnt_pine_crithab, rast.zone_cond_bc_kennedy_siding_crithab, rast.zone_cond_bc_moberly_crithab, rast.zone_cond_bc_narraway_crithab,  rast.zone_cond_bc_quintette_crithab = critical habitat areas to apply emergency order no harvest


```{r, update critical habitat constraints}
require (DBI)
...
check <- dbGetQuery (castordb, "SELECT * from zoneconstraints WHERE reference_zone = 'rast.zone_cond_bc_burnt_pine_crithab' OR reference_zone = 'rast.zone_cond_bc_kennedy_siding_crithab' OR reference_zone = 'rast.zone_cond_bc_moberly_crithab' OR reference_zone = 'rast.zone_cond_bc_quintette_crithab' OR reference_zone = 'rast.zone_cond_bc_narraway_crithab'")

dbExecute (castordb, "UPDATE zoneconstraints SET type = 'nh' WHERE reference_zone = 'rast.zone_cond_bc_burnt_pine_crithab'") 
dbExecute (castordb, "UPDATE zoneconstraints SET type = 'nh' WHERE reference_zone = 'rast.zone_cond_bc_kennedy_siding_crithab'") 
dbExecute (castordb, "UPDATE zoneconstraints SET type = 'nh' WHERE reference_zone = 'rast.zone_cond_bc_moberly_crithab'") 
dbExecute (castordb, "UPDATE zoneconstraints SET type = 'nh' WHERE reference_zone = 'rast.zone_cond_bc_quintette_crithab'") 
dbExecute (castordb, "UPDATE zoneconstraints SET type = 'nh' WHERE reference_zone = 'rast.zone_cond_bc_narraway_crithab'") 
...
scenario = data.table (name = "central_group_canada_order_may2023",
                       description = "Scenario where Canada places a no harvest emergency order over critical habitat for Burnt Pine, Kennedy Siding, Klinse-Za (Moberly and Scott East), Quintette and Narraway herds. Max. non-declining harvest flow = 1.5 M m3/yr. Adjacency was set to 0m. Priority queue = Closest to disturbance first, oldest second. Minimum volume: 150 m3/ha. No road recovery (300 years). Partnership areas included (A2, B2, B3, B4 = no harvest; A1, B1, B5 = 90% > 40 years old).")
