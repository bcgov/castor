...
rast.central_group_chra_032023 = 3rd order watershed units with early seral forest constraints

```{r, update partnerhsip zone constraints}
require (DBI)
source (paste0(here::here(), "/R/functions/R_Postgres.R"))
castordb <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/scenarios/central_group/inputs/central_group_march2023_castordb.sqlite"))

check <- dbGetQuery (castordb, "SELECT * from zoneconstraints WHERE reference_zone = 'rast.central_group_chra_032023'")

dbExecute (castordb, "UPDATE zoneconstraints SET threshold = 100 WHERE reference_zone = 'rast.central_group_chra_032023' AND threshold = 41")
dbExecute (castordb, "UPDATE zoneconstraints SET percentage = 50 WHERE reference_zone = 'rast.central_group_chra_032023' AND percentage = 90")
dbExecute (castordb, "UPDATE zoneconstraints SET multi_condition = NULL WHERE reference_zone = 'rast.central_group_chra_032023' AND multi_condition = ''")
...
### partition
A partition was set as vol > 150 to ensure stands with a minimum of 150m3/ha were ahrvested, i.e., low productivity stands wouldn't be harvested. 

### harvestFlow
1,425,000 m3/year a non-declining maximum harvest flow over a 200 year period.
...
scenario = data.table (name = "central_group_chra_march2023",
                       description = "Caribou Habitat Recovery Areas (CHRA) scenario. Constraint = 50% or more of the CHRA unit is > 100 years old forest, except in BWBS pine dominated stands, where it must be > 60 years old forest. Max. non-declining harvest flow = 1.425 M m3/yr. Adjacency was set to 0m. Priority queue = Closest to disturbance first, oldest second. Minimum volume: 150 m3/ha. No road recovery (300 years). Partnership areas included (A2, B2, B3, B4 = no harvest; A1, B1, B5 = 90% > 40 years old).")
