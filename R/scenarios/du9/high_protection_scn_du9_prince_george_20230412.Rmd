---
title: "High Protection Scenario Simulation for DU9 Caribou Recovery Planning in Prince George TSA"
author: ""
date: "12 April 2023"
output:
  html_document: 
...
zone_cond_pri_old_deferral = old growth deferral areas as defined by the Technical Advisory Panel (TAP) in 2022; set as not harvest areas
rast.zone_smc_zones_20210315 = caribou protection zones; all high, medium and low areas are included in Wells Gray North, Barkerville, Columbia North, Wells Gray South, Hart Ranges, Central Selkirks, Groundhog and North Cariboo herds. Core areas are no harvest and matrix areas are maximum 12% > 40 years old

```{r, check and update zone constraints}
require (DBI)
source (paste0(here::here(), "/R/functions/R_Postgres.R"))
castordb <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/scenarios/du9/inputs/du9_prince_george_april2023_castordb.sqlite"))

check <- dbGetQuery (castordb, "SELECT * from zoneconstraints WHERE reference_zone = 'rast.zone_smc_zones_20210315'")

dbExecute (castordb, "UPDATE zoneconstraints SET type = '' WHERE reference_zone = 'rast.zone_smc_zones_20210315' AND zoneid IN (75, 76, 77, 78)") # turn off Narrow Lake areas
...
### harvestFlow
4,850,000 m3/year provided a non-declining maximum harvest flow over a 200 year period. (6, 5.5, 4.9)
...
inputDir <- file.path(paste0(here::here(), "/R/scenarios/du9/inputs")) %>% reproducible::checkPath(create = TRUE)
outputDir <- file.path(paste0(here::here(), "/R/scenarios/du9/outputs"))
...
parameters <- list(
  ...
  dataCastor = list(dbName='castor',
                         saveCastorDB = FALSE,
                         sqlite_dbname = "du9_prince_george_april2023", 
                         useCastorDB = paste0(here::here(), "/R/scenarios/du9/inputs/du9_prince_george_april2023_castordb.sqlite"),
  ...
  forestryCastor = list(harvestBlockPriority = "dist, age DESC", # "dist, age DESC, vol DESC"
                      ...
                      activeZoneConstraint = c("rast.zone_cond_beo", 
                                           ...
                                           "rast.zone_cond_fsw", 
                                           "rast.zone_cond_cw",
                                           "rast.central_grp_partner_areas_072022", # partnership areas 
                                           "rast.zone_cond_pri_old_deferral", # old growth deferrals
...
scenario = data.table (name = "high_protection_du9_prince_george_tsa_april2023",
                       description = "High, medium and low caribou protection zones added to business-as-usual (BAU) scenario. Caribou protection zone core areas are no harvest and matrix areas are maximum 12% > 40 years old. Max. non-declining harvest flow = 4.85 M m3/yr. Adjacency was set to 0m. Priority queue = Closest to disturbance first, oldest second. Minimum volume: 150 m3/ha. No road recovery (300 years). Partnership areas included (A2, B2, B3, B4 = no harvest; A1, B1, B5 = 90% > 40 years old).")
...
harvestFlow <- rbindlist(list(data.table(compartment ="Prince_George_TSA",
                                     ...
                                     period = rep( seq (from = 1,
                                                      ...
                                                      by = 1),
                                                1), 
                                     flow = 24250000, #4,850,000m3/year 
                                     partition_type = 'live') 
))
