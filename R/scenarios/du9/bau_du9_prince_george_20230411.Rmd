---
title: "Business-as-usual Simulation for DU9 Caribou Recovery Planning in Prince George TSA"
author: ""
date: "11 April 2023"
output:
  html_document: 
...
db <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/scenarios/du9/inputs/du9_prince_george_april2023_castordb.sqlite"))

pixels <- data.table (dbGetQuery(db, "SELECT * FROM pixels;"))
thlb.area <- sum(pixels$thlb, na.rm = T) # 2,185,987 hectares
...
lrsy <- sum (yield.counts$total_vol) # 6,216,914 m3
yield.counts [, weight_vol := (tvol*(count_thlb/thlb.area))]
mean.vol.ha <- sum (yield.counts$weight_vol) # 281 m3
yield.counts [, weight_mai := (mai*(count_thlb/thlb.area))]
mean.mai <- sum (yield.counts$weight_mai) # 2.84 m3
dbDisconnect (db)  

```


```{r, check zone constraints}
require (DBI)
...
### harvestFlow
5,500,000 m3/year provided a non-declining maximum harvest flow over a 200 year period.
...
scenario = data.table (name = "bau_du9_prince_george_tsa_april2023",
                       description = "Business-as-usual (BAU) scenario. Max. non-declining harvest flow = 5.5 M m3/yr. Adjacency was set to 0m. Priority queue = Closest to disturbance first, oldest second. Minimum volume: 150 m3/ha. No road recovery (300 years). Partnership areas included (A2, B2, B3, B4 = no harvest; A1, B1, B5 = 90% > 40 years old).")
...
harvestFlow <- rbindlist(list(data.table(compartment ="Prince_George_TSA",
                                     ...
                                     period = rep( seq (from = 1,
                                                      ...
                                                      by = 1),
                                                1), 
                                     flow = 27500000, #5,500,000m3/year 
                                     partition_type = 'live') 
))
