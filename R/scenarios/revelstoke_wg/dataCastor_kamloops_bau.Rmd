...
---
title: "Database creation for Kamloops TSA BAU Analysis"
author: "Tyler Muhly"
...
parameters <-  list (
        ...
        dataCastor = list(dbName = 'castor',
                          saveCastorDB = TRUE,
                          sqlite_dbname = "kamloops_bau_20231205", 
        ...
        uploadCastor = list(aoiName = 'revelstoke_wg20231205',
...
scenario = data.table (name = "load_data_kamloops_bau", description = "dataCastor output")
...
kamdb <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/scenarios/revelstoke_complex/kamloops_bau_20230203_castordb.sqlite"))

pixels <- data.table (dbGetQuery(kamdb, "SELECT * FROM pixels;"))
thlb.area <- sum(pixels$thlb, na.rm = T) # ~931 000 hectares
yields <- data.table (dbGetQuery(kamdb, "SELECT * FROM yields;"))
yields [, mai := (tvol/age)]
