...
---
title: "Database creation for Arrow TSA bau Analysis"
author: "Tyler Muhly"
...
parameters <-  list (
        ...
        dataCastor = list(dbName = 'castor',
                          saveCastorDB = TRUE,
                          sqlite_dbname = "arrow_bau_20231205", 
                          ...
                          nameForestInventoryTable = "veg_comp_lyr_r1_poly2022",
        ...
        uploadCastor = list(aoiName = 'revelstoke_complex_wg',
                            dbInfo  = list(Sys.getenv("DB_HOST"), 
                                           Sys.getenv("DB_USER"), 
                                           Sys.getenv("DB_PASS"),
                                           Sys.getenv("DB_NAME")))
        )

scenario = data.table (name = "load_data_arrow_bau", description = "dataCastor output for Arrow TSA")
...
db <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/scenarios/revelstoke_complex/arrow_bau_20230203_castordb.sqlite"))

pixels <- data.table (dbGetQuery(db, "SELECT * FROM pixels;"))
thlb.area <- sum(pixels$thlb, na.rm = T) # 156 509 hectares
