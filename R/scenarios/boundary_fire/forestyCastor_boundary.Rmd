...
parameters <-  list (
        ...
        dataCastor = list(dbHost=keyring::key_get('dbhost', keyring = 'postgreSQL'),
                    ...
                    dbPass= keyring::key_get('dbpass', keyring = 'postgreSQL'),
                         saveCastorDB = FALSE,
                          dbName = 'castor',
                          useCastorDB = "C:/Work/caribou/castor/boundary_fire_castordb.sqlite",
        ...
        fireCastor = list(calculateInterval =1,
                    ...
                    numberFireReps = 1),
        climateCastor= list(maxRun =1,
                            gcm='CanESM5', #"ACCESS_ESM1_5", "CanESM5", "MPI_ESM1_2_HR" Note that MPI_ESM1_2HR seems to only have r1i1p1f1 and r2i1p1f1 for ssp 126 and ssp245. 
                            gcmname='CanESM5', 
                            ssp ='ssp245', #"ssp126", ""ssp245", "ssp370"
                            climateData = "future",
                            run = "r10i1p1f1", #r10i1p1f1","r1i1p1f1","r2i1p1f1", "r3i1p1f1"
...
```{r}
#Outputs from STSM
harvest.schedule<-data.table(readr::read_tsv("C:/Work/caribou/castor/R/scenarios/boundary_fire/harvest_sched/logDebug.txt", col_names = TRUE))
...
ras.pixelid<-rast("C:/Work/caribou/castor/R/scenarios/boundary_fire/harvest_sched/tsa02_ogc_id.tif") 
harvest.schedule<-data.table(read_tsv("C:/Work/caribou/castor/R/scenarios/boundary_fire/harvest_sched/logDebug.txt", col_names = TRUE))
#currBlockId is not unique across the sim. It resets back to one by each period. Assign a unique blockid to each block
harvest.schedule[, blockid:=paste0( yr, yearInPeriod,"0", currBlockId)][, blockid:=as.integer(blockid)]
