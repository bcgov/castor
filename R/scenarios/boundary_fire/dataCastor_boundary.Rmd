...
```{r}
castordb <- dbConnect(RSQLite::SQLite(), dbname =paste0 (here::here (), "/R/scenarios/boundary_fire/boundary_fire_castordb.sqlite"))
...
dbExecute(castordb, "UPDATE pixels set blockid = pixelid;")
dbExecute(castordb, "DELETE from blocks;")

#get the landings
out<-data.table(dbGetQuery(castordb, "select pixelid as blockid, pixelid as landing from pixels order by pixelid"))

dbBegin (castordb)
rs <- dbSendQuery (castordb, "INSERT into blocks (blockid, landing) values (:blockid, :landing)", out)
dbClearResult (rs)
dbCommit (castordb)
