...
con <- dbConnect ("PostgreSQL",
                  ...
                  dbname = keyring::key_get("vmdbname", keyring="postgreSQL"),
				          port = "5432")
data.harvest <- dbGetQuery (con, "SELECT * FROM lakes_tsa.harvest")
dbDisconnect (con)
data.summ <- data.harvest %>% # get summary data
              filter (scenario == "lakes_chil_hele0d_m15d") %>% # lakes_bau, lakes_wct_nh, lakes_chil_nh, lakes_chil_hele0d_m15d, lakes_all_herdbounds_nh

                  summarise (sum (volume)/40)


table.volume = data.table (
  harvest_unit = c ("Lakes TSA", "Lakes TSA", "Lakes TSA", "Lakes TSA", "Lakes TSA"),
  scenario = c ("BAU", "No harvest in Takla herd", "No harvest in Tweedsmuir critical habitat", "35% disturbance in Tweedsmuir matrix critical habitat", "No harvest in Takla herd and Tweedsmuir critical habitat"),
  adjacency = c (3, 3, 3, 3, 3),
  recovery = c (0, 40, 40, 40, 40),
  annual_volume = c (759935, 754456, 526749, 564774, 512700),
  diff_annual_volume = c (0, 5479, 233186, 195161, 247235),
  perc_diff_tot_volume = c ("0", "1%", "31%", "26%", "33%")
)
kable (table.volume,
