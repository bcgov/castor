...
con <- dbConnect ("PostgreSQL",
                  ...
                  dbname = keyring::key_get("vmdbname", keyring="postgreSQL"),
				          port = "5432")
data.harvest <- dbGetQuery (con, "SELECT * FROM arrow_tsa.harvest")
dbDisconnect (con)
data.summ <- data.harvest %>% # get summary data
              filter (scenario == "arrow_all_ch_he0d_m15d") %>% # lakes_bau, lakes_wct_nh, lakes_chil_nh, lakes_chil_hele0d_m15d, lakes_all_herdbounds_nh, arrow_bau, arrow_centselk_nh, arrow_centselk_ch_he0d_m15d, arrow_all_ch_he0d_m15d
...
table.volume = data.table (
  ...
  scenario = c ("BAU", "No harvest Central Selkirks", "35% disturbance in Central Selkirks matrix critical habitat", "No harvest in all critical habitat", "35% disturbance in all matrix critical habitat",
