...
dbExecute(clusdb, "UPDATE zoneconstraints SET percentage = 12 where reference_zone = 'rast.zone_cond_eccc_purcell_central_crithab_or_herd' AND zoneid = 2") 

dbGetQuery(clusdb, "SELECT * from zoneConstraints WHERE reference_zone = 'rast.zone_cond_eccc_purcells_south_crithab_or_herd'")
dbExecute(clusdb, "UPDATE zoneconstraints SET threshold = 0 where reference_zone = 'rast.zone_cond_eccc_purcells_south_crithab_or_herd' AND zoneid = 2") 
...
# scenario = data.table (name = "cranbrook_bau", description = "Business-as-usual case. Harvest flow = 758,000m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "cranbrook_purcell_central_nh",
# description = "No harvest in Purcell Central critical habitat. Harvest flow = 758,000m3/yr. Adjacency = 3m. Oldest first.")
...
# scenario = data.table (name = "cranbrook_purcells_south_nh",description = "No harvest in Purcells South critical habitat. Harvest flow = 780,000m3/yr. Adjacency = 3m. Oldest first.")
 scenario = data.table (name = "cranbrook_purcells_south_ch_he0d_m12d",
                        description = "No harvest in Purcells South high elevation critical habitat, maximum 12% disturbance in Purcells South matrix critical habitat. Harvest flow = 780,000m3/yr. Adjacency = 3m. Oldest first.")
...
# scenario = data.table (name = "cranbrook_purcell_central_ch_he0d_m12d",
 #                       description = "No harvest in Purcell Central high elevation critical habitat, maximum 12% disturbance in Purcell Central matrix critical habitat. Adjacency was set to 3m.")
...
harvestFlow <- rbindlist(list(data.table(compartment ="Cranbrook_TSA",
                                         ...
                                         period = rep( seq (from = 1, # run the 
                                                    ...
                                                    1), 
                                         flow = 3790000,
                                         partition_type = 'live') #760,000m3/yr 
                            )
                        )
