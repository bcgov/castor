...
- used oldest first (no info on this in TSR docs)
...
## 'harvestFlow'
Effective February 16, 2011, the  AAC for the Kalum TSA was set at 424,000 cubic metres; the maximum even-flow harvest level was 400,558 cubic metres per year.

- here I used 300m3 as min volume for harvest (no info on this in TSR docs)

- I started with 400,000, 500,000, 450,000, 430,000, 425,000; final

#UPDATE SCENARIO
```{r, zone_update}
#STEP 1: Connect to the clusdb.sqlite database for the AOI
clusdb <- dbConnect(RSQLite::SQLite(), dbname = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/Kalum_TSA_clusdb.sqlite")) # connect to clusdb -- set the locations of the sqlite
...
# rast.zone_cond_bc_telkwa_crithab  rast.zone_cond_bc_tweedsmuir_crithab   rast.zone_du7_scenario_20210330
...
dbExecute(clusdb, "UPDATE zoneconstraints SET type = 'nh' where reference_zone = 'rast.zone_du7_scenario_20210330' AND (zoneid = 77) OR (zoneid = 78) OR (zoneid = 79) OR (zoneid = 80) OR (zoneid = 81)")
dbExecute(clusdb, "UPDATE zoneconstraints SET type = '' where reference_zone = 'rast.zone_du7_scenario_20210330' AND (zoneid = 71) OR (zoneid = 72) OR (zoneid = 74)") 
...
# Below makes priority forest stands no harvest and all of the matrix and LEWR habitat areas max 15% disturbance; NOTE all zones high priority
dbExecute(clusdb, "UPDATE zoneconstraints SET type = 'nh' where reference_zone = 'rast.zone_du7_scenario_20210330' AND (zoneid = 71) OR (zoneid = 72) OR (zoneid = 77) OR (zoneid = 78) OR (zoneid = 79) OR (zoneid = 80) OR (zoneid = 81)") 
...
parameters <- list(
  ...
  dataLoaderCLUS = list( 
                           dbName='clus',
                           save_clusdb = TRUE,
                           sqlite_dbname = "Kalum_TSA",
                           useCLUSdb = paste0(here::here(), "/R/SpaDES-modules/dataLoaderCLUS/Kalum_TSA_clusdb.sqlite"),
                           #Study Area
                           nameBoundaryFile = "tsa_nw_coast_no_tfl",
                           nameBoundaryColumn = "tsa_name",
                           nameBoundary = "Kalum_TSA",
  ...
  uploaderCLUS = list(aoiName = 'kalum_tsa', # name of the schema that gets uploaded to postgres
...
# scenario = data.table (name = "kalum_bau",
#                        description = "Business-as-usual case. Harvest flow = 425,000m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "kalum_du7_grp1_nh",
#                        description = "No harvest in group 1 (Itcha, Rainbows, Charlotte, Tweeds, and Telkwa) critical habitat that overlaps with the harvest unit (e.g., TSA or TFL). Harvest flow = 425,000m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "kalum_du7_grp1_he0d_mlewr12d",
#                        description = "No harvest in group 1 (Itcha, Rainbows, Charlotte, Tweeds, and Telkwa) HEWSR and LESR critical habitat; harvest allowed in LEWR and matrix critical habitat up to 12% disturbance (no buffer) in forested area. Harvest flow = 425,000m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "kalum_du7_grp1_hi_priority",
#                        description = "No harvest priority stands in high priority zones in group 1 (Itcha, Rainbows, Charlotte, Tweeds, and Telkwa) HEWSR and LESR critical habitat; harvest allowed in high priority zones in LEWR and matrix critical habitat up to 15% disturbance (no buffer) in forested area. Harvest allowed in low and medium priority zones. NTOE: Only high priority zones in this unit.  Harvest flow = 950,000m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "kalum_tweeds_nh",
#                        description = "No harvest in Tweedsmuir critical habitat that overlaps with the harvest unit (e.g., TSA or TFL). Harvest flow = 425,000m3/yr. Adjacency = 3m. Oldest first.")
# scenario = data.table (name = "kalum_tweeds_he0d_mlewr12d",
#                        description = "No harvest in Tweedsmuir HEWSR and LESR critical habitat; harvest allowed in LEWR and matrix critical habitat up to 12% disturbance (no buffer) in forested area. Harvest flow = 425,000m3/yr. Adjacency = 3m. Oldest first.")

scenario = data.table (name = "kalum_tweeds_hi_priority",
                       description = "No harvest priority stands in high priority zones in Tweedsmuir HEWSR and LESR critical habitat; harvest allowed in high priority zones in LEWR and matrix critical habitat up to 15% disturbance (no buffer) in forested area. Harvest allowed in low and medium priority zones. NOTE: Only high priority zones in this unit.  Harvest flow = 950,000m3/yr. Adjacency = 3m. Oldest first.")



harvestFlow <- rbindlist(list(data.table(compartment ="Kalum_TSA",
                                     partition = ' vol > 299 ', 
                                     period = rep( seq (from = 1, # run the 
                                                ...
                                                1), 
                                     flow = 2125000,
                                     partition_type = 'live') #425,000m3/yr
))
