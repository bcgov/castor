...
The 2003 VRI infromation was used to set the similarity metric for the pre-blocking alogrithum. Both 2003 VRI photo-interpreted crown closure and height were used to calculate the mahalanobis distance for determing the similarity metric. Both crown closure and height were scaled (mean subtracted and divided by the standard deviation) to remove any variable dominance in the distance calculation. The mahalanobis distance ($D^2$) was calculated  as
