...
The SCRM can be conceptualized using a topographic view of a watershed. Watersheds define a network of 'ridges' that represent the boundaries where water would drain towards. In the harvest blocking problem these ridges are boundaries of homogenous harvest untis. To complete the region merging, the idea is to find sinks across the image where the rain would drain towards. Assuming these sink areas are 'water springs' from which the uplift of water would fill these areas, object instantiation becomes apparent. As water fills a sink, these areas represent contiguous areas with similar features (i.e., elevation). Various size constraints can then be used to stop the process of merging which allows objects to be delineated.
