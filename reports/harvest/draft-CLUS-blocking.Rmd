...
The spatial simulation of cutblock size can be approached by 1) sampling a random variable from a distribution (estimated empirically) and 2) pre-solve the forest area into blocks by aggregating based on rules. It is important to note that the number of cutblocks and their respective sizes are largely related, given if the size of a stand is artifically set too small, a greater footprint could result from the scheduler, as it looks to find more wood.
