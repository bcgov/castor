...
### Model Parameters
In addition to using the same initial and projected forest conditions, the models were set-up with the same timber supply modeling parameters. This included a minimum harvest age of 80 years old, minimum harvest volume of 150 m^3^/ha and no adjacency constraints. Each scenario was simulated to achieve a maximum sustainable harvest flow, i.e., maximum volume that is harvested at each time interval with never going down, over a 250 year simulation period. Both models run at a 1 ha spatial resolution (here referred to as a *pixel*). 

Both models have flexibility in how minimum harvest criteria and the harvest queue is parameterized. Castor's approach is relatively simple, as it allows the user to parameterize an SQL query of forest stand characteristics (e.g., age, volume, height), in ascending or decending order. Multiple criteria can be used, but they are hierarchical. For example, a harvest queue of oldest and then highest volume, would sort the forest stands by age, and then within age, sort them by highest volume. STSM allows for multiple harvest criteria to be integrated, where criteria are multiplied to calculate a preference index. For example, age and volume are multiplied to create an index of harvest preference, where forest stands with high values for the product of the two criteria are prioritized.  It is possible to program Castor to calculate a ranking based on multiple criteria, but this capability does not exist in the current iteration. 

Other differences between Castor and STSM are that STSM updates its harvest queue four times a year, whereas Castor's queue is calculated once at the start of a time interval, and STSM includes a 'look-ahead' function, where the model determines whether a pixel or block achieves the minimum harvest criteria (e.g., 80 years old, minimum harvest volume of 150 m^3^/ha) by the end of the time interval. For example, it checks whether the target is achieved at years 10 in a 10 years interval. If it meets the criteria by the end of the interval than the stand is available for harvest. 
...
* modified harvest constraint; annual time step; no blocking, no roads 

Two simulations had no roading and no harvest block definition, i.e., each 1 ha pixel was considered a harvest 'block'. Of these two simulations, one was simulated with an annual time step to compare the two models without the effect of interpolating yield values between decadal periods. A second was simulated with a decadal time step to show the effect of the different yield interpolation methods used by each model (see *Yield Update* below). The blocking scenario allowed for comparison of how each model simulated harvest blocks (see *Harvest Blocking* below).

The no harvest constraint scenario essentially removed a hypothetical zone of interest from the THLB. The modified harvest scenario applied an age-based constraint to the same zone of interest, where 50% of productive forest land base of the zone (i.e., forested areas with a site productivity greater than 5) had to be greater than or equal to 175 years of age.    
...
#### Castor
The Castor model updates growth and yield values at the mid-point of a simulation period. Therefore, if a simulation period is a decade, in the first period Castor updates growth and yield estimates to year 5 of the simulation by interpolating the estimates between the initial (year 0) and 10 year estimates from the growth and yield model. For example, if the initial stand volume is 150 m^3^/ha at year 0, and 170 m^3^/ha at year 10, then the Castor model will use 160 m^3^/ha as the value for the first interval. The age of forest stands harvested at the mid-point is set to 0. Castor then updates stand age and growth and yield estimates at 10 year intervals from there, but the key point is that things like volume harvested are first accounted for at the middle of the interval, not the beginning or end of the interval.  

#### STSM
In contrast to Castor, STSM does not update growth and yield estimates at the mid-point of an interval. Instead, it accounts for growth and yield estimates at each year of the interval. Therefore, teh harvest request is divided equally by the number of years in the interval. For example, when using a 10 year interval, it will update and harvest stand volume at year 1, year 2, year 3, etc., up to year 10. The interval that each stand is harvested is multiplied by -1 (e.g., year 1 = -1, year 2 = -2, year 3 = -3,  etc.) and then in the next interval stands are aged 10 years (i.e., stand ages become 9, 8, 7, etc.). In addition it harvests stands at the start of the interval, i.e., the harvest request begins at year 0, and thus the volume is not updated in the first year. 

###  Harvest Blocking
Scenarios with and without blocking were simulated to compare how the two models created harvest blocks, and its potential effects on harvest flow. When blocking was not simulated, both models selected harvest areas at a 1 ha spatial resolution (a pixel) using the harvest priority criteria (i.e., oldest first). Therefore, each block was harvested interdependently, in order of its age, and there was no defined spatial pattern or aggregation of pixels to form blocks.    

#### Castor
Castor uses the agglomerative hierarchical clustering method to pre-define harvest blocks prior to the harvest simulation. This method searches the landscape and groups pixels (i.e., ha) into blocks with similar forest characteristics (i.e., crown closure and height) and in close spatial proximity (i.e., Euclidean distance). It evaluates whether to cluster pixels together using the mahalanobis distance statistic, which measures how far a pixels stand characteristics are from the distribution of the mean value of those characteristics in neighbouring pixels. If the mahalanobis distance is below a threshold (in this case the default value of 6), then a pixel gets grouped into the cluster, otherwise it gets grouped into another cluster, or forms its own cluster.     

Once clustering is complete, block sizes are determined by drawing from a distribution of block sizes, where larger blocks are placed on the landscape first and subsequently smaller blocks are added until blocking is complete.

#### STSM
STSM uses a spreading algorithm to create blocks during the simulation. It selects 1 ha pixels based on the harvest priority queue, then selects a blocks size from a uniform block size distribution. The model then 'spreads' to neighbouring pixels based on age threshold criteria, where it is only able to spread to a neighbouring pixel that is within 20% of the age of the origin pixel, up to the selected block size. Block size targets are considered 'soft', i.e., they are approximate targets, and they must be a minimum of 10 ha in size.   

### Roads
Castor and STSM have developed different approaches to create road networks related to forest harvest. Simulations of both models were run to compare outputs of the roading algorithms and assess whether they achieve similar results. Below provides a detailed description of the different approaches used by each model. 

#### Castor
Castor can simulate roads using multiple approaches, but the preferred method, described here, uses a dynamic minimum spanning tree (MST) to create roads within a simulation. Castor pre-defines potential road networks by identifying a pixel on the edge of each pre-defined harvest block (see *Harvest Blocking*, above) as a 'landing' from which timber will be hauled. Castor then solves the least-cost path between each landing, nearest existing road and edge of the area of interest using Dijkstra’s algorithm, where the least costs path form the landing to neighboring cells (in a queen's case, where all 8 neighbouring cells are considered) is solved iteratively to identify the cells with the least cost 'path'. 

During the simulation, as harvest blocks are selected by the harvest priority queue, Castor 'builds' the network of roads in a MST. The MST connects all the selected landings without any loops or cycles using Kruskal's algorithm, which sorts the potential routes by their cost, and adds the least costly road connections, without a loop or cycle, until the netwrok is completed.

The least-cost path is solved using a spatial cost model, where each pixel was attributed a cost of building a road based on factors such as terrain, water and land designations. The same cost-surface model was used in both the Castor model and STSM, but the relative difference in cost values were inflated for use in Castor by taking the square of the cost values. Instead of values between 1 and 62, the values were scaled between 1 and 3844. This was necessary to fit reasonable least-cost paths in Castor.
...
STSM creates a simulated road network by selecting locations in areas of the THLB that are greater than 1 km from an existing road. Locations further from an existing have a higher probability of being selected. It then iteratively connects the random locations to destination points following a least-cost path until all locations are within 1km from an existing road. Therefore, an entire road network that can connect to within 1 km of any portion of THLB is created prior to the simulation. During the simulation, as blocks are selected in the harvest queue, a landing is selected in the block and a straight line (as a crow flies) road is created to connect it to the road network (existing or pre-processed roads).    

When running the simulation here, an additional constraint was included to limit the harvest queue to blocks less than 2 km form an active road. This had the effect of limiting the extent of road development from one time interval to the next. 

### Forest Cover Constraints
Here we included two constraint scenarios to compare the effect of the two different approaches of Castor and STSM to setting aside areas to meet a forest cover constraints. We created a 'dummy' zone over the THLB in the eastern portion of the area of interest. In one scenario we set this as a no harvest zone (essentially removing it from THLB), and in a second scenario where a forest cover constraint of 75% of the productive forest (i.e., forested areas with a site index > 5) had to be greater than or equal to 175 years old. The former scenario was run on a decadal time step and the latter scenario was run with an annual time step to facilitate comparison. 

Both models use the approach of defining the constraint variable of interest, i.e., the forest stand or landscape condition that is desired, the desired threshold of the variable (i.e., greater than or less than a target value for the variable), the percentage of the zone that must achieve the threshold and the land base definition (i.e., denominator) over which to apply the percentage.  

#### Castor
The Castor model sets forest cover constraints by reserving pixels (1 ha areas) in the zone that are above the threshold for the variable of interest (e.g., age, height, volume), in order from the highest to lowest values, up to when a sufficient percentage of area has been reserved. For example, here the reserve area is 75% of the productive forest in the zone greater than 175 years old, therefore Castor reserves areas of the zone that are greater than 175 years old, starting with the oldest pixels, up to when 75% of the zone ha been reserved. If the reserve area is not achievable, than it becomes a *de facto* no harvest area. Constraints are re-calculated at each interval of the simulation period. Note that in cases where there are multiple overlapping zones with different forest cover constraints, Castor prioritizes selecting pixels in a zone that were previously reserved to meet another constraint. 
...
```{r harvest flow contraint, no harvest}
...
```{r harvest flow constraint, modifed harvest}
...
When blocking was implemented in both models, the timber supply was reduced by a similar amount, again, with a 2% difference between them. However, in this case Castor harvested 2% less than STSM, suggesting that the blocking algorithm used in Castor was more constraining than the algorithm used in STSM. It's not clear why Castor's approach to blocking was more constraining, but it may be that the pre-blocking algorithm provides less spatial freedom in the harvest compared to STSM, which uses a spread algorithm. 
