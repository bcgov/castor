...
* harvest constraint; decadal time step; no blocking, no roads 
...
###  Yield Update
The Castor and STSM models have different approaches for updating growth and yield values (e.g., volume, height) over time.
...
When the models were run with a hypothetical no harvest zone in the eastern portion of the area of interest (outlined in green over the timber harvest land base), the Castor model harvested 108,500 m^3^/year, 19% less than without the no harvest zone, and the STSM model harvested xxxxxx m^3^/year, xx% less than without the no harvest zone.
...
ggplot (tab.harvest.flow, aes (x = timeperiod, y = volume, color = scenario)) +
              ...
              theme (legend.position = c (0.15, 0.15),
                     ...
                     legend.text = element_text( size = 8)) +
              scale_x_continuous (breaks = seq(0, 250, 25)) +
              scale_y_continuous (label = comma, breaks = seq (100000, 140000, 2000)) +
              theme_classic()
