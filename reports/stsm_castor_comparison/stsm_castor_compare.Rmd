...
Another key difference between Castor and STSM is that Castor's preference order is updated four times a year whereas Castor's order is calculate once at the start of a time interval. However, the effect of the fine-scale re-ordering was not tested in this analysis
...
###  Harvest Blocking
In this analysis, scenarios with and without blocking were simulated to compare how the two models created harvest blocks, and its potential effects on harvest flow. When blocking was not simulated, both models selected harvest areas at a 1 ha spatial resolution (a pixel) using the harvest priority criteria (i.e., oldest first). Therefore, each block was harvested interdependently, in order of its age, and there was no defined spatial pattern or aggregation of pixels to form blocks.    

#### Castor
Castor uses the agglomerative hierarchical clustering method to pre-define harvest blocks. This method searches the landscape and groups pixels (i.e., ha) into blocks with similar forest characteristics (i.e., crown closure and height) and in close spatial proximity (i.e., Euclidean distance). It evaluates whether to cluster pixels together using the mahalanobis distance statistic, which measures how far a pixels stand characteristics are from the distribution of the mean value of those characteristics in neighbouring pixels. If the mahalanobis distance is below a threshold (in this case the default value of 6), then a pixel gets grouped into the cluster, otherwise it gets grouped into another cluster, or forms its own cluster.     

Once clustering is complete, block sizes are determined by drawing from a distribution of block sizes, where larger blocks are placed on the landscape first and subsequently smaller blocksare added until blocking is complete.

#### STSM
STSM uses a spreading algortihm to
...
- rooks case
- building min density of roads; efficiency 
...
### Constraints
- no harvest zone
- for same zone, applied a harvest constraint where 75% of the productive forest (i.e., forested areas with a site index > 5) had to be greater than or equal to 250 years old; (note that i used the aflb data form STSM)
...
- constraints from minimum of threshold (e.g., age) to increaign values up until teh percetnate is achieved; so in case of an age cosntaint, with a 100 year threhold, ti tcoudl reserve satnd from 100 to 120 years old adn leave stands > 120 uyears old if the percentage is met
...
### Partial Harvest Constraint Simulation
When the models were run with a modified harvest zone (i.e., at least 50% of the zone greater than 175 years old forest) in the eastern portion of the area of interest (outlined in green over the timber harvest land base), the Castor model harvested 118,500 m^3^/year, 11% less than without the modified harvest zone, and the STSM model harvested xxxx m^3^/year, xx% less than without the no modified zone. The difference between the two models was xxxxx m^3^/year, or approximately 4%

```{r harvest flow constraint, modifed harvest}

tab.harvest.flow <- castor.harvest.flow [scenario == "", c ("scenario", "timeperiod", "volume")]
tab.harvest.flow$model <- "castor"
...
- Castor and STSM models are closely aligned in terms of estimating long-term timber supply; suggesting the overall logic of both models is consistent.
...
- advantage of Castor is that it's dynamic and coudl repond to landscape change; advnatage of STSM is....

- differences in periods of road development were likely due to contraints in STSM on road development (i.e., blocks could only be activated within 2km of an existing roads)

- effects of no harvest constraints were consistent with overall differences (~2%)
