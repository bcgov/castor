...
- avoided patches near pipelines (\beta = 0.107) 
- selected for 1 to 5 year old (\beta = 0.068) beetle killed stands; but avoided 6 to 9 year 
  beetle killed old stands (\beta = -0.201) 
- avoided all burn age classes (\beta = -0.019 to -0.252), but weak (\beta = -0.019) and not 
  statistically significant (*p-value* = 0.344) for over 25 years old burns
- selected patches that had deeper winter snow (\beta = 0.311), higher summer precipitation (\beta = 
  0.091) and higher summer temperature (\beta = 0.082)
- selected BWBSmw (\beta = 0.021) compared to BWBSmk, but not statistically significant (*p-value* = 
  0.400); avoided BWBSwk 3 (\beta = -0.535) compared to BWBSmk
- strongly selected for treed bog (\beta = 0.582), other (\beta = 0.341), nutrient poor fen 
  (\beta = 0.493) and conifer swamp (\beta = 0.328) wetland types relative to upland conifer 
- selected for nutirent rich fen (\beta = 0.192)relative to upland conifer
- strongly avoided upland deciduous (\beta = -0.732) relative to upland conifer, and avoided decdious   swamp (\beta = -0.100)
- selected for patches with higher percentage of bryoid cover (\beta = 0.149), shrub cover (\beta = 
  0.135) and herb cover (\beta = 0.073) 
- selected for pacthes with older  (\beta = 0.112), but shorter (\beta = -0.131) and closed (\beta = 
  0.140) forest stadns


- in genreal, in summer there is was a mix of avoidance of 'productive' areas (i.e., riparian areas, deciduous wetland cover types, burned areas), but greater use of areas near younger cutblocks and warmer, wetter areas, so a bit conflicting in terms of forage and predation risk; 
- interestingly, a lag effect of snow?, where caribou selected areas in summer that had deeper snow in winter; unclear what the mechanism behind this might be 
...
early winter, 
- visually and statsically cosnsitent with Demars: overlap in defintion of high and low quality habitat pacthes; selected treed bogs and fens; avoid decisuous adn upland types; avoid  lienar features and select early seral/immature forest
