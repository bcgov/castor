...
- selected for pacthes with older  (\beta = 0.075), but shorter (\beta = -0.214) adn open (\beta = 
  0.083) forest stadns
...
##### k-fold validation
###### DU6, Early Winter 
- stong validation results, slopes of lieanr models between expected adn observed coutns of cariobu locatiosn in RSF bins were signfcaitly different form zero, but not 1, and intercepts were nto sigfncatly different form 0 (Table xx, Figure xx); adjusted *R^2* values were greater than 0.98 and $$\chi^2$$ values were not signcant (Table xx)
 - visual vlaiation looks good (add fig form GIS or try adn do it in R - use quantile classifcation)
