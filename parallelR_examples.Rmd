...
Note inorder for this work have to set up SSH. On windows this may require downloading an OpenSSH server and having PuTTy.
...
### 2. Functions and Environments
Setting up the functions and environments from which the workers will process. In _snow_, these take a functions object as an argument and then send it to the worker. This functions must be serialized into a strem of bytes
...
#Takes two arguments: the cluster object, and an expression that is evaluated on each of the workers. It #returns the result from each of the workers in a list. However, doesn't allow parameters to be past. Try: #ClusterCall()
...
### clusterApplyLB()
This function schedules tasks to workers as the need them. Thus, tasks with uneven loads (i.e., tasks that would finish at differnt times) would be balanced. This function is more efficent if some tasks take longer than others or some workers are slower, e.g., > 10 seconds wasted due to round robin
```{r}
#example sys.sleep function
snow.time(
...
### clusterSplit()
This function can be used similar to cases that use parLapply(), the difference is it doesn't need to use lapply() to call the users function. It will split the tasks according to the number of workers. This is useful for the user to define components of the function that are parallel.
