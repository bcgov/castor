...
library(bcdata)
library(dplyr)
#source (paste0(here(),"/functions/R_Postgres.R"))
...
wha_poly <- bcdc_query_geodata("WHSE_WILDLIFE_MANAGEMENT.WCP_WILDLIFE_HABITAT_AREA_POLY") %>%
  collect()

wha_poly$ogc_fid <- seq_len(nrow(wha_poly))

#fasterize 
wha.ras<-fasterize::fasterize(sf= wha_poly, raster = prov.rast , field = "ogc_fid")
...
wha_cond<-wha_poly %>% dplyr::select(ogc_fid, HABITAT_AREA_ID, TIMBER_HARVEST_CODE, TAG, FEATURE_CODE, APPROVAL_DATE)

#assign all no harvest as zoneid = 0 
ogc_fids_noharv<-wha_cond %>% filter(TIMBER_HARVEST_CODE == 'NO HARVEST ZONE') %>% dplyr::select(ogc_fid)
zone_wha<-data.table(zoneid = 0, type = 'nh', variable = '', 
                     threshold = 0, reference_zone='rast.zone_wha', percentage =0, 
                     ndt =0, label = 'wha_no_harv', multi_condition = '', denom = '')

#set all to 0 in the raster as no harvest
wha.ras[wha.ras[] %in% ogc_fids_noharv$ogc_fid] <- 0

wha_cond<-wha_cond %>% filter(TIMBER_HARVEST_CODE !='NO HARVEST ZONE') #get only the conditional harvesting ones now.
...
'6-001', '6-002', '6-003', '6-283', '6-284', '6-285', '6-286', '7-004', '7-005', '7-006', '7-007', '7-008', '7-009', '7-010', '7-011', '7-017','8-020', '8-022', '8-023', '8-024', '8-025', '8-033', '8-053', '8-054', '8-055', '8-056', '8-063', '8-064', '8-239', '8-243', '8-244', '8-368', '8-369', '8-370', '8-371',
...
```{r, 2-012}
# Order refers to Mountain Beaver https://www.env.gov.bc.ca/wld/documents/wha/APRU-2-012_ord.pdf
...
# no specific harvest requirements so we will remove this one.
wha_tags <- c('2-012')
ogc_fids_rm <- wha_cond[wha_cond$TAG %in% wha_tags, "ogc_fid"]
wha.ras[wha.ras[] %in% ogc_fids_rm] <- NA
wha_cond <- wha_cond[!(wha_cond$TAG %in% wha_tags), ] #remove from table
...
# complicated order, but essentially appears to be partial harvest; <40% of the basal area harvested; also trying to retain trees >30cm to >50cm dbh
# order modeled as 60% > 20 m tall to retain large diameter trees; this may be too liberal as the historic harvest in this area appears minimal; can also implement a pseudo-partial harvesting by setting blocks as 1 ha in size so that the cut is more likely to get dispersed
...
# complicated order, but essentially appears to be partial harvest; 10% retained for wildlife tree patches and then <40% of the remaining basal area harvested; also trying to retain trees >30cm to >50cm dbh
# order modeled as 65% > 20 m tall to retain large diameter trees; this may be too liberal as the historic harvest in this area appears minimal; can also implement a pseudo-partial harvesting by setting blocks as 1 ha in size so that the cut is more likely to get dispersed 
...
ogc_fids_rm<-wha_cond[wha_cond$TAG %in% wha_tags, c("ogc_fid", "TAG")]
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'height', threshold = 5, reference_zone='rast.zone_wha', percentage =70, ndt =0, label = ogc_fids_rm$TAG,
                                       multi_condition = '', denom = ''))
wha_cond<-wha_cond[!(wha_cond$TAG %in% wha_tags), ] #remove from table
```
...
```{r, '4-056','4-057','4-058','4-059','4-060','4-061','4-062','4-063','4-064','4-289', '4-292', '4-293', '4-297', '4-299', '4-300','4-301',' 4-310', '4-316', '4-317', '4-290', '4-291', '4-294', '4-295', '4-296', '4-298', '4-308',' 4-309',' 4-311', '4-315'}
wha_tags<-c('4-056','4-057','4-058','4-059','4-060','4-061','4-062','4-063','4-064', '4-289', '4-292', '4-293', '4-297', '4-299', '4-300','4-301','4-310', '4-316', '4-317', '4-290', '4-291', '4-294', '4-295', '4-296', '4-298', '4-308','4-309','4-311', '4-315')
ogc_fids_rm<-wha_cond[wha_cond$TAG %in% wha_tags, c("ogc_fid", "TAG")]
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'height', threshold = 5, reference_zone='rast.zone_wha', percentage =70, ndt =0, label = ogc_fids_rm$TAG,
                                       multi_condition = '', denom = ''))
wha_cond<-wha_cond[!(wha_cond$TAG %in% wha_tags), ] #remove from table
...
ogc_fids_rm<-wha_cond[wha_cond$TAG %in% wha_tags,c("ogc_fid", "TAG") ]
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'height', threshold = 5, reference_zone='rast.zone_wha', percentage =50, ndt =0, label = ogc_fids_rm$TAG,
                                       multi_condition = '', denom = ''))
wha_cond<-wha_cond[!(wha_cond$TAG %in% wha_tags), ] #remove from table
```
...
wha_tags<-c('4-098')
ogc_fids_rm<-wha_cond[wha_cond$TAG %in% wha_tags,c("ogc_fid", "TAG") ]
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'height', threshold = 5, reference_zone='rast.zone_wha', percentage =80, ndt =0, label = ogc_fids_rm$TAG,
                                       multi_condition = '', denom = ''))
wha_cond<-wha_cond[!(wha_cond$TAG %in% wha_tags), ] #remove from table
```
...
### 4-281 to 4-288, 4-306, 4-307 and 4-312 bull trout and westslope cutthroat trout
All remaining tags are for specified areas. 
Forest harvest is not permitted within the Enhanced Riparian Reserve Zones adjacent 
to streams as identified in Table 2, except as needed to accommodate the construction of a stream crossing as described in GWM 17.  
12. Forest harvest is not permitted within the Enhanced Riparian Reserve Zones adjacent to wetlands and lakes as identified in Tables 3 and 4 unless no hydrological connection exists between the feature(s) and downstream S1 to S6 streams. 
13. Forest harvest activities within the Enhanced Riparian Management Zones identified 
in Tables 2, 3 and 4 must:  
a. be planned and conducted in a manner that minimizes the potential for 
windthrow within the Enhanced Riparian Reserve Zones, including completion 
of a Windthrow Hazard Assessment (Appendix A);   
b. retain a minimum of 50% of basal area  

As I am not sure how to specify reserve zones Im going to just go with retain a minimum of 50% basal area. 



```{r, w4-281 to 4-288 4-306 4-307 and 4-312}
wha_tags<-c('4-281','4-282', '4-283','4-284','4-285','4-286', '4-287', '4-288', '4-306', '4-307', '4-312')
ogc_fids_rm<-wha_cond[wha_cond$TAG %in% wha_tags, c("ogc_fid", "TAG")]
# maintain 50% basal area; used height >5m as proxy for basal area
zone_wha <- rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, 
                                       type = 'ge', variable = 'height', 
                                       threshold = 5, 
                                       reference_zone = 'rast.zone_wha', 
...
### 4-303 - 4-305 - Lewis woodpecker
https://www.env.gov.bc.ca/wld/documents/wha/LEWO_4-303_to_4-305.pdf
Do not construct roads. Do not conduct timber harvesting, except to maintain an open forest condition of 
35% canopy closure or less. When harvesting to maintain the open forest condition of 35% canopy closure or 
less, ensure that trees greater than 30cm in diameter at breast height are not removed within 
400 m of known nest trees; and 
harvesting activities do not occur between May 1 and August 31 (breeding period). 
```{r, w4303 to w4305}
# set as no harvest
wha_tags<-c('4-303','4-304','4-305')
ogc_fids_rm<-wha_cond[wha_cond$TAG %in% wha_tags, "ogc_fid"]
wha.ras[wha.ras[] %in% ogc_fids_rm]<-0 #reassign in the raster
wha_cond<-wha_cond[!(wha_cond$TAG %in% wha_tags), ] #remove from table
```

### 4-313, 4-314, 4-320 - Wolverine
https://www.env.gov.bc.ca/wld/documents/wha/GUGU-4-313_4-314_4-320_ord.pdf
No harvest in the core area. For the management zone: Primary forest activities must not occur from January 15 through May 31.This tag is for the management zone so remove

```{r, w4-313 4-314 4-320}
#get the ogc_fids that belong to these.
wha_tags<-c('4-313', '4-314', '4-320')
ogc_fids_rm<-wha_cond[wha_cond$TAG %in% wha_tags, "ogc_fid"]
wha.ras[wha.ras[] %in% ogc_fids_rm]<-NA
wha_cond<-wha_cond[!(wha_cond$TAG %in% wha_tags), ] #remove from table
```

### 4-319 - Great Blue Herron (https://www.env.gov.bc.ca/wld/documents/wha/GBHE_4-319_ord.pdf)
Within the management zone, primary forest activities must not occur from February 15 
through August 31. This tag is for the management zone so remove.
...
ogc_fids_rm<-wha_cond[wha_cond$TAG %in% wha_tags, c("ogc_fid","TAG")]
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 80, reference_zone='rast.zone_wha', percentage =45, ndt =0, label = ogc_fids_rm$TAG,
                                       multi_condition = '', denom = ''))
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 100, reference_zone='rast.zone_wha', percentage =37, ndt =0, label = ogc_fids_rm$TAG,
                                       multi_condition = '', denom = ''))
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 120, reference_zone='rast.zone_wha', percentage =30, ndt =0, label = ogc_fids_rm$TAG,
                                       multi_condition = '', denom = ''))
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 140, reference_zone='rast.zone_wha', percentage =25, ndt =0, label = ogc_fids_rm$TAG,
                                       multi_condition = '', denom = ''))
wha_cond<-wha_cond[!(wha_cond$TAG %in% wha_tags), ] #remove from table

wha_tags<-c('5-086') #40% > 70 and 20% > 80
ogc_fids_rm<-wha_cond[wha_cond$TAG %in% wha_tags, c("ogc_fid","TAG")]
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 70, reference_zone='rast.zone_wha', percentage =40, ndt =0, label = ogc_fids_rm$TAG,
                                       multi_condition = '', denom = ''))
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 80, reference_zone='rast.zone_wha', percentage =20, ndt =0, label = ogc_fids_rm$TAG,
                                       multi_condition = '', denom = ''))
wha_cond<-wha_cond[!(wha_cond$TAG %in% wha_tags), ] #remove from table
...
ogc_fids_rm<-wha_cond[wha_cond$TAG %in% wha_tags,c("ogc_fid", "TAG") ]
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 80, reference_zone='rast.zone_wha', percentage =66, ndt =0, label = ogc_fids_rm$TAG,
                                       multi_condition = '', denom = ''))
wha_cond<-wha_cond[!(wha_cond$TAG %in% wha_tags), ] #remove from table
```

### 5-118 Mountain Caribou 
No-harvest zone; parameterizing it here with a unique ID so I can modify it (make it harvestable) for a caribou scenario.
```{r, w5188}
#For the wha's that have modfied harvesting --consider 66% of the area > 80 years
wha_tags<-c('5-118')
ogc_fids_rm<-wha_cond[wha_cond$TAG %in% wha_tags,c("ogc_fid", "TAG") ]
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'nh', variable = '', threshold = 0, reference_zone='rast.zone_wha', percentage =0, ndt =0, label = ogc_fids_rm$TAG,
                                       multi_condition = '', denom = ''))
wha_cond<-wha_cond[!(wha_cond$TAG %in% wha_tags), ] #remove from table
```
...
ogc_fids_rm<-wha_cond[wha_cond$TAG %in% wha_tags,c("ogc_fid", "TAG") ]
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 100, reference_zone='rast.zone_wha', percentage =25, ndt =0, label = ogc_fids_rm$TAG,
                                       multi_condition = '', denom = ''))
wha_cond<-wha_cond[!(wha_cond$TAG %in% wha_tags), ] #remove from table
```
...
### 6-291 - Caribou
https://www.env.gov.bc.ca/wld/documents/wha/RATA_6-291_ord.pdf
No primary forest activities are permitted between April 15 and July 15, excluding 
reforestation and road maintenance activities on existing roads required to prevent 
damage to streams and wetlands, as defined in FPPR. 
No timber harvest of lichen-bearing sites measuring 1 ha or more is permitted. 

Southside Management zone (ogc_fid == 6900)
Harvest 
Timber harvest will result in or provide for:  
a) At least 70% of the forested area being older than 100 years of age. 
b) A maximum of 10% of the forested area being younger than 40 years of 
age

Calving Management zone and High Elevation Management Zone
Timber harvest will result in no more than 17% of the forested area being younger 
than 20 years of age at any time. 

Migration Corridor Management zones:
At least 30% of the forested area within the migration corridor zone in 
the north management area being older than 80 years of age.
At least 40% of the forested area within the migration corridor zone in 
the south management area being older than 80 years of age.

Movement zone
At least 70% of the forested area being older than 80 years of age. 

South Management zone - south
Timber harvest will result in or provide for:  
a) At least 70% of the forested area being older than 100 years of age. 
b) A maximum of 10% of the forested area being younger than 40 years of 
age.
...
```{r, 6-291}
#Look at the FEATURE_NOTEs to see what the FEATURE_CODE means
wha_poly %>% filter(TAG == '6-291') %>% select(HABITAT_AREA_ID, FEATURE_NOTES, FEATURE_CODE, TIMBER_HARVEST_CODE, ogc_fid)

# Core zone is a no harvst zone so its already removed.


wha_tags<-c('6-291') 
# Southside Management zone

ogc_fids_rm<-wha_cond %>% filter(TAG == '6-291' & HABITAT_AREA_ID == 1103916)
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 40, reference_zone='rast.zone_wha', percentage = 90, ndt =0, label = ogc_fids_rm$TAG, multi_condition = '', denom = ''))

zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 100, reference_zone='rast.zone_wha', percentage = 70, ndt =0, label = ogc_fids_rm$TAG, multi_condition = '', denom = ''))

#Calving Management zone and High Elevation Management Zone
ogc_fids_rm<-wha_cond %>% filter(TAG == '6-291' & HABITAT_AREA_ID %in% c(1103917, 1103920,1103921))
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 20, reference_zone='rast.zone_wha', percentage = 83, ndt =0, label = ogc_fids_rm$TAG, multi_condition = '', denom = ''))

# Migration Corridor - north
ogc_fids_rm<-wha_cond %>% filter(TAG == '6-291' & HABITAT_AREA_ID %in% c(1103919))
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 80, reference_zone='rast.zone_wha', percentage = 30, ndt =0, label = ogc_fids_rm$TAG, multi_condition = '', denom = ''))

# Migration Corridor - south
ogc_fids_rm<-wha_cond %>% filter(TAG == '6-291' & HABITAT_AREA_ID %in% c(1103918))
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 80, reference_zone='rast.zone_wha', percentage = 40, ndt =0, label = ogc_fids_rm$TAG, multi_condition = '', denom = ''))

# Movement zone
ogc_fids_rm<-wha_cond %>% filter(TAG == '6-291' & HABITAT_AREA_ID %in% c(1103924))
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 80, reference_zone='rast.zone_wha', percentage = 70, ndt =0, label = ogc_fids_rm$TAG, multi_condition = '', denom = ''))

# General Managemetn zone - 
ogc_fids_rm<-wha_cond %>% filter(TAG == '6-291' & HABITAT_AREA_ID %in% c(,1103922,1103923))
wha.ras[wha.ras[] %in% ogc_fids_rm]<-NA


wha_cond<-wha_cond[!(wha_cond$TAG %in% wha_tags), ]
...
ogc_fids_rm<-wha_cond[wha_cond$TAG %in% wha_tags, c("ogc_fid", "TAG")]
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'crownclosure', threshold = 20, reference_zone='rast.zone_wha', percentage = 40, ndt =0, label = ogc_fids_rm$TAG, multi_condition = '', denom = ''))
wha_cond<-wha_cond[!(wha_cond$TAG %in% wha_tags), ] #remove from table
```
...
wha_tags<-c('8-232')
ogc_fids_rm<-wha_cond[wha_cond$TAG %in% wha_tags,c("ogc_fid", "TAG") ]
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'height', threshold = 19.5, reference_zone='rast.zone_wha', percentage =10, ndt =0, label = ogc_fids_rm$TAG,
                                       multi_condition = '', denom = ''))
wha_cond<-wha_cond[!(wha_cond$TAG %in% wha_tags), ] #remove from table
```

### 8-233 - Caribou 
GWM 1: Forest practices will result in a minimum of 30% of the timber exhibiting heights of at least 14 m, or have undergone self-pruning of the lower branches to allow for suitable security and movement between winter ranges. see http://www.env.gov.bc.ca/wld/documents/wha/RATA_8-233_ord.pdf
```{r, w8233}
# > 30 % having height > 14 m
wha_tags<-c('8-233')
ogc_fids_rm<-wha_cond[wha_cond$TAG %in% wha_tags,c("ogc_fid", "TAG") ]
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'height', threshold = 14, reference_zone='rast.zone_wha', percentage =30, ndt =0, label = ogc_fids_rm$TAG,
                                       multi_condition = '', denom = ''))
wha_cond<-wha_cond[!(wha_cond$TAG %in% wha_tags), ] #remove from table
```
...
ogc_fids_rm<-wha_cond[wha_cond$TAG %in% wha_tags,c("ogc_fid", "TAG") ]
zone_wha<-rbind(zone_wha, data.table(zoneid = ogc_fids_rm$ogc_fid, type = 'ge', variable = 'age', threshold = 40, reference_zone='rast.zone_wha', percentage =50, ndt =0, label = ogc_fids_rm$TAG,
                                       multi_condition = '', denom = ''))
wha_cond<-wha_cond[!(wha_cond$TAG %in% wha_tags), ] #remove from table
```
...
system("cmd.exe", input = paste0('raster2pgsql -s 3005 -d -I -C -M -N 2147483648  ', here(), '/wha.tif -t 100x100 rast.zone_wha |  psql postgres://', keyring::key_get('dbuser', keyring = 'postgreSQL'), ':', keyring::key_get('dbpass', keyring = 'postgreSQL'), '@', keyring::key_get('dbhost', keyring = 'postgreSQL'), ':5432/clus'), show.output.on.console = FALSE, invisible = TRUE)
...
DBI::dbWriteTable(conn, c("zone", "wha"), value= df, row.names = FALSE, overwrite = TRUE) 
# dbExecute (conn, "DELETE FROM zone.constraints WHERE reference_zone = 'zone.wha_2021';")
dbExecute(conn, "ALTER TABLE zone.wha ADD column start INT")
dbExecute(conn, "ALTER TABLE zone.wha ADD column stop INT")
dbExecute(conn, "ALTER TABLE zone.wha ALTER COLUMN denom TYPE VARCHAR;")
dbExecute(conn, "UPDATE zone.wha set denom = null where denom is not null;")
dbExecute(conn, "UPDATE zone.wha set multi_condition = null where multi_condition is not null;")
dbExecute(conn, "UPDATE zone.wha SET start = 0 WHERE start IS not null;")
dbExecute(conn, "UPDATE zone.wha SET stop = 250 WHERE stop IS not null;")

dbExecute (conn, paste0("ALTER TABLE zone.wha INHERIT zone.constraints"))
