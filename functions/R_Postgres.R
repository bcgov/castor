...
#   if (is.null(conn)){
#     conn<-dbConnect(RPostgres::Postgres(), host=keyring::key_get('dbhost', keyring = 'postgreSQL'), dbname = keyring::key_get('dbname', keyring = 'postgreSQL'), port='5432' ,user=keyring::key_get('dbuser', keyring = 'postgreSQL') ,password= keyring::key_get('dbpass', keyring = 'postgreSQL'))
#   }else{
#     conn<-dbConnect(RPostgres::Postgres(), host=conn["host"], dbname = conn["dbname"], port=conn["port"] ,user=conn["user"],password= conn["pass"])
#   }
