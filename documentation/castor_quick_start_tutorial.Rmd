...
When you click Create Project, RStudio will clone the Castor repository into the selected folder. You’ll see that the directory structure matches the GitHub view. However, the subfolders inside the modules/ directory will initially appear empty. This is expected.

### 2.1 Initializing the Castor Modules (Git Submodules)
...
### 2.2 Alternative: Clone Castor and all modules in one step
...
### 2.3 Advanced: Downloading selected submodules only
...
To download only selected modules, first clone the Castor repository as described above, but do not initialize submodules yet.
...
Once the Castor repository and its modules have been cloned, most users will not need to modify the Castor source code. Instead, analyses should be performed by:
- configuring input data,
- adjusting model parameters, and
- running Castor using the existing scripts and modules.

To keep your local code up to date with the official Castor version, periodically click Pull in the Git tab. This will download any updates made by the Castor development team.
...
Castor uses locally managed PostgreSQL databases rather than a shared network database. To keep database credentials secure, usernames and passwords are not stored directly in the Castor source code. Instead, Castor uses the R package `keyring` to store database credentials securely on your local machine (e.g. Windows Credential Manager or macOS Keychain). This allows Castor scripts and modules to retrieve credentials at runtime without exposing them in plain text or committing them to Git.

Using `keyring` has several advantages:
- Credentials are stored locally and encrypted
- Sensitive information is never written into scripts or configuration files
- Scripted workflows can authenticate to PostgreSQL securely and automatically

Many functions within the Castor model assume that database credentials are available via `keyring`. For example, several spatial and database utility functions retrieve connection details using calls such as `keyring::key_get()` when establishing PostgreSQL connections. As a result, setting up the keyring is the recommended and simplest way to enable database access when using Castor. Before running Castor workflows for the first time, users must store their PostgreSQL connection details in the keyring.

### 4.1 How to set up your keyring

An R Markdown helper document is provided in the Castor repository:
functions/keyring_init.Rmd
<https://github.com/bcgov/castor/tree/main/functions/keyring_init.Rmd>
...
Note: These credentials are only for your PostgreSQL database.
They are not the same as your GitHub credentials—GitHub and PostgreSQL are completely independent systems.

## 5. How to create a Forest Harvest Scenario

Now that you are familiar with the software, some of the model code and some of the Castor data, we want to introduce you to the process of developing a model to address a hypothetical forest management scenario. As an example we will develop the layers necessary to run a scenario for the Boundary TSA. However, this same approach can be used for any area of interest. 

### 5.1 Creating a shapefile for your area of interest

An R Markdown helper document is provided in the Castor repository:
`functions/study_area_boundaries.Rmd`  
https://github.com/bcgov/castor/tree/main/functions/study_area_boundaries.Rmd
...
The simulator modules within the Castor model do not run directly off of the PostgreSQL database. Instead Castor has a module called *dataCastor* that consolidates data from the PostgreSQL database into a portable SQLite database. The SQLite database contains all the data needed to run the simulator modules for a defined area of interest. This approach provides a lot of flexibility, as it creates a self-contained, relatively low memory database that can be shared among users. One way to think about the Castor data structure, is that the PostgreSQL database serves as the central repository for stable, provincial-scale datasets, such as for example, the forest inventory. The *dataCastor* compiles a specified subset of this data for an area of interest and groups the information into an SQLite database.  

### 5.1 Getting Aquainted with a Castor SQLite database

In this section you will use the *dataCastor* module to create a simple SQLite database. The data used in this example are synthetic and provided for instructional purposes only. The goal of this exercise is to become familiar with how the Castor SQLite database is structured and how to connect to it in order to run queries and retrieve information.
...
At the top of this code chunk you will find the required package imports (`library()` calls), sourced functions (`source()` calls), environment configuration (e.g. `Sys.setenv()`), and directory paths (`setPaths()`) for module inputs and outputs.
...
```
* NOTE:
- The zoneConstraints can begin and stop according to the user defined 'start' and 'stop' columns
