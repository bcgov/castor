...
This section describes the standard workflow for preparing inputs to run a Castor simulation.  
All steps are script‑based to ensure transparency, reproducibility, and flexibility across study areas and planning objectives.

Castor inputs fall into four broad classes:

1. **Core spatial inputs**: Define the area of interest and spatial structure
2. **Growth and yield inputs**: Describe forest growth within the area of interest
3. **Spatial constraints and scenarios**: Define management rules and policy objectives
4. **Optional module-specific spatial inputs**: Provide additional spatial information required by selected Castor modules
...
#### 5.1.2 Landscape attributes

In addition to knowing where your study takes place you also need to provide information about the landscape in that area. 
Key attributes include which parts of you study area are forested and eligible for harvest.

The (Castor_input_layers.Rmd)[]provides an example script of how 

# 5.2 Growth and Yield Inputs

Growth and yield inputs describe **how forests within the area of interest grow through time**.  
They define the biophysical behaviour of the forest and are required for all Castor simulations.

Because growth and yield information is specific to the landscape being modelled, it is prepared **after** the area of interest and forest inventory have been defined.

Growth and yield inputs are typically:
- constructed once per project,
- reused across multiple management scenarios,
- updated only when inventory, yield models, or regeneration assumptions change.
...
- **Natural (unharvested) stands**  
  Represented using VDYP yield curves.

- **Managed (harvested and regenerated) stands**  
  Represented using TASS/TIPSY yield curves.

At the start of a simulation, each analysis unit is assigned a **current yield curve**.  
When a harvest occurs, Castor:
- resets stand age to zero, and
- switches the stand to a **future managed yield curve**.

Yield curves therefore encode all growth and regeneration assumptions used by the model.

---

## Growth and yield preparation steps

### Script reference  
**Growth and Yield sections in `Scripts for creating spatial inputs for Castor.Rmd`**

### 1. Ingest growth model outputs

Required inputs include:

- VDYP output tables for natural stands
- TIPSY/TASS output tables for managed stands (current condition)
- TIPSY/TASS output tables representing post‑harvest regeneration assumptions (future condition)

Supporting reference tables are used to identify stand origin, species composition, and other metadata required for yield processing.

---

### 2. Construct current‑condition yield curves

For each feature or analysis unit:

- Managed stands:
  - derive volume, height, basal area, and crown closure from TIPSY/TASS outputs
  - extend curves to the full simulation horizon (e.g., to age 250)
- Natural stands:
  - derive yield attributes from VDYP outputs
  - supply placeholder values where attributes are unavailable

Managed and natural curves are combined into a **single long‑format table**, indexed by feature ID and stand age.

---

### 3. Construct future yield curves

Future yield curves represent forest growth following harvest.

- Derived from post‑harvest TIPSY/TASS outputs
- Separate curves may be created for deciduous/mixedwood and conifer‑only stands
- Natural VDYP curves are retained for stands that remain unmanaged

---

### 4. Link yield curves to Analysis Units

An Analysis Unit (AU) table links spatial units to yield curves using yield curve identifiers (YCIDs).

Each AU is assigned:
- a **current YCID** (used at simulation start), and
- a **future YCID** (used after harvest).

By convention:
- negative YCIDs identify natural yield curves,
- positive YCIDs identify managed yield curves.

---

### 5. Create YCID rasters and yield tables

Two rasters are created:
- a **current YCID raster**, and
- a **future YCID raster**.

Final Castor‑ready yield tables are written in the canonical format:
