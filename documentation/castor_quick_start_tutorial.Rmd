...
The Castor repository is located [here](https://github.com/bcgov/castor.git). You will notice the GitHub webpage contains a 'folder' structure code.  

This can be considered as a 'main' copy of the code for all of the repository users. GitHub allows for multiple people to work with the code simultaneously, with the GitHub version housing a stable and recent version of the code. Cloning the repository enables you to work with a local copy of the code. 
...
This command downloads and initializes all the module repositories referenced by the version of Castor you have cloned.
...
## 4. Set-up a Keyring
Castor now uses locally managed PostgreSQL databases rather than a shared network machine. To keep database credentials secure, usernames and passwords are not stored directly in the Castor code. Instead, Castor uses the R package keyring to store your credentials securely on your computer (e.g., in the Windows Credential Manager or macOS Keychain). This allows R scripts to retrieve credentials at runtime without exposing them in plain text.

We use keyring because credentials remain local, encrypted, and never committed to Git.
Scripted workflows can securely authenticate to your PostgreSQL database.
All castor modules already contain calls to keyring::key_get() for connecting to Postgres so setting up the keyring is the easiest way to access your local PostgreSQL database


How to set up your keyring
An R Markdown helper document is provided in the Castor repository:
functions/keyring_init.Rmd
<https://github.com/bcgov/castor/blob/main/functions/keyring_init.Rmd>
...
paths <- list(
  modulePath = paste0(here::here(),"/SpaDES-modules"),
  outputPath = paste0(here::here(),"/SpaDES-modules/dataCastor")
)
...
```
You should see several tables in this relational database. A description of the relationships between tables can be found [here](https://github.com/bcgov/castor/blob/main/R/SpaDES-modules/dataCastor/README.md). 
