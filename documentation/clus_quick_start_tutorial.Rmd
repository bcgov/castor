...
The CLUS repository is located [here](https://github.com/bcgov/clus.git). You will notice the GitHub webpage contains a 'folder' structure with all the code. This can be considered as a 'master' copy of the code for all of the repository users. GitHub allows for multiple people to work with the code simultaneously, with the GitHub version housing a stable and recent version of the code. Cloning the repository enables you to work with a local copy of the code. 
...
```{r, example connect to .sqlite database, message = F, echo = T, eval = F}
library (DBI)
...
```{r, forestryCLUS module example code, message = F, echo = T, eval = F}
library (SpaDES.core)
library (data.table)
source (paste0(here::here(), "/R/functions/R_Postgres.R"))
...
``` {r, example yaml header from .Rmd file, message = F, echo = T, eval = F}
---
