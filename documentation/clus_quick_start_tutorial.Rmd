...
You will need to download Java software to run the blockingCLUS module. The latest version can be found [here](https://jdk.java.net/). Download the zip file and install the software. Note the file path where the application is saved (e.g., "C:/Program Files/Java/jdk-14.0.1"). You will also need to set-up Java in your Windows Environment Variables. Open the "System Properties" window, click on the "Advanced" tab, click on "Environment Variables..." and create a "New" Systems variable called "JAVA_HOME" with the value as the location of the "bin" folder in your JAVA directory (e.g., C:/Program Files/Java/jdk-14.0.1/bin). Next click on "Path" in the system variables, click "Edit...", click "New" and paste in the same path to the "bin" folder. To check that things were installed properly, open a "Command Prompt" window, enter "java -version". It should return the version of tha JAVA software you just installed. 
...
Within the .Rmd you will notice there is some text describing the module and a brief summary of the Revelstoke TSA. This is followed by a 'chunk' of R code. Within this chunk are all teh paramters needed to run the functions in the dataLoaderCLUS.R file. At the top of the chunk yuo will see a list of 'libraries'. These are the R libraries (packages) needed to run all the fucntions in the code. You will also see a 'source' command, that indicates the location of some custome R_Postgres functions. The source coammne will run all of the fucntions in the code so they can be used in the dataLoaderCLUS code.      
 ...
 features of the SpaDES module structure (e.g., "setPaths", "times", "mySim" functions). 





T:\FOR\VIC\HTS\ANA\PROJECTS\CLUS\Data\sql_tsa_tfl_databases
