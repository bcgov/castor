...
Next you will see a bunch of file paths for indicating where the modules (*moduleDir*), inputs (*inputDir*), outputs (*outputDir*) and cache (*cacheDir*) are located. Again, we use the [here()](https://www.rdocumentation.org/packages/here/versions/0.1) function to set relative paths. Be sure you have the correct directory for the SpaDES modules. You can change the location of the other objects if you like, but we recommend using them as-is. 

You will likely recognize the *times* parameter from *dataLoaderCLUS*. It is a  more important in the *forestryCLUS* module since we are simulating forest harvest over time. Provide the start time as 0 and end time as 40. We will simulate forest harvest at 5 years intervals, so 40 intervals are needed to achieve a 200 year simulation. Feel free to change the number of intervals, but make sure is in consistent with the *periodLength* parameter in *growingStockCLUS* (we will provide some more explanation on that below).

Next, again you will recognize the *parameters* list, that consists of a list of all the modules and parameters within them. The *dataLoaderCLUS* parameters should be consistent what you used when running that module previously. However, the *forestryCLUS* module only uses a handful of these directly. In particular, the path to the sqlite database needs to be set (*useCLUSdb*). In addition, you can modify the *nameZoneRasters* you want to include in a given scenario (which we will do below). Otherwise, the parameters can generally be ignored, as they will not impact the *forestryCLUS* simulation analysis.   

Since we used the *blockingCLUS* module as part of *dataLoaderCLUS* to pre-define the harvest blocks, the most important parameter to set for *forestryCLUS* is to set *blockMethod* to 'pre'. This lets *forestryCLUS* know that the blocks were pre-defined in the database. The other parameters are not important in this case. However, if you did not use the pre-blocking method in *dataLoaderCLUS*, you can use the 'dynamic' method in *forestryCLUS*. This creates harvest blocks 'on-the-fly' by randomly spreading, following the size distribution from natural disturbance types (i.e. the *patchZone* parameter and *patchSizeDist* object). Using the 'dynamic' method requires the other parameters to be set. The *useSpreadProbRas* needs to be set to TRUE, so that the model will restrict the location of cutblocks (i.e., allow them to "spread") to within the THLB.   
The *forestryCLUS* module conisists of 
...
- there is a parameter ('scenario') to name the scenario and provide a brief description of it
  ...
  - keep all scenarios and variations in parameters as you run them (comment them out) so there is a record
