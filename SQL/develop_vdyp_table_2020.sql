...
DELETE FROM vdyp_output_2020 WHERE layer_id = 'D'; /*Remove the dead layers*/
select distinct(layer_id) from vdyp_output_2020;

WITH prim_lookup AS (SELECT feature_id, layer_level_code FROM vdyp_input_layer_2020 
					 WHERE forest_cover_rank_code = 1)
DELETE FROM vdyp_output_2020 B
USING prim_lookup C
...
WITH prim_lookup AS (select feature_id, layer_level_code FROM (
	...
	over (partition by feature_id) as max from vdyp_input_layer_2020  where feature_id in 
	(select feature_id from vdyp_output_2020 group by feature_id having count(feature_id) > 35)) as foo 
...
Update vdyp_output_2020 SET  prj_pcnt_stock  = NULL, prj_site_index = NULL, prj_dom_ht = NULL, prj_lorey_ht = NULL, prj_diameter = NULL,
...
select distinct(for_cover_rank_cd) from veg_comp_lyr_r1_poly2020;

/*a. Create a simpflified VEG_COMP table with the necessay information for the aggregated key*/
CREATE TABLE vdyp_vri2020 AS SELECT feature_id, opening_id, reference_year, bec_zone_code, bclcs_level_3,  stand_percentage_dead,
basal_area, crown_closure, crown_closure_class_cd, proj_age_1, bclcs_level_2 , proj_age_class_cd_1, proj_height_1, proj_height_class_cd_1, 
site_index, line_3_tree_species, species_cd_1, species_pct_1, species_cd_2, species_pct_2, species_cd_3,
species_pct_3, species_cd_4,species_pct_4, polygon_area, shape as geometry  
FROM public.veg_comp_lyr_r1_poly2020
WHERE bclcs_level_2 = 'T' AND species_pct_1 > 0 AND for_cover_rank_cd = '1';

delete from vdyp_vri2020 where UPPER(species_cd_1) IN ('G','GP','GR','J','JD','JH','JR','K','KC','OA','OB','OC','OD','OE','OF','OG','OH','OI','Q','QE','QG','QW','R','RA','T','TW','U','UA','UP',
'V','VB','VP','VS','VV','VW','W','WA','WB','WD','WP','WS','WT','X','XC','XH','YP','Z','ZC','ZH'); /*non commercial species*/ 

CREATE INDEX vdyp_vri2020_feature_id_idx /*Create an index which will be used to link with the vdyp output*/
  ON public.vdyp_vri2020
  USING btree
  (feature_id);
...
Update vdyp_vri2020 set species_cd_1 =
CASE 
...
Update vdyp_vri2020 set species_cd_2 =
CASE 
...
Update vdyp_vri2020 set species_pct_1 = (species_pct_1 + species_pct_2) WHERE species_cd_1 = species_cd_2; 
Update vdyp_vri2020 set species_cd_2 = NULL, species_pct_2 = NULL WHERE species_cd_1 = species_cd_2;

/*Make the aggregated sepcies call*/
Alter TAble vdyp_vri2020 Add Column l3spp text ;
Update vdyp_vri2020 set l3spp = CASE
...
--Site index
Update vdyp_vri2020 set site_index = round(site_index / 2 ) * 2; /*round the site index to nearest meter*/

--Percent Moratility
Alter TAble vdyp_vri2020 Add Column dead_class integer ;
Update vdyp_vri2020 set dead_class =  1 WHERE stand_percentage_dead <= 25;
Update vdyp_vri2020 set dead_class =  2 WHERE stand_percentage_dead > 25 and stand_percentage_dead  <= 50;
Update vdyp_vri2020 set dead_class =  3 WHERE stand_percentage_dead > 50 and stand_percentage_dead <= 75;
Update vdyp_vri2020 set dead_class =  4 WHERE stand_percentage_dead  > 75;

Alter TAble vdyp_vri2020 Add Column yc_grp text ;
Update vdyp_vri2020 set yc_grp = NULL;
Update vdyp_vri2020 set yc_grp = CONCAT(bec_zone_code,'_',l3spp,'_', site_index,'_',crown_closure_class_cd,'_', proj_height_class_cd_1,'_', dead_class) WHERE bec_zone_code IS NOT NULL
...
select distinct(yc_grp)  FROM vdyp_vri2020 ;
--select * from vdyp_vri2020 where yc_grp like 'IDF_SwAt_12_4_3_%';
--select * from vdyp_output_2020 where feature_id in (8716062, 14785444) order by prj_total_age; 
/* Height Classes
1	0 – 10.4m
2	10.5 – 19.4m
3	19.5 – 28.4m
...
create table vdyp_2020 as (SELECT * from (
...
FROM vdyp_output_2020
FULL JOIN (SELECT feature_id, yc_grp, opening_id, polygon_area, bclcs_level_2 FROM vdyp_vri2020) as b
ON vdyp_output_2020.feature_id = b.feature_id) as foo);

select distinct(yc_grp) from vdyp_2020;
/*----------OMISSION-----*/
DELETE from vdyp_2020 where yc_grp Is NULL;
DELETE from vdyp_2020 where feature_id IN (select feature_id from vdyp_2020 where prj_total_age is NULL);

alter table vdyp_2020 add column perdec1 double precision default 0.0; 
update vdyp_2020 set perdec1 = case when species_1_code IN ('AC','ACB','ACT','AT','AX','D','DR','E','EA','EP','ES','EW','M','MB','MV') then prj_sp1_vol_dwb end;

alter table vdyp_2020 add column perdec2 double precision; 
update vdyp_2020 set perdec2 = case when species_2_code IN ('AC','ACB','ACT','AT','AX','D','DR','E','EA','EP','ES','EW','M','MB','MV') then prj_sp2_vol_dwb end;

alter table vdyp_2020 add column perdec3 double precision; 
update vdyp_2020 set perdec3 = case when species_3_code IN ('AC','ACB','ACT','AT','AX','D','DR','E','EA','EP','ES','EW','M','MB','MV') then prj_sp3_vol_dwb end;

alter table vdyp_2020 add column perdec4 double precision; 
update vdyp_2020 set perdec4 = case when species_4_code IN ('AC','ACB','ACT','AT','AX','D','DR','E','EA','EP','ES','EW','M','MB','MV') then prj_sp4_vol_dwb end;

alter table vdyp_2020 add column perdec5 double precision; 
update vdyp_2020 set perdec5 = case when species_5_code IN ('AC','ACB','ACT','AT','AX','D','DR','E','EA','EP','ES','EW','M','MB','MV') then prj_sp5_vol_dwb end;

alter table vdyp_2020 add column perdec6 double precision; 
update vdyp_2020 set perdec6 = case when species_6_code IN ('AC','ACB','ACT','AT','AX','D','DR','E','EA','EP','ES','EW','M','MB','MV') then prj_sp6_vol_dwb end;

alter table vdyp_2020 add column decper double precision default 0.0; 
update vdyp_2020 set decper = ((coalesce(perdec1,0.0) + coalesce(perdec2,0.0) + coalesce(perdec3,0.0)+coalesce(perdec4,0.0) + coalesce(perdec5,0.0) + coalesce(perdec6,0.0)) / prj_vol_dwb)
where prj_vol_dwb > 0.0;
update vdyp_2020 set decper = ROUND(CAST(decper as numeric), 2);



create table vdyp_total_area_2020 as (SELECT * from (select vdyp_2020.yc_grp, polygon_area, total_area, case when total_area > 0 then polygon_area/total_area else 0 end as wt, feature_id, opening_id, 
										bclcs_level_2, prj_total_age, 
prj_site_index, prj_dom_ht, prj_lorey_ht, prj_diameter, prj_tph, prj_ba, prj_vol_ws, prj_vol_cu, 
										 prj_vol_d, prj_vol_dw, prj_vol_dwb, decper
FROM vdyp_2020
FULL JOIN (select yc_grp,  sum(polygon_area) as total_area 
							   from vdyp_2020 where prj_total_age = 10 group by yc_grp) as b
ON vdyp_2020.yc_grp = b.yc_grp) as foo);

--Go to R and create yc_vdyp_vat_2020 in yieldCurveGeneration.rmd
--Check
create Table check2020 as select faa.feature_id, vdyp2020, vri2020 from ((select distinct(vdyp_output_2020.feature_id), (1) as vdyp2020 from vdyp_output_2020) as faa left join
						  (select feature_id , (1) as vri2020 from veg_comp_lyr_r1_poly2020) as foo
						  On foo.feature_id = faa.feature_id);
select * from check2020 where vri2020 is null;

create table yc_vdyp_2020 as select yc_vdyp_vat_2020.yc_grp, yc_vdyp_vat_2020.ycid, age, site_index, vol_ws, tvol, 
vol_cu, qmd, basalarea, tph,height, lorey_ht, dec_pcnt, eca from 
((select prj_total_age as age, yc_grp,
(sum(case when prj_site_index is not null then prj_site_index*wt end) / sum(case when prj_site_index is not null then wt end)) as site_index,
(sum(case when prj_vol_ws is not null then prj_vol_ws*wt end) / sum(case when prj_vol_ws is not null then wt end)) as vol_ws,
(sum(case when prj_vol_dwb is not null then prj_vol_dwb*wt end) / sum(case when prj_vol_dwb is not null then wt end)) as tvol,
(sum(case when prj_vol_cu is not null then prj_vol_cu*wt end) / sum(case when prj_vol_cu is not null then wt end)) as vol_cu,
(sum(case when prj_diameter is not null then prj_diameter*wt end) / sum(case when prj_diameter is not null then wt end)) as qmd,
(sum(case when prj_ba is not null then prj_ba*wt end) / sum(case when prj_ba is not null then wt end)) as basalarea,
(sum(case when prj_tph is not null then prj_tph*wt end) / sum(case when prj_tph is not null then wt end)) as tph,
(sum(case when prj_dom_ht is not null then prj_dom_ht*wt end) / sum(case when prj_dom_ht is not null then wt end)) as height,
(sum(case when prj_lorey_ht is not null then prj_lorey_ht*wt end) / sum(case when prj_lorey_ht is not null then wt end)) as lorey_ht,
(sum(case when decper is not null then decper*wt end) / sum(case when decper is not null then wt end)) as dec_pcnt,
(0) as eca 
  FROM vdyp_total_area_2020 where wt > 0 group by yc_grp, age) as foo
LEFT JOIN yc_vdyp_vat_2020 
on foo.yc_grp =yc_vdyp_vat_2020.yc_grp);


--select * from vdyp_test2 order by yc_grp limit 10000;
select * from yc_vdyp_2020 where yc_grp like 'BWBS_At(Ac)%';
/*--------------------------------------------------------------------------*/
/*Step 6. build the yieldcurve table. */

Update yc_vdyp_2020 set tvol = 0 where tvol is NULL;

--add the lower limit of the yield curve
insert into yc_vdyp_2020 (ycid,  yc_grp, vol_ws, vol_cu, qmd, basalarea, tph, age, tvol, height, lorey_ht, dec_pcnt, eca)
select distinct(ycid),  yc_grp, (0.0) as vol_ws, (0.0) as vol_cu, (0.0) as qmd, (0.0) as basalarea, (0.0) as tph, (0.0) as age, (0.0) as tvol, (0.0) as height, (0.0) as lorey_ht, (0.0) as dec_pcnt, (0) as eca
from yc_vdyp_2020;

/*STEP 7. Formatting */

--select * from yc_vdyp order by yc_grp, age limit 1000;
update yc_vdyp_2020 set dec_pcnt = ROUND(CAST(dec_pcnt as numeric), 2);
update yc_vdyp_2020 set height = ROUND(CAST(height as numeric), 2);
update yc_vdyp_2020 set tvol = ROUND(CAST(tvol as numeric), 2);

ALTER TABLE yc_vdyp_2020
ALTER COLUMN eca TYPE double precision;

ALTER TABLE yc_vdyp_2020
ALTER COLUMN ycid TYPE integer;

update yc_vdyp_2020 set qmd = 0 WHERE qmd is null;
update yc_vdyp_2020 set basalarea = 0 WHERE basalarea is null;
update yc_vdyp_2020 set height = 0 WHERE height is null;

---GO TO R and add in crownclosure via vdyp_crown_closure_imputation
