...
You also need to develop and understand the data structures used to run CLUS. CLUS is programmed to connect to a [PostgreSQL database](https://www.postgresql.org/). We use PostgreSQL because it is a open source relational database with high performance capabilities (e.g., store and query large datasets efficiently and quickly), and it is widely used in the Forest Analysis and Inventory Branch (FAIB). We do not go into detail on how to use PostgreSQL here, but some knowledge of the database will faciliate your abiltiy to use CLUS (although, we note that you could program the interface between R and any preferred database). CLUS connects to a PostgreSQL database using the ["DBI" package](https://cran.r-project.org/web/packages/DBI/index.html). We have it configured to run on a PostgreSQL dfintion 



tjhese configuraation will need to be modified t o alocal machine or permission to use our db
...
Detailed information on the modules are provided in .rmd files within each module folder.  
